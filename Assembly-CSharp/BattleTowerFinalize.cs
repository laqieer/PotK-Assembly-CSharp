...
public class BattleTowerFinalize : BattleMonoBehaviour
{
  protected override IEnumerator Start_Battle()
  {
    ...
    else
    {
      ...
      if (overkillEnemyIDs.Length != 0)
        battleTowerFinalize.env.core.enemyUnits.value.ForEach((System.Action<BL.Unit>) (x =>
        {
  ...
  {
    BattleTowerFinalize battleTowerFinalize = this;
    Future<WebAPI.Response.TowerBattleRetire> ft = WebAPI.TowerBattleRetire(battleTowerFinalize.env.core.phaseState.turnCount, battleTowerFinalize.env.core.enemyUnits.value.Select<BL.Unit, int>((Func<BL.Unit, int>) (x => x.killCount)).ToArray<int>(), (System.Action<WebAPI.Response.UserError>) (e => WebAPI.DefaultUserErrorCallback(e)));
