...
public class BattleAIController : BattleMonoBehaviour
{
  ...
  private bool moveUnit(BL.UnitPosition up, int row, int column)
  {
    ...
    if (moveRoute.Count > 0)
      this.btm.setTargetBeforeMoveUnit(moveRoute.First<BL.Panel>(), (System.Action) (() => this.btm.setScheduleAction((System.Action) (() =>
      {
        up.startMoveRoute(moveRoute, this.battleManager.defaultUnitSpeed, this.env, (System.Action) (() =>
        {
  ...
  private IEnumerator actionAIUnits(float waitForShowAiStopButton, System.Action endAction)
  {
    ...
    while (battleAiController1.env.core.aiActionOrder.value != null && battleAiController1.env.core.aiActionOrder.value.Count > 0)
    {
      ...
      if (aiUnit.actionResults == null & isPvpNextScene)
        battleAiController1.btm.setScheduleAction((System.Action) (() => sm.backScene()));
      while (!battleAiController1.enable)
      {
        waitForShowAiStopButton -= Time.deltaTime;
        yield return (object) null;
      ...
      if (aiUnit.action != null)
        battleAiController1.btm.setScheduleAction((System.Action) (() => aiUnit.action()));
        ...
        foreach (ActionResult actionResult in aiUnit.actionResults)
        {
          ...
          if (duelResult != null)
          {
            if (duelResult.isHeal & isPvpNextScene)
              battleAiController1.btm.setScheduleAction(closure_5 ?? (closure_5 = (System.Action) (() => sm.backScene())));
            BL.Unit attack = duelResult.attack;
            BL.Unit defense = duelResult.defense;
            if (!isPvpNextScene)
              battleAiController1.btm.setScheduleAction((System.Action) (() => closure_4.env.core.lookDirection(attack, defense)), 0.5f);
            do
            {
              waitForShowAiStopButton -= Time.deltaTime;
              yield return (object) null;
          ...
          else
          {
            if (ar.terminate)
            {
              battleAiController1.btm.setScheduleAction(closure_9 ?? (closure_9 = (System.Action) (() => up.completeActionUnit(battleAiController.env.core, true))));
              break;
            }
            do
            {
