...
{
  public static List<SupplyItem> DeckList(this List<SupplyItem> xs) => xs.Where<SupplyItem>((Func<SupplyItem, bool>) (x => x.IsShowDeck)).ToList<SupplyItem>();
  ...
  public static List<SupplyItem> Copy(this List<SupplyItem> xs) => xs.Select<SupplyItem, SupplyItem>((Func<SupplyItem, SupplyItem>) (x => x.Copy())).ToList<SupplyItem>();

  public static SupplyItem FindByEntityId(this List<SupplyItem> xs, int entityId)
  {
    foreach (SupplyItem supplyItem in xs)
    {
      if (supplyItem.Supply.ID == entityId)
        return supplyItem;
    }
  ...
  public static SupplyItem[] ShowSupplyItems(this List<SupplyItem> xs) => xs.SelectMany<SupplyItem, SupplyItem>((Func<SupplyItem, IEnumerable<SupplyItem>>) (x => (IEnumerable<SupplyItem>) x.ShowSupplyItems())).ToArray<SupplyItem>();
}
