...
public class DetailUnitParameter : MonoBehaviour
{
  ...
  public IEnumerator Init(int unitID, int position)
  {
    if (MasterData.UnitUnit.ContainsKey(unitID))
    {
      ...
      this.manageSizeWidget.SetDimensions(this.skillObjects[activeIndex].UiWidgetSize.x, this.skillObjects[activeIndex].UiWidgetSize.y);
      if ((UnityEngine.Object) this.skillObjects[activeIndex].unitExtraSetting.maskTexture != (UnityEngine.Object) null)
      {
        this.unitObjects[index1].charaSprite.maskTexture = this.skillObjects[activeIndex].unitExtraSetting.maskTexture;
        this.unitObjects[index1].charaSprite.transform.localPosition = this.skillObjects[activeIndex].unitExtraSetting.characterPosition;
        this.unitObjects[index1].charaSprite.yOffsetPixel = this.skillObjects[activeIndex].unitExtraSetting.characterOffsetY;
      }
      if ((UnityEngine.Object) this.skillObjects[activeIndex].unitShadowExtraSetting.maskTexture != (UnityEngine.Object) null)
      {
        this.unitObjects[index1].charaShadowSprite.maskTexture = this.skillObjects[activeIndex].unitShadowExtraSetting.maskTexture;
        this.unitObjects[index1].charaShadowSprite.transform.localPosition = this.skillObjects[activeIndex].unitShadowExtraSetting.characterPosition;
    ...
    DetailUnitParameter.UnitObjectData data)
  {
    ...
    Future<UnityEngine.Sprite> f = unit.LoadSpriteLarge();
    IEnumerator e = f.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    ...
    if (skillsCount > 9)
    {
      for (int index = 9; index < data.linkSkillIcons.Length; ++index)
        data.linkSkillIcons[index].gameObject.SetActive(true);
      ...
      data.jobBaseSprite.height += this.addedHeightWhenAddSkills;
    }
    ...
    if (isDualTypeSkill)
    {
      ...
      if ((unitTypeButtons != null ? (__nonvirtual (unitTypeButtons.Count) > 0 ? 1 : 0) : 0) != 0)
      {
        ...
        foreach (DetailUnitParameter.UnitTypeSkillObject unitTypeSkillObject in unitTypeSkillObjects)
          unitTypeSkillObject.skillBGColor.color = (Color) detailUnitParameter1.CharactorSkillBgColors[element];
        foreach (DetailUnitParameter.UnitTypeButton unitTypeButton in skillObject.unitTypeButtons)
        {
          ...
          if ((unitSkillList != null ? (__nonvirtual (unitSkillList.Count) > 0 ? 1 : 0) : 0) == 0)
          {
            unitTypeSkillButton.btnUnitType.enabled = false;
            unitTypeSkillButton.btnUnitType.GetComponent<BoxCollider>().enabled = false;
            ((IEnumerable<UISprite>) unitTypeSkillButton.btnUnitType.GetComponentsInChildren<UISprite>()).ForEach<UISprite>((System.Action<UISprite>) (x => x.color = (Color) new Color32((byte) 70, (byte) 70, (byte) 70, byte.MaxValue)));
          }
          else
          {
            ...
            for (int i = 0; i < ((IEnumerable<DetailUnitParameter.UnitTypeSkillObject>) unitTypeSkillObjects).Count<DetailUnitParameter.UnitTypeSkillObject>(); ++i)
            {
              ...
              while (e.MoveNext())
                yield return e.Current;
              e = (IEnumerator) null;
              unitTypeSkillObjects[i].unitTypeSkillIcons.Find(closure_6 ?? (closure_6 = (Predicate<DetailUnitParameter.UnitTypeSkillIcon>) (x => x.unitType == unitTypeSkillButton.unitType))).icon = ft.Result;
              ft = (Future<UnityEngine.Sprite>) null;
            }
            EventDelegate.Set(unitTypeSkillButton.btnUnitType.onClick, (EventDelegate.Callback) (() =>
            {
              if ((UnityEngine.Object) closure_5.oldUnitTypeButton != (UnityEngine.Object) null)
                closure_5.oldUnitTypeButton.isEnabled = true;
              unitTypeSkillButton.btnUnitType.isEnabled = false;
        ...
        for (int index = 0; index < ((IEnumerable<DetailUnitParameter.UnitTypeSkillObject>) unitTypeSkillObjects).Count<DetailUnitParameter.UnitTypeSkillObject>(); ++index)
        {
          unitTypeSkillObjects[index].skillIcon = detailUnitParameter1.battleSkillIconPrefab.CloneAndGetComponent<BattleSkillIcon>(unitTypeSkillObjects[index].skillIconRoot.gameObject);
      ...
      if ((unitTypeButtons != null ? (__nonvirtual (unitTypeButtons.Count) > 0 ? 1 : 0) : 0) != 0)
      {
        ...
        foreach (DetailUnitParameter.UnitTypeButton unitTypeButton in skillObject.unitTypeButtons)
        {
          ...
          else
          {
            ...
            EventDelegate.Set(unitTypeSkillButton.btnUnitType.onClick, (EventDelegate.Callback) (() =>
            {
              if ((UnityEngine.Object) closure_7.oldUnitTypeButton != (UnityEngine.Object) null)
                closure_7.oldUnitTypeButton.isEnabled = true;
              unitTypeSkillButton.btnUnitType.isEnabled = false;
        ...
        unitTypeSkillObject.skillIcon = detailUnitParameter1.battleSkillIconPrefab.CloneAndGetComponent<BattleSkillIcon>(unitTypeSkillObject.skillIconRoot.gameObject);
  ...
  private void setEventPopupSkillDetail(UIButton btn, BattleskillSkill skill) => EventDelegate.Set(btn.onClick, (EventDelegate.Callback) (() => this.popupSkillDetail(skill)));
  ...
  private class UnitTypeSkillIcon
  {
    ...
    {
      this.unitType = type;
      this.icon = (UnityEngine.Sprite) null;
    }
  }
