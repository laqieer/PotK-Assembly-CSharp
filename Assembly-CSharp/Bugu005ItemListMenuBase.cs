...
public class Bugu005ItemListMenuBase : BackButtonMenuBase
{
  ...
  protected System.Action removeReisouCallback;
  ...
  protected Future<GameObject> GetSortAndFilterPopupGameObject() => Res.Prefabs.popup.popup_Item_Sort__anim_popup01.Load<GameObject>();

  public virtual Persist<Persist.ItemSortAndFilterInfo> GetPersist() => (Persist<Persist.ItemSortAndFilterInfo>) null;

  public virtual Persist<Persist.ReisouSortAndFilterInfo> GetReisouPersist() => (Persist<Persist.ReisouSortAndFilterInfo>) null;
  ...
  protected virtual void UpdateInvetoryItem(InventoryItem invItem, PlayerItem item) => invItem.Item.Set(item, this.enabledExpireDate);

  protected virtual void UpdateInvetoryItem(InventoryItem invItem, PlayerMaterialGear item) => invItem.Item.Set(item);
  ...
  protected virtual IEnumerator InitExtension()
  {
    ...
    this.removeReisouCallback = (System.Action) null;
  ...
  {
    int num = itemList != null ? itemList.Sum<PlayerItem>((Func<PlayerItem, int>) (x => !x.isWeapon() ? x.quantity : 1)) : 0;
    if (materialItemList != null)
      num += materialItemList.Sum<PlayerMaterialGear>((Func<PlayerMaterialGear, int>) (x => x.quantity));
    return num;
  }

  protected int GetItemListFavoriteCnt(List<PlayerItem> itemList) => itemList != null ? itemList.Count<PlayerItem>((Func<PlayerItem, bool>) (x => x.favorite)) : 0;

  protected int GetItemListEquipCount(List<PlayerItem> itemList) => itemList != null ? itemList.Count<PlayerItem>((Func<PlayerItem, bool>) (x => x.ForBattle)) : 0;
  ...
  protected virtual InventoryItem CreateInventoryItem(PlayerItem target) => new InventoryItem(target, this.enabledExpireDate);
  ...
  protected void CreatePlayerItems()
  {
    ...
    {
      ItemIcon component = UnityEngine.Object.Instantiate<GameObject>(this.ItemIconPrefab).GetComponent<ItemIcon>();
  ...
  protected IEnumerator OpenReisouDetailPopupAsync(GameCore.ItemInfo item)
  {
    ...
    else
    {
      ...
      {
        if ((UnityEngine.Object) itemListMenuBase.reisouPopupDualSkillPrefab == (UnityEngine.Object) null)
        {
  ...
  private void ResetItemIcon(int allItemIdx) => this.AllItemIcon[allItemIdx].gameObject.SetActive(false);
