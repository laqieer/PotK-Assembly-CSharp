...
public class Shop00723PopupSkillSkill : MonoBehaviour
{
  ...
  private Shop00723PopupSkillSkill.SkillMode GetSkilllabelMode(
  ...
  private IEnumerator coInitialize(Shop00723Menu menu)
  {
    ...
    if (num != 0)
    {
      ...
      NGUITools.AdjustDepth(component.gameObject, this.base_.depth + 1);
      iconSprite = component.iconSprite;
      iconSprite.SetDimensions(60, 55);
    }
    else
    {
      ...
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      NGUITools.AdjustDepth(scIcon.gameObject, this.base_.depth + 1);
    ...
    {
      iconSprite.color = Shop00723PopupSkillSkill.COLOR_GRAY;
  ...
  private void Awake() => this.GetComponent<BoxCollider>().enabled = false;
