...
public class DLC
{
  ...
  public static string GetPath(string fileName) => System.IO.Path.Combine(System.IO.Path.Combine(DLC.ResourceDirectory, (Convert.ToInt32(fileName[fileName.Length - 10]) % 4).ToString()), fileName);
  ...
  public static HashSet<string> GetEntries()
  {
    ...
    foreach (string entry in FileManager.GetEntries(DLC.ResourceDirectory))
    {
      if (entry.Length == 1)
      {
        string[] entries = FileManager.GetEntries(System.IO.Path.Combine(DLC.ResourceDirectory, entry));
        stringList.AddRange((IEnumerable<string>) ((IEnumerable<string>) entries).ToList<string>());
      }
    }
    return new HashSet<string>((IEnumerable<string>) stringList);
  }
  ...
  public long GetDownloadedSize(bool includesExistFile = false) => (includesExistFile ? this.GetDownloadSize(true) - this.GetDownloadSize() : 0L) + ((IEnumerable<DLC.Consumer>) this.consumers).Sum<DLC.Consumer>((Func<DLC.Consumer, long>) (x => (long) x.Downloaded));

  public bool DownloadRequired => (ulong) this.GetDownloadSize() > 0UL;

  public long GetDownloadSize(bool includesExistFile = false) => this.dataDic.Where<KeyValuePair<string, DLC.Data>>((Func<KeyValuePair<string, DLC.Data>, bool>) (x => includesExistFile || !x.Value.Exists)).Sum<KeyValuePair<string, DLC.Data>>((Func<KeyValuePair<string, DLC.Data>, long>) (x => (long) x.Value.FileSize));

  public long GetStoredSize(bool includesExistFile = false) => (includesExistFile ? this.GetStoreSize(true) - this.GetStoreSize() : 0L) + ((IEnumerable<DLC.Consumer>) this.consumers).Sum<DLC.Consumer>((Func<DLC.Consumer, long>) (x => (long) x.Stored));

  public long GetStoreSize(bool includesExistFile = false) => this.dataDic.Where<KeyValuePair<string, DLC.Data>>((Func<KeyValuePair<string, DLC.Data>, bool>) (x => includesExistFile || !x.Value.Exists)).Sum<KeyValuePair<string, DLC.Data>>((Func<KeyValuePair<string, DLC.Data>, long>) (x => (long) x.Value.FileSize));
  ...
  private class Data
  {
    ...
    public string Url => this.AssetBundle ? DLC.UrlBase + "ab/" + this.Name : DLC.UrlBase + "sa/" + this.Name;
  ...
  private class Consumer : IDisposable
  {
    ...
    private IEnumerator WWWRun_Loop_for_IsMoved(DLC.Data data)
    {
      ...
      ResourceManager.CreateSaveDLCDir(System.IO.Path.GetDirectoryName(writePath));
      DLC.WWWFile www;
      while (true)
      {
    ...
    private void logForbiddenError(string url)
    {
      if (!(this.error.Substring(0, 3) == "403"))
        return;
      List<int> intList = url.AllIndexesOf("/", StringComparison.OrdinalIgnoreCase);
      if (intList.Count >= 2)
      {
