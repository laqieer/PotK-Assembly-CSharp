...
public class Clash : MonoBehaviour
{
  ...
  private void Update()
  {
    switch (this.state)
    {
      case Clash.State.Start:
        ...
        this.posX = this.gameObject.transform.localPosition.x;
        if (this.isSkip)
        {
          TweenPosition.Begin(this.gameObject, 0.02f, new Vector3(0.0f, this.posY, 0.0f));
          TweenScale.Begin(this.gameObject, 0.02f, new Vector3(this.scale, this.scale, 0.0f));
        }
        TweenPosition.Begin(this.gameObject, this.scaleTime, new Vector3(0.0f, this.posY, 0.0f));
        TweenScale.Begin(this.gameObject, this.scaleTime, new Vector3(this.scale, this.scale, 0.0f));
        this.state = Clash.State.ScaleUp;
        break;
      case Clash.State.ScaleUp:
        if ((double) this.gameObject.transform.localScale.x != (double) this.scale)
          break;
      ...
      case Clash.State.ScaleDown:
        if (this.isSkip)
        {
          TweenPosition.Begin(this.gameObject, 0.04f, new Vector3(this.posX, 0.0f, 0.0f));
          TweenScale.Begin(this.gameObject, 0.04f, new Vector3(this.ScaleX, this.ScaleY, 0.0f));
        }
        else
        {
          TweenPosition.Begin(this.gameObject, this.scaleTime * 3f, new Vector3(this.posX, 0.0f, 0.0f));
          TweenScale.Begin(this.gameObject, this.scaleTime * 3f, new Vector3(this.ScaleX, this.ScaleY, 0.0f));
        }
        this.state = Clash.State.End;
        break;
      case Clash.State.End:
        if ((double) this.gameObject.transform.localScale.x != (double) this.ScaleX)
          break;
        this.isClash = false;
        this.isSkip = false;
        if (this.isloop)
  ...
  public void EndClash()
  {
    if (!this.isClash)
      return;
    this.windowObj.transform.localPosition = new Vector3(0.0f, 0.0f, 0.0f);
    TweenPosition.Begin(this.gameObject, 0.0f, new Vector3(this.posX, 0.0f, 0.0f));
    TweenScale.Begin(this.gameObject, 0.0f, new Vector3(this.ScaleX, this.ScaleY, 0.0f));
    this.clash = 0.0f;
    this.waitTime = 0.0f;
    this.isClash = false;
    this.isSkip = false;
