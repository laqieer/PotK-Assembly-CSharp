...
public class GachaPickupSelectMenu : BackButtonMenuBase
{
  ...
  private System.Action onChanged_;
  ...
  public IEnumerator doLoadResources()
  {
    ...
    IEnumerator e;
    if (!(bool) (UnityEngine.Object) this.prefabIcon_)
    {
    ...
    if (!(bool) (UnityEngine.Object) this.prefabDetail_)
    {
    ...
    if (!(bool) (UnityEngine.Object) this.prefabFilter_)
    {
    ...
    if (!(bool) (UnityEngine.Object) this.prefabFilterParts_)
    {
  ...
  public IEnumerator onStartSceneAsync(GachaModuleGacha module, System.Action callbackChanged)
  {
    ...
    bool bError = false;
    IEnumerator e = Singleton<NGGameDataManager>.GetInstance().downloadGachaPickupSelect(pickupSelectMenu.module_.id, new System.Action<WebAPI.Response.GachaG301PickupSelectPlayerPickup>(pickupSelectMenu.createInfos), (System.Action<WebAPI.Response.UserError>) (err =>
    {
    ...
    if (!bError)
    {
      ...
      {
        pickupSelectMenu.btnClear_.gameObject,
        pickupSelectMenu.btnOk_.gameObject
    ...
    foreach (GachaPickupSelectMenu.Info allInfo in this.allInfos_)
    {
      switch (allInfo.data)
      {
        case UnitUnit unitUnit:
          stringList.Add(unitUnit.GetUIResourcePaths(false).First<string>());
          break;
        case GearGear gearGear:
          stringList.Add(gearGear.ResourcePaths().First<string>());
          break;
      }
    }
    IEnumerator e = OnDemandDownload.waitLoadSomethingResource((IEnumerable<string>) stringList, false);
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
  }
  ...
  private void resetIcons(bool bLimit, List<IEnumerator> batchLst)
  {
    ...
    for (int index = 0; index < num; ++index)
    {
      ...
      this.icons_[index].icon.transform.localPosition = this.gridSetting_.calcLocalPosition(index);
    }
  ...
  private void createIcons()
  {
    ...
    for (int index = 0; index < this.icons_.Length; ++index)
    {
      ...
      GameObject gameObject = this.prefabIcon_.Clone(this.scroll_.transform);
  ...
  {
    Tuple<GameObject, GameObject> tuple = this.gridSetting_.setScrollRange(this.scroll_.transform, "top", "bottom", this.displayInfos_.Length);
    ...
    this.scrollBaseY_ = this.scroll_.transform.localPosition.y;
    this.logScrollY_ = new float?();
  }

  private IEnumerator batchCoroutines(List<IEnumerator> lst)
  {
    foreach (IEnumerator enumerator in lst)
    {
  ...
  private void updateScroll()
  {
    if (this.isWaitSort_ || !this.isInitialize_ || this.displayInfos_.Length <= this.gridSetting_.quantityScreen)
      return;
    float y = this.scroll_.transform.localPosition.y;
    ...
    float max = (float) (Mathf.Max(0, this.displayInfos_.Length - this.gridSetting_.quantityScreen - 1) / this.gridSetting_.column * this.gridSetting_.height);
    float num2 = (float) (this.gridSetting_.height * this.gridSetting_.rowInstance);
    float num3 = Mathf.Clamp(y - this.scrollBaseY_, 0.0f, max);
    int num4 = Mathf.Min(this.displayInfos_.Length, this.icons_.Length);
    bool isMaxSelected = this.isMaxSelected;
    bool flag;
    do
    {
      flag = false;
      for (int icon_index = 0; icon_index < num4; ++icon_index)
      {
        PickupSelectIcon icon = this.icons_[icon_index].icon;
        Transform transform = icon.transform;
        GameObject gameObject = icon.gameObject;
        float num5 = transform.localPosition.y + num3;
        if ((double) num5 > (double) num1)
        {
          int num6 = (this.displayInfos_.Length + this.gridSetting_.column - 1) / this.gridSetting_.column * this.gridSetting_.column;
          int num7 = icon.position >= 0 ? icon.position + this.gridSetting_.quantityInstance : num6;
          if (icon.position >= 0 && num7 < num6)
          {
            transform.localPosition = new Vector3(transform.localPosition.x, transform.localPosition.y - num2, 0.0f);
            if (num7 >= this.displayInfos_.Length)
            {
              gameObject.SetActive(false);
              icon.setPosition(num7);
            }
            else
              this.updateIcon(num7, icon_index, isMaxSelected);
            flag = true;
          }
        }
        else if ((double) num5 < -((double) num2 - (double) num1))
        {
  ...
  private void waitStartPopup(IEnumerator e, GameObject go) => Singleton<PopupManager>.GetInstance().monitorCoroutine(this.doWaitStartPopup(e, go));
  ...
  private void updateSelectedInfo()
  {
    ...
    this.btnClear_.isEnabled = count != this.selectedIcons_.Length;
    this.btnOk_.isEnabled = count == 0;
  }
  ...
  private IEnumerator doUpload()
  {
    ...
    if (pickupSelectMenu.module_ != null & flag)
    {
      ...
      if (pickupSelectMenu.module_.is_change_pickup_select)
        PopupCommonNoYes.Show(instance.PICKUPSELECT_POPUP_SELECTED_CONFIRM_TITLE, instance.PICKUPSELECT_POPUP_SELECTED_CONFIRM_RESELECTABLE, (System.Action) (() => nWait = 1), (System.Action) (() => nWait = 2), NGUIText.Alignment.Left);
      else
        PopupCommonNoYes.Show(instance.PICKUPSELECT_POPUP_SELECTED_CONFIRM_TITLE, instance.PICKUPSELECT_POPUP_SELECTED_CONFIRM_LOCK, (System.Action) (() => nWait = 1), (System.Action) (() => nWait = 2), NGUIText.Alignment.Left);
      while (nWait == 0)
  ...
  public void onClickedFilter()
  {
    ...
    this.popupFilter_.Initialize(this.prefabFilterParts_, this.filter_, new System.Action(this.filtered), new System.Action(this.changeFilterSetting));
  }
  ...
  private void changeFilterSetting() => this.popupFilter_.SetUnitNum(this.filterBy().Length, this.allInfos_.Length);

  private GachaPickupSelectMenu.Info[] filterBy()
  {
    Dictionary<UnitGroupHead, List<int>> groupIDs = (bool) (UnityEngine.Object) this.popupFilter_ ? this.popupFilter_.selectedGroupIDs : this.filter_.selectedGroupIDs;
  ...
  private void setLabelFilter()
  {
    ...
    if (sprite != null)
    {
      ...
      this.sprFilter_.width = sprite.width;
      this.sprFilter_.height = sprite.height;
      this.sprFilter_.gameObject.SetActive(true);
    }
    else
      this.sprFilter_.gameObject.SetActive(false);
  }
  ...
  private class Info
  {
    ...
    public Info(int i, GearGear g, bool bGearBody, bool np) => this.init(i, g, bGearBody, new int?(), np);
