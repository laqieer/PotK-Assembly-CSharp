...
public class Shop007231Scene : NGSceneBase
{
  ...
  public static void changeScene(bool isStack = true) => Singleton<NGSceneManager>.GetInstance().changeScene(Shop007231Scene.NAME_SCENE, isStack);
  ...
  public IEnumerator onStartSceneAsync()
  {
    ...
    if (selectTicketSummaryArray != null)
    {
      ...
      IEnumerable<SM.SelectTicket> source = ((IEnumerable<SM.SelectTicket>) selectTicketArray).Where<SM.SelectTicket>((Func<SM.SelectTicket, bool>) (t => ((IEnumerable<int>) ticketIds).Contains<int>(t.id)));
      MasterDataTable.SelectTicket[] ticketsDataList = MasterData.SelectTicketList;
      IOrderedEnumerable<SM.SelectTicket> orderedEnumerable = source.OrderBy<SM.SelectTicket, DateTime>((Func<SM.SelectTicket, DateTime>) (x => !x.end_at.HasValue ? DateTime.MaxValue : x.end_at.Value)).ThenBy<SM.SelectTicket, int>((Func<SM.SelectTicket, int>) (s => this.GetTicketsPriority(ticketsDataList, playerUnitTickets.FirstOrDefault<PlayerSelectTicketSummary>((Func<PlayerSelectTicketSummary, bool>) (x => x.ticket_id == s.id)))));
      e1 = shop007231Scene.mainMenu_.coInitialize((IEnumerable<SM.SelectTicket>) orderedEnumerable, playerUnitTickets);
      while (e1.MoveNext())
        yield return e1.Current;
      e1 = (IEnumerator) null;
