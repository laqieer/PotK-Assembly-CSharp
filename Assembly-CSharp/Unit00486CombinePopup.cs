...
public class Unit00486CombinePopup : BackButtonMenuBase
{
  ...
  public void Init(UnitDetailIcon unitDetailIcon, UnitIconInfo unitIconInfo, Unit00486Menu menu)
  {
    ...
    this.menu = menu;
    if (menu.GetType() == typeof (Unit00486Menu))
      this.mode = Unit00468Scene.Mode.Unit0048;
    else if (menu.GetType() == typeof (Unit00487Menu))
        ...
        foreach (CalcUnitCompose.ComposeType composeType in Enum.GetValues(typeof (CalcUnitCompose.ComposeType)))
        {
          switch (this.mode)
          {
            case Unit00468Scene.Mode.Unit0048:
              dictionary[composeType] = CalcUnitCompose.getComposeValue(this.menu.baseUnit, array, composeType);
              continue;
            case Unit00468Scene.Mode.Unit00420:
              dictionary[composeType] = CalcUnitCompose.getBuildupValue(this.menu.baseUnit, array, composeType);
              continue;
            default:
              continue;
          }
  ...
  private Dictionary<CalcUnitCompose.ComposeType, int> GetOneUpperParamDict()
  {
    ...
    foreach (CalcUnitCompose.ComposeType composeType in Enum.GetValues(typeof (CalcUnitCompose.ComposeType)))
    {
      ...
      {
        case Unit00468Scene.Mode.Unit0048:
          dictionary[composeType] = CalcUnitCompose.getComposeValue(this.menu.baseUnit, material, composeType);
          continue;
        case Unit00468Scene.Mode.Unit00420:
          dictionary[composeType] = CalcUnitCompose.getBuildupValue(this.menu.baseUnit, material, composeType);
          continue;
        default:
          continue;
      }
  ...
  private Dictionary<CalcUnitCompose.ComposeType, int> GetComposeMaxDict()
  {
    ...
    {
      case Unit00468Scene.Mode.Unit0048:
        dictionary.Add(CalcUnitCompose.ComposeType.HP, this.menu.baseUnit.compose_hp_max);
        dictionary.Add(CalcUnitCompose.ComposeType.STRENGTH, this.menu.baseUnit.compose_strength_max);
        dictionary.Add(CalcUnitCompose.ComposeType.INTELLIGENCE, this.menu.baseUnit.compose_intelligence_max);
        dictionary.Add(CalcUnitCompose.ComposeType.VITALITY, this.menu.baseUnit.compose_vitality_max);
        dictionary.Add(CalcUnitCompose.ComposeType.MIND, this.menu.baseUnit.compose_mind_max);
        dictionary.Add(CalcUnitCompose.ComposeType.AGILITY, this.menu.baseUnit.compose_agility_max);
        dictionary.Add(CalcUnitCompose.ComposeType.DEXTERITY, this.menu.baseUnit.compose_dexterity_max);
        dictionary.Add(CalcUnitCompose.ComposeType.LUCKY, this.menu.baseUnit.compose_lucky_max);
        break;
      case Unit00468Scene.Mode.Unit00420:
        dictionary.Add(CalcUnitCompose.ComposeType.HP, this.menu.baseUnit.unit.buildup_limit_release_id.hp_limit_release_cnt);
        dictionary.Add(CalcUnitCompose.ComposeType.STRENGTH, this.menu.baseUnit.unit.buildup_limit_release_id.strength_limit_release_cnt);
        dictionary.Add(CalcUnitCompose.ComposeType.INTELLIGENCE, this.menu.baseUnit.unit.buildup_limit_release_id.intelligence_limit_release_cnt);
        dictionary.Add(CalcUnitCompose.ComposeType.VITALITY, this.menu.baseUnit.unit.buildup_limit_release_id.vitality_limit_release_cnt);
        dictionary.Add(CalcUnitCompose.ComposeType.MIND, this.menu.baseUnit.unit.buildup_limit_release_id.mind_limit_release_cnt);
        dictionary.Add(CalcUnitCompose.ComposeType.AGILITY, this.menu.baseUnit.unit.buildup_limit_release_id.agility_limit_release_cnt);
        dictionary.Add(CalcUnitCompose.ComposeType.DEXTERITY, this.menu.baseUnit.unit.buildup_limit_release_id.dexterity_limit_release_cnt);
        dictionary.Add(CalcUnitCompose.ComposeType.LUCKY, this.menu.baseUnit.unit.buildup_limit_release_id.lucky_limit_release_cnt);
        break;
    }
    return dictionary;
  }
  ...
  private int GetBuildParamUpSelectMax()
  {
    ...
    {
      int num3 = this.menu.baseUnit.strength.level_up_max_status - (this.menu.baseUnit.strength.level + this.menu.baseUnit.strength.buildup + otherUpperParamDict[CalcUnitCompose.ComposeType.STRENGTH]);
      if (num3 < 0)
        num3 = 0;
      num2 = Math.Max((int) Math.Ceiling((double) num3 / (double) oneUpperParamDict[CalcUnitCompose.ComposeType.STRENGTH]), num2);
    }
    if (oneUpperParamDict[CalcUnitCompose.ComposeType.INTELLIGENCE] > 0)
    {
      int num3 = this.menu.baseUnit.intelligence.level_up_max_status - (this.menu.baseUnit.intelligence.level + this.menu.baseUnit.intelligence.buildup + otherUpperParamDict[CalcUnitCompose.ComposeType.INTELLIGENCE]);
      if (num3 < 0)
        num3 = 0;
      num2 = Math.Max((int) Math.Ceiling((double) num3 / (double) oneUpperParamDict[CalcUnitCompose.ComposeType.INTELLIGENCE]), num2);
    }
    if (oneUpperParamDict[CalcUnitCompose.ComposeType.VITALITY] > 0)
    {
      int num3 = this.menu.baseUnit.vitality.level_up_max_status - (this.menu.baseUnit.vitality.level + this.menu.baseUnit.vitality.buildup + otherUpperParamDict[CalcUnitCompose.ComposeType.VITALITY]);
      if (num3 < 0)
        num3 = 0;
      num2 = Math.Max((int) Math.Ceiling((double) num3 / (double) oneUpperParamDict[CalcUnitCompose.ComposeType.VITALITY]), num2);
    }
    if (oneUpperParamDict[CalcUnitCompose.ComposeType.MIND] > 0)
    {
      int num3 = this.menu.baseUnit.mind.level_up_max_status - (this.menu.baseUnit.mind.level + this.menu.baseUnit.mind.buildup + otherUpperParamDict[CalcUnitCompose.ComposeType.MIND]);
      if (num3 < 0)
        num3 = 0;
      num2 = Math.Max((int) Math.Ceiling((double) num3 / (double) oneUpperParamDict[CalcUnitCompose.ComposeType.MIND]), num2);
    }
    if (oneUpperParamDict[CalcUnitCompose.ComposeType.AGILITY] > 0)
    {
      int num3 = this.menu.baseUnit.agility.level_up_max_status - (this.menu.baseUnit.agility.level + this.menu.baseUnit.agility.buildup + otherUpperParamDict[CalcUnitCompose.ComposeType.AGILITY]);
      if (num3 < 0)
        num3 = 0;
      num2 = Math.Max((int) Math.Ceiling((double) num3 / (double) oneUpperParamDict[CalcUnitCompose.ComposeType.AGILITY]), num2);
    }
    if (oneUpperParamDict[CalcUnitCompose.ComposeType.DEXTERITY] > 0)
    {
      int num3 = this.menu.baseUnit.dexterity.level_up_max_status - (this.menu.baseUnit.dexterity.level + this.menu.baseUnit.dexterity.buildup + otherUpperParamDict[CalcUnitCompose.ComposeType.DEXTERITY]);
      if (num3 < 0)
        num3 = 0;
      num2 = Math.Max((int) Math.Ceiling((double) num3 / (double) oneUpperParamDict[CalcUnitCompose.ComposeType.DEXTERITY]), num2);
    }
    if (oneUpperParamDict[CalcUnitCompose.ComposeType.LUCKY] > 0)
    {
      int num3 = this.menu.baseUnit.lucky.level_up_max_status - (this.menu.baseUnit.lucky.level + this.menu.baseUnit.lucky.buildup + otherUpperParamDict[CalcUnitCompose.ComposeType.LUCKY]);
      if (num3 < 0)
        num3 = 0;
      num2 = Math.Max((int) Math.Ceiling((double) num3 / (double) oneUpperParamDict[CalcUnitCompose.ComposeType.LUCKY]), num2);
    }
  ...
  private int GetBreakThroughSelectMax()
  {
    ...
    int num3 = unit1.breakthrough_limit - num2;
    if (num3 < 0)
      num3 = 0;
    return num3;
  }
  ...
  private int GetTrustSelectMax()
  {
    ...
    int num3 = 0;
    if ((double) num2 > 0.0)
      num3 = Mathf.CeilToInt(num2 / this.unitIconInfo.unit.TrustMaterialUnit(this.menu.baseUnit).increase_value);
    return num3;
  }
    ...
    foreach (UnitIconInfo selectedUnitIcon in this.menu.SelectedUnitIcons)
    {
      if (selectedUnitIcon != this.unitIconInfo && selectedUnitIcon.playerUnit.unit.IsMaterialUnitSkillUp)
      {
        if (selectedUnitIcon.playerUnit.unit.skillup_type == 99)
        {
          ...
          {
            int num = (int) battleskillSkillType;
            dictionary1[(BattleskillSkillType) num] += selectedUnitIcon.tempSelectedCount;
          }
        }
        else
        {
  ...
  private int GetBuildupUnitySelectMax()
  {
    ...
    int b = PlayerUnit.GetUnityValueMax() * 100;
    int a = 0;
    if (valueUpPattern != null)
      a = Mathf.CeilToInt(this.menu.selectedUnityValue * 100f) + Mathf.CeilToInt(this.menu.selectedBuildupUnityValue * 100f) + Mathf.CeilToInt(this.menu.baseUnit.unityTotal * 100f);
    else if (pureValueUpPattern != null)
      a = Mathf.CeilToInt(this.menu.selectedUnityValue * 100f) + Mathf.CeilToInt((float) (this.menu.baseUnit.unity_value * 100));
    int num1 = Mathf.Min(a, b);
    int num2 = b - num1;
    int num3 = 0;
    if (valueUpPattern != null)
      num3 = Mathf.CeilToInt(valueUpPattern.up_value * 100f);
    else if (pureValueUpPattern != null)
      num3 = Mathf.CeilToInt(100f);
    int num4 = num3;
    return (num2 + num4 - 1) / num3;
  }
  ...
  {
    UnitDetailIcon component1 = script.gameObject.Clone(this.dyn_thum).GetComponent<UnitDetailIcon>();
    component1.Button.enabled = false;
    ...
    foreach (Component component2 in component1.unitIconParent.transform)
      component2.GetComponent<UnitIcon>().Button.enabled = false;
    component1.Dir_select_check.SetActive(false);
    NGTween.playTweens(component1.GetComponentsInChildren<UITweener>(true), NGTween.Kind.GRAYOUT, true);
  }

  private void SetUnitUpParamter(
    PlayerUnit baseUnit,
    PlayerUnit[] materialUnits,
    System.Action<UILabel, int, int, int> SetUpperParamterLabel)
  {
    int num1 = 0;
    int num2 = 0;
    int num3 = 0;
  ...
  private void ChangeCurrentValue(bool isSliderValue = true)
  {
    ...
    this.SetUnitUpParamter(this.menu.baseUnit, playerUnitList.ToArray(), new System.Action<UILabel, int, int, int>(this.menu.SetUpperParamterLabel));
    long num = 0;
    switch (this.mode)
    {
      case Unit00468Scene.Mode.Unit0048:
    ...
    if (num > this.menu.Player.money)
      this.zenyCostValue.color = Color.red;
    else
      this.zenyCostValue.color = Color.white;
