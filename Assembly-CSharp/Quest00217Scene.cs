...
public class Quest00217Scene : NGSceneBase
{
  ...
  public static void ChangeScene(bool stack) => Singleton<NGSceneManager>.GetInstance().changeScene(Quest00217Scene.DefaultSceneName, stack);

  public static void ChangeScene(bool stack, int activeTabIndex) => Singleton<NGSceneManager>.GetInstance().changeScene(Quest00217Scene.DefaultSceneName, (stack ? 1 : 0) != 0, (object) activeTabIndex);
  ...
  public IEnumerator onStartSceneAsync(int activeTabIndex)
  {
    ...
    this.menu.GetComponent<UIRect>().alpha = 1f;
    IEnumerator e;
    if (!this.IsInit)
    {
      this.IsInit = true;
  ...
  private void StartTutorialAdvice() => Singleton<TutorialRoot>.GetInstance().ForceShowAdviceInNextButton("2020_quest002_17", new Dictionary<string, Func<Transform, UIButton>>()
  {
    {
      "chapter_EventQuest",
  ...
  private void OnDisable()
  {
    if (this.offAlphaByDisable_)
      this.menu.GetComponent<UIRect>().alpha = 0.0f;
