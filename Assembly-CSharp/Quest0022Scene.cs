...
public class Quest0022Scene : NGSceneBase
{
  ...
  public IEnumerator onStartSceneAsync(int L, int M, int S)
  {
    ...
    bool bgCreate = (UnityEngine.Object) quest0022Scene.bgchange.Current == (UnityEngine.Object) null;
    IEnumerator e;
    if (bgCreate)
    {
      if (hardmode)
    ...
    ((IEnumerable<TweenPosition>) quest0022Scene.GetComponentsInChildren<TweenPosition>()).ForEach<TweenPosition>((System.Action<TweenPosition>) (x => this.menu.startAllpostween.Add(x)));
    ((IEnumerable<TweenAlpha>) quest0022Scene.GetComponentsInChildren<TweenAlpha>()).ForEach<TweenAlpha>((System.Action<TweenAlpha>) (x => this.menu.startAllalphatween.Add(x)));
    if (quest0022Scene.bgchange.NotTween)
    {
      quest0022Scene.menu.startAllpostween.ForEach((System.Action<TweenPosition>) (x =>
      {
        if (x.tweenGroup != -11 && x.tweenGroup != -12)
          return;
        x.gameObject.transform.localPosition = x.to;
      }));
      quest0022Scene.menu.startAllalphatween.ForEach((System.Action<TweenAlpha>) (x =>
      {
        if (x.tweenGroup != -11 && x.tweenGroup != -12)
          return;
  ...
  public IEnumerator onStartSceneAsync_sea(int XL, int L, int M, int S)
  {
    ...
    quest0022Scene.menu.startAllalphatween = new List<TweenAlpha>();
    // ISSUE: reference to a compiler-generated method
    ((IEnumerable<TweenPosition>) quest0022Scene.GetComponentsInChildren<TweenPosition>()).ForEach<TweenPosition>(new System.Action<TweenPosition>(quest0022Scene.\u003ConStartSceneAsync_sea\u003Eb__23_0));
    // ISSUE: reference to a compiler-generated method
    ((IEnumerable<TweenAlpha>) quest0022Scene.GetComponentsInChildren<TweenAlpha>()).ForEach<TweenAlpha>(new System.Action<TweenAlpha>(quest0022Scene.\u003ConStartSceneAsync_sea\u003Eb__23_1));
    if (quest0022Scene.bgchange.NotTween)
    {
      quest0022Scene.menu.startAllpostween.ForEach((System.Action<TweenPosition>) (x =>
      {
