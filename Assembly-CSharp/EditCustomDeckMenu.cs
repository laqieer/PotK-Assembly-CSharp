...
public class EditCustomDeckMenu : BackButtonMenuBase
{
  ...
  private UIScrollView scrollView => !(bool) (UnityEngine.Object) this.scrollView_ ? (this.scrollView_ = this.scroll_.uiScrollView) : this.scrollView_;

  private UIGrid grid => !(bool) (UnityEngine.Object) this.grid_ ? (this.grid_ = this.scrollView.GetComponentInChildren<UIGrid>()) : this.grid_;
  ...
  public IEnumerator onStartSceneAsync(bool bBackScene, int deckNumber)
  {
    ...
    if (!bBackScene)
    {
      ...
      if (m.dicPage_ != null)
      {
        foreach (KeyValuePair<GameObject, EditCustomDeckMenu.UnitPage> keyValuePair in m.dicPage_)
        {
          if ((UnityEngine.Object) keyValuePair.Key != (UnityEngine.Object) null)
            UnityEngine.Object.Destroy((UnityEngine.Object) keyValuePair.Key);
        }
        m.dicPage_.Clear();
      }
      else
      ...
      switch (m.sortie)
      {
        case EditCustomDeckScene.Sortie.Normal:
          if (deckNumber < 0)
            deckNumber = Persist.deckOrganized.Data.customNumber;
          flag = false;
          break;
        case EditCustomDeckScene.Sortie.Versus:
          if (deckNumber < 0)
            deckNumber = Persist.versusDeckOrganized.Data.customNumber;
          flag = true;
          break;
        case EditCustomDeckScene.Sortie.Colosseum:
          if (deckNumber < 0)
            deckNumber = Persist.colosseumDeckOrganized.Data.customNumber;
          flag = true;
          break;
        case EditCustomDeckScene.Sortie.GuildRaid:
          if (deckNumber < 0)
            deckNumber = Persist.guildRaidLastSortie.Data.customDeckNumber;
          flag = false;
          break;
        default:
          if (deckNumber < 0)
      ...
      if ((UnityEngine.Object) m.objDecision_ != (UnityEngine.Object) null)
      {
      ...
      UnityEngine.Object.Destroy((UnityEngine.Object) node.center);
      m.grid.onReposition = (UIGrid.OnReposition) (() =>
      {
      ...
      m.currentIndex = ((IEnumerable<PlayerCustomDeck>) m.decks_).FirstIndexOrNull<PlayerCustomDeck>((Func<PlayerCustomDeck, bool>) (x => x.deck_number == deckNumber)) ?? 0;
      yield return (object) m.doCreatePage(m.currentIndex);
      m.scrollView.transform.localPosition = new Vector3(-m.grid.cellWidth * (float) m.currentIndex, 0.0f, 0.0f);
    ...
    else if (m.isUpdated_)
    {
      ...
      e = m.dicPage_.Values.FirstOrDefault<EditCustomDeckMenu.UnitPage>((Func<EditCustomDeckMenu.UnitPage, bool>) (x => x.index == this.currentIndex)).panel.doInitialize(m.currentDeck);
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
    }
  ...
  private void setCurrentDeckInfo()
  {
    ...
    int num1 = ((IEnumerable<PlayerUnit>) playerUnits).Sum<PlayerUnit>((Func<PlayerUnit, int>) (x => x.cost));
    int maxCost = Player.Current.max_cost;
    bool flag = num1 <= maxCost;
    if (flag)
      this.txtCost_.SetTextLocalize(string.Format("{0}/{1}", (object) num1, (object) maxCost));
    else
      this.txtCost_.SetTextLocalize(string.Format("[ff0000]{0}[-]/{1}", (object) num1, (object) maxCost));
    PlayerUnit leaderUnit = this.leaderUnit;
    PlayerUnitLeader_skills s = (object) leaderUnit != null ? leaderUnit.leader_skill : (PlayerUnitLeader_skills) null;
    int num2 = s != null ? s.skill_id : 0;
    if (!this.leaderSkillId_.HasValue || this.leaderSkillId_.Value != num2)
    {
      this.leaderSkillId_ = new int?(num2);
      if (s != null)
      {
        this.leaderSkillParam_ = new PopupSkillDetails.Param(s);
        ...
        this.btnLeaderSkillDetail_.isEnabled = true;
      }
      else
      {
        ...
        this.btnLeaderSkillDetail_.isEnabled = false;
      }
    }
    if (!(bool) (UnityEngine.Object) this.btnDecision_)
      return;
    this.btnDecision_.isEnabled = flag && ((IEnumerable<int>) this.currentDeck.player_unit_ids).Any<int>((Func<int, bool>) (i => (uint) i > 0U));
  }
  ...
  protected override void backScene()
  {
    ...
    string fromScene = this.fromScene;
    if (!(fromScene == "colosseum023_4"))
    {
      if (!(fromScene == "versus026_2") && !(fromScene == "versus026_10"))
      {
        if (fromScene == "unit004_6_0822")
        {
          switch (this.sortie)
          {
            case EditCustomDeckScene.Sortie.Versus:
  ...
  private int validLastDeckNumber
  {
    get
    {
      ...
      if (!((IEnumerable<int>) this.currentDeck.player_unit_ids).Any<int>((Func<int, bool>) (i => (uint) i > 0U)))
        deckNumber = this.decks_[((IEnumerable<PlayerCustomDeck>) this.decks_).FirstIndexOrNull<PlayerCustomDeck>((Func<PlayerCustomDeck, bool>) (x => ((IEnumerable<int>) x.player_unit_ids).Any<int>((Func<int, bool>) (i => (uint) i > 0U)))) ?? 0].deck_number;
      return deckNumber;
    }
  }
  ...
  private bool checkEmpty(PlayerCustomDeck target) => !((IEnumerable<int>) target.player_unit_ids).Any<int>((Func<int, bool>) (i => i.IsValid()));
  ...
  public void onClickedEditName()
  {
    if (this.IsPushAndSet())
      return;
    PopupEditDeckName.show(this.prefabEditName_, this.txtName_.text, this.maxLengthName_, new System.Action<string>(this.onSetName), (System.Action) (() => {}));
  }
  ...
  public void onClickedSelectUnits()
  {
    ...
    {
      setUnits = new System.Action<int[]>(this.onSetUnits)
    }));
  }

  private void onSetUnits(int[] unitIds)
  {
    this.isUpdated_ = this.currentDeck.updateUnits(unitIds, this.playerUnits, this.playerGears, this.awakeSkills, new System.Action<int, int>(this.callbackSwapPosition));
    this.editMode_ = EditCustomDeckMenu.EditMode.UnitMulti;
  }

  private void callbackSwapPosition(int aIndex, int bIndex) => this.currentPanel.callbackSwapPosition(aIndex, bIndex);
  ...
  public void onClickedSelectUnit(int index)
  {
    ...
    {
      index = index,
      setUnit = new System.Action<EditUnitParam.Single, int>(this.onSetUnit)
    }));
  }
  ...
  {
    this.isUpdated_ = this.currentDeck.updateUnit(s.index, id, this.playerUnits, this.playerGears, this.awakeSkills, new System.Action<int, int>(this.callbackSwapPosition));
  ...
  public void onClickedGear(PlayerCustomDeckUnit_parameter_list param, int slotNo)
  {
    ...
    })).ToArray<PlayerItem>(), ((IEnumerable<int>) param.player_reisou_ids).ToArray<int>(), new System.Action<int, int, int>(this.onSetGear)));
  }

  private void onSetGear(int unitIndex, int slotNo, int gearId) => this.isUpdated_ |= this.currentDeck.setGear(this.playerGears, unitIndex, slotNo, gearId);
  ...
  public void onClickedReisou(PlayerCustomDeckUnit_parameter_list param, int slotNo)
  {
    ...
    else
    {
      Singleton<PopupManager>.GetInstance().dismiss();
      PlayerItem[] playerGears = this.playerGears;
      Unit0044ReisouScene.changeScene(true, new EditReisouParam(this.currentDeck, param.createGear(playerGears, slotNo, true), this.currentDeck.createReisouReference(this.playerUnits, this.playerGears), param.index, slotNo, playerGears, new System.Action<int, int, int>(this.onSetReisou)));
    }
  }

  private void onSetReisou(int unitIndex, int slotNo, int reisouId) => this.isUpdated_ |= this.currentDeck.setReisou(unitIndex, slotNo, reisouId);
  ...
  private IEnumerator doPopupReisouDetail(PlayerItem reisou, PlayerItem gear)
  {
    ...
    if (reisou.gear.isMythologyReisou())
    {
      ...
      go.SetActive(false);
      e = go.GetComponent<PopupReisouDetailsDualSkill>().Init(new GameCore.ItemInfo(reisou), (PlayerItem) null, true, (System.Action) null, false, false, false, gear);
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      go.SetActive(true);
  ...
  public void onClickedOverkillers(PlayerCustomDeckUnit_parameter_list param, int slotNo)
  {
    ...
    Unit004OverkillersSlotUnitSelectScene.changeScene(new EditOverkillersParam(this.currentDeck, playerUnit, this.currentDeck.createOverkillersReference(playerUnits), param.index, slotNo, playerUnit.getOverkillersUnit(playerUnits, slotNo), playerUnits, new System.Action<int, int, int>(this.onSetOverkillers)));
  }

  private void onSetOverkillers(int unitIndex, int slotNo, int unitId) => this.isUpdated_ |= this.currentDeck.setOverkillers(unitIndex, slotNo, unitId);
  ...
  {
    Singleton<PopupManager>.GetInstance().dismiss();
    Unit004ExtraskillEquipListScene.changeScene(true, new EditAwakeSkillParam(this.currentDeck, param.createPlayerUnit(this.playerUnits, awakeSkills: this.awakeSkills), this.currentDeck.createAwakeSkillReference(this.playerUnits), param.index, this.awakeSkills, new System.Action<int, int>(this.onSetAwakeSkill)));
  }

  private void onSetAwakeSkill(int unitIndex, int skillId) => this.isUpdated_ |= this.currentDeck.setAwakeSkill(unitIndex, skillId);
  ...
  private EditCustomDeckPanel currentPanel => !(bool) (UnityEngine.Object) this.currentPanel_ ? (this.currentPanel_ = this.dicPage_.Values.First<EditCustomDeckMenu.UnitPage>((Func<EditCustomDeckMenu.UnitPage, bool>) (x => x.index == this.currentIndex)).panel) : this.currentPanel_;

  private EditCustomDeckMenu.UnitNode createNode()
  {
    EditCustomDeckMenu.UnitNode unitNode = new EditCustomDeckMenu.UnitNode()
    {
      center = new GameObject("node")
    };
    unitNode.center.transform.localScale = Vector3.one;
    unitNode.center.transform.localPosition = Vector3.zero;
    unitNode.center.transform.localRotation = Quaternion.identity;
    GameObject self = new GameObject("target");
    self.SetParent(unitNode.center);
    unitNode.target = self.transform;
    return unitNode;
  }

  private EditCustomDeckMenu.UnitNode createNode(
    EditCustomDeckMenu.UnitNode original)
  {
    EditCustomDeckMenu.UnitNode unitNode = new EditCustomDeckMenu.UnitNode()
    {
      center = this.scroll_.instantiateParts(original.center, false)
    };
    unitNode.target = unitNode.center.transform.GetChild(0);
    return unitNode;
  }
  ...
  private void updateBlank()
  {
    foreach (EditCustomDeckMenu.UnitPage unitPage in this.dicPage_.Values)
    {
      if (!unitPage.isWaitLoad && Mathf.Abs(unitPage.index - this.currentIndex) > 1 && !this.blankPages_.Contains(unitPage))
      {
        ...
        unitPage.panel.gameObject.SetActive(false);
        unitPage.index = -1;
  ...
  private UIDragScrollView[] dragScrollViews
  {
    get
    {
      ...
      this.dragScrollViews_ = ((IEnumerable<UIDragScrollView>) this.GetComponentsInChildren<UIDragScrollView>()).Where<UIDragScrollView>((Func<UIDragScrollView, bool>) (x => (UnityEngine.Object) x.scrollView == (UnityEngine.Object) this.scrollView)).ToArray<UIDragScrollView>();
      return this.dragScrollViews_;
    }
  }
  ...
  private List<int> createLoadReserves(int center, float dir = 0.0f)
  {
    ...
    {
      int num1 = center + 1;
      if (num1 < this.numDeck_ && intList.Count < capacity)
        intList.Add(num1);
      int num2 = center - 1;
      if (num2 >= 0 && intList.Count < capacity)
        intList.Add(num2);
    }
    else if ((double) dir > 0.0)
    {
      ...
      if (num1 < this.numDeck_ && intList.Count < capacity)
        intList.Add(num1);
      int num2 = num1 + 1;
      if (num2 < this.numDeck_ && intList.Count < capacity)
        intList.Add(num2);
    }
    else
    {
      int num1 = center - 1;
      if (num1 >= 0 && intList.Count < capacity)
        intList.Add(num1);
      int num2 = num1 - 1;
      if (num2 >= 0 && intList.Count < capacity)
        intList.Add(num2);
