...
public class MasterDataReader
{
  ...
  public string ReadStringOrNull(bool intern = false) => !this.ReadBool() ? (string) null : this.ReadString(intern);

  public float ReadFloat()
  {
    float[] numArray = new float[1];
    Buffer.BlockCopy((Array) this.buf, this.n, (Array) numArray, 0, 4);
    this.n += 4;
    return numArray[0];
  }
