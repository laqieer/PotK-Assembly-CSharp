...
public class UnitDetailIcon : UnitIconBase
{
  ...
  private void SetLabel(PlayerUnit playerUnit, PlayerUnit basePlayerUnit, bool isTrustMaterial = false)
  {
    ...
    if (((IEnumerable<bool>) flagArray).Count<bool>((Func<bool, bool>) (b => b)) > 1)
    {
      ...
      this.blink.resetBlinks((IEnumerable<GameObject>) gameObjectList);
  ...
  protected override void showBottomInfoEx(UnitSortAndFilter.SORT_TYPES sort, GameObject target) => this.UnitIcon.ShowBottomInfo(sort);
