...
namespace CustomDeck
{
  public static class Util
  {
    ...
    public static Util.RestoreUnit checkCreateRestoreUnit(PlayerUnit target) => target != (PlayerUnit) null && (((IEnumerable<int?>) target.equip_gear_ids).Any<int?>((Func<int?, bool>) (x => x.HasValue)) || ((IEnumerable<int>) target.over_killers_player_unit_ids).Any<int>((Func<int, bool>) (x => x > 0)) || ((IEnumerable<int?>) target.equip_awake_skill_ids).Any<int?>((Func<int?, bool>) (x => x.HasValue))) ? new Util.RestoreUnit(target) : (Util.RestoreUnit) null;

    public static Util.RestoreGear checkCreateRestoreGear(PlayerItem target) => target != (PlayerItem) null && target.gear != null && target.equipped_reisou_player_gear_id != 0 ? new Util.RestoreGear(target) : (Util.RestoreGear) null;
    ...
    public class RestoreGear
    {
      ...
      public RestoreGear(PlayerItem target) => this.equipped_reisou_player_gear_id = target.equipped_reisou_player_gear_id;
    }
  }
}
