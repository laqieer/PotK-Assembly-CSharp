...
public class Explore033DeckEditMenu : Unit00468Menu
{
  ...
  protected override void updateTxtCostValue(int cost = 0)
  {
    ...
    int num1 = this.ibtnYes.isEnabled ? 1 : 0;
    bool flag = false;
    if (this.mMode == Explore033DeckEditScene.Mode.Challenge && this.selectedUnitIcons.Count < 3)
      flag = true;
    this.ibtnYes.isEnabled = this.totalCost > 0 && !flag;
    int num2 = this.ibtnYes.isEnabled ? 1 : 0;
    if (num1 == num2)
      return;
    this.ibtnYes.transform.GetChild(0).GetComponent<UISprite>().color = this.totalCost > 0 ? Color.white : Color.gray;
  }
  ...
  public IEnumerator InitializeAsync(Explore033DeckEditScene.Mode mode)
  {
    ...
    ((IEnumerable<GameObject>) explore033DeckEditMenu.linkCharacters).ForEach<GameObject>((System.Action<GameObject>) (v => v.transform.Clear()));
    ...
    PlayerUnit[] array = ((IEnumerable<PlayerUnit>) playerUnitArray).Where<PlayerUnit>((Func<PlayerUnit, bool>) (x => x.unit.IsNormalUnit)).ToArray<PlayerUnit>();
    explore033DeckEditMenu.InitializeInfo((IEnumerable<PlayerUnit>) array, (IEnumerable<PlayerMaterialUnit>) null, Persist.unit00468SortAndFilter, false, false, false, true, true, (System.Action) (() => this.InitializeAllUnitInfosExtend(deck)));
  ...
  public IEnumerator onBackSceneAsync(Explore033DeckEditScene.Mode mode)
  {
    Explore033DeckEditMenu explore033DeckEditMenu = this;
    PlayerUnit[] playerUnitArray = SMManager.Get<PlayerUnit[]>();
    yield return (object) explore033DeckEditMenu.UpdateInfoAndScroll(((IEnumerable<PlayerUnit>) playerUnitArray).Where<PlayerUnit>((Func<PlayerUnit, bool>) (x => x.unit.IsNormalUnit)).ToArray<PlayerUnit>());
    explore033DeckEditMenu.UpdateInfomation();
  }

  private IEnumerator loadResources()
  {
    if ((UnityEngine.Object) this.mConfirmPopupPrefab == (UnityEngine.Object) null)
    {
  ...
  protected override void CreateUnitIconAction(int info_index, int unit_index)
  {
    ...
    unitIcon.onLongPress = (System.Action<UnitIconBase>) (x =>
    {
  ...
  protected override IEnumerator DeckEditAsync()
  {
    ...
    yield return (object) dataManager.EditDeck(deckIndex, player_unit_ids, total_combat, (System.Action) (() => saveFailed = true));
    if (saveFailed)
    {
  ...
  public override void IbtnYes()
  {
    ...
    {
      case Explore033DeckEditScene.Mode.Explore:
        Singleton<PopupManager>.GetInstance().open(this.mConfirmPopupPrefab).GetComponent<PopupCommonNoYes>().SetDelegate((System.Action) (() => this.StartCoroutine(this.DeckEditAsync())));
        break;
      case Explore033DeckEditScene.Mode.Challenge:
