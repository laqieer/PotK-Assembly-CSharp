...
public class Quest00217Scroll : BannerBase
{
  ...
  protected DateTime nowLocalTime => this.lastServerTime.AddSeconds((double) Time.time - (double) this.lastLocalTime);
  ...
  {
    if ((UnityEngine.Object) this.ClearedToday == (UnityEngine.Object) null || this.ClearedToday.activeSelf == bActive)
      return;
    this.ClearedToday.SetActive(bActive);
    Quest00217Scroll.ColorSet colorSet = bActive ? this.colorDisabled : this.colorNormal;
    this.Button.gameObject.GetComponent<UI2DSprite>().color = this.Button.defaultColor = colorSet.default_;
    this.Button.hover = colorSet.hover_;
    this.Button.pressed = colorSet.pressed_;
  }

  protected void setQuestLock()
  {
    bool flag = (uint) this.initData.entryConditionID > 0U;
    if ((UnityEngine.Object) this.dirQuestLock == (UnityEngine.Object) null)
      return;
    this.dirQuestLock.SetActive(flag);
    if (flag)
      this.setQuestLockText();
    if ((UnityEngine.Object) this.ClearedToday != (UnityEngine.Object) null && this.ClearedToday.activeSelf)
      return;
  ...
  protected virtual void setQuestLockText() => this.txtLock.SetTextLocalize(MasterData.QuestExtraEntryConditions[this.initData.entryConditionID].text);
