...
namespace LogKit
{
  public class Logger
  {
    ...
    public void Post(string tag, LogLevel logLevel, string message) => this.Post(tag, logLevel, nameof (message), message);
    ...
    public static string GetLogFilePath(string key, Guid logId) => Logger.GetLogFilePath(key, logId.ToString());

    public static string GetLogFilePath(string key, string logId = null) => logId == null ? string.Format("{0}/{1}", (object) Logger.OutDirectory, (object) key) : string.Format("{0}/{1}/{2}", (object) Logger.OutDirectory, (object) key, (object) logId);
  }
}
