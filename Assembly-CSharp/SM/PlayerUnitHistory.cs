...
namespace SM
{
  ...
  public class PlayerUnitHistory : KeyCompare
  {
    ...
    public bool HasEvolutionUnitSkill(UnitUnit unit) => ((IEnumerable<UnitSkillEvolution>) MasterData.UnitSkillEvolutionList).Where<UnitSkillEvolution>((Func<UnitSkillEvolution, bool>) (x => x.unit.ID == unit.ID)).Select<UnitSkillEvolution, int>((Func<UnitSkillEvolution, int>) (x => x.ID)).Any<int>((Func<int, bool>) (x => ((IEnumerable<int?>) this.skill_ids).Contains<int?>(new int?(x))));
    ...
    public bool isUnlockedSEASkill => this.over_killers_release_status >= PlayerUnit.SEASkillUnlockConditions - 1;
