...
namespace SM
{
  ...
  public class PlayerAffiliation : KeyCompare
  {
    ...
    public bool onGvgOperation => this.guild.gvg_status == GvgStatus.matching || this.guild.gvg_status == GvgStatus.preparing || (this.guild.gvg_status == GvgStatus.fighting || this.guild.gvg_status == GvgStatus.aggregating) || this.guild.gvg_status == GvgStatus.finished;

    public bool onGvgTransition => this.guild.gvg_status == GvgStatus.preparing || this.guild.gvg_status == GvgStatus.fighting;
    ...
    public GuildPlayerInfo Player => ((IEnumerable<GuildMembership>) PlayerAffiliation.Current.guild.memberships).Single<GuildMembership>((Func<GuildMembership, bool>) (x => x.player.player_id == SM.Player.Current.id)).player;

    public bool isGuildMember() => this.status != GuildMembershipStatus.not_exist && this.status != GuildMembershipStatus.applicant && this.status != GuildMembershipStatus.withdraw;
    ...
    public int gvg_period_id => !this.guild.active_gvg_period_id.HasValue ? 0 : this.guild.active_gvg_period_id.Value;
