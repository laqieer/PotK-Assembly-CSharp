...
namespace SM
{
  ...
  public class PlayerSeaDeck : KeyCompare
  {
    ...
    public int total_combat => ((IEnumerable<PlayerUnit>) this.player_units).Where<PlayerUnit>((Func<PlayerUnit, bool>) (unit => unit != (PlayerUnit) null)).Sum<PlayerUnit>((Func<PlayerUnit, int>) (unit => Judgement.NonBattleParameter.FromPlayerUnit(unit).Combat));

    public int cost => ((IEnumerable<PlayerUnit>) this.player_units).Where<PlayerUnit>((Func<PlayerUnit, bool>) (unit => unit != (PlayerUnit) null)).Sum<PlayerUnit>((Func<PlayerUnit, int>) (unit => unit.cost));
    ...
    public static DeckInfo createDeckInfo(PlayerSeaDeck deck) => (DeckInfo) new PlayerSeaDeck.ImpPlayerSeaDeck(deck);

    public static DeckInfo createDeckInfo(PlayerDeck deck) => (DeckInfo) new PlayerSeaDeck.ImpPlayerSeaDeck(deck);
    ...
    private class ImpPlayerSeaDeck : DeckInfo
    {
      ...
      private PlayerSeaDeck convSeaDeck(PlayerDeck d) => new PlayerSeaDeck()
      {
...
label_4:
      ...
      public override string name => Consts.Format(Consts.GetInstance().UNIT_0046_MENU, (IDictionary) new Hashtable()
      {
        {
          (object) "num",
      ...
      public override int[] player_unit_ids => this.player_unit_ids_ ?? (this.player_unit_ids_ = ((IEnumerable<int?>) this.deck_.player_unit_ids).Select<int?, int>((Func<int?, int>) (i => !i.HasValue ? 0 : i.Value)).ToArray<int>());
