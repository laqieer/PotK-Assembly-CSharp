...
public class Unit0042Scene : NGSceneBase
{
  ...
  private static bool IsSeaScene() => Singleton<NGGameDataManager>.GetInstance().IsSea || Singleton<NGSceneManager>.GetInstance().IsSeaByChangeScene();
  ...
  private static string getSceneName(bool bStack, bool bUnitTerminal = false)
  {
    ...
    string str = !bUnitTerminal ? "unit004_2" : "unit004_2_terminal";
    if (instance2.IsSea)
      str += "_sea";
    return str;
  }
        ...
        if (param.playerUnit == (PlayerUnit) null || param.controlFlags.IsOn(Control.OverkillersSlot) && param.playerUnit.isAnyOverkillersUnits && param.playerUnit.cache_overkillers_units == null)
        {
          if (!unit0042Scene.init)
          {
            ...
            if (Singleton<NGGameDataManager>.GetInstance().IsSea)
            {
              ...
              Future<WebAPI.Response.SeaFriendStatus> futureF = WebAPI.SeaFriendStatus(player_id, targetPlayerUnitId, new System.Action<WebAPI.Response.UserError>(unit0042Scene.\u003ConStartSceneAsync\u003Eb__33_0));
              e = futureF.Wait();
              while (e.MoveNext())
                yield return e.Current;
              e = (IEnumerator) null;
            ...
            {
              // ISSUE: reference to a compiler-generated method
              Future<WebAPI.Response.FriendStatus> futureF = WebAPI.FriendStatus(player_id, targetPlayerUnitId, new System.Action<WebAPI.Response.UserError>(unit0042Scene.\u003ConStartSceneAsync\u003Eb__33_1));
              e = futureF.Wait();
              while (e.MoveNext())
                yield return e.Current;
              e = (IEnumerator) null;
  ...
  public override IEnumerator onEndSceneAsync()
  {
    ...
    {
      foreach (UITweener tween in unit0042Scene.tweens)
      {
        if ((UnityEngine.Object) tween != (UnityEngine.Object) null)
          tween.enabled = true;
      }
  ...
  public void onStartScene(Unit0042Scene.SavedBootParam param) => this.onStartScene((Unit0042Scene.BootParam) param);
  ...
  private void AutoOpenUnityToutaPopupCoroutine()
  {
    ...
    switch (instance.fromPopup)
    {
      case NGGameDataManager.FromPopup.Unit0042SceneUnity:
        instance.OnceOpenPopup<Future<GameObject>>(this.menu.unityDetailPrefabs, (NGMenuBase) this.menu, new System.Action(this.menu.preOpenUnityPopup));
        break;
      case NGGameDataManager.FromPopup.Unit0042SceneRecommend:
        instance.OnceOpenPopup<GameObject>(this.menu.recommendPrefabs, (NGMenuBase) this.menu, new System.Action(this.menu.preOpenRecomendPopup));
        break;
      ...
      case NGGameDataManager.FromPopup.Unit0042SceneCharacterQuest:
        instance.OnceOpenPopup<GameObject>((GameObject[]) null, (NGMenuBase) this.menu, new System.Action(this.menu.preOpenCharacterQuestPopup));
        break;
      default:
        return;
    }
  ...
  public class BootParam
  {
    ...
    public bool limited
    {
      ...
      set => this.controlFlags = value ? this.controlFlags.Set(Control.Limited) : this.controlFlags.Clear(Control.Limited);
    }

    public bool isGuest
    {
      get => this.controlFlags.IsOn(Control.Guest);
      set => this.controlFlags = value ? this.controlFlags.Set(Control.Guest) : this.controlFlags.Clear(Control.Guest);
    }

    public bool isMemory
    {
      get => this.controlFlags.IsOn(Control.Memory);
      set => this.controlFlags = value ? this.controlFlags.Set(Control.Memory) : this.controlFlags.Clear(Control.Memory);
    }

    public bool isMaterial
    {
      get => this.controlFlags.IsOn(Control.Material);
      set => this.controlFlags = value ? this.controlFlags.Set(Control.Material) : this.controlFlags.Clear(Control.Material);
    }

    public bool isReincarnationType
    {
      get => this.controlFlags.IsOn(Control.ReincarnationType);
      set => this.controlFlags = value ? this.controlFlags.Set(Control.ReincarnationType) : this.controlFlags.Clear(Control.ReincarnationType);
    }

    public bool isToutaPlusNoEnable
    {
      get => this.controlFlags.IsOn(Control.ToutaPlusNoEnable);
      set => this.controlFlags = value ? this.controlFlags.Set(Control.ToutaPlusNoEnable) : this.controlFlags.Clear(Control.ToutaPlusNoEnable);
    }

    public bool isMyGvgAtkDeck
    {
      get => this.controlFlags.IsOn(Control.MyGvgAtkDeck);
      set => this.controlFlags = value ? this.controlFlags.Set(Control.MyGvgAtkDeck) : this.controlFlags.Clear(Control.MyGvgAtkDeck);
    }

    public bool isMyGvgDefDeck
    {
      get => this.controlFlags.IsOn(Control.MyGvgDefDeck);
      set => this.controlFlags = value ? this.controlFlags.Set(Control.MyGvgDefDeck) : this.controlFlags.Clear(Control.MyGvgDefDeck);
    }

    public bool isNotUpdate
    {
      get => this.controlFlags.IsOn(Control.NotUpdate);
      set => this.controlFlags = value ? this.controlFlags.Set(Control.NotUpdate) : this.controlFlags.Clear(Control.NotUpdate);
    }
