...
public class MovePath : MonoBehaviour
{
  ...
  private void Update()
  {
    switch (this.state)
    {
      case MovePath.State.Stop:
        this.transform.localPosition = this.Core.GetPointsLocalPosition(0, this.transform.lossyScale);
        break;
      case MovePath.State.Play:
        this.transform.localPosition = this.Core.UpdateCurve(this.addTime * Time.deltaTime, Vector3.one);
  ...
  public Vector3[] GetPointPositions()
  {
    ...
    for (int index = 0; index < this.points.Count; ++index)
      vector3Array[index] = this.points[index].LocalPosition;
    return vector3Array;
  }
