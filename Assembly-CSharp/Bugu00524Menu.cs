...
public class Bugu00524Menu : Bugu005SelectItemListMenuBase
{
  ...
  public override Persist<Persist.ItemSortAndFilterInfo> GetPersist() => Persist.bugu0052RepairSortAndFilter;

  protected override List<PlayerItem> GetItemList() => ((IEnumerable<PlayerItem>) SMManager.Get<PlayerItem[]>()).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.isWeapon() && x.broken)).ToList<PlayerItem>();

  protected override IEnumerator InitExtension()
  {
    Bugu00524Menu menu = this;
    if ((UnityEngine.Object) menu.popup0052Prefab == (UnityEngine.Object) null)
    {
  ...
  protected override void BottomInfoUpdate() => this.multipleRepairInterface.SetSelectItem(this.SelectItemList);
  ...
  private IEnumerator ShowRepairConfirmPopup(
    ...
    int price,
    int bCnt,
    System.Action<bool, int, int> act)
  {
  ...
  public void ibtnRepairRareMedal(int rareMedalCnt)
  {
    if (this.IsPushAndSet())
      return;
    this.StartCoroutine(this.ShowRepairConfirmPopup(Bugu0052MedalPopup.CurrencyKind.RareMedal, rareMedalCnt, 0, new System.Action<bool, int, int>(this.RepairConfirmPopupYesBtnAction)));
  }
  ...
  public void ibtnRepair(int price, int boostCnt)
  {
    if (this.IsPush)
      return;
    this.StartCoroutine(this.ShowRepairConfirmPopup(Bugu0052MedalPopup.CurrencyKind.Money, price, boostCnt, new System.Action<bool, int, int>(this.RepairConfirmPopupYesBtnAction)));
  }
  ...
  public IEnumerator RepairAPI()
  {
    ...
    if (!Singleton<CommonRoot>.GetInstance().isLoading)
    {
      ...
      Future<WebAPI.Response.ItemGearRepair> future = WebAPI.ItemGearRepair(bugu00524Menu.SelectItemList.ToGearId().ToArray(), (System.Action<WebAPI.Response.UserError>) (e =>
      {
  ...
  private IEnumerator RepairPoweredAPI(bool isReraMedal, int price, int boostCnt)
  {
    ...
    if (!Singleton<CommonRoot>.GetInstance().isLoading)
    {
      ...
      else
      {
        ...
        Future<WebAPI.Response.ItemGearRepairList> future = WebAPI.ItemGearRepairList(bet, medal, array, (System.Action<WebAPI.Response.UserError>) (e =>
        {
          WebAPI.DefaultUserErrorCallback(e);
          MypageScene.ChangeSceneOnError();
        }));
