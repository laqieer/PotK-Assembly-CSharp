...
public class RecipeSortAndFilter : SortAndFilter
{
  ...
  private System.Action<List<bool>> onUpdateFilter;
  private System.Action<Persist.RecipeSortAndFilterInfo> onDicision;
  ...
  public void Initialize(
    ...
    System.Action<List<bool>> onUpdateFilter,
    System.Action<Persist.RecipeSortAndFilterInfo> onDicision)
  {
  ...
  public void SetItemNum(int displaynum, int allnum) => this.targetItemNum.SetText(((Func<int, string>) (num => num <= 0 ? "[FF0000]" : "[FFFE27]"))(displaynum) + (object) displaynum + "[-]/" + (object) allnum);
  ...
  private void SetSortFilterLabelColor()
  {
    ...
    {
      this.sortLabel.color = new Color(1f, 1f, 0.0f);
      this.filterLabel.color = flag ? new Color(0.5f, 0.5f, 0.0f) : new Color(0.5f, 0.5f, 0.5f);
    }
    else
    {
      this.sortLabel.color = new Color(0.5f, 0.5f, 0.0f);
      this.filterLabel.color = flag ? new Color(1f, 1f, 0.0f) : new Color(1f, 1f, 1f);
    }
  }

  private void SetListTypeBtn(RecipeSortAndFilter.MODE_TYPES mode)
  {
    if ((UnityEngine.Object) this.ListBtns[0] != (UnityEngine.Object) null && (UnityEngine.Object) this.ListBtns[1] != (UnityEngine.Object) null)
    {
      this.modeType = mode;
  ...
  private void SetSortTabLabel(RecipeSortAndFilter.SORT_TYPES sortTypes) => this.sortLabel.SetText(this.sortLabelDic[sortTypes]);
  ...
  private void SetFilterTabLabel(List<bool> filter)
  {
    if (filter.Any<bool>((Func<bool, bool>) (v => v)))
    {
      ...
      this.filterLabel.color = new Color(1f, 1f, 0.0f);
    }
    else
    {
      this.filterLabel.SetText(Consts.GetInstance().SORT_POPUP_LABEL_FILTER_OFF);
      this.filterLabel.color = new Color(1f, 1f, 1f);
    }
  }
  ...
  public override void IbtnAllFilterSelect()
  {
    ...
    {
      if (!((UnityEngine.Object) this.FilterBtns[index] == (UnityEngine.Object) null))
  ...
  private void InitColorRecipeExist(bool enabled) => this.isRecipeExistBtn.SetColor(enabled ? Color.white : Color.gray);

  public static UISprite SortSpriteLabel(
    RecipeSortAndFilter.SORT_TYPES type,
    UISprite SortSprite)
  {
    string name = RecipeSortAndFilter.SetSortLabelSpriteName(type);
    if (!string.IsNullOrEmpty(name))
    {
      ...
      SortSprite.GetComponent<UIWidget>().SetDimensions(sprite.width, sprite.height);
    }
    return SortSprite;
  }
