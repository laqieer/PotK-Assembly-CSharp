...
public class Unit00492Menu : UnitMenuBase
{
  ...
  public IEnumerator coInitialize(Unit00492Menu.Param param)
  {
    ...
    unit00492Menu.InitializeInfoEx((IEnumerable<PlayerUnit>) param.units_, (IEnumerable<PlayerMaterialUnit>) null, UnitSortAndFilter.SORT_TYPES.GetOrder, SortAndFilter.SORT_TYPE_ORDER_BUY.ASCENDING, false, false, true, true, false, false, (System.Action) null, unit00492Menu.isBattleFirst, unit00492Menu.isTowerEntry);
  ...
  private void setupIconCustom(UnitIconBase icon)
  {
    ...
    else
    {
      ...
      {
        icon.Gray = false;
        icon.onClick = (System.Action<UnitIconBase>) (ui => this.onSelectUnitIcon(ui));
      }
      ...
      else
      {
        if (this.isSelected(playerUnit))
          ...
          icon.Gray = true;
        icon.onClick = (System.Action<UnitIconBase>) (ui => {});
      }
    }
  }

  private bool isSelected(PlayerUnit unit) => this.param_.baseUnit_ != (PlayerUnit) null && this.param_.baseUnit_.id == unit.id || ((IEnumerable<PlayerUnit>) this.param_.selectedUnits_).Any<PlayerUnit>((Func<PlayerUnit, bool>) (pu => pu.id == unit.id));
