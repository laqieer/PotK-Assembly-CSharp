...
public class Friend0085Menu : FriendBarBase
{
  ...
  public IEnumerator InitFriendScroll()
  {
    ...
    if (friend0085Menu.allFriendInfo.Count > 0)
    {
      ...
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      friend0085Menu.DirBulk.transform.gameObject.SetActive(true);
      friend0085Menu.DirNofriend.SetActive(false);
    }
    else
    {
      friend0085Menu.DirBulk.transform.gameObject.SetActive(false);
  ...
  private void FriendScrollAction(int info_index, int bar_index) => ((FriendScrollParts) this.allFriendBar[bar_index]).Set(this);
}
