...
public class SeaTalkMessageMenu : BackButtonMenuBase
{
  ...
  private void SetReplyButton(bool isOn)
  {
    if (isOn)
    {
      this.replyButton.isEnabled = true;
      this.replyButtonLabelTween.enabled = true;
      this.replyButtonLabelTween.GetComponent<UILabel>().SetTextLocalize("[ffffff]返信する[-]");
      this.replyArrow.SetActive(true);
    }
    else
    {
      this.replyButton.isEnabled = false;
      this.replyButtonLabelTween.enabled = false;
      this.replyButtonLabelTween.GetComponent<UILabel>().SetTextLocalize("[949494]返信する[-]");
  ...
  private void Awake()
  {
    this.t_scrollView = this.scrollView.transform;
    ...
    this.initReplyChoiceViewBaseHeight = this.replyChoiceViewBase.height;
  }
  ...
  public IEnumerator onBackSceneUpdateGift()
  {
    if (this.replyIconParent.childCount > 0)
    {
      ...
      {
        this.ReplyGiftUpdate(this.replyIconParent.GetChild(0).gameObject.GetComponent<ItemIcon>(), info.playerTalkMessage.condition_id.Value);
        yield break;
      }
    }
  }
    ...
    Sea030HomeMenu homeMenu)
  {
    ...
    {
      IEnumerator e;
      if ((UnityEngine.Object) this.seaTalkTutorial == (UnityEngine.Object) null)
      {
      ...
      e = popup.GetComponent<SimpleScrollContentsPopup>().Initialize((System.Action) (() =>
      {
  ...
  private IEnumerator SetPartnerUnit()
  {
    ...
    seaTalkMessageMenu.unitIconPrefab = seaTalkMessageMenu.unitIconPrefab.Clone(seaTalkMessageMenu.transform);
    ...
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    unitIcon.Button.gameObject.SetActive(false);
  ...
  private void SetScrollSettings(bool isUnread)
  {
    ...
    if ((double) num1 >= (double) this.scrollView.panel.height)
    {
      if (this.infos.Count - 1 > this.talkMessageResponse.last_index)
      {
        ...
        if (isUnread)
        {
          ...
          {
            float num2 = Mathf.Abs(seaTalkMessageInfo.PositionY) - seaTalkMessageInfo.UpperHeight / 2f;
            if ((double) Mathf.Abs(this.infos[this.infos.Count - 1].PositionY) + (double) this.infos[this.infos.Count - 1].UnderHeight / 2.0 - (double) num2 >= (double) this.scrollView.panel.height)
              y = Mathf.Abs(seaTalkMessageInfo.PositionY) - seaTalkMessageInfo.UpperHeight / 2f;
            else
              this.talkMessageResponse.last_index = this.infos.Count - 1;
          }
          else
          ...
          y = num1 - this.scrollView.panel.height;
        this.scrollView.MoveRelative(new Vector3(0.0f, y, 0.0f));
      }
      this.dragScrollView.enabled = true;
    }
    if (this.infos.Count - 1 <= this.talkMessageResponse.last_index)
    ...
    else
    {
      ...
      this.dragScrollView.enabled = false;
    }
  }
  ...
  private void InitReplyChoice()
  {
    ...
    {
      talkReply.gameObject.SetActive(false);
      UnityEngine.Object.DestroyImmediate((UnityEngine.Object) talkReply.gameObject);
    }
    this.talkReplys.Clear();
    this.replyIconParent.Clear();
    this.replyChoiceViewBase.height = this.initReplyChoiceViewBaseHeight;
  }
  ...
  private IEnumerator CreateReplyChoice()
  {
    ...
    else
    {
      ...
      else
      {
        ...
        System.Action<CallMessage> onReplyAction = (System.Action<CallMessage>) (m_replyMessage => this.OnTalkReply(m_replyMessage));
        foreach (CallMessage callMessage2 in callMessageList)
        {
          SeaTalkReply component = (callMessage2.message_type_id != 6 ? this.replyChoicePrefab.Clone(this.replyChoiceView.transform) : this.replyGivePrefab.Clone(this.replyChoiceView.transform)).GetComponent<SeaTalkReply>();
          component.Init(callMessage2, onReplyAction, this.talkUnitInfo, lastInfo.playerTalkMessage);
        ...
        {
          this.replyItem.gameObject.SetActive(true);
          ...
          if (lastInfo.playerTalkMessage.condition_id.HasValue)
          {
            ...
            if (!(conditionId.GetValueOrDefault() == num & conditionId.HasValue))
            {
              ...
              if (gear != null)
              {
                ...
                itemIcon.onClick = (System.Action<ItemIcon>) (_ => this.OnGiftListScenePopup(gear));
              }
              else
                this.ReplyGiftUpdate(itemIcon, reward_id);
            }
        ...
        else if (messageTypeId == TalkMessageType.Question)
        {
          if (((IEnumerable<CallMessage>) MasterData.CallMessageList).Where<CallMessage>((Func<CallMessage, bool>) (x => x.message_set_id == lastCallMessage.message_set_id)).Any<CallMessage>((Func<CallMessage, bool>) (x => x.message_type_id == 8)))
          {
            ...
            this.replyItem.gameObject.SetActive(false);
        }
        else
          this.replyItem.gameObject.SetActive(false);
      }
  ...
  private void ReplyGiftUpdate(ItemIcon itemIcon, int gear_id)
  {
    ...
    if (playerMaterialGear == (PlayerMaterialGear) null || playerMaterialGear.quantity <= 0)
    {
      ...
      this.talkReplys.First<SeaTalkReply>((Func<SeaTalkReply, bool>) (x => x.CallMessage.message_type_id == 6)).replyButton.isEnabled = false;
    }
    else
    {
      itemIcon.Gray = false;
      this.replyLabel.text = "贈り物を渡す";
      this.talkReplys.First<SeaTalkReply>((Func<SeaTalkReply, bool>) (x => x.CallMessage.message_type_id == 6)).replyButton.isEnabled = true;
    }
  }
  ...
  private void ReplyProposeItemUpdate(ItemIcon itemIcon)
  {
    ...
    if (playerMaterialGear == (PlayerMaterialGear) null || playerMaterialGear.quantity <= 0)
    {
      ...
      this.talkReplys[0].replyButton.isEnabled = false;
    }
    else
    {
      itemIcon.Gray = false;
      this.replyLabel.text = "誓約締結アイテムを消費する";
      this.talkReplys[0].replyButton.isEnabled = true;
    }
  }

  private void OnTalkReply(CallMessage callMessage) => this.StartCoroutine(this.IOnTalkReply(callMessage));
  ...
  private IEnumerator IOnTalkReply(CallMessage selectCallMessage)
  {
    ...
    {
      Future<WebAPI.Response.SeaTalkGift> api = WebAPI.SeaTalkGift(info.playerTalkMessage.condition_id.Value, this.talkUnitInfo.unit.same_character_id, (System.Action<WebAPI.Response.UserError>) (e =>
      {
    ...
    {
      Future<WebAPI.Response.SeaTalkReply> api = WebAPI.SeaTalkReply(selectCallMessage.ID, this.talkUnitInfo.unit.same_character_id, (System.Action<WebAPI.Response.UserError>) (e =>
      {
        WebAPI.DefaultUserErrorCallback(e);
        MypageScene.ChangeSceneOnError();
        Singleton<CommonRoot>.GetInstance().HideLoadingLayer();
  ...
  private IEnumerator AddInfosAnimation(List<SeaTalkMessageInfo> addInfos)
  {
    ...
    for (int i = 0; i < addInfos.Count; ++i)
    {
      ...
      if (info.MessageViewType == TalkMessageViewType.Date || info.MessageViewType == TalkMessageViewType.Gift)
      {
        ...
        this.OnMove(this.scrollView.GetComponent<UIPanel>());
      }
      else
      {
        if (info.MessageViewType == TalkMessageViewType.Partner)
        {
          ...
          if ((UnityEngine.Object) this.writing != (UnityEngine.Object) null)
            this.writing.gameObject.SetActive(true);
        }
        this.scrollView.ResetPosition();
        this.SetScrollSettings(false);
        this.OnMove(this.scrollView.GetComponent<UIPanel>());
        ...
        {
          this.writing.gameObject.SetActive(false);
  ...
  private void UpdateReplyChoiceView()
  {
    ...
    else
    {
      ...
      float y = transform.localPosition.y - (float) (((double) this.replyMonologue.height + 20.0) / 2.0);
      this.replyMonologue.transform.localPosition = new Vector3(transform.localPosition.x, y, transform.localPosition.z);
      this.replyTweenY += (float) this.replyMonologue.height + 20f;
      float num1 = y - (float) (((double) this.replyMonologue.height + 20.0) / 2.0);
      foreach (SeaTalkReply talkReply in this.talkReplys)
      {
        float height = talkReply.GetHeight();
        talkReply.transform.localPosition = new Vector3(0.0f, num1 - height / 2f, 0.0f);
        float num2 = height + 20f;
        num1 -= num2;
        this.replyTweenY += num2;
      }
      if (this.replyIconParent.childCount > 0)
      {
        this.replyItem.localPosition = new Vector3(0.0f, (float) ((double) num1 - 68.0 - 20.0), 0.0f);
        this.replyTweenY += 156f;
      }
      this.replyTweenY += 23f;
      this.replyChoiceViewBase.height += Mathf.RoundToInt(this.replyTweenY);
    }
  }
  ...
  public IEnumerator Conclusion()
  {
    ...
    if (callMessage != null && callMessage.message_type_id == 8 && !((IEnumerable<PlayerTalkPartner>) seaTalkMessageMenu.talkPartnerResponse.partners).Any<PlayerTalkPartner>((Func<PlayerTalkPartner, bool>) (x => x.letter.call_status == 3)))
    {
      ...
      Singleton<CommonRoot>.GetInstance().ShowLoadingLayer(2);
      Future<WebAPI.Response.SeaCallConclusion> api = WebAPI.SeaCallConclusion(seaTalkMessageMenu.talkUnitInfo.unit.same_character_id, (System.Action<WebAPI.Response.UserError>) (e =>
      {
        WebAPI.DefaultUserErrorCallback(e);
        MypageScene.ChangeSceneOnError();
      }));
  ...
  {
    Vector3[] worldCorners = scrollViewPanel.worldCorners;
    for (int index = 0; index < 4; ++index)
    {
      Vector3 position = worldCorners[index];
      Vector3 vector3 = scrollViewPanel.transform.InverseTransformPoint(position);
    ...
    for (int index = num1 + 1; index < this.partnerMessages.Count; ++index)
      this.partnerMessages[index].gameObject.SetActive(false);
    for (int index = num2 + 1; index < this.playerMessages.Count; ++index)
      this.playerMessages[index].gameObject.SetActive(false);
    for (int index = num3 + 1; index < this.dateMessages.Count; ++index)
      this.dateMessages[index].gameObject.SetActive(false);
    for (int index = num4 + 1; index < this.giftMessages.Count; ++index)
      this.giftMessages[index].gameObject.SetActive(false);
    if (flag)
      return;
    this.unreadLine.gameObject.SetActive(false);
  }
  ...
  private void MessageClone(SeaTalkMessageInfo info)
  {
    ...
    {
      case TalkMessageViewType.Partner:
        original = this.messagePartnerPrefab;
        break;
      case TalkMessageViewType.Player:
        original = this.messagePlayerPrefab;
        break;
      case TalkMessageViewType.Date:
        original = this.messageDatePrefab;
        break;
      case TalkMessageViewType.Gift:
        original = this.messageGiftPrefab;
        break;
      case TalkMessageViewType.Writing:
        original = this.messageWritingPrefab;
        break;
      default:
        Debug.LogError((object) "想定していないTalkMessageViewTypeです");
        break;
    ...
    gameObject.transform.parent = this.t_scrollView;
    gameObject.transform.localScale = Vector3.one;
    gameObject.transform.localPosition = Vector3.zero;
    gameObject.transform.localRotation = Quaternion.identity;
    SeaTalkMessage component = gameObject.GetComponent<SeaTalkMessage>();
    switch (info.MessageViewType)
    {
      case TalkMessageViewType.Partner:
        this.partnerMessages.Add(gameObject.transform);
        this.partnerMessagesComponents.Add(component);
        component.BaseUnitIcon = this.unitIconPrefab;
        break;
      case TalkMessageViewType.Player:
        this.playerMessages.Add(gameObject.transform);
        this.playerMessagesComponents.Add(component);
        break;
      case TalkMessageViewType.Date:
        this.dateMessages.Add(gameObject.transform);
        this.dateMessagesComponents.Add(component);
        break;
      case TalkMessageViewType.Gift:
        this.giftMessages.Add(gameObject.transform);
        this.giftMessagesComponents.Add(component);
        break;
      case TalkMessageViewType.Writing:
        this.writing = gameObject.transform;
  ...
  public void OnReply()
  {
    ...
    this.SetReplyButton(false);
    this.replyCloseScrollView.enabled = true;
    this.dragScrollView.enabled = false;
  }
  ...
  public void OnReplyClose()
  {
    ...
    this.replyCloseButton.SetActive(false);
    this.SetReplyButton(true);
    this.replyCloseScrollView.enabled = false;
    this.dragScrollView.enabled = true;
  }
  ...
  private IEnumerator OpenCallDestructionPopup()
  {
    ...
    popup.SetActive(false);
    yield return (object) popup.GetComponent<SeaTalkDestruction>().Init(seaTalkMessageMenu.playerCallLetter, seaTalkMessageMenu.talkUnitInfo, new System.Action(seaTalkMessageMenu.IbtnBack));
    popup.SetActive(true);
  }
  ...
  private void IbtnBack()
  {
    ...
    if (info.playerTalkMessage.expire_at.HasValue && ServerTime.NowAppTimeAddDelta() > info.playerTalkMessage.expire_at.Value)
      SeaTalkMessageMenu.SeaTalkPartnerRefresh();
    if ((UnityEngine.Object) this.seaHomeMenu != (UnityEngine.Object) null)
    {
