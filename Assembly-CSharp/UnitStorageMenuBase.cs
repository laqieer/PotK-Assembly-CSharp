...
public class UnitStorageMenuBase : UnitSelectMenuBase
{
  ...
  public Persist<Persist.UnitSortAndFilterInfo> StorageMenuSortAndFilterInfo => this.sortAndFilterInfo;
    ...
    bool isBack = false)
  {
    ...
    else
    {
      ...
      unitStorageMenuBase.InitializeInfoEx((IEnumerable<PlayerUnit>) unitStorageMenuBase.displayUnits, (IEnumerable<PlayerMaterialUnit>) null, sortInfo, false, false, true, true, true, false, (System.Action) (() => this.InitializeAllUnitInfosExtend(playerDeck)));
  ...
  {
    this.txtStorageUnitPossession.color = storageCount >= player.max_unit_reserves ? Color.red : Color.white;
    this.txtStorageUnitPossession.SetTextLocalize(string.Format("{0}/{1}", (object) storageCount, (object) player.max_unit_reserves));
    this.TxtNumberpossession.color = this.playerUnits.Length >= player.max_units ? Color.red : Color.white;
  ...
  public override void UpdateInfomation()
  {
    ...
    this.TxtNumberselect.color = count >= this.SelectMax ? Color.red : Color.white;
  ...
  private void SetEnableIbtnClear(int selectedUnitsCount)
  {
    if (selectedUnitsCount > 0)
      this.ibtnClear.isEnabled = true;
    else
      this.ibtnClear.isEnabled = false;
  }
