...
public class Guild028117Popup : BackButtonMenuBase
{
  ...
  public void Initialize(GuildInfoPopup popup)
  {
    ...
    this.dragScrollCollision = this.dragScrollViewWidget.GetComponent<BoxCollider>();
    this.input.gameObject.AddComponent<UIDragScrollView>();
    this.inputFieldCollision = this.input.gameObject.AddComponent<BoxCollider>();
  ...
  private IEnumerator SendGuildStatement()
  {
    ...
    Future<WebAPI.Response.GuildBroadcastMessage> ft = WebAPI.GuildBroadcastMessage(guild028117Popup.input.value.ToConverter(), false, new System.Action<WebAPI.Response.UserError>(guild028117Popup.ErrorCallback));
  ...
  {
    if (this.statement.GetComponent<UIWidget>().height > this.scrollContainerWidget.height)
    {
      this.dragScrollViewWidget.bottomAnchor.target = this.statement.transform;
  ...
  {
    if (!((UnityEngine.Object) this.input.GetComponent<UILabel>() != (UnityEngine.Object) null))
      return;
    this.maxWord.SetTextLocalize(Consts.Format(Consts.GetInstance().POPUP_028_1_1_7_STATEMENT_LIMIT, (IDictionary) new Hashtable()
    {
      {
