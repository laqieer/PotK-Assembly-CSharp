...
public class Popup00191DataDownloadMenu : BackButtonMenuBase
{
  ...
  public void Init(Story00191Menu menu)
  {
    ...
    System.Action<long, UILabel, UILabel, SpreadColorButton> action1 = (System.Action<long, UILabel, UILabel, SpreadColorButton>) ((size, txtSize, txtAlert, btn) =>
    {
      txtAlert.gameObject.SetActive(false);
      if ((ulong) size > freeSize || size <= 0L)
      {
        txtAlert.gameObject.SetActive(size > 0L);
        btn.isEnabled = false;
      }
    ...
    System.Action<long, UILabel, SpreadColorButton> action2 = (System.Action<long, UILabel, SpreadColorButton>) ((size, txtSize, btn) =>
    {
      btn.isEnabled = size > 0L;
  ...
  public void IbtnVoiceDownload() => this.StartCoroutine(this.Download(this.voiceDownloadPaths, this.GetPopupDownLoadMessage(Consts.GetInstance().POPUP_DOWNLOAD_VOICE_MESSAGE, this.voiceDownloadSize)));

  public void IbtnMovieDownload() => this.StartCoroutine(this.Download(this.movieDownloadPaths, this.GetPopupDownLoadMessage(Consts.GetInstance().POPUP_DOWNLOAD_MOVIE_MESSAGE, this.movieDownloadSize)));

  public void IbtnEtcDownload() => this.StartCoroutine(this.Download(this.etcDownloadPaths, this.GetPopupDownLoadMessage(Consts.GetInstance().POPUP_DOWNLOAD_ETC_MESSAGE, this.etcDownloadSize)));

  public void IbtnVoiceDelete() => this.StartCoroutine(this.Delete(this.voiceDeleteFileNames, this.GetPopupDeleteMessage(Consts.GetInstance().POPUP_DELETE_VOICE_MESSAGE, this.voiceDeleteSize)));

  public void IbtnMovieDelete() => this.StartCoroutine(this.Delete(this.movieDeleteFileNames, this.GetPopupDeleteMessage(Consts.GetInstance().POPUP_DELETE_MOVIE_MESSAGE, this.movieDeleteSize)));

  private IEnumerator Download(List<string> paths, string popupMessage)
  {
    bool? popup_yes_select = new bool?();
    PopupCommonYesNo.Show(Consts.GetInstance().POPUP_DOWNLOAD_TITLE, popupMessage, (System.Action) (() => popup_yes_select = new bool?(true)), (System.Action) (() => popup_yes_select = new bool?(false)));
    ...
    if (popup_yes_select.Value)
    {
      ...
      ModalWindow.Show(Consts.GetInstance().POPUP_DOWNLOAD_TITLE, Consts.GetInstance().POPUP_DOWNLOAD_DONE, (System.Action) (() =>
      {
  ...
  {
    bool? popup_yes_select = new bool?();
    PopupCommonYesNo.Show(Consts.GetInstance().POPUP_DELETE_TITLE, popupMessage, (System.Action) (() => popup_yes_select = new bool?(true)), (System.Action) (() => popup_yes_select = new bool?(false)));
    while (!popup_yes_select.HasValue)
      yield return (object) null;
    if (popup_yes_select.Value)
    {
      ...
      yield return (object) new WaitForSeconds(1f);
      Singleton<CommonRoot>.GetInstance().HideLoadingLayer();
      ModalWindow.Show(Consts.GetInstance().POPUP_DELETE_TITLE, Consts.GetInstance().POPUP_DELETE_DONE, (System.Action) (() =>
      {
        this.menu.IsPush = false;
        this.menu.IbtnDetaDownload();
      }));
  ...
  private string GetPopupDownLoadMessage(string message, long size) => string.Format(message, (object) this.GetHummableDownLoadSize(size));

  private string GetPopupDeleteMessage(string message, long size) => string.Format(message, (object) this.GetHummableDeleteSize(size));

  private string GetHummableDownLoadSize(long size) => Consts.Format(Consts.GetInstance().POPUP_DOWNLOAD_SIZE, (IDictionary) new Hashtable()
  {
    {
      (object) "value",
  ...
  private string GetHummableDeleteSize(long size) => Consts.Format(Consts.GetInstance().POPUP_DOWNLOAD_SIZE, (IDictionary) new Hashtable()
  {
    {
      (object) "value",
      (object) Consts.GetInstance().hummableSize(size)
