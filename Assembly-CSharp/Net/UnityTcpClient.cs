...
namespace Net
{
  public class UnityTcpClient : IDisposable
  {
    ...
    public Future<None> Connect(string hostname, int port) => new Future<None>((Func<Promise<None>, IEnumerator>) (promise => this.ConnectE(hostname, port, promise)));

    private IEnumerator ConnectE(string hostname, int port, Promise<None> promise) => this.RunWithTimeout<None>(None.Value, this.ConnectTimeout, promise, (Action<object, None>) ((sync, result) =>
    {
      try
      {
    ...
    public Future<UnityTcpClient.ReadResult> Read(int bytes) => new Future<UnityTcpClient.ReadResult>((Func<Promise<UnityTcpClient.ReadResult>, IEnumerator>) (promise => this.ReadE(bytes, promise)));

    private IEnumerator ReadE(int bytes, Promise<UnityTcpClient.ReadResult> promise) => this.RunWithTimeout<UnityTcpClient.ReadResult>(new UnityTcpClient.ReadResult(), this.ReceiveTimeout, promise, (Action<object, UnityTcpClient.ReadResult>) ((sync, result) =>
    {
    ...
    public Future<None> Write(byte[] buf) => new Future<None>((Func<Promise<None>, IEnumerator>) (promise => this.WriteE(buf, promise)));

    private IEnumerator WriteE(byte[] buf, Promise<None> promise) => this.RunWithTimeout<None>(None.Value, this.SendTimeout, promise, (Action<object, None>) ((sync, result) => this.GetStreamByWorker().Write(buf, 0, buf.Length)));
