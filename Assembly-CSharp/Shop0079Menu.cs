...
public class Shop0079Menu : BackButtonMenuBase
{
  ...
  public IEnumerator Init(WebAPI.Response.CoinbonusHistory history)
  {
    ...
    yield return (object) shop0079Menu.charge.Init(shop0079Menu.gameObject.GetComponent<PurchaseView>(), Shop0079Menu.coinbonusHistory);
  ...
  private ShopTabType InitTab()
  {
    ...
    {
      this.tabButtons[1].gameObject.SetActive(false);
    }
    else
    {
    ...
    {
      this.tabButtons[2].gameObject.SetActive(false);
    }
    else
    {
    ...
    {
      this.tabButtons[3].gameObject.SetActive(false);
    }
    else
    {
  ...
  private IEnumerator InitChargeTab()
  {
    ...
    this.\u003C\u003E1__state = -1;
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E2__current = (object) shop0079Menu.charge.Init(shop0079Menu.gameObject.GetComponent<PurchaseView>(), Shop0079Menu.coinbonusHistory);
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = 1;
    return true;
  }
  ...
  private void ChangeTab(ShopTabType tabType)
  {
    ...
    for (int index = 0; index < this.tabButtons.Length; ++index)
    {
      if ((ShopTabType) index == tabType)
      {
        ...
        this.tabOn[index].gameObject.SetActive(true);
        this.tabOff[index].gameObject.SetActive(false);
      }
      else
      {
        this.tabButtons[index].SetSprite(this.tabButtons[index].disabledSprite);
        this.tabOn[index].gameObject.SetActive(false);
        this.tabOff[index].gameObject.SetActive(true);
      }
    }
    for (int index = 0; index < this.tabPages.Count; ++index)
    {
      if ((ShopTabType) index == tabType)
        this.tabPages[index].gameObject.SetActive(true);
      else
        this.tabPages[index].gameObject.SetActive(false);
    }
  ...
  public void IbtnNo()
  {
    ...
    SeaGlobalMenu objectOfType1 = UnityEngine.Object.FindObjectOfType<SeaGlobalMenu>();
    if ((UnityEngine.Object) objectOfType1 != (UnityEngine.Object) null)
      objectOfType1.UpdateBikkuriIcon();
    ShopTopMenu objectOfType2 = UnityEngine.Object.FindObjectOfType<ShopTopMenu>();
  ...
  private static IEnumerator NormalRecive(
    int productType,
    int masterPackId,
    int addCoin)
  {
    ...
    int addCoin)
  {
    ...
    yield return (object) Shop0079Menu.DoPurchaseOrReciveSucceededResult(rewardList, (uint) response.player_presents.Length > 0U, addCoin);
  }
    ...
    int addCoin)
  {
    ...
    foreach (GiftRewardSchema reward in response.rewards)
      rewardList.Add(new ShopItemGetResultInfo((MasterDataTable.CommonRewardType) reward.reward_type_id, reward.reward_id, reward.reward_quantity));
    yield return (object) Shop0079Menu.DoPurchaseOrReciveSucceededResult(rewardList, (uint) response.player_presents.Length > 0U, addCoin, true);
  }
    ...
    bool isHotDeal = false)
  {
    ...
    if (isHotDeal)
    {
      e = component.Init(rewardList, isOverflow, new System.Action(Shop0079Menu.OnFinishHotDealItemGetResult));
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
    }
    ...
    {
      e = component.Init(rewardList, isOverflow, new System.Action(Shop0079Menu.OnFinishShopItemGetResult));
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
    }
