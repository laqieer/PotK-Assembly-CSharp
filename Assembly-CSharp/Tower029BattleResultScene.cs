...
public class Tower029BattleResultScene : NGSceneBase
{
    ...
    bool isShowDamage)
  {
    ...
    if (!battleResultScene.isInitialized)
    {
      ...
      {
        (ResultMenuBase) battleResultScene.GetComponent<Tower029ResultScore>(),
        (ResultMenuBase) battleResultScene.GetComponent<BattleUI05RewardMenu>(),
        (ResultMenuBase) null
      };
      ...
      if (result.stage_clear_rewards != null && result.stage_clear_rewards.Length != 0)
        battleResultScene.sequences.Add((ResultMenuBase) battleResultScene.GetComponent<BattleUI05ClearBonusMenu>());
      if (((IEnumerable<TowerStage>) MasterData.TowerStageList).Count<TowerStage>((Func<TowerStage, bool>) (s => s.tower_id == result.tower_id)) == result.floor)
        battleResultScene.sequences.Add((ResultMenuBase) battleResultScene.GetComponent<Tower029TowerClearAnim>());
