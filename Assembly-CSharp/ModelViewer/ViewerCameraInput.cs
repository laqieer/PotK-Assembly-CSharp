  ...
  public class ViewerCameraInput
  {
    ...
    private int UpdateNewInputParameter()
    {
      ...
      for (int index2 = 0; index2 < num; ++index2)
      {
        ...
        {
          int touchId = this.GetTouchId(index2);
          if (((IEnumerable<ViewerCameraInput.TouchParameter>) this.touchParameters).FirstOrDefault<ViewerCameraInput.TouchParameter>((Func<ViewerCameraInput.TouchParameter, bool>) (x => x.TouchId == touchId)) == null)
          {
            ...
            touchParameter.TouchId = this.GetTouchId(index2);
            touchParameter.FirstTouchPosition = this.GetTouchPosition(index2);
            touchParameter.CurrentTouchPosition = this.GetTouchPosition(index2);
            touchParameter.BeforeTouchPosition = touchParameter.CurrentTouchPosition;
    ...
    {
      this.cameraMove *= Mathf.Pow(this.dumpingRatio, Time.deltaTime);
      ...
      if (touchParameter == null)
        return;
      this.cameraMove = UICamera.currentCamera.ScreenToWorldPoint(touchParameter.CurrentTouchPosition) - UICamera.currentCamera.ScreenToWorldPoint(touchParameter.BeforeTouchPosition);
    }

    private void UpdateCameraRotateParameter()
    {
      this.cameraRotation *= Mathf.Pow(this.dumpingRatio, Time.deltaTime);
      ViewerCameraInput.TouchParameter touchParameter = (ViewerCameraInput.TouchParameter) null;
      if (this.CheckOtherThanTouchDevice())
    ...
    private bool IsPinch() => this.touchCount >= 2 && (double) Vector2.Dot((Vector2) (this.touchParameters[0].CurrentTouchPosition - this.touchParameters[0].BeforeTouchPosition), (Vector2) (this.touchParameters[1].CurrentTouchPosition - this.touchParameters[1].BeforeTouchPosition)) < 0.0;
    ...
    private int GetTouchContinueIndex(int touchId)
    {
      ...
      {
        if (touchId == this.GetTouchId(index) && this.GetTouchState(index) != TouchPhase.Began)
          return index;
      }
      return -1;
    }
    ...
    private TouchPhase GetTouchState(int index)
    {
      ...
      if (Input.GetMouseButton(1))
        return TouchPhase.Moved;
      return Input.GetMouseButtonUp(1) ? TouchPhase.Ended : TouchPhase.Canceled;
    }

    private Vector3 GetTouchPosition(int index)
    {
      if (!this.CheckOtherThanTouchDevice())
        return (Vector3) Input.GetTouch(index).position;
