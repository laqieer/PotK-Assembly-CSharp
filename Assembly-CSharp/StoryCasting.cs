...
public class StoryCasting
{
  ...
  public string buildScript(string baseScript, Hashtable replaseExtension) => this.replaceText(this.overwriteText(this.removeComment(baseScript)), replaseExtension);
  ...
  private string replaceText(string text, Hashtable replaseExtension)
  {
    ...
    return regex.Replace(text, (MatchEvaluator) (match =>
    {
      ...
      Match match1 = replaceValue.Match(input);
      ...
      if (replaseExtension != null && replaseExtension.ContainsKey((object) input))
        return (string) replaseExtension[(object) input];
      Match match2 = funcPattern.Match(input);
      StoryCasting.CastUnit castUnit2;
      if (match2.Success && this.dicCast_.TryGetValue(match2.Groups[G_UNIT].Value, out castUnit2))
      {
        string str = castUnit2.execute(match2.Groups[G_FUNC].Value);
  ...
  private class CastUnit
  {
    ...
    public string execute(string func) => !string.IsNullOrEmpty(func) && func == "unitName" && this.unit_ != null ? this.unit_.name : string.Empty;
  }
}
