...
public class Unit00483StatusGauge : MonoBehaviour
{
    ...
    bool reduction)
  {
    ...
    float posX1 = this.blueGauge.transform.localPosition.x + (float) gaugeWidth1;
    float posX2 = posX1 + (float) gaugeWidth2;
    float posX3 = posX2 + (float) gaugeWidth3;
    float posX4 = posX3 + (float) gaugeWidth4;
    this.SetGaugeInfo(this.blueGauge, gaugeWidth1, this.blueGauge.transform.localPosition.x);
    ...
    this.blueGauge.gameObject.SetActive((uint) gaugeWidth1 > 0U);
    this.yellowGauge.gameObject.SetActive((uint) gaugeWidth5 > 0U);
    this.whiteGauge.gameObject.SetActive((uint) gaugeWidth2 > 0U);
    this.whiteAddGauge.gameObject.SetActive((uint) gaugeWidth3 > 0U);
    if (isComposeMax)
    {
    ...
    {
      this.greenGauge.gameObject.SetActive((uint) gaugeWidth4 > 0U);
    ...
    bool isComposeMax)
  {
    ...
    this.valueLabel.color = num == 0 ? Color.white : Color.yellow;
    ...
    this.greenGaugeL.gameObject.SetActive((uint) gaugeWidthCeil1 > 0U);
    this.yellowGaugeL.gameObject.SetActive((uint) gaugeWidthCeil2 > 0U);
    this.blueGauge.gameObject.SetActive((uint) width > 0U);
    this.yellowGauge.gameObject.SetActive((uint) gaugeWidthCeil6 > 0U);
    this.whiteGauge.gameObject.SetActive((uint) gaugeWidthCeil3 > 0U);
    this.whiteAddGauge.gameObject.SetActive((uint) gaugeWidthCeil4 > 0U);
    if (isComposeMax)
    {
      this.greenGauge.gameObject.SetActive(false);
      this.blueMaxGauge.gameObject.SetActive(true);
    ...
    {
      this.greenGauge.gameObject.SetActive((uint) gaugeWidthCeil5 > 0U);
      this.blueMaxGauge.gameObject.SetActive(false);
    }
