...
public class Shop00723Menu : BackButtonMenuBase
{
  ...
  private IEnumerator coExchangeUnit()
  {
    ...
    Future<WebAPI.Response.SelectticketSpend> future = WebAPI.SelectticketSpend(this.currentSample_.ID, this.currentType_ != UnitTypeEnum.random ? (int) this.currentType_ : 1, 1, (System.Action<WebAPI.Response.UserError>) (e =>
    {
    ...
    if (result != null)
    {
      ...
      this.playerUnitTicket_ = this.unitTicket_.id != 0 ? ((IEnumerable<PlayerSelectTicketSummary>) selectTicketSummaryArray).FirstOrDefault<PlayerSelectTicketSummary>((Func<PlayerSelectTicketSummary, bool>) (pt => pt.ticket_id == this.unitTicket_.id)) : ((IEnumerable<PlayerSelectTicketSummary>) selectTicketSummaryArray).FirstOrDefault<PlayerSelectTicketSummary>();
  ...
  private int GetExchangeCount(int rewardID)
  {
    ...
    if (selectTicketChoices != null)
    {
      ...
      num = exchangeCountList == null || !selectTicketChoices.exchangeable_count.HasValue ? (!selectTicketChoices.exchangeable_count.HasValue ? int.MaxValue : selectTicketChoices.exchangeable_count.Value) : selectTicketChoices.exchangeable_count.Value - exchangeCountList.exchange_count;
    }
    return num;
  }
