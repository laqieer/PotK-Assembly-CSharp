...
public class Raid032BattleResultScene : NGSceneBase
{
    ...
    WebAPI.Response.GuildraidBattleFinish result)
  {
    ...
    else
    {
      ...
      yield return (object) battleResultScene.LoadBossUnit(battleResultScene.quest_s_id, result.loop_count, (System.Action<PlayerUnit>) (u => bossUnit = u));
      ...
      battleResultScene.sequences.Add((IRaidResultMenu) battleResultScene.GetComponent<Raid032BattleResultMenu>());
      if (((result.damage_rewards == null ? 0 : ((uint) result.damage_rewards.Length > 0U ? 1 : 0)) | (result.defeat_rewards == null ? (false ? 1 : 0) : ((uint) result.defeat_rewards.Length > 0U ? 1 : 0))) != 0)
        battleResultScene.sequences.Add((IRaidResultMenu) battleResultScene.GetComponent<Raid032BattleResultRewardPopupMenu>());
  ...
  private IEnumerator LoadBossUnit(
    int raid_quest_s_id,
    int loopCount,
    System.Action<PlayerUnit> complete)
  {
    ...
    WebAPI.Response.GuildraidBattleFinish result)
  {
    ...
    {
      bool next = false;
      battleResultScene.StartCoroutine(PopupCommon.Show(Consts.GetInstance().GUILD_RAID_RP_BACK_TITLE, Consts.GetInstance().GUILD_RAID_RP_BACK_MESSAGE, (System.Action) (() => next = true)));
      while (!next)
    ...
    battleResultScene.touchToNext.SetActive(true);
    battleResultScene.touchToNext.GetComponent<BoxCollider>().enabled = false;
    while (seqe.MoveNext())
    {
      e = seqe.Current.Run();
      while (e.MoveNext())
