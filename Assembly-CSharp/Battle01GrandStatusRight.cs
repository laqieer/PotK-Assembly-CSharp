...
public class Battle01GrandStatusRight : NGBattleMenuBase
{
  ...
  protected override void LateUpdate_Battle()
  {
    if (this.dropMoneyModified.isChangedOnce())
    {
      ...
      {
        List<Tuple<BattleskillSkill, int, int>> tupleList = new List<Tuple<BattleskillSkill, int, int>>();
        if (u.is_leader || u.friend || u.playerUnit.is_enemy_leader || u.playerUnit.is_guest && u.is_helper)
          tupleList.AddRange(((IEnumerable<PlayerUnitLeader_skills>) u.playerUnit.leader_skills).Select<PlayerUnitLeader_skills, Tuple<BattleskillSkill, int, int>>((Func<PlayerUnitLeader_skills, Tuple<BattleskillSkill, int, int>>) (skill => new Tuple<BattleskillSkill, int, int>(skill.skill, skill.level, 0))));
        return ((IEnumerable<Tuple<BattleskillSkill, int, int>>) u.unitAndGearSkills).Concat<Tuple<BattleskillSkill, int, int>>((IEnumerable<Tuple<BattleskillSkill, int, int>>) tupleList).Where<Tuple<BattleskillSkill, int, int>>((Func<Tuple<BattleskillSkill, int, int>, bool>) (x => x.Item1.skill_type == BattleskillSkillType.passive || x.Item1.skill_type == BattleskillSkillType.leader)).SelectMany(x => ((IEnumerable<BattleskillEffect>) x.Item1.Effects).Where<BattleskillEffect>((Func<BattleskillEffect, bool>) (effect => effect.EffectLogic.Enum == BattleskillEffectLogicEnum.ratio_money && effect.checkLevel(x.Item2))).Select(effect => new
        {
          effect = effect,
          level = x.Item2
        }));
      ...
      foreach (var data in this.env.core.playerUnits.value.SelectMany(u =>
      {
        ...
        if (u.is_leader || u.friend || u.playerUnit.is_enemy_leader || u.playerUnit.is_guest && u.is_helper)
          tupleList.AddRange(((IEnumerable<PlayerUnitLeader_skills>) u.playerUnit.leader_skills).Select<PlayerUnitLeader_skills, Tuple<BattleskillSkill, int, int>>((Func<PlayerUnitLeader_skills, Tuple<BattleskillSkill, int, int>>) (skill => new Tuple<BattleskillSkill, int, int>(skill.skill, skill.level, 0))));
        List<\u003C\u003Ef__AnonymousType19<BattleskillEffect, int, int>> list = ((IEnumerable<Tuple<BattleskillSkill, int, int>>) u.unitAndGearSkills).Concat<Tuple<BattleskillSkill, int, int>>((IEnumerable<Tuple<BattleskillSkill, int, int>>) tupleList).Where<Tuple<BattleskillSkill, int, int>>((Func<Tuple<BattleskillSkill, int, int>, bool>) (x => x.Item1.skill_type == BattleskillSkillType.passive || x.Item1.skill_type == BattleskillSkillType.leader)).SelectMany(x => ((IEnumerable<BattleskillEffect>) x.Item1.Effects).Where<BattleskillEffect>((Func<BattleskillEffect, bool>) (effect =>
        {
