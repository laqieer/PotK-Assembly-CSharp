...
public class NGxFaceSprite : MonoBehaviour
{
  ...
  private Dictionary<string, UnityEngine.Sprite> faceSpriteDict = new Dictionary<string, UnityEngine.Sprite>();
  [SerializeField]
  private int unitID;
  ...
  private void clearSpriteCache()
  {
    ...
    this.mask.MainUI2DSprite.sprite2D = (UnityEngine.Sprite) null;
  }

  public IEnumerator ChangeFace(string faceName_)
  {
    this.faceName = faceName_;
    Future<UnityEngine.Sprite> mainFuture;
    IEnumerator e;
    if ((UnityEngine.Object) this.mask.MainUI2DSprite.sprite2D == (UnityEngine.Object) null)
    {
      if (this.unitID <= 999)
      {
    ...
    if (this.faceName == "normal")
      this.updateFaceSprite(this.faceName, (UnityEngine.Sprite) null);
    ...
    else if (this.unitID <= 999)
    {
      if (MobUnits.HasSpriteFace(this.unitID, this.faceName))
      {
        ...
        while (e.MoveNext())
          yield return e.Current;
        e = (IEnumerator) null;
        UnityEngine.Sprite result = mainFuture.Result;
        UnityEngine.Sprite sprite2 = UnityEngine.Sprite.Create(result.texture, result.rect, Vector2.zero, 1f, 0U, SpriteMeshType.FullRect);
        sprite2.name = this.faceName;
  ...
  public void updateFaceSprite(string faceName, UnityEngine.Sprite sprite)
  {
    this.faceSpriteDict[faceName] = sprite;
    this.FaceUI2DSprite.sprite2D = sprite;
    if ((UnityEngine.Object) sprite == (UnityEngine.Object) null)
      return;
    this.FaceUI2DSprite.SetDimensions(sprite.texture.width, sprite.texture.height);
    Vector3 localPosition = this.FaceUI2DSprite.gameObject.transform.localPosition;
    ...
    {
      UI2DSprite component = this.FaceUI2DSprite.transform.parent.gameObject.GetComponent<UI2DSprite>();
      num1 += (float) (component.sprite2D.texture.width / 2 - (unitUnitStory2.face_x + sprite.texture.width / 2)) * this.mask.scale;
      num2 += (float) (component.sprite2D.texture.height / 2 - (unitUnitStory2.face_y + sprite.texture.height / 2)) * this.mask.scale;
    }
    this.FaceUI2DSprite.gameObject.transform.localScale = new Vector3(this.mask.scale, this.mask.scale, 1f);
    this.FaceUI2DSprite.gameObject.transform.localPosition = new Vector3(-num1, -num2, localPosition.z);
    this.FaceUI2DSprite.depth = this.FaceUI2DSprite.transform.parent.gameObject.GetComponent<UIWidget>().depth + 1;
  }
  ...
  private void Update()
  {
    if (this.isEnable)
    {
      this.FaceUI2DSprite.enabled = true;
      this.isEnable = false;
    }
    if ((double) this.mask.spriteAlpha < 1.0)
      this.FaceUI2DSprite.enabled = false;
    else
      this.isEnable = true;
  }
}
