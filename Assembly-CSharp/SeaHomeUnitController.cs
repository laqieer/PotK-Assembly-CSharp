...
public class SeaHomeUnitController : MonoBehaviour
{
  ...
  public void Clear()
  {
    ...
    this.waitCount = 0;
    UnityEngine.Object.DestroyObject((UnityEngine.Object) this.unitObject);
  ...
  public IEnumerator Init(GameObject shadowEffect)
  {
    ...
    CapsuleCollider orAddComponent = homeUnitController.bipTransform.gameObject.GetOrAddComponent<CapsuleCollider>();
    ...
    homeUnitController.unitObject.SetParent(homeUnitController.transform.gameObject);
    homeUnitController.targetPosition = homeUnitController.nowPosition = SeaHomeUnitController.outPositions[UnityEngine.Random.Range(0, 100000) % SeaHomeUnitController.outPositions.Length];
  ...
  private void Update()
  {
    ...
    else if (this.nowPosition != this.targetPosition)
    {
      ...
      Vector3 vector3 = this.direction * this.velocity * Time.deltaTime;
    ...
    else
    {
      ...
      else if (this.NowSit)
      {
        switch (this.sitActionList.Shuffle<int>().First<int>())
        {
          ...
          case 1:
            ...
            {
              this.SetIndexToAnimation("sit_action_index", UnityEngine.Random.Range(0, 100000) % this.sitActionCount);
      ...
      else if (this.owner.CheckDance(this) && UnityEngine.Random.Range(0, 100) <= 5)
      {
      ...
      else
      {
        switch (this.actionList.Shuffle<int>().First<int>())
        {
          case 0:
            ...
            break;
          case 1:
            this.SetIndexToAnimation("action_index", UnityEngine.Random.Range(0, 100000) % this.actionCount);
  ...
  private bool Move(SeaHomeUnitController.UnitPositions target)
  {
    ...
    this.direction = this.GetPosition(this.targetPosition) - this.GetPosition(this.nowPosition);
    this.direction.Normalize();
    if (this.waitCount > 1 && UnityEngine.Random.Range(0, 10000) <= 1000)
      this.SetIndexToAnimation("wait_index", UnityEngine.Random.Range(0, 100) % this.waitCount);
    else
  ...
  public void CheckStopMove()
  {
    ...
    Vector3 vector3_1 = this.direction * this.velocity * ((IEnumerable<AnimatorClipInfo>) this.myAnimator.GetCurrentAnimatorClipInfo(0)).FirstOrDefault<AnimatorClipInfo>((Func<AnimatorClipInfo, bool>) (x => animeState.IsName(x.clip.name))).clip.length;
    Vector3 vector3_2 = this.unitTransform.localPosition + vector3_1;
    Vector3 position = this.GetPosition(this.nowPosition);
    Vector3 vector3_3 = position;
    if ((double) (vector3_2 - vector3_3).sqrMagnitude <= (double) (this.GetPosition(this.targetPosition) - vector3_1 / 2f - position).sqrMagnitude)
      return;
    this.SetTriggerToAnimation("wait");
    this.SetUnitStatus(SeaHomeUnitController.UnitStatus.Stand);
  }
  ...
  public SeaHomeUnitController.UnitPositions GetTransformNearPosition()
  {
    ...
    SeaHomeUnitController.UnitPositions unitPositions = SeaHomeUnitController.UnitPositions.outR;
    foreach (SeaHomeUnitController.UnitPositions allPosition in SeaHomeUnitController.allPositions)
    {
      float sqrMagnitude = (this.GetPosition(allPosition) - this.unitTransform.localPosition).sqrMagnitude;
      if ((double) num > (double) sqrMagnitude)
      {
        unitPositions = allPosition;
        num = sqrMagnitude;
      }
    }
  ...
  private void SetIndexToAnimation(string name, int index) => this.myAnimator.SetInteger(name, index);
  ...
  public void ResetActionWait() => this.actionWait = UnityEngine.Random.Range(1f, 3f);

  public void SetLookuped() => this.owner.SetLookupedAuto(this);

  public void ResetLookuped() => this.owner.ResetLookupedAuto(this);

  public bool IsTouch(GameObject bipObject) => (UnityEngine.Object) this.bipTransform != (UnityEngine.Object) null && this.bipTransform.gameObject.Equals((object) bipObject);

  public override string ToString() => this.unitData.Unit != null ? this.unitData.Unit.name : string.Empty;
