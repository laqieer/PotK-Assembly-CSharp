...
public class PopupOverkillersSlotRelease : BackButtonPopupBase
{
  ...
  private IEnumerator Start()
  {
    ...
    UIPanel panel = overkillersSlotRelease.GetComponent<UIPanel>();
    panel.alpha = 0.0f;
    panel.UpdateAnchors();
    ...
    else
    {
      ...
      overkillersSlotRelease.btnRelease_.isEnabled = overkillersSlotRelease.ngFlags_ == PopupOverkillersSlotRelease.NGFlags.Clear;
    ...
    if (overkillersSlotRelease.condition_.gem <= 0)
    {
      ...
      if (Singleton<NGGameDataManager>.GetInstance().IsSea)
        overkillersSlotRelease.popupBaseSprite.SetDimensions(overkillersSlotRelease.popupBaseSprite.width, 610);
      else
        overkillersSlotRelease.popupBaseSprite.SetDimensions(overkillersSlotRelease.popupBaseSprite.width, 670);
    }
    ...
    overkillersSlotRelease.colliderMaterialDialog_.size = (Vector3) panel.GetViewSize();
    overkillersSlotRelease.colliderMaterialDialog_.transform.position = Vector3.zero;
    Singleton<PopupManager>.GetInstance().startOpenAnime(overkillersSlotRelease.gameObject);
  }
    ...
    PlayerMaterialUnit material)
  {
    ...
    if (src != null)
    {
      ...
      icon.onClick = new System.Action<UnitIconBase>(overkillersSlotRelease.\u003CinitMaterial\u003Eb__30_0);
