...
public class ExploreChallengePopup : BackButtonMenuBase
{
  ...
  private UnityEngine.Sprite maskOpponent;
  ...
  public IEnumerator Initialize(ExploreFooter footer)
  {
    ...
    Future<UnityEngine.Sprite> maskOpponentF = new ResourceObject("GUI/explore_other/slc_unit_mask_Opponent").Load<UnityEngine.Sprite>();
    e1 = maskOpponentF.Wait();
    while (e1.MoveNext())
      yield return e1.Current;
    e1 = (IEnumerator) null;
    this.maskOpponent = maskOpponentF.Result;
    maskOpponentF = (Future<UnityEngine.Sprite>) null;
    if (!WebAPI.IsResponsedAtRecent("ExploreChallengeBoot") || dataMgr.Gladiators == null)
    {
      Future<WebAPI.Response.ExploreChallengeBoot> futureF = WebAPI.ExploreChallengeBoot((System.Action<WebAPI.Response.UserError>) (e =>
      {
  ...
  private IEnumerator SetChallengeDeckIcon()
  {
    ...
    Future<UnityEngine.Sprite> futureSprite = leaderUnit.unit.LoadSpriteLarge(leaderUnit.job_id, 1f);
    IEnumerator e = futureSprite.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    this.mask_Chara.sprite2D = futureSprite.Result;
    Future<UnityEngine.Sprite> futureMask = new ResourceObject("GUI/explore_other/slc_unit_mask_Leader").Load<UnityEngine.Sprite>();
    e = futureMask.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    this.mask_Chara.GetComponent<NGxMaskSpriteWithScale>().maskTexture = futureMask.Result.texture;
    ...
    for (int i = 1; i < deck.player_units.Length; ++i)
    {
      ...
      unitPlayer.Button.isEnabled = false;
  ...
  public static UnityEngine.Sprite LoadGearKindSprite(
    GearKindEnum kind,
  ...
  private IEnumerator SetOpponents()
  {
    ...
    ((IEnumerable<GameObject>) this.dirCustome).ForEach<GameObject>((System.Action<GameObject>) (x => x.SetActive(false)));
    if (arrayMax == 1)
    {
  ...
  private IEnumerator SetOpponentsInfo(int index, ChallengeNpc data)
  {
    ...
    Future<UnityEngine.Sprite> futureSprite = unitUnit.LoadSpriteLarge(data.leader_unit_job_id, 1f);
    IEnumerator e = futureSprite.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    this.opponentMaskCharas[index].sprite2D = futureSprite.Result;
    this.opponentMaskCharas[index].GetComponent<NGxMaskSpriteWithScale>().maskTexture = this.maskOpponent.texture;
  }
  ...
  private int[] GetRandomShuffle(int arrayMax, int rangeMax)
  {
    ...
    Array.Copy((Array) array, (Array) numArray2, arrayMax);
    return numArray2;
  }
  ...
  private IEnumerator BattleStart(int index)
  {
    ...
    {
      if ((UnityEngine.Object) exploreChallengePopup.nowPopup == (UnityEngine.Object) null)
        exploreChallengePopup.nowPopup = ModalWindow.ShowYesNo(Consts.GetInstance().EXPLORE_POPUP_REPAIR_CONFIRM_TITLE, Consts.GetInstance().EXPLORE_POPUP_REPAIR_CONFIRM_MESSAGE, new System.Action(exploreChallengePopup.RepairConfirmationYesBtn), new System.Action(exploreChallengePopup.RepairConfirmationNoBtn)).gameObject;
    }
    else
    {
      PlayerDeck deck = Singleton<ExploreDataManager>.GetInstance().GetChallengeDeck();
      if (deck.cost > SMManager.Get<Player>().max_cost)
        exploreChallengePopup.nowPopup = ModalWindow.Show(Consts.GetInstance().EXPLORE_POPUP_COSTOVER_TITLE, Consts.GetInstance().EXPLORE_POPUP_COSTOVER_DESCRIPTION, (System.Action) (() => this.StartCoroutine(this.IsPushOff()))).gameObject;
      ...
      else
      {
        ...
        yield return (object) Singleton<ExploreDataManager>.GetInstance().SaveSuspendData((System.Action) (() => saveFailed = true));
        ...
        else
        {
          ...
          Future<WebAPI.Response.ExploreChallengeStart> futureF = WebAPI.ExploreChallengeStart(exploreChallengePopup.gladiators[index1].player_id, deck.total_combat, (System.Action<WebAPI.Response.UserError>) (error =>
          {
