...
public class NGDuelDataManager : Singleton<NGDuelDataManager>
{
  ...
  public ClipEventEffectData GetPreloadClipEventEffectData(string name) => string.IsNullOrEmpty(name) || !this.preloadClipEventEffectData.ContainsKey(name) ? (ClipEventEffectData) null : this.preloadClipEventEffectData[name];

  public void AddPreloadClipEventEffectData(ClipEventEffectData effectData)
  {
    if ((UnityEngine.Object) effectData == (UnityEngine.Object) null || string.IsNullOrEmpty(effectData.name) || this.preloadClipEventEffectData.ContainsKey(effectData.name))
      return;
    this.preloadClipEventEffectData.Add(effectData.name, effectData);
  }
  ...
  public GameObject GetPreloadDuelEffect(string name, Transform parent = null)
  {
    ...
    GameObject gameObject = this.preloadDuelEffect[name].Clone(parent);
    this.destroyList.Add(gameObject);
    return gameObject;
  }

  public void AddPreloadDuelEffect(GameObject prefab)
  {
    if ((UnityEngine.Object) prefab == (UnityEngine.Object) null || string.IsNullOrEmpty(prefab.name) || this.preloadDuelEffect.ContainsKey(prefab.name))
      return;
    this.preloadDuelEffect.Add(prefab.name, prefab);
  }

  public Texture GetDuelCutin(int unitId) => !this.duelCutin.ContainsKey(unitId) ? (Texture) null : this.duelCutin[unitId];
  ...
  private void ClearFullCache()
  {
    ...
    UnityEngine.Object.Destroy((UnityEngine.Object) this.mMapObject);
  ...
  public void ClearOneTimeDuelCache()
  {
    ...
    foreach (UnityEngine.Object destroy in this.destroyList)
    ...
    Light directionalLight)
  {
    ...
    directionalLight.transform.rotation = this.mMapData.getDuelDirectionalLightRotate();
  ...
  {
    if ((UnityEngine.Object) this.mMapObject == (UnityEngine.Object) null)
      return;
  ...
  public void StartTurnEffectBackgroundLoad(int turnNum) => this.StartCoroutine(this.loadTurnEffect(turnNum));
    ...
    NGDuelUnit enemyDUnit)
  {
    ...
    foreach (HashSet<string> stringSet in stringSetArray)
    {
      ...
      this.attackEffectLoader[index] = new NGDuelDataManager.DuelEffectLoader<GameObject>(futureList, new System.Action<GameObject>(this.AddPreloadDuelEffect));
      ++index;
    }
  }
  ...
  private IEnumerator loadPreloadCutinByTurnNumber(int turnNum)
  {
    if (((IEnumerable<List<UnitUnit>>) this.preloadCutinUnitMasterLists).Count<List<UnitUnit>>() > turnNum && this.preloadCutinUnitMasterLists[turnNum] != null)
    {
      foreach (UnitUnit unitUnit in this.preloadCutinUnitMasterLists[turnNum])
      {
        ...
        if (!((UnityEngine.Object) this.GetDuelCutin(unit.ID) != (UnityEngine.Object) null))
        {
          Future<UnityEngine.Sprite> fs = unit.LoadCutin();
  ...
  public IEnumerator LoadUnitControllerDuelEffect(
    NGDuelUnit playerDUnit,
    NGDuelUnit enemyDUnit)
  {
    NGDuelDataManager ngDuelDataManager = this;
  ...
  private void setupMagicBulletEffectLoader(DuelResult duelResult)
  {
    ...
    foreach (HashSet<string> stringSet in stringSetArray)
    {
      ...
      this.magicBulletLoader[index2] = new NGDuelDataManager.DuelEffectLoader<GameObject>(futureList, (System.Action<GameObject>) (f =>
      {
  ...
  private void setupClipEventEffectLoader(DuelResult duelResult)
  {
    ...
    foreach (HashSet<string> stringSet in stringSetArray)
    {
      ...
      this.clipEventEffectLoader[index2] = new NGDuelDataManager.DuelEffectLoader<ClipEventEffectData>(futureList, new System.Action<ClipEventEffectData>(this.AddPreloadClipEventEffectData));
      ++index2;
    }
  }
  ...
  public Future<RuntimeAnimatorController> LoadDuelCamera(string name) => this.cameraAnimator.ContainsKey(name) ? Future.Single<RuntimeAnimatorController>(this.cameraAnimator[name]) : Singleton<ResourceManager>.GetInstance().Load<RuntimeAnimatorController>(string.Format("Animators/Camera/{0}", (object) name)).Then<RuntimeAnimatorController>((Func<RuntimeAnimatorController, RuntimeAnimatorController>) (x =>
  {
  ...
  private class DuelEffectLoader<T>
  {
    ...
    public DuelEffectLoader(List<Future<T>> futureList, System.Action<T> onceLoadCallback)
    {
