...
public class Versus02610Menu : Versus026MatchBase
{
    ...
    WebAPI.Response.PvpBoot pvpInfo)
  {
      ...
      Singleton<TutorialRoot>.GetInstance().ForceShowAdviceInNextButton(Player.Current.IsClassMatchRanking() ? "pvp_class2" : "pvp_class1", new Dictionary<string, Func<Transform, UIButton>>()
      {
        ...
        },
        {
          ...
          (Func<Transform, UIButton>) (root =>
          {
            ...
            childInFind.GetChildInFind("ibtn_Ranking").gameObject.SetActive(Player.Current.IsClassMatchRanking());
            childInFind.GetChildInFind("ibtn_Ranking_comingsoon").gameObject.SetActive(!Player.Current.IsClassMatchRanking());
            return (UIButton) null;
          })
        }
      }, new System.Action(versus02610Menu.\u003CInit\u003Eb__42_2));
    }
    else
    {
    ...
    {
      versus02610Menu.btnRanking.isEnabled = false;
      versus02610Menu.slcBtnRanking.color = Color.gray;
      versus02610Menu.btnRewardInfo.isEnabled = false;
      versus02610Menu.slcBtnRewardInfo.color = Color.gray;
      versus02610Menu.btnHowto.isEnabled = false;
      versus02610Menu.slcBtnHowto.color = Color.gray;
    }
    else
    {
      versus02610Menu.btnRanking.isEnabled = true;
      versus02610Menu.slcBtnRanking.color = Color.white;
      versus02610Menu.btnRewardInfo.isEnabled = true;
      versus02610Menu.slcBtnRewardInfo.color = Color.white;
      versus02610Menu.btnHowto.isEnabled = true;
      versus02610Menu.slcBtnHowto.color = Color.white;
    }
  ...
  private void createMatchRemainLamp()
  {
    ...
    for (int index = 0; index < player.mp_max; ++index)
    {
      ...
      if (index >= player.mp)
      {
        gameObject.transform.GetChildInFind("slc_icon_Multi_Lamp_01_on").gameObject.SetActive(false);
      }
      else
      {
  ...
  {
    if ((UnityEngine.Object) this.LampIconPrefab == (UnityEngine.Object) null)
    {
  ...
  private IEnumerator RankingEndPopup()
  {
    ...
    Future<WebAPI.Response.PvpRankingClose> f = WebAPI.PvpRankingClose((System.Action<WebAPI.Response.UserError>) (e =>
    {
  ...
  private void InitializeDisplay()
  {
    ...
    this.btnRanking.gameObject.SetActive(flag);
  ...
  private void SetTxtCondition(UILabel l)
  {
    ...
    switch (condition2)
    {
      ...
      case PvpClassKind.Condition.STAY_TOPCLASS:
        Color color1 = (Color) new Color32((byte) 10, (byte) 223, (byte) 29, byte.MaxValue);
        l.color = color1;
      ...
      case PvpClassKind.Condition.UP:
        Color color2 = (Color) new Color32(byte.MaxValue, byte.MaxValue, (byte) 0, byte.MaxValue);
        l.color = color2;
      ...
      case PvpClassKind.Condition.TITLE_TOPCLASS:
        ...
        {
          Color color3 = (Color) new Color32(byte.MaxValue, (byte) 105, (byte) 0, byte.MaxValue);
          l.color = color3;
          l.SetText(instance.VERSUS_002610CONDITION_TITLE);
          break;
        }
        Color color4 = (Color) new Color32(byte.MaxValue, (byte) 105, (byte) 0, byte.MaxValue);
        l.color = color4;
  ...
  private void SetRankGauge()
  {
    ...
    int max = 10;
    PvpClassKind pvpClassKind = MasterData.PvpClassKind[this.pvpInfo.current_class];
    this.slcRankGaugeBlue.width = (pvpClassKind.stay_point - 1) * width / max;
    this.slcRankGaugeGreen.width = (pvpClassKind.up_point - 1) * width / max;
    this.slcRankGaugeYellow.width = (pvpClassKind.title_point - 1) * width / max;
    this.slcRankGaugeBlue.gameObject.SetActive(pvpClassKind.stay_point > 0);
    this.slcRankGaugeGreen.gameObject.SetActive(pvpClassKind.up_point - pvpClassKind.stay_point > 0);
    this.slcRankGaugeYellow.gameObject.SetActive(pvpClassKind.title_point - pvpClassKind.up_point > 0);
    this.slcRankGaugeRed.gameObject.SetActive(true);
    this.slcRankGaugeNow.transform.localPosition = new Vector3(this.slcRankGaugeRed.transform.localPosition.x + (float) (Mathf.Clamp(this.pvpInfo.pvp_class_record.current_season_win_count, 0, max) * width / max), this.slcRankGaugeNow.transform.localPosition.y);
  }
  ...
  private IEnumerator ShowAggregatePopup()
  {
    ...
    IEnumerator e = PopupCommon.Show(instance1.VERSUS_002610AGGREGATE_TITLE, instance1.VERSUS_002610AGGREGATE_DESCRIPTION, (System.Action) (() =>
    {
  ...
  protected override bool IsMatchingBeginCheck() => SMManager.Get<Player>().mp > 0 && !this.pvpInfo.rank_aggregate;
  ...
  {
    Future<WebAPI.Response.PvpRanking> f = WebAPI.PvpRanking((System.Action<WebAPI.Response.UserError>) (e =>
    {
      Singleton<CommonRoot>.GetInstance().isLoading = false;
      WebAPI.DefaultUserErrorCallback(e);
      MypageScene.ChangeSceneOnError();
  ...
  private IEnumerator createRewardTumnails()
  {
    ...
    if (this.responsePvpRanking != null && ((IEnumerable<RankingGroup>) this.responsePvpRanking.ranking_groups).Count<RankingGroup>() > 0)
    {
      ...
      for (int index = 0; index < classRankingRewardArray.Length; ++index)
      {
        ...
        }, new Vector3((float) (-biasX * (num - 1) / 2 + biasX * i), 0.0f));
        if (++i >= num)
          break;
      }
  ...
  private IEnumerator SceneUpdate(bool isUIUpdate = true)
  {
    ...
    Future<WebAPI.Response.PvpBoot> future = WebAPI.PvpBoot((System.Action<WebAPI.Response.UserError>) (e =>
    {
      WebAPI.DefaultUserErrorCallback(e);
      MypageScene.ChangeSceneOnError();
    }));
  ...
  {
    this.btnSeasonFinish.gameObject.SetActive(isEnable);
    this.btnBreakRepair.gameObject.SetActive(!isEnable);
    this.btnStartMatch.gameObject.SetActive(!isEnable);
    if (isEnable)
      return;
    this.SetMatchButton();
  }

  public void StartSceneUpdate(System.Action act = null, bool isUIUpdate = true)
  {
