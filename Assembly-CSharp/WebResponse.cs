...
public class WebResponse
{
  ...
  public static WebResponse Zero() => new WebResponse()
  {
    Status = 0,
    Header = string.Empty,
    Bytes = (byte[]) null,
  ...
  public int? getJsonIntOrNull(string key) => !(this.getJsonValueObject(key) is long jsonValueObject) ? new int?() : new int?((int) jsonValueObject);

  public string getJsonStringOrNull(string[] keys) => ((IEnumerable<string>) keys).Select<string, object>((Func<string, object>) (x => this.getJsonValueObject(x))).FirstOrDefault<object>((Func<object, bool>) (x => x != null)) as string;
