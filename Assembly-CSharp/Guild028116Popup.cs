...
public class Guild028116Popup : BackButtonMenuBase
{
  ...
  private IEnumerator SendCancelRequest()
  {
    ...
    else if (applied)
    {
      ...
      Singleton<PopupManager>.GetInstance().open(result).GetComponent<GuildOkPopup>().Initialize(Consts.Format(Consts.GetInstance().POPUP_028_1_1_6_TITLE), Consts.Format(Consts.GetInstance().POPUP_028_1_1_6_NO_APPLICANT), ok: ((System.Action) (() =>
      {
        Singleton<CommonRoot>.GetInstance().ShowLoadingLayer(0);
