...
public class Popup00230HuntingTargetDetail : BackButtonMenuBase
{
  ...
  private UnityEngine.Sprite GetGearKindSprite(GearKind kind)
  {
    string empty = string.Empty;
    return Resources.Load<UnityEngine.Sprite>(kind.Enum == GearKindEnum.smith || kind.Enum == GearKindEnum.accessories || (kind.Enum == GearKindEnum.dummy || kind.Enum == GearKindEnum.none) ? "Icons/Materials/GuideWeaponBtn/slc_unique_wepon_idle" : string.Format("Icons/Materials/GuideWeaponBtn/slc_{0}_idle", (object) kind.Enum.ToString()));
  }
    ...
    GameObject questListPrefab)
  {
    ...
    e = (IEnumerator) null;
    ((IEnumerable<UI2DSprite>) this.gearKindIcons).ForEach<UI2DSprite>((System.Action<UI2DSprite>) (x => x.gameObject.SetActive(false)));
    List<GearKind> gearKindList = new List<GearKind>();
    ((IEnumerable<EnemyTopInfo>) infos).ForEach<EnemyTopInfo>((System.Action<EnemyTopInfo>) (x => gearKindList.Add(MasterData.UnitUnit[x.unit_id].initial_gear.kind)));
    ...
    for (int index = 0; index < num3; ++index)
    {
      ...
      this.gearKindIcons[index].gameObject.SetActive(true);
    }
    ...
    foreach (EnemyDetailInfoQuest_infos questInfo in detailData.enemy_detail_infos.quest_infos)
    {
      ...
      GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(questListPrefab);
