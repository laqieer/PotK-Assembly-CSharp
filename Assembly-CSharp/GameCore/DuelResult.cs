...
namespace GameCore
{
  ...
  public class DuelResult : ActionResult
  {
    ...
    public override ActionResultNetwork ToNetworkLocal(BL env) => (ActionResultNetwork) new DuelResultNetwork()
    {
    ...
    public int[] playerUnitBeforeAilmentEffectIDs() => !this.isPlayerAttack ? this.beforeDefenderAilmentEffectIDs : this.beforeAttakerAilmentEffectIDs;

    public BL.Unit enemyUnit() => !this.isPlayerAttack ? this.attack : this.defense;

    public int[] enemyUnitBeforeAilmentEffectIDs() => !this.isPlayerAttack ? this.beforeAttakerAilmentEffectIDs : this.beforeDefenderAilmentEffectIDs;

    public AttackStatus playerAttackStatus() => !this.isPlayerAttack ? this.defenseAttackStatus : this.attackAttackStatus;

    public AttackStatus enemyAttackStatus() => !this.isPlayerAttack ? this.attackAttackStatus : this.defenseAttackStatus;

    public AttackStatus playerColosseumNAS() => !this.isPlayerAttack ? this.colosseumNewDAS : this.colosseumNewAAS;

    public AttackStatus enemyColosseumNAS() => !this.isPlayerAttack ? this.colosseumNewAAS : this.colosseumNewDAS;

    public int playerColosseumFirstAttack() => !this.isPlayerAttack ? this.colosseumDefenseFirstAttack : this.colosseumAttackFirstAttack;

    public int enemyColosseumFirstAttack() => !this.isPlayerAttack ? this.colosseumAttackFirstAttack : this.colosseumDefenseFirstAttack;

    public int playerAttackDamage() => !this.isPlayerAttack ? this.defenseDamage : this.attackDamage;

    public int enemyAttackDamage() => !this.isPlayerAttack ? this.attackDamage : this.defenseDamage;

    public bool isHeal => this.attackAttackStatus.magicBullet != null && this.attackAttackStatus.magicBullet.isHeal;

    public IntimateDuelSupport attackDuelSupport
    {
      get => !this.attackDuelSupportId.HasValue ? (IntimateDuelSupport) null : MasterData.IntimateDuelSupport[this.attackDuelSupportId.Value];
      set => this.attackDuelSupportId = value == null ? new int?() : new int?(value.ID);
    }

    public IntimateDuelSupport defenseDuelSupport
    {
      get => !this.defenseDuelSupportId.HasValue ? (IntimateDuelSupport) null : MasterData.IntimateDuelSupport[this.defenseDuelSupportId.Value];
      set => this.defenseDuelSupportId = value == null ? new int?() : new int?(value.ID);
    }

    public int sumSwapHealDamage(bool isAttacker) => ((IEnumerable<BL.DuelTurn>) this.turns).Sum<BL.DuelTurn>((Func<BL.DuelTurn, int>) (x => x.isAtackker != isAttacker ? x.defenderSwapHealDamage : x.attackerSwapHealDamage));
  }
}
