...
namespace GameCore
{
  public class Lex<T> where T : class
  {
    ...
    public T Try(string expect1, string expect2, Func<T> callback) => this.Try(expect1, (Func<T>) (() =>
    {
      T obj = callback();
      this.Skip(expect2);
      return obj;
    ...
    public string Show() => string.Format("tokens='{0}' pos={1} text={2}", (object) string.Join(" ", this.tokens), (object) this.pos, (object) this.Text);
  }
}
