...
public class Quest00221DetailMenu : BackButtonMenuBase
{
  ...
  public IEnumerator coInitialize(QuestDetailData data)
  {
    ...
    for (int index = 0; index < data.kinds.Length && index < this.iconKinds_.Length; ++index)
    {
      ...
      this.iconKinds_[index].gameObject.SetActive(true);
    }
    ...
    for (int index = 0; index < data.elements.Length && index < this.iconElements_.Length; ++index)
    {
      ...
      this.iconElements_[index].gameObject.SetActive(true);
    }
    int i;
    if (data.ailments.Length != 0)
    {
      ...
      {
        Future<UnityEngine.Sprite> ld2 = data.ailments[i].LoadBattleSkillIcon();
        e = ld2.Wait();
        while (e.MoveNext())
          yield return e.Current;
        e = (IEnumerator) null;
        this.iconAilments_[i].sprite2D = ld2.Result;
        this.iconAilments_[i].gameObject.SetActive(true);
        this.setEventClickedSkillIcon(this.iconAilments_[i].gameObject, data.ailments[i]);
        ld2 = (Future<UnityEngine.Sprite>) null;
      }
    }
  ...
  private void setEventClickedSkillIcon(GameObject go, BattleskillSkill skill) => EventDelegate.Set(go.GetComponent<UIButton>().onClick, (EventDelegate.Callback) (() =>
  {
    if (this.currentSkillDetail_ == skill && (Object) this.objSkillDetail_ != (Object) null && this.objSkillDetail_.GetComponentInChildren<Battle0171111Event>().DialogConteiner.activeSelf)
      return;
    this.currentSkillDetail_ = skill;
    if ((Object) this.objSkillDetail_ == (Object) null)
  ...
  {
    CreateIconObject cio = this.grid_.gameObject.GetOrAddComponent<CreateIconObject>();
    IEnumerator e = cio.CreateThumbnail(data.rewardType, data.rewardId, data.quantity, isButton: false);
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    cio.GetIcon().transform.localScale = (Vector3) this.scaleDropIcon_;
  }

  private UnityEngine.Sprite getSpriteGearKind(GearKindEnum eKind)
  {
    string path;
    switch (eKind)
    {
  ...
  private UnityEngine.Sprite getSprite(string path) => Resources.Load<UnityEngine.Sprite>(path);
