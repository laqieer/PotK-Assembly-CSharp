...
namespace DeckOrganization
{
  public class Creator
  {
    ...
    protected void setResult(IEnumerable<DeckPosition> deck) => this.setResult(deck != null ? deck.Where<DeckPosition>((Func<DeckPosition, bool>) (dp => dp.unit_ != null)).Select<DeckPosition, PlayerUnit>((Func<DeckPosition, PlayerUnit>) (dp => dp.unit_.unit_)).ToList<PlayerUnit>() : (List<PlayerUnit>) null);

    protected void setResult(IEnumerable<Unit> units) => this.setResult(units != null ? units.Select<Unit, PlayerUnit>((Func<Unit, PlayerUnit>) (u => u?.unit_)).ToList<PlayerUnit>() : (List<PlayerUnit>) null);

    protected void setResult(List<PlayerUnit> units) => this.result_ = units == null || units.Count<PlayerUnit>((Func<PlayerUnit, bool>) (u => u != (PlayerUnit) null)) < this.minQuantity_ ? (List<PlayerUnit>) null : units;
...
label_40:
    ...
    {
      List<DeckPosition> deckPositionList = this.regulars_ != null ? ((IEnumerable<PlayerUnit>) this.regulars_).Select<PlayerUnit, DeckPosition>((Func<PlayerUnit, int, DeckPosition>) ((u, i) =>
      {
        DeckPosition deckPosition = new DeckPosition(i);
        if (u != (PlayerUnit) null)
          deckPosition.setUnit(new Unit(u, true));
        return deckPosition;
      })).ToList<DeckPosition>() : new List<DeckPosition>();
      int count = deckPositionList.Count;
      while (count < setnum)
        deckPositionList.Add(new DeckPosition(count++));
      return deckPositionList;
    }

    protected IEnumerable<Unit> getUnits(List<PlayerUnit> regulars = null) => regulars == null || regulars.Count <= 0 ? ((IEnumerable<PlayerUnit>) this.playerUnits_).Select<PlayerUnit, Unit>((Func<PlayerUnit, Unit>) (u => new Unit(u))) : ((IEnumerable<PlayerUnit>) this.playerUnits_).Where<PlayerUnit>((Func<PlayerUnit, bool>) (u => !regulars.Any<PlayerUnit>((Func<PlayerUnit, bool>) (a => a.id == u.id)))).Select<PlayerUnit, Unit>((Func<PlayerUnit, Unit>) (u => new Unit(u)));

    protected IEnumerable<Unit> basicSort(IEnumerable<Unit> units) => (IEnumerable<Unit>) units.OrderByDescending<Unit, int>((Func<Unit, int>) (u => u.unit_.level)).ThenByDescending<Unit, int>((Func<Unit, int>) (u => u.unit_.unit.FinalEvolutionCost)).ThenByDescending<Unit, int>((Func<Unit, int>) (u => u.param_.Combat)).ThenByDescending<Unit, int>((Func<Unit, int>) (u => u.unit_.id)).ThenByDescending<Unit, DateTime>((Func<Unit, DateTime>) (u => u.unit_.created_at));
