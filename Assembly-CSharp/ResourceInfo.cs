...
public class ResourceInfo : IEnumerable<ResourceInfo.Resource>, IEnumerable
{
  ...
  public static ResourceInfo.Resource SearchResourceInfo(
    string path,
  ...
  public ResourceInfo.Resource SearchResourceInfo(string path) => ResourceInfo.SearchResourceInfo(path, this);
  ...
  public struct Step
  {
    ...
    public string GetSharedMaterials(int num, string key) => this._sharedMaterials[num][key].name[1] == null ? this._sharedMaterials[num][key].name[0] : this._sharedMaterials[num][key].name[0] + this._sharedMaterials[num][key].name[1];

    public FilterMode GetSharedMaterialFilterMode(int num, string key) => this._sharedMaterials[num][key].filterMode;

    public TextureWrapMode GetSharedMaterialTextureWrapMode(int num, string key) => this._sharedMaterials[num][key].wrapMode;
  ...
  public struct Resource
  {
    ...
    private string[] GetStepPath(string path)
    {
      ...
      if (num1 <= 0)
      {
        strArray[0] = string.Intern(path);
      }
      else
      {
        int num2 = num1 + 1;
        strArray[0] = string.Intern(path.Substring(0, num2));
        strArray[1] = string.Intern(path.Substring(num2));
      }
      return strArray;
    }

    public bool IsSound() => ((IEnumerable<string>) ResourceInfo.Resource.SoundExtension).Contains<string>(Path.GetExtension(this._value._ext));

    public bool IsMovie() => ((IEnumerable<string>) ResourceInfo.Resource.MovieExtension).Contains<string>(Path.GetExtension(this._value._ext));
  }
  ...
  private class PathComparer : IComparer<ResourceInfo.Resource>
  {
    ...
    public int Compare(ResourceInfo.Resource x, ResourceInfo.Resource y) => Comparer<string>.Default.Compare(x._key, y._key);
  }
}
