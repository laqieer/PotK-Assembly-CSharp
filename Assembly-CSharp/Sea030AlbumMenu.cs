...
public class Sea030AlbumMenu : BackButtonMenuBase
{
  ...
  private Dictionary<int, UnityEngine.Sprite> illustDictionary = new Dictionary<int, UnityEngine.Sprite>();
  ...
  private void SetAlbumList()
  {
    for (int index = 0; index < this.playerAlbums.Length; ++index)
    {
      ...
      SeaAlbum component = this.scrollParts.instantiateParts(this.AlbumPrefab, false).GetComponent<SeaAlbum>().GetComponent<SeaAlbum>();
  ...
  private IEnumerator SetData()
  {
    ...
    Future<WebAPI.Response.SeaAlbumIndex> f = WebAPI.SeaAlbumIndex(new System.Action<WebAPI.Response.UserError>(sea030AlbumMenu.\u003CSetData\u003Eb__26_0));
  ...
  {
    Future<UnityEngine.Sprite> imageF = Singleton<ResourceManager>.GetInstance().Load<UnityEngine.Sprite>(string.Format("Album/{0}/slc_album_s", (object) album.ID));
  ...
  private void UpdateInfo(int index = 0)
  {
    ...
    this.btnLeft.SetActive((uint) index > 0U);
  }

  private void SetBtnZoom(PlayerAlbum playerAlbum) => this.BtnZoom.isEnabled = !((IEnumerable<PlayerAlbumPiece>) playerAlbum.player_album_piece).Any<PlayerAlbumPiece>((Func<PlayerAlbumPiece, bool>) (x => !x.is_open));

  public void OnBtnChangeIllustration() => Sea030AlbumIllustrationScene.ChangeScene(this.mainPanel.width, this.playerAlbums[this.currentAlbumId].album_id, true);
  ...
  public void OnBtnRewardPopup()
  {
    ...
    if (seaAlbum == null || (UnityEngine.Object) this.rewardPopup == (UnityEngine.Object) null || this.IsPushAndSet())
      return;
  ...
  private IEnumerator OpenRewardPopup(int reward_group_id)
  {
    ...
    IEnumerator e = popup.GetComponent<Popup030SeaAlbumRewardConfirmationMenu>().Init(reward_group_id, new System.Action(sea030AlbumMenu.PushReset));
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
  ...
  public void OnBtnDetail()
  {
    ...
    if (album == null)
      return;
    List<SeaAlbumPiece> list = this.albumPieceList.Where<SeaAlbumPiece>((Func<SeaAlbumPiece, bool>) (x => x.album_id == album.ID)).ToList<SeaAlbumPiece>();
    if ((UnityEngine.Object) this.detailPopup == (UnityEngine.Object) null || this.IsPushAndSet())
      return;
  ...
  private IEnumerator OpenDetailPopup(
    ...
    List<SeaAlbumPiece> pieceList)
  {
    Sea030AlbumMenu sea030AlbumMenu = this;
    GameObject popup = sea030AlbumMenu.detailPopup.Clone();
    popup.SetActive(false);
    IEnumerator e = popup.GetComponent<Popup030SeaDetailMenu>().Init(playerAlbum, pieceList, new System.Action(sea030AlbumMenu.PushReset));
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    Singleton<PopupManager>.GetInstance().open(popup, isCloned: true);
