...
public class PopupEditCustomDeck : BackButtonPopupBase
{
  ...
  private IEnumerator Start()
  {
    ...
    popupEditCustomDeck.scroll_.enabled = false;
    if (popupEditCustomDeck.units_.Length > 1)
    {
      GameObject gameObject = popupEditCustomDeck.editView_.gameObject;
    ...
    popupEditCustomDeck.GetComponent<AnchorCustomAdjustment>().resetAnchors((Transform[]) null);
    yield return (object) null;
    foreach (UnityEngine.Object componentsInChild in popupEditCustomDeck.GetComponentsInChildren<UIDragScrollView>())
    ...
    popupEditCustomDeck.grid_.onReposition = new UIGrid.OnReposition(popupEditCustomDeck.\u003CStart\u003Eb__12_0);
    popupEditCustomDeck.grid_.Reposition();
    yield return (object) null;
    popupEditCustomDeck.changeCenter(popupEditCustomDeck.currentIndex_, false);
    popupEditCustomDeck.scroll_.enabled = true;
    popupEditCustomDeck.requestChange_ = new int?();
    yield return (object) null;
    Singleton<PopupManager>.GetInstance().startOpenAnime(popupEditCustomDeck.gameObject, popupEditCustomDeck.isDisabledOpenSe_);
  }
  ...
  {
    GameObject[] array = ((IEnumerable<PopupEditCustomDeckUnit>) this.pages_).Select<PopupEditCustomDeckUnit, GameObject>((Func<PopupEditCustomDeckUnit, GameObject>) (x => x.gameObject)).ToArray<GameObject>();
