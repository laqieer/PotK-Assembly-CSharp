...
public class Quest0022MissionDescription : MonoBehaviour
{
    ...
    PlayerMissionHistory[] hists)
  {
    ...
    for (int i = 0; i < missions.Length; ++i)
    {
      ...
      Quest0022MissionList missionItem = menu.missionItem.Clone(this.grid.transform).GetComponent<Quest0022MissionList>();
      missionItem.transform.localPosition = new Vector3(0.0f, (float) -i * this.grid.cellHeight, 0.0f);
      IEnumerator e = missionItem.SetValue(missions[i], clearFlag);
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      this.MissionList.Add(missionItem);
      this.ThumbnailList.Add(missionItem.LinkParent.gameObject);
  ...
  public virtual IEnumerator LoadAnimation(int maxEffectCount)
  {
    ...
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    Transform[] array = h.Result.Clone(missionDescription.transform).transform.GetChildren().ToArray<Transform>();
    ((IEnumerable<Transform>) array).ForEach<Transform>((System.Action<Transform>) (x => x.gameObject.SetActive(false)));
    for (int index1 = 0; index1 < missionDescription.MissionList.Count && index1 <= maxEffectCount; ++index1)
    {
      if (index1 < array.Length)
      {
        ...
        missionDescription.MissionList[index1].Animation = array[index1].gameObject;
      }
      else
      {
        int index2 = index1 % array.Length;
        GameObject gameObject = array[index2].gameObject.Clone(missionDescription.ThumbnailList[index1].transform);
  ...
  {
    if (trigger == null)
      this.completeAnimator.gameObject.SetActive(true);
    else
