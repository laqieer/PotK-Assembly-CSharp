...
public class BattleUI02SubMenu : BattleBackButtonMenuBase
{
  ...
  protected override IEnumerator Start_Battle()
  {
    ...
    {
      Transform transform = battleUi02SubMenu.statusPlayerList[index].GetComponent<Battle02StatusScrollParts>().chilObject[0].transform;
      ...
      battleUi02SubMenu.statusPlayerList[index].transform.localPosition = new Vector3(battleUi02SubMenu.statusPlayerList[index].transform.localPosition.x - 100000f, battleUi02SubMenu.statusPlayerList[index].transform.localPosition.y, battleUi02SubMenu.statusPlayerList[index].transform.localPosition.z);
    }
    for (int index = 0; index < battleUi02SubMenu.statusEnemyList.Length; ++index)
    {
      Transform transform = battleUi02SubMenu.statusEnemyList[index].GetComponent<Battle02StatusScrollParts>().chilObject[0].transform;
      transform.localPosition = new Vector3(transform.localPosition.x + 100000f, transform.localPosition.y, transform.localPosition.z);
      battleUi02SubMenu.statusEnemyList[index].transform.localPosition = new Vector3(battleUi02SubMenu.statusEnemyList[index].transform.localPosition.x - 100000f, battleUi02SubMenu.statusEnemyList[index].transform.localPosition.y, battleUi02SubMenu.statusEnemyList[index].transform.localPosition.z);
    }
  ...
  private void SetActiceButton(BattleUI02SubMenu.Tabname tabName, bool forcingUpdate = false)
  {
    ...
    {
      ((IEnumerable<Battle02StatusScrollParts>) this.statusEnemyList).ForEach<Battle02StatusScrollParts>((System.Action<Battle02StatusScrollParts>) (v => v.gameObject.SetActive(false)));
      for (int index = 0; index < this.statusPlayerList.Length; ++index)
      {
        this.statusPlayerList[index].transform.parent.gameObject.SetActive((BattleUI02SubMenu.Tabname) index == tabName);
        this.statusPlayerList[index].gameObject.SetActive((BattleUI02SubMenu.Tabname) index == tabName);
      }
    }
    else
    {
      ((IEnumerable<Battle02StatusScrollParts>) this.statusPlayerList).ForEach<Battle02StatusScrollParts>((System.Action<Battle02StatusScrollParts>) (v => v.gameObject.SetActive(false)));
      for (int index = 0; index < this.statusEnemyList.Length; ++index)
      {
        this.statusEnemyList[index].transform.parent.gameObject.SetActive((BattleUI02SubMenu.Tabname) index == tabName);
        this.statusEnemyList[index].gameObject.SetActive((BattleUI02SubMenu.Tabname) index == tabName);
      }
    }
  }
