...
public class FriendBarBase : BackButtonMenuBase
{
  ...
  protected void SetSortLabel()
  {
    for (int index = 0; index < this.sortSprites.Length; ++index)
      this.sortSprites[index].gameObject.SetActive(index == Persist.sortOrder.Data.Friend);
  }
  ...
  protected void SortAndSetIcons()
  {
    ...
    for (int index = 0; index < Mathf.Min(this.MaxValue, this.allFriendBar.Count); ++index)
      this.scroll.AddColumn1(this.allFriendBar[index].gameObject, this.barWidth, this.barHeight);
  ...
  protected void ScrollUpdate()
  {
    ...
    do
    {
      ...
      foreach (GameObject gameObject in this.scroll)
      {
        ...
        int? nullable = this.allFriendInfo.FirstIndexOrNull<FriendBarInfo>((Func<FriendBarInfo, bool>) (v => (UnityEngine.Object) v.scroll != (UnityEngine.Object) null && (UnityEngine.Object) v.scroll.gameObject == (UnityEngine.Object) unit));
  ...
  protected IEnumerator CreateScrollBase(GameObject prefab)
  {
    ...
    for (int index = 0; index < Mathf.Min(this.MaxValue, this.allFriendInfo.Count); ++index)
      this.allFriendBar.Add(UnityEngine.Object.Instantiate<GameObject>(prefab).GetComponent<FriendScrollBar>());
    this.scroll.Reset();
    for (int index = 0; index < Mathf.Min(this.MaxValue, this.allFriendBar.Count); ++index)
      this.scroll.AddColumn1(this.allFriendBar[index].gameObject, this.barWidth, this.barHeight);
    this.scroll.CreateScrollPointHeight(this.barHeight, this.allFriendInfo.Count);
  ...
  protected void ResetScroll(int index)
  {
    ...
    this.allFriendInfo.Where<FriendBarInfo>((Func<FriendBarInfo, bool>) (a => (UnityEngine.Object) a.scroll == (UnityEngine.Object) scroll)).ForEach<FriendBarInfo>((System.Action<FriendBarInfo>) (b => b.scroll = (FriendScrollBar) null));
  }
