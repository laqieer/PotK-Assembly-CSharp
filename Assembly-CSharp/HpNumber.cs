...
public class HpNumber : BattleMonoBehaviour
{
  ...
  private void SetBaseNumber(int _number) => this.SetNumber(_number, this.base_keta_roots, this.base_nums);

  private void SetDamageNumber(int _number) => this.SetNumber(_number, this.damage_keta_roots, this.damage_nums);

  private void SetHealNumber(int _number) => this.SetNumber(_number, this.heal_keta_roots, this.heal_nums);
  ...
  private void SetNumber(int _number, GameObject[] keta_roots, GameObject[][] numbers)
  {
    ...
    for (int index = 1; index < keta_roots.Length; ++index)
      max = max * 10 + 9;
    int num1 = Mathf.Clamp(_number, 0, max);
    int num2 = 1;
    for (int index1 = 0; index1 < keta_roots.Length; ++index1)
    {
      ((IEnumerable<GameObject>) numbers[index1]).ForEach<GameObject>((System.Action<GameObject>) (obj => obj.SetActive(false)));
      if (num1 >= num2 || index1 == 0)
      {
        int index2 = num1 / num2 % 10;
