...
public class Versus0268VictoryDetail : MonoBehaviour
{
  ...
  private IEnumerator SetConditionSprite(PvpClassKind c, int nowWin)
  {
    ...
    if ((c.stay_point != 0 && c.stay_point == nowWin || (c.up_point == nowWin || c.title_point == nowWin)) && this.arrowAnim)
    {
      ...
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      this.slcClassCondition.gameObject.SetActive(false);
    }
    else
    {
      ...
      else
      {
        ...
        this.slcClassCondition.MakePixelPerfect();
      }
    }
  }
  ...
  private IEnumerator SetConditionEffect(PvpClassKind.Condition condition)
  {
    ...
    ((IEnumerable<PopupClassChange>) this.NextGaugeAnimation.GetComponents<PopupClassChange>()).ForEach<PopupClassChange>((System.Action<PopupClassChange>) (x => x.ChangeSprite(condition)));
  }
  ...
  private void SetRankGauge(PvpClassKind c)
  {
    ...
    this.slcRankGaugeBlue.gameObject.SetActive(c.stay_point > 0);
    this.slcRankGaugeGreen.gameObject.SetActive(c.up_point - c.stay_point > 0);
    this.slcRankGaugeYellow.gameObject.SetActive(c.title_point - c.up_point > 0);
  ...
  private void SetArrow(int win)
  {
    ...
    this.slcRankGaugeNowParent.transform.localPosition = new Vector3(this.slcRankGaugeRed.transform.localPosition.x + (float) (Mathf.Clamp(win, 0, max) * width / max), this.slcRankGaugeNowParent.transform.localPosition.y);
  }
  ...
  private IEnumerator StartAnim()
  {
    ...
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = -1;
    tween = versus0268VictoryDetail.slcRankGaugeNow.GetComponent<UITweener>();
