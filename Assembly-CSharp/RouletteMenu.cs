...
public class RouletteMenu : NGMenuBase
{
  ...
  private IEnumerator InitializeCutIn(int cutInPattern)
  {
    if (cutInPattern != 0)
    {
      ...
      Future<UnityEngine.Sprite> cutInSprite;
      switch (cutInPattern)
      {
        case 1:
  ...
  private Future<UnityEngine.Sprite> GetCutInSprite(int cutInUnitId) => cutInUnitId == -999 ? Singleton<ResourceManager>.GetInstance().Load<UnityEngine.Sprite>("AssetBundle/Resources/Characters/ottimo/ottimo_cutin") : MasterData.UnitUnit[cutInUnitId].LoadCutin();
