...
public class Bugu00561Menu : BackButtonMenuBase
{
  ...
  public IEnumerator InitDetailedScreen(GameCore.ItemInfo item)
  {
    ...
    if ((UnityEngine.Object) this.GearDetail != (UnityEngine.Object) null)
      this.GearDetail.SetActive(true);
    Future<UnityEngine.Sprite> spriteF = item.gear.LoadSpriteBasic();
    IEnumerator e = spriteF.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    ...
    int index = 0)
  {
    ...
    bugu00561Menu.slc_NewIcon.gameObject.SetActive(isNew);
    if (isNew)
      bugu00561Menu.slc_NewIcon.PlayForward();
    if ((UnityEngine.Object) bugu00561Menu.scroll != (UnityEngine.Object) null)
      bugu00561Menu.scroll.scrollView.enabled = bugu00561Menu.gearList.Length > 1;
    ...
    for (int index1 = 0; index1 < bugu00561Menu.objectCnt; ++index1)
    {
      bugu00561Menu.detailObject[index1] = UnityEngine.Object.Instantiate<GameObject>(bugu00561Menu.buguDetail);
      bugu00561Menu.objectList.Add(bugu00561Menu.detailObject[index1]);
      bugu00561Menu.scroll.Add(bugu00561Menu.detailObject[index1]);
      Unit00443BuguList component = bugu00561Menu.detailObject[index1].GetComponent<Unit00443BuguList>();
      bugu00561Menu.detailPrefabDict.Add(bugu00561Menu.detailObject[index1], component);
    }
    yield return (object) null;
    bugu00561Menu.scroll.ResolvePosition();
    bugu00561Menu.scroll.scrollView.transform.localPosition = new Vector3(-bugu00561Menu.scroll.grid.cellWidth * (float) bugu00561Menu.currentIndex, 0.0f, 0.0f);
    ...
    for (int index1 = 0; index1 < bugu00561Menu.objectList.Count; ++index1)
      bugu00561Menu.objectList[index1].transform.localPosition = end < bugu00561Menu.gearList.Length - 1 ? new Vector3(bugu00561Menu.scroll.grid.cellWidth * (float) (end + index1 + 1), 0.0f, 0.0f) : new Vector3(bugu00561Menu.scroll.grid.cellWidth * (float) (start - (index1 + 1)), 0.0f, 0.0f);
  }
  ...
  private void SetInfo(GameCore.ItemInfo item)
  {
    ...
    this.LinkItem.gameObject.SetActive(false);
    if (!item.isSupply)
    {
      if ((UnityEngine.Object) this.TxtTitle != (UnityEngine.Object) null)
        this.TxtTitle.SetTextLocalize(item.gear.name);
  ...
  public IEnumerator CreatePages(int gearIndex, bool isChangePage = false)
  {
    ...
    Vector3 gridPos = this.scroll.grid.transform.localPosition;
    ...
    d.index = gearIndex;
    go.transform.localPosition = new Vector3(this.scroll.grid.cellWidth * (float) gearIndex, 0.0f, 0.0f);
    this.scroll.grid.transform.localPosition = gridPos;
  ...
  private void SetMenuInformation(int idx)
  {
    ...
    if (idx == 0 && (UnityEngine.Object) this.leftArrows != (UnityEngine.Object) null)
      this.leftArrows.SetActive(false);
    if (idx == this.gearList.Length - 1 && (UnityEngine.Object) this.rightArrows != (UnityEngine.Object) null)
      this.rightArrows.SetActive(false);
