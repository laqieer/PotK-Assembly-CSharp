...
public class Unit00410Menu : UnitSelectMenuBase
{
    ...
    Unit00410Menu.FromType fromType = Unit00410Menu.FromType.AlertUnitOver)
  {
    ...
    if (unit00410Menu.sortSprites_ != null && unit00410Menu.sortSprites_.Length > index && (UnityEngine.Object) unit00410Menu.sortSprites_[index] != (UnityEngine.Object) null)
    ...
    unit00410Menu.InitializeInfoEx((IEnumerable<PlayerUnit>) playerUnits, (IEnumerable<PlayerMaterialUnit>) playerMaterialUnits, persistData, isEquip, false, true, true, true, false, (System.Action) (() => this.InitializeAllUnitInfosExtend(playerDeck)));
  ...
  {
    if (!((UnityEngine.Object) mono != (UnityEngine.Object) null))
      return;
    mono.enabled = false;
  }

  private int SelectedUnitCount() => this.selectedUnitIcons.Select<UnitIconInfo, int>((Func<UnitIconInfo, int>) (u => u.SelectedCount)).Sum();
  ...
  public override void UpdateInfomation()
  {
    ...
    foreach (UnitIconInfo selectedUnitIcon in this.selectedUnitIcons)
    {
      ...
      if (unit != null && unit.playerUnit != (PlayerUnit) null)
      {
        ...
        if (!this.isMemoryAlert)
          this.isMemoryAlert = ((IEnumerable<int?>) nullableArray).Any<int?>((Func<int?, bool>) (x =>
          {
            if (!x.HasValue)
              return false;
            int? nullable = x;
    ...
    this.TxtNumberzeny.color = num2 + player.money <= Consts.GetInstance().MONEY_MAX ? Color.white : Color.red;
    this.TxtNumberpossession.SetTextLocalize(string.Format("{0}", (object) num3));
    this.TxtNumberpossession.color = num3 + player.medal <= Consts.GetInstance().MEDAL_MAX ? Color.white : Color.red;
    this.TxtNumberselect.color = num1 >= this.SelectMax ? Color.red : Color.white;
  ...
  private IEnumerator IbtnYesAsync()
  {
    ...
    if (menu.selectedUnitIcons.Any<UnitIconInfo>((Func<UnitIconInfo, bool>) (icon => icon.playerUnit.unit.rarity.index > 1)))
    {
      ...
      menu.selectedUnitIcons.ForEach((System.Action<UnitIconInfo>) (v => ls.AddRange((IEnumerable<PlayerUnit>) unit00410Menu.ExpandPlayerUnits(v, v.SelectedCount))));
  ...
  public void AllUpdateUnitIcons()
  {
    ...
    {
      if ((UnityEngine.Object) allUnitInfo.icon != (UnityEngine.Object) null && allUnitInfo.button_enable)
      {
  ...
  private IEnumerator OpenPopup(UnitIconInfo unitIconInfo)
  {
    ...
    List<System.Action> callbackList = new List<System.Action>();
    callbackList.Add(new System.Action(((UnitSelectMenuBase) unit00410Menu).CreateSelectUnitList));
    callbackList.Add(new System.Action(unit00410Menu.AllUpdateUnitIcons));
    callbackList.Add(new System.Action(((UnitSelectMenuBase) unit00410Menu).UpdateInfomation));
