...
public class MypageEditMenu : UnitSelectMenuBase
{
  ...
  protected IEnumerator CreateBottomInformationObject()
  {
    ...
    foreach (GameObject linkCharacter in mypageEditMenu.linkCharacters)
      mypageEditMenu.CreateUnitObject(linkCharacter).gameObject.SetActive(false);
  }

  private IEnumerator SetSelectUnit(UnitIconInfo info)
  {
    MypageEditMenu mypageEditMenu = this;
    GameObject gameObject = mypageEditMenu.linkCharacters[info.select].transform.GetChildInFind(mypageEditMenu.IconObjName).gameObject;
    ...
    bool isEquip)
  {
    ...
    ((IEnumerable<GameObject>) mypageEditMenu.linkCharacters).ForEach<GameObject>((System.Action<GameObject>) (v => v.transform.Clear()));
    ...
    mypageEditMenu.InitializeInfo((IEnumerable<PlayerUnit>) playerUnits, (IEnumerable<PlayerMaterialUnit>) null, Persist.mypageEditorSortAndFilter, isEquip, false, false, true, true, (System.Action) (() => {}));
  ...
  protected IEnumerator DeckEditApi(int deckTypeID, int deckNumber, int[] playerUnitIds)
  {
    ...
    if (Singleton<NGGameDataManager>.GetInstance().IsSea)
    {
      ...
      e1 = WebAPI.SeaDeckEdit(deckNumber, playerUnitIds, new System.Action<WebAPI.Response.UserError>(mypageEditMenu.\u003CDeckEditApi\u003Eb__21_0)).Wait();
      while (e1.MoveNext())
  ...
  protected override void CreateUnitIconCache(int info_index, int unit_index, PlayerUnit baseUnit = null)
  {
    ...
    {
      allUnitIcon.blinkDeckStatus.gameObject.SetActive(true);
  ...
  private void setLongPressEvent(UnitIconBase unitIcon, int info_index) => unitIcon.setLongPressEvent((System.Action) (() => Unit0042Scene.changeSceneMypageEdit(this.displayUnitInfos[info_index].playerUnit, this.getUnits())));
}
