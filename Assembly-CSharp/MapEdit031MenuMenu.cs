...
public class MapEdit031MenuMenu : MapEditMenuBase
{
  ...
  protected override IEnumerator initializeAsync()
  {
    ...
    mapEdit031MenuMenu.widgetMapCheck_ = mapEdit031MenuMenu.mapCheck_.GetComponent<UIWidget>();
  ...
  protected override void onEnable()
  {
    ...
    this.btnReset_.isEnabled = this.topMenu_.data_.dicOrnament_.Count > 0;
  ...
  private IEnumerator doConfirmResetLayout()
  {
    ...
    bool bWait = true;
    bool bOk = false;
    ModalWindow.ShowYesNo(instance.MAPEDIT_031_TITLE_CONFIRM_RESETLAYOUT, instance.MAPEDIT_031_MESSAGE_CONFIRM_RESETLAYOUT, (System.Action) (() =>
    {
      bWait = false;
      bOk = true;
    ...
    if (bOk)
    {
      mapEdit031MenuMenu.btnReset_.isEnabled = false;
  ...
  private IEnumerable<Tuple<int, int>> sortPositions(
    IEnumerable<Tuple<int, int>> lstPos)
  {
  ...
  {
    this.widgetMapCheck_.alpha = 0.0f;
    IEnumerator e = this.doResetInformation(this.lstPosition_);
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    TweenAlpha.Begin(this.widgetMapCheck_.gameObject, 0.5f, 1f);
  }
