...
public class PopupCoinDetail : BackButtonMenuBase
{
  ...
  public IEnumerator Init(int? common_ticket_id)
  {
    ...
    else
    {
      ...
      int val = playerCommonTicket == null ? 0 : playerCommonTicket.quantity;
      popupCoinDetail.ticket_quantity = NC.Clamp(0, Consts.GetInstance().SUBCOIN_DISP_MAX, val);
      CommonTicket ticketMaster = MasterData.CommonTicket[popupCoinDetail.ticket_id];
      Future<UnityEngine.Sprite> future = ticketMaster.LoadIconMSpriteF();
