...
public class Mypage00181ScrollParts : MonoBehaviour
{
  ...
  private string GetText(string text, int maxText) => text.Length > maxText ? text.Substring(0, maxText) : text;

  private void SetData(OfficialInformationArticle info, NGMenuBase menu) => ((MonoBehaviour) menu).StartCoroutine(this.LoadBanner(info.title_img_url));
