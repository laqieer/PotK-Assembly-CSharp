...
namespace Gsc.Purchase.API.Request
{
  public class ChargeV2Limit : Gsc.Network.Request<ChargeV2Limit, Gsc.Purchase.API.Response.ChargeV2Limit>
  {
    ...
    public override string GetPath() => SDK.Configuration.Env.PurchaseApiPrefix + "/v2/limit?currency=" + this.Currency + string.Join("", this.Prices.Select<float, string>((Func<float, string>) (x => "&price=" + (object) x)).ToArray<string>());
