  ...
  public class AmazonAppStore : FlowWithPurchaseKit
  {
    ...
    protected override IWebTask CreateFulfillmentTask(PurchaseKit.PurchaseResponse response)
    {
      ...
      foreach (PurchaseKit.PurchaseData purchaseData in response.Values)
      {
        ...
        string data0 = response.Meta.Data0;
        ...
        ((WebTask<Verify, Verify.Response>) (webTask = (IWebTask) new Verify(data0, currencyCode, (float) num, id).GetTask(WebTaskAttribute.Reliable | WebTaskAttribute.Silent | WebTaskAttribute.Parallel))).OnResponse((VoidCallbackWithError<Verify.Response>) ((r, e) =>
        {
          if (e != null)
            hasError = true;
