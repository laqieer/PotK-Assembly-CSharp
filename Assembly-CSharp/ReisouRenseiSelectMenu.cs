...
public class ReisouRenseiSelectMenu : Bugu005SelectItemListMenuBase
{
  ...
  private bool IsActiveDrillingItem(PlayerItem item) => item.isReisou() && this.BaseItem != null && (item.id != this.BaseItem.itemID && item.gear != null) && this.IsActiveDrillingIcon(item.gear);

  private bool IsActiveDrillingMaterial(PlayerMaterialGear item) => item.gear.isReisou() && this.BaseItem != null && (item.id != this.BaseItem.itemID && item.gear != null) && this.IsActiveDrillingIcon(item.gear);
  ...
  protected override IEnumerator InitExtension()
  {
    ...
    if (renseiSelectMenu.firstInit)
    {
      ...
      // ISSUE: reference to a compiler-generated method
      renseiSelectMenu.FirstSelectItemList.ForEach(new System.Action<InventoryItem>(renseiSelectMenu.\u003CInitExtension\u003Eb__38_2));
    }
    else if (renseiSelectMenu.SelectItemList != null && renseiSelectMenu.SelectItemList.Count > 0)
    {
      ...
      renseiSelectMenu.SelectItemList.Clear();
      // ISSUE: reference to a compiler-generated method
      System.Action<InventoryItem> action = new System.Action<InventoryItem>(renseiSelectMenu.\u003CInitExtension\u003Eb__38_6);
      list.ForEach(action);
    }
    ...
    renseiSelectMenu.removeReisouCallback = new System.Action(renseiSelectMenu.cbRemoveReisou);
...
label_6:
  ...
  private IEnumerator ShowMaterialsPopup(InventoryItem item)
  {
    ...
    e = (IEnumerator) null;
    // ISSUE: reference to a compiler-generated method
    renseiSelectMenu.materialPopup.SetOnOKClick(new System.Action<InventoryItem>(renseiSelectMenu.\u003CShowMaterialsPopup\u003Eb__51_0));
  }
  ...
  private void AutoRensei()
  {
    ...
    if (this.BaseItem.playerItem != (PlayerItem) null && !this.BaseItem.isEquipReisouLvMax)
    {
      ...
      {
        List<InventoryItem> list = this.InventoryItems.Where<InventoryItem>((Func<InventoryItem, bool>) (x => this.DisplayItems.Contains(x) && x.Item.gear.isHolyReisou() && (!this.equipedReisouIdList.Contains(x.Item.itemID) && ((IEnumerable<int>) this.reisouRenseiMasterID).Contains<int>(x.Item.masterID)) && x.Item.gearLevel == 1 && x.Item.gear.rarity.index == 1)).OrderBy<InventoryItem, int>((Func<InventoryItem, int>) (x => x.Item.masterID)).ThenByDescending<InventoryItem, int>((Func<InventoryItem, int>) (x => x.Item.itemID)).ToList<InventoryItem>();
        int num3 = ((IEnumerable<ReisouRankExp>) MasterData.ReisouRankExpList).FirstOrDefault<ReisouRankExp>((Func<ReisouRankExp, bool>) (x => x.ID == this.BaseItem.playerItem.gear_level_limit)).from_exp - this.BaseItem.playerItem.gear_total_exp;
        for (int index = 0; index < list.Count && this.SelectItemList.Count < 99; ++index)
        {
