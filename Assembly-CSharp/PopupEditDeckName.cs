...
public class PopupEditDeckName : BackButtonPopupBase
{
  ...
  private System.Action<string> onSetName_;
  private System.Action onClose_;
  private const int MAX_DECKNAME = 10;

  public static Future<GameObject> createPrefabLoader() => new ResourceObject("Prefabs/quest002_8/popup_TeamName_Edit").Load<GameObject>();

  public static GameObject show(
    GameObject prefab,
    string text,
    int maxLength,
    System.Action<string> eventSet,
  ...
  private void initialize(string text, int maxLength, System.Action<string> eventSet, System.Action eventClose)
  {
    ...
    this.input_.onValidate = new UIInput.OnValidate(this.onValidate);
  }
  ...
  private void initialize(UILabel uiLabel, PlayerCustomDeck target)
  {
    ...
    this.initialize(target.name, 10, new System.Action<string>(this.defSet), new System.Action(this.defClose));
  }

  private char onValidate(string text, int charIndex, char addedChar) => addedChar.IsInvalidInput() ? char.MinValue : addedChar;
  ...
  private void defSet(string text) => this.StartCoroutine(PopupEditDeckName.doSaveDeckName(this.label_, this.target_, text));
