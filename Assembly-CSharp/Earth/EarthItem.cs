...
namespace Earth
{
  ...
  public class EarthItem : BL.ModelBase
  {
    ...
    public static EarthItem Create(int itemID, int quantity, int boxID = 1) => new EarthItem()
    {
      mItemID = itemID,
      mQuantity = quantity,
      mBattleSetCount = 0
    ...
    public List<PlayerItem> GetPlayerItemList()
    {
      ...
      int a = this.mQuantity - this.mBattleSetCount;
      if (a > 0)
      {
        ...
        foreach (PlayerItem playerItem in (IEnumerable<PlayerItem>) this.playerItems.Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.box_type_id == 1)).OrderByDescending<PlayerItem, int>((Func<PlayerItem, int>) (x => x.quantity)))
        {
          if (a > 0)
          {
            playerItem.quantity = Mathf.Min(a, 99);
            a -= playerItem.quantity;
          }
          else
    ...
    public string GetSeverString() => string.Format(EarthItem.serverDataFormat, (object) this.mItemID, (object) this.mQuantity, (object) this.mBattleSetCount);

    public static EarthItem JsonLoad(Dictionary<string, object> json) => new EarthItem()
    {
