...
namespace Earth
{
  public class EarthDataManager : Singleton<EarthDataManager>
  {
    ...
    public static void CreateInstance() => Singleton<NGGameDataManager>.GetInstance().gameObject.AddComponent<EarthDataManager>().mInit = false;

    public static void DestoryInstance() => UnityEngine.Object.Destroy((UnityEngine.Object) Singleton<EarthDataManager>.GetInstanceOrNull());
      ...
      System.Action<WebAPI.Response.UserError> userErrorCallback = null)
    {
      if (!this.mInit)
      {
        ...
        {
          rq = WebAPI.ZeroLoad((System.Action<WebAPI.Response.UserError>) (error =>
          {
    ...
    public static void SetZeroLoad(Future<WebAPI.Response.ZeroLoad> zero) => EarthDataManager.zeroLoad = zero;
    ...
    private EarthQuestProgress GetQuestProgress(Dictionary<string, object> json) => EarthQuestProgress.JsonLoad((Dictionary<string, object>) json["quest_progress"], (System.Action) (() => {}));
    ...
    private void JsonLoad(Dictionary<string, object> json)
    {
      ...
      foreach (Dictionary<string, object> json1 in (List<object>) json["gears"])
      {
        EarthGear earthGear = EarthGear.JsonLoad(json1);
        this.mGears.Add(earthGear.ID, earthGear);
      }
      foreach (Dictionary<string, object> json1 in (List<object>) json["supplies"])
      {
        EarthItem earthItem = EarthItem.JsonLoad(json1);
        this.mItems.Add(earthItem.ID, earthItem);
      }
      foreach (Dictionary<string, object> json1 in (List<object>) json["character_intimates"])
      {
        EarthCharacterIntimate characterIntimate = EarthCharacterIntimate.JsonLoad(json1);
        this.mCharacterIntimates.Add(characterIntimate.key, characterIntimate);
      }
      if (json.ContainsKey("quest_keys"))
      {
        foreach (Dictionary<string, object> json1 in (List<object>) json["quest_keys"])
        {
          EarthQuestKey earthQuestKey = EarthQuestKey.JsonLoad(json1);
          this.mQuestKeys.Add(earthQuestKey.keyID, earthQuestKey);
        }
      }
      Dictionary<string, object> json2 = (Dictionary<string, object>) json["quest_progress"];
      bool isCallClearCharacterBattleIndex = false;
      this.mQuestProgress = EarthQuestProgress.JsonLoad(json2, (System.Action) (() => isCallClearCharacterBattleIndex = true));
      this.mUserProperties = new BL.ClassValue<Dictionary<MasterDataTable.CommonRewardType, long>>(new Dictionary<MasterDataTable.CommonRewardType, long>());
      ...
      {
        foreach (Dictionary<string, object> json1 in (List<object>) json["key_quest_play_data"])
        {
          EarthQuestKeyPlayData questKeyPlayData = EarthQuestKeyPlayData.JsonLoad(json1);
    ...
    public EarthQuestKey[] GetQuestKeys() => this.mQuestKeys.Values.Where<EarthQuestKey>((Func<EarthQuestKey, bool>) (x => x != null)).OrderBy<EarthQuestKey, int>((Func<EarthQuestKey, int>) (x => x.ID)).ToArray<EarthQuestKey>();

    public PlayerUnit[] GetPlayerUnits() => this.mCharacters.Values.Where<EarthCharacter>((Func<EarthCharacter, bool>) (x => !x.isFall && !x.isDesert)).OrderBy<EarthCharacter, int>((Func<EarthCharacter, int>) (x => x.index)).Select<EarthCharacter, PlayerUnit>((Func<EarthCharacter, PlayerUnit>) (x => x.GetPlayerUnit())).Where<PlayerUnit>((Func<PlayerUnit, bool>) (x => x != (PlayerUnit) null)).ToArray<PlayerUnit>();

    public PlayerUnit[] GetEnableSortiePlayerUnits() => this.mCharacters.Values.Where<EarthCharacter>((Func<EarthCharacter, bool>) (x => !x.isFall && !x.isDesert && !this.questProgress.isImpossibleOfSortie(x.character.ID) && !x.unit.IsMaterialUnit)).OrderBy<EarthCharacter, int>((Func<EarthCharacter, int>) (x => x.index)).Select<EarthCharacter, PlayerUnit>((Func<EarthCharacter, PlayerUnit>) (x => x.GetPlayerUnit())).Where<PlayerUnit>((Func<PlayerUnit, bool>) (x => x != (PlayerUnit) null)).ToArray<PlayerUnit>();

    public PlayerUnit GetPlayerUnit(int id) => this.mCharacters.ContainsKey(id) ? this.mCharacters[id].GetPlayerUnit() : (PlayerUnit) null;

    public int SetCharacterIndex(int playerunit_id, int index) => index > 1 && this.mCharacters.ContainsKey(playerunit_id) ? this.mCharacters[playerunit_id].SetIndex(index) : -1;

    public int GetEventQuestDeckCharacterCount() => ((IEnumerable<EarthCharacter>) this.characters).Count<EarthCharacter>((Func<EarthCharacter, bool>) (x => x.eventQuestBattleIndex > 0 && !x.isFall));

    public void ForceDeselectEventQuestBattleIndex() => ((IEnumerable<EarthCharacter>) this.characters).ForEach<EarthCharacter>((System.Action<EarthCharacter>) (x => x.SetEventQuestBattleIndex(0)));
    ...
    public void ClearCharacterBattleIndex()
    {
      ...
      ((IEnumerable<EarthCharacter>) this.characters).ForEach<EarthCharacter>((System.Action<EarthCharacter>) (x => x.SetBattleIndex(0)));
      ...
      ((IEnumerable<EarthCharacter>) this.characters).OrderBy<EarthCharacter, int>((Func<EarthCharacter, int>) (x => x.index)).ForEach<EarthCharacter>((System.Action<EarthCharacter>) (x =>
      {
    ...
    public int SetCharacterBattleIndex(int playerunit_id, int index) => index > 1 && this.mCharacters.ContainsKey(playerunit_id) ? this.mCharacters[playerunit_id].SetBattleIndex(index) : -1;
    ...
    public PlayerUnit[] GetEvolutionPattern(int playerUnitID, int[] patternIDs) => this.mCharacters.ContainsKey(playerUnitID) ? this.mCharacters[playerUnitID].GetEvolutionPlayerUnits(patternIDs).ToArray() : (PlayerUnit[]) null;
      ...
      List<int> materailUnitIDs)
    {
      if (this.mCharacters.ContainsKey(playerUnitID))
      {
        ...
        materailUnitIDs.ForEach((System.Action<int>) (x => this.mCharacters[x].SetDeath()));
    ...
    public PlayerItem[] GetPlayerItems() => this.mItems.Values.SelectMany<EarthItem, PlayerItem>((Func<EarthItem, IEnumerable<PlayerItem>>) (x => (IEnumerable<PlayerItem>) x.GetPlayerItemList())).Concat<PlayerItem>(this.mGears.Values.Where<EarthGear>((Func<EarthGear, bool>) (x => !x.isLost)).Select<EarthGear, PlayerItem>((Func<EarthGear, PlayerItem>) (x => x.GetPlayerItem()))).ToArray<PlayerItem>();

    public PlayerCharacterIntimate[] GetPlayerCharacterIntimates() => this.mCharacterIntimates.Values.Select<EarthCharacterIntimate, PlayerCharacterIntimate>((Func<EarthCharacterIntimate, PlayerCharacterIntimate>) (x => x.GetPlayerCharacterIntimate())).ToArray<PlayerCharacterIntimate>();
    ...
    public int GetShopPurchaseCount(int id) => this.mShopPurchaseHistory.ContainsKey(id) ? this.mShopPurchaseHistory[id] : 0;

    public bool IsKeyQuestOpen(int keyId) => this.mKeyQuestOpen.ContainsKey(keyId) && this.mKeyQuestOpen[keyId].Open;

    public int GetKeyQuestPlayCount(int keyId)
    {
      int num = 0;
      if (this.mKeyQuestOpen.ContainsKey(keyId))
        num = this.mKeyQuestOpen[keyId].PlayCount;
      return num;
    }
      ...
      bool isRetreatEnable)
    {
      ...
      if (instance.getCloudAnimEnabled())
      {
        ...
        instance.StartCloudAnimEnd((System.Action) (() => Singleton<NGBattleManager>.GetInstance().startBattle(info)));
      }
      else
    ...
    public Future<BattleEnd> BattleFinish(WebAPI.Request.BattleFinish request, BE be)
    {
      ...
      this.mUserProperties.commit();
      foreach (WebAPI.Request.BattleFinish.UnitResult unit1 in request.units)
      {
        WebAPI.Request.BattleFinish.UnitResult unit = unit1;
      ...
      foreach (EarthDesertCharacter earthDesertCharacter in ((IEnumerable<EarthDesertCharacter>) MasterData.EarthDesertCharacterList).Where<EarthDesertCharacter>((Func<EarthDesertCharacter, bool>) (x =>
      {
        if (x.desert_logic == EarthJoinLogicType.quest_clear)
        {
          ...
          {
            int num2 = (int) result;
            return be.core.battleInfo.quest_type != CommonQuestType.Earth ? num2 == be.core.battleInfo.quest_s_id : num2 == this.questProgress.currentEpisode.ID;
          }
        }
        return false;
      })))
    ...
    public List<string> createResourceLoadList()
    {
      ...
      foreach (EarthCharacter earthCharacter in this.mCharacters.Values)
        stringList.AddRange((IEnumerable<string>) instance.PathsFromUnit(earthCharacter.unit));
      return stringList;
    }
    ...
    public IEnumerator DownloadEarthBGM()
    {
      ...
      IEnumerator e = OnDemandDownload.waitLoadSomethingResource((IEnumerable<string>) stringSet, false);
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
    }
