...
public class Quest0528MenuMapChipNum : MonoBehaviour
{
  ...
  {
    Quest0528MenuMapChipNum.Digit digit = Quest0528MenuMapChipNum.Digit.One;
    ...
    if (number == 0)
    {
      ...
      this.oneDigitSprite.gameObject.SetActive(false);
      this.tensDigitSprite[0].gameObject.SetActive(false);
      this.tensDigitSprite[1].gameObject.SetActive(false);
    }
    else
    {
      if (number > 9)
      {
        digit = Quest0528MenuMapChipNum.Digit.Tens;
        ((IEnumerable<UISprite>) this.tensDigitSprite).ForEach<UISprite>((System.Action<UISprite>) (x => x.gameObject.SetActive(true)));
        ...
        this.tensDigitSprite[0].SetDimensions(atlasSprite1.width, atlasSprite1.height);
        ...
        this.tensDigitSprite[1].SetDimensions(atlasSprite2.width, atlasSprite2.height);
      }
      else
      {
        this.oneDigitSprite.gameObject.SetActive(true);
        ...
        this.oneDigitSprite.SetDimensions(atlasSprite.width, atlasSprite.height);
      }
      ((IEnumerable<GameObject>) this.dirDigit).ToggleOnce((int) digit);
    }
  }
  ...
  public void StartSelectAnim()
  {
    ...
    ((IEnumerable<UITweener>) this.selectObject[index].GetComponents<UITweener>()).ForEach<UITweener>((System.Action<UITweener>) (x =>
    {
      x.ResetToBeginning();
      x.PlayForward();
    }));
