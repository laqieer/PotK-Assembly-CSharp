...
public class Raid032BattleMenu : BackButtonMenuBase, IBattlePreparationPopup
{
  ...
  {
    if (!this.isCustomDeckMode || Util.checkUnlockedPlayerLevel(Player.Current.level) && ((IEnumerable<PlayerCustomDeck>) SMManager.Get<PlayerCustomDeck[]>()).Any<PlayerCustomDeck>((Func<PlayerCustomDeck, bool>) (x => ((IEnumerable<int>) x.player_unit_ids).Any<int>((Func<int, bool>) (i => (uint) i > 0U)))))
      return;
  ...
  private IEnumerator AutoDeckEditAsync()
  {
    ...
    if (bok)
    {
      ...
      Creator deckCreator = new Creator((PlayerUnit[]) null, array, (List<Filter>) null, 1, 5, SMManager.Get<Player>().max_cost);
  ...
  private IEnumerator sortieBattle(DeckInfo deck)
  {
    ...
    // ISSUE: explicit non-virtual call
    // ISSUE: explicit non-virtual call
    Future<WebAPI.Response.GuildraidBattleStart> ft = WebAPI.GuildraidBattleStart(deck.deck_number, deck.deck_type_id, is_simulation, raid032BattleMenu.loop_count, playerUnitIds, raid032BattleMenu.mMasterData.ID, support_player_id, __nonvirtual (raid032BattleMenu.GetPopupFriend()) != (PlayerUnit) null ? __nonvirtual (raid032BattleMenu.GetPopupFriend()).id : 0, new System.Action<WebAPI.Response.UserError>(raid032BattleMenu.webErrorCallback));
    IEnumerator e = ft.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
