...
public static class InventoryItemExtension
{
  ...
  public static InventoryItem FindByItem(
    this List<InventoryItem> invItem,
    GameCore.ItemInfo findItem)
  {
    return invItem.FirstOrDefault<InventoryItem>((Func<InventoryItem, bool>) (x => x.Item != null && x.Item.itemID == findItem.itemID));
  }

  public static InventoryItem FindByItemIndex(
    this List<InventoryItem> invItem,
    GameCore.ItemInfo findItem)
  {
  ...
  public static List<int> ToGearId(this List<InventoryItem> xs)
  {
    ...
    foreach (InventoryItem inventoryItem in xs.Where<InventoryItem>((Func<InventoryItem, bool>) (x => x.Item.itemType == GameCore.ItemInfo.ItemType.Gear)))
      intList.Add(inventoryItem.Item.itemID);
    return intList;
  }
  ...
  public static List<int> ToMaterialCounts(this List<InventoryItem> xs)
  {
    ...
    List<int> intList = new List<int>();
    foreach (InventoryItem inventoryItem in inventoryItems)
      intList.Add(inventoryItem.Item.tempSelectedCount);
    return intList;
  }
  ...
  public static List<int> ToNotSupplyID(this List<InventoryItem> xs)
  {
    ...
    foreach (InventoryItem inventoryItem in xs.Where<InventoryItem>((Func<InventoryItem, bool>) (x => x.Item.itemType == GameCore.ItemInfo.ItemType.Compse || x.Item.itemType == GameCore.ItemInfo.ItemType.Exchangable)))
    {
      ...
      List<PlayerItem> list = ((IEnumerable<PlayerItem>) playerItemArray).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.gear != null && x.gear.ID == s.Item.masterID)).ToList<PlayerItem>();
  ...
  public static List<int> ToEntityIdBySupply(this List<InventoryItem> xs)
  {
    ...
    List<int> intList = new List<int>();
    foreach (InventoryItem inventoryItem in inventoryItems)
      intList.Add(inventoryItem.Item.masterID);
    return intList;
  }
  ...
  public static List<int> ToSelectQuantityBySupply(this List<InventoryItem> xs)
  {
    ...
    List<int> intList = new List<int>();
    foreach (InventoryItem inventoryItem in inventoryItems)
      intList.Add(inventoryItem.selectCount);
    return intList;
  }
  ...
  public static List<long> ToSelectQuantityByMaterial(this List<InventoryItem> xs)
  {
    ...
    foreach (InventoryItem inventoryItem in inventoryItems)
      longList.Add((long) inventoryItem.selectCount);
    return longList;
  }

  private static int GetSortRank(GameCore.ItemInfo item) => !item.gear.kind.isEquip || item.gear.disappearance_type_GearDisappearanceType != 0 ? item.gear.kind_GearKind : 0;
    ...
    bool[] filters)
  {
    ...
    {
      InventoryItem inventoryItem2 = inventoryItemList1[index];
      if (InventoryItemExtension.CheckWeaponType(inventoryItem2, filters) && InventoryItemExtension.CheckRarity(inventoryItem2, filters) && (InventoryItemExtension.CheckFavorite(inventoryItem2, filters) && InventoryItemExtension.CheckLevel(inventoryItem2, filters)) && (InventoryItemExtension.CheckEquip(inventoryItem2, filters) && InventoryItemExtension.CheckIsReisou(inventoryItem2, filters) && InventoryItemExtension.CheckAttackType(inventoryItem2, filters)) && (!filters[24] && !filters[25] && (!filters[9] && !filters[23]) && (!filters[28] && !filters[30] && !filters[40]) || (InventoryItemExtension.CheckWeapon(inventoryItem2, filters) || InventoryItemExtension.CheckSpecialWeapon(inventoryItem2, filters) || (InventoryItemExtension.CheckWeaponSmith(inventoryItem2, filters) || InventoryItemExtension.CheckAlchemist(inventoryItem2, filters)) || (InventoryItemExtension.CheckMoney(inventoryItem2, filters) || InventoryItemExtension.CheckWeaponMaterial(inventoryItem2, filters) || InventoryItemExtension.CheckPresent(inventoryItem2, filters)))))
        inventoryItemList2.Add(inventoryItem2);
    }
    if (inventoryItem1 != null)
      inventoryItemList2.Insert(0, inventoryItem1);
    return (IEnumerable<InventoryItem>) inventoryItemList2;
  ...
  private static bool CheckAlchemist(InventoryItem item, bool[] filters) => item.Item.isDrilling && filters[28];

  private static bool CheckWeaponSmith(InventoryItem item, bool[] filters) => item.Item.gear != null && item.Item.gear.kind.Enum == GearKindEnum.smith && !item.Item.isExchangable && filters[9];
  ...
  private static bool CheckDrilled(InventoryItem item, bool[] filters) => filters[29] || item.Item.gear == null || item.Item.gearExp < 1 && item.Item.gearLevel <= 1;

  private static bool CheckWeaponMaterial(InventoryItem item, bool[] filters) => item.Item.isWeaponMaterial && filters[30];
  ...
  private static bool CheckPresent(InventoryItem item, bool[] filters) => item.Item.gear != null && item.Item.gear.kind.Enum == GearKindEnum.sea_present && filters[40];
    ...
    bool[] filters)
  {
    ...
    {
      InventoryItem inventoryItem2 = inventoryItemList1[index];
      if (InventoryItemExtension.CheckReisouWeaponType(inventoryItem2, filters) && InventoryItemExtension.CheckReisouRarity(inventoryItem2, filters) && (InventoryItemExtension.CheckReisouEquip(inventoryItem2, filters) && InventoryItemExtension.CheckReisouDrilled(inventoryItem2, filters)) && InventoryItemExtension.CheckReisouType(inventoryItem2, filters))
        inventoryItemList2.Add(inventoryItem2);
    }
    if (inventoryItem1 != null)
      inventoryItemList2.Insert(0, inventoryItem1);
