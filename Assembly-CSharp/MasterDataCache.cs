...
public static class MasterDataCache
{
  ...
  private static IEnumerable<KeyValuePair<K, T>> AssocListSelect<K, T>(
    T[] list,
    Func<T, K> getID)
  {
    T[] objArray = list;
  ...
  public static T[] GetList<K, T>(string key, Func<MasterDataReader, T> create, Func<T, K> getID) => MasterDataCache.GetOrCreate<K, T>(key, create, getID)?.List;
  ...
  private static IEnumerable<KeyValuePair<M, T[]>> Select<M, T>(
    Dictionary<M, List<T>> d)
  {
