...
public class GearStatusUpParam : MonoBehaviour
{
    ...
    GearKindEnum gearkindEnum)
  {
    ...
    gearMaterialData.Where<GameCore.ItemInfo>((Func<GameCore.ItemInfo, bool>) (x => x.gear.kind.Enum == gearkindEnum)).ToList<GameCore.ItemInfo>().ForEach((System.Action<GameCore.ItemInfo>) (x => gearMaterialParamRate += max ? 0 : logic.MaterialRank(x.gearLevel)));
  ...
  private void SetGaugeScale(GameObject obj, float val) => obj.transform.localScale = new Vector3(val, 1f, 1f);
  ...
  private void SetUpValue(int value)
  {
    if (value == 0)
    {
      this.lblUpVal.transform.parent.gameObject.SetActive(false);
    }
    else
    {
      this.lblUpVal.transform.parent.gameObject.SetActive(true);
