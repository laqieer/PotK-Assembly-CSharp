...
public class Unit004JobChangeMenu : BackButtonMenuBase, IDetailMenuContainer
{
  ...
  public IEnumerator initializeAsync(SceneParam param)
  {
    ...
    else
    {
      ...
      {
        Future<WebAPI.Response.UnitPreviewJob> dpd = WebAPI.UnitPreviewJob(unit004JobChangeMenu.entityUnit_.id, (System.Action<WebAPI.Response.UserError>) (error =>
        {
  ...
  private void reset3DModelAmbientLightColor() => RenderSettings.ambientLight = Consts.GetInstance().UI3DMODEL_AMBIENT_COLOR;
    ...
    for (int i = 0; i < this.changeUnits_.Length; ++i)
    {
      ...
      if (this.setupCursor(i))
      {
        ...
        if (!(p == (PlayerUnit) null))
        {
          ...
          UnityEngine.Sprite jobFrameSprite = this.getJobFrameSprite(p.job_id, isCurrent, isDefaultJob, isShort, isJobX);
  ...
  private EventDelegate.Callback createMethodSelectedJob(int index) => (EventDelegate.Callback) (() => this.onSelectedJob(index));

  private bool setupCursor(int index)
  {
    Unit004JobChangeMenu.CursorType cursorType = this.cursorTypes_[index];
    cursorType.Long?.gameObject.SetActive(false);
    cursorType.Short?.gameObject.SetActive(false);
    ...
    bool isJobX)
  {
    ...
    {
      Future<UnityEngine.Sprite> ldFrame = this.LoadSpriteJobFrame(bCurrent, isDefault, isShort, isJobX);
      yield return (object) ldFrame.Wait();
      this.dicJobSelector_[key] = ldFrame.Result;
    }
  }

  private UnityEngine.Sprite getJobFrameSprite(
    int jobid,
    bool bCurrent,
    bool isDefault,
    bool isShort,
  ...
  public Future<UnityEngine.Sprite> LoadSpriteJobFrame(
    bool isCurrent,
    bool isDefault,
    bool isShort,
    bool isJobX)
  ...
  private void setEventSelectedJob(UIButton button, int index) => EventDelegate.Set(button.onClick, (EventDelegate.Callback) (() => this.onSelectedJob(index)));
  ...
  private IEnumerator initDetails()
  {
    ...
    if (!((UnityEngine.Object) unit004JobChangeMenu.lnkDetail_ == (UnityEngine.Object) null))
    {
      ...
      for (int n = 0; n < unit004JobChangeMenu.changeUnits_.Length; ++n)
      {
        ...
        if (n != unit004JobChangeMenu.entityCursorIndex_ && unit004JobChangeMenu.changeUnits_[n] == (PlayerUnit) null)
        {
        ...
        else
        {
          ...
          IEnumerator e;
          if (n == unit004JobChangeMenu.entityCursorIndex_)
          {
            unit004JobChangeMenu.details_[n].normal.updatedJobAbility = new System.Action(unit004JobChangeMenu.updatedJobAbility);
            e = unit004JobChangeMenu.details_[n].initAsyncDiffMode(unit004JobChangeMenu.entityUnit_, (PlayerUnit) null, 2, (IDetailMenuContainer) unit004JobChangeMenu);
            while (e.MoveNext())
          ...
          {
            e = unit004JobChangeMenu.details_[n].initAsyncDiffMode(unit004JobChangeMenu.changeUnits_[n], unit004JobChangeMenu.entityUnit_, 2, (IDetailMenuContainer) unit004JobChangeMenu);
            while (e.MoveNext())
              yield return e.Current;
            e = (IEnumerator) null;
          }
          unit004JobChangeMenu.details_[n].gameObject.SetActive(n == unit004JobChangeMenu.firstIndex_);
        }
      }
    }
  }
  ...
  public bool isCustomPushAndSet() => this.isInitialize_ || this.isBlockTouch || this.IsPushAndSet();

  private bool isBlockTouch => this.isLoading3DModel_ || Singleton<CommonRoot>.GetInstance().isLoading;
  ...
  private IEnumerator doCheckJobChange()
  {
    ...
    confirmPopupArray1[1] = new Unit004JobChangeMenu.ConfirmPopup()
    {
      ...
      desc_ = string.Format(instance.JOB_CHANGE_X_DESC, (object) afterUnit.getJobData().name, (object) playerUnit != null ? (object) playerUnit.getJobData().name : (object) (string) null)
    };
