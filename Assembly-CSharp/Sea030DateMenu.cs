...
public class Sea030DateMenu : BackButtonMenuBase
{
    ...
    SeaHomeTimeZone _timeZone)
  {
    ...
    IEnumerator e1;
    if ((UnityEngine.Object) sea030DateMenu.dateConfirmPopupPrefab == (UnityEngine.Object) null)
    {
    ...
    List<SwitchUnitComponentBase> list = ((IEnumerable<SwitchUnitComponentBase>) sea030DateMenu.GetComponentsInChildren<SwitchUnitComponentBase>(true)).ToList<SwitchUnitComponentBase>();
    for (int index = 0; index < list.Count; ++index)
      list[index].SwitchMaterial(_current2DUnitData.UnitID);
    if ((bool) (UnityEngine.Object) sea030DateMenu.onedariIcon)
      sea030DateMenu.onedariIcon.gameObject.SetActive(false);
    if ((bool) (UnityEngine.Object) sea030DateMenu.aodateIcon)
    ...
    if (PerformanceConfig.GetInstance().IsOnedariDate)
    {
      ...
      {
        Future<WebAPI.Response.SeaCallDateCondition> future = WebAPI.SeaCallDateCondition(_current2DUnitData.Unit.same_character_id, (System.Action<WebAPI.Response.UserError>) (e =>
        {
        ...
        if (future.Result.conditions.condition_type_id.HasValue)
        {
          switch ((TalkMessageConditonType) future.Result.conditions.condition_type_id.Value)
          {
            case TalkMessageConditonType.GoDate:
              ...
              sea030DateMenu.SetOnedariIcon(sea030DateMenu.BtnSpots[spotDisplaySetting1.datespot.ID - 1].transform);
              break;
            ...
            case TalkMessageConditonType.GoDateRed:
              IEnumerable<SeaDateHitExpansionLottery> source = ((IEnumerable<SeaDateHitExpansionLottery>) MasterData.SeaDateHitExpansionLotteryList).Where<SeaDateHitExpansionLottery>((Func<SeaDateHitExpansionLottery, bool>) (x => x.time_zone_id_SeaHomeTimeZone != 0 && (uint) x.date_spot_id_SeaDateDateSpot > 0U));
              ...
              {
                sea030DateMenu.SetOnedariIcon(sea030DateMenu.BtnSpots[spotDisplaySetting2.datespot.ID - 1].transform);
                sea030DateMenu.onedariIcon.gameObject.SetActive(seaDateHit.time_zone_id.WithIn(_now));
                break;
              }
              if ((bool) (UnityEngine.Object) sea030DateMenu.onedariIcon)
              {
  ...
  {
    if (!(bool) (UnityEngine.Object) this.onedariIcon)
      return;
    this.onedariIcon.gameObject.SetActive(true);
    this.onedariIcon.SetParent(tran);
    this.onedariIcon.localPosition = (Vector3) new Vector2(0.0f, 56f);
  }
  ...
  private void OpenDateConfirmPopup(SeaDateDateSpotDisplaySetting setting) => Singleton<PopupManager>.GetInstance().open(this.dateConfirmPopupPrefab).GetComponent<Sea030HomeDateConfirmPopup>().Init(this.current2DUnitData.PlayerUnit, setting, new System.Action<SeaDateDateSpotDisplaySetting>(this.DepartForDate));
