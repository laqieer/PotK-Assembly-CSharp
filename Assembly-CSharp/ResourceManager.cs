...
public class ResourceManager : Singleton<ResourceManager>
{
  ...
  private SimpleCache<string, UnityEngine.Object> cache;
...
label_2:
      ...
      if (dlc.Error != null)
      {
        ...
        bool waitRetry = true;
        ModalWindow.ShowRetryTitle(Consts.GetInstance().dlc_fail_download_title, dlc.Error, (System.Action) (() => waitRetry = false), (System.Action) (() => StartScript.Restart()));
        while (waitRetry)
  ...
  public static string DLCVersion
  {
    get
    {
      ...
      return !((UnityEngine.Object) instance == (UnityEngine.Object) null) ? instance.resourceVersion : "";
    }
  ...
  public IEnumerator SaveResourceInfo(byte[] buf)
  {
    ...
    try
    {
      ...
      {
        using (ZlibUtilStream zlibUtilStream = ZlibUtilStream.DecompressBytes(buf, 0, buf.Length, ZlibFormat.Gzip))
        {
          while (true)
          {
  ...
  private SimpleCache<string, UnityEngine.Object> Cache
  {
    ...
    {
      if (this.cache == null)
        this.cache = new SimpleCache<string, UnityEngine.Object>((Func<string, Promise<UnityEngine.Object>, IEnumerator>) ((key, promise) => this.InternalLoad(key, promise)), (Func<UnityEngine.Object, long>) (_ => 1L), 10L);
      return this.cache;
    }
  }

  private IEnumerator SetStep(UnityEngine.Object res, ResourceInfo.Step step)
  {
    ...
    if ((UnityEngine.Object) go != (UnityEngine.Object) null)
    {
      ...
      if (type == ResourceInfo.StepsType.UISprite)
      {
        ...
        UISprite sprite = go.transform.FindByFullPath(path).GetComponent<UISprite>();
      ...
      if (type == ResourceInfo.StepsType.UIAtlas)
      {
        ...
        while (e.MoveNext())
          yield return e.Current;
        e = (IEnumerator) null;
        go.transform.FindByFullPath(transformPath).GetComponent<UIAtlas>().spriteMaterial.SetTexture("_MainTex", (Texture) future.Result);
      ...
      if (type == ResourceInfo.StepsType.UILable)
      {
        transformPath = step._path;
        string transform = step._transform;
        Future<UnityEngine.Font> future = Singleton<ResourceManager>.GetInstance().Load<UnityEngine.Font>(transform);
        e = future.Wait();
        while (e.MoveNext())
          yield return e.Current;
        e = (IEnumerator) null;
        go.transform.FindByFullPath(transformPath).GetComponent<UILabel>().trueTypeFont = future.Result;
        transformPath = (string) null;
        future = (Future<UnityEngine.Font>) null;
      }
      if (type == ResourceInfo.StepsType.Renderer)
      {
        Renderer renderer = go.transform.FindByFullPath(step._path).GetComponent<Renderer>();
        if (step._sharedMaterials != null)
        {
          ...
          for (int i = 0; i < num; ++i)
          {
            ...
            {
              transformPath = key;
              Future<UnityEngine.Object> future = Singleton<ResourceManager>.GetInstance().Load<UnityEngine.Object>(step.GetSharedMaterials(i, transformPath));
              e = future.Wait();
              while (e.MoveNext())
                yield return e.Current;
              e = (IEnumerator) null;
              Texture2D result1 = future.Result as Texture2D;
              if ((UnityEngine.Object) result1 != (UnityEngine.Object) null)
              {
                result1.filterMode = step.GetSharedMaterialFilterMode(i, transformPath);
                result1.wrapMode = step.GetSharedMaterialTextureWrapMode(i, transformPath);
                renderer.sharedMaterials[i].SetTexture(transformPath, (Texture) result1);
              }
              else
              {
                UnityEngine.Font result2 = future.Result as UnityEngine.Font;
                if ((UnityEngine.Object) result2 != (UnityEngine.Object) null)
                {
                  renderer.gameObject.GetComponent<TextMesh>().font = result2;
                  renderer.sharedMaterial = result2.material;
                  renderer.sharedMaterials[i] = result2.material;
                }
                else
                {
                  future = (Future<UnityEngine.Object>) null;
                  transformPath = (string) null;
                }
              }
            }
  ...
  public IEnumerator SetSteps(string path, UnityEngine.Object obj)
  {
  ...
  private IEnumerator InternalLoad(string path, Promise<UnityEngine.Object> promise)
  {
    ...
    if (string.IsNullOrEmpty(info._key))
    {
      ...
      promise.Result = (UnityEngine.Object) null;
    }
    else
    {
      IEnumerator e;
      ...
      Future<UnityEngine.Object> future = this.loaderDic[info._value._path_type].Load(path, ref info);
      e = future.Wait();
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      if (!future.HasResult || future.Result == (UnityEngine.Object) null)
      {
  ...
  public bool Contains(string path) => !string.IsNullOrEmpty(this.resourceInfo.SearchResourceInfo(path)._key);

  public Future<T> LoadOrNull<T>(string path) where T : UnityEngine.Object => !Singleton<ResourceManager>.GetInstance().Contains(path) ? Future.Single<T>(default (T)) : Singleton<ResourceManager>.GetInstance().Load<T>(path);

  public Future<T> Load<T>(string path, float pixelsPerUnitForSprite = 1f) where T : UnityEngine.Object => typeof (T) != typeof (UnityEngine.Sprite) ? this.Cache.Get(path).Then<T>((Func<UnityEngine.Object, T>) (obj => (T) obj)) : this.Cache.Get(path).Then<T>((Func<UnityEngine.Object, T>) (obj =>
  {
    SpriteMeshType meshType = SpriteMeshType.FullRect;
    uint extrude = 100;
    if (obj == (UnityEngine.Object) null)
      obj = (UnityEngine.Object) Resources.Load<Texture2D>("Sprites/1x1_alpha0");
    Texture2D texture = (Texture2D) obj;
    UnityEngine.Sprite sprite = UnityEngine.Sprite.Create(texture, new Rect(0.0f, 0.0f, (float) texture.width, (float) texture.height), new Vector2(0.5f, 0.5f), pixelsPerUnitForSprite, extrude, meshType);
    sprite.name = texture.name;
    return sprite as T;
  }));

  public Future<T> Load<T>(string path, System.Action<T> callback) where T : UnityEngine.Object
  {
  ...
  public T LoadImmediatelyForSmallObject<T>(string path) where T : UnityEngine.Object
  {
    if (this.resourceInfo == null)
      return default (T);
    UnityEngine.Object @object = this.Cache.TryGet(path);
  ...
  public Future<T> LoadFromDownloadOrCache<T>(string path) where T : UnityEngine.Object
  {
    if (this.resourceInfo == null)
      return (Future<T>) null;
    ResourceInfo.Resource context = this.resourceInfo.SearchResourceInfo(path);
    return string.IsNullOrEmpty(context._key) ? (Future<T>) null : this.loaderDic[context._value._path_type].DownloadOrCache(path, ref context).Then<T>((Func<UnityEngine.Object, T>) (obj => (T) obj));
  }
  ...
  public static bool IsMobVoice(string path) => !ResourceManager.naviVoice.IsMatch(path) && ResourceManager.mobVoice.IsMatch(path);
  ...
  private void InitPaths()
  {
    ...
    System.Action<AssocList<int, List<string>>, int, string> action = (System.Action<AssocList<int, List<string>>, int, string>) ((dic, n, str) =>
    {
