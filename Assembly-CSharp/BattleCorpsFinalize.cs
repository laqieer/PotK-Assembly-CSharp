...
public class BattleCorpsFinalize : BattleMonoBehaviour
{
  protected override IEnumerator Start_Battle()
  {
    ...
    Future<WebAPI.Response.QuestCorpsBattleFinish> api = WebAPI.QuestCorpsBattleFinish(battleCorpsFinalize.env.core.phaseState.turnCount, battleCorpsFinalize.env.core.battleInfo.battleId, string.Empty, 0, ((IEnumerable<BL.Unit>) array).Select<BL.Unit, int>((Func<BL.Unit, int>) (x => x.hp)).ToArray<int>(), ((IEnumerable<BL.Unit>) array).Select<BL.Unit, int>((Func<BL.Unit, int>) (x => x.playerUnit.id)).ToArray<int>(), ((IEnumerable<BL.Unit>) array).Select<BL.Unit, int>((Func<BL.Unit, int>) (x => x.killCount)).ToArray<int>(), source2.Select<BL.Unit, int>((Func<BL.Unit, int>) (x => x.hp)).ToArray<int>(), source2.Select<BL.Unit, int>((Func<BL.Unit, int>) (x => x.hpMax)).ToArray<int>(), source2.Select<BL.Unit, int>((Func<BL.Unit, int>) (x => x.playerUnit.id)).ToArray<int>(), source1.Select<WebAPI.Request.BattleFinish.SupplyResult, int>((Func<WebAPI.Request.BattleFinish.SupplyResult, int>) (x => x.supply_id)).ToArray<int>(), source1.Select<WebAPI.Request.BattleFinish.SupplyResult, int>((Func<WebAPI.Request.BattleFinish.SupplyResult, int>) (x => x.use_quantity)).ToArray<int>(), battleCorpsFinalize.env.core.isWin, (System.Action<WebAPI.Response.UserError>) (e =>
    {
