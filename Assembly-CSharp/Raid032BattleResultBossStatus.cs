...
public class Raid032BattleResultBossStatus : MonoBehaviour
{
    ...
    int hpRatio)
  {
    for (int index = 0; index < this.damageRewardBase.Length; ++index)
    {
      ...
      this.damageRewardAnchors[index].gameObject.SetActive(!this.isEndless);
    }
    for (int i = 0; i < this.damageRewardAnchors.Length; ++i)
    {
      ...
      GameObject gameObject = this.damageRewardAnchors[i].transform.Find("slc_Get").gameObject;
    ...
    int hpRatio)
  {
    ...
    component.Gray = isGotten;
    GameObject gameObject = this.killRewardAnchor.transform.Find("slc_Get").gameObject;
    if ((UnityEngine.Object) gameObject != (UnityEngine.Object) null)
      gameObject.SetActive(isGotten);
  ...
  private void UpdateIfGaugeAnimating()
  {
    if (!this.isGaugeAnimating)
      return;
    this.nowHpLabel.SetTextLocalize((int) ((double) this.hpGauge.transform.localScale.x * (double) this.maxHp));
    if (this.isSimulation)
      return;
    for (int index = 0; index < this.rewardIconEntities.Count; ++index)
    {
      int animatingHpRatio = (int) ((double) this.hpGauge.transform.localScale.x * 10000.0);
