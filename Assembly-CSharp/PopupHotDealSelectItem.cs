...
public class PopupHotDealSelectItem : MonoBehaviour
{
  ...
  private System.Action<HotDealInfo> ButtonCallback;
  ...
  public IEnumerator Initalize(
    ...
    HotDealInfo data,
    System.Action<HotDealInfo> buttonCallback)
  {
    this.Parent = parent;
    ...
    {
      e = iconPrefeb.CloneAndGetComponent<PopupHotDealSelectItemIcon>(this.Grid.transform).Initialize(MasterDataTable.CommonRewardType.coin, -999, additionalPaidCoin);
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      ++iconCnt;
    ...
    for (int index = 0; index < dealRewardSchemaArray.Length; ++index)
    {
      ...
      e = iconPrefeb.CloneAndGetComponent<PopupHotDealSelectItemIcon>(this.Grid.transform).Initialize((MasterDataTable.CommonRewardType) dealRewardSchema.reward_type_id, dealRewardSchema.reward_id, dealRewardSchema.reward_quantity);
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      ++iconCnt;
  ...
  private void UpdateEndingTime()
  {
    ...
    else
    {
      ...
      if (num < 0)
      {
        ...
        this.RestTimeLbl.color = (Color) new Color32((byte) 128, (byte) 128, (byte) 128, byte.MaxValue);
        num = 0;
      }
  ...
  {
    UITweener component = this.Grid.GetComponent<UITweener>();
    if (component.enabled || !this.TweenFlg)
      return;
    this.LeftArrow.gameObject.SetActive(false);
