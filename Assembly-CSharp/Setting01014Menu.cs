...
public class Setting01014Menu : BackButtonMenuBase
{
  ...
  public void Initialize()
  {
    ...
    this.InpId01.GetComponent<UIInput>().defaultText = this.SetTextPlayerComment(player.comment);
    this.InpId01.GetComponent<UIInput>().onValidate = new UIInput.OnValidate(this.onValidate);
    this.InpId01.GetComponent<UIInput>().value = this.SetTextPlayerComment(player.comment);
    this.InpId01.GetComponent<UIInput>().caretColor = Color.black;
  }

  private string SetTextPlayerComment(string comment) => comment == string.Empty ? Consts.GetInstance().FRIEND_COMMENT_DEFAULT : comment;
  ...
  public void onChangeInput()
  {
    if (this.IsPush)
      return;
    this.InpId01.SetTextLocalize(this.InpId01.GetComponent<UIInput>().value);
  }
  ...
  private IEnumerator CommentEdit()
  {
    ...
    Future<WebAPI.Response.PlayerCommentEdit> result = WebAPI.PlayerCommentEdit(newComment, new System.Action<WebAPI.Response.UserError>(setting01014Menu.\u003CCommentEdit\u003Eb__7_0));
  ...
  public void ErrDialog() => this.StartCoroutine(PopupCommon.Show(Consts.GetInstance().SETTING_01014_POPUP_TITLE, Consts.Format(Consts.GetInstance().SETTING_01014_ERROR_MESSAGE)));
}
