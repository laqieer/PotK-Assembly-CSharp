...
public class SimpleScrollContentsPopup : BackButtonMenuBase
{
  ...
  private System.Action mOnCloseCallback;

  public IEnumerator Initialize(System.Action onCloseCallback)
  {
    ...
    for (int index = 0; index < gameObjectArray.Length; ++index)
    {
      ...
      {
        Debug.LogWarning((object) string.Format("content {0} is not SimpleScrollContent", (object) gameObject.name));
      }
      else
      {
  ...
  private void UpdateCloseButton() => this.mCloseButton.isEnabled = this.mCurrentPageIndex == this.mPgaeIndexMax;
