...
public class Popup02688Menu : Popup02610Base
{
  ...
  private void isActiveLeftButton(PvpClassKind c)
  {
    ...
    if (remainingMatches > 0)
    {
      ...
      this.ExpendButton.gameObject.SetActive(false);
      this.isActiveLeftButton(c, remainingMatches, this.ContinueButton);
      this.txtConfirmCheck2.SetText(this.ContinueButton.isEnabled ? Consts.GetInstance().VERSUS_002688POPUP_CONFIRMCHECK2 : Consts.GetInstance().VERSUS_002688POPUP_CONFIRMCHECK3);
    }
    else
    {
      this.ContinueButton.gameObject.SetActive(false);
      this.ExpendButton.gameObject.SetActive(true);
  ...
  private IEnumerator SeasonClose()
  {
    Popup02688Menu popup02688Menu = this;
    Singleton<PopupManager>.GetInstance().onDismiss();
    Future<WebAPI.Response.PvpSeasonClose> f = WebAPI.PvpSeasonClose((System.Action<WebAPI.Response.UserError>) (e =>
    {
  ...
  private IEnumerator CreateClassChange()
  {
    ...
    int num1 = (int) pvpClassKind.ClassCondition(popup02688Menu.pvpInfo.pvp_class_record.current_season_win_count, true);
    int num2 = pvpClassKind.isLowestClass ? 1 : 0;
    component.ChangeSprite((PvpClassKind.Condition) num1, num2 != 0);
  }
}
