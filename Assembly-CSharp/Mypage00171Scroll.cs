...
public class Mypage00171Scroll : MonoBehaviour
{
  ...
  private string GetPresentName(PlayerPresent present) => CommonRewardType.GetRewardName((MasterDataTable.CommonRewardType) present.reward_type_id.Value, present.reward_id.HasValue ? present.reward_id.Value : 0, present.reward_quantity.HasValue ? present.reward_quantity.Value : 0);
    ...
    switch (this.rewardType)
    {
      ...
      case MasterDataTable.CommonRewardType.material_unit:
        ...
        if (MasterData.UnitUnit.TryGetValue(present.reward_id.Value, out unitUnit))
        {
          ...
          ((IEnumerable<GameObject>) this.BtnObject).ForEach<GameObject>((System.Action<GameObject>) (x => x.SetActive(false)));
          break;
        }
        ((IEnumerable<GameObject>) this.BtnObject).ForEach<GameObject>((System.Action<GameObject>) (x => x.SetActive(false)));
        break;
