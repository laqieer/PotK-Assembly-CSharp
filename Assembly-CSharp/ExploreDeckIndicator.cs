...
public class ExploreDeckIndicator : MonoBehaviour
{
  ...
  public IEnumerator InitializeDeckInfo(PlayerUnit[] deckUnits)
  {
    ...
    Future<UnityEngine.Sprite> futureMask = new ResourceObject("GUI/explore_other/slc_unit_mask_Edit").Load<UnityEngine.Sprite>();
    e = futureMask.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    this.mMaskLeaderChara.GetComponent<NGxMaskSpriteWithScale>().maskTexture = futureMask.Result.texture;
  ...
  private IEnumerator createUnitIcon(PlayerUnit unit, int index)
  {
    ...
    {
      iconScript.onLongPress = (System.Action<UnitIconBase>) null;
      iconScript.BreakWeapon = unit.IsBrokenEquippedGear;
      iconScript.SpecialIcon = false;
    }
    else
      iconScript.SetEmpty();
    iconScript.onClick = (System.Action<UnitIconBase>) null;
    iconScript.Button.isEnabled = false;
