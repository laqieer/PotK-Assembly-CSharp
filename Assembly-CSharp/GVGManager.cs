...
public class GVGManager : Singleton<GVGManager>, IGameEngine
{
  ...
  public string victory_condition => Consts.Format(Consts.GetInstance().GUILD_BATTLE_VICTORY_CONDITION, (IDictionary) new Hashtable()
  {
    {
      (object) "cnt",
  ...
  public string victory_sub_condition => Consts.Format(Consts.GetInstance().GUILD_BATTLE_SUB_CONDITION1, (IDictionary) new Hashtable()
  {
    {
      (object) "cnt",
      (object) this.bm.battleInfo.gvgSetting.turns.ToLocalizeNumberText()
  ...
  public Future<None> startGVG(BattleInfo battleInfo, bool isRestart) => new Future<None>((Func<Promise<None>, IEnumerator>) (promise => this._startGVG(promise, battleInfo, isRestart)));

  private IEnumerator _startGVG(
    ...
    bool isRestart)
  {
    ...
    else
    {
      ...
      else if (this.bm.environment.core.phaseState.state != BL.Phase.battle_start)
      {
        ...
        this.bm.environment.core.phaseState.setStateWith(BL.Phase.pvp_restart, this.bm.environment.core, (System.Action) (() => {}));
      }
    }
    e = this.initPrefabs();
    while (e.MoveNext())
  ...
  private IEnumerator initPrefabs()
  {
    ...
    IEnumerator e;
    if ((UnityEngine.Object) this.finishPrefab == (UnityEngine.Object) null)
    {
  ...
  public static GVGManager createGVGManager()
  {
    ...
    if ((UnityEngine.Object) gvgManager == (UnityEngine.Object) null)
  ...
  private HashSet<BL.Panel> createFormationPanels(int order)
  {
    ...
    foreach (GvgStageFormation gvgStageFormation in ((IEnumerable<GvgStageFormation>) MasterData.GvgStageFormationList).Where<GvgStageFormation>((Func<GvgStageFormation, bool>) (x => x.stage.ID == ec.stage.id)).Where<GvgStageFormation>((Func<GvgStageFormation, bool>) (x => x.player_order == order)))
      panelSet.Add(ec.getFieldPanel(gvgStageFormation.formation_y - 1, gvgStageFormation.formation_x - 1));
    return panelSet;
  }
  ...
  private bool timeLimitp() => this._stopwatch != null && this._stopwatch.IsRunning && this._timeLimit.value == 0;
  ...
  private IEnumerator doGvgMain()
  {
    ...
    {
      IEnumerator e;
      while (((UnityEngine.Object) gvgManager.btm == (UnityEngine.Object) null || gvgManager.btm.isRunning || (sm.changeSceneQueueCount > 0 || !sm.isSceneInitialized) || (!gvgManager.bm.isBattleEnable || gvgManager._isWaitAction)) && (gvgManager.bm.isBattleEnable || ec.phaseState.state != BL.Phase.pvp_disposition))
      {
      ...
      if (gvgManager.nextStateWaitAction != null && gvgManager.checkNextStateFlags())
      {
        ...
        gvgManager.nextStateWaitAction = (System.Action) null;
      }
      else
      {
        switch (ec.phaseState.state)
        {
          ...
          case BL.Phase.pvp_restart:
            ...
            gvgManager.btm.setScheduleAction(new System.Action(gvgManager.\u003CdoGvgMain\u003Eb__116_0));
  ...
  {
    this.bm.battleEffects.startEffect((Transform) null, 3f, popupPrefab: this.finishPrefab, cloneAction: ((System.Action<GameObject>) (o => o.GetComponent<PopupPvpMatchResult>().setResult(this.getFinishBattle(), this.playerOrder))));
  ...
  private int calcPoint(BL.Unit unit) => !unit.isFacility ? (int) ((double) this.setting.point_base_factor + ((double) (unit.unit.rarity.index + 1) * (double) this.setting.point_rarity_factor + (double) unit.playerUnit.cost * (double) this.setting.point_cost_factor) * (unit.is_leader ? (double) this.setting.point_leader_factor : (double) this.setting.point_no_leader_factor)) : 0;

  private int calcRespawnCount(BL.Unit unit) => (int) ((double) this.setting.respawn_base_factor + ((double) (unit.unit.rarity.index + 1) * (double) this.setting.respawn_rarity_factor + (double) unit.playerUnit.cost * (double) this.setting.respawn_cost_factor));
  ...
  private IEnumerator closeGvg()
  {
    ...
    {
      IEnumerator e = this.forceClose((System.Action) (() => ModalWindow.Show(Consts.GetInstance().POPUP_GUILD_BATTLE_RESUME_ERROR_TITLE, Consts.GetInstance().POPUP_GUILD_BATTLE_RESUME_ERROR_MESSAGE, (System.Action) (() => end = true))));
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
    }
    else
      ModalWindow.Show(Consts.GetInstance().POPUP_GUILD_BATTLE_RESUME_ERROR_TITLE, Consts.GetInstance().POPUP_GUILD_BATTLE_RESUME_ERROR_MESSAGE, (System.Action) (() => end = true));
    while (!end)
      yield return (object) null;
  }
  ...
  private IEnumerator forceClose(System.Action action)
  {
    ...
    IEnumerator e1 = WebAPI.GvgBattleForceClose(GuildUtil.gvgBattleIDServer, (System.Action<WebAPI.Response.UserError>) (e =>
    {
