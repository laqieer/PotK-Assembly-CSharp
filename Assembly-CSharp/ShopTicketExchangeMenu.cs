...
public class ShopTicketExchangeMenu : BackButtonMenuBase
{
  ...
  public void IbtnMaterialTicket()
  {
    ...
    if (!this.isInitMaterialTicket)
      this.MaterialTicketGrid.transform.Clear();
    this.MaterialTicketButton.isEnabled = false;
    this.KillersTicketButton.isEnabled = true;
    this.MaterialTicketButtonLabel.color = Color.white;
    this.KillersTicketButtonLabel.color = ShopCommon.TabDisableTextColor;
  ...
  public void IbtnKillersTicket()
  {
    ...
    if (!this.isInitKillersTicket)
      this.KillersTicketGrid.transform.Clear();
    this.MaterialTicketButton.isEnabled = true;
    this.KillersTicketButton.isEnabled = false;
    this.MaterialTicketButtonLabel.color = ShopCommon.TabDisableTextColor;
    this.KillersTicketButtonLabel.color = Color.white;
  ...
  private List<PlayerSelectTicketSummary> GetPlayerSelectTicketSummary(
    TicketType ticketType)
  {
    ...
    if (selectTicketSummaryArray == null || selectTicketSummaryArray.Length == 0)
      return selectTicketSummaryList;
    foreach (PlayerSelectTicketSummary selectTicketSummary in selectTicketSummaryArray)
    {
      PlayerSelectTicketSummary playerSelectTicket = selectTicketSummary;
      if (playerSelectTicket != null && playerSelectTicket.quantity > 0)
      {
        ...
        switch (ticketType)
        {
          case TicketType.Material:
            ...
            {
              selectTicketSummaryList.Add(playerSelectTicket);
              continue;
            }
            continue;
  ...
  private IEnumerator ChangeCreateScrollItems(TicketType ticketType)
  {
    ...
    if (playerSelectTickets.Count > 0)
    {
      ...
      foreach (SM.SelectTicket selectTicket in (IEnumerable<SM.SelectTicket>) orderedEnumerable)
      {
        ...
        GameObject gameObject = this.ItemPrefab.Clone(grid.transform);
        IEnumerator e;
        switch (ticketType)
        {
          case TicketType.Material:
  ...
  private int GetTicketsPriority(PlayerSelectTicketSummary playerUnitTicket) => ((IEnumerable<MasterDataTable.SelectTicket>) MasterData.SelectTicketList).First<MasterDataTable.SelectTicket>((Func<MasterDataTable.SelectTicket, bool>) (x => x.ID == playerUnitTicket.ticket_id)).priority;
  ...
  public void onStartScene() => this.enabled = true;

  public void onEndScene() => this.enabled = false;
}
