...
public class Unit00420ResetPopup : BackButtonMenuBase
{
  ...
  public IEnumerator Init(
    Unit004ReinforcePage bMenu,
    PlayerUnit bUnit,
    PlayerMaterialUnit rUnit)
  {
    this.menu = bMenu;
    this.baseUnit = bUnit;
    ...
    e = (IEnumerator) null;
    GameObject unitIconGo = UnityEngine.Object.Instantiate<GameObject>(prefabF.Result);
    ...
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    this.linkCharacter.gameObject.SetActive(false);
    unitIconGo.gameObject.SetParent(this.linkCharacter.gameObject, 0.85f);
    this.linkCharacter.gameObject.SetActive(true);
    ...
    int num = this.resetUnit == null ? 0 : this.resetUnit.quantity;
    ...
    this.txtDescriptionNoUnit.gameObject.SetActive(false);
    this.txtDescriptionNoReinforce.gameObject.SetActive(false);
    this.ibtnPopupClose.SetActive(false);
    this.ibtnPopupNormal.SetActive(false);
    if (bUnit.buildup_count == 0)
    {
      this.txtDescriptionNoReinforce.gameObject.SetActive(true);
      this.ibtnPopupClose.SetActive(true);
    }
    else if (num == 0)
    {
      this.txtDescriptionNoUnit.gameObject.SetActive(true);
      this.ibtnPopupClose.SetActive(true);
    }
    else
  ...
  public IEnumerator reset()
  {
    ...
    if (unit00420ResetPopup.resetUnit != null && !Singleton<CommonRoot>.GetInstance().isLoading)
    {
      ...
      intList2.Add(1);
      Future<WebAPI.Response.UnitBuildup> paramF = WebAPI.UnitBuildup(unit00420ResetPopup.baseUnit.id, intList1.ToArray(), intList2.ToArray(), (System.Action<WebAPI.Response.UserError>) (error =>
      {
