...
public class BattleTimeManager : BattleManagerBase
{
  ...
  public void setTargetBeforeMoveUnit(BL.Panel panel, System.Action endAction)
  {
    BattleTimeManager.TargetCamera targetCamera = new BattleTimeManager.TargetCamera(panel, 0.0f, (System.Action) null, endAction, true);
    targetCamera.EnableBeforeMoveUnit();
    this.setSchedule((Schedule) targetCamera);
  }

  public void setTargetMoveUnit(BL.Panel panel, float time)
  {
    BattleTimeManager.TargetCamera targetCamera = new BattleTimeManager.TargetCamera(panel, 0.0f, (System.Action) null, (System.Action) null, false);
    targetCamera.EnableMoveUnit(time);
    this.setSchedule((Schedule) targetCamera);
  }
  ...
  public void setCurrentUnit(BL.Unit unit, float wait = 0.1f, bool isWaitCameraMove = false) => this.setCurrentUnit(unit, new Func<BL.Unit, bool>(this.IsCameraMove), new System.Action<BL.Unit>(this.environment.setCurrentUnit_), wait, isWaitCameraMove);

  public void setCurrentUnitByPlayerInput(BL.Unit unit) => this.setCurrentUnit(unit, new Func<BL.Unit, bool>(this.IsCameraMoveByPlayerInput), new System.Action<BL.Unit>(this.environment.setCurrentUnitByPlayerInput), 0.1f, false);

  private void setCurrentUnit(
    BL.Unit unit,
    Func<BL.Unit, bool> isCameraMove,
    System.Action<BL.Unit> setCurrentAction,
    float wait,
    bool isWaitCameraMove)
  {
    if (unit != (BL.Unit) null)
    {
      if (isCameraMove(unit))
        this.setTargetUnit(this.environment.core.getUnitPosition(unit), wait, func: ((System.Action) (() => setCurrentAction(unit))), isWaitCameraMove: isWaitCameraMove);
      else
        setCurrentAction(unit);
    }
    else
      this.setScheduleAction((System.Action) (() => setCurrentAction((BL.Unit) null)), wait);
  }

  private bool IsCameraMove(BL.Unit unit) => this.environment.core.phaseState.state != BL.Phase.pvp_disposition;

  private bool IsCameraMoveByPlayerInput(BL.Unit unit) => this.environment.core.phaseState.state != BL.Phase.pvp_disposition && unit != (BL.Unit) null && unit.isPlayerForce;
  ...
  public void changeSceneWithReturnWait(
    ...
    System.Action endAction = null,
    System.Action sceneEndAction = null,
    params object[] args)
  {
