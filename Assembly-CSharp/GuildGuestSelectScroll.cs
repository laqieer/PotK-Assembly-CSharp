...
public class GuildGuestSelectScroll : MonoBehaviour
{
  ...
  private System.Action<GvgCandidate> decideButtonAction;
  ...
  private IEnumerator SetUnitIcon(GvgCandidate friend, bool isRaidHelper = false)
  {
    ...
    if (friend != null)
    {
      ...
      if (isRaidHelper)
        this.unitIcon.onClick = (System.Action<UnitIconBase>) (x =>
        {
        ...
        this.unitIcon.onClick = (System.Action<UnitIconBase>) (x => this.StartCoroutine(this.onClickIcon(friend.player_id)));
    }
    ...
    Future<UnityEngine.Sprite> image = EmblemUtility.LoadEmblemSprite(friend.player_emblem_id);
    e = image.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
  ...
  public IEnumerator InitializeAsync(
    ...
    System.Action<GvgCandidate> action,
    bool isRaidHelper = false)
  {
    ...
    if (friend != null)
    {
      ...
      if (mode == GuildGuestSelectScroll.MODE.Def)
      {
        ...
        this.btnDecide.isEnabled = friend.using_player_id == null;
      }
      else
      {
    ...
    this.btnDecide.GetComponent<BoxCollider>().size = (Vector3) this.btnDecide.GetComponent<UISprite>().localSize;
