...
public class PopupUnityValueDetail : BackButtonPopupBase
{
  ...
  public static void show(
    ...
    System.Action<System.Action> beforeChangeSceneAction,
    System.Action<PopupUtility.SceneTo> changedOtherScene,
    ...
    bool disableChangeQuest)
  {
    ...
    unityValueDetail.gameObject.GetComponent<UIRect>().alpha = 0.0f;
    ...
    int num1 = (double) number >= (double) PlayerUnit.GetUnityValueMax() ? 0 : buildupUnity.GetDecimalAsPercent();
    unityValueDetail.txtBuildup_.SetTextLocalize(integer);
    unityValueDetail.txtBuildupDec_.SetTextLocalize(num1);
    unityValueDetail.gaugeBuildupDec_.setValue(num1, 99, false);
    ...
    if (array1.Length != 0)
    {
      ...
      if (eventSIDsList.Length != 0)
      {
        ...
        for (int index = 0; index < questConverterDataList.Count; ++index)
        {
          ...
          stageItemPrefab.Clone(unityValueDetail.stageListGrid.transform).GetComponent<PopupUnityValueDetailStageItem>().Initialize(stageData, beforeChangeSceneAction, changedOtherScene, disableChangeQuest);
        }
      ...
      {
        PlayerStoryQuestS[] array2 = SMManager.Get<PlayerStoryQuestS[]>();
        ...
        for (int index = 0; index < storySIDsList.Length; ++index)
        {
          ...
          PlayerStoryQuestS playerStoryQuestS = Array.Find<PlayerStoryQuestS>(array2, (Predicate<PlayerStoryQuestS>) (x => x._quest_story_s == currentID));
        ...
        for (int index = 0; index < questConverterDataList.Count; ++index)
          stageItemPrefab.Clone(unityValueDetail.stageListGrid.transform).GetComponent<PopupUnityValueDetailStageItem>().Initialize(questConverterDataList[index], beforeChangeSceneAction, changedOtherScene, disableChangeQuest);
        for (int index = 0; index < intList.Count; ++index)
        {
          int currentID = intList[index];
    ...
    Singleton<PopupManager>.GetInstance().startOpenAnime(unityValueDetail.gameObject);
  }

  private IEnumerable<int> parseQuestSIDs(string sids) => ((IEnumerable<string>) sids.Split(':')).Select<string, int>((Func<string, int>) (x => int.Parse(x.Replace(".0", ""))));
