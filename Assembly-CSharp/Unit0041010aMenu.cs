...
public class Unit0041010aMenu : BackButtonMenuBase
{
  ...
  private IEnumerator UnitSellAsync()
  {
    ...
    if (!Singleton<CommonRoot>.GetInstance().isLoading)
    {
      ...
      unit0041010aMenu.sellUnitIcons.ForEach((System.Action<PlayerUnit>) (ic =>
      {
      ...
      Future<WebAPI.Response.UnitSell> ft = WebAPIUtil.UnitSell(player_material_unit_ids.ToArray(), player_unit_ids.ToArray(), (System.Action<WebAPI.Response.UserError>) (error =>
      {
  ...
  public IEnumerator SetSelectedUnitIcons(List<PlayerUnit> icons, Unit00410Menu menu)
  {
    ...
    foreach (PlayerUnit icon1 in icons)
    {
      ...
      {
        GameObject unitIconGo = UnityEngine.Object.Instantiate<GameObject>(prefab);
        ...
        this.LinkCharacter[i].gameObject.SetActive(false);
        unitIconGo.gameObject.SetParent(this.LinkCharacter[i].gameObject, 0.85f);
        this.LinkCharacter[i].gameObject.SetActive(true);
