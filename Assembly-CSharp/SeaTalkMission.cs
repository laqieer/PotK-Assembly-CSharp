...
public class SeaTalkMission : BackButtonPopupWindow
{
  ...
  public IEnumerator Init(
    ...
    PlayerCallMission[] playerCallMissions)
  {
    ...
    Future<UnityEngine.Sprite> f = talkUnitInfo.unit.LoadSpriteLarge(talkUnitInfo.unit.job_UnitJob, 1f);
  ...
  private IEnumerator CreateMissions()
  {
    ...
    foreach (PlayerCallMission playerCallMission in this.playerCallMissions)
    {
      if (playerCallMission.mission_status == 4)
        playerCallMissionList.Add(playerCallMission);
      else
        first.Add(playerCallMission);
    }
    this.playerCallMissions = first.Concat<PlayerCallMission>((IEnumerable<PlayerCallMission>) playerCallMissionList).ToList<PlayerCallMission>();
    int clearCount = 0;
    int height = 0;
    ...
    for (int i = 0; i < this.playerCallMissions.Count; ++i)
    {
      ...
      if (playerCallMission.count >= callMission.number_times)
        ++clearCount;
      GameObject go = this.missionItemPrefab.Clone(this.scrollView.transform);
    ...
    foreach (Component component in this.scrollView.transform)
  ...
  {
    if ((bool) (UnityEngine.Object) UnityEngine.Object.FindObjectOfType<SeaTalkMissionRewardReceive>())
      return;
