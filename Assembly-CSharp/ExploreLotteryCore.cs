...
public class ExploreLotteryCore : Singleton<ExploreLotteryCore>
{
  ...
  protected override void Initialize() => this.mDataManager = Singleton<ExploreDataManager>.GetInstance();

  public IEnumerator CalcTaskOnBackGround(
    TimeSpan calcSpan,
    bool easyCalc = true,
    System.Action errorCallback = null)
  {
    ...
    else
    {
      ...
      {
        if ((double) Time.realtimeSinceStartup - (double) realtimeSinceStartup > 0.0333333350718021)
        {
  ...
  {
    private UnityEngine.Random.State mState;
    ...
    public ExploreRandom(int seed, long counter)
    {
      ...
      this.Count = counter;
      this.mState = UnityEngine.Random.state;
    ...
    public int Range(int min, int max)
    {
      UnityEngine.Random.State state = UnityEngine.Random.state;
      ++this.Count;
      UnityEngine.Random.state = this.mState;
