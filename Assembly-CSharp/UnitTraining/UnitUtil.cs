...
namespace UnitTraining
{
  public static class UnitUtil
  {
    ...
    public static bool checkAnyInheritance(PlayerUnit target) => target.dexterity.inheritance != 0 || target.agility.inheritance != 0 || (target.mind.inheritance != 0 || target.strength.inheritance != 0) || (target.vitality.inheritance != 0 || target.hp.inheritance != 0 || target.intelligence.inheritance != 0) || (uint) target.lucky.inheritance > 0U;

    public static EvolutionType getEvolutionType(
      UnitUnit target,
      ref List<UnitEvolutionPattern> result)
    {
      UnitEvolutionPattern[] evolutionPatternArray = target.IsEvolution ? target.EvolutionPattern : (UnitEvolutionPattern[]) null;
      if (evolutionPatternArray == null || evolutionPatternArray.Length == 0)
        return EvolutionType.Limit;
      if (result != null)
        result.AddRange((IEnumerable<UnitEvolutionPattern>) evolutionPatternArray);
