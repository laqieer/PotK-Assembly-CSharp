...
public class SlotDebug : MonoBehaviour
{
  ...
  private UnityEngine.Sprite sprite_p1;
  private UnityEngine.Sprite sprite_p2;
  private UnityEngine.Sprite sprite_p3;
  [SerializeField]
  ...
  public IEnumerator Init()
  {
    ...
    slotDebug.animator = slotDebug.GetComponent<Animator>();
  ...
  private void Update()
  {
    if (!this.animator.GetCurrentAnimatorStateInfo(0).IsName("result") || !this.slot100 || (double) this.animator.GetCurrentAnimatorStateInfo(0).normalizedTime < 1.0)
      return;
    this.slot100 = false;
  ...
  private void LilleResultTextureChange()
  {
    ...
    this.sprite_p1 = UnityEngine.Sprite.Create(this.GetTextureByName(this.main_script.textureNameList_1[this.main_script.stopTextureId_1 - 1]), new Rect(0.0f, 0.0f, 128f, 128f), new Vector2(0.5f, 0.5f), 100f, 0U, SpriteMeshType.FullRect);
    this.sprite_p2 = UnityEngine.Sprite.Create(this.GetTextureByName(this.main_script.textureNameList_2[this.main_script.stopTextureId_2 - 1]), new Rect(0.0f, 0.0f, 128f, 128f), new Vector2(0.5f, 0.5f), 100f, 0U, SpriteMeshType.FullRect);
    this.sprite_p3 = UnityEngine.Sprite.Create(this.GetTextureByName(this.main_script.textureNameList_3[this.main_script.stopTextureId_3 - 1]), new Rect(0.0f, 0.0f, 128f, 128f), new Vector2(0.5f, 0.5f), 100f, 0U, SpriteMeshType.FullRect);
  ...
  private void SetTextureByName(string texture_name, GameObject target_gb, int setid) => target_gb.GetComponent<Renderer>().materials[setid].mainTexture = (Texture) this.textures.SingleOrDefault<Texture2D>((Func<Texture2D, bool>) (f => f.name == texture_name));

  private Texture2D GetTextureByName(string texture_name) => this.textures.SingleOrDefault<Texture2D>((Func<Texture2D, bool>) (f => f.name == texture_name));
  ...
  private void NextState()
  {
    ...
    {
      Debug.Log((object) "2");
      this.black_renderer.enabled = !this.slot100;
