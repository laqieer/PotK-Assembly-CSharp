...
public class Shop0574ConfirmPopup : BackButtonMenuBase
{
  ...
  private System.Action<EarthShopArticle, GameObject, int> EndAction;
  ...
  public void Init(
    ...
    System.Action<EarthShopArticle, GameObject, int> endAction)
  {
    ...
    this.IbtnPopupMinus.isEnabled = false;
  ...
  public void onChangeInputQuantity(bool isInit = true)
  {
    ...
    if (this.quantity <= this.INPUT_MIN)
      ...
      this.IbtnPopupMinus.isEnabled = true;
    if ((long) this.GetTotalPrice() > this.possesiveMoney)
      this.IbtnPopupYes.isEnabled = false;
    else
      this.IbtnPopupYes.isEnabled = true;
    this.SetTotlaPrice();
    this.InpDescription.SetTextLocalize(this.quantity);
  }
