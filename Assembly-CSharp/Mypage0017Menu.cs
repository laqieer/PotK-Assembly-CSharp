...
public class Mypage0017Menu : BackButtonMenuBase
{
  ...
  public static Tuple<List<PlayerPresent>, bool> createReceiveList(
    PlayerPresent[] presents)
  {
    ...
    PlayerItem[] playerItemArray1 = SMManager.Get<PlayerItem[]>();
    SeaPlayer seaPlayer = SMManager.Get<SeaPlayer>();
    PlayerSeaDeck[] playerSeaDeckArray = SMManager.Get<PlayerSeaDeck[]>();
    int num1 = ((IEnumerable<PlayerItem>) ((IEnumerable<PlayerItem>) playerItemArray1).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.entity_type == MasterDataTable.CommonRewardType.gear && !x.isReisou())).ToArray<PlayerItem>()).Count<PlayerItem>();
    int num2 = ((IEnumerable<PlayerItem>) ((IEnumerable<PlayerItem>) playerItemArray1).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.entity_type == MasterDataTable.CommonRewardType.gear && x.isReisou())).ToArray<PlayerItem>()).Count<PlayerItem>();
    int num3 = 0;
    int num4 = 0;
    int num5 = 0;
    long num6 = 0;
    ...
    PlayerItem[] playerItemArray2 = SMManager.Get<PlayerItem[]>().AllSupplies();
    int num13 = 0;
    int idx = 1;
    while (num13 < MasterData.SupplySupply.Count<KeyValuePair<int, SupplySupply>>())
    {
      int cnt = 0;
      ((IEnumerable<PlayerItem>) playerItemArray2).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.supply.ID == MasterData.SupplySupply[idx].ID)).ForEach<PlayerItem>((System.Action<PlayerItem>) (x => cnt += x.quantity));
      numArray[MasterData.SupplySupply[idx].ID] = cnt;
      ++num13;
      idx++;
    }
    PlayerQuestKey[] playerQuestKeyArray = SMManager.Get<PlayerQuestKey[]>();
    foreach (PlayerQuestKey playerQuestKey in playerQuestKeyArray)
      dictionary1.Add(playerQuestKey.quest_key_id, playerQuestKey.quantity);
    PlayerRecoveryItem[] playerRecoveryItemArray = SMManager.Get<PlayerRecoveryItem[]>();
    foreach (PlayerRecoveryItem playerRecoveryItem in playerRecoveryItemArray)
      ...
      else
      {
        switch (present.reward_type_id.Value)
        {
          ...
          case 18:
            ...
            continue;
          case 19:
            PlayerQuestKey playerQuestKey = ((IEnumerable<PlayerQuestKey>) playerQuestKeyArray).Where<PlayerQuestKey>((Func<PlayerQuestKey, bool>) (x =>
            {
          ...
          case 39:
            ...
            PlayerRecoveryItem playerRecoveryItem = ((IEnumerable<PlayerRecoveryItem>) playerRecoveryItemArray).Where<PlayerRecoveryItem>((Func<PlayerRecoveryItem, bool>) (x =>
            {
            ...
            {
              rewardQuantity = present.reward_quantity;
              int num15 = dictionary2[playerRecoveryItem.recovery_item_id];
              nullable = rewardQuantity.HasValue ? new int?(rewardQuantity.GetValueOrDefault() + num15) : new int?();
  ...
  public IEnumerator Init(PlayerPresent[] presents)
  {
    ...
    {
      this.IBtnGetAll.duration = 0.0f;
      this.IBtnGetAll.isEnabled = false;
    }
    else
      this.IBtnGetAll.isEnabled = true;
    ...
    {
      this.IBtnDeleteAll.isEnabled = true;
    }
    else
    {
      this.IBtnDeleteAll.duration = 0.0f;
      this.IBtnDeleteAll.isEnabled = false;
    }
  ...
  public void SaveScrollPosition() => this.scrollPositionBeforePresent = this.scroll.scrollView.transform.localPosition.y;
  ...
  private void IbtnReceive(PlayerPresent present)
  {
    ...
    PlayerQuestKey[] playerQuestKeyArray = SMManager.Get<PlayerQuestKey[]>();
    PlayerRecoveryItem[] playerRecoveryItemArray = SMManager.Get<PlayerRecoveryItem[]>();
    int num1 = ((IEnumerable<PlayerItem>) ((IEnumerable<PlayerItem>) playerItemArray).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.entity_type == MasterDataTable.CommonRewardType.gear && !x.isReisou())).ToArray<PlayerItem>()).Count<PlayerItem>();
    int num2 = ((IEnumerable<PlayerItem>) ((IEnumerable<PlayerItem>) playerItemArray).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.entity_type == MasterDataTable.CommonRewardType.gear && x.isReisou())).ToArray<PlayerItem>()).Count<PlayerItem>();
    ...
    else if (present.reward_type_id.Value == 30)
    {
      ...
      if (seaPlayer == null || playerSeaDeckArray == null || playerSeaDeckArray.Length == 0)
        ModalWindow.Show("海上編未プレイ", "海上編をはじめていないため、DPは受け取れません。", (System.Action) (() => this.IsPush = false));
  ...
  protected void ScrollUpdate()
  {
    ...
    do
    {
      ...
      foreach (GameObject gameObject in this.scroll)
      {
        ...
        {
          int? nullable = this.allPresentInfos.FirstIndexOrNull<PresentInfo>((Func<PresentInfo, bool>) (v => (UnityEngine.Object) v.scroll != (UnityEngine.Object) null && (UnityEngine.Object) v.scroll.gameObject == (UnityEngine.Object) unit));
  ...
  private void ResetScroll(int index)
  {
    ...
    this.allPresentInfos.Where<PresentInfo>((Func<PresentInfo, bool>) (a => (UnityEngine.Object) a.scroll == (UnityEngine.Object) scroll)).ForEach<PresentInfo>((System.Action<PresentInfo>) (b => b.scroll = (Mypage0017Scroll) null));
  }
  ...
  private IEnumerator CreateScrollBase(GameObject prefab)
  {
    ...
    for (int index = 0; index < Mathf.Min(12, this.allPresentInfos.Count); ++index)
      this.allScroll.Add(UnityEngine.Object.Instantiate<GameObject>(prefab).GetComponent<Mypage0017Scroll>());
    this.scroll.Reset();
    for (int index = 0; index < Mathf.Min(12, this.allScroll.Count); ++index)
      this.scroll.AddColumn1(this.allScroll[index].gameObject, 604, 135);
