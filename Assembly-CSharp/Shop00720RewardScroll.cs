...
public class Shop00720RewardScroll : MonoBehaviour
{
  ...
  private void SetReelPattern(List<UnityEngine.Sprite> pattern, string txt, Shop00720Prefabs prefabs) => prefabs.DirSlotPattern.Clone(this.Pattern.transform).GetComponent<Shop00720ReelPattern>().Init(pattern, txt);
  ...
  private IEnumerator SetReward(List<Shop00720RewardData> data, Shop00720Prefabs prefabs)
  {
    ...
    foreach (var data1 in data.Select((s, i) => new
    {
      s = s,
      i = i
    }))
    {
      var n = data1;
      GameObject reward = prefab.Clone(shop00720RewardScroll.gameObject.transform);
  ...
  private void AdjastHeight(int times)
  {
    ...
    BoxCollider component2 = this.gameObject.GetComponent<BoxCollider>();
