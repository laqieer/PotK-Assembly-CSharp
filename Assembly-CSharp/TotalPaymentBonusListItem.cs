...
public class TotalPaymentBonusListItem : MonoBehaviour
{
    ...
    TotalPaymentBonusPopup parent)
  {
    ...
    if (MasterData.TotalPaymentBonusContent.TryGetValue(bonus.bonus_item_id, out content))
    {
      ...
      {
        e = iconPrefab.CloneAndGetComponent<ItemIconDetail>(this.mIconGrid.transform).Init(reward.reward_type, reward.reward_id, reward.reward_quantity);
        while (e.MoveNext())
          yield return e.Current;
        e = (IEnumerator) null;
      }
    ...
    if (bonus.is_received)
    {
      ...
      this.mGetButton.isEnabled = bonus.is_archived;
  }

  private void SetPaidValue(int require) => this.mPaidValueLbl.SetTextLocalize(string.Format("累計姫石[ffff00]{0}[-]個\n消費で受取可能", (object) require));

  public void OnPushOpenButton() => this.mParentPopup.OnListItemGetButton(this.mBonusItemId);

  private List<TotalPaymentBonusReward> ConvertToRewardList(
    TotalPaymentBonusContent content)
  {
    List<TotalPaymentBonusReward> paymentBonusRewardList = new List<TotalPaymentBonusReward>();
    ...
    foreach (string str in rewardId.Split(chArray))
    {
      ...
      if (double.TryParse(str.Trim(), out result))
      {
        ...
        paymentBonusRewardList.Add(paymentBonusReward);
      }
    }
    return paymentBonusRewardList;
  }
}
