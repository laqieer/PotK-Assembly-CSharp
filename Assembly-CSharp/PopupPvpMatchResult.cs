...
public class PopupPvpMatchResult : NGBattleMenuBase
{
  ...
  protected override IEnumerator Start_Battle()
  {
    ...
    Future<UnityEngine.Sprite> spriteF = MasterData.UnitUnit[popupPvpMatchResult.env.core.playerUnits.value[0].unitId].LoadSpriteLarge(popupPvpMatchResult.env.core.playerUnits.value[0].playerUnit.job_id, 1f);
  ...
  public IEnumerator Start_Battle_Debug()
  {
    ...
    Future<UnityEngine.Sprite> spriteF = MasterData.UnitUnit[100113].LoadSpriteLarge();
    IEnumerator e = spriteF.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
  ...
  private void setResultBase()
  {
    ...
    if (n < this.effects.Length)
      ((IEnumerable<GameObject>) this.effects).ForEach<GameObject>((System.Action<GameObject>) (x => x.SetActive((UnityEngine.Object) x == (UnityEngine.Object) this.effects[n])));
    if (victoryEffect == PvpVictoryEffectEnum.great_effect || victoryEffect == PvpVictoryEffectEnum.excellent_effect)
    {
      ...
      ((IEnumerable<GameObject>) this.effects_win).ForEach<GameObject>((System.Action<GameObject>) (x => x.SetActive((UnityEngine.Object) x == (UnityEngine.Object) this.effects_win[m])));
    }
    else
      ((IEnumerable<GameObject>) this.effects_win).ForEach<GameObject>((System.Action<GameObject>) (x => x.SetActive(false)));
    this.linkObj.sprite2D = this.sprite;
    this.linkObj.color = Color.Lerp(Color.white, Color.gray, n == 0 ? 0.0f : 1f);
    this.linkObj.GetComponent<NGxMaskSpriteWithScale>().FitMask();
    this.effectParent.SetActive(true);
    switch (victoryEffect)
    {
