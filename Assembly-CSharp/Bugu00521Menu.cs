...
public class Bugu00521Menu : Bugu005SelectItemListMenuBase
{
  ...
  public override Persist<Persist.ItemSortAndFilterInfo> GetPersist() => Persist.bugu0052CompositeSortAndFilter;

  protected override List<PlayerItem> GetItemList() => ((IEnumerable<PlayerItem>) SMManager.Get<PlayerItem[]>()).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.isWeapon() && x.gear.kind.Enum != GearKindEnum.accessories || x.isComposeManaSeed())).ToList<PlayerItem>();
  ...
  protected override List<PlayerMaterialGear> GetMaterialList()
  {
    ...
    })).ForEach<PlayerMaterialGear>((System.Action<PlayerMaterialGear>) (x =>
    {
  ...
  protected override IEnumerator InitExtension()
  {
    ...
    if (bugu00521Menu.firstInit)
    {
      ...
      if (bugu00521Menu.FirstSelectItemList != null && bugu00521Menu.FirstSelectItemList.Count > 0)
      {
        ...
        // ISSUE: reference to a compiler-generated method
        bugu00521Menu.FirstSelectItemList.ForEach(new System.Action<GameCore.ItemInfo>(bugu00521Menu.\u003CInitExtension\u003Eb__13_1));
      }
    }
    else if (bugu00521Menu.SelectItemList != null && bugu00521Menu.SelectItemList.Count > 0)
    {
      ...
      bugu00521Menu.SelectItemList.Clear();
      // ISSUE: reference to a compiler-generated method
      System.Action<InventoryItem> action = new System.Action<InventoryItem>(bugu00521Menu.\u003CInitExtension\u003Eb__13_3);
      list.ForEach(action);
    }
    return false;
  }
  ...
  protected override void BottomInfoUpdate()
  {
    ...
    int num = this.SelectItemList != null ? this.SelectItemList.Count : 0;
    ...
    this.txtSlectedcountNum.SetTextLocalize(num);
    this.txtSlectedcountNum.color = num < this.SelectMax ? Color.white : Color.red;
    int compositeCost = CalcItemCost.GetCompositeCost(this.SelectItemList);
    this.txtSpendzenieNum.SetTextLocalize(compositeCost);
    this.txtSpendzenieNum.color = player.money >= (long) compositeCost ? Color.white : Color.red;
    this.ibtnYes.isEnabled = num > 0 && player.money >= (long) compositeCost;
  }
  ...
  protected override bool DisableTouchIcon(InventoryItem item)
  {
    ...
    if (this.SelectItemList.Count<InventoryItem>() > 0 && item.Item.gear != null)
    {
      ...
      GearGear gearGear = selectItem.Item != null ? selectItem.Item.gear : (GearGear) null;
      if (gearGear != null)
      {
        if (!gearGear.isComposeManaSeed())
        {
          if (this.DisableTouchIconDefault(item))
            return true;
          return item.Item.isWeapon && item.Item.isComposeManaSeed;
        }
        if (this.DisableTouchIconDefault(item) || gearGear.ID != item.Item.masterID || item.Item.ForBattle && item != selectItem)
          return true;
        return !this.SelectItemList.Any<InventoryItem>((Func<InventoryItem, bool>) (x => x == item)) && this.SelectItemList.Where<InventoryItem>((Func<InventoryItem, bool>) (y => y != null && y.Item != null)).Sum<InventoryItem>((Func<InventoryItem, int>) (x => x.Item.gearAccessoryRemainingAmount)) >= gearGear.manaSeedRecoveryLimit;
      }
    }
  ...
  public override void IbtnBack()
  {
    ...
    System.Action<GameCore.ItemInfo> action = (System.Action<GameCore.ItemInfo>) (x => invList.Add(new InventoryItem(x)));
    list.ForEach(action);
