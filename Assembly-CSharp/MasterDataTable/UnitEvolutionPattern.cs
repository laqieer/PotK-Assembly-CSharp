...
namespace MasterDataTable
{
  ...
  public class UnitEvolutionPattern
  {
    ...
    public static UnitEvolutionPattern Parse(MasterDataReader reader) => new UnitEvolutionPattern()
    {
      ID = reader.ReadInt(),
    ...
    public static int[] getGenealogyIds(int unitId)
    {
      ...
      if (genealogy.Length == 0)
        return new int[0];
      int[] numArray = new int[genealogy.Length + 1];
      numArray[0] = genealogy[0].unit_UnitUnit;
      for (int index = 0; index < genealogy.Length; ++index)
        numArray[index + 1] = genealogy[index].target_unit_UnitUnit;
      return numArray;
    }
