...
namespace MasterDataTable
{
  ...
  public class ExploreEnemy
  {
    ...
    public static ExploreEnemy Parse(MasterDataReader reader) => new ExploreEnemy()
    {
      ID = reader.ReadInt(),
    ...
    public BattleskillSkill magicBullet
    {
      get
      {
        ...
        if (!MasterData.BattleskillSkill.TryGetValue(this.magicBullet_BattleskillSkill.Value, out battleskillSkill))
    ...
    private PlayerUnit createPlayerUnit()
    {
      ...
      playerUnit.is_enemy_leader = (uint) playerUnit.leader_skills.Length > 0U;
    ...
    private WeakPoint createWeakPoint() => new WeakPoint(this.parseEnumStringArray<CommonElement>(this.weakPoint_element), this.parseEnumStringArray<GearKindEnum>(this.weakPoint_weapon), this.parseEnumStringArray<UnitTypeEnum>(this.weakPoint_unitType));
    ...
    private SortedDictionary<int, int> createDropDeck()
    {
      ...
      foreach (ExploreDropTable exploreDropTable in ((IEnumerable<ExploreDropTable>) MasterData.ExploreDropTableList).Where<ExploreDropTable>((Func<ExploreDropTable, bool>) (x => x.deck_id == this.drop_deck_id)))
      {
        ...
        sortedDictionary.Add(key, exploreDropTable.drop_reward_id);
      }
