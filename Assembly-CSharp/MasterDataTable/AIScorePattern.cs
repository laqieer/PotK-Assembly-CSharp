...
namespace MasterDataTable
{
  ...
  public class AIScorePattern
  {
    ...
    public AIScorePattern.AICalculator Attack(PlayerUnit unit) => this.getCalculator("a" + (object) unit.id, unit.ai_attack, "attack");

    public AIScorePattern.AICalculator Move(PlayerUnit unit) => this.getCalculator("m" + (object) unit.id, unit.ai_move, "move");

    public AIScorePattern.AICalculator Heal(PlayerUnit unit) => this.getCalculator("h" + (object) unit.id, unit.ai_heal, "heal");

    public AIScorePattern.AICalculator Use(PlayerUnit unit) => string.IsNullOrEmpty(unit.ai_use) ? (AIScorePattern.AICalculator) null : this.getCalculator("u" + (object) unit.id, unit.ai_use, "use");

    private AIScorePattern.AICalculator getCalculator(
      string name,
      string exp_,
      string group)
    {
    ...
    public static AIScorePattern Parse(MasterDataReader reader) => new AIScorePattern()
    {
      ID = reader.ReadInt(),
