...
namespace MasterDataTable
{
  ...
  public class GearGear
  {
    ...
    public bool hasAttackClass => this.gearClassification != null && this.gearClassification.attack_classification != GearAttackClassification.none;
    ...
    private GearClassificationPattern classificationDefault => Array.Find<GearClassificationPattern>(MasterData.GearClassificationPatternList, (Predicate<GearClassificationPattern>) (x => x.kind_GearKind == this.kind_GearKind));
    ...
    public List<List<GearGearSkill>> rememberSkills
    {
      get
      {
        List<List<GearGearSkill>> gearGearSkillListList = new List<List<GearGearSkill>>();
        List<GearGearSkill> list = ((IEnumerable<GearGearSkill>) MasterData.GearGearSkillList).Where<GearGearSkill>((Func<GearGearSkill, bool>) (x => x.gear.ID == this.ID)).ToList<GearGearSkill>();
        if (list.Count > 0)
        {
          foreach (IGrouping<int, GearGearSkill> source in list.GroupBy<GearGearSkill, int>((Func<GearGearSkill, int>) (x => x.skill_group)))
            gearGearSkillListList.Add(new List<GearGearSkill>((IEnumerable<GearGearSkill>) source.OrderBy<GearGearSkill, int>((Func<GearGearSkill, int>) (x => x.release_rank))));
        }
        return gearGearSkillListList;
      }
    }
    ...
    public List<List<GearReisouSkill>> rememberReisouSkills
    {
      get
      {
        List<List<GearReisouSkill>> gearReisouSkillListList = new List<List<GearReisouSkill>>();
        ...
        if (list.Count > 0)
        {
          foreach (IGrouping<int, GearReisouSkill> source in list.GroupBy<GearReisouSkill, int>((Func<GearReisouSkill, int>) (x => x.skill_group)))
            gearReisouSkillListList.Add(new List<GearReisouSkill>((IEnumerable<GearReisouSkill>) source.OrderBy<GearReisouSkill, int>((Func<GearReisouSkill, int>) (x => x.release_rank))));
        }
        return gearReisouSkillListList;
      }
    }
    ...
    public Dictionary<int, int?> specificationOfEquipmentUnitJobs
    {
      get
      {
        if (this._specificationOfEquipmentUnitJobs == null)
          this._specificationOfEquipmentUnitJobs = !this.specification_of_equipment_unit_group_id.HasValue ? new Dictionary<int, int?>() : ((IEnumerable<GearSpecificationOfEquipmentUnit>) MasterData.GearSpecificationOfEquipmentUnitList).Where<GearSpecificationOfEquipmentUnit>((Func<GearSpecificationOfEquipmentUnit, bool>) (x => x.group_id == this.specification_of_equipment_unit_group_id.Value && (uint) x.unit_id > 0U)).ToDictionary<GearSpecificationOfEquipmentUnit, int, int?>((Func<GearSpecificationOfEquipmentUnit, int>) (x => x.unit_id), (Func<GearSpecificationOfEquipmentUnit, int?>) (x => x.job_id));
        return this._specificationOfEquipmentUnitJobs;
      }
    }

    public bool hasSpecificationOfEquipmentUnits => this.specification_of_equipment_unit_group_id.HasValue;

    public bool enableEquipmentUnit(PlayerUnit unit) => unit.IsAllEquipUnit ? !((IEnumerable<IgnoreGear>) MasterData.IgnoreGearList).Any<IgnoreGear>((Func<IgnoreGear, bool>) (v => v.gear_id == this.ID)) : this.enableEquipmentUnitWithoutCheckAllEquipUnit(unit);
    ...
    public bool isMaterial() => this.kind.Enum == GearKindEnum.smith || this.kind.Enum == GearKindEnum.drilling || (this.kind.Enum == GearKindEnum.special_drilling || this.kind.Enum == GearKindEnum.sea_present) || this.kind.Enum == GearKindEnum.call_gift;

    public bool isCallGift() => this.kind.Enum == GearKindEnum.call_gift;

    public bool isReisou() => (uint) this.reisou_type > 0U;
    ...
    public bool isManaSeed() => this.kind.Enum == GearKindEnum.accessories && this.disappearance_type_GearDisappearanceType == 1;

    public static int manaSeedRecoveryAmount => !GearGear.manaSeedRecoveryAmount_.HasValue ? (GearGear.manaSeedRecoveryAmount_ = new int?(int.Parse(Consts.GetInstance().MANASEED_RECOVERY_AMOUNT))).Value : GearGear.manaSeedRecoveryAmount_.Value;

    public int manaSeedRecoveryLimit => (this.disappearance_num.HasValue ? this.disappearance_num.Value : 0) * GearGear.manaSeedRecoveryAmount;
    ...
    public CommonElement GetElement()
    {
      ...
      if (source1.Any<GearGearSkill>())
      {
        ...
        if (source4.Any<BattleskillSkill>())
          commonElement = source4.First<BattleskillSkill>().element;
      }
    ...
    public static GearGear Parse(MasterDataReader reader) => new GearGear()
    {
      ID = reader.ReadInt(),
    ...
    public GearGearDescription gear_description
    {
      get
      {
        GearGearDescription gearGearDescription;
        if (!MasterData.GearGearDescription.TryGetValue(this.gear_description_GearGearDescription, out gearGearDescription))
          Debug.LogError((object) ("Key not Found: MasterData.GearGearDescription[" + (object) this.gear_description_GearGearDescription + "]"));
        return gearGearDescription;
      }
    }
    ...
    public GearModelKind model_kind
    {
      get
      {
        ...
        if (!MasterData.GearModelKind.TryGetValue(this.model_kind_GearModelKind, out gearModelKind))
    ...
    public GearGear resource_reference_gear_id
    {
      ...
      {
        GearGear gearGear;
        if (!MasterData.GearGear.TryGetValue(this.resource_reference_gear_id_GearGear, out gearGear))
    ...
    public GearKind special_drilling_kind
    {
      get
      {
        ...
        GearKind gearKind;
        if (!MasterData.GearKind.TryGetValue(this.special_drilling_kind_GearKind.Value, out gearKind))
    ...
    public GearClassificationPattern classification
    {
      get
      {
        ...
        GearClassificationPattern classificationPattern;
        if (!MasterData.GearClassificationPattern.TryGetValue(this.classification_GearClassificationPattern.Value, out classificationPattern))
    ...
    public GearExpireDate expire_date
    {
      get
      {
        ...
        GearExpireDate gearExpireDate;
        if (!MasterData.GearExpireDate.TryGetValue(this.expire_date_GearExpireDate.Value, out gearExpireDate))
          Debug.LogError((object) ("Key not Found: MasterData.GearExpireDate[" + (object) this.expire_date_GearExpireDate.Value + "]"));
        return gearExpireDate;
      }
    }
    ...
    public Future<UnityEngine.Sprite> LoadSpriteThumbnail() => Singleton<ResourceManager>.GetInstance().Load<UnityEngine.Sprite>(string.Format("AssetBundle/Resources/Gears/{0}/2D/weapon_thum", (object) this.resource_reference_gear_id.ID));

    public Future<UnityEngine.Sprite> LoadSpriteBasic(float pixelsPerUnit = 1f) => Singleton<ResourceManager>.GetInstance().Load<UnityEngine.Sprite>(string.Format("AssetBundle/Resources/Gears/{0}/2D/weapon_basic", (object) this.resource_reference_gear_id.ID), pixelsPerUnit);

    public Future<GameObject> LoadModel() => Singleton<ResourceManager>.GetInstance().Load<GameObject>(string.Format("Gears/{0}/3D/prefab", (object) this.resource_reference_gear_id.ID));
  }
}
