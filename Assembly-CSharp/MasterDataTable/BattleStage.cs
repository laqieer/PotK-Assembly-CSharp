...
namespace MasterDataTable
{
  ...
  public class BattleStage
  {
    ...
    public void ApplyLandforms(System.Action<int, int, BattleMapLandform> f)
    {
    ...
    public BattleStageEnemy[] Enemies => ((IEnumerable<BattleStageEnemy>) MasterData.BattleStageEnemyList).Where<BattleStageEnemy>((Func<BattleStageEnemy, bool>) (x => x.stage.ID == this.ID)).ToArray<BattleStageEnemy>();

    public BattleStagePlayer[] Players => ((IEnumerable<BattleStagePlayer>) MasterData.BattleStagePlayerList).Where<BattleStagePlayer>((Func<BattleStagePlayer, bool>) (x => x.stage.ID == this.ID)).ToArray<BattleStagePlayer>();

    public BattleEarthStageGuest[] Guests => ((IEnumerable<BattleEarthStageGuest>) MasterData.BattleEarthStageGuestList).Where<BattleEarthStageGuest>((Func<BattleEarthStageGuest, bool>) (x => x.stage.ID == this.ID)).ToArray<BattleEarthStageGuest>();

    public BattleFieldEffectStage[] FieldEffectStages => ((IEnumerable<BattleFieldEffectStage>) MasterData.BattleFieldEffectStageList).Where<BattleFieldEffectStage>((Func<BattleFieldEffectStage, bool>) (x => x.stage.ID == this.ID)).ToArray<BattleFieldEffectStage>();

    public float[] busLevel => new float[8]
    {
      this.bus_id_0_level,
      this.bus_id_1_level,
      this.bus_id_2_level,
    ...
    public static BattleStage Parse(MasterDataReader reader) => new BattleStage()
    {
      ID = reader.ReadInt(),
