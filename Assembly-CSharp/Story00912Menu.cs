...
public class Story00912Menu : BackButtonMenuBase
{
  ...
  private IEnumerator SetCharacterLargeImage(
    int id,
    int id2,
    int id3,
  ...
  {
    Story00912Menu story00912Menu = this;
    ...
    source.OrderBy<PlayerHarmonyQuestS, int>((Func<PlayerHarmonyQuestS, int>) (x => x.quest_harmony_s.ID)).ForEach<PlayerHarmonyQuestS>((System.Action<PlayerHarmonyQuestS>) (qu =>
    {
    ...
    for (int index = 0; index < questsList.Count; ++index)
    {
      ...
      story00912Menu.ScrollContainer.Add(gameObject);
      gameObject.transform.localPosition = new Vector3(gameObject.transform.localPosition.x, (float) -index * story00912Menu.ScrollContainer.grid.cellHeight, gameObject.transform.localPosition.z);
      gameObject.GetComponent<AnimationApplyBase>().Reset();
      gameObject.GetComponent<Story0096EpisodeParts>().setData(questsList[index], playbackList[index], (NGMenuBase) story00912Menu, continuousDataList);
    }
  }
