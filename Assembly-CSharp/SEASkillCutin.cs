...
public class SEASkillCutin : MonoBehaviour
{
  ...
  public static IEnumerator Show(Transform parent, PlayerUnit unit, int settingSpeed)
  {
    ...
    if (unit.cache_overkillers_unit_ids != null && unit.cache_overkillers_unit_ids.Length != 0 && unit.cache_overkillers_unit_ids.Length == unit.cache_overkillers_unit_job_ids.Length)
    {
      ...
      {
        Future<UnityEngine.Sprite> spriteF = killersArray[i].LoadSpriteLarge(unit.cache_overkillers_unit_job_ids[i], 100f);
        e = spriteF.Wait();
        while (e.MoveNext())
  ...
  public IEnumerator Initialize(List<UnityEngine.Sprite> sprites, int settingSpeed)
  {
    ...
    ParticleSystem[] componentsInChildren = seaSkillCutin.GetComponentsInChildren<ParticleSystem>(true);
    float num2;
    switch (settingSpeed)
    {
      case 1:
    ...
    foreach (ParticleSystem particleSystem in componentsInChildren)
    {
      ...
      string name = particleSystem.gameObject.name;
      if (name.Equals("eff_Sparks") || name.Equals("eff_sparks01") || name.Equals("eff_sparks02"))
        main.simulationSpeed *= num2;
      else
        main.simulationSpeed /= (float) settingSpeed;
    }
    for (int index = 0; index < seaSkillCutin.BG_Tex_Roop.Length; ++index)
    {
      UnityEngine.Material material = new UnityEngine.Material(seaSkillCutin.BG_Tex_Roop[index].material);
      Vector4 vector4 = new Vector4(seaSkillCutin.BgTexRoopSpeed[index].x / (float) settingSpeed, seaSkillCutin.BgTexRoopSpeed[index].y / (float) settingSpeed);
      material.SetVector("_UVScroll", vector4);
      seaSkillCutin.BG_Tex_Roop[index].material = material;
    }
  ...
  private IEnumerator PlaySE(string SoundEffectName, float delayTime = 0.0f)
  {
    ...
    if (!((Object) null == (Object) sm) && !string.IsNullOrEmpty(SoundEffectName))
    {
