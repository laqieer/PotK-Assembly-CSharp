...
public static class SMManager
{
  ...
  public static void DeleteList<T>(object[] ids) where T : KeyCompare
  {
    if (ids.Length == 0 || !SMManager.Contains<T[]>())
      return;
    SMManager.Value obj = SMManager.dic.Get()[typeof (T[])];
    if (!(obj.obj is T[] objArray) || objArray.Length == 0 || !objArray[0].hasKey)
      return;
    Dictionary<object, T> dictionary = ((IEnumerable<T>) objArray).ToDictionary<T, object>((Func<T, object>) (x => x.Key));
  ...
  public static T Get<T>() where T : class => !SMManager.Contains<T>() ? default (T) : (T) SMManager.dic.Get()[typeof (T)].obj;

  public static long Revision<T>() where T : class => !SMManager.Contains<T>() ? 0L : SMManager.dic.Get()[typeof (T)].revision;
