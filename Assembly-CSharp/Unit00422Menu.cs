...
public class Unit00422Menu : BackButtonMenuBase
{
  ...
  {
    this.lpivot_Friendly_Gauge.width = 0;
  ...
  private IEnumerator InitPlayer(PlayerUnit playerUnit)
  {
    ...
    for (int i = 0; i < characterIntimateLength; ++i)
    {
      ...
      GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(futurePrefab.Result);
      ...
      unitIcon.onClick = new System.Action<UnitIconBase>(unit00422Menu.\u003CInitPlayer\u003Eb__20_1);
    ...
    unit00422Menu.lpivot_Friendly_Gauge.width = 0;
  ...
  protected virtual void SetBottomMode(UnitIcon icon) => icon.BottomModeValue = UnitIconBase.BottomMode.Friendly;

  protected virtual void SetPosessionText(int value, int max) => this.TxtOwnnumber.SetTextLocalize(string.Format("{0}/{1}", (object) value, (object) max));
  ...
  private IEnumerator SetMainCharacter(PlayerUnit playerUnit)
  {
    ...
    Future<UnityEngine.Sprite> futureSprite = playerUnit.unit.LoadSpriteLarge();
    IEnumerator e = futureSprite.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    this.mask_Chara.sprite2D = futureSprite.Result;
    Future<UnityEngine.Sprite> futureMask = Res.GUI._004_2_2_sozai.mask_Chara.Load<UnityEngine.Sprite>();
    e = futureMask.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
  ...
  private void UpdateSelectCharacter(UnitIconBase unitIcon)
  {
    ...
    this.allUnitIcon.ForEach((System.Action<UnitIcon>) (icon => this.SetFriendlyEffect((UnitIconBase) icon, false)));
    this.SetFriendlyEffect(unitIcon, true);
  }

  protected virtual void SetFriendlyEffect(UnitIconBase icon, bool value) => icon.SetFriendlyEffect(value);
