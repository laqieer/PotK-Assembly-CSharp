...
public class Startup00014BonusIcon
{
  ...
  public void Clear()
  {
    foreach (Component iconPosition in this.iconPositionList)
      UnityEngine.Object.Destroy((UnityEngine.Object) iconPosition.gameObject);
    foreach (UnityEngine.Object icon in this.IconList)
  ...
  private IEnumerator SetIcons(List<LoginbonusReward> loginBonusRewardList, int s_index)
  {
    ...
    else
    {
      for (int i = 0; i < this.iconPositionList.Count; ++i)
      {
        ...
        CreateIconObject target = this.iconPositionList[i].gameObject.GetOrAddComponent<CreateIconObject>();
  ...
  public void ListEnable(bool flag) => this.IconList.ForEach((System.Action<GameObject>) (x => x.SetActive(flag)));
}
