...
public class Guild0282MemberScroll : MonoBehaviour
{
  ...
  private IEnumerator SetData(GuildMembership info)
  {
    ...
    this.txt_nonparticipation.gameObject.SetActive(onGvg && !info.is_defense_membership);
    ...
    if (onGvg)
    {
      ...
      for (int index = 0; index < this.battleCountIcon.Length; ++index)
      {
        ...
        if ((UnityEngine.Object) battleCountIcon.objOff != (UnityEngine.Object) null)
  ...
  private IEnumerator SetLastLoginText(bool onGvg, GuildMembership info)
  {
    if (onGvg)
    {
      this.txt_last_login_at.gameObject.SetActive(false);
    }
    else
    {
      ...
      this.txt_last_login_at.gameObject.SetActive(true);
  ...
  public void onDefenseMemberButton()
  {
    ...
    {
      this.defenseListPopup.SelectedKeyPriorityValuePlayerIDDic.ForEach<KeyValuePair<int, string>>((System.Action<KeyValuePair<int, string>>) (x =>
      {
  ...
  {
    this.dir_defense_member_select.SetActive((uint) this.guildMemberInfo.defense_priority > 0U);
    if (this.guildMemberInfo.defense_priority == 0)
      return;
