...
public class Battle01PVPEnemyUnits : NGBattleMenuBase
{
  ...
  private IEnumerator cloneUnitThum(BL.Unit unit, GameObject prefab, Transform parent)
  {
    ...
    up.onClick = (System.Action<UnitIconBase>) (x => this.OpenPopupInfo(unit));
  }

  public void OpenPopupInfo(BL.Unit unit) => Singleton<PopupManager>.GetInstance().open(this.popupInfoPrefab).GetComponent<BattleUI01UnitInformation>().InitFromPVP(unit, false);
}
