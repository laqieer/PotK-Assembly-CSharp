...
public class NGSoundManager : Singleton<NGSoundManager>
{
  ...
  private bool delayReinitialize;
  ...
  public List<CriAtomEx.CueInfo> GetBGMCueList()
  {
    return ((IEnumerable<CriAtomEx.CueInfo>) this.bgmAcb.GetCueInfoList()).ToList<CriAtomEx.CueInfo>();
  }

  public List<CriAtomEx.CueInfo> GetSECueList()
  {
    return ((IEnumerable<CriAtomEx.CueInfo>) this.seAcb.GetCueInfoList()).ToList<CriAtomEx.CueInfo>();
  }

  public List<CriAtomEx.CueInfo> GetAcbCueList(string acbFile)
  {
    return !this.checkSoundData(this.voiceCueSheetName, this.maxStreamVoice, acbFile) ? new List<CriAtomEx.CueInfo>() : ((IEnumerable<CriAtomEx.CueInfo>) CriAtom.GetAcb(acbFile).GetCueInfoList()).ToList<CriAtomEx.CueInfo>();
  }

  public static string platform => "android";

  protected override void Initialize() => this.isInitialize = false;
  ...
  public void CheckInitialize(bool forcingCompleted = false)
  {
    ...
    if (!forcingCompleted)
    {
      if (this.initializeState == NGSoundManager.InitializeState.DLC_BEFORE && (!ResourceDownloader.Completed || ResourceDownloader.Error != null))
        return;
      if (this.delayReinitialize)
        return;
    }
    try
    {
      if (ResourceDownloader.Completed && ResourceDownloader.Error == null || forcingCompleted)
      {
        ...
        CriAtom.AddCueSheet("AA", Singleton<ResourceManager>.GetInstance().ResolveStreamingAssetsPath(NGSoundManager.platform + "/" + this.afterBgmName + "_acb"), Singleton<ResourceManager>.GetInstance().ResolveStreamingAssetsPath(NGSoundManager.platform + "/" + this.afterBgmName + "_awb"));
        CriAtom.AddCueSheet("BB", Singleton<ResourceManager>.GetInstance().ResolveStreamingAssetsPath(NGSoundManager.platform + "/" + this.afterSeName + "_acb"), (string) null);
    ...
    float delay = 0.0f)
  {
    ...
    return (IEnumerator) new NGSoundManager.\u003CFade\u003Ec__Iterator8EA()
    {
      delay = delay,
      duration = duration,
      player = player,
  ...
  public int PlaySe(string clip, bool isLoop = false, float delay = 0.0f, int channel = -1, bool delayReinitialize = false)
  {
    this.delayReinitialize = delayReinitialize;
    this.CheckInitialize();
  ...
  {
    if (id == "VO_9999" || !this.isInitialize || !this.checkSoundData(this.voiceCueSheetName, this.maxStreamVoice, id))
      return -1;
  ...
  {
    // ISSUE: object of a compiler-generated type is created
    return (IEnumerator) new NGSoundManager.\u003CdelayPlay\u003Ec__Iterator8EB()
    {
      delay = delay,
      player = player,
      \u003C\u0024\u003Edelay = delay,
  ...
  {
    // ISSUE: object of a compiler-generated type is created
    return (IEnumerator) new NGSoundManager.\u003CpauseCorutine\u003Ec__Iterator8EC()
    {
      player = player,
      \u003C\u0024\u003Eplayer = player
    };
  ...
  private bool checkSoundData(List<string> fileList, int maxValue, string file)
  {
    ...
    string name = file;
    if (!Singleton<ResourceManager>.GetInstance().Contains(NGSoundManager.platform + "/" + name + "_acb") || !Singleton<ResourceManager>.GetInstance().Contains(NGSoundManager.platform + "/" + name + "_acb"))
      return false;
    string str1 = Singleton<ResourceManager>.GetInstance().ResolveStreamingAssetsPath(NGSoundManager.platform + "/" + name + "_acb");
    string str2 = Singleton<ResourceManager>.GetInstance().ResolveStreamingAssetsPath(NGSoundManager.platform + "/" + name + "_awb");
    ...
    float delay = 0.0f)
  {
    ...
    NGSoundManager.\u003CBgmCrossFade\u003Ec__Iterator8ED fadeCIterator8Ed = new NGSoundManager.\u003CBgmCrossFade\u003Ec__Iterator8ED();
    return (IEnumerator) fadeCIterator8Ed;
  }
  ...
  {
    // ISSUE: object of a compiler-generated type is created
    return (IEnumerator) new NGSoundManager.\u003CBgmCrossFade\u003Ec__Iterator8EE()
    {
      player = player,
      aisac = aisac,
      duration = duration,
