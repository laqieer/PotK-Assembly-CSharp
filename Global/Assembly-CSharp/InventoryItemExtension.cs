...
public static class InventoryItemExtension
{
  ...
  public static void Check(this List<InventoryItem> xs)
  {
    foreach (InventoryItem inventoryItem in xs)
      Debug.Log((object) ("------SelectFlag[" + (object) inventoryItem.select + "]--------ID:" + (object) inventoryItem.Item.id + "--------Index:" + (object) inventoryItem.index + "--------slCount:" + (object) inventoryItem.selectCount));
  }
  ...
  {
    IEnumerable<InventoryItem> inventoryItems = xs.Where<InventoryItem>((Func<InventoryItem, bool>) (x => x.Item._entity_type == 3));
