...
public class BattleCameraController : BattleMonoBehaviour
{
  ...
  protected float translateSmoothTime = 0.17f;
  ...
  public void onDrag(Vector2 delta)
  {
    if (Object.op_Equality((Object) this.cCamera, (Object) null))
      return;
    this.screenPoint.x -= delta.x * this.dragBoost;
    this.screenPoint.y -= delta.y * this.dragBoost;
  ...
  public void onPinch(float val)
  {
    this.sightDistance += val / 100f;
    if ((double) this.sightDistance < (double) this.battleManager.sightDistances[0])
      this.sightDistance = this.battleManager.sightDistances[0];
    else if ((double) this.sightDistance > (double) this.battleManager.sightDistances[this.battleManager.sightDistances.Length - 1])
      this.sightDistance = this.battleManager.sightDistances[this.battleManager.sightDistances.Length - 1];
    this.cameraTransform.localPosition = new Vector3(0.0f, 0.0f, -this.sightDistance);
  }
}
