...
namespace Earth
{
  ...
  public class EarthCharacter : BL.ModelBase
  {
    ...
    {
      return (double) Random.value < (double) Mathf.Min(0.95f, growth) ? 1 : 0;
    }
    ...
    public void AddExperience(int experience, Dictionary<int, int> gearProficiencyExperiences)
    {
      ...
      if (this.unitLevel.level > unitLevel.level)
      {
        int num1 = this.unitLevel.level - unitLevel.level;
      ...
      foreach (KeyValuePair<int, int> proficiencyExperience in gearProficiencyExperiences)
      {
        ...
        {
          Dictionary<int, int> proficiencyExperiences;
          int key;
          (proficiencyExperiences = this.mGearProficiencyExperiences)[key = proficiencyExperience.Key] = proficiencyExperiences[key] + proficiencyExperience.Value;
        }
        else
    ...
    public int EquipGear(int gear_id)
    {
      ...
      if (this.mPlayerUnit != (PlayerUnit) null)
        this.mPlayerUnit.equip_gear_ids[0] = this.mEquipeGearID == 0 ? new int?() : new int?(this.mEquipeGearID);
    ...
    public PlayerUnit GetPlayerUnit(bool isCopy = false)
    {
      ...
      this.mPlayerUnit.max_level = this.mPlayerUnit.unit.parameter_data._initial_max_level;
      this.mPlayerUnit.move = this.mPlayerUnit.unit.job.movement;
      this.mPlayerUnit.hp.initial = this.mPlayerUnit.unit.hp_initial + this.mPlayerUnit.unit.job.hp_initial;
      this.mPlayerUnit.hp.level = this.mLevelupHp;
      this.mPlayerUnit.strength.initial = this.mPlayerUnit.unit.strength_initial + this.mPlayerUnit.unit.job.strength_initial;
      this.mPlayerUnit.strength.level = this.mLevelupStrength;
      this.mPlayerUnit.intelligence.initial = this.mPlayerUnit.unit.intelligence_initial + this.mPlayerUnit.unit.job.intelligence_initial;
      this.mPlayerUnit.intelligence.level = this.mLevelupIntelligence;
      this.mPlayerUnit.vitality.initial = this.mPlayerUnit.unit.vitality_initial + this.mPlayerUnit.unit.job.vitality_initial;
      this.mPlayerUnit.vitality.level = this.mLevelupVitality;
      this.mPlayerUnit.mind.initial = this.mPlayerUnit.unit.mind_initial + this.mPlayerUnit.unit.job.mind_initial;
      this.mPlayerUnit.mind.level = this.mLevelupMind;
      this.mPlayerUnit.agility.initial = this.mPlayerUnit.unit.agility_initial + this.mPlayerUnit.unit.job.agility_initial;
      this.mPlayerUnit.agility.level = this.mLevelupAgility;
      this.mPlayerUnit.dexterity.initial = this.mPlayerUnit.unit.dexterity_initial + this.mPlayerUnit.unit.job.dexterity_initial;
      this.mPlayerUnit.dexterity.level = this.mLevelupDexterity;
      this.mPlayerUnit.lucky.initial = this.mPlayerUnit.unit.lucky_initial + this.mPlayerUnit.unit.job.lucky_initial;
    ...
    public List<PlayerUnit> GetEvolutionPlayerUnits(int[] patternIDs)
    {
      ...
      foreach (int patternId in patternIDs)
      {
        ...
        {
          PlayerUnit targetUnit = new PlayerUnit();
          targetUnit.hp = new PlayerUnitHp();
          targetUnit.strength = new PlayerUnitStrength();
          targetUnit.intelligence = new PlayerUnitIntelligence();
          targetUnit.vitality = new PlayerUnitVitality();
          targetUnit.mind = new PlayerUnitMind();
          targetUnit.agility = new PlayerUnitAgility();
          targetUnit.dexterity = new PlayerUnitDexterity();
          targetUnit.lucky = new PlayerUnitLucky();
          targetUnit.equip_gear_ids = new int?[1];
          targetUnit._unit = evolutionPattern.target_unit.ID;
          targetUnit.id = this.mCharactorID;
          targetUnit.is_enemy = false;
          targetUnit.favorite = false;
          targetUnit._unit_type = 1;
          int exp = 0;
          UnitLevel unitLevel = ((IEnumerable<UnitLevel>) MasterData.UnitLevelList).Where<UnitLevel>((Func<UnitLevel, bool>) (x => x.pattern_id == targetUnit.unit.parameter_data._level_pattern_id)).FirstOrDefault<UnitLevel>((Func<UnitLevel, bool>) (x => x.from_exp <= exp && x.to_exp >= exp));
          targetUnit.total_exp = exp;
          targetUnit.exp = exp - unitLevel.from_exp;
          targetUnit.exp_next = unitLevel.to_exp - exp;
          targetUnit.level = unitLevel.level;
          targetUnit.max_level = targetUnit.unit.parameter_data._initial_max_level;
          targetUnit.move = targetUnit.unit.job.movement;
          targetUnit.hp.initial = targetUnit.unit.hp_initial + targetUnit.unit.job.hp_initial;
          targetUnit.hp.level = this.mLevelupHp;
          targetUnit.strength.initial = targetUnit.unit.strength_initial + targetUnit.unit.job.strength_initial;
          targetUnit.strength.level = this.mLevelupStrength;
          targetUnit.intelligence.initial = targetUnit.unit.intelligence_initial + targetUnit.unit.job.intelligence_initial;
          targetUnit.intelligence.level = this.mLevelupIntelligence;
          targetUnit.vitality.initial = targetUnit.unit.vitality_initial + targetUnit.unit.job.vitality_initial;
          targetUnit.vitality.level = this.mLevelupVitality;
          targetUnit.mind.initial = targetUnit.unit.mind_initial + targetUnit.unit.job.mind_initial;
          targetUnit.mind.level = this.mLevelupMind;
          targetUnit.agility.initial = targetUnit.unit.agility_initial + targetUnit.unit.job.agility_initial;
          targetUnit.agility.level = this.mLevelupAgility;
          targetUnit.dexterity.initial = targetUnit.unit.dexterity_initial + targetUnit.unit.job.dexterity_initial;
          targetUnit.dexterity.level = this.mLevelupDexterity;
          targetUnit.lucky.initial = targetUnit.unit.lucky_initial + targetUnit.unit.job.lucky_initial;
          targetUnit.lucky.level = this.mLevelupLucky;
          targetUnit.equip_gear_ids[0] = new int?();
          if (this.mEquipeGearID != 0)
            targetUnit.equip_gear_ids[0] = new int?(this.mEquipeGearID);
          List<PlayerUnitGearProficiency> unitGearProficiencyList = new List<PlayerUnitGearProficiency>();
          foreach (KeyValuePair<int, int> proficiencyExperience in this.mGearProficiencyExperiences)
          ...
          targetUnit.gear_proficiencies = unitGearProficiencyList.ToArray();
          targetUnit.leader_skills = new PlayerUnitLeader_skills[0];
          targetUnit.skills = ((IEnumerable<UnitSkill>) MasterData.UnitSkillList).Where<UnitSkill>((Func<UnitSkill, bool>) (x => x.unit.ID == targetUnit.unit.ID && x.level <= unitLevel.level)).Select<UnitSkill, PlayerUnitSkills>((Func<UnitSkill, PlayerUnitSkills>) (x => new PlayerUnitSkills()
          {
            skill_id = x.skill.ID,
            level = 1
          })).ToArray<PlayerUnitSkills>();
          evolutionPlayerUnits.Add(targetUnit);
        }
      }
