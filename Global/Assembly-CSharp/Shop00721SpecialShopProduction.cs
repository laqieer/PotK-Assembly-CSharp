...
public class Shop00721SpecialShopProduction : MonoBehaviour
{
  ...
  private void SetTimeString(DateTime serverTime, DateTime? endTime)
  {
    ...
    else
    {
      ...
      this.txtTime.SetTextLocalize(Consts.Lookup("SHOP_0074_SCROLL_UPDATE_SERVER_TIME", (IDictionary) new Hashtable()
      {
        {
          (object) "time",
  ...
  private void SetRechargeTime(DateTime serverTime, PlayerShopArticle article)
  {
    ...
    this.txtProductNum.SetTextLocalize(Consts.Lookup("SHOP_00721_SPECIAL_SHOP_PRODUCTION_UPDATE_SERVER_TIME", (IDictionary) new Hashtable()
    {
      {
        (object) "time",
        (object) string.Format(Consts.Lookup("SHOP_00721_SPECIAL_SHOP_PRODUCTION_UPDATE_SERVER_TIME2"), (object) num1, (object) num2)
      }
    }));
  }
    ...
    if (article.article.limit.HasValue || article.article.daily_limit.HasValue)
    {
      ...
      else
      {
        ...
        if (article.article.unit == string.Empty)
          this.txtProductNum.SetTextLocalize(string.Format(Consts.Lookup("SHOP_0074_SCROLL_ARTICLE_LIMIT_VALUE"), (object) article.limit.Value));
        else
          this.txtProductNum.SetTextLocalize(Consts.Lookup("SHOP_0074_SCROLL_ARTICLE_LIMIT_VALUE_EXT", (IDictionary) new Hashtable()
          {
            {
              (object) "num",
    ...
    int idx)
  {
    ...
    return (IEnumerator) new Shop00721SpecialShopProduction.\u003CInit\u003Ec__Iterator3E1()
    {
      idx = idx,
      menu = menu,
      shopID = shopID,
