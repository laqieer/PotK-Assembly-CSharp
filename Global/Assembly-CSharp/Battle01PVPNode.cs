...
public class Battle01PVPNode : BattleMonoBehaviour
{
  ...
  protected override IEnumerator Start_Battle()
  {
    ...
    return (IEnumerator) new Battle01PVPNode.\u003CStart_Battle\u003Ec__Iterator709()
    {
      \u003C\u003Ef__this = this
    };
  }
  ...
  {
    if (this.isDead || !this.battleManager.isPvp || !this.phaseStateModified.isChangedOnce())
      return;
    switch (this.phaseStateModified.value.state)
    {
      ...
      case BL.Phase.pvp_exception:
        ...
        if (flag)
        {
          foreach (BL.Panel panel in this.pvpManager.formationPanel)
            panel.setAttribute(BL.PanelAttribute.playermove);
          this.setCameraTarget(this.pvpManager.formationPanel);
          this.inputObserver.setDispositionMode(this.pvpManager.formationPanel);
        }
        else
        {
          foreach (BL.Panel panel in this.pvpManager.formationPanel)
  ...
  {
    // ISSUE: object of a compiler-generated type is created
    return (IEnumerator) new Battle01PVPNode.\u003CdoDead\u003Ec__Iterator70A()
    {
      \u003C\u003Ef__this = this
    };
  }
  ...
  private void setCameraTarget(HashSet<BL.Panel> pl)
  {
    ...
    Vector3 vector3_2 = this.pvpManager.playerOrder != 0 ? Vector3.op_UnaryNegation(this.cameraOffset) : this.cameraOffset;
