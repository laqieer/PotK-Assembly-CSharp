...
public class BattleInputObserver : BattleMonoBehaviour
{
  ...
  protected override IEnumerator Start_Battle()
  {
    ...
    return (IEnumerator) new BattleInputObserver.\u003CStart_Battle\u003Ec__Iterator832()
    {
      \u003C\u003Ef__this = this
    };
  }
  ...
  private void OnDisable()
  {
    ...
    this.pinchUtil.onPinch = (Action<float>) null;
  }
      ...
      if (value != null)
      {
        foreach (BL.Unit target in value.targets)
        {
          ...
          if (value.selectFunc != null)
            this.objectManager.setButton(fieldPanel, value.isHeal);
        }
      }
  ...
  public void setTargetSelectMode(List<BL.Unit> targets, bool isOwn, Action<BL.Unit> func)
  {
    ...
    this.selectTargets = new BattleInputObserver.SelectTarget(targets, isOwn, func);
    this.cameraController.onCancel();
    this.unitController.onCancel();
  ...
  {
    if (!this.battleManager.isBattleEnable || this.pinchUtil.IsPinching)
      return;
  ...
  private class SelectTarget
  {
    ...
    public SelectTarget(List<BL.Unit> targets, bool isHeal, Action<BL.Unit> selectFunc)
    {
