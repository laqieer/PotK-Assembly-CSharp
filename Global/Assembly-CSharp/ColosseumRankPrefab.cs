...
public class ColosseumRankPrefab : MonoBehaviour
{
  ...
  public void Reset(GameObject unknownObject)
  {
    ...
    this.textRankReward.SetTextLocalize(Consts.Lookup("COLOSSEUM_RANK_PREFAB_RESET_01"));
    this.textRankReward.color = this.gray;
    this.textRewardName.text = string.Empty;
    this.textRewardName.color = this.gray;
    this.textPoint.SetTextLocalize(Consts.Lookup("COLOSSEUM_RANK_PREFAB_RESET_02"));
    this.textRankName.SetTextLocalize(Consts.Lookup("COLOSSEUM_RANK_PREFAB_RESET_03"));
    ...
    Action<ColosseumRank, int, int> rankDetailAction)
  {
    ...
    return (IEnumerator) new ColosseumRankPrefab.\u003CSet\u003Ec__Iterator89E()
    {
      nextPoint = nextPoint,
      fromPoint = fromPoint,
      rankInfo = rankInfo,
