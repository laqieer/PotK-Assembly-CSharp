...
namespace GooglePlayGames.Native
{
  public class NativeRealtimeMultiplayerClient : IRealTimeMultiplayerClient
  {
      ...
      RealTimeMultiplayerListener listener)
    {
      lock (this.mSessionLock)
      {
        ...
        else
        {
          ...
          {
            RealtimeRoomConfig config = roomConfigBuilder.SetMinimumAutomatchingPlayers(minOpponents).SetMaximumAutomatchingPlayers(maxOpponents).SetVariant(variant).SetExclusiveBitMask(exclusiveBitMask).Build();
            using (config)
            {
              using (GooglePlayGames.Native.PInvoke.RealTimeEventListenerHelper helper = NativeRealtimeMultiplayerClient.HelperForSession(newSession))
                newSession.StartRoomCreation(this.mNativeClient.GetUserId(), (Action) (() => this.mRealtimeManager.CreateRoom(config, helper, new Action<RealtimeManager.RealTimeRoomResponse>(newSession.HandleRoomResponse))));
            }
          }
        }
      }
    ...
    private static GooglePlayGames.Native.PInvoke.RealTimeEventListenerHelper HelperForSession(
      NativeRealtimeMultiplayerClient.RoomSession session)
    {
      ...
      RealTimeMultiplayerListener listener)
    {
      lock (this.mSessionLock)
      {
        ...
        else
        {
          ...
          this.mRealtimeManager.ShowPlayerSelectUI(minOpponents, maxOppponents, true, (Action<PlayerSelectUIResponse>) (response =>
          {
            ...
            {
              this.mCurrentSession.MinPlayersToStart = (uint) ((int) response.MinimumAutomatchingPlayers() + response.Count<string>() + 1);
              using (RealtimeRoomConfigBuilder roomConfigBuilder = RealtimeRoomConfigBuilder.Create())
              {
                roomConfigBuilder.SetVariant(variant);
                roomConfigBuilder.PopulateFromUIResponse(response);
                using (RealtimeRoomConfig config = roomConfigBuilder.Build())
                {
                  using (GooglePlayGames.Native.PInvoke.RealTimeEventListenerHelper helper = NativeRealtimeMultiplayerClient.HelperForSession(newRoom))
                    newRoom.StartRoomCreation(this.mNativeClient.GetUserId(), (Action) (() => this.mRealtimeManager.CreateRoom(config, helper, new Action<RealtimeManager.RealTimeRoomResponse>(newRoom.HandleRoomResponse))));
                }
              }
            }
          }));
    ...
    public void AcceptFromInbox(RealTimeMultiplayerListener listener)
    {
      lock (this.mSessionLock)
      {
        ...
        else
        {
          this.mCurrentSession = newRoom;
          this.mCurrentSession.ShowingUI = true;
          this.mRealtimeManager.ShowRoomInboxUI((Action<RealtimeManager.RoomInboxUIResponse>) (response =>
          {
            this.mCurrentSession.ShowingUI = false;
            ...
            else
            {
              ...
              using (GooglePlayGames.Native.PInvoke.RealTimeEventListenerHelper helper = NativeRealtimeMultiplayerClient.HelperForSession(newRoom))
              {
                ...
                newRoom.StartRoomCreation(this.mNativeClient.GetUserId(), (Action) (() => this.mRealtimeManager.AcceptInvitation(invitation, helper, (Action<RealtimeManager.RealTimeRoomResponse>) (acceptResponse =>
                {
                  using (invitation)
                  {
    ...
    public void AcceptInvitation(string invitationId, RealTimeMultiplayerListener listener)
    {
      ...
      NativeRealtimeMultiplayerClient.\u003CAcceptInvitation\u003Ec__AnonStoreyC0F invitationCAnonStoreyC0F1 = new NativeRealtimeMultiplayerClient.\u003CAcceptInvitation\u003Ec__AnonStoreyC0F();
      // ISSUE: reference to a compiler-generated field
      invitationCAnonStoreyC0F1.invitationId = invitationId;
      // ISSUE: reference to a compiler-generated field
      invitationCAnonStoreyC0F1.\u003C\u003Ef__this = this;
      lock (this.mSessionLock)
      {
        NativeRealtimeMultiplayerClient.RoomSession newRoom = new NativeRealtimeMultiplayerClient.RoomSession(this.mRealtimeManager, listener);
        if (this.mCurrentSession.IsActive())
        ...
        else
        {
          ...
          this.mRealtimeManager.FetchInvitations((Action<RealtimeManager.FetchInvitationsResponse>) (response =>
          {
            ...
            {
              // ISSUE: variable of a compiler-generated type
              NativeRealtimeMultiplayerClient.\u003CAcceptInvitation\u003Ec__AnonStoreyC0F invitationCAnonStoreyC0F = invitationCAnonStoreyC0F1;
              // ISSUE: variable of a compiler-generated type
              NativeRealtimeMultiplayerClient.\u003CAcceptInvitation\u003Ec__AnonStoreyC0E invitationCAnonStoreyC0E = this;
              foreach (GooglePlayGames.Native.PInvoke.MultiplayerInvitation invitation1 in response.Invitations())
              {
                GooglePlayGames.Native.PInvoke.MultiplayerInvitation invitation = invitation1;
                using (invitation)
                {
                  if (invitation.Id().Equals(invitationId))
                  {
                    ...
                    Logger.d("Setting MinPlayersToStart with invitation to : " + (object) this.mCurrentSession.MinPlayersToStart);
                    using (GooglePlayGames.Native.PInvoke.RealTimeEventListenerHelper helper = NativeRealtimeMultiplayerClient.HelperForSession(newRoom))
                    {
                      newRoom.StartRoomCreation(this.mNativeClient.GetUserId(), (Action) (() => this.mRealtimeManager.AcceptInvitation(invitation, helper, new Action<RealtimeManager.RealTimeRoomResponse>(newRoom.HandleRoomResponse))));
                      return;
                    }
                  }
                }
    ...
    public Invitation GetInvitation() => this.mCurrentSession.GetInvitation();

    public void LeaveRoom() => this.mCurrentSession.LeaveRoom();
    ...
    public Participant GetSelf() => this.mCurrentSession.GetSelf();
    ...
    public bool IsRoomConnected() => this.mCurrentSession.IsRoomConnected();
    ...
    {
      return (object) presented != null ? presented : defaultValue;
    }
    ...
    private class RoomSession
    {
      ...
      {
        get => this.mShowingUI;
        set => this.mShowingUI = value;
      }

      internal uint MinPlayersToStart
      {
        get => this.mMinPlayersToStart;
        set => this.mMinPlayersToStart = value;
      }

      internal RealtimeManager Manager() => this.mManager;

      internal bool IsActive() => this.mState.IsActive();

      internal string SelfPlayerId() => this.mCurrentPlayerId;

      public void SetInvitation(Invitation invitation) => this.mInvitation = invitation;

      public Invitation GetInvitation() => this.mInvitation;
      ...
      internal void EnterState(
        NativeRealtimeMultiplayerClient.State handler,
        bool fireStateEnteredEvent)
      {
      ...
      internal void ShowWaitingRoomUI() => this.mState.ShowWaitingRoomUI(this.MinPlayersToStart);
      ...
      internal void OnParticipantStatusChanged(
        NativeRealTimeRoom room,
        GooglePlayGames.Native.PInvoke.MultiplayerParticipant participant)
      {
        lock (this.mLifecycleLock)
      ...
      internal void OnDataReceived(
        NativeRealTimeRoom room,
        GooglePlayGames.Native.PInvoke.MultiplayerParticipant sender,
        byte[] data,
        bool isReliable)
      {
      ...
      internal virtual Participant GetSelf() => this.mState.GetSelf();
      ...
      internal virtual bool IsRoomConnected() => this.mState.IsRoomConnected();
    }
    ...
    internal abstract class State
    {
      ...
      internal virtual void OnParticipantStatusChanged(
        NativeRealTimeRoom room,
        GooglePlayGames.Native.PInvoke.MultiplayerParticipant participant)
      {
    ...
    private abstract class MessagingEnabledState : NativeRealtimeMultiplayerClient.State
    {
      ...
      internal MessagingEnabledState(
        NativeRealtimeMultiplayerClient.RoomSession session,
        NativeRealTimeRoom room)
      {
      ...
      internal override sealed void OnParticipantStatusChanged(
        NativeRealTimeRoom room,
        GooglePlayGames.Native.PInvoke.MultiplayerParticipant participant)
      {
      ...
      internal virtual void HandleParticipantStatusChanged(
        NativeRealTimeRoom room,
        GooglePlayGames.Native.PInvoke.MultiplayerParticipant participant)
      {
      }
    ...
    private class BeforeRoomCreateStartedState : NativeRealtimeMultiplayerClient.State
    {
      ...
      internal BeforeRoomCreateStartedState(
        NativeRealtimeMultiplayerClient.RoomSession session)
      {
    ...
    private class RoomCreationPendingState : NativeRealtimeMultiplayerClient.State
    {
      ...
      internal RoomCreationPendingState(
        NativeRealtimeMultiplayerClient.RoomSession session)
      {
        this.mContainingSession = Misc.CheckNotNull<NativeRealtimeMultiplayerClient.RoomSession>(session);
      }
      ...
      internal override bool IsActive() => true;
    ...
    private class ConnectingState : NativeRealtimeMultiplayerClient.MessagingEnabledState
    {
        ...
        foreach (GooglePlayGames.Native.PInvoke.MultiplayerParticipant participant in room.Participants())
        {
          using (participant)
          {
            if (participant.IsConnectedToRoom())
              stringSet.Add(participant.Id());
          }
        }
        if (this.mConnectedParticipants.Equals((object) stringSet))
        {
          Logger.w("Received connected set callback with unchanged connected set!");
        }
        else
        {
          IEnumerable<string> source1 = this.mConnectedParticipants.Except<string>((IEnumerable<string>) stringSet);
          ...
          {
            IEnumerable<string> source2 = stringSet.Except<string>((IEnumerable<string>) this.mConnectedParticipants);
            ...
            else
            {
              ...
              this.mConnectedParticipants = stringSet;
              this.mSession.OnGameThreadListener().RoomSetupProgress(this.mPercentComplete);
            }
          }
        }
      ...
      internal override void HandleParticipantStatusChanged(
        NativeRealTimeRoom room,
        GooglePlayGames.Native.PInvoke.MultiplayerParticipant participant)
      {
    ...
    private class ActiveState : NativeRealtimeMultiplayerClient.MessagingEnabledState
    {
      ...
      internal override bool IsRoomConnected() => true;
      ...
      {
        foreach (Participant self in this.mParticipants.Values)
        {
          if (self.Player != null && self.Player.id.Equals(this.mSession.SelfPlayerId()))
            return self;
        }
        return (Participant) null;
      }
      ...
      internal override void HandleConnectedSetChanged(NativeRealTimeRoom room)
      {
        ...
        List<string> list1 = source1.Where<string>((Func<string, bool>) (peerId => !peerId.Equals(selfId))).ToList<string>();
        List<string> list2 = source2.Where<string>((Func<string, bool>) (peerId => !peerId.Equals(selfId))).ToList<string>();
        if (list1.Count > 0)
        {
          list1.Sort();
          this.mSession.OnGameThreadListener().PeersConnected(list1.Where<string>((Func<string, bool>) (peer => !peer.Equals(selfId))).ToArray<string>());
        }
        if (list2.Count <= 0)
          return;
        list2.Sort();
        this.mSession.OnGameThreadListener().PeersDisconnected(list2.Where<string>((Func<string, bool>) (peer => !peer.Equals(selfId))).ToArray<string>());
      }
    ...
    private class ShutdownState : NativeRealtimeMultiplayerClient.State
    {
      ...
      internal ShutdownState(
        NativeRealtimeMultiplayerClient.RoomSession session)
      {
        this.mSession = Misc.CheckNotNull<NativeRealtimeMultiplayerClient.RoomSession>(session);
      }

      internal override bool IsActive() => false;

      internal override void LeaveRoom() => this.mSession.OnGameThreadListener().LeftRoom();
    }
    ...
    private class AbortingRoomCreationState : NativeRealtimeMultiplayerClient.State
    {
      ...
      internal AbortingRoomCreationState(
        NativeRealtimeMultiplayerClient.RoomSession session)
      {
        this.mSession = Misc.CheckNotNull<NativeRealtimeMultiplayerClient.RoomSession>(session);
      }
