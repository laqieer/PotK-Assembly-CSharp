...
namespace GooglePlayGames.Native
{
  internal class NativeSavedGameClient : ISavedGameClient
  {
      ...
      Action<SavedGameRequestStatus, ISavedGameMetadata> completedCallback)
    {
      this.mSnapshotManager.Open(filename, NativeSavedGameClient.AsDataSource(source), Types.SnapshotConflictPolicy.MANUAL, (Action<GooglePlayGames.Native.PInvoke.SnapshotManager.OpenResponse>) (response =>
      {
        ...
        else if (response.ResponseStatus() == CommonErrorStatus.SnapshotOpenStatus.VALID_WITH_CONFLICT)
        {
          ...
          NativeSavedGameClient.NativeConflictResolver resolver = new NativeSavedGameClient.NativeConflictResolver(this.mSnapshotManager, response.ConflictId(), original, unmerged, completedCallback, (Action) (() => this.InternalManualOpen(filename, source, prefetchDataOnConflict, conflictCallback, completedCallback)));
          if (!prefetchDataOnConflict)
          {
            conflictCallback((IConflictResolver) resolver, (ISavedGameMetadata) original, (byte[]) null, (ISavedGameMetadata) unmerged, (byte[]) null);
          }
          else
          {
            NativeSavedGameClient.Prefetcher prefetcher = new NativeSavedGameClient.Prefetcher((Action<byte[], byte[]>) ((originalData, unmergedData) => conflictCallback((IConflictResolver) resolver, (ISavedGameMetadata) original, originalData, (ISavedGameMetadata) unmerged, unmergedData)), completedCallback);
            this.mSnapshotManager.Read(original, new Action<GooglePlayGames.Native.PInvoke.SnapshotManager.ReadResponse>(prefetcher.OnOriginalDataRead));
            this.mSnapshotManager.Read(unmerged, new Action<GooglePlayGames.Native.PInvoke.SnapshotManager.ReadResponse>(prefetcher.OnUnmergedDataRead));
          }
        }
        else
        {
      ...
      Action<SavedGameRequestStatus, byte[]> completedCallback)
    {
      ...
      if (!(metadata is NativeSnapshotMetadata metadata1))
      {
    ...
    private static SavedGameRequestStatus AsRequestStatus(
      CommonErrorStatus.SnapshotOpenStatus status)
    {
    ...
    private class NativeConflictResolver : IConflictResolver
    {
      ...
      internal NativeConflictResolver(
        GooglePlayGames.Native.PInvoke.SnapshotManager manager,
        string conflictId,
        NativeSnapshotMetadata original,
        NativeSnapshotMetadata unmerged,
        Action<SavedGameRequestStatus, ISavedGameMetadata> completeCallback,
        Action retryOpen)
      {
    ...
    private class Prefetcher
    {
      ...
      internal Prefetcher(
        Action<byte[], byte[]> dataFetchedCallback,
        Action<SavedGameRequestStatus, ISavedGameMetadata> completedCallback)
      {
