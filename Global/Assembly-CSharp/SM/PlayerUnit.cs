...
namespace SM
{
  ...
  public class PlayerUnit : KeyCompare
  {
    ...
    public int buildupLimitBreakCnt
    {
      get
      {
        PlayerUnitSkills playerUnitSkills = ((IEnumerable<PlayerUnitSkills>) this.skills).FirstOrDefault<PlayerUnitSkills>((Func<PlayerUnitSkills, bool>) (x => x.skill.skill_type == BattleskillSkillType.growth && x.skill_id == 1000000001));
        return playerUnitSkills != null ? playerUnitSkills.level : 0;
      }
    }
    ...
    {
      PlayerCharacterIntimate characterIntimate = ((IEnumerable<PlayerCharacterIntimate>) (SMManager.Get<PlayerCharacterIntimate[]>() ?? new PlayerCharacterIntimate[0])).SingleOrDefault<PlayerCharacterIntimate>((Func<PlayerCharacterIntimate, bool>) (x =>
      {
        if (x.character.ID == this.unit.character.ID && x.target_character.ID == target.unit.character.ID)
          return true;
        return x.character.ID == target.unit.character.ID && x.target_character.ID == this.unit.character.ID;
      }));
    ...
    {
      return ((IEnumerable<PlayerUnitSkills>) this.skills).Where<PlayerUnitSkills>((Func<PlayerUnitSkills, bool>) (x => x.skill.skill_type != BattleskillSkillType.magic && x.skill.skill_type != BattleskillSkillType.leader)).ToArray<PlayerUnitSkills>();
    }
    ...
    {
      return ResourceManager.Load<RuntimeAnimatorController>(string.Format("Animators/duel/{0}", (object) this.duelAnimatorControllerName));
    }
    ...
    {
      return ResourceManager.Load<RuntimeAnimatorController>(string.Format("Animators/duel_win/{0}", (object) this.winAnimatorControllerName));
    }
