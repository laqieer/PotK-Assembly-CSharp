...
public class Guide0114Menu : GuideZukanListManager
{
  ...
  public IEnumerator onInitMenuAsync()
  {
    ...
    return (IEnumerator) new Guide0114Menu.\u003ConInitMenuAsync\u003Ec__Iterator4C1()
    {
      \u003C\u003Ef__this = this
    };
  }
  ...
  public override void SortAndFilter()
  {
    ...
    {
      if (obj.history_group_number == 0 || obj.published_at == new DateTime() || !(obj.published_at < Singleton<NGGameDataManager>.GetInstance().homeStartUp.last_signed_in_at))
        return;
    ...
    if (gearKindEnumList.Count > 0)
    {
      foreach (GearKindEnum gearKindEnum in gearKindEnumList)
      {
        ...
        foreach (GuideUnitUnit unit1 in this.unitList)
        {
          ...
          if (((IEnumerable<GearGear>) MasterData.GearGearList).Where<GearGear>((Func<GearGear, bool>) (x => x.history_group_number == unit.Gear.history_group_number)).FirstIndexOrNull<GearGear>((Func<GearGear, bool>) (x => (GearKindEnum) x.kind_GearKind == gearKind)).HasValue && !guideUnitUnitList1.Exists((Predicate<GuideUnitUnit>) (x => x.Gear.history_group_number == unit.Gear.history_group_number)))
