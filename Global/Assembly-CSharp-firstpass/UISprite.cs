...
public class UISprite : UIWidget
{
    ...
    switch (this.type)
    {
      ...
      case UISprite.Type.Filled:
        if (!Object.op_Inequality((Object) this.mainTexture, (Object) null) || atlasSprite == null)
          break;
        int num1 = Mathf.RoundToInt(this.atlas.pixelSize * (float) (atlasSprite.width + atlasSprite.paddingLeft + atlasSprite.paddingRight));
  ...
  public override Vector4 drawingDimensions
  {
    get
    {
      ...
      Vector4 vector4 = Vector4.op_Multiply(this.border, this.atlas.pixelSize);
    ...
    BetterList<Color32> cols)
  {
    ...
    Vector2 vector2_2 = Vector2.op_Multiply(vector2_1, this.atlas.pixelSize);
    Color color = this.color;
    color.a = this.finalAlpha;
    Color32 color32 = Color32.op_Implicit(!this.atlas.premultipliedAlpha ? color : NGUITools.ApplyPMA(color));
          ...
          if (this.centerType != UISprite.AdvancedType.Invisible || index1 != 1 || index3 != 1)
          {
            ...
            if (index1 == 1 && index3 == 1)
            {
              if (this.centerType == UISprite.AdvancedType.Tiled)
              {
                ...
                for (float v0y = y1; (double) v0y < (double) y2; v0y += vector2_2.y)
                {
                  float v0x = x1;
                  float u1y = UISprite.mTempUVs[index4].y;
                  float v1y = v0y + vector2_2.y;
                  if ((double) v1y > (double) y2)
                  {
                    u1y = Mathf.Lerp(y3, u1y, (y2 - v0y) / vector2_2.y);
                    v1y = y2;
                  }
                  for (; (double) v0x < (double) x2; v0x += vector2_2.x)
                  {
                    float v1x = v0x + vector2_2.x;
                    float u1x = UISprite.mTempUVs[index2].x;
                    if ((double) v1x > (double) x2)
                    {
                      u1x = Mathf.Lerp(x3, u1x, (x2 - v0x) / vector2_2.x);
                      v1x = x2;
                    }
            ...
            else if (index1 == 1)
            {
              if (index3 == 0 && this.bottomType == UISprite.AdvancedType.Tiled || index3 == 2 && this.topType == UISprite.AdvancedType.Tiled)
              {
                ...
                for (float v0x = x4; (double) v0x < (double) x5; v0x += vector2_2.x)
                {
                  float v1x = v0x + vector2_2.x;
                  float u1x = UISprite.mTempUVs[index2].x;
                  if ((double) v1x > (double) x5)
                  {
                    u1x = Mathf.Lerp(x6, u1x, (x5 - v0x) / vector2_2.x);
                    v1x = x5;
                  }
            ...
            else if (index3 == 1)
            {
              if (index1 == 0 && this.leftType == UISprite.AdvancedType.Tiled || index1 == 2 && this.rightType == UISprite.AdvancedType.Tiled)
              {
                ...
                for (float v0y = y8; (double) v0y < (double) y9; v0y += vector2_2.y)
                {
                  float u1y = UISprite.mTempUVs[index4].y;
                  float v1y = v0y + vector2_2.y;
                  if ((double) v1y > (double) y9)
                  {
                    u1y = Mathf.Lerp(y10, u1y, (y9 - v0y) / vector2_2.y);
                    v1y = y9;
                  }
