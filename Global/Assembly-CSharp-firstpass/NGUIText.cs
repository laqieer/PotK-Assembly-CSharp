...
public static class NGUIText
{
    ...
    for (int index = 0; index < length; ++index)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          if (symbol != null)
          {
            float num4 = num1 + (float) symbol.offsetX * NGUIText.fontScale;
            float num5 = num4 + (float) symbol.width * NGUIText.fontScale;
            if (Mathf.RoundToInt(num1 + (float) symbol.advance * NGUIText.fontScale) > NGUIText.rectWidth)
            {
              sb.Append('\n');
              ++appendLineCount;
          ...
          else
          {
            ...
            if (glyph != null)
            {
              ...
              float num6 = glyph.v0.x + num1;
              float num7 = glyph.v1.x + num1;
              float advance = glyph.advance;
              float num8 = sub != 0 ? (float) (((double) NGUIText.finalSpacingX + (double) glyph.advance) * 0.75) : NGUIText.finalSpacingX + glyph.advance;
              if ((double) NGUIText.finalSpacingX < 0.0)
                advance += NGUIText.finalSpacingX;
              if (Mathf.RoundToInt(num1 + advance) > NGUIText.rectWidth)
              {
                ...
                {
                  num1 = xs[xs.size - 1];
                  num3 = glyph.v0.x + num1;
                  num7 = glyph.v1.x + num1;
                }
                else
                {
                  num3 = num6 - num1;
  ...
  public static string GetStandardizationTextEN(string text) => text;
