...
public class Unit00443indicator : MonoBehaviour
{
  ...
  public IEnumerator InitSkillDialog()
  {
    ...
    return (IEnumerator) new Unit00443indicator.\u003CInitSkillDialog\u003Ec__Iterator31B()
    {
      \u003C\u003Ef__this = this
    };
  }
  ...
  public void Init(GameCore.ItemInfo targetGear)
  {
    Judgement.GearParameter gearParam = Judgement.GearParameter.FromPlayerGear(targetGear);
    this.SetParam(targetGear.gear, gearParam, targetGear.gearLevel, targetGear.gearLevelLimit);
    ...
    float num = (float) this.SlcGauge.width * ((float) targetGear.gearExp / (float) (targetGear.gearExpNext + targetGear.gearExp));
    if ((double) num == 0.0)
    {
