...
public class GearStatusUpParam : MonoBehaviour
{
    ...
    GearKindEnum gearkindEnum)
  {
    ...
    gearMaterialData.Where<GameCore.ItemInfo>((Func<GameCore.ItemInfo, bool>) (x => x.gear.kind.Enum == gearkindEnum)).ToList<GameCore.ItemInfo>().ForEach((Action<GameCore.ItemInfo>) (x => gearMaterialParamRate += !max ? logic.MaterialRank(x.gearLevel) : 0));
