...
public class CommonRewardType
{
  ...
  public CommonRewardType(int type, int id, int quantity, bool is_new = false)
  {
    ...
    {
      this.materialGear = ((IEnumerable<PlayerMaterialGear>) SMManager.Get<PlayerMaterialGear[]>()).Where<PlayerMaterialGear>((Func<PlayerMaterialGear, bool>) (Gear => Gear.id == id)).FirstOrDefault<PlayerMaterialGear>();
      this.gearGear = ((IEnumerable<GearGear>) MasterData.GearGearList).FirstOrDefault<GearGear>((Func<GearGear, bool>) (fd => fd.ID == id));
    }
    else
    {
  ...
  public IEnumerator CreateIcon(Transform parent)
  {
    ...
    return (IEnumerator) new CommonRewardType.\u003CCreateIcon\u003Ec__IteratorBEF()
    {
      parent = parent,
      \u003C\u0024\u003Eparent = parent,
      \u003C\u003Ef__this = this
    ...
    switch (type)
    {
      ...
      case MasterDataTable.CommonRewardType.unit_ticket:
        ...
        Hashtable args2 = new Hashtable()
        {
          {
            (object) "Name",
            (object) (!MasterData.UnitTicket.ContainsKey(id) ? Consts.GetInstance().MYPAGE_0017_UNITTICKET_COMMONNAME : MasterData.UnitTicket[id].short_name)
          },
          {
            (object) "Count",
            (object) quantity
  ...
  public static string GetRewardName(MasterDataTable.CommonRewardType type, int id)
  {
    ...
    switch (type)
    {
      ...
      case MasterDataTable.CommonRewardType.unit_ticket:
        string empty3 = string.Empty;
        return !MasterData.UnitTicket.ContainsKey(id) ? Consts.GetInstance().MYPAGE_0017_UNITTICKET_COMMONNAME : MasterData.UnitTicket[id].short_name;
      default:
  ...
  public static string GetRewardQuantity(MasterDataTable.CommonRewardType type, int id, int quantity)
  {
    ...
    switch (type)
    {
      ...
      case MasterDataTable.CommonRewardType.unit_ticket:
        return Consts.Format(Consts.GetInstance().SPECIAL_SHOP_UNITTICKET_NUM, (IDictionary) new Hashtable()
        {
          {
            (object) "Count",
            (object) quantity
