...
namespace SevenZip.Compression.LZ
{
  public class BinTree : InWindow, IInWindowStream, IMatchFinder
  {
    ...
    public uint GetMatches(uint[] distances)
    {
      ...
      uint matches = 0;
      uint num2 = this._pos <= this._cyclicBufferSize ? 0U : this._pos - this._cyclicBufferSize;
      ...
      while (num8 > num2 && cutValue-- != 0U)
      {
        ...
        uint index11 = (uint) ((num25 > this._cyclicBufferPos ? (int) this._cyclicBufferPos - (int) num25 + (int) this._cyclicBufferSize : (int) this._cyclicBufferPos - (int) num25) << 1);
    ...
    public void Skip(uint num)
    {
      do
      {
        ...
        while (num6 > num2 && cutValue-- != 0U)
        {
          ...
          uint index4 = (uint) ((num7 > this._cyclicBufferPos ? (int) this._cyclicBufferPos - (int) num7 + (int) this._cyclicBufferSize : (int) this._cyclicBufferPos - (int) num7) << 1);
