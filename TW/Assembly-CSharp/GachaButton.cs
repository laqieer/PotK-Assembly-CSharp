...
public class GachaButton : MonoBehaviour
{
  ...
  public void IbtnGachaTicket()
  {
    if (!this.menu_.CheckGachaTicket(this.gacha_data_) || this.menu_.IsPushAndSet())
      return;
    this.menu_.scene.GachaType = this.gachaType;
    this.InitPlayGachaTicketForRoll(this.gacha_data_, this.menu_, 10);
    this.InitPlayGachaTicket(this.gacha_data_, this.menu_);
  }

  private void InitPlayGachaTicket(GachaModuleGacha gachaData, Gacha0063Menu menu)
  {
    if (gachaData.roll_count == 1)
      return;
    Singleton<PopupManager>.GetInstance().open(menu.gachaTicketPopupPrefab).GetComponent<Popup00635Menu>().Init(gachaData, (System.Action) (() => this.StartCoroutine(this.PlayTicket(1, gachaData))));
  }

  private void InitPlayGachaTicketForRoll(
    ...
    int max_return)
  {
    ...
    if (rollCount > 1)
      return;
    int num = ((IEnumerable<PlayerGachaTicket>) SMManager.Get<Player>().gacha_tickets).FirstOrDefault<PlayerGachaTicket>((Func<PlayerGachaTicket, bool>) (x => x.ticket_id == gachaData.payment_id.Value)).quantity / gachaData.payment_amount;
    int executeNum = 1;
    for (int index = 2; index <= num && rollCount * index <= max_return; ++index)
      executeNum = index;
    System.Action[] playGacha = new System.Action[2]
    {
      (System.Action) (() => this.StartCoroutine(this.PlayTicket(1, gachaData))),
      (System.Action) (() => this.StartCoroutine(this.PlayTicket(executeNum, gachaData)))
    };
    Singleton<PopupManager>.GetInstance().open(menu.gachaTicketMultiPopupPrefab).GetComponent<Popup00636Menu>().Init(gachaData, playGacha, executeNum);
  }
  ...
  public IEnumerator Play(int num = 1, GachaModuleGacha gacha_data = null)
  {
    ...
    return (IEnumerator) new GachaButton.\u003CPlay\u003Ec__Iterator432()
    {
      num = num,
  ...
  {
    // ISSUE: object of a compiler-generated type is created
    return (IEnumerator) new GachaButton.\u003CPlayTicket\u003Ec__Iterator433()
    {
      num = num,
      gacha_data = gacha_data,
      \u003C\u0024\u003Enum = num,
