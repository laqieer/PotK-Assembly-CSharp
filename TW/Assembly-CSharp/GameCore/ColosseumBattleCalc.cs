...
namespace GameCore
{
  public static class ColosseumBattleCalc
  {
    ...
    public static AttackStatus selectAttackStatus(
      ...
      int battleCount)
    {
      AttackStatus[] attackStatusArray = ColosseumBattleCalc.getAttackStatusArray(attack, attackNeighbors, attackDeckUnits, attackEquippedGear, defense, defenseNeighbors, defenseDeckUnits, defenseEquippedGear, isAttack, attackHp, isSample, defenseHp, battleCount);
      ...
      return attackStatus.magicBullet == null ? attackStatus : ((IEnumerable<AttackStatus>) attackStatusArray).OrderBy<AttackStatus, int>((Func<AttackStatus, int>) (y => y.magicBullet.cost)).Last<AttackStatus>();
    }
      ...
      int battleCount)
    {
      if (((IEnumerable<BL.MagicBullet>) attack.magicBullets).Any<BL.MagicBullet>())
        return ((IEnumerable<BL.MagicBullet>) attack.magicBullets).Where<BL.MagicBullet>((Func<BL.MagicBullet, bool>) (x => x != null && x.isAttack && attackHp > x.cost * BattleFuncs.attackCount((BL.ISkillEffectListUnit) attack, (BL.ISkillEffectListUnit) defense))).Select<BL.MagicBullet, AttackStatus>((Func<BL.MagicBullet, AttackStatus>) (x => ColosseumBattleCalc.getAttackStatus(x, attack, attackNeighbors, attackDeckUnits, attackEquippedGear, defense, defenseNeighbors, defenseDeckUnits, defenseEquippedGear, isAttack, attackHp, isSample, defenseHp, battleCount))).OrderByDescending<AttackStatus, int>((Func<AttackStatus, int>) (x => x.attack)).ToArray<AttackStatus>();
      return new AttackStatus[1]
      {
        ColosseumBattleCalc.getAttackStatus((BL.MagicBullet) null, attack, attackNeighbors, attackDeckUnits, attackEquippedGear, defense, defenseNeighbors, defenseDeckUnits, defenseEquippedGear, isAttack, attackHp, isSample, defenseHp, battleCount)
      };
    }
    ...
    private static AttackStatus getAttackStatus(
      ...
      int battleCount)
    {
      Judgement.BeforeDuelParameter colosseumSingle = Judgement.BeforeDuelParameter.CreateColosseumSingle(attack, magicBullet, attackNeighbors, attackDeckUnits, attackEquippedGear, defense, (BL.MagicBullet) null, defenseNeighbors, defenseDeckUnits, defenseEquippedGear, isAttack, isSample, attackHp, defenseHp, battleCount);
    ...
    public static DuelColosseumResult calcColosseumDuel(
      ...
      int battleCount)
    {
      return ColosseumBattleCalc.calcColosseumDuelAttack(playerUnit, playerUnitHp, playerEquippedGear, opponentUnit, opponentUnitHp, opponentEquippedGear, bonusList, battleCount);
    }
      ...
      int battleCount)
    {
      ...
      AttackStatus status1 = ColosseumBattleCalc.selectAttackStatus(playerUnit, new BL.Unit[0], array1, playerEquippedGear, opponentUnit, new BL.Unit[0], array2, opponentEquippedGear, true, playerUnitHp, true, opponentUnitHp, battleCount);
      AttackStatus status2 = ColosseumBattleCalc.selectAttackStatus(opponentUnit, new BL.Unit[0], array2, opponentEquippedGear, playerUnit, new BL.Unit[0], array1, playerEquippedGear, true, opponentUnitHp, true, playerUnitHp, battleCount);
      duelColosseumResult.playerBeforeBonusParam = status1 == null ? (ColosseumBeforBonusParam) null : new ColosseumBeforBonusParam(status1);
      duelColosseumResult.opponentBeforeBonusParam = status2 == null ? (ColosseumBeforBonusParam) null : new ColosseumBeforBonusParam(status2);
      ...
      duelColosseumResult.playerAttackStatus = ColosseumBattleCalc.selectAttackStatus(playerUnit, new BL.Unit[0], array1, playerEquippedGear, opponentUnit, new BL.Unit[0], array2, opponentEquippedGear, true, playerUnitHp, true, opponentUnitHp, battleCount);
      duelColosseumResult.opponentAttackStatus = ColosseumBattleCalc.selectAttackStatus(opponentUnit, new BL.Unit[0], array2, opponentEquippedGear, playerUnit, new BL.Unit[0], array1, playerEquippedGear, true, opponentUnitHp, true, playerUnitHp, battleCount);
      BL.Unit unit1;
      int num1;
      ...
      duelColosseumResult.colosseumNewPAS = ColosseumBattleCalc.selectAttackStatus(playerUnit, new BL.Unit[0], array1, playerEquippedGear, opponentUnit, new BL.Unit[0], array2, opponentEquippedGear, attackOrder == 0, playerUnitHp, false, opponentUnitHp, battleCount);
      duelColosseumResult.colosseumNewOAS = ColosseumBattleCalc.selectAttackStatus(opponentUnit, new BL.Unit[0], array2, opponentEquippedGear, playerUnit, new BL.Unit[0], array1, playerEquippedGear, attackOrder != 0, opponentUnitHp, false, playerUnitHp, battleCount);
      bool invokedAttackerPrayer = false;
      bool invokedDefenderPrayer = false;
      for (int index = 0; index < 4; ++index)
      {
        bool flag = false;
        AttackStatus attackAS = ColosseumBattleCalc.selectAttackStatus(unit1, new BL.Unit[0], unitArray1, playerItem1, unit2, new BL.Unit[0], unitArray2, playerItem2, !flag, num1, false, num2, battleCount);
        AttackStatus defenseAS = ColosseumBattleCalc.selectAttackStatus(unit2, new BL.Unit[0], unitArray2, playerItem2, unit1, new BL.Unit[0], unitArray1, playerItem1, flag, num2, false, num1, battleCount);
      ...
      bool invokedDefenderPrayer)
    {
      ...
      if (!isAttakerSwap)
      {
        ColosseumBattleCalc.calcMultiAttack(turns, hp, true, attack, attackAS, defense, defenseAS, invokedDefenderPrayer, defenseHp, false);
        ColosseumBattleCalc.calcMultiAttack(turns, hp, false, defense, defenseAS, attack, attackAS, invokedAttackerPrayer, attackHp, false);
        if (!hp.attackerCantOneMore && attackAS != null && BattleFuncs.canOneMore(attackAS.duelParameter.attackerUnitParameter, attackAS.duelParameter.defenderUnitParameter, (BL.ISkillEffectListUnit) attack, (BL.ISkillEffectListUnit) defense))
          ColosseumBattleCalc.calcMultiAttack(turns, hp, true, attack, attackAS, defense, defenseAS, invokedDefenderPrayer, defenseHp, true);
        if (!hp.defenderCantOneMore && defenseAS != null && BattleFuncs.canOneMore(defenseAS.duelParameter.attackerUnitParameter, defenseAS.duelParameter.defenderUnitParameter, (BL.ISkillEffectListUnit) defense, (BL.ISkillEffectListUnit) attack))
          ColosseumBattleCalc.calcMultiAttack(turns, hp, false, defense, defenseAS, attack, attackAS, invokedAttackerPrayer, attackHp, true);
      }
      else
      {
        ColosseumBattleCalc.calcMultiAttack(turns, hp, false, defense, defenseAS, attack, attackAS, invokedAttackerPrayer, attackHp, false);
      ...
      bool isOneMoreAttack)
    {
      ...
      for (int index = BattleFuncs.attackCount((BL.ISkillEffectListUnit) attack, (BL.ISkillEffectListUnit) defense); index > 0; --index)
        BattleFuncs.calcSingleAttack(turns, hp, isAttacker, (BL.ISkillEffectListUnit) attack, attackStatus, (BL.ISkillEffectListUnit) defense, defenseStatus, 0, ColosseumBattleCalc.random.Get(), false, true, false, invokedPrayer, defenseHp, isOneMoreAttack);
    }
    ...
    public static ColosseumResult calcColosseum(ColosseumEnvironment env)
    {
      ...
      for (int index = 1; index <= 5; ++index)
      {
        ...
        if (env.playerUnitDict[index] != null && env.opponentUnitDict[index] != null)
          result = ColosseumBattleCalc.calcColosseumDuel(env.playerUnitDict[index], env.playerUnitDict[index].hp, env.playerGearDict[index], env.opponentUnitDict[index], env.opponentUnitDict[index].hp, env.opponentGearDict[index], env.bonusList, index);
        else if (env.playerUnitDict[index] == null && env.opponentUnitDict[index] == null)
          result = new DuelColosseumResult();
        else if (env.playerUnitDict[index] != null)
        {
          result = new DuelColosseumResult();
          result.player = env.playerUnitDict[index];
          result.playerActiveBonus = ColosseumBattleCalc.SetEnableColosseumBonusEffect(env.playerUnitDict[index], env.bonusList);
          result.isDieOpponent = true;
          result.isPlayerFirstAttacker = true;
        }
        else if (env.opponentUnitDict[index] != null)
        {
          result = new DuelColosseumResult();
          result.opponent = env.opponentUnitDict[index];
          result.opponentActiveBonus = ColosseumBattleCalc.SetEnableColosseumBonusEffect(env.opponentUnitDict[index], env.bonusList);
