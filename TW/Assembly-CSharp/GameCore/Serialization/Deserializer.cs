...
namespace GameCore.Serialization
{
  internal class Deserializer
  {
    ...
    private object GetBlob(int objectId, TypeObject typeObj, TreeObject treeObj, Type type)
    {
      object blob;
    ...
    public object Deserialize(int treeId)
    {
      ...
      Type type = this.cs.BindToType(typeObj.typeInfo);
      if (type.IsBlob())
      {
    ...
    private Array DeserializeArray(Type elementType, TreeObject treeObj, object obj, bool isNew)
    {
      if (!treeObj.fields.ContainsKey("lengths"))
      {
        int[] numArray = (int[]) this.Deserialize(treeObj.fields["__elems__"]);
        Array array = !isNew ? (Array) obj : Array.CreateInstance(elementType, numArray.Length);
      ...
      Array array1 = !isNew ? (Array) obj : Array.CreateInstance(elementType, numArray1);
