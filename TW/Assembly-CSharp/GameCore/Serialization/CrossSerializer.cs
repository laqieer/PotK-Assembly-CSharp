...
namespace GameCore.Serialization
{
  ...
  {
    private Dictionary<GameCore.TypeInfo, Type> typeBindCache = new Dictionary<GameCore.TypeInfo, Type>();
    public Func<GameCore.TypeInfo, Type> TypeBinder;
    ...
    static CrossSerializer()
    {
      ...
      CrossSerializer.Specialized.RegisterMutable(typeof (List<bool>), (Action<SerializeInfo, object>) ((info, obj) => info.TypeObject.SetArray(((List<bool>) obj).ToArray())), (Action<SerializeInfo, object, bool>) ((info, obj, isNew) =>
      {
        if (isNew)
        {
          info.Type.GetConstructor(new Type[1]
          {
      ...
      {
        Type type1 = info.Type;
        Type[] genericArguments = info.GenericArguments;
      ...
      }), (Action<SerializeInfo, object, bool>) ((info, obj, isNew) =>
      {
        ...
        Type type = info.Type;
        if (isNew)
          type.GetConstructor(Type.EmptyTypes).Invoke(obj, (object[]) null);
        else
    ...
    internal Type BindToType(GameCore.TypeInfo typeInfo)
    {
      Type type1;
      if (this.typeBindCache.TryGetValue(typeInfo, out type1))
        return type1;
      Type type2 = this.TypeBinder != null ? this.TypeBinder(typeInfo) : typeInfo.Type;
    ...
    {
      private static Dictionary<Type, CrossSerializer.Specialized.Value> info = new Dictionary<Type, CrossSerializer.Specialized.Value>();
      private static Dictionary<Type, CrossSerializer.Specialized.ValueM> infoM = new Dictionary<Type, CrossSerializer.Specialized.ValueM>();
      ...
      public static bool TryGetValue(Type type, out CrossSerializer.Specialized.Value value)
      {
      ...
      public static bool TryGetValueMutable(Type type, out CrossSerializer.Specialized.ValueM value)
      {
