...
namespace GameCore
{
  ...
  public class BL
  {
    ...
    [NonSerialized]
    private Dictionary<Tuple<int, int, int, int, int>, Dictionary<int, List<BL.AttackStatusCacheContainer>>> mAttackStatusCacheDic;
    [NonSerialized]
      ...
      int moveCost)
    {
      if (!this.isMoveOKPanel(panel, unit, enabledIgnoreMoveCost, moveCost))
        return false;
      return isRebirth || !(this.getFieldUnitAI(panel) is BL.AIUnit fieldUnitAi) || fieldUnitAi.unit == unit || fieldUnitAi.unit.isDead || fieldUnitAi.getForceID(this) == this.getForceID(unit);
    }
    ...
    {
      foreach (BL.Panel zocPanel in this.getZocPanels(u, r, c))
    ...
    public void resetZocPanels(
      ...
      bool isAI = false)
    {
      IEnumerable<BL.Panel> zocPanels1 = this.getZocPanels(u, r0, c0);
      IEnumerable<BL.Panel> zocPanels2 = this.getZocPanels(u, r1, c1);
      foreach (BL.Panel panel in zocPanels1.Except<BL.Panel>(zocPanels2))
        panel.removeZocUnit(u.originalUnit, isAI);
      foreach (BL.Panel panel in zocPanels2.Except<BL.Panel>(zocPanels1))
        panel.addZocUnit(u.originalUnit, isAI);
    }

    public IEnumerable<BL.Panel> getZocPanels(BL.ISkillEffectListUnit unit, int row, int column)
    {
      return BattleFuncs.getSkillZocPanels(unit, row, column);
    }
    ...
    private Dictionary<Tuple<int, int, int, int, int>, Dictionary<int, List<BL.AttackStatusCacheContainer>>> attackStatusCacheDic
    {
      get
      {
        if (this.mAttackStatusCacheDic == null)
          this.mAttackStatusCacheDic = new Dictionary<Tuple<int, int, int, int, int>, Dictionary<int, List<BL.AttackStatusCacheContainer>>>();
    ...
    private void addAttackStatusCacheDicList(
      ...
      AttackStatus[] data)
    {
      Tuple<int, int, int, int, int> key1 = this.makeAttackStatusCacheKey(attack, attackPanel, defense, defensePanel);
    ...
    private int makePanelId(BL.Panel panel) => panel.index;
    ...
    private Tuple<int, int, int, int, int> makeAttackStatusCacheKey(
      BL.ISkillEffectListUnit attack,
      BL.Panel attackPanel,
      BL.ISkillEffectListUnit defense,
      BL.Panel defensePanel)
    {
      return new Tuple<int, int, int, int, int>(this.makeUnitId(attack), this.makeUnitId(defense), this.makePanelId(attackPanel), this.makePanelId(defensePanel), BL.fieldDistance(attackPanel, defensePanel));
    }
    ...
    public HashSet<BL.ISkillEffectListUnit> getCharismaTargetUnits(BL.ISkillEffectListUnit unit)
    {
      ...
      {
        if (enabledCharismaEffects.Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.GetInt("effect_target") == effectTarget)))
        {
    ...
    public HashSet<BL.ISkillEffectListUnit> getOnemanChargeUnits(BL.ISkillEffectListUnit unit)
    {
      ...
      for (int searchTarget = 0; searchTarget <= 1; ++searchTarget)
      {
        if (unit is BL.AIUnit)
        {
          foreach (BL.AIUnit unit1 in this.aiUnitPositions.value.Where<BL.AIUnit>((Func<BL.AIUnit, bool>) (x => ((IEnumerable<BL.ForceID>) forceId[searchTarget]).Contains<BL.ForceID>(this.getForceID(x.originalUnit)))))
          {
            if (BattleFuncs.getEnabledOnemanChargeEffects((BL.ISkillEffectListUnit) unit1).Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.GetInt("search_target") == searchTarget || x.effect.GetInt("search_target") == 2)))
              onemanChargeUnits.Add((BL.ISkillEffectListUnit) unit1);
          }
        }
        else
        {
          foreach (BL.Unit unit2 in ((IEnumerable<BL.ForceID>) forceId[searchTarget]).SelectMany<BL.ForceID, BL.Unit>((Func<BL.ForceID, IEnumerable<BL.Unit>>) (x => (IEnumerable<BL.Unit>) this.getForceUnitList(x))))
          {
            if (BattleFuncs.getEnabledOnemanChargeEffects((BL.ISkillEffectListUnit) unit2).Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.GetInt("search_target") == searchTarget || x.effect.GetInt("search_target") == 2)))
              onemanChargeUnits.Add((BL.ISkillEffectListUnit) unit2);
          }
        }
      }
    ...
    public List<BL.ExecuteSkillEffectResult> completedExecuteSkillEffects(BL.UnitPosition up)
    {
      ...
      if (up.unit.skillEffects.AilmentExecuted(this, (BL.ISkillEffectListUnit) up.unit))
        up.unit.commit();
      return skillEffectResultList;
    }
      ...
      BL env)
    {
      ...
      List<BL.ISkillEffectListUnit> list1 = ((IEnumerable<BL.ForceID>) new BL.ForceID[3]
      {
        BL.ForceID.player,
        BL.ForceID.enemy,
        BL.ForceID.neutral
      }).SelectMany<BL.ForceID, BL.ISkillEffectListUnit>((Func<BL.ForceID, IEnumerable<BL.ISkillEffectListUnit>>) (x => BattleFuncs.getForceUnits(x, false, true))).ToList<BL.ISkillEffectListUnit>();
      foreach (BL.ISkillEffectListUnit skillEffectListUnit in list1)
      {
        BL.ISkillEffectListUnit unit = skillEffectListUnit;
        dictionary[unit] = unit.skillEffects.All().Count<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => !BattleFuncs.isSealedSkillEffect(unit, x)));
      }
      foreach (BL.Unit target1 in targets)
      {
        BL.Unit target = target1;
        BL.UnitPosition unitPosition1 = this.getUnitPosition(target);
        int moveCost = unitPosition1.moveCost;
        bool flag2 = target.HasEnabledSkillEffect(BattleskillEffectLogicEnum.ignore_move_cost);
        bool flag3 = target.HasEnabledSkillEffect(BattleskillEffectLogicEnum.slip_thru);
        bool isDontMove = target.IsDontMove;
        List<BL.SkillEffect> list2 = BattleFuncs.getEnabledCharismaEffects((BL.ISkillEffectListUnit) target).Where<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => !BattleFuncs.isSealedSkillEffect((BL.ISkillEffectListUnit) target, x))).ToList<BL.SkillEffect>();
        foreach (BattleskillEffect effect1 in skill.Effects)
        {
          ...
          if (effect1.effect_logic.HasTag(BattleskillEffectTag.immediately))
          {
            switch (effect1.effect_logic.Enum)
            {
              ...
              case BattleskillEffectLogicEnum.remove_skilleffect:
                int investType = !effect1.HasKey("invest_type") ? 0 : effect1.GetInt("invest_type");
                target.skillEffects.RemoveEffect(effect1.GetInt("logic_id"), !effect1.HasKey("skill_id") ? 0 : effect1.GetInt("skill_id"), !effect1.HasKey("skill_type") ? 0 : effect1.GetInt("skill_type"), env, (BL.ISkillEffectListUnit) target, (Func<BL.SkillEffect, bool>) (e =>
                {
                  if (e.baseSkill.skill_type == BattleskillSkillType.leader || e.baseSkill.skill_type == BattleskillSkillType.passive && e.baseSkill.range_effect_passive_skill || e.effect.effect_logic.Enum == BattleskillEffectLogicEnum.fix_hp || e.effect.effect_logic.Enum == BattleskillEffectLogicEnum.ratio_hp)
                    return true;
                  if (investType == 0)
                    return false;
                  if (investType == 1 && e.isBaseSkill)
                    return true;
                  return investType == 2 && !e.isBaseSkill;
                }));
                target.commit();
                continue;
              ...
              case BattleskillEffectLogicEnum.reduct_release_skill_turn:
                ...
                }, this.getForceID(unitPosition1.unit)).GetEnumerator())
                {
                    ...
                    if (effect1.GetInt("element") == 0 || (CommonElement) effect1.GetInt("element") == unit.playerUnit.GetElement())
                    {
                      ...
                      foreach (BL.Skill skill2 in unit.skills)
                      {
                        if (skill2.remain.HasValue)
                        {
                          ...
                          if (skill2.skill.max_use_count != 0 && num4 > skill2.skill.max_use_count)
                            num4 = skill2.skill.max_use_count;
          ...
          else
          {
            ...
            if (effect1.effect_logic.Enum == BattleskillEffectLogicEnum.zoc)
            {
              BL.UnitPosition unitPosition2 = this.getUnitPosition(target);
              env.addZocPanels((BL.ISkillEffectListUnit) target, unitPosition2.originalRow, unitPosition2.originalColumn);
              flag1 = true;
            }
          }
        }
        if (unitPosition1.moveCost != moveCost || isDontMove != target.IsDontMove || flag2 != target.HasEnabledSkillEffect(BattleskillEffectLogicEnum.ignore_move_cost) || flag3 != target.HasEnabledSkillEffect(BattleskillEffectLogicEnum.slip_thru))
          this.getUnitPosition(target).clearMovePanelCache();
        List<BL.SkillEffect> list3 = BattleFuncs.getEnabledCharismaEffects((BL.ISkillEffectListUnit) target).Where<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => !BattleFuncs.isSealedSkillEffect((BL.ISkillEffectListUnit) target, x))).ToList<BL.SkillEffect>();
        IEnumerable<BL.SkillEffect> effects = list2.Union<BL.SkillEffect>((IEnumerable<BL.SkillEffect>) list3).Except<BL.SkillEffect>(list2.Intersect<BL.SkillEffect>((IEnumerable<BL.SkillEffect>) list3));
        foreach (BL.ISkillEffectListUnit charismaTargetUnit in BattleFuncs.getCharismaTargetUnits((BL.ISkillEffectListUnit) target, effects))
      ...
      {
        BL.ISkillEffectListUnit unit = skillEffectListUnit;
        if (unit.skillEffects.All().Count<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => !BattleFuncs.isSealedSkillEffect(unit, x))) != dictionary[unit])
        {
          unit.skillEffects.commit();
          if (unit is BL.Unit)
            (unit as BL.Unit).commit();
        }
      }
      ...
      BL env)
    {
      ...
      {
        int hp = unit.hp;
        unit.hp -= skill.getHpCost(unit, true);
        f(unitList, unit, hp);
      }
      else
    ...
    public class AIUnit : BL.UnitPosition, BL.ISkillEffectListUnit
    {
      ...
      {
        if (env.aiDeads.value.Contains(u))
          return;
        ...
        env.removeZocPanels((BL.ISkillEffectListUnit) this, this.mOriginalRow, this.mOriginalColumn, true);
      }

      private bool canSpawnp(BL.Unit unit, BL env)
      {
        return !env.aiUnits.value.Any<BL.AIUnit>((Func<BL.AIUnit, bool>) (u => u.unit == unit)) && !unit.isDead && !unit.isSpawned && !env.aiDeads.value.Any<BL.AIUnit>((Func<BL.AIUnit, bool>) (u => u.unit == unit));
      }
    ...
    public class DropData : BL.ModelBase
    {
      ...
      public bool isDropBox
      {
        get
        {
          return this.mReward.Type == MasterDataTable.CommonRewardType.gear || this.mReward.Type == MasterDataTable.CommonRewardType.material_gear || this.mReward.Type == MasterDataTable.CommonRewardType.supply || this.mReward.Type == MasterDataTable.CommonRewardType.unit || this.mReward.Type == MasterDataTable.CommonRewardType.material_unit || this.mReward.Type == MasterDataTable.CommonRewardType.money || this.mReward.Type == MasterDataTable.CommonRewardType.gacha_ticket;
        }
      }
      ...
      public void execute(BL.Unit unit, BL env)
      {
        if (this.mIsCompleted)
          return;
        MasterDataTable.CommonRewardType type = this.reward.Type;
        switch (type)
        {
          case MasterDataTable.CommonRewardType.unit:
label_8:
            ++env.dropUnit.value;
            goto case MasterDataTable.CommonRewardType.player_exp;
          case MasterDataTable.CommonRewardType.supply:
            ++env.dropItem.value;
            goto case MasterDataTable.CommonRewardType.player_exp;
          case MasterDataTable.CommonRewardType.gear:
...
label_10:
    ...
    public class DuelTurn
    {
      ...
      public BL.DuelTurnNetwork ToNetwork(BL env)
      {
        return new BL.DuelTurnNetwork()
        {
          ...
          investUnit = this.investUnit != null ? ((IEnumerable<BL.ISkillEffectListUnit>) this.investUnit).Select<BL.ISkillEffectListUnit, int>((Func<BL.ISkillEffectListUnit, int>) (x =>
          {
            int? network = x.originalUnit.ToNetwork(env);
            return network.HasValue ? network.Value : -1;
          })).ToArray<int>() : (int[]) null,
          investSkillIds = this.investSkillIds,
          attackerConsumeSkillEffects = this.attackerConsumeSkillEffects,
          defenderConsumeSkillEffects = this.defenderConsumeSkillEffects,
      ...
      public static BL.DuelTurn FromNetwork(BL.DuelTurnNetwork nw, BL env)
      {
        ...
        return new BL.DuelTurn()
        {
          ...
          investUnit = nw.investUnit != null ? (BL.ISkillEffectListUnit[]) ((IEnumerable<int>) nw.investUnit).Select<int, BL.Unit>((Func<int, BL.Unit>) (x => BL.UnitPosition.FromNetwork(new int?(x), env).unit)).ToArray<BL.Unit>() : (BL.ISkillEffectListUnit[]) null,
          investSkillIds = nw.investSkillIds,
          attackerConsumeSkillEffects = nw.attackerConsumeSkillEffects,
          defenderConsumeSkillEffects = nw.defenderConsumeSkillEffects,
    ...
    public class Panel : BL.ModelBase
    {
      ...
      {
        List<BL.Unit> unitList = !isAI ? this.zocUnits : this.zocAIUnits;
        if (unitList.Count > 0)
        {
          BL.ForceID[] targetForce = env.getTargetForce(unit);
    ...
    public class UnitPosition : BL.ModelBase
    {
      ...
      public void resetOriginalPosition(BL env)
      {
        int r0 = -100;
        int c0 = -100;
        ...
        if (this.mUnit.isEnable && !this.mUnit.isDead)
          env.resetZocPanels((BL.ISkillEffectListUnit) this.mUnit, r0, c0, this.mOriginalRow, this.mOriginalColumn);
        ++this.revision;
      }
    ...
    public class Skill : BL.ModelBase
    {
      ...
      public int getHpCost(BL.Unit unit, bool isExcludeFixCost = false)
      {
        int hpCost = 0;
        if (!isExcludeFixCost)
          hpCost += this.skill.consume_hp;
    ...
    public class SkillEffectList : BL.ModelBase
    {
      ...
      public bool IsSealedSkill(int skill_id, BL.SkillEffect effect = null)
      {
        return this.effects.Where<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.effect_logic.Enum == BattleskillEffectLogicEnum.seal)).Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x =>
        {
          ...
          if (MasterData.BattleskillSkill.TryGetValue(skill_id, out battleskillSkill))
          {
            if (effect != null)
            {
              int num = !x.effect.HasKey("invest_type") ? 0 : x.effect.GetInt("invest_type");
              if (num == 1 && effect.isBaseSkill || num == 2 && !effect.isBaseSkill)
                return false;
            }
      ...
      {
        if (!this.effects.Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.effect_logic.Enum == BattleskillEffectLogicEnum.zoc)))
          return;
        int originalRow;
        int originalColumn;
        switch (unit)
      ...
      {
        this.Clean(logic, skillId, skillType, env, unit, funcExtraCheck);
      }

      public void RemoveEffect(BL.Unit invocationUnit, BL env, BL.ISkillEffectListUnit unit)
      {
        List<BL.SkillEffect> list = this.effects.Where<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.unit != null && x.unit == invocationUnit)).ToList<BL.SkillEffect>();
        foreach (BL.SkillEffect skillEffect in list)
          this.effects.Remove(skillEffect);
        if (!list.Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.effect_logic.Enum == BattleskillEffectLogicEnum.zoc)))
          return;
      ...
      public void RemoveAilmentEffect(BL env, BL.ISkillEffectListUnit unit)
      {
        foreach (int logic in this.effects.Where<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.baseSkill.skill_type == BattleskillSkillType.ailment)).Select<BL.SkillEffect, int>((Func<BL.SkillEffect, int>) (x => (int) x.effect.effect_logic.Enum)))
          this.RemoveEffect(logic, env, unit);
      }
      ...
      public void TurnStart(int turn, BL env, BL.ISkillEffectListUnit unit)
      {
        ...
        {
          if (removedBaseSkillEffect.turnRemain.HasValue)
            removedBaseSkillEffect.turnRemain = new int?(removedBaseSkillEffect.turnRemain.Value - 1);
        }
      ...
      {
        bool flag = this.effects.Where<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (effect => effect.effectEnded || effect.deathDisable)).Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (effect => effect.effect.effect_logic.Enum == BattleskillEffectLogicEnum.zoc));
        if (flag)
          this.removeZocEffect(env, unit);
      ...
      {
        bool flag = logic == 1001079 && this.effects.Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.effect_logic.Enum == (BattleskillEffectLogicEnum) logic));
        if (flag)
          this.removeZocEffect(env, unit);
        ...
        Func<BL.SkillEffect, bool> funcExtraCheck = null)
      {
        ...
        {
          if (logic != 0 && (BattleskillEffectLogicEnum) logic != effect.effect.effect_logic.Enum || skillId != 0 && skillId != effect.baseSkill.ID || skillType != 0 && (BattleskillSkillType) skillType != effect.baseSkill.skill_type)
            return true;
        ...
        bool flag = this.effects.Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.effect_logic.Enum == BattleskillEffectLogicEnum.zoc && !remainEffects.Contains(x)));
        if (flag)
          this.removeZocEffect(env, unit);
        ...
        Func<BL.SkillEffect, bool> f)
      {
        ...
        BL.SkillEffectList.\u003CWhere\u003Ec__Iterator22 whereCIterator22 = new BL.SkillEffectList.\u003CWhere\u003Ec__Iterator22()
        {
          e = e,
          f = f,
          \u003C\u0024\u003Ee = e,
        ...
        whereCIterator22.\u0024PC = -2;
        return (IEnumerable<BL.SkillEffect>) whereCIterator22;
      }
        ...
        Func<List<BL.SkillEffect>, List<BL.SkillEffect>> enabled)
      {
        // ISSUE: object of a compiler-generated type is created
        // ISSUE: variable of a compiler-generated type
        BL.SkillEffectList.\u003CWhereAndGroupBy\u003Ec__Iterator23 groupByCIterator23 = new BL.SkillEffectList.\u003CWhereAndGroupBy\u003Ec__Iterator23()
        {
          e = e,
          enabled = enabled,
          f = f,
        ...
        // ISSUE: reference to a compiler-generated field
        groupByCIterator23.\u0024PC = -2;
        return (IEnumerable<BL.SkillEffect>) groupByCIterator23;
      }
...
label_9:
      ...
      public int CanUseSkill(BattleskillSkill skill, BL.ISkillEffectListUnit unit)
      {
        bool flag1 = ((IEnumerable<BattleskillEffect>) skill.Effects).Any<BattleskillEffect>((Func<BattleskillEffect, bool>) (x => !x.effect_logic.HasTag(BattleskillEffectTag.immediately)));
        bool flag2 = ((IEnumerable<BattleskillEffect>) skill.Effects).Any<BattleskillEffect>((Func<BattleskillEffect, bool>) (x => x.effect_logic.HasTag(BattleskillEffectTag.immediately)));
        bool flag3 = false;
        bool flag4 = false;
        if (flag1)
          flag3 = !this.HasEffect(skill);
        if (flag2)
          flag4 = ((Func<bool>) (() =>
          {
            foreach (BattleskillEffect battleskillEffect in ((IEnumerable<BattleskillEffect>) skill.Effects).Where<BattleskillEffect>((Func<BattleskillEffect, bool>) (x => x.effect_logic.HasTag(BattleskillEffectTag.immediately))))
            {
              if (battleskillEffect.effect_logic.Enum != BattleskillEffectLogicEnum.invest_skilleffect_im)
                return true;
              int key = battleskillEffect.GetInt("skill_id");
              if (key != 0 && MasterData.BattleskillSkill.ContainsKey(key))
              {
                BattleskillSkill battleskillSkill = MasterData.BattleskillSkill[key];
                if (((IEnumerable<BattleskillEffect>) battleskillSkill.Effects).Any<BattleskillEffect>((Func<BattleskillEffect, bool>) (x => x.effect_logic.Enum != BattleskillEffectLogicEnum.seal)))
                  return true;
                foreach (BattleskillEffect effect in battleskillSkill.Effects)
                {
                  Func<int, bool> func1 = (Func<int, bool>) (sid => ((IEnumerable<BL.Skill>) unit.originalUnit.duelSkills).Any<BL.Skill>((Func<BL.Skill, bool>) (x => (sid == 0 || x.skill.ID == sid) && ((IEnumerable<BattleskillEffect>) x.skill.Effects).Any<BattleskillEffect>((Func<BattleskillEffect, bool>) (y => y.effect_logic.Enum != BattleskillEffectLogicEnum.invest_element && y.effect_logic.Enum != BattleskillEffectLogicEnum.effect_element)))));
                  Func<int, int, int, bool> func2 = (Func<int, int, int, bool>) ((sid, stype, sinvest) => unit.skillEffects.All().Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x =>
                  {
                    if (x.baseSkill.skill_type == BattleskillSkillType.ailment || x.unit != null && (x.baseSkill.skill_type == BattleskillSkillType.leader || x.baseSkill.skill_type == BattleskillSkillType.passive && x.baseSkill.range_effect_passive_skill) && (x.unit.index != unit.originalUnit.index || x.unit.isPlayerControl != unit.originalUnit.isPlayerControl) || sid != 0 && x.baseSkillId != sid || stype != 0 && (BattleskillSkillType) stype != x.baseSkill.skill_type)
                      return false;
                    if (sinvest == 0 || sinvest == 1 && !x.isBaseSkill)
                      return true;
                    return sinvest == 2 && x.isBaseSkill;
                  })));
                  int num1 = !effect.HasKey("invest_type") ? 0 : effect.GetInt("invest_type");
                  int num2 = effect.GetInt("skill_id");
                  int num3 = effect.GetInt("skill_type");
                  switch (num3)
                  {
                    case 0:
                      if (func2(num2, 1, 0) || func2(num2, 2, 0) || func2(num2, 3, num1) || func2(num2, 6, 0) || func2(num2, 7, 0) || func1(num2))
                        return true;
                      break;
                    case 1:
                    case 2:
        ...
        return flag1 && flag2 ? (!flag3 && !flag4 ? 1 : 0) : (flag1 ? (flag3 ? 0 : 1) : (flag4 ? 0 : 2));
      }
      ...
      public void RecoveryRemovedSkillEffects()
      {
        foreach (BL.SkillEffect removedBaseSkillEffect in this.removedBaseSkillEffects)
          this.Add(removedBaseSkillEffect);
...
label_7:
    ...
    public class Unit : BL.ModelBase, IDisposable, BL.ISkillEffectListUnit
    {
      ...
      public void setIsDead(bool v, BL env, int turn = 0)
      {
        if (this.mIsDead == v)
          return;
        ...
        else
        {
          ...
          this.skillEffects.RecoveryRemovedSkillEffects();
      ...
      {
        // ISSUE: object of a compiler-generated type is created
        return (IEnumerator) new BL.Unit.\u003CInitAIExtention\u003Ec__Iterator24()
        {
