...
public class CommonHeaderExp : CommonHeaderBase
{
  ...
  private IEnumerator CreatePrefab()
  {
    ...
    return (IEnumerator) new CommonHeaderExp.\u003CCreatePrefab\u003Ec__IteratorB3C()
    {
      \u003C\u003Ef__this = this
    };
  }
  ...
  protected override void Update()
  {
    ...
    if (this.UpdateItems() && this.items.Value != null)
      this.SetItemCount(((IEnumerable<PlayerItem>) this.items.Value).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.entity_type == MasterDataTable.CommonRewardType.gear)).ToArray<PlayerItem>().Length, this.player.Value.max_items);
