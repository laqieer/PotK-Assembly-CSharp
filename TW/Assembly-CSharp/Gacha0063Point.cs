...
public class Gacha0063Point : Gacha0063hindicator
{
  ...
  private void SetGachBtnInfo(Player playerData, GachaButton btn, GachaModuleGacha gachaData)
  {
    ...
    if (max_play_nam1 == 1)
    {
      if (max_play_nam2 >= 1)
      {
        ...
        ((IEnumerable<UISprite>) componentsInChildren).ForEach<UISprite>((Action<UISprite>) (x => x.color = Color.white));
        ((Behaviour) component2).enabled = true;
      }
      else
      {
        component1.color = Color.gray;
        ((IEnumerable<UISprite>) componentsInChildren).ForEach<UISprite>((Action<UISprite>) (x => x.color = Color.gray));
    ...
    else
    {
      ...
      {
        component1.color = Color.white;
        this.ChangeColor(((Component) btn).transform, Color.white);
        ((Behaviour) component2).enabled = true;
      }
      else
      {
        ...
        component1.color = Color.gray;
        this.ChangeColor(((Component) btn).transform, Color.gray);
        ((Behaviour) component2).enabled = false;
      }
    }
  }
  ...
  public override IEnumerator Set(GameObject detailPopup)
  {
    ...
    return (IEnumerator) new Gacha0063Point.\u003CSet\u003Ec__Iterator442()
    {
      \u003C\u003Ef__this = this
    };
  }
