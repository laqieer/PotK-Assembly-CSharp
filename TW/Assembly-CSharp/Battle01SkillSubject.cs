...
public class Battle01SkillSubject : NGBattleMenuBase
{
  ...
  private void OnEnable()
  {
    ...
    if (componentsInChildren.Length <= 0)
      return;
    componentsInChildren[0].setTargets(this.skillTargets, skill.isOwn, this.skillGrayTargets);
  }
  ...
  protected override IEnumerator Start_Original()
  {
    ...
    Battle01SkillSubject.\u003CStart_Original\u003Ec__Iterator929 originalCIterator929 = new Battle01SkillSubject.\u003CStart_Original\u003Ec__Iterator929();
    return (IEnumerator) originalCIterator929;
  }
  ...
  public void setSkillTargets(BL.Unit unit, BL.Skill skill, List<BL.Unit> targets)
  {
    ...
    this.skillTargets = targets.Where<BL.Unit>((Func<BL.Unit, bool>) (x => x.skillEffects.CanUseSkill(skill.skill, (BL.ISkillEffectListUnit) x) == 0)).ToList<BL.Unit>();
    this.skillGrayTargets = targets.Where<BL.Unit>((Func<BL.Unit, bool>) (x => x.skillEffects.CanUseSkill(skill.skill, (BL.ISkillEffectListUnit) x) == 1)).ToList<BL.Unit>();
    ...
    if (componentsInChildren1.Length > 0)
      componentsInChildren1[0].setTargets(this.skillTargets, skill.isOwn, this.skillGrayTargets);
