...
public static class RarityIcon
{
  ...
  public static string GetSpriteName(UnitUnit unit, bool isDetail = true, bool isSame = false)
  {
    ...
    if (!isSame)
    {
      UnitUnit[] array = ((IEnumerable<UnitUnit>) MasterData.UnitUnitList).Where<UnitUnit>((Func<UnitUnit, bool>) (x =>
      {
        if (unit.same_character_id != x.same_character_id)
          return false;
        DateTime? publishedAt = x.published_at;
        return publishedAt.HasValue && publishedAt.Value < Singleton<NGGameDataManager>.GetInstance().last_signed_in_at;
      })).OrderByDescending<UnitUnit, int>((Func<UnitUnit, int>) (x => x.rarity.index)).ToArray<UnitUnit>();
