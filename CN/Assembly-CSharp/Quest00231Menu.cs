...
public class Quest00231Menu : BackButtonMenuBase
{
  ...
  [SerializeField]
  private UISprite ibtnAllSprite;
  [SerializeField]
  private UISprite ibtnPresonalSprite;
  [SerializeField]
  private UISprite ibtnGuildSprite;
  [SerializeField]
  ...
  private void ResetScrollObject(EventPointType type)
  {
    ...
    if (this.listType == EventPointType.all)
    {
      ...
      this.ibtnAllSprite.color = PunkColor.button_on;
      this.ibtnPresonalSprite.color = PunkColor.button_off;
    }
    else if (this.listType == EventPointType.personal)
    {
      ((Component) this.scrollContainerPersonal).gameObject.SetActive(true);
      this.ibtnPresonalSprite.color = PunkColor.button_on;
      if (this.isGuild)
      {
        ((Component) this.scrollContainerGuild).gameObject.SetActive(false);
        this.ibtnGuildSprite.color = PunkColor.button_off;
      }
      else
      {
        ((Component) this.scrollContainerAll).gameObject.SetActive(false);
        this.ibtnAllSprite.color = PunkColor.button_off;
      }
    }
    else
    {
      ...
      ((Component) this.scrollContainerAll).gameObject.SetActive(false);
      ((Component) this.scrollContainerPersonal).gameObject.SetActive(false);
      this.ibtnGuildSprite.color = PunkColor.button_on;
      this.ibtnPresonalSprite.color = PunkColor.button_off;
    }
  }
    ...
    PunitiveExpeditionEventReward[] rewardList)
  {
    ...
    return (IEnumerator) new Quest00231Menu.\u003CInit\u003Ec__Iterator263()
    {
      eventInfo = eventInfo,
