...
namespace GameCore.Serialization
{
  public class BinaryFormatter : ICrossFormatter
  {
    ...
    private void SaveCore(int rootId, TypeObject[] objects, TreeObject[] trees, Stream stream)
    {
      ...
      int minObjectId = objects.Length != 0 ? ((IEnumerable<TypeObject>) objects).Min<TypeObject>((Func<TypeObject, int>) (x => x.objectId)) : 0;
      binaryWriter.Write7BitInt(minObjectId);
      int minTreeId = trees.Length != 0 ? ((IEnumerable<TreeObject>) trees).Min<TreeObject>((Func<TreeObject, int>) (x => x.treeId)) : 0;
