  ...
  public class BattleLogicInitializer
  {
    ...
    private void initializeStage(BattleInfo info, BL env)
    {
      ...
      stage.ApplyLandforms((Action<int, int, BattleMapLandform>) ((x, y, landform) =>
      {
        ...
        env.setFeildPanel(y, x, landform.landform.ID, tuple != null ? tuple.Item1 : 0, fieldEvent, winArea, loseArea, battleReinforcements);
      }));
    }
      ...
      BL env)
    {
      ...
      return (IEnumerator) new BattleLogicInitializer.\u003CcreateEnemyUnits\u003Ec__Iterator1A()
      {
        info = info,
        env = env,
        enemies = enemies,
    ...
    {
      // ISSUE: object of a compiler-generated type is created
      return (IEnumerator) new BattleLogicInitializer.\u003CinitializeWave\u003Ec__Iterator1B()
      {
        info = info,
        wave = wave,
        env = env,
      ...
      int positionId)
    {
      ...
      if (info.quest_type == CommonQuestType.Earth || info.quest_type == CommonQuestType.EarthExtra)
        unitPositionList.AddRange(guests.Select<BL.Unit, BattleEarthStageGuest, BL.UnitPosition>((IEnumerable<BattleEarthStageGuest>) info.EarthGuests, (Func<BL.Unit, BattleEarthStageGuest, BL.UnitPosition>) ((a, b) =>
        {
          ...
          guestPosition.resetOriginalPosition();
    ...
    {
      // ISSUE: object of a compiler-generated type is created
      return (IEnumerator) new BattleLogicInitializer.\u003CcreateUnitPositions\u003Ec__Iterator1C()
      {
        info = info,
        players = players,
        env = env,
      ...
      int level)
    {
      if (!targetOnly && skill.target_type == BattleskillTargetType.myself)
      {
        foreach (BattleskillEffect effect in skill.Effects)
          unit.skillEffects.Add(BL.SkillEffect.FromMasterData(unit, effect, skill, level));
      }
      ...
      bool targetOnly = false)
    {
      ...
      {
        if (unit.is_leader || unit.friend || unit.playerUnit.is_enemy_leader)
        {
            ...
            if (!gearGearSkill.skill.range_effect_passive_skill)
            {
              if (!targetOnly)
              {
                foreach (BattleskillEffect effect in gearGearSkill.skill.Effects)
                  unit.skillEffects.Add(BL.SkillEffect.FromMasterData(effect, gearGearSkill.skill, gearGearSkill.skill_level));
              }
            }
            else
        ...
        foreach (Bonus pvpBonus in info.pvp_bonus_list)
        {
          if (Bonus.IsEnableBonus(unit, pvpBonus, info.pvp_start_date))
          {
            foreach (BattleskillEffect effect in pvpBonus.skill.Effects)
              unit.skillEffects.Add(BL.SkillEffect.FromMasterData(effect, pvpBonus.skill, 1));
          }
        }
      }
    }
    ...
    {
      // ISSUE: object of a compiler-generated type is created
      return (IEnumerator) new BattleLogicInitializer.\u003CinitializeData\u003Ec__Iterator1D()
      {
        env = env,
    ...
    {
      // ISSUE: object of a compiler-generated type is created
      return (IEnumerator) new BattleLogicInitializer.\u003CdoStart\u003Ec__Iterator1E()
      {
        battleInfo = battleInfo,
        env = env,
        \u003C\u0024\u003EbattleInfo = battleInfo,
    ...
    private void doBonus(
      BL.Unit u,
      Dictionary<int, List<Bonus>> groups,
      string pvp_start_date,
      StartParameter parm)
    {
      using (BL.Unit unit = u.enableCache())
      {
        List<Bonus> bonusList = new List<Bonus>();
        foreach (List<Bonus> source1 in groups.Values)
        {
          if (source1.All<Bonus>((Func<Bonus, bool>) (b => PvpLogic.IsEnable(unit, b.category, b.logic, b.value, pvp_start_date, parm))))
          {
            foreach (IGrouping<int, Bonus> source2 in source1.GroupBy<Bonus, int>((Func<Bonus, int>) (x => x._skill)))
              bonusList.Add(source2.First<Bonus>());
          }
        }
        foreach (Bonus bonus in bonusList)
        {
          foreach (BattleskillEffect effect in bonus.skill.Effects)
            unit.skillEffects.Add(BL.SkillEffect.FromMasterData(effect, bonus.skill, 1));
        }
      }
    }
  }
