...
public class BacklogManager : MonoBehaviour
{
  ...
  private IEnumerator Initialize()
  {
    ...
    return (IEnumerator) new BacklogManager.\u003CInitialize\u003Ec__Iterator51A()
    {
      \u003C\u003Ef__this = this
    };
  }
  ...
  public void Add(string name, string msg)
  {
    if (msg == string.Empty)
      return;
    string str = this.NormalizeTextColor(msg);
    this.log_data.Add("[" + (this.log_data.Count % 2 != 0 ? (object) "FFFFCC" : (object) "FFFFFF") + "]" + name + (object) '\n' + str + (object) '\n' + (object) '\n');
  }
  ...
  private string NormalizeTextColor(string input)
  {
    string pattern = "\\[.*?\\]";
