...
namespace MasterDataTable
{
  ...
  public class AIScorePattern
  {
    ...
    {
      private static GameGlobalVariable<Dictionary<string, Calculator>> expCache = GameGlobalVariable<Dictionary<string, Calculator>>.New();
      ...
      public AICalculator(int id_, string name, string optional_exp = null)
      {
        ...
        Dictionary<string, Calculator> dictionary = AIScorePattern.AICalculator.expCache.Get();
        if (dictionary.TryGetValue(str, out this.calc))
          return;
        this.calc = new Calculator(str);
      ...
      public string Show(Dictionary<string, float> vars)
      {
        ...
        list.Add("ext");
        return list.Select<string, string, string>((IEnumerable<string>) this.exps, (Func<string, string, string>) ((x, y) => string.Format("{0}({1:0.0000})", (object) x, (object) new Calculator(y).Eval(vars)))).Join(" * ");
      }
