...
public class Guild028117Popup : BackButtonMenuBase
{
  ...
  private IEnumerator SendGuildStatement()
  {
    ...
    return (IEnumerator) new Guild028117Popup.\u003CSendGuildStatement\u003Ec__Iterator69F()
    {
      \u003C\u003Ef__this = this
    };
  }
  ...
  {
    ((Component) this.dragScrollViewWidget).GetComponent<BoxCollider>().size = new Vector3(((Component) this.dragScrollViewWidget).GetComponent<BoxCollider>().size.x, this.dragScrollViewWidget.localSize.y);
    ((Component) this.dragScrollViewWidget).GetComponent<BoxCollider>().center = new Vector3(((Component) this.dragScrollViewWidget).GetComponent<BoxCollider>().center.x, (float) (-(double) this.dragScrollViewWidget.localSize.y * 0.5));
  }
