...
namespace SM
{
  ...
  public class PlayerUnit : KeyCompare
  {
    ...
    public bool IsSpecialEffectForRankingEvent(QuestScoreBonusTimetable[] tables)
    {
      bool flag = false;
      if (tables.Length <= 0)
        return flag;
      foreach (QuestScoreBonusTimetable table in tables)
      {
        QuestScoreBonusRule[] rules = table.rules;
        if (rules.Length > 0)
        {
