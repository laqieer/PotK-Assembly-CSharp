...
public class PopupEditCustomDeckUnit : MonoBehaviour
{
  ...
  [SerializeField]
  private UnityEngine.Sprite sprAddReisou;
  [SerializeField]
    ...
    PlayerCustomDeckUnit_parameter_list param)
  {
    ...
    if (editCustomDeckUnit.jobs_.Length > 1)
    {
      ...
      for (int no = 0; no < editCustomDeckUnit.jobs_.Length; ++no)
      {
        ...
        {
          editCustomDeckUnit.jobChange_.buttons[no].isEnabled = false;
          Color disabledColor = editCustomDeckUnit.jobChange_.buttons[no].disabledColor;
          foreach (UIWidget componentsInChild in editCustomDeckUnit.jobChange_.buttons[no].GetComponentsInChildren<UILabel>())
      ...
      for (int length = editCustomDeckUnit.jobs_.Length; length < editCustomDeckUnit.jobChange_.buttons.Length; ++length)
        editCustomDeckUnit.jobChange_.buttons[length].gameObject.SetActive(false);
    ...
    for (int index = 1; index < array.Length; ++index)
      vector3Array[index] = array[index] - array[index - 1];
    ...
    bool bFocus)
  {
    if (bFocus)
    {
      btn.GetComponent<UIWidget>().color = btn.defaultColor = btn.hover = btn.disabledColor = colors.normal;
      btn.pressed = colors.pressed;
      btn.isEnabled = false;
    }
    else
    {
      btn.GetComponent<UIWidget>().color = btn.defaultColor = btn.hover = colors.normal;
      btn.pressed = colors.pressed;
      btn.isEnabled = true;
    }
  }

  private void setJobChangeButtonEvent(UIButton btn, int no) => EventDelegate.Set(btn.onClick, (EventDelegate.Callback) (() => this.changeJob(no)));
  ...
  private IEnumerator doSetUnitSprite(int jobId)
  {
    ...
    {
      Future<UnityEngine.Sprite> ld = this.unit_.LoadSpriteLarge(jobId, 1f);
      ...
      this.sprUnit_.GetComponent<NGxMaskSpriteWithScale>().FitMask();
    }
  }
...
label_12:
  ...
  private int? getJingiIndex(PlayerItem[] gears, bool bAwakedUnit, bool bOpendEquippedGear3) => !bOpendEquippedGear3 ? new int?() : Gears.getIndexJingiSpace(((IEnumerable<PlayerItem>) gears).Select<PlayerItem, GearGear>((Func<PlayerItem, GearGear>) (x => (object) x == null ? (GearGear) null : x.gear)).ToArray<GearGear>(), bAwakedUnit, true);
  ...
  private void onLongPressedReisou(int slotNo) => this.menu_.onLongPressedReisou(this.param_, slotNo);
  ...
  private void onClickedOverkillers(int slotNo) => this.menu_.onClickedOverkillers(this.param_, slotNo);

  private void onLongPressedOverkillers(int slotNo) => this.menu_.onLongPressedOverkillers(this.param_, slotNo);
