...
public class Quest0528PlayerStatus : MonoBehaviour
{
  ...
  {
    this.character.gameObject.transform.Clear();
    this.unitIcon = normalPrefab.CloneAndGetComponent<UnitIcon>(this.character.gameObject.transform);
  ...
  {
    this.weapon.gameObject.transform.Clear();
    this.gearIcon = gearKindPrefab.CloneAndGetComponent<GearKindIcon>(this.weapon.gameObject.transform);
    NGUITools.AdjustDepth(this.gearIcon.gameObject, this.weapon.depth);
  ...
  private void CreateBattleSkillIcon(GameObject battleSkillIconPrefab)
  {
    ...
    {
      dynAilment.gameObject.transform.Clear();
      BattleSkillIcon component = battleSkillIconPrefab.CloneAndGetComponent<BattleSkillIcon>(dynAilment.gameObject.transform);
    ...
    int? remainTurn)
  {
    ...
    {
      this.skillIcons[index].gameObject.SetActive(true);
  ...
  private IEnumerator ShowDetailStatusPopup()
  {
    ...
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    popup.gameObject.GetComponent<UIWidget>().alpha = 0.0f;
    ...
    popup.gameObject.GetComponent<UIWidget>().alpha = 1f;
