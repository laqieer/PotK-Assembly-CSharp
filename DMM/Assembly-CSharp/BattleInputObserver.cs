...
public class BattleInputObserver : BattleMonoBehaviour
{
  ...
  public void setFuncCheckCancelPressed(Func<bool, bool> func = null) => this.checkCancelPressed = func;
  ...
  protected override IEnumerator Start_Battle()
  {
    ...
    Battle01CommandWait.SetOnClickAction(new System.Action(battleInputObserver.ResetClickedSamePanelCounter));
    return false;
  }
  ...
  protected override void LateUpdate_Battle()
  {
    ...
    this.UpdateFirstDownPanelLongPressTime((System.Action) (() =>
    {
  ...
  public bool containsGrayTargetSelect(BL.Unit unit) => this.isTargetSelectMode && this.selectTargets.grayTargets.Contains(unit);
  ...
  public HashSet<BL.Panel> getMovePanels(BL.UnitPosition up) => !this.isDispositionMode ? up.movePanels : this.dispositionPanels;

  private bool containsMovePanels(BL.Panel panel, BL.UnitPosition up) => (this.isDispositionMode ? BattleFuncs.moveCompletePanels_(this.dispositionPanels, up.unit, isOriginal: false) : up.completePanels).Contains(panel);
  ...
  private void UpdateFirstDownPanelLongPressTime(System.Action action)
  {
  ...
  private void ChangeUnitPosition(BL.UnitPosition up)
  {
    ...
    if ((UnityEngine.Object) this.pvpNode == (UnityEngine.Object) null)
      this.pvpNode = this.battleManager.getUiNode().gameObject.GetComponent<Battle01PVPNode>();
    if (!((UnityEngine.Object) this.pvpNode != (UnityEngine.Object) null))
      return;
    this.pvpNode.DispositionDecide.onClick();
  }

  private bool CanChangeUnitPosition(BL.UnitPosition up) => up != null && up.unit != (BL.Unit) null && (up.unit.isPlayerControl && this.env.core.unitCurrent.unit != (BL.Unit) null) && this.env.core.unitCurrent.unit != up.unit && this.env.core.unitCurrent.unit.isPlayerControl;
    ...
    else
    {
      ...
      if (!this.isDispositionMode)
      {
        if (this.env.core.phaseState.state == BL.Phase.player)
        {
          ...
          {
            if (up.isLocalMoved)
              up.cancelMove(this.env, (System.Action) (() => this.env.core.unitCurrent.commit()));
          }
        }
        this.env.core.fieldCurrent.value = panel;
  ...
  private void MoveUnit(BL.Panel panel, BL.UnitPosition up)
  {
    ...
    this.btm.setTargetBeforeMoveUnit(moveRoute.First<BL.Panel>(), (System.Action) (() =>
    {
  ...
  private bool IsClickedSamePanel(BL.Panel panel, BL.UnitPosition up) => this.env.core.isTouchWait.value && this.battleManager.getUiNode().IsEnabledCommandWait && (up != null && !(up.unit == (BL.Unit) null)) && (!this.env.core.isCompleted(up.unit) && up.unit.isPlayerControl && (!(this.env.core.unitCurrent.unit == (BL.Unit) null) && !(this.env.core.unitCurrent.unit != up.unit))) && panel == this.lastClickedPanel;
  ...
  private void UpdateLastClickedPanelByModifiedUI()
  {
    ...
    if ((UnityEngine.Object) uiNode == (UnityEngine.Object) null || uiNode.IsEnabledCommandWait)
      return;
    this.ResetClickedSamePanelCounter();
  }
  ...
  private void ResetDispositionModeUnit(BL.Unit unit, float initial_direction)
  {
    ...
    if (up.isLocalMoved)
      up.cancelMove(this.env, (System.Action) (() => up.direction = initial_direction));
    else
