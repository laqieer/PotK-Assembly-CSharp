...
public class SortieDeckMainPanel : MonoBehaviour
{
  ...
  private System.Action<SortieDeckMainPanel.DeckMode> eventModeChanged_;
  ...
  public System.Action<DeckInfo> eventDeckChanged { get; set; }
  ...
  public System.Action<PlayerUnit, string, int> eventClickedFriendUnit { get; set; }

  public System.Action eventClickedEditItem { get; set; }

  public System.Action<int> eventClickedSupply { get; set; }

  public System.Action eventClickedBattleSetting { get; set; }

  public System.Action eventClickedEditDeck { get; set; }

  public DeckInfo[] decks { get; private set; }

  public DeckInfo currentDeck { get; private set; }

  public Quest0028Indicator currentIndicator => this.decks == null || this.decks.Length == 0 || this.currentIndex < 0 ? (Quest0028Indicator) null : this.indicators_[this.currentIndex];
  ...
  public IEnumerator doLoadResources()
  {
    ...
    IEnumerator e;
    if (!(bool) (UnityEngine.Object) this.prefabUnitIcon)
    {
    ...
    if (!(bool) (UnityEngine.Object) this.prefabItemIcon)
    {
    ...
    if (!(bool) (UnityEngine.Object) this.prefabEditDeckName)
    {
  ...
  public IEnumerator doInitDecks(DeckInfo[] deckInfos, int[] usedUnitIds)
  {
    ...
    sortieDeckMainPanel.scroll_.enabled = false;
    sortieDeckMainPanel.scroll_.SeEnable = false;
    sortieDeckMainPanel.scroll_.destroyParts(false);
    sortieDeckMainPanel.decks = ((IEnumerable<DeckInfo>) deckInfos).Where<DeckInfo>((Func<DeckInfo, bool>) (x => (uint) ((IEnumerable<int>) x.player_unit_ids).FirstOrDefault<int>() > 0U)).ToArray<DeckInfo>();
    ...
    if ((bool) (UnityEngine.Object) sortieDeckMainPanel.btnAutoDeck_)
      sortieDeckMainPanel.btnAutoDeck_.isEnabled = sortieDeckMainPanel.deckMode == SortieDeckMainPanel.DeckMode.Normal;
    ...
    UIGrid grid = sortieDeckMainPanel.scroll_.scrollView.GetComponentInChildren<UIGrid>();
    grid.onReposition = (UIGrid.OnReposition) (() =>
    {
      this.scroll_.uiScrollView.ResetPosition();
      grid.onReposition = (UIGrid.OnReposition) null;
    });
    ...
    int firstIndex = sortieDeckMainPanel.firstDeckNumber;
    firstIndex = ((IEnumerable<DeckInfo>) sortieDeckMainPanel.decks).FirstIndexOrNull<DeckInfo>((Func<DeckInfo, bool>) (x => x.deck_number == firstIndex)) ?? 0;
    ...
    sortieDeckMainPanel.scroll_.enabled = true;
    sortieDeckMainPanel.currentIndex = -1;
    sortieDeckMainPanel.leaderUnit = (PlayerUnit) null;
    sortieDeckMainPanel.updateDeck(firstIndex);
    sortieDeckMainPanel.scroll_.resetCenterItem(firstIndex);
    yield return (object) null;
  ...
  private int firstDeckNumber
  {
    get
    {
      ...
      int num;
      if (!this.dicStackNumber_.TryGetValue(this.deckMode, out num))
      {
        num = this.deckMode == SortieDeckMainPanel.DeckMode.Custom ? guildRaidLastSortie.Data.customDeckNumber : guildRaidLastSortie.Data.deckNumber;
        this.dicStackNumber_[this.deckMode] = num;
      }
      return num;
    }
  }
  ...
  private void updateDeck(int index)
  {
    ...
    UIButton btnLeaderSkill = this.btnLeaderSkill_;
    PlayerUnit leaderUnit = this.leaderUnit;
    int num = ((object) leaderUnit != null ? leaderUnit.leader_skill?.skill : (BattleskillSkill) null) != null ? 1 : 0;
    btnLeaderSkill.isEnabled = num != 0;
    this.buguButtonEnableSet();
    System.Action<DeckInfo> eventDeckChanged = this.eventDeckChanged;
  ...
  public void BtnWeaponChangeSlot()
  {
    ...
    for (int index = 0; index < this.lnkFriendEquipps_.Length; ++index)
      this.lnkFriendEquipps_[index].gameObject.SetActive(!this.lnkFriendEquipps_[index].gameObject.activeSelf);
  }

  private SortieDeckMainPanel.FriendInfo getFriendInfo(
    string player_id,
    int unit_id)
  {
  ...
  public IEnumerator doInitFriend(
    PlayerUnit friend,
    System.Action<WebAPI.Response.UserError> errorCallback = null)
  {
  ...
  private IEnumerator doInitFriend(PlayerUnit unit, string player_id, int unit_id)
  {
    ...
    this.lnkFriendEquipps_[0].gameObject.SetActive(true);
    this.lnkFriendEquipps_[1].gameObject.SetActive(true);
    this.lnkFriendEquipps_[2].gameObject.SetActive(false);
    if (unit != (PlayerUnit) null && string.IsNullOrEmpty(player_id) && unit_id == 0)
    {
      player_id = unit.player_id;
      unit_id = unit.id;
    }
    uIcon.onClick = (System.Action<UnitIconBase>) (_ => this.onClickedFriendUnit(unit, player_id, unit_id));
    if (unit != (PlayerUnit) null)
      uIcon.onLongPress = (System.Action<UnitIconBase>) (_ => this.onLongPressedFriendUnit(unit, player_id, unit_id));
    else
      uIcon.onLongPress = (System.Action<UnitIconBase>) (_ => this.onClickedFriendUnit());
    this.buguButtonEnableSet();
  }

  private void buguButtonEnableSet()
  {
    if (!((UnityEngine.Object) this.btnBuguChange != (UnityEngine.Object) null))
      return;
  ...
  {
    System.Action<PlayerUnit, string, int> clickedFriendUnit = this.eventClickedFriendUnit;
    if (clickedFriendUnit == null)
      return;
    clickedFriendUnit(unit, player_id, unit_id);
  }

  private void onLongPressedFriendUnit(PlayerUnit unit, string player_id, int unit_id) => Unit0042Scene.changeSceneFriendUnit(true, player_id, unit_id);
  ...
  {
    icon.onClick = (System.Action<ItemIcon>) (_ => this.onClickedEditItem());
    icon.EnableLongPressEvent((System.Action<GameCore.ItemInfo>) (_ => this.onClickedItemIcon(itemId)));
  }

  public void onClickedEditItem()
  {
    System.Action eventClickedEditItem = this.eventClickedEditItem;
  ...
  {
    System.Action<int> eventClickedSupply = this.eventClickedSupply;
  ...
  public void initModeSwitch(
    bool[] bEnables,
    System.Action<SortieDeckMainPanel.DeckMode> callbackChanged)
  {
    ...
    for (int index = 0; index < this.objDeckModeButtons_.Length; ++index)
    {
      ...
      this.objDeckModeButtons_[index].GetComponent<UIButton>().isEnabled = flag;
    }
  }
  ...
  private void changeMode(SortieDeckMainPanel.DeckMode mode)
  {
    ...
    if (this.deckMode == mode)
      return;
    this.deckMode = mode;
    System.Action<SortieDeckMainPanel.DeckMode> eventModeChanged = this.eventModeChanged_;
  ...
  {
    GameObject prefabUnitIcon = this.prefabUnitIcon;
    PlayerUnit leaderUnit = this.leaderUnit;
    PlayerUnit friendUnit1 = this.friendUnit;
    PlayerUnit friendUnit2 = this.friendUnit;
    BattleskillSkill friendLeaderSkill = (object) friendUnit2 != null ? friendUnit2.leader_skill?.skill : (BattleskillSkill) null;
    IEnumerator e = PopupLeaderFriendSkill.show(prefabUnitIcon, false, leaderUnit, friendUnit1, friendLeaderSkill);
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
  }
  ...
  {
    System.Action clickedBattleSetting = this.eventClickedBattleSetting;
  ...
  {
    System.Action eventClickedEditDeck = this.eventClickedEditDeck;
  ...
  private class FriendInfo
  {
    ...
    public FriendInfo(WebAPI.Response.FriendStatus status) => this.makePlayerUnit(status.target_leader_unit, status.target_player_items, status.target_player_reisou_items, status.target_player_awake_skills, status.target_leader_unit_over_killers);
