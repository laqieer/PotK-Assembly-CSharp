...
public class Unit0542Scene : NGSceneBase
{
    ...
    int unitID)
  {
    ...
    if (playerUnit == (PlayerUnit) null)
    {
      if (!this.init)
      {
        Future<WebAPI.Response.FriendStatus> futureF = WebAPI.FriendStatus(player_id, unitID, (System.Action<WebAPI.Response.UserError>) (error =>
        {
    ...
    {
      PlayerUnit[] playerUnits1;
      if (playerUnits == null)
        playerUnits1 = new PlayerUnit[1]{ playerUnit };
      else
        playerUnits1 = ((IEnumerable<PlayerUnit>) playerUnits).Where<PlayerUnit>((Func<PlayerUnit, bool>) (x => x != (PlayerUnit) null)).ToArray<PlayerUnit>();
      if (!this.init)
      {
        this.menu.scrollView.Clear();
        ((IEnumerable<PlayerUnit>) playerUnits1).ForEach<PlayerUnit>((System.Action<PlayerUnit>) (pu =>
        {
          if (pu.id != playerUnit.id)
            return;
          playerUnit = pu;
        }));
        e = this.menu.Init(playerUnit, playerUnits1, false, Unit0542Scene.limited, false, true);
        while (e.MoveNext())
          yield return e.Current;
        e = (IEnumerator) null;
        this.init = true;
      ...
      else
      {
        ...
        ((IEnumerable<PlayerUnit>) playerUnits1).ForEach<PlayerUnit>((System.Action<PlayerUnit>) (x =>
        {
        ...
        {
          e = this.menu.UpdateAllPageForEarth(((IEnumerable<PlayerUnit>) playerUnits1).ToArray<PlayerUnit>(), Unit0542Scene.limited);
          while (e.MoveNext())
            yield return e.Current;
          e = (IEnumerator) null;
        }
  ...
  {
    Unit0542Scene unit0542Scene = this;
    IEnumerator e = unit0542Scene.menu.onEndSceneAsync();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    if (unit0542Scene.tweens != null)
    {
      foreach (Behaviour tween in unit0542Scene.tweens)
        tween.enabled = true;
      NGTween.setOnTweenFinished(unit0542Scene.tweens, (MonoBehaviour) unit0542Scene, "oneFrameWait");
    }
    else
