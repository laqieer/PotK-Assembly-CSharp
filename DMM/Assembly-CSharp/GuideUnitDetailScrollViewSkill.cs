...
public class GuideUnitDetailScrollViewSkill : MonoBehaviour
{
  ...
  public IEnumerator init(UnitUnit unit, PlayerUnitHistory history)
  {
    ...
    foreach (UnitSkill unitSkill in ((IEnumerable<UnitSkill>) unit.RememberUnitAllSkills()).Where<UnitSkill>((Func<UnitSkill, bool>) (x => (uint) x.unit_type > 0U)))
    ...
    List<PopupSkillDetails.Param> objList1 = this.setElementSkill();
    List<PopupSkillDetails.Param> objList2 = this.setGrowthSkill();
    List<PopupSkillDetails.Param> objList3 = this.setMultiSkill();
    List<PopupSkillDetails.Param> objList4 = this.setCommandSkill();
    List<PopupSkillDetails.Param> objList5 = this.setGrantSkill();
    List<PopupSkillDetails.Param> objList6 = this.setDuelSkill();
    List<PopupSkillDetails.Param> objList7 = this.setExtraSkill();
    this.skillParams = first.Concat<PopupSkillDetails.Param>((IEnumerable<PopupSkillDetails.Param>) objList1).Concat<PopupSkillDetails.Param>((IEnumerable<PopupSkillDetails.Param>) objList2).Concat<PopupSkillDetails.Param>((IEnumerable<PopupSkillDetails.Param>) objList5).Concat<PopupSkillDetails.Param>((IEnumerable<PopupSkillDetails.Param>) objList7).Concat<PopupSkillDetails.Param>((IEnumerable<PopupSkillDetails.Param>) objList3).Concat<PopupSkillDetails.Param>((IEnumerable<PopupSkillDetails.Param>) objList4).Concat<PopupSkillDetails.Param>((IEnumerable<PopupSkillDetails.Param>) objList6).ToArray<PopupSkillDetails.Param>();
  ...
  private List<PopupSkillDetails.Param> setCommandSkill()
  {
    ...
    for (int i = 0; index1 < this.dir_Command_Skill_have.Length && i < skills.Length; i++)
    {
      if (!this.setSkills.Contains(skills[i].skill_BattleskillSkill) && !this.prencessSkillIDs.Any<int>((Func<int, bool>) (x => x == skills[i].skill_BattleskillSkill)))
      {
        ...
        bool isLearn = ((IEnumerable<int?>) this.history.skill_ids).Contains<int?>(new int?(skill2.ID));
        this.createBattleSkillIcon(this.dir_Command_Skill_have[index1++], skill1, isLearn);
        objList.Add(new PopupSkillDetails.Param(skill2, UnitParameter.SkillGroup.Command));
      }
    }
    if (index1 < this.dir_Command_Skill_have.Length && this.unit.hasSEASkills)
    {
      PlayerUnitSkills skill1 = this.unit.SEASkill.getSkill(0);
      BattleskillSkill skill2 = skill1.skill;
      this.createBattleSkillIcon(this.dir_Command_Skill_have[index1], skill2);
      this.setIconEvent(this.dir_Command_Skill_have[index1++], skill2);
      objList.Add(PopupSkillDetails.Param.createBySEASkillView(skill1, this.history.isUnlockedSEASkill, 0));
    }
    for (int index2 = 0; index2 < index1; ++index2)
    {
      this.slc_Command_Skill_none[index2].gameObject.SetActive(false);
  ...
  {
    this.slc_Extra_Skill_none.gameObject.SetActive(this.unit.trust_target_flag);
    this.slc_Extra_SkillNone_base.gameObject.SetActive(!this.unit.trust_target_flag);
    this.txt_ExtraSkill.gameObject.SetActive(this.unit.trust_target_flag);
