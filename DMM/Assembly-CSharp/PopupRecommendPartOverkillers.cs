...
public class PopupRecommendPartOverkillers : PopupRecommendPart
{
  ...
  public override IEnumerator doInitialize(PlayerUnit playerUnit, UnitUnit target)
  {
    ...
    {
      recommendPartOverkillers.gameObject.SetActive(false);
    }
    else
    {
      recommendPartOverkillers.gameObject.SetActive(true);
      ...
      icon1.setLevelText("--");
      icon1.Button.enabled = false;
      icon1.buttonBoxCollider.enabled = false;
      icon1 = (UnitIcon) null;
      ...
      {
        recommendPartOverkillers.iconSkill_.gameObject.SetActive(true);
        Future<GameObject> ldSkill = Res.Prefabs.BattleSkillIcon._battleSkillIcon.Load<GameObject>();
        yield return (object) ldSkill.Wait();
        BattleSkillIcon icon2 = ldSkill.Result.Clone(recommendPartOverkillers.iconSkill_.transform).GetComponent<BattleSkillIcon>();
        yield return (object) icon2.Init(recommendPartOverkillers.overkillersSkill_.skill);
        icon2.iconSprite.depth = recommendPartOverkillers.iconSkill_.depth + 1;
        ldSkill = (Future<GameObject>) null;
        icon2 = (BattleSkillIcon) null;
      }
      else
        recommendPartOverkillers.iconSkill_.gameObject.SetActive(false);
    }
  }

  public void onClickedSkill()
  {
    if (this.overkillersSkill_ == null || (UnityEngine.Object) this.menu.skillDetailPrefab == (UnityEngine.Object) null)
      return;
