  ...
  public class PlayerCustomDeckUnit_parameter_list : KeyCompare
  {
    ...
    public bool setUnit(int id, PlayerUnit[] units, PlayerItem[] gears, PlayerAwakeSkill[] skills)
    {
      ...
      if (playerUnit1 != (PlayerUnit) null)
      {
        ...
        {
          PlayerItem playerItem = array1[slotIndex];
          GearGear gearGear = (object) playerItem != null ? playerItem.gear : (GearGear) null;
          if (array1[slotIndex] == (PlayerItem) null || !gearGear.checkCanEquipByProficiency(playerUnit1, gearGear.is_jingi ? unitGearProficiency : (PlayerUnitGearProficiency) null))
          {
    ...
    public bool checkBlank()
    {
      ...
      if (this.player_unit_id != 0)
        ...
        flag = false;
      else if (((IEnumerable<int>) this.player_gear_ids).Any<int>((Func<int, bool>) (i => (uint) i > 0U)))
        flag = false;
      return flag;
    }
    ...
    public PlayerCustomDeckUnit_parameter_list cloneBySave(
      UnitUnit unit)
    {
      PlayerCustomDeckUnit_parameter_list unitParameterList = (PlayerCustomDeckUnit_parameter_list) this.MemberwiseClone();
      unitParameterList.player_gear_ids = ((IEnumerable<int>) this.player_gear_ids).ToArray<int>();
    ...
    private void setGears(PlayerUnit target, PlayerItem[] gears, bool bSetPrimaryReisou)
    {
      ...
      PlayerUnit playerUnit1 = target;
      PlayerItem equippedGear = target.FindEquippedGear(gears);
      PlayerItem playerItem1 = (object) equippedGear != null ? equippedGear.Clone() : (PlayerItem) null;
      playerUnit1.primary_equipped_gear = playerItem1;
      PlayerUnit playerUnit2 = target;
      PlayerItem equippedGear2 = target.FindEquippedGear2(gears);
      PlayerItem playerItem2 = (object) equippedGear2 != null ? equippedGear2.Clone() : (PlayerItem) null;
      playerUnit2.primary_equipped_gear2 = playerItem2;
      PlayerUnit playerUnit3 = target;
      PlayerItem equippedGear3 = target.FindEquippedGear3(gears);
      PlayerItem playerItem3 = (object) equippedGear3 != null ? equippedGear3.Clone() : (PlayerItem) null;
      playerUnit3.primary_equipped_gear3 = playerItem3;
    ...
    {
      PlayerItem playerItem = this.getGear(gears, index);
      if (playerItem != (PlayerItem) null)
      {
        playerItem = playerItem.Clone();
        playerItem.resetByCustomDeck(bUnitEquipped);
      }
      return playerItem;
    }
    ...
    public int getGearId(int index) => this.player_gear_ids.Length <= index ? 0 : this.player_gear_ids[index];

    public PlayerItem createReisou(PlayerItem[] gears, int index)
    {
      PlayerItem playerItem = this.getReisou(gears, index);
      if (playerItem != (PlayerItem) null)
        playerItem = playerItem.Clone();
      return playerItem;
    }
    ...
    public int getReisouId(int index) => this.player_reisou_ids.Length <= index ? 0 : this.player_reisou_ids[index];

    public PlayerUnit[] createOverkillers(PlayerUnit[] units) => PlayerCustomDeck.isCustom(((IEnumerable<PlayerUnit>) units).FirstOrDefault<PlayerUnit>()) ? ((IEnumerable<int>) this.over_killers_ids).Select<int, PlayerUnit>((Func<int, PlayerUnit>) (i =>
    {
    ...
    {
      PlayerUnit playerUnit = i == 0 || i == -1 ? (PlayerUnit) null : Array.Find<PlayerUnit>(units, (Predicate<PlayerUnit>) (x => x.id == i));
      if (playerUnit != (PlayerUnit) null)
      {
        playerUnit = playerUnit.Clone();
        playerUnit.resetByCustomDeck();
      }
      return playerUnit;
    })).ToArray<PlayerUnit>();
