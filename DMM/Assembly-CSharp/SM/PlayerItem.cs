...
namespace SM
{
  ...
  public class PlayerItem : KeyCompare
  {
    ...
    public PlayerItem(GearGear gear, MasterDataTable.CommonRewardType entity_type) => this.Set(gear, entity_type);
    ...
    public static bool operator ==(PlayerItem lhs, PlayerItem rhs) => (object) lhs == null ? (object) rhs == null : lhs.Equals(rhs);
...
label_3:
    ...
    public GearGear gear => this.entity_type == MasterDataTable.CommonRewardType.gear ? MasterData.GearGear[this.entity_id] : (GearGear) null;

    public SupplySupply supply => this.entity_type == MasterDataTable.CommonRewardType.supply ? MasterData.SupplySupply[this.entity_id] : (SupplySupply) null;

    public PlayerItem equipReisou => ((IEnumerable<PlayerItem>) SMManager.Get<PlayerItem[]>()).FirstOrDefault<PlayerItem>((Func<PlayerItem, bool>) (x => x.id == this.equipped_reisou_player_gear_id));

    public bool isReisouSet => (uint) this.equipped_reisou_player_gear_id > 0U;
    ...
    {
      get => this.reisouHolyLv > 0 ? this.reisouHolyLv : this.GetPlayerMythologyGearStatus().holy_gear_level;
    ...
    {
      get => this.reisouChaosLv > 0 ? this.reisouChaosLv : this.GetPlayerMythologyGearStatus().chaos_gear_level;
      set => this.reisouChaosLv = value;
    }

    public void SetPlayerMythologyGearStatusCache(PlayerMythologyGearStatus value) => this._playerMythologyGearStatusCache = value;
    ...
    public GearReisouSkill[] getReisouSkills(
      int equip_gear_id,
      bool isIgnoreWeaponGroup = false)
    {
      ...
      if (this.gear.isReisou())
      {
        ...
        if (source3.Count > 0)
        {
          foreach (IGrouping<int, GearReisouSkill> source4 in source3.GroupBy<GearReisouSkill, int>((Func<GearReisouSkill, int>) (x => x.skill_group)))
            source1.Add(source4.OrderByDescending<GearReisouSkill, int>((Func<GearReisouSkill, int>) (x => x.release_rank)).First<GearReisouSkill>());
        }
      }
    ...
    public bool isWeapon() => !this.isSupply() && !this.isCompse() && (!this.isExchangable() && !this.isReisou());
    ...
    public bool isCompse() => !this.isSupply() && this.gear.kind.Enum == GearKindEnum.smith && this.gear.compose_kind.kind.Enum != GearKindEnum.smith;
    ...
    public GearReisouFusion GetReisouFusionPossibleRecipe(PlayerItem[] playerItems)
    {
      ...
      if (this.isHolyReisou())
      {
        foreach (GearReisouFusion gearReisouFusion in MasterData.GearReisouFusion.Values)
        {
          if (gearReisouFusion.holy_ID_GearGear == this.entity_id)
            return gearReisouFusion;
        }
      }
      else if (this.isChaosReisou())
      {
        foreach (GearReisouFusion gearReisouFusion in MasterData.GearReisouFusion.Values)
        {
          if (gearReisouFusion.chaos_ID_GearGear == this.entity_id)
            return gearReisouFusion;
        }
      }
      return (GearReisouFusion) null;
    ...
    public GearReisouFusion GetReisouFusionMineRecipe()
    {
      ...
      foreach (GearReisouFusion gearReisouFusion in MasterData.GearReisouFusion.Values)
      {
        if (gearReisouFusion.mythology_ID_GearGear == this.gear.ID)
          return gearReisouFusion;
      }
      return (GearReisouFusion) null;
    }
    ...
    public int GetReisouRankLimit()
    {
      ...
      {
        if (reisouRankIncr.gear_kind.Enum == this.gear.kind.Enum && reisouRankIncr.group_id == this.gear.rank_incr_group && reisouRankIncr.level >= num)
          num = reisouRankIncr.level;
      }
      return num;
    }
    ...
    public BL.Item toBLItem() => new BL.Item()
    {
