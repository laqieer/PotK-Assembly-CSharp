...
namespace SM
{
  ...
  public class Player : KeyCompare
  {
    ...
    public bool CheckMaxHavingGear() => ((IEnumerable<PlayerItem>) SMManager.Get<PlayerItem[]>()).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.entity_type == MasterDataTable.CommonRewardType.gear && !x.isReisou())).Count<PlayerItem>() >= this.max_items;

    public bool CheckMaxHavingReisou() => ((IEnumerable<PlayerItem>) SMManager.Get<PlayerItem[]>()).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.entity_type == MasterDataTable.CommonRewardType.gear && x.isReisou())).Count<PlayerItem>() >= this.max_reisou_items;
    ...
    public bool CheckLimitOverMaxUnitReserves() => SMManager.Get<PlayerUnitReservesCount>().count > this.max_unit_reserves;
    ...
    {
      bool res = false;
      ((IEnumerable<string>) this.extension.Split(':')).ForEach<string>((System.Action<string>) (x =>
      {
        if (!(x == str))
          return;
        res = true;
    ...
    public bool IsCalledUnit(UnitUnit unit) => unit != null && this.IsCalledUnit(unit.same_character_id);

    public bool IsCalledUnit(int same_character_id) => ((IEnumerable<int>) this.call_skill_same_character_ids).Contains<int>(same_character_id);
    ...
    public Player(Dictionary<string, object> json)
    {
      ...
      foreach (object obj in (List<object>) json[nameof (gacha_tickets)])
        playerGachaTicketList.Add(obj == null ? (PlayerGachaTicket) null : new PlayerGachaTicket((Dictionary<string, object>) obj));
      ...
      foreach (object obj in (List<object>) json[nameof (call_divorce_histories)])
        callDivorceHistoryList.Add(obj == null ? (PlayerCallDivorceHistory) null : new PlayerCallDivorceHistory((Dictionary<string, object>) obj));
