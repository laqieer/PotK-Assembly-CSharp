...
namespace SM
{
  ...
  public class PlayerCustomDeck : KeyCompare
  {
    ...
    public PlayerCustomDeck(Dictionary<string, object> json)
    {
      ...
      foreach (object obj in (List<object>) json[nameof (unit_parameter_list)])
        unitParameterListList.Add(obj == null ? (PlayerCustomDeckUnit_parameter_list) null : new PlayerCustomDeckUnit_parameter_list((Dictionary<string, object>) obj));
    ...
    public static DeckInfo createDeckInfo(PlayerCustomDeck deck) => (DeckInfo) new PlayerCustomDeck.ImpPlayerCustomDeck(deck);

    public static DeckInfo createGuildRaidDeckInfo(PlayerCustomDeck deck) => (DeckInfo) new PlayerCustomDeck.ImpGuildRaidCustomDeck(deck);

    public bool isClone { get; private set; }

    public bool isModified
    {
      get => this.isModified_ || ((IEnumerable<PlayerCustomDeckUnit_parameter_list>) this.unit_parameter_list).Any<PlayerCustomDeckUnit_parameter_list>((Func<PlayerCustomDeckUnit_parameter_list, bool>) (x => x.isModified));
    ...
    public PlayerCustomDeck cloneBySave(PlayerUnit[] units)
    {
      PlayerCustomDeck playerCustomDeck = (PlayerCustomDeck) this.MemberwiseClone();
      playerCustomDeck.isClone = true;
      playerCustomDeck.player_unit_ids = ((IEnumerable<int>) this.player_unit_ids).Where<int>((Func<int, bool>) (i => (uint) i > 0U)).ToArray<int>();
    ...
    {
      SaveDeckUnitIds[] saveDeckUnitIdsArray = new SaveDeckUnitIds[Mathf.Max(this.member_limit, maxSpace)];
      for (int index = 0; index < this.unit_parameter_list.Length; ++index)
      {
        PlayerCustomDeckUnit_parameter_list unitParameter = this.unit_parameter_list[index];
        saveDeckUnitIdsArray[index] = new SaveDeckUnitIds()
        {
      ...
      for (int length = this.unit_parameter_list.Length; length < saveDeckUnitIdsArray.Length; ++length)
        saveDeckUnitIdsArray[length] = new SaveDeckUnitIds()
        {
          gears = new int[0],
          reisous = new int[0],
          skill = 0,
    ...
    public bool updateUnits(
      ...
      System.Action<int, int> callbackSwap)
    {
      bool flag = false;
    ...
    {
      return ((IEnumerable<int>) this.player_unit_ids).Where<int>((Func<int, bool>) (i => (uint) i > 0U)).Select<int, PlayerCustomDeckUnit_parameter_list>((Func<int, PlayerCustomDeckUnit_parameter_list>) (n => Array.Find<PlayerCustomDeckUnit_parameter_list>(this.unit_parameter_list, (Predicate<PlayerCustomDeckUnit_parameter_list>) (x => x.player_unit_id == n)))).Select<PlayerCustomDeckUnit_parameter_list, PlayerUnit>((Func<PlayerCustomDeckUnit_parameter_list, PlayerUnit>) (y => y.createPlayerUnit(units, gears, awakeSkills))).ToArray<PlayerUnit>();
    }

    public Dictionary<int, PlayerUnit> createGearReference(PlayerUnit[] units)
    {
      Dictionary<int, PlayerUnit> dictionary = new Dictionary<int, PlayerUnit>(this.unit_parameter_list.Length);
        ...
        if (p.player_unit_id != 0)
        {
          ...
          for (int index = 0; index < p.player_gear_ids.Length; ++index)
          {
            if (p.player_gear_ids[index] != 0)
              dictionary[p.player_gear_ids[index]] = playerUnit;
          }
        }
      }
      return dictionary;
    ...
    public Dictionary<int, Tuple<PlayerUnit, PlayerItem>> createReisouReference(
      ...
      PlayerItem[] gears)
    {
      Dictionary<int, Tuple<PlayerUnit, PlayerItem>> dictionary = new Dictionary<int, Tuple<PlayerUnit, PlayerItem>>(this.unit_parameter_list.Length);
      foreach (PlayerCustomDeckUnit_parameter_list unitParameter in this.unit_parameter_list)
      {
        PlayerCustomDeckUnit_parameter_list p = unitParameter;
        if (p.player_unit_id != 0)
        {
          ...
          for (int n = 0; n < p.player_reisou_ids.Length; ++n)
          {
            if (p.player_gear_ids[n] != 0 && p.player_reisou_ids[n] != 0)
              dictionary[p.player_reisou_ids[n]] = Tuple.Create<PlayerUnit, PlayerItem>(playerUnit, Array.Find<PlayerItem>(gears, (Predicate<PlayerItem>) (y => y.id == p.player_gear_ids[n])));
          }
        }
      }
      return dictionary;
    ...
    public Dictionary<int, PlayerUnit> createOverkillersReference(
      PlayerUnit[] units)
    {
      Dictionary<int, PlayerUnit> dictionary = new Dictionary<int, PlayerUnit>(this.unit_parameter_list.Length);
      foreach (PlayerCustomDeckUnit_parameter_list unitParameter in this.unit_parameter_list)
          ...
          for (int index = 0; index < p.over_killers_ids.Length; ++index)
          {
            switch (p.over_killers_ids[index])
            {
              ...
              case 0:
                continue;
              default:
                dictionary[p.over_killers_ids[index]] = playerUnit;
                continue;
            }
          }
        }
    ...
    public Dictionary<int, PlayerUnit> createAwakeSkillReference(
      PlayerUnit[] units)
    {
      Dictionary<int, PlayerUnit> dictionary = new Dictionary<int, PlayerUnit>(this.unit_parameter_list.Length);
      foreach (PlayerCustomDeckUnit_parameter_list unitParameter in this.unit_parameter_list)
      {
        ...
        if (p.player_unit_id != 0 && p.awake_skill_id != 0)
          dictionary[p.awake_skill_id] = Array.Find<PlayerUnit>(units, (Predicate<PlayerUnit>) (x => x.id == p.player_unit_id));
      }
      return dictionary;
    }
    ...
    public PlayerUnit[] player_units
    {
      get
      {
        if (!((IEnumerable<int>) this.player_unit_ids).Any<int>((Func<int, bool>) (i => (uint) i > 0U)))
        ...
        PlayerUnit[] playerUnitArray = new PlayerUnit[this.member_limit];
        for (int index = 0; index < playerUnitArray.Length; ++index)
        {
          ...
          if (unitParameterList != null)
            playerUnitArray[index] = unitParameterList.createPlayerUnit(units, array, awakeSkills);
        }
        return playerUnitArray;
      }
    }

    public int total_combat => ((IEnumerable<PlayerUnit>) this.player_units).Sum<PlayerUnit>((Func<PlayerUnit, int>) (x => !(x != (PlayerUnit) null) ? 0 : Judgement.NonBattleParameter.FromPlayerUnit(x).Combat));

    public int cost => ((IEnumerable<PlayerUnit>) this.player_units).Sum<PlayerUnit>((Func<PlayerUnit, int>) (x => (object) x == null ? 0 : x.cost));

    public static PlayerUnit cloneByCustom(PlayerUnit target) => (PlayerUnit) new PlayerCustomDeck.ImpPlayerUnit(target);

    public static bool isCustom(PlayerUnit target) => target is PlayerCustomDeck.ImpPlayerUnit;

    public static PlayerItem cloneByCustom(PlayerItem target) => (PlayerItem) new PlayerCustomDeck.ImpPlayerGear(target);
    ...
    private class ImpPlayerCustomDeck : DeckInfo
    {
      ...
      public override PlayerUnit[] player_units => this.player_units_ ?? (this.player_units_ = this.deck_.player_units);

      public override int total_combat => !this.total_combat_.HasValue ? (this.total_combat_ = new int?(this.deck_.total_combat)).Value : this.total_combat_.Value;

      public override int cost => !this.cost_.HasValue ? (this.cost_ = new int?(this.deck_.cost)).Value : this.cost_.Value;
    }
