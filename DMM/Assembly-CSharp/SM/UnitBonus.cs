...
namespace SM
{
  ...
  public class UnitBonus : KeyCompare
  {
    ...
    public UnitBonus(Dictionary<string, object> json)
    {
      ...
      foreach (object obj in (List<object>) json[nameof (quest_setting_list)])
        questSettingList.Add(obj == null ? (QuestSetting) null : new QuestSetting((Dictionary<string, object>) obj));
    ...
    public Period eventPeriod
    {
      get
      {
        Period[] periodArray = SMManager.Get<Period[]>();
        return periodArray != null ? ((IEnumerable<Period>) periodArray).FirstOrDefault<Period>((Func<Period, bool>) (x => x.period_id == this.period_id)) : (Period) null;
      }
    }

    public bool IsTargetQuestS(int quest_type, int quest_s_id) => ((IEnumerable<QuestSetting>) this.quest_setting_list).Any<QuestSetting>((Func<QuestSetting, bool>) (x => x.quest_type_id == quest_type && x.quest_s_id == quest_s_id));

    public bool IsTargetQuestM(int quest_type, int quest_m_id) => ((IEnumerable<QuestSetting>) this.quest_setting_list).Any<QuestSetting>((Func<QuestSetting, bool>) (x => x.quest_type_id == quest_type && x.quest_s_id == quest_m_id));

    public static UnitBonus[] getActiveUnitBonus(
      DateTime serverTime,
      int? questType = null,
      int? qusetID = null)
    {
      UnitBonus[] unitBonusArray = SMManager.Get<UnitBonus[]>();
      return unitBonusArray != null ? ((IEnumerable<UnitBonus>) unitBonusArray).Where<UnitBonus>((Func<UnitBonus, bool>) (x =>
      {
