...
namespace SM
{
  ...
  public class PlayerUnitJob_abilities : KeyCompare
  {
    ...
    public int skill2_id => !this.skill2_id_.HasValue ? (this.skill2_id_ = new int?(this.master != null ? (this.master.skill2_BattleskillSkill.HasValue ? this.master.skill2_BattleskillSkill.Value : 0) : 0)).Value : this.skill2_id_.Value;
    ...
    public BattleskillSkill skill
    {
      get
      {
        ...
        if (this.skill_id != 0 && !MasterData.BattleskillSkill.TryGetValue(this.skill_id, out battleskillSkill))
    ...
    public JobCharacteristicsLevelupPattern current_levelup_pattern
    {
      get
      {
        JobCharacteristicsLevelupPattern characteristicsLevelupPattern = (JobCharacteristicsLevelupPattern) null;
        if (this.master != null && this.level >= 0 && this.master.levelup_patterns.Length > this.level)
          MasterData.JobCharacteristicsLevelupPattern.TryGetValue(this.master.levelup_patterns[this.level], out characteristicsLevelupPattern);
        return characteristicsLevelupPattern;
      }
    }
      ...
      int checkLevel = -1)
    {
      ...
      List<KeyValuePair<UnitUnit, int>> keyValuePairList = new List<KeyValuePair<UnitUnit, int>>();
      ...
      for (int index = 0; index < jobMaterialGroupArray.Length; ++index)
      {
        if (jobMaterialGroupArray[index] != null && nullableArray[index].HasValue)
        {
          ...
          if (jobMaterialUsed != null)
          {
            ...
            if (unit2 != null)
              keyValuePairList.Add(new KeyValuePair<UnitUnit, int>(unit2, nullableArray[index].Value));
          }
        }
      }
    ...
    public string levelmaxBonusTypeText1 => this.genLevelmaxBonusTypeText(this.master.levelmax_bonus);

    public string levelmaxBonusTypeText2 => this.genLevelmaxBonusTypeText(this.master.levelmax_bonus2);

    public string levelmaxBonusTypeText3 => this.genLevelmaxBonusTypeText(this.master.levelmax_bonus3);
