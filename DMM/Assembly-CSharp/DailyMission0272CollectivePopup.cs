...
public class DailyMission0272CollectivePopup : BackButtonMonoBehaiviour
{
    ...
    GameObject[] itemPrefabs)
  {
    ...
    for (int index = 0; index < count; ++index)
      itemPrefab.Clone(this.grid.transform).GetComponent<DailyMission0272CollectivePopupList>().Initialize(reward_list[index], index);
    bool flag = !point_reward_list.IsNullOrEmpty<PointReward>();
    if (flag)
    {
      this.pointRewards = point_reward_list.ToArray();
      Vector2 localPosition = (Vector2) this.topPointReward.transform.localPosition;
      localPosition.y = -this.grid.cellHeight * (float) count;
      this.topPointReward.transform.localPosition = (Vector3) localPosition;
  ...
  private IEnumerator Start()
  {
    ...
    if (bExistPointReward)
    {
      ...
      {
        IEnumerator e = mission0272CollectivePopup.pointRewardPrefab.Clone(mission0272CollectivePopup.gridPoint.transform).GetComponent<MissionPointRewardDetailItemController>().Init(mission0272CollectivePopup.pointRewards[n], true, n == nlast);
        while (e.MoveNext())
    ...
    Singleton<PopupManager>.GetInstance().startOpenAnime(mission0272CollectivePopup.gameObject);
  }
