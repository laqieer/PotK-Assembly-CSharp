...
public class DuelSupport : MonoBehaviour
{
  ...
  public void setNumbers(int hit, int dodge, int critical, int dodgecritical)
  {
    ...
    if (hit == 0 && dodge == 0 && (critical == 0 && dodgecritical == 0))
      return;
    this.itemHeight *= this.gameObject.transform.localScale.x;
    ...
    GameObject strGO)
  {
    ...
    Vector3 position = strGO.transform.parent.gameObject.transform.position;
    position.y -= this.itemHeight * (float) this.dispCount;
    strGO.transform.parent.gameObject.transform.position = position;
    ++this.dispCount;
  }
  ...
  {
    int num1 = num % 100;
    int[] numArray = new int[2]{ 0, num1 / 10 };
    numArray[0] = num1 % 10;
    return numArray;
  }
  ...
  private IEnumerator disableThis(float duration)
  {
    ...
    if (num != 0)
    {
      ...
      // ISSUE: reference to a compiler-generated field
      this.\u003C\u003E1__state = -1;
      duelSupport.gameObject.SetActive(false);
      return false;
    }
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = -1;
