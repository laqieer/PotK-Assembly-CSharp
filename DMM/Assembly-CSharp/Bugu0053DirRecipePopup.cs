...
public class Bugu0053DirRecipePopup : BackButtonMenuBase
{
    ...
    Dictionary<int, Tuple<List<GameCore.ItemInfo>, int>> playerGearDicDescending)
  {
    ...
    foreach (GearCombineRecipe gearRecipe in gearRecipes)
    {
      ...
      {
        e = component.Init(recipePopup, gearRecipe, allRecipes, buttonEvent, playerGearDic, playerGearDicDescending, itemIconPrefab, recipePopup.recipeListPopupPrefabs, new System.Action(recipePopup.IbtnClose), player_.money, 0);
        while (e.MoveNext())
          yield return e.Current;
        e = (IEnumerator) null;
      }
      else
        recipePopup.StartCoroutine(component.Init(recipePopup, gearRecipe, allRecipes, buttonEvent, playerGearDic, playerGearDicDescending, itemIconPrefab, recipePopup.recipeListPopupPrefabs, new System.Action(recipePopup.IbtnClose), player_.money, beforeLoadingCount - 4));
      ++beforeLoadingCount;
    }
  ...
  public void IbtnClose()
  {
    if (this.IsPush)
      return;
    this.gameObject.GetComponent<NGTweenParts>().isActive = false;
  }
