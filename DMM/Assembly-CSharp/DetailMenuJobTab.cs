...
public class DetailMenuJobTab : MonoBehaviour
{
  ...
  public bool initialize(
    ...
    bool bLimit,
    System.Action<DetailMenuJobAbilityParts, PlayerUnitJob_abilities, bool, bool> eventClicked,
    System.Action<UIButton, int> eventChanged)
  {
    if ((bool) (UnityEngine.Object) this.defaultPanel_)
    ...
    {
      PlayerUnit playerUnit1 = array2[index1].Item1;
      int num2 = array2[index1].Item2;
      MasterDataTable.UnitJob job = playerUnit1.getJobData();
      DetailMenuJobTab.JobParts part = this.jobChange_.parts[index1];
      bool isVertexX = job.is_vertex_x;
      bool flag = intSet.Contains(job.ID);
      if ((UnityEngine.Object) part.topNormal != (UnityEngine.Object) null)
        part.topNormal.SetActive(!isVertexX);
      if ((UnityEngine.Object) part.topX != (UnityEngine.Object) null)
      ...
      int num3 = 0;
      int count = 0;
      for (int index2 = 0; index2 < jobAbilities2.Length; ++index2)
      {
        BattleskillSkill skill = jobAbilities2[index2].skill;
        if (!skill.IsLand)
        {
          if (part.abilityTags.Length > count)
          {
            DetailMenuJobTab.AbilityTag abilityTag = part.abilityTags[count++];
            string text2 = jobAbilities2[index2].level.ToString();
            if (text2 == "0" && !flag)
              text2 = "-";
            ...
            if (MasterData.JobCharacteristics.TryGetValue(jobAbilities2[index2].job_ability_id, out jobCharacteristics))
            {
              XLevelLimits xlevelLimits = jobCharacteristics.xlevel_limits;
              if (xlevelLimits != null)
                num3 += xlevelLimits.getLimit(jobAbilities2[index2].level);
            }
          }
          else
            break;
      ...
      if (isVertexX)
      {
        if (flag)
        {
          ...
          int num4 = jobProficiencies != null ? jobProficiencies.proficiency : 0;
          txtProficiency.SetTextLocalize(num4);
        }
        else
        {
      ...
      if ((bool) (UnityEngine.Object) part.button)
      {
        if (bLimit || !((IEnumerable<int?>) playerUnit.changed_job_ids).Contains<int?>(new int?(job.ID)))
        {
          part.button.gameObject.SetActive(false);
        }
        else
        {
          part.button.gameObject.SetActive(true);
  ...
  private void resetTab() => ((IEnumerable<GameObject>) new GameObject[2]
  {
