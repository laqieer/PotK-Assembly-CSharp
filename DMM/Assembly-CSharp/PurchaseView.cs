...
public class PurchaseView : MonoBehaviour, IPurchaseFlowListener, IPurchaseResultListener
{
  ...
  {
    if (!((UnityEngine.Object) this.displayTouchEffectController != (UnityEngine.Object) null))
      return;
  ...
  public IEnumerator Init(NGxScroll ngS, WebAPI.Response.CoinbonusHistory h)
  {
    ...
    if ((UnityEngine.Object) listener.displayTouchEffectController != (UnityEngine.Object) null)
  ...
  public void Confirm(PurchaseFlow flow, ProductInfo product) => ModalWindow.ShowYesNo("確認が必要です", string.Format("{0}を{1}で購入しますか？", (object) product.LocalizedTitle, (object) product.LocalizedPrice), (System.Action) (() => flow.Confirmed(true)), (System.Action) (() => flow.Confirmed(false)));

  public void OnInvalidBirthday(PurchaseFlow flow) => PurchaseBehavior.ShowPopupWithMessage(Consts.GetInstance().SHOP_99981_MENU_01, Consts.GetInstance().SHOP_99981_MENU_02);
  ...
  public void OnInsufficientBalances() => PurchaseBehavior.ShowPopupYesNo("購入エラー", "DMMポイントが不足しています。チャージしますか？", (System.Action) (() => Application.OpenURL("https://point.dmm.com/choice/pay?basket_service_type=freegame")));
  ...
  {
    PurchaseBehaviorLoadingLayer.Disable();
    PurchaseBehavior.ShowPopupWithMessage("購入エラー", "すでに姫石を購入済みです。データを再ダウンロードします。", (System.Action) (() =>
    {
    ...
    if (!this.isInitialized)
    {
      ...
      foreach (ProductInfo productsInfo in productsInfos)
      {
        ...
        if (activeProductData.type != 3 && activeProductData.type != 4 && (activeProductData.type != 5 && activeProductData.type != 7))
        {
          ...
          {
            for (int index2 = 0; index2 < this.history.simple_packs.Length; ++index2)
            {
              SimplePackInfo packInfo = this.history.simple_packs[index2];
