...
public class Battle02UIPlayerStatusDetail : Battle02MenuBase
{
  ...
  private GameObject createIcon(GameObject prefab, Transform trans)
  {
    ...
    componentInChildren1.depth = NGUITools.CalculateNextDepth(trans.gameObject);
    return gameObject;
  }

  private string getAttackRange(BL.Unit v) => string.Format("{0} - {1}", (object) ((IEnumerable<int>) v.attackRange).Min(), (object) ((IEnumerable<int>) v.attackRange).Max());
  ...
  public override void UpdateData()
  {
    ...
    this.spaTypeIconParent[0].gameObject.GetComponent<UI2DSprite>().enabled = false;
    this.spaTypeIconParent[1].gameObject.GetComponent<UI2DSprite>().enabled = false;
