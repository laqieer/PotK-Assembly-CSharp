...
public class Quest0028Indicator : MonoBehaviour
{
    ...
    GameObject prefabItemIcon)
  {
    ...
    for (i = 0; i < deckInfo.player_units.Length; ++i)
    {
      ...
      int num = (object) playerUnit != null ? playerUnit.id : 0;
      e = this.LoadUnitPrefab(i, this.DeckUnitData[i], ((IEnumerable<int>) usedUnitIds).Contains<int>(num));
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
    }
    ...
    bool sortieNot = false)
  {
    ...
    if (unit != (PlayerUnit) null)
    {
      ...
      else
      {
        ...
        {
          unitScript.onClick = (System.Action<UnitIconBase>) (x => this.ChangeDetailScene(unit, index));
  ...
  private IEnumerator doInitUnitGears(int index, PlayerUnit unit, bool bUsed = false)
  {
    ...
    for (int i = 0; i < slotNum; ++i)
    {
      ...
      if (Singleton<NGGameDataManager>.GetInstance().IsSea)
        gearIcon.transform.localScale = new Vector3(0.65f, 0.65f);
      else
        gearIcon.transform.localScale = new Vector3(1f, 1f);
      ...
      gearIcon.onClick = (System.Action<ItemIcon>) (_ => this.ChangeWpEquipScene(unit, changeGearIndex));
      gearIcon.Gray = bUsed;
    }
