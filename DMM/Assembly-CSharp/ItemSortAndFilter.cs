...
public class ItemSortAndFilter : SortAndFilter
{
  ...
  public System.Action SortFilterItemNum;
  [SerializeField]
  ...
  public void Initialize(Bugu005ItemListMenuBase menu, bool setUI = false)
  {
    ...
    if (persist != null)
    {
      try
      {
        ...
        {
          if (!((UnityEngine.Object) filterBtn == (UnityEngine.Object) null) && filterBtn.FilterType < (ItemSortAndFilter.FILTER_TYPES) num2)
    ...
    {
      this.isEquipBtn.gameObject.SetActive(true);
      ...
      for (int index = 0; index < this.SortBtns.Length; ++index)
        this.SortBtns[index].gameObject.SetActive(true);
    }
    else
    {
      this.isEquipBtn.gameObject.SetActive(false);
      for (int index = 0; index < this.SortBtns.Length; ++index)
      {
        if (this.SortBtns[index].SortType == ItemSortAndFilter.SORT_TYPES.Rank || this.SortBtns[index].SortType == ItemSortAndFilter.SORT_TYPES.RankMax || this.SortBtns[index].SortType == ItemSortAndFilter.SORT_TYPES.GetOrder)
          this.SortBtns[index].gameObject.SetActive(false);
      }
    }
    if (setUI)
    {
  ...
  private void SetFilterLabelColor()
  {
    ...
    if (this.modeType == ItemSortAndFilter.ModeTypes.Sort)
    {
      ...
      if (flag)
        this.filterLabel.color = new Color(0.5f, 0.5f, 0.0f);
      else
        this.filterLabel.color = new Color(0.5f, 0.5f, 0.5f);
    }
    else
    {
  ...
  private void SetSortTabLabel() => this.sortLabel.SetText(this.sortLabelDic[this.menuBase.SortCategory]);
