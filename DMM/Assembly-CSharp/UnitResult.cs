...
public class UnitResult : MonoBehaviour
{
    ...
    Unit004832Menu.ResultPlayerUnit resultPlayerUnit)
  {
    ...
    if (resultPlayerUnit.afterPlayerUnit.is_trust)
    {
      ...
      unitResult.slcTextDearDegree.width = atlasSprite.width;
      unitResult.slcTextDearDegree.height = atlasSprite.height;
      unitResult.dirDearDegree.SetChildren(unitResult.dirDearDegree.transform.GetChildren().Select<Transform, GameObject>((Func<Transform, GameObject>) (x => x.gameObject)).ToArray<GameObject>());
      unitResult.dirDearDegree.enabled = true;
    ...
    {
      unitResult.txtToutaValue.gameObject,
      unitResult.txtToutaUpAmount.gameObject
    });
    unitResult.dirToutaBlink.enabled = true;
