...
namespace UniLinq
{
  public static class Enumerable
  {
    ...
    public static IEnumerable<TResult> Repeat<TResult>(TResult element, int count) => count >= 0 ? Enumerable.CreateRepeatIterator<TResult>(element, count) : throw new ArgumentOutOfRangeException();

    private static IEnumerable<TResult> CreateRepeatIterator<TResult>(
      TResult element,
      int count)
    {
      for (int i = 0; i < count; ++i)
    ...
    private static IEnumerable<TSource> CreateReverseIterator<TSource>(
      IEnumerable<TSource> source)
    {
    ...
    {
      Check.SourceAndKeySelector((object) source, (object) keySelector);
      List<TSource> sourceList1 = (List<TSource>) null;
      ...
      {
        TKey key = keySelector(source1);
        List<TSource> sourceList2;
        if ((object) key == null)
        {
          if (sourceList1 == null)
            sourceList1 = new List<TSource>();
          sourceList2 = sourceList1;
        }
        else if (!lookup.TryGetValue(key, out sourceList2))
        {
          sourceList2 = new List<TSource>();
          lookup.Add(key, sourceList2);
        }
        sourceList2.Add(source1);
      }
      return (ILookup<TKey, TSource>) new Lookup<TKey, TSource>(lookup, (IEnumerable<TSource>) sourceList1);
    }
    ...
    {
      Check.SourceAndKeyElementSelectors((object) source, (object) keySelector, (object) elementSelector);
      List<TElement> elementList1 = (List<TElement>) null;
      ...
      {
        TKey key = keySelector(source1);
        List<TElement> elementList2;
        if ((object) key == null)
        {
          if (elementList1 == null)
            elementList1 = new List<TElement>();
          elementList2 = elementList1;
        }
        else if (!lookup.TryGetValue(key, out elementList2))
        {
          elementList2 = new List<TElement>();
          lookup.Add(key, elementList2);
        }
        elementList2.Add(elementSelector(source1));
      }
      return (ILookup<TKey, TElement>) new Lookup<TKey, TElement>(lookup, (IEnumerable<TElement>) elementList1);
    }
    ...
    private static Exception EmptySequence() => (Exception) new InvalidOperationException("Sequence contains no elements");

    private static Exception NoMatchingElement() => (Exception) new InvalidOperationException("Sequence contains no matching element");

    private static Exception MoreThanOneElement() => (Exception) new InvalidOperationException("Sequence contains more than one element");

    private static Exception MoreThanOneMatchingElement() => (Exception) new InvalidOperationException("Sequence contains more than one matching element");
