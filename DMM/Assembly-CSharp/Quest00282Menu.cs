...
public class Quest00282Menu : BackButtonMenuBase
{
  ...
  private void SetElementBtnState(int index)
  {
    ...
    {
      SpreadColorButton component = this.elementBtn[index1].gameObject.GetComponent<SpreadColorButton>();
      if (index1 != index)
      {
        component.SetColor(Color.gray);
        this.elementBtn[index1].gameObject.GetComponentInChildren<UITexture>(true).color = Color.gray;
      }
      else
      {
        component.SetColor(Color.white);
        this.elementBtn[index1].gameObject.GetComponentInChildren<UITexture>(true).color = Color.white;
      }
    }
  }
  ...
  public void SetEnableOtherElementBtn(bool isEnabled)
  {
    ...
    for (int index = 0; index < this.elementBtn.Length; ++index)
    {
      if (this.element_index != index)
        this.elementBtn[index].isEnabled = isEnabled;
    }
  }
  ...
  private IEnumerator ChangeElement(CommonElement type, int index)
  {
    ...
    quest00282Menu.scroll.scrollView.gameObject.GetComponent<UIPanel>().enabled = false;
    ...
    bool backSceneSelected = true)
  {
    ...
    if (!Singleton<NGGameDataManager>.GetInstance().IsSea)
    {
      for (int index = 0; index < quest00282Menu.elementBtn.Length; ++index)
        quest00282Menu.elementBtn[index].gameObject.SetActive(true);
    ...
    {
      for (int index = 0; index < quest00282Menu.elementBtn.Length; ++index)
        quest00282Menu.elementBtn[index].gameObject.SetActive(false);
    }
  ...
  private IEnumerator RefreshFavoriteMember()
  {
    ...
    if (Singleton<NGGameDataManager>.GetInstance().IsSea)
    {
      ((IEnumerable<SeaPlayerHelper>) SMManager.Get<SeaPlayerHelper[]>()).ToList<SeaPlayerHelper>().ForEach((System.Action<SeaPlayerHelper>) (x => helper2.Add(new Helper(x))));
    }
    else
    {
      Future<WebAPI.Response.PlayerHelpers> helpers2 = WebAPI.PlayerHelpers(0, (System.Action<WebAPI.Response.UserError>) (e => WebAPI.DefaultUserErrorCallback(e)));
      e1 = helpers2.Wait();
      while (e1.MoveNext())
      ...
      ((IEnumerable<PlayerHelper>) SMManager.Get<PlayerHelper[]>()).ToList<PlayerHelper>().ForEach((System.Action<PlayerHelper>) (x => helper2.Add(new Helper(x))));
  ...
  {
    Future<WebAPI.Response.PlayerHelpers> helpers = WebAPI.PlayerHelpers((int) element, (System.Action<WebAPI.Response.UserError>) (e => WebAPI.DefaultUserErrorCallback(e)));
  ...
  private IEnumerator GetCurrentElementFriendList(CommonElement element)
  {
    ...
    if (quest00282Menu.elementDic.ContainsKey(element))
    {
      ...
      foreach (Helper key in quest00282Menu.elementDic[element])
        dictionary.Add(key, (int) (dateTime - key.target_player_last_signed_in_at).TotalSeconds);
    }
    yield return (object) dictionary;
  }
  ...
  private IEnumerator InitFriendList(Dictionary<Helper, int> sortList)
  {
    ...
    {
      GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(menu.FriendListPrefab);
    ...
    GameObject gameObject1 = UnityEngine.Object.Instantiate<GameObject>(menu.FriendListPrefab);
    ...
    menu.scroll.scrollView.gameObject.GetComponent<UIPanel>().enabled = true;
    Singleton<CommonRoot>.GetInstance().HideLoadingLayer();
    menu.SetEnableOtherElementBtn(true);
    menu.SetEnableFriendBtn(true);
  }

  private bool existGuildMember(Dictionary<Helper, int> dic) => !dic.Where<KeyValuePair<Helper, int>>((Func<KeyValuePair<Helper, int>, bool>) (x => x.Key.is_guild_member)).FirstOrDefault<KeyValuePair<Helper, int>>().Equals((object) new KeyValuePair<Helper, int>());
