...
public class Bugu00510Menu : BackButtonMenuBase
{
  ...
  {
    if (!(bool) (UnityEngine.Object) this.sortPopupPrefab)
    {
  ...
  public IEnumerator StartAsync(bool isForceSetup = false)
  {
    ...
    if (Bugu00510Scene.isInit)
    {
      ...
      bugu00510Menu.dirRecipe.gameObject.GetComponent<NGTweenParts>().isActive = false;
    ...
    if (bugu00510Menu.currentRecipe == null || isForceSetup || bugu00510Menu.isIntegrated)
    {
      ...
      ((IEnumerable<PlayerMaterialGear>) SMManager.Get<PlayerMaterialGear[]>()).ForEach<PlayerMaterialGear>((System.Action<PlayerMaterialGear>) (x => itemInfoList.Add(new GameCore.ItemInfo(x))));
...
label_5:
        return false;
      })).GroupBy<GearCombineRecipe, int>((Func<GearCombineRecipe, int>) (x => MasterData.GearGear[x.combined_gear_id].group_id)).ForEach<IGrouping<int, GearCombineRecipe>>((System.Action<IGrouping<int, GearCombineRecipe>>) (x => this.recipeDataList.Add(new RecipeData((IEnumerable<GearCombineRecipe>) x, x.Any<GearCombineRecipe>((Func<GearCombineRecipe, bool>) (y => this.CheckEnableRecipe(playerItemData, y)))))));
  ...
  private List<RecipeData> Filter(List<RecipeData> recipes, List<bool> filter) => recipes.FilterBy(filter.ToArray()).ToList<RecipeData>();

  private List<RecipeData> Sort(
  ...
  private void SetScrollItem(List<RecipeData> recipes)
  {
    ...
    for (int index = 0; index < Mathf.Min(ItemIcon.MaxValue, recipes.Count); ++index)
    {
      ...
      this.scroll.Add(itemIcon.gameObject, ItemIcon.Width, ItemIcon.Height);
  ...
  private void IconClicked(ItemIcon itemIcon)
  {
    if (this.IsPush)
      return;
    this.StartCoroutine(this.OpenRecipePopup(this.recipeDataList.FirstOrDefault<RecipeData>((Func<RecipeData, bool>) (x => (UnityEngine.Object) x.icon == (UnityEngine.Object) itemIcon))));
  }

  private void GearDetail(ItemIcon itemIcon)
  {
    RecipeData recipe = this.recipeDataList.FirstOrDefault<RecipeData>((Func<RecipeData, bool>) (x => (UnityEngine.Object) x.icon == (UnityEngine.Object) itemIcon));
  ...
  {
    if (recipes[recipeIndex] != null && (UnityEngine.Object) recipes[recipeIndex].icon == (UnityEngine.Object) this.allItemIcon[iconCount])
      return;
    recipes.Where<RecipeData>((Func<RecipeData, bool>) (a => (UnityEngine.Object) a.icon == (UnityEngine.Object) this.allItemIcon[iconCount])).ForEach<RecipeData>((System.Action<RecipeData>) (b => b.icon = (ItemIcon) null));
  ...
  {
    if (Singleton<PopupManager>.GetInstance().isOpen || (UnityEngine.Object) this.sortPopupPrefab == (UnityEngine.Object) null)
      return;
    GameObject prefab = this.sortPopupPrefab.Clone();
    RecipeSortAndFilter sortAndFilter = prefab.GetComponent<RecipeSortAndFilter>();
    System.Action<List<bool>> onUpdateFilter = (System.Action<List<bool>>) (f => sortAndFilter.SetItemNum(this.recipeDataList.FilterBy(f.ToArray()).Count<RecipeData>(), this.recipeDataList.Count));
    System.Action<Persist.RecipeSortAndFilterInfo> onDicision = (System.Action<Persist.RecipeSortAndFilterInfo>) (info =>
    {
  ...
  private IEnumerator SortIcon()
  {
    ...
    this.recipeDataList.ForEach((System.Action<RecipeData>) (x => x.icon = (ItemIcon) null));
    this.SortFilter();
  ...
  {
    if (this.SortLabel == null || (UnityEngine.Object) this.SortLabel[0] == (UnityEngine.Object) null)
      return;
  ...
  private List<PlayerItem> GetItemList() => ((IEnumerable<PlayerItem>) SMManager.Get<PlayerItem[]>()).Where<PlayerItem>((Func<PlayerItem, bool>) (x => !x.broken)).ToList<PlayerItem>();

  private List<PlayerMaterialGear> GetMaterialList() => ((IEnumerable<PlayerMaterialGear>) SMManager.Get<PlayerMaterialGear[]>()).Where<PlayerMaterialGear>((Func<PlayerMaterialGear, bool>) (x =>
  {
  ...
  protected override void Update()
  {
    ...
    float base_y = this.scroll.scrollView.transform.localPosition.y - this.scroolStartY;
  ...
  private bool UpdateScroll(int checkHeight, float base_y, float add_height)
  {
    ...
    foreach (GameObject gameObject in this.scroll)
    {
      ...
      int? nullable = this.displayDataList.FirstIndexOrNull<RecipeData>((Func<RecipeData, bool>) (v => (UnityEngine.Object) v.icon != (UnityEngine.Object) null && (UnityEngine.Object) v.icon.gameObject == (UnityEngine.Object) item));
