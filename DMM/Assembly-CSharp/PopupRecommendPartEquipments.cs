...
public class PopupRecommendPartEquipments : PopupRecommendPart
{
  ...
  public override IEnumerator doInitialize(PlayerUnit playerUnit, UnitUnit target)
  {
    ...
    if (MasterData.UnitRecommend.TryGetValue(playerUnit.unit.same_character_id, out unitRecommend))
    {
      ...
      recommendPartEquipments.isExistQuests_ = unitRecommend.story_quests != null && unitRecommend.story_quests.Length != 0 || unitRecommend.sea_quests != null && unitRecommend.sea_quests.Length != 0 || unitRecommend.extra_quests != null && (uint) unitRecommend.extra_quests.Length > 0U;
    }
    bool flag1 = recommendPartEquipments.targetGear_ != null;
    if (!flag1 && target.rarity.index < 5)
    {
      recommendPartEquipments.gameObject.SetActive(false);
    }
    else
    {
      ...
      else
      {
        ...
        if (lstSkills.Count > 0)
        {
          ...
          for (; index < recommendPartEquipments.lnkSkills_.Length; ++index)
            recommendPartEquipments.lnkSkills_[index].gameObject.SetActive(false);
          recommendPartEquipments.skills_ = skillParams.ToArray();
          ldSkill = (Future<GameObject>) null;
        }
        else
          ((IEnumerable<Transform>) recommendPartEquipments.lnkSkills_).Select<Transform, GameObject>((Func<Transform, GameObject>) (x => x.gameObject)).SetActives(false);
  ...
  {
    if ((UnityEngine.Object) this.menu.skillDetailPrefab == (UnityEngine.Object) null || this.skills_ == null || this.skills_.Length <= index)
      return;
    PopupSkillDetails.show(this.menu.skillDetailPrefab, this.skills_, index);
  }

  private void onPopupQuestList() => PopupGearQuestList.open(this.prefabQuest_, this.playerUnit_, this.menu.onChangedQuest, (System.Action<PopupUtility.SceneTo>) null, this.menu.isDisabledChangeQuest);
  ...
  private GearGear getSpecialGear()
  {
    ...
    foreach (GearSpecificationOfEquipmentUnit specificationOfEquipmentUnit in ((IEnumerable<GearSpecificationOfEquipmentUnit>) MasterData.GearSpecificationOfEquipmentUnitList).Where<GearSpecificationOfEquipmentUnit>((Func<GearSpecificationOfEquipmentUnit, bool>) (y => groups.Contains(y.group_id) && (uint) y.unit_id > 0U)))
    {
      UnitUnit unitUnit;
