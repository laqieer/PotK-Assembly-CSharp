...
public class LumpToutaMaterialConfirmMenu : UnitSelectMenuBase
{
    ...
    Dictionary<PlayerUnit, List<PlayerUnit>> allSamePlayerUnits)
  {
    ...
    e = (IEnumerator) null;
    if ((UnityEngine.Object) materialConfirmMenu1.toutaIconPrefab == (UnityEngine.Object) null)
    {
    ...
    foreach (PlayerUnit selectedBasePlayerUnit1 in selectedBasePlayerUnits)
    {
      ...
      List<Tuple<PlayerMaterialUnit, UnitUnit, float>> tupleList = new List<Tuple<PlayerMaterialUnit, UnitUnit, float>>();
      foreach (Tuple<PlayerMaterialUnit, int[]> tuple in ((IEnumerable<Tuple<PlayerMaterialUnit, int[]>>) valueUpMaterials).Where<Tuple<PlayerMaterialUnit, int[]>>((Func<Tuple<PlayerMaterialUnit, int[]>, bool>) (o => ((IEnumerable<int>) o.Item2).Contains<int>(baseUnitUnit.ID))))
      {
        ...
        tupleList.Add(Tuple.Create<PlayerMaterialUnit, UnitUnit, float>(tuple.Item1, unit, unityValueUpPattern.up_value));
      }
      List<Tuple<PlayerMaterialUnit, UnitUnit, float>> list = materialConfirmMenu1.sortUnityValueUpMaterials((IEnumerable<Tuple<PlayerMaterialUnit, UnitUnit, float>>) tupleList).Take<Tuple<PlayerMaterialUnit, UnitUnit, float>>(30).ToList<Tuple<PlayerMaterialUnit, UnitUnit, float>>();
    ...
    materialConfirmMenu1.InitializeInfoEx((IEnumerable<PlayerUnit>) playerUnitList.ToArray(), (IEnumerable<PlayerMaterialUnit>) playerMaterialUnitList.ToArray(), (Persist<Persist.UnitSortAndFilterInfo>) null, false, false, false, true, true, false, new System.Action(materialConfirmMenu1.\u003CStartAsync\u003Eb__14_2));
    ...
    do
    {
      ...
      else
      {
        for (int index = 0; remainValue > 0 && index < swork.materials.Length; ++index)
        {
          ...
          {
            int num4 = Mathf.Min(material.material.quantity, remainValue / lastValue);
            material.quantity = num4;
          }
  ...
  private LumpToutaMaterialConfirmMenu.UnityValueUpMaterial findUnitValueUpMaterial(
    UnitUnit u)
  {
    LumpToutaMaterialConfirmMenu.UnityValueUpMaterial unityValueUpMaterial = (LumpToutaMaterialConfirmMenu.UnityValueUpMaterial) null;
    foreach (LumpToutaMaterialConfirmMenu.ShareWork shareWork in this.dicShareWorks.Values)
    {
      unityValueUpMaterial = Array.Find<LumpToutaMaterialConfirmMenu.UnityValueUpMaterial>(shareWork.materials, (Predicate<LumpToutaMaterialConfirmMenu.UnityValueUpMaterial>) (x => x.material._unit == u.ID));
      if (unityValueUpMaterial != null)
        break;
    }
    return unityValueUpMaterial;
  }
  ...
  private IEnumerator UnityUpIconSelect(UnitIcon selectUnitIcon)
  {
    ...
    System.Action<int> onOk = (System.Action<int>) (selectedCount =>
    {
  ...
  protected override void CreateUnitIconAction(int info_index, int unit_index)
  {
    ...
    allUnitIcon.onLongPress = (System.Action<UnitIconBase>) (x => Unit0042Scene.changeSceneEvolutionUnit(true, info.playerUnit, this.getUnits(), true, IsToutaPlusNoEnable: true));
  }
  ...
  private IEnumerator Combine()
  {
    ...
    // ISSUE: reference to a compiler-generated method
    component.callbackNo = new System.Action(materialConfirmMenu.\u003CCombine\u003Eb__32_0);
