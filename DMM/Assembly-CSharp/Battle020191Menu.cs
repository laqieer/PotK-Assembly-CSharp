...
public class Battle020191Menu : BackButtonMenuBase
{
    ...
    bool fromUnitBase = false)
  {
    ...
    if (skills.Length != 0)
    {
      ...
      Future<UnityEngine.Sprite> skillIcon = skills[0].skill.LoadBattleSkillIcon();
      e = skillIcon.Wait();
      while (e.MoveNext())
      ...
      skillIcon = (Future<UnityEngine.Sprite>) null;
    }
    if (fromUnitBase)
    {
      battle020191Menu.CharaSprite.SetActive(false);
      battle020191Menu.GetComponent<BoxCollider>().enabled = false;
      battle020191Menu.GetComponent<UIButton>().enabled = false;
    }
    else
    {
    ...
    if (!fromEarth)
      ...
      battle020191Menu.princessTypeSprite.gameObject.SetActive(false);
    ...
    battle020191Menu.gaugeMax = ((IEnumerable<int>) numArray).Max();
  ...
  private IEnumerator Play()
  {
    ...
    foreach (Battle020191Menu.PlayData play in this.PlayList)
    {
      ...
      if (num > 0)
      {
        ...
        while (!this.isSkip && tween.enabled)
