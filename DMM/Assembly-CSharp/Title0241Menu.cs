...
public class Title0241Menu : BackButtonMenuBase
{
  ...
  protected Future<GameObject> GetSortAndFilterPopupGameObject() => Res.Prefabs.popup.popup_024_3__anim_popup01.Load<GameObject>();

  public virtual Persist<Persist.EmblemSortAndFilterInfo> GetPersist() => Persist.emblemSortAndFilter;
  ...
  {
    Title0241Menu title0241Menu = this;
    GameObject gameObject = title0241Menu.ScrollPrefab.Clone(title0241Menu.grid.transform);
    bool isNew = false;
    DateTime? time = new DateTime?();
    if (hasEmblem)
    {
      PlayerEmblem playerEmblem = ((IEnumerable<PlayerEmblem>) title0241Menu.emblems).Where<PlayerEmblem>((Func<PlayerEmblem, bool>) (x => x.emblem_id == emblem.ID)).First<PlayerEmblem>();
      isNew = playerEmblem.is_new;
      time = new DateTime?(playerEmblem.created_at);
    }
    IEnumerator e = gameObject.GetComponent<Title0241Scroll>().Init(hasEmblem, emblem.ID, emblem.rarity_EmblemRarity, emblem.description, emblem.ID == title0241Menu.curEmblemID, isNew, time, new System.Action(title0241Menu.EmblemsScrollUpdate), title0241Menu.target_player_id == Player.Current.id, (BackButtonMenuBase) title0241Menu);
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
  }
  ...
  private IEnumerator ListSorting()
  {
    ...
    foreach (Component component in title0241Menu1.grid.transform)
    ...
    title0241Menu1.grid.gameObject.SetActive(true);
  ...
  {
    Future<UnityEngine.Sprite> sprF = EmblemUtility.LoadEmblemSprite(this.curEmblemID);
