    ...
    ICollection
  {
    ...
    public int Capacity
    {
      ...
      set
      {
        ...
        if (this.count > 0)
        {
          Array.Copy((Array) this.keys, 0, (Array) keyArray, 0, this.count);
          Array.Copy((Array) this.values, 0, (Array) objArray, 0, this.count);
        }
        this.keys = keyArray;
    ...
    public TKey[] Keys
    {
      get
      {
        TKey[] keyArray = new TKey[this.count];
        Array.Copy((Array) this.keys, (Array) keyArray, keyArray.Length);
        return keyArray;
      }
    }
    ...
    public TValue[] Values
    {
      get
      {
        TValue[] objArray = new TValue[this.count];
        Array.Copy((Array) this.values, (Array) objArray, objArray.Length);
        return objArray;
      }
    }
    ...
    public void RemoveWhere(Func<KeyValuePair<TKey, TValue>, bool> predicate)
    {
      ...
      {
        this.keys[index2] = default (TKey);
        this.values[index2] = default (TValue);
      }
      this.count = index1;
    }

    public override string ToString() => "{" + Enumerable.Range(0, this.count).Select<int, string>((Func<int, string>) (i => "\"{0}\":{1}".F((object) this.keys[i], (object) this.values[i]))).Join(", ") + "}";
    ...
    IDictionaryEnumerator IDictionary.GetEnumerator() => (IDictionaryEnumerator) new AssocList<TKey, TValue>.DictEnum(this.GetEnumerator());
