...
namespace GameCore
{
  public class AILisp : IELisp
  {
    ...
    private void init(BL e)
    {
      ...
      {
        List<BL.AIUnit> source = new List<BL.AIUnit>();
        ...
        return source.Any<BL.AIUnit>() ? SExp.toLispList<BL.AIUnit>((IEnumerable<BL.AIUnit>) source) : (object) null;
      }));
      ...
      {
        BL.AIUnit unit = this.checkType<BL.AIUnit>("get-target-route", args, 0);
        BL.Panel fieldPanel = this.battleEnv.getFieldPanel((BL.UnitPosition) unit);
        return SExp.toLispList<BL.Panel>((IEnumerable<BL.Panel>) this.createTargetRoute((BL.UnitPosition) unit, fieldPanel, this.getTargetPanel(unit, fieldPanel)).Item1);
      }));
...
label_6:
      ...
      {
        BL.AIUnit aiUnit1 = this.checkType<BL.AIUnit>("create-attack-status-list", args, 0);
        BL.AIUnit aiUnit2 = this.checkType<BL.AIUnit>("create-attack-status-list", args, 1);
        bool flag = this.nth(2, args) != null;
        BL.AIUnit aiUnit3 = aiUnit2;
        int num = flag ? 1 : 0;
        return SExp.toLispList<AttackStatus>((IEnumerable<AttackStatus>) BattleFuncs.getAttackStatusArray((BL.UnitPosition) aiUnit1, (BL.UnitPosition) aiUnit3, true, num != 0, true));
      }));
      this.defun("hit-attack-status-cache", (Func<Cons, object>) (args =>
      {
        BL.AIUnit aiUnit1 = this.checkType<BL.AIUnit>("hit-attack-status-cache", args, 0);
        BL.AIUnit aiUnit2 = this.checkType<BL.AIUnit>("hit-attack-status-cache", args, 1);
        bool isHeal = this.nth(2, args) != null;
        BL.Unit[] attackNeighbors;
        BL.Unit[] defenseNeighbors;
        int move_distance;
        int move_range;
        BattleFuncs.makeAttackStatusArgs((BL.UnitPosition) aiUnit1, (BL.UnitPosition) aiUnit2, true, isHeal, out attackNeighbors, out defenseNeighbors, out move_distance, out move_range, isAI: true);
        return !this.battleEnv.getAttackStatusCache((BL.ISkillEffectListUnit) aiUnit1, this.battleEnv.getFieldPanel((BL.UnitPosition) aiUnit1), attackNeighbors, (BL.ISkillEffectListUnit) aiUnit2, this.battleEnv.getFieldPanel((BL.UnitPosition) aiUnit2), defenseNeighbors, aiUnit1.hp, true, isHeal, move_distance, move_range, true, out AttackStatus[] _) ? (object) null : this.trueObject;
      }));
      this.defun("create-action-result", (Func<Cons, object>) (args =>
      {
        BL.AIUnit aiUnit1 = this.checkType<BL.AIUnit>("create-action-result", args, 0);
        AttackStatus attackStatus = this.checkType<AttackStatus>("create-action-result", args, 1);
        BL.AIUnit aiUnit2 = this.checkType<BL.AIUnit>("create-action-result", args, 2);
        BL.Panel fieldPanel1 = this.battleEnv.getFieldPanel(aiUnit1.originalRow, aiUnit1.originalColumn);
        BL.Panel fieldPanel2 = this.battleEnv.getFieldPanel(aiUnit1.row, aiUnit1.column);
        BL.Panel fieldPanel3 = this.battleEnv.getFieldPanel(aiUnit2.row, aiUnit2.column);
        ...
        BL.AIUnit aiUnit4 = aiUnit2;
        BL.Panel defensePanel = fieldPanel3;
        int move_distance = num1;
        int move_range = num2;
        DuelResult duelResult = BattleFuncs.calcDuel(attackStatus, (BL.ISkillEffectListUnit) aiUnit3, attackPanel, (BL.ISkillEffectListUnit) aiUnit4, defensePanel, move_distance, move_range, true);
        duelResult.isMove = true;
        duelResult.row = aiUnit1.row;
        duelResult.column = aiUnit1.column;
      ...
      {
        BL.AIUnit aiUnit1 = this.checkType<BL.AIUnit>("set-current", args, 0);
        BL.AIUnit aiUnit2 = this.checkType<BL.AIUnit>("set-current", args, 1);
      ...
      this.defun("set-current-targets", (Func<Cons, object>) (args =>
      {
        ...
        Cons args1 = this.checkTypeCons("set-current-targets", args, 0);
        int num = SExp.length((object) args1);
        for (int idx = 0; idx < num; ++idx)
          this.currentTargets.Add(this.checkType<BL.AIUnit>("set-current-targets", args1, idx));
      ...
      if (!(str2 == "own"))
      {
        if (!(str2 == "target"))
        {
          if (!(str2 == "panel"))
          {
            ...
            {
              if (!(str2 == "battle"))
                obj = str2 == "current" ? (str1 == "unit" ? (object) this.currentUnit : (str1 == "target" ? (object) this.currentTarget : (str1 == "panel" ? (object) this.currentPanel : (str1 == "skill" ? (object) this.currentSkill : (object) null)))) : (object) null;
              else if (this.battleVariables.ContainsKey(str1))
              {
                obj = this.battleVariables[str1];
              }
              else
              {
                if (!(str1 == "sum_own"))
                {
                  if (!(str1 == "sum_enemy"))
                  {
                    if (!(str1 == "turn"))
                    {
                      if (!(str1 == "is_own_team_can_attacked"))
                      {
                        obj = str1 == "sum_enemy_range_1" ? (object) this.numberDic.numberObject(this.battleEnv.getAroundEnemyUnitsCount(this.currentUnit, 1)) : (str1 == "sum_enemy_range_2" ? (object) this.numberDic.numberObject(this.battleEnv.getAroundEnemyUnitsCount(this.currentUnit, 2)) : (object) null);
                      }
                      else
                      {
                        obj = (object) null;
                        foreach (BL.AIUnit aiUnit in this.battleEnv.aiUnits.value)
                        {
                          if (this.getDangerPanels().Contains(this.battleEnv.getFieldPanel(aiUnit.originalRow, aiUnit.originalColumn)))
                          {
                            obj = this.trueObject;
                            break;
                          }
                      ...
                      obj = (object) this.numberDic.numberObject(this.battleEnv.phaseState.turnCount);
                  }
                  else
                    obj = (object) this.numberDic.numberObject(this.battleEnv.getTargetAIUnits(this.currentUnit, BL.Unit.TargetAttribute.attack).Count);
                }
                else
                  obj = (object) this.numberDic.numberObject(this.battleEnv.getTargetAIUnits(new BL.ForceID[1]
                  {
                    this.currentUnit.getForceID(this.battleEnv)
                  }, BL.Unit.TargetAttribute.attack).Count);
                this.battleVariables[str1] = obj;
              }
            }
            else
              obj = this.getSkillVariable(this.currentSkill, this.currentTargets, str1);
          }
          else
            obj = this.getPanelVariable(this.currentPanel, this.currentUnit, this.currentTarget, str1);
        }
        else
          obj = this.getUnitVariable(this.currentTarget, this.currentUnit, (AttackStatus) null, str1);
      }
      else
        obj = this.getUnitVariable(this.currentUnit, this.currentTarget, this.currentAttackStatus, str1);
      return obj;
    }
    ...
    private object getPanelVariable(BL.Panel panel, BL.AIUnit unit, BL.AIUnit target, string sym)
    {
      ...
      {
        case "enemy_distance":
          obj = (object) this.numberDic.numberObject(this.createTargetRoute((BL.UnitPosition) unit, panel, this.getTargetPanel(unit, panel)).Item1.Count);
          break;
        case "eva":
          obj = (object) this.numberDic.numberObject(incr.evasion_incr);
          break;
        case "heal":
          obj = (object) this.numberDic.numberObject(incr.hp_healing_ratio);
          break;
        case "is_danger":
          obj = this.getDangerPanels().Contains(panel) ? this.trueObject : (object) null;
          break;
        case "magic":
          obj = (object) this.numberDic.numberObject(incr.magic_defense_incr);
          break;
        case "move_distance":
          obj = (object) this.numberDic.numberObject(this.battleEnv.getTargetRouteWithCache((BL.UnitPosition) unit, this.battleEnv.getFieldPanel(unit.originalRow, unit.originalColumn), panel, false).Item1.Count);
          break;
        case "phisics":
          obj = (object) this.numberDic.numberObject(incr.physical_defense_incr);
          break;
      }
      return obj;
    }
    ...
    private object getSkillVariable(BL.Skill skill, List<BL.AIUnit> targets, string sym)
    {
      ...
      if (sym == "sum_target_num")
        return (object) this.numberDic.numberObject(targets.Count);
      if (!(sym == "sum_target_hp"))
      {
        if (!(sym == "sum_target_combat"))
          return (object) null;
        int i = 0;
    ...
    {
      BL.Panel panel1 = unit.GetTargetPanelOrNull(this.battleEnv);
      if (panel1 == null)
      {
        panel1 = this.getAIGroupTarget(unit);
        if (panel1 == null)
        {
          ...
          {
            panel1 = this.battleEnv.getFieldPanel((BL.UnitPosition) unit);
          }
          else
          {
            int num1 = 1000000000;
    ...
    private List<BL.UnitPosition> getValidSkillTargets(BL.AIUnit aiUnit, BL.Skill skill)
    {
        ...
        if (skill.isNonSelect)
        {
          if (!source.Any<BL.UnitPosition>((Func<BL.UnitPosition, bool>) (x =>
          {
            if (!provokeUnits.Contains(x.unit))
              return false;
            BL.AIUnit aiUnit1 = (BL.AIUnit) x;
            return aiUnit1.skillEffects.CanUseSkill(skill.skill, skill.level, (BL.ISkillEffectListUnit) aiUnit1, this.battleEnv, (BL.ISkillEffectListUnit) aiUnit, skill.nowUseCount) == 0;
          })))
      ...
      foreach (BL.UnitPosition unitPosition in source)
      {
        BL.AIUnit aiUnit1 = (BL.AIUnit) unitPosition;
        if (aiUnit1.skillEffects.CanUseSkill(skill.skill, skill.level, (BL.ISkillEffectListUnit) aiUnit1, this.battleEnv, (BL.ISkillEffectListUnit) aiUnit, skill.nowUseCount) == 0)
          unitPositionList1.Add(unitPosition);
      }
      ...
      if (battleskillEffect != null && unitPositionList1.Count < battleskillEffect.GetInt(BattleskillEffectLogicArgumentEnum.min_unit_count))
        unitPositionList1.Clear();
      return unitPositionList1;
    }
