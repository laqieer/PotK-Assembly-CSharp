...
namespace GameCore.LispCore
{
  public class SExpReader
  {
    ...
    private Decimal? numberObject(Decimal d) => this.numberDic != null ? this.numberDic.numberObject(d) : new Decimal?(d);
    ...
    private int readObj(int p)
    {
      ...
      string str = this.readCharAt(this.str, p);
      if (str == "(")
    ...
    private int readList(int p)
    {
      ...
      {
        p = this.readObj(p);
        objectList.Add(this.data);
        p = this.removeWhiteSpace(p);
        if (this.readCharAt(this.str, p) == ".")
        {
          Cons lispList = SExp.toLispList<object>((IEnumerable<object>) objectList) as Cons;
          p = this.readObj(p + 1);
      ...
      this.data = objectList.Count != 0 ? SExp.toLispList<object>((IEnumerable<object>) objectList) : (object) null;
      return p + 1;
    }
    ...
    private bool whiteSpacep(string c) => c == " " || c == "\b" || (c == "\f" || c == "\r") || (c == "\n" || c == "\t");
