...
namespace GameCore
{
  public class ItemInfo
  {
    ...
    public ItemInfo(PlayerItem item, bool bExpireDate = false, Func<bool> checkEquipped = null) => this.Set(item, bExpireDate, checkEquipped);
    ...
    public GearGear gear => this.itemType == ItemInfo.ItemType.Supply ? (GearGear) null : MasterData.GearGear[this.masterID];

    public SupplySupply supply => this.itemType != ItemInfo.ItemType.Supply ? (SupplySupply) null : MasterData.SupplySupply[this.masterID];

    public bool isWeapon => this.itemType == ItemInfo.ItemType.Gear && !MasterData.GearGear[this.masterID].isReisou();

    public bool isReisou => this.itemType == ItemInfo.ItemType.Gear && MasterData.GearGear[this.masterID].isReisou();
    ...
    public int GetSortLevel()
    {
      ...
      if (this.gear.isMythologyReisou() && (!this.gear.drilling_exp_mythology_id.HasValue || this.gear.drilling_exp_mythology_id.Value == 0))
      {
        ...
        num = mythologyGearStatus.holy_gear_level + mythologyGearStatus.chaos_gear_level;
      }
      else
        num = this.gearLevel + this.gearLevelUnLimit;
      return num;
    }
    ...
    public int GetSortLevelLimit()
    {
      ...
      {
        PlayerMythologyGearStatus mythologyGearStatus = this.playerItem.GetPlayerMythologyGearStatus();
        num = mythologyGearStatus.holy_gear_level_limit + mythologyGearStatus.chaos_gear_level_limit;
      }
      else
        num = this.gearLevelLimit;
      return num;
    }
    ...
    public bool isComposeManaSeed => this.gear != null && this.gear.isComposeManaSeed() && this.gearAccessoryRemainingAmount < this.gear.manaSeedRecoveryLimit;
    ...
    public string Description() => this.itemType == ItemInfo.ItemType.Supply ? this.supply.description : string.Empty;

    public long SellPrice() => this.itemType == ItemInfo.ItemType.Supply ? (long) this.supply.sell_price : (long) this.gear.sell_price;
    ...
    public int intelligence_incremental => !this.isWeaponOrReisou ? 0 : this.gear.intelligence_incremental;
    ...
    public int dexterity_incremental => !this.isWeaponOrReisou ? 0 : this.gear.dexterity_incremental;

    public int lucky_incremental => !this.isWeaponOrReisou ? 0 : this.gear.lucky_incremental;

    public PlayerItem playerItem => ((IEnumerable<PlayerItem>) SMManager.Get<PlayerItem[]>()).FirstOrDefault<PlayerItem>((Func<PlayerItem, bool>) (x => x.id == this.itemID));

    public Future<UnityEngine.Sprite> LoadSpriteThumbnail() => this.itemType == ItemInfo.ItemType.Supply ? this.supply.LoadSpriteThumbnail() : this.gear.LoadSpriteThumbnail();
