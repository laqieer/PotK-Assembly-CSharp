...
namespace GameCore
{
  public class ColosseumEnvironmentInitializer
  {
    ...
    private static BL.MagicBullet createMagicBullet(PlayerUnitSkills playerSkill) => new BL.MagicBullet()
    {
      skillId = playerSkill.skill.ID
    };

    private static BL.MagicBullet createMagicBullet(GearGearSkill gearSkill) => new BL.MagicBullet()
    {
      skillId = gearSkill.skill.ID
    };

    private static BL.MagicBullet createMagicBullet(GearReisouSkill reisouSkill) => new BL.MagicBullet()
    {
      skillId = reisouSkill.skill.ID
    };

    private static BL.MagicBullet createMagicBullet(PlayerAwakeSkill awakeSkill) => new BL.MagicBullet()
    {
      skillId = awakeSkill.masterData.ID
    };

    private static BL.MagicBullet createMagicBullet(IAttackMethod attackMethod) => new BL.MagicBullet()
    {
    ...
    public static BL.Unit createUnitByPlayerUnit(PlayerUnit pu, int index, bool friend)
    {
      ...
      unit.weapon = weapon;
      unit.optionWeapons = ((IEnumerable<IAttackMethod>) pu.battleOptionAttacks).Where<IAttackMethod>((Func<IAttackMethod, bool>) (x => x.kind.Enum != GearKindEnum.magic)).Select<IAttackMethod, BL.Weapon>((Func<IAttackMethod, BL.Weapon>) (y => new BL.Weapon(y))).ToArray<BL.Weapon>();
      unit.gearLeftHand = pu.isLeftHandWeapon;
      unit.gearDualWield = pu.isDualWieldWeapon;
      unit.index = index;
      unit.friend = friend;
      unit.ougi = skill;
      unit.skills = list2.Concat<BL.Skill>(second2).Concat<BL.Skill>(second5).Concat<BL.Skill>(second8).ToArray<BL.Skill>();
      unit.magicBullets = first1.Concat<BL.MagicBullet>(second4).Concat<BL.MagicBullet>(second7).Concat<BL.MagicBullet>(second9).Concat<BL.MagicBullet>(second10).ToArray<BL.MagicBullet>();
      foreach (BL.MagicBullet magicBullet in unit.magicBullets)
        magicBullet.setPrefabName(unit.job);
      unit.duelSkills = first2.Concat<BL.Skill>(second1).Concat<BL.Skill>(second3).Concat<BL.Skill>(second6).OrderByDescending<BL.Skill, int>((Func<BL.Skill, int>) (x => x.skill.weight)).ThenBy<BL.Skill, int>((Func<BL.Skill, int>) (x => x.id)).ThenByDescending<BL.Skill, int>((Func<BL.Skill, int>) (x => x.level)).ToArray<BL.Skill>();
      ...
      ColosseumEnvironment env_)
    {
      ...
      foreach (KeyValuePair<int, BL.Unit> keyValuePair in colosseumEnvironment.opponentUnitDict)
      {
        ...
        if (!(unit == (BL.Unit) null))
        {
          ...
          PlayerItem playerItem3 = colosseumEnvironment.opponentGearDict3[keyValuePair.Key];
          if (playerItem3 != (PlayerItem) null)
          {
            foreach (GearGearSkill skill in playerItem3.skills)
            {
          ...
          PlayerItem playerItem4 = colosseumEnvironment.opponentReisouDict[keyValuePair.Key];
          if (playerItem1 != (PlayerItem) null && playerItem4 != (PlayerItem) null)
          {
            foreach (GearReisouSkill reisouSkill in playerItem4.getReisouSkills(playerItem1.entity_id))
            {
              if (reisouSkill.skill.skill_type != BattleskillSkillType.passive || !reisouSkill.skill.range_effect_passive_skill)
          ...
          PlayerItem playerItem5 = colosseumEnvironment.opponentReisouDict2[keyValuePair.Key];
          if (playerItem2 != (PlayerItem) null && playerItem5 != (PlayerItem) null)
          {
            foreach (GearReisouSkill reisouSkill in playerItem5.getReisouSkills(playerItem2.entity_id))
            {
              if (reisouSkill.skill.skill_type != BattleskillSkillType.passive || !reisouSkill.skill.range_effect_passive_skill)
          ...
          PlayerItem playerItem6 = colosseumEnvironment.opponentReisouDict3[keyValuePair.Key];
          if (playerItem3 != (PlayerItem) null && playerItem6 != (PlayerItem) null)
          {
            foreach (GearReisouSkill reisouSkill in playerItem6.getReisouSkills(playerItem3.entity_id))
            {
              if (reisouSkill.skill.skill_type != BattleskillSkillType.passive || !reisouSkill.skill.range_effect_passive_skill)
