...
namespace GameCore
{
  public static class ColosseumBattleCalc
  {
    ...
    {
      bool flag = false;
      AttackStatus[] attackStatusArray1 = new AttackStatus[0];
      AttackStatus[] attackStatusArray2 = new AttackStatus[0];
      AttackStatus[] attackStatusArray3 = new AttackStatus[attack.optionWeapons.Length];
      if (((IEnumerable<BL.MagicBullet>) attack.magicBullets).Any<BL.MagicBullet>())
      {
        flag = true;
        attackStatusArray1 = ((IEnumerable<BL.MagicBullet>) attack.magicBullets).Where<BL.MagicBullet>((Func<BL.MagicBullet, bool>) (x => x != null && x.isAttack && attackHp > x.cost)).Select<BL.MagicBullet, AttackStatus>((Func<BL.MagicBullet, AttackStatus>) (x => ColosseumBattleCalc.getAttackStatus(x, (BL.Weapon) null, attack, attackNeighbors, attackDeckUnits, attackEquippedGear, attackEquippedGear2, attackEquippedGear3, attackEquippedReisou, attackEquippedReisou2, attackEquippedReisou3, defense, defenseNeighbors, defenseDeckUnits, defenseEquippedGear, defenseEquippedGear2, defenseEquippedGear3, defenseEquippedReisou, defenseEquippedReisou2, defenseEquippedReisou3, isAttack, attackHp, isSample, defenseHp, battleCount, colosseumTurn))).ToArray<AttackStatus>();
      }
      if (!flag || attack.unit.magic_warrior_flag)
        attackStatusArray2 = new AttackStatus[1]
        {
      ...
      for (int index = 0; index < attack.optionWeapons.Length; ++index)
        attackStatusArray3[index] = ColosseumBattleCalc.getAttackStatus((BL.MagicBullet) null, attack.optionWeapons[index], attack, attackNeighbors, attackDeckUnits, attackEquippedGear, attackEquippedGear2, attackEquippedGear3, attackEquippedReisou, attackEquippedReisou2, attackEquippedReisou3, defense, defenseNeighbors, defenseDeckUnits, defenseEquippedGear, defenseEquippedGear2, defenseEquippedGear3, defenseEquippedReisou, defenseEquippedReisou2, defenseEquippedReisou3, isAttack, attackHp, isSample, defenseHp, battleCount, colosseumTurn);
      return ((IEnumerable<AttackStatus>) attackStatusArray2).Concat<AttackStatus>(((IEnumerable<AttackStatus>) attackStatusArray3).Concat<AttackStatus>((IEnumerable<AttackStatus>) attackStatusArray1)).ToArray<AttackStatus>();
    }
      ...
      int battleCount)
    {
      ...
      for (int index = 0; index < 4; ++index)
      {
        ...
        BL.DuelTurn[] turns = ColosseumBattleCalc.calcTurns(unit1, num1, attackStatus1, unit2, num2, attackStatus2, flag, invokedAttackerPrayer, invokedDefenderPrayer, colosseumTurn);
        ...
        if (turns.Length != 0)
        {
          if (!flag)
          {
            turns[0].attackerStatus = attackStatus1;
            turns[0].defenderStatus = attackStatus2;
          }
          else
          {
            turns[0].defenderStatus = attackStatus1;
            turns[0].attackerStatus = attackStatus2;
          }
        }
        int num3 = 0;
        int num4 = 0;
        foreach (BL.DuelTurn duelTurn in turns)
        {
        ...
        duelTurnList.AddRange((IEnumerable<BL.DuelTurn>) turns);
        ...
        BattleFuncs.consumeSkillEffects(turns, (BL.ISkillEffectListUnit) unit1, (BL.ISkillEffectListUnit) unit2, true);
    ...
    private static void CalcCommandAndReleaseSkill(BL.Unit unit, BL.Unit target, int attackOrder)
    {
      ...
      else
      {
        ...
        foreach (PlayerAwakeSkill playerAwakeSkill in ((IEnumerable<PlayerAwakeSkill>) playerAwakeSkillArray).Where<PlayerAwakeSkill>((Func<PlayerAwakeSkill, bool>) (v => checkAttackOrderSkill(v.masterData, v.level))))
        ...
        {
          float num = Mathf.Clamp(Mathf.Floor((float) (25.0 - (double) Mathf.Pow((float) unit.ougi.useTurn, 2f) * 0.0900000035762787)), 3f, 100f);
          if (ColosseumBattleCalc.random.Get().RangeInt(0, 100) < (int) num)
          {
    ...
    public static ColosseumResult calcColosseum(
      ColosseumEnvironment env,
