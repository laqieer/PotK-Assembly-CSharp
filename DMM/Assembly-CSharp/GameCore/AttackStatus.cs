...
namespace GameCore
{
  ...
  public class AttackStatus
  {
    ...
    public float originalAttack => !this.isMagic ? this.duelParameter.CalcPhysicalAttack(this.attackRate) : this.duelParameter.CalcMagicAttack(this.attackRate);

    public float originalAttackExcludeBaseDamage => !this.isMagic ? this.duelParameter.CalcPhysicalAttack(this.attackRate, true) : this.duelParameter.CalcMagicAttack(this.attackRate, true);
    ...
    public int expectationDamage() => (int) ((double) (this.attack * this.attackCount) * (double) this.hit);
    ...
    public CommonElement? GetOverwriteElement() => this.overwriteElement == (CommonElement) 0 ? new CommonElement?() : new CommonElement?(this.overwriteElement);
    ...
    public float realHit => NC.Clampf(0.0f, 1f, this.hit + Mathf.Sin((float) ((1.0 - (double) this.hit) * 2.0 * 3.14159274101257)) / 10f);

    public bool calcHit(float value) => (double) this.realHit >= (double) value;

    public string ShowAttackStatus(BL.ISkillEffectListUnit attack, BL.ISkillEffectListUnit defense) => string.Format("magic({0}) attack({1}) heal({2}) count({3}) hit({4}) critical({5})", this.magicBullet != null ? (object) (this.magicBullet.name + " : " + (object) this.magicBullet.cost) : (object) "-", (object) attack, (object) this.healAttack(attack, defense), (object) this.attackCount, (object) this.dexerityDisplay(), (object) this.criticalDisplay());
  }
}
