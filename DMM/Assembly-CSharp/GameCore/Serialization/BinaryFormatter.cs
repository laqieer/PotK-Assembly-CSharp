...
namespace GameCore.Serialization
{
  public class BinaryFormatter : ICrossFormatter
  {
    ...
    private TreeObject LoadTreeObject(
      BinaryReader reader,
      int minTreeId,
      int minObjectId)
    {
    ...
    public void Save(int rootId, TypeObject[] objects, TreeObject[] trees, Stream stream)
    {
      if (this.compressing)
      {
        using (ZlibUtilStream zlibUtilStream = ZlibUtilStream.Compress(stream, ZlibFormat.Deflate, ZlibCompressionLevel.DefaultCompression))
      ...
      out TreeObject[] trees)
    {
      if (this.compressing)
      {
        using (ZlibUtilStream zlibUtilStream = ZlibUtilStream.Decompress(stream, ZlibFormat.Deflate))
    ...
    public static CrossSerializer MakeSerializer() => new CrossSerializer((ICrossFormatter) new BinaryFormatter());

    public static CrossSerializer MakeSerializer(bool compressing) => new CrossSerializer((ICrossFormatter) new BinaryFormatter(compressing));
  }
}
