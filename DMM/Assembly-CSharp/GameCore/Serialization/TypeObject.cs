...
namespace GameCore.Serialization
{
  public class TypeObject
  {
    ...
    {
      T[] objArray = TypeObject.OneArray<T>.Value;
      objArray[0] = value;
      int count = Buffer.ByteLength((Array) objArray);
      this.buf = new byte[count];
      Buffer.BlockCopy((Array) objArray, 0, (Array) this.buf, 0, count);
    }

    private T GetT<T>() where T : struct
    {
      T[] objArray = TypeObject.OneArray<T>.Value;
      Buffer.BlockCopy((Array) this.buf, 0, (Array) objArray, 0, this.buf.Length);
      return objArray[0];
    }
    ...
    {
      T[] objArray = new T[this.length.Value];
      Buffer.BlockCopy((Array) this.buf, 0, (Array) objArray, 0, this.buf.Length);
      return objArray;
    }
    ...
    private static byte[] ToBytes(string xs) => Enumerable.Range(0, xs.Length / 2).Select<int, byte>((Func<int, byte>) (x => Convert.ToByte(xs.Substring(x * 2, 2), 16))).ToArray<byte>();
