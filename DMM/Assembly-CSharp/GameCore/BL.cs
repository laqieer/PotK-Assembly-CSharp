...
namespace GameCore
{
  ...
  public class BL
  {
      ...
      bool includeJumping = false)
    {
      foreach (BL.AIUnit aiUnit in this.aiUnitPositions.value)
      {
        if (!aiUnit.isDead && (includeJumping || !aiUnit.IsJumping))
        {
          if (isOriginal)
          {
            if (aiUnit.originalRow == row && aiUnit.originalColumn == column)
              return (BL.UnitPosition) aiUnit;
          }
          else if (aiUnit.row == row && aiUnit.column == column)
            return (BL.UnitPosition) aiUnit;
        }
      }
    ...
    public BL.UnitPosition getFieldUnitAI(BL.Panel panel, bool isOriginal = false, bool includeJumping = false) => this.getFieldUnitAI(panel.row, panel.column, isOriginal, includeJumping);
    ...
    public int getAroundEnemyUnitsCount(BL.AIUnit aiUnit, int range) => BattleFuncs.getForceUnitsWithinRange(aiUnit.row, aiUnit.column, range, this.getTargetForce(aiUnit.unitPosition.unit, aiUnit.IsCharm), true).Count;

    public static int fieldDistance(BL.AIUnit p1, BL.AIUnit p2) => BL.fieldDistance(p1.row, p1.column, p2.row, p2.column);

    public static int fieldDistance(BL.AIUnit p1, BL.Panel p2) => BL.fieldDistance(p1.row, p1.column, p2.row, p2.column);

    public static int fieldDistance(BL.Panel p1, BL.AIUnit p2) => BL.fieldDistance(p1.row, p1.column, p2.row, p2.column);

    public BL.Unit getBossUnit()
    {
      if (this.condition.type != BL.ConditionType.bossdown)
        return (BL.Unit) null;
      foreach (BL.Unit unit in this.enemyUnits.value)
      {
        if (unit.playerUnit.id == this.condition.bossId)
          return unit;
      }
      return (BL.Unit) null;
    }
      ...
      BattleVictoryAreaCondition[] area_condition)
    {
      ...
      if (area_condition != null && area_condition.Length != 0)
      {
        foreach (BattleVictoryAreaCondition victoryAreaCondition in area_condition)
        {
          ...
          if (fieldUnit != null)
            unitPositionList.Add(fieldUnit);
        }
      }
      return unitPositionList;
    }

    public BL.UnitPosition currentUnitPosition => this.unitCurrent.unit == (BL.Unit) null ? new BL.UnitPosition() : this.getUnitPosition(this.unitCurrent.unit);
    ...
    public void setCurrentField(int row, int column) => this.fieldCurrent.value = this.fieldMatrix[row, column];
    ...
    public BL.UnitPosition getFieldUnit(int row, int column, bool original = false, bool includeJumping = false)
    {
      foreach (BL.UnitPosition unitPosition in this.unitPositions.value)
      {
        ...
        if (unitPosition.unit.isEnable && !unitPosition.unit.isDead && (num1 == row && num2 == column) && (includeJumping || !unitPosition.unit.IsJumping))
          return unitPosition;
      }
      return (BL.UnitPosition) null;
    }

    public BL.UnitPosition getFieldUnit(BL.Panel panel, bool original = false, bool includeJumping = false) => this.getFieldUnit(panel.row, panel.column, original, includeJumping);
    ...
    public BL.UnitPosition[] getFieldUnits(BL.Panel panel, bool original = false, bool isDead = false) => this.getFieldUnits(panel.row, panel.column, original, isDead);

    public BL.UnitPosition getUnitPositionById(int id)
    {
      foreach (BL.UnitPosition unitPosition in this.unitPositions.value)
      {
        if (unitPosition.id == id)
          return unitPosition;
      }
      return (BL.UnitPosition) null;
    }
    ...
    public BL.Panel getFieldPanel(int row, int column) => row < 0 || column < 0 || (row >= this.getFieldHeight() || column >= this.getFieldWidth()) ? (BL.Panel) null : this.fieldMatrix[row, column];

    public IEnumerable<BL.Panel> getAllPanel() => ((IEnumerable) this.fieldMatrix).Cast<BL.Panel>();

    public BL.Panel getFieldPanel(BL.UnitPosition up, bool original = false) => original ? this.getFieldPanel(up.originalRow, up.originalColumn) : this.getFieldPanel(up.row, up.column);
    ...
    public BL.UnitPosition getUnitPosition(BL.Unit unit) => this.unitPositions.value.Find((Predicate<BL.UnitPosition>) (up => up.unit == unit));
    ...
    public void lookDirection(BL.UnitPosition s, BL.Unit d) => this.lookDirection(s, this.getUnitPosition(d));

    public void lookDirection(BL.Unit s, BL.Unit d) => this.lookDirection(this.getUnitPosition(s), this.getUnitPosition(d));

    public static int fieldDistance(BL.Panel p1, BL.Panel p2) => BL.fieldDistance(p1.row, p1.column, p2.row, p2.column);

    public static int fieldDistance(BL.UnitPosition p1, BL.UnitPosition p2) => BL.fieldDistance(p1.row, p1.column, p2.row, p2.column);

    public static int fieldDistance(BL.UnitPosition p1, BL.Panel p2) => BL.fieldDistance(p1.row, p1.column, p2.row, p2.column);

    public static int fieldDistance(BL.Panel p1, BL.UnitPosition p2) => BL.fieldDistance(p1.row, p1.column, p2.row, p2.column);

    public static int fieldDistance(int r1, int c1, int r2, int c2) => Mathf.Abs(r1 - r2) + Mathf.Abs(c1 - c2);
    ...
    public int[] getReinforcementIDsToPanel(BL.UnitPosition up) => this.getFieldPanel(up.originalRow, up.originalColumn)?.getReinforcementIDsToPanel(this.getForceID(up.unit));
    ...
    public void initializeIntimatePvp()
    {
      ...
      foreach (BL.Unit unit1 in array2)
      {
        BL.Unit unit = unit1;
    ...
    private int makeUnitId(BL.ISkillEffectListUnit unit) => unit is BL.AIUnit ? (unit as BL.AIUnit).id : this.getUnitPosition(unit.originalUnit).id;
    ...
    public HashSet<BL.ISkillEffectListUnit> getCharismaTargetUnits(
      BL.ISkillEffectListUnit unit)
    {
      HashSet<BL.ISkillEffectListUnit> skillEffectListUnitSet = new HashSet<BL.ISkillEffectListUnit>();
        ...
        if (enabledCharismaEffects.Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.HasKey(BattleskillEffectLogicArgumentEnum.effect_target) ? x.effect.GetInt(BattleskillEffectLogicArgumentEnum.effect_target) == effectTarget : effectTarget == 1)))
        {
          if (unit is BL.AIUnit)
          {
            foreach (BL.AIUnit aiUnit in this.aiUnitPositions.value.Where<BL.AIUnit>((Func<BL.AIUnit, bool>) (x => ((IEnumerable<BL.ForceID>) forceId[effectTarget]).Contains<BL.ForceID>(this.getForceID(x.originalUnit)))))
              skillEffectListUnitSet.Add((BL.ISkillEffectListUnit) aiUnit);
          }
          else
          {
            foreach (BL.Unit unit1 in ((IEnumerable<BL.ForceID>) forceId[effectTarget]).SelectMany<BL.ForceID, BL.Unit>(closure_0 ?? (closure_0 = (Func<BL.ForceID, IEnumerable<BL.Unit>>) (x => (IEnumerable<BL.Unit>) this.getForceUnitList(x)))))
              skillEffectListUnitSet.Add((BL.ISkillEffectListUnit) unit1);
          }
        }
      }
      return skillEffectListUnitSet;
    }

    public HashSet<BL.ISkillEffectListUnit> getOnemanChargeUnits(
      BL.ISkillEffectListUnit unit)
    {
      HashSet<BL.ISkillEffectListUnit> skillEffectListUnitSet = new HashSet<BL.ISkillEffectListUnit>();
      BL.ForceID[][] forceId = new BL.ForceID[2][];
      ...
      for (int searchTarget = 0; searchTarget <= 1; searchTarget++)
      {
        foreach (BL.ISkillEffectListUnit unit1 in !(unit is BL.AIUnit) ? (IEnumerable<BL.ISkillEffectListUnit>) ((IEnumerable<BL.ForceID>) forceId[searchTarget]).SelectMany<BL.ForceID, BL.Unit>(closure_0 ?? (closure_0 = (Func<BL.ForceID, IEnumerable<BL.Unit>>) (x => (IEnumerable<BL.Unit>) this.getForceUnitList(x)))) : (IEnumerable<BL.ISkillEffectListUnit>) this.aiUnitPositions.value.Where<BL.AIUnit>((Func<BL.AIUnit, bool>) (x => ((IEnumerable<BL.ForceID>) forceId[searchTarget]).Contains<BL.ForceID>(this.getForceID(x.originalUnit)))))
        {
          ...
          {
            skillEffectListUnitSet.Add(unit1);
          }
          else
          {
    ...
    public void useMagicBulletWith(
      ...
      System.Action<Dictionary<BL.Unit, int>, List<BL.Unit>, List<BL.Unit>> f,
      BL env)
    {
      ...
      List<BL.Unit> unitList1 = tuple.Item1;
      List<BL.Unit> unitList2 = tuple.Item3;
      ...
      List<BL.Unit> list = targets.Concat<BL.Unit>((IEnumerable<BL.Unit>) unitList1).Distinct<BL.Unit>().ToList<BL.Unit>();
      int healHpTotal = 0;
      ...
      int hp = unit.hp;
      if (hp >= 1 && BattleFuncs.applyServantsJoy((BL.ISkillEffectListUnit) unit, healHpTotal) && !unitList2.Contains(unit))
        unitList2.Add(unit);
      if (hp >= 1 && unit.hp <= 0)
      {
        NGBattleManager instance = Singleton<NGBattleManager>.GetInstance();
      ...
      f(dictionary, list, targets.Concat<BL.Unit>((IEnumerable<BL.Unit>) unitList2).Distinct<BL.Unit>().ToList<BL.Unit>());
      this.getUnitPosition(unit).actionActionUnit(this);
    }

    public virtual BL.SkillResultUnit createSkillResultUnit(BL.UnitPosition up) => new BL.SkillResultUnit(up, this);
      ...
      foreach (BL.SkillEffect skillEffect in up.unit.skillEffects.Where(BattleskillEffectLogicEnum.self_rebirth))
      {
        if (up.unit.isDead)
        {
          ...
          if (turn - num1 >= skillEffect.effect.GetInt(BattleskillEffectLogicArgumentEnum.turn))
          {
            ...
            up.unit.rebirth(this, false, (uint) skillEffect.effect.GetInt(BattleskillEffectLogicArgumentEnum.is_reset_completed) > 0U);
      ...
      XorShift random = null)
    {
      ...
      {
        Decimal num1 = (Decimal) investEffect.effect.GetFloat(BattleskillEffectLogicArgumentEnum.rebirth_percentage);
        if (!(num1 != 0M) || !(num1 * 1000000M <= (Decimal) random.NextFixed(1000000U)))
        {
          BL.ExecuteSkillEffectResult skillEffectResult = new BL.ExecuteSkillEffectResult()
          {
          ...
          forceResetCompleted = (uint) investEffect.effect.GetInt(BattleskillEffectLogicArgumentEnum.is_reset_completed) > 0U;
          ...
          if (investEffect.useRemain.HasValue)
          {
            ...
            int num2 = 1;
            if (useRemain1.GetValueOrDefault() >= num2 & useRemain1.HasValue)
            {
    ...
    public List<BL.ExecuteSkillEffectResult> executeSkillEffects(BL.UnitPosition up)
    {
      ...
      System.Action<BL.SkillEffect, System.Action<BL.UnitPosition>> action = (System.Action<BL.SkillEffect, System.Action<BL.UnitPosition>>) ((effect, act) =>
      {
      ...
      foreach (BL.SkillEffect skillEffect in up.unit.skillEffects.Where(BattleskillEffectLogicEnum.ratio_recovery))
      {
        ...
        action(effect, (System.Action<BL.UnitPosition>) (target =>
        {
      ...
      {
        BL.SkillEffect effect = skillParam.effect;
        BL.UnitPosition up1 = ((Tuple<BL.UnitPosition, int>) skillParam.param).Item1;
        ...
        if (up1.unit.hp > 0)
        {
          ...
          int index = skillEffectResult.targets.IndexOf(up1);
          BL.Panel fieldPanel = this.getFieldPanel(up1);
          if (index == -1)
          {
            skillEffectResult.target_prev_hps.Add(up1.unit.hp);
          ...
          {
            up1.unit.hp += BattleFuncs.getHealValue((BL.ISkillEffectListUnit) up1.unit, fieldPanel, healValue, effect.baseSkill.skill_type);
            skillEffectResult.target_hps[index] = up1.unit.hp;
          }
        }
      }
    ...
    private IEnumerable<BL.SkillEffect> getFacilitySkillPhaseEffects(
      ...
      int phase)
    {
      foreach (BL.SkillEffect skillEffect in ((IEnumerable<BL.FacilitySkillLogicEffect>) logicEffects).SelectMany<BL.FacilitySkillLogicEffect, BL.SkillEffect>((Func<BL.FacilitySkillLogicEffect, IEnumerable<BL.SkillEffect>>) (le => unit.skillEffects.Where(le.logicEnum()))))
      {
        if ((skillEffect.effect.HasKey(BattleskillEffectLogicArgumentEnum.phase) ? skillEffect.effect.GetInt(BattleskillEffectLogicArgumentEnum.phase) : 0) == phase)
          yield return skillEffect;
      }
    }

    public IEnumerable<BL.ExecuteSkillEffectResult> executePhaseSkillEffects(
      BL.UnitPosition up)
    {
                ...
                else if (num4 == 2)
                {
                  foreach (BL.ISkillEffectListUnit forceUnit in BattleFuncs.getForceUnits(this.getForceID(headerEffect.investUnit), isAI, true))
                  {
                    if (forceUnit.hp > 0)
                    {
                      ...
                      BL.Panel panel2 = BattleFuncs.getPanel(unitPosition.row, unitPosition.column);
                      dictionary[forceUnit] += BattleFuncs.getHealValue(forceUnit, panel2, healValue, headerEffect.baseSkill.skill_type);
                    }
                  }
                }
              }
        ...
        foreach (BL.ISkillEffectListUnit key in dictionary.Keys)
        {
          if (key.hp > 0)
          {
            ...
            if (skillEffectResult2 != null)
            {
              ...
              skillEffectResult2.second_target_prev_hps.Add(hp2);
      ...
      List<BL.Unit> destructFacilities)
    {
      ...
      bool flag1 = false;
      bool flag2 = false;
      BL.ISkillEffectListUnit skillEffectListUnit1 = targets.Count > 0 ? targets[0] : (BL.ISkillEffectListUnit) null;
      ...
      foreach (BL.ISkillEffectListUnit skillEffectListUnit2 in skillEffectListUnits)
      {
        BL.ISkillEffectListUnit target = skillEffectListUnit2;
        ...
        foreach (BattleskillEffect battleskillEffect in source1)
        {
          ...
          {
            if (BattleFuncs.canUseImmediateSkillEffect(effect, target, out bool _, useUnit, skillEffectListUnit1, callIsPlayer, battleParameter) && (panels.Count != 1 || BattleFuncs.canUseImmediatePanelSkillEffect(effect, panels[0], useUnit)))
            {
...
label_20:
              ...
              System.Action<int> action1 = (System.Action<int>) (healValue =>
              {
                int num1 = targetPrevHp + healValue;
                if (num1 <= target.originalUnit.parameter.Hp)
                  return;
                Decimal num2 = effect.HasKey(BattleskillEffectLogicArgumentEnum.over_heal_shield_percentage) ? (Decimal) effect.GetFloat(BattleskillEffectLogicArgumentEnum.over_heal_shield_percentage) : 0M;
                if (num2 <= 0M)
                  return;
                ...
                int num3 = (int) Math.Ceiling((Decimal) (num1 - target.originalUnit.parameter.Hp) * num2);
                int num4 = effect1.GetInt(BattleskillEffectLogicArgumentEnum.value);
                if (num3 > num4)
                  num3 = num4;
                int num5 = num4 - num3;
                ...
                float[] numArray;
                if (num5 != 0)
                  numArray = new float[1]{ (float) num5 };
                else
                  numArray = (float[]) null;
                skillEffect.work = numArray;
              });
              System.Action<BL.ISkillEffectListUnit> action2 = closure_1 ?? (closure_1 = (System.Action<BL.ISkillEffectListUnit>) (u =>
              {
                if (isAI)
                  return;
                NGBattleManager instance = Singleton<NGBattleManager>.GetInstance();
              ...
              switch (effect.EffectLogic.Enum)
              {
                ...
                case BattleskillEffectLogicEnum.ratio_heal:
                  if (target.hp >= 1)
                  {
                    int num1 = BattleFuncs.getHealValue(target, BattleFuncs.getPanel(unitPosition1.row, unitPosition1.column), (int) ((Decimal) target.originalUnit.parameter.Hp * (Decimal) effect.GetFloat(BattleskillEffectLogicArgumentEnum.percentage)), skill.skill_type);
                    int num2 = effect.HasKey(BattleskillEffectLogicArgumentEnum.limit) ? effect.GetInt(BattleskillEffectLogicArgumentEnum.limit) : 0;
                    if (num2 != 0)
                    {
                      if (num1 > 0 && num1 > num2)
                        num1 = num2;
                ...
                case BattleskillEffectLogicEnum.invest_skilleffect_im:
                  int num6 = effect.GetInt(BattleskillEffectLogicArgumentEnum.skill_id);
                  if (num6 != 0 && MasterData.BattleskillSkill.ContainsKey(num6) && MasterData.BattleskillSkill[num6].skill_type == BattleskillSkillType.ailment)
                  {
                    ...
                    BL.SkillEffect[] array = BattleFuncs.getAilmentTriggerSkillEffects(num6, target, useUnit, BattleFuncs.getPanel(unitPosition1.row, unitPosition1.column)).ToArray<BL.SkillEffect>();
                    BL.SkillEffect perfectAilmentResist = BattleFuncs.getPerfectAilmentResist(BattleFuncs.getAilmentResistEffects(num6, target, useUnit));
                    if (perfectAilmentResist != null)
                    {
                      if (perfectAilmentResist.useRemain.HasValue)
                      {
                        ...
                        if (!isAI)
                        {
                          ...
                          int num1 = 0;
                          if (useRemain2.GetValueOrDefault() == num1 & useRemain2.HasValue)
                    ...
                    else
                    {
                      ...
                      if (BattleFuncs.isAilmentInvest(lottery, num6, target, useUnit, random, new int?(), out useResistEffects, (TurnHp) null, new int?(), new int?(), false))
                      {
                        BL.Skill[] skillArray = BattleFuncs.ailmentInvest(num6, target);
                        if (skillArray != null)
                        {
                          foreach (BL.Skill skill1 in skillArray)
                          {
                            foreach (BattleskillEffect effect1 in skill1.skill.Effects)
                              target.skillEffects.Add(BL.SkillEffect.FromMasterData(effect1, skill1.skill, 1, investUnit: (useUnit != null ? useUnit.originalUnit : (BL.Unit) null), investSkillId: investSkillId, investTurn: this.phaseState.absoluteTurnCount));
                          }
                          if (!isAI)
                            target.originalUnit.commit();
                        }
                    ...
                    if (array != null)
                    {
                      foreach (BL.SkillEffect skillEffect in array)
                      {
                        ...
                        {
                          BattleskillSkill skill1 = MasterData.BattleskillSkill[key];
                          foreach (BattleskillEffect effect1 in skill1.Effects)
                            target.skillEffects.Add(BL.SkillEffect.FromMasterData(effect1, skill1, 1, investUnit: target.originalUnit, investSkillId: skillEffect.baseSkillId, investTurn: this.phaseState.absoluteTurnCount), checkEnableUnit: target);
                          if (!isAI)
                            target.originalUnit.commit();
                        }
                      }
                  ...
                  }, this.getForceID(unitPosition1.unit), (isAI ? 1 : 0) != 0).GetEnumerator())
                  {
                    while (enumerator.MoveNext())
                    {
                      ...
                      if (effect.GetInt(BattleskillEffectLogicArgumentEnum.element) == 0 || (CommonElement) effect.GetInt(BattleskillEffectLogicArgumentEnum.element) == iskillEffectListUnit.originalUnit.playerUnit.GetElement())
                      {
                        ...
                        {
                          int num1 = effect.GetInt(BattleskillEffectLogicArgumentEnum.value);
                          if (num1 > 0)
                            iskillEffectListUnit.ougi.useTurn -= num1;
                          else if (num1 < 0)
                          {
                ...
                case BattleskillEffectLogicEnum.recovery_command_skill_use:
                  ...
                  }, this.getForceID(unitPosition1.unit), (isAI ? 1 : 0) != 0).GetEnumerator())
                  {
                    while (enumerator.MoveNext())
                    {
                      ...
                      {
                        int num1 = effect.GetInt(BattleskillEffectLogicArgumentEnum.value);
                        foreach (BL.Skill skill1 in iskillEffectListUnit.skills)
                        {
                          BL.Skill s = skill1;
                          if (s.remain.HasValue && (num1 <= 0 || !((IEnumerable<BattleskillEffect>) s.skill.Effects).Any<BattleskillEffect>((Func<BattleskillEffect, bool>) (x => x.EffectLogic.Enum == BattleskillEffectLogicEnum.cant_recovery_command_skill_use && x.checkLevel(s.level)))))
                          {
                            BL.Skill skill2 = s;
                            int? remain = skill2.remain;
                            int num2 = num1;
                            skill2.remain = remain.HasValue ? new int?(remain.GetValueOrDefault() + num2) : new int?();
                            int num3 = s.useCount + (s.level - 1);
                            if (s.maxUseCount != 0 && num3 > s.maxUseCount)
                              num3 = s.maxUseCount;
                            remain = s.remain;
                            int num4 = num3;
                            if (remain.GetValueOrDefault() > num4 & remain.HasValue)
                            {
                              s.remain = new int?(num3);
                            }
                            else
                            {
                              remain = s.remain;
                              int num5 = 0;
                              if (remain.GetValueOrDefault() < num5 & remain.HasValue)
                                s.remain = new int?(0);
                            }
                          }
                        }
                ...
                case BattleskillEffectLogicEnum.fix_rebirth:
                  ...
                  {
                    target.originalUnit.rebirth(env, false, (uint) effect.GetInt(BattleskillEffectLogicArgumentEnum.is_reset_completed) > 0U);
                    continue;
                  }
                  continue;
                ...
                case BattleskillEffectLogicEnum.rescue:
                  if (useUnit != null)
                  {
                    ...
                    if (column1 < 0)
                      column1 = 0;
                    BL.Unit[] unitArray = new BL.Unit[1]
                    {
                      target.originalUnit
                    };
                    if (BattleFuncs.isResetPositionOK(target.originalUnit, row1, column1, move, (IEnumerable<BL.Unit>) unitArray))
                    {
                    ...
                    for (int index1 = 1; index1 <= 50; ++index1)
                    {
                      ...
                      {
                        if (BattleFuncs.isResetPositionOK(target.originalUnit, row2, column2, move, (IEnumerable<BL.Unit>) unitArray))
                        {
                ...
                case BattleskillEffectLogicEnum.ratio_immediate_damage:
                  ...
                  int num7 = effect.GetInt(BattleskillEffectLogicArgumentEnum.min_hp);
                    ...
                    while (enumerator.MoveNext())
                    {
                      ...
                      if (iskillEffectListUnit.hp > 0)
                      {
                        ...
                        if (damage1 >= 1)
                        {
                          ...
                          if (iskillEffectListUnit.hp > num7)
                          {
                            iskillEffectListUnit.hp -= damage1;
                            if (iskillEffectListUnit.hp < num7)
                              iskillEffectListUnit.hp = num7;
                          }
                ...
                case BattleskillEffectLogicEnum.attract:
                  ...
                  {
                    BL.UnitPosition unitPosition2 = BattleFuncs.iSkillEffectListUnitToUnitPosition(useUnit);
                    int[] attractDelta = BattleFuncs.getAttractDelta(unitPosition2, unitPosition1);
                    BL.Unit[] unitArray = new BL.Unit[2]
                    {
                      useUnit.originalUnit,
                      target.originalUnit
                    };
                    if (BattleFuncs.isResetPositionOK(useUnit.originalUnit, unitPosition2.row + attractDelta[0], unitPosition2.column + attractDelta[1], unitPosition2.unit.parameter.Move, (IEnumerable<BL.Unit>) unitArray) && BattleFuncs.isResetPositionOK(target.originalUnit, unitPosition1.row + attractDelta[0], unitPosition1.column + attractDelta[1], unitPosition1.unit.parameter.Move, (IEnumerable<BL.Unit>) unitArray))
                    {
                      RecoveryUtility.resetPosition(unitPosition2, unitPosition2.row + attractDelta[0], unitPosition2.column + attractDelta[1], env, true, true);
                      ...
                      if (!isAI)
                      {
                        env.setCurrentField(unitPosition2.row, unitPosition2.column);
                        continue;
                      }
                      continue;
                    }
                ...
                case BattleskillEffectLogicEnum.shift_break:
                  int posY;
                  int posX;
                  if (useUnit != null && skillEffectListUnit1 != null && BattleFuncs.getShiftBreakPosition(useUnit, skillEffectListUnit1, effect.GetInt(BattleskillEffectLogicArgumentEnum.range), out posY, out posX))
                  {
                    ...
                    {
                      BattleskillSkill skill1 = MasterData.BattleskillSkill[key1];
                      foreach (BattleskillEffect effect1 in skill1.Effects)
                        target.skillEffects.Add(BL.SkillEffect.FromMasterData(effect1, skill1, 1, investUnit: useUnit.originalUnit, investSkillId: skill.ID, investTurn: this.phaseState.absoluteTurnCount), checkEnableUnit: target);
                      if (!isAI)
                        target.originalUnit.commit();
                    }
                    if (effect.GetInt(BattleskillEffectLogicArgumentEnum.is_complete) == 0)
                    {
                      ...
                      {
                        BattleskillSkill skill1 = MasterData.BattleskillSkill[key2];
                        foreach (BattleskillEffect effect1 in skill1.Effects)
                          target.skillEffects.Add(BL.SkillEffect.FromMasterData(effect1, skill1, 1, isDontDisplay: true));
                      }
                      if (!isAI)
                        target.originalUnit.commit();
                    }
                ...
                case BattleskillEffectLogicEnum.keep_away:
                  if (useUnit != null)
                  {
                    ...
                    List<Tuple<int, int, int>> penetratePosition = BattleFuncs.getPenetratePosition(unitPosition2.row, unitPosition2.column, unitPosition1.row, unitPosition1.column, penetrateCount);
                    ...
                    using (IEnumerator<Tuple<int, int, int>> enumerator = penetratePosition.OrderByDescending<Tuple<int, int, int>, int>((Func<Tuple<int, int, int>, int>) (x => x.Item3)).GetEnumerator())
                    {
                      while (enumerator.MoveNext())
                      {
                        ...
                        if (BattleFuncs.isResetPositionOK(target.originalUnit, current.Item1, current.Item2, unitPosition1.unit.parameter.Move, (IEnumerable<BL.Unit>) unitArray))
                        {
                ...
                case BattleskillEffectLogicEnum.call_reinforcements:
                  ...
                  {
                    int num1 = callReinforcement._battleskill_effect.GetInt(BattleskillEffectLogicArgumentEnum.unit_group_id);
                    int num2 = callReinforcement._battleskill_effect.GetInt(BattleskillEffectLogicArgumentEnum.target_unit_max);
                    ...
                    foreach (BL.UnitPosition unitPosition2 in this.unitPositions.value)
                    {
                      bool flag3 = false;
                      if (unitPosition2.unit.playerUnit.reinforcement != null)
                      {
                        if (num1 > 0)
                        {
                          int? groupId = unitPosition2.unit.playerUnit.group_id;
                          int num3 = num1;
                          if (!(groupId.GetValueOrDefault() == num3 & groupId.HasValue))
                            continue;
                        }
                        if (!unitPosition2.unit.isEnable)
                          flag3 = true;
                        else if (unitPosition2.unit.isDead)
                          flag3 = true;
                        if (flag3)
                          unitPositionList1.Add(unitPosition2);
                      }
                    }
                    if (num2 <= 0 || unitPositionList1.Count <= num2)
                    {
                    ...
                    else
                    {
                      for (int index = 0; index < num2; ++index)
                      {
                        ...
                        BL.UnitPosition unitPosition2 = unitPositionList1[(int) ((long) randomValue % (long) unitPositionList1.Count)];
                        unitPositionList1.Remove(unitPosition2);
                        unitPositionList2.Add(unitPosition2);
                      }
                    }
                ...
                case BattleskillEffectLogicEnum.changing:
                  ...
                  {
                    BL.UnitPosition unitPosition2 = BattleFuncs.iSkillEffectListUnitToUnitPosition(useUnit);
                    int row = unitPosition2.row;
                    int column = unitPosition2.column;
                    applyChangeSkillEffects.clearMovePanelCacheAll = true;
                    RecoveryUtility.resetPosition(unitPosition2, unitPosition1.row, unitPosition1.column, env, true, true);
                    if (effect.GetInt(BattleskillEffectLogicArgumentEnum.is_complete) == 0)
                    {
                      val1_1 = Math.Max(val1_1, 1);
                      int[] numArray = new int[3]
                      ...
                      foreach (int key in numArray)
                      {
                        BattleskillSkill skill1 = MasterData.BattleskillSkill[key];
                        foreach (BattleskillEffect effect1 in skill1.Effects)
                          useUnit.skillEffects.Add(BL.SkillEffect.FromMasterData(effect1, skill1, 1, isDontDisplay: true));
                      }
                      if (!isAI)
                ...
                case BattleskillEffectLogicEnum.transformation:
                  ...
                  int num8 = effect.HasKey(BattleskillEffectLogicArgumentEnum.is_complete) ? effect.GetInt(BattleskillEffectLogicArgumentEnum.is_complete) : 0;
                  if (num8 != 0)
                  {
                ...
                case BattleskillEffectLogicEnum.jump:
                  ...
                  {
                    int key1 = effect.GetInt(BattleskillEffectLogicArgumentEnum.skill_id);
                    if (MasterData.BattleskillSkill.ContainsKey(key1))
                    {
                      int key2 = effect.GetInt(BattleskillEffectLogicArgumentEnum.aiming_skill_id);
                      if (MasterData.BattleskillSkill.ContainsKey(key2))
                      {
                        BattleskillSkill skill1 = MasterData.BattleskillSkill[key2];
                        if (skill1.Effects.Length != 0)
                        {
                          BattleskillEffect effect1 = skill1.Effects[0];
                          if (effect1.effect_logic.Enum == BattleskillEffectLogicEnum.aiming)
                          {
                            int num1 = BattleFuncs.getAllUnits(isAI, true, includeJumping: true).Max<BL.ISkillEffectListUnit>((Func<BL.ISkillEffectListUnit, int>) (x =>
                            {
                              IEnumerable<BL.SkillEffect> source = x.skillEffects.Where(BattleskillEffectLogicEnum.jump);
                              return !source.Any<BL.SkillEffect>() ? 0 : source.Max<BL.SkillEffect>((Func<BL.SkillEffect, int>) (y => (int) y.work[1])) + 1;
                            }));
                            bool flag3 = (effect.HasKey(BattleskillEffectLogicArgumentEnum.effect_target) ? effect.GetInt(BattleskillEffectLogicArgumentEnum.effect_target) : 0) == 0 || useUnit == target;
                            BL.ISkillEffectListUnit unit = flag3 ? useUnit : target;
                            BL.SkillEffect effect2 = BL.SkillEffect.FromMasterData(effect, skill, level, investUnit: useUnit.originalUnit, investSkillId: investSkillId, isDontDisplay: true, investTurn: this.phaseState.absoluteTurnCount);
                            effect2.work = new float[3]
                            {
                              (float) unitPosition1.id,
                              (float) num1,
                              (float) env.getForceID(useUnit.originalUnit)
                            };
                            unit.skillEffects.Add(effect2);
                            BattleskillSkill skill2 = MasterData.BattleskillSkill[key1];
                            foreach (BattleskillEffect effect3 in skill2.Effects)
                              unit.skillEffects.Add(BL.SkillEffect.FromMasterData(effect3, skill2, 1, investUnit: useUnit.originalUnit, investSkillId: investSkillId, isDontDisplay: true, investTurn: this.phaseState.absoluteTurnCount));
                            BL.SkillEffect effect4 = BL.SkillEffect.FromMasterData(effect1, skill1, 1, investUnit: useUnit.originalUnit, investSkillId: investSkillId, investTurn: this.phaseState.absoluteTurnCount);
                            target.skillEffects.Add(effect4);
                            ...
                            if (!flag3)
                            {
                              ...
                              if (!isAI)
                              {
                                ...
                                if (actionUnits.value.Contains(unitPosition2))
                                {
                                  actionUnits.value.Remove(unitPosition2);
                                  actionUnits.commit();
                                }
                              }
                              else
                              ...
                              unitPosition2.recoveryCompleteUnit();
                              applyChangeSkillEffects.clearMovePanelCacheAll = true;
                            }
                            if (flag3)
                            {
                ...
                case BattleskillEffectLogicEnum.invest_land_tag:
                  if (useUnit != null && panels.Count == 1)
                  {
                    int num1 = effect.GetInt(BattleskillEffectLogicArgumentEnum.min_range);
                    int num2 = effect.GetInt(BattleskillEffectLogicArgumentEnum.max_range);
                    ...
                    }).GetEnumerator())
                    {
                      while (enumerator.MoveNext())
                      {
                        ...
                        if (!isAI)
                        {
                          ...
                          if (fieldUnits != null)
                          {
                            foreach (BL.UnitPosition unitPosition2 in fieldUnits)
                              unitPosition2.unit.skillEffects.LandTagModified.commit();
                          }
                ...
                case BattleskillEffectLogicEnum.inhale:
                  if (useUnit != null && panels.Count == 1)
                  {
                    ...
                    BL.UnitPosition unitPosition2 = BattleFuncs.iSkillEffectListUnitToUnitPosition(useUnit);
                    BL.Panel panel = BattleFuncs.getPanel(unitPosition2.row, unitPosition2.column);
                    using (List<BL.UnitPosition>.Enumerator enumerator = targets1.GetEnumerator())
                    {
                      while (enumerator.MoveNext())
                      {
                        ...
                        if (!BattleFuncs.getAnchorGroundEffects(iskillEffectListUnit, useUnit, BattleFuncs.getPanel(current.row, current.column), panel).Any<BL.SkillEffect>() && BattleFuncs.checkInvokeSkillEffect(packedSkillEffect, useUnit, iskillEffectListUnit))
                        {
                          ...
                          int row1 = panels[0].row;
                          int column1 = panels[0].column;
                          bool countMoveDistance = iskillEffectListUnit == useUnit;
                          if (BattleFuncs.isResetPositionOK(iskillEffectListUnit.originalUnit, row1, column1, move, (IEnumerable<BL.Unit>) unitArray, isAI))
                          {
                            RecoveryUtility.resetPosition(current, row1, column1, env, true, countMoveDistance);
                            applyChangeSkillEffects.clearMovePanelCacheAll = true;
                            applyChangeSkillEffects.setMoveUnitDistance = !countMoveDistance;
                          }
                          else
                          {
                            int num1 = current.row - row1;
                            int num2 = current.column - column1;
                            if (num1 > 0 && num2 >= 0)
                            {
                              if (Mathf.Abs(num1) >= Mathf.Abs(num2))
                                ++row1;
                            ...
                            else
                            {
                              ...
                              for (int index1 = 1; index1 <= 50; ++index1)
                              {
                                ...
                                for (int index2 = 0; index2 < index1; ++index2)
                                {
                                  if (BattleFuncs.isResetPositionOK(iskillEffectListUnit.originalUnit, row2, column2, move, (IEnumerable<BL.Unit>) unitArray, isAI))
                                  {
                                    RecoveryUtility.resetPosition(current, row2, column2, env, true, countMoveDistance);
                                    applyChangeSkillEffects.clearMovePanelCacheAll = true;
                                    applyChangeSkillEffects.setMoveUnitDistance = !countMoveDistance;
                                    flag3 = true;
                                    break;
                ...
                case BattleskillEffectLogicEnum.facility_creation:
                  if (useUnit != null && panels.Count == 1)
                  {
                    BL.UnitPosition unitPosition2 = (BL.UnitPosition) null;
                    BL.UnitPosition unitPosition3 = (BL.UnitPosition) null;
                    int num1 = int.MaxValue;
                    int val1_2 = -1;
                    int num2 = effect.GetInt(BattleskillEffectLogicArgumentEnum.facility_unit_id);
                    if (!isAI)
                    {
                      foreach (BL.UnitPosition unitPosition4 in this.unitPositions.value)
                      {
                        BL.Unit unit = unitPosition4.unit;
                        if (unit.isFacility && unit.facility.isSkillUnit)
                        {
                          ...
                          bool flag3 = skillUnitIndex.GetValueOrDefault() == index & skillUnitIndex.HasValue && unit.facility.thisForce == env.getForceID(useUnit.originalUnit) && unit.playerUnit._unit == num2;
                          if (unit.isEnable && !unit.isDead)
                          {
                            val1_2 = Math.Max(val1_2, unit.facilitySpawnOrder);
                            if (flag3 && unit.facilitySpawnOrder < num1)
                            {
                              num1 = unit.facilitySpawnOrder;
                              unitPosition3 = unitPosition4;
                            }
                          }
                          else if (flag3 && unitPosition2 == null)
                            unitPosition2 = unitPosition4;
                        }
                      }
                      if (unitPosition2 == null && unitPosition3 != null)
                      {
                        unitPosition2 = unitPosition3;
                        destructFacilities.Add(unitPosition2.unit);
                      }
                    }
                    if (unitPosition2 != null)
                    {
                      BL.UnitPosition unitPosition4 = unitPosition2;
                      unitPosition4.row = panels[0].row;
                      unitPosition4.column = panels[0].column;
                      unitPosition4.direction = 0.0f;
                      NGBattleManager instance = Singleton<NGBattleManager>.GetInstance();
                      if (instance.isPvp && instance.order == 1)
                        unitPosition4.direction = 180f;
                      if (!isAI)
                      {
                        BL.Unit unit = unitPosition4.unit;
                        ...
                        unitPosition4.resetOriginalPosition(env, resetOriginalToOuterPosition: true);
                ...
                case BattleskillEffectLogicEnum.reduct_command_skill_use:
                  ...
                  int num9 = effect.GetInt(BattleskillEffectLogicArgumentEnum.value);
                  if (targetSkillId == 0 && targetLogicId == 0 && (!effect.HasKey(BattleskillEffectLogicArgumentEnum.not_random) || effect.GetInt(BattleskillEffectLogicArgumentEnum.not_random) == 0))
                  {
                    for (int index1 = 0; index1 < num9; ++index1)
                    {
                      ...
                      if (array.Length != 0)
                      {
                        int index2 = (int) random.NextFixed((uint) array.Length);
                        BL.Skill skill1 = ((IEnumerable<BL.Skill>) array).ElementAt<BL.Skill>(index2);
                  ...
                  using (IEnumerator<BL.Skill> enumerator = BattleFuncs.getReductCommandSkillUseTargetSkills(target, targetSkillId, targetLogicId, useUnit, effect.skill_BattleskillSkill).GetEnumerator())
                  {
                    while (enumerator.MoveNext())
                    {
                      ...
                      BL.Skill skill1 = current;
                      int? remain1 = skill1.remain;
                      int num1 = num9;
                      skill1.remain = remain1.HasValue ? new int?(remain1.GetValueOrDefault() - num1) : new int?();
                      int? remain2 = current.remain;
                      int num2 = 0;
                      if (remain2.GetValueOrDefault() < num2 & remain2.HasValue)
      ...
      XorShift random = null)
    {
      ...
      if (battleskillEffect != null)
      {
        ...
        {
          BattleskillSkill skill1 = MasterData.BattleskillSkill[key];
          foreach (BattleskillEffect effect in skill1.Effects)
            unit.skillEffects.Add(BL.SkillEffect.FromMasterData(effect, skill1, 1, isDontDisplay: true));
        }
      }
      ...
      if (val1 != 1)
      {
        foreach (BL.SkillEffect skillEffect in skillEffects)
        {
          nullable1 = new int?();
          int? nullable2 = nullable1;
          skillEffect.moveDistance = nullable2;
        }
      }
      else
      {
    ...
    public void useSkillWith(
      ...
      List<BL.Panel> panels,
      BL.BattleSkillResult bsr,
      System.Action<BL.UseSkillWithResult> f,
      BL env,
      XorShift random = null)
    {
    ...
    public List<BL.UnitPosition> getSkillTargetUnits(BL.UnitPosition up, BL.Skill skill) => this.getSkillTargetUnits(up.unit, up.row, up.column, skill, up is BL.AIUnit);
      ...
      IEnumerable<BL.SkillEffect> useSkillEffects = null)
    {
      ...
      if (unit.hp > 0 && env.phaseState.absoluteTurnCount > 0)
      {
            ...
            if ((!skillLogicEffect.isCheckSeal() || !BattleFuncs.isSealedSkillEffect(unit, effect)) && (upMove == null || skillLogicEffect.checkMoveUnitInvoke(upMove)))
            {
              ...
              else
              {
                ...
                if (BattleFuncs.checkInvokeSkillEffectCommon(pse2, colosseumTurn))
                {
                  ...
                  nullable = new int?();
                  int? unitHp = nullable;
                  if (BattleFuncs.checkInvokeSkillEffectSelf(pse3, unit1, unitHp: unitHp) && pse1.CheckLandTag(panel, isAI))
                  {
        ...
        if (length > 0 && skillParams.Count > 0)
        {
          ...
          BL.ISkillEffectListUnit moveUnit = upMove != null ? BattleFuncs.unitPositionToISkillEffectListUnit(upMove) : (BL.ISkillEffectListUnit) null;
          ...
          foreach (BattleFuncs.SkillParam skillParam in BattleFuncs.gearSkillParamCategoryFilter(skillParams))
          {
            ...
            if (!skillLogicEffect.isPanelTarget())
            {
              ...
              foreach (BL.UnitPosition unitPosition in targets)
              {
                ...
                if (iskillEffectListUnit.hp > 0)
                {
                  flag = true;
                  int hp = iskillEffectListUnit.hp;
                  applyChangeSkillEffects.add(unitPosition, iskillEffectListUnit, unitPosition == upMove);
                  skillLogicEffect.init(env, pse, forceIdArray, targetForce, up, unit, isAI, extData);
                  skillLogicEffect.execute(unitPosition, iskillEffectListUnit, applyChangeSkillEffects, (BL.Panel) null, moveUnit, parameter, targets);
                  if (iskillEffectListUnit.hp < hp)
                  {
                    if (skillLogicEffect.damageBySwapHeal)
                    {
                      if (iskillEffectListUnit.hp <= 0 && deads != null)
                    ...
                    else
                    {
                      ...
                      if (iskillEffectListUnit.hp <= 0)
                      {
                        if (!isAI)
                        {
                          if (skillEffectListUnit != null)
                          {
                            ...
                            iskillEffectListUnit.originalUnit.killedBy = skillEffectListUnit.originalUnit;
                          }
                          if (env.getForceID(iskillEffectListUnit.originalUnit) == BL.ForceID.player)
                            env.updateIntimateByDefense(iskillEffectListUnit.originalUnit);
                        }
                      ...
                      if (!isAI && skillEffectListUnit != null && !iskillEffectListUnit.originalUnit.isFacility)
                      {
                        if (!skillEffectListUnit.originalUnit.isFacility)
                          skillEffectListUnit.originalUnit.attackDamage += hp - iskillEffectListUnit.hp;
                        else
                          iskillEffectListUnit.originalUnit.addReceivedLandformDamage(hp - iskillEffectListUnit.hp);
                      }
                    }
                  }
                  ...
                  if (index2 == -1)
                  {
                    ...
                    skillEffectResult.target_hps.Add(iskillEffectListUnit.hp);
                    skillEffectResult.disp_target_hps.Add(skillLogicEffect.isDispHpNumber());
                  }
                  else
                  {
                    skillEffectResult.target_hps[index2] = iskillEffectListUnit.hp;
            ...
            else
            {
              ...
              foreach (BL.Panel targetPanel in panelArray)
              {
                flag = true;
                skillLogicEffect.init(env, pse, forceIdArray, targetForce, up, unit, isAI, extData);
                skillLogicEffect.execute((BL.UnitPosition) null, (BL.ISkillEffectListUnit) null, applyChangeSkillEffects, targetPanel, moveUnit, (Judgement.BattleParameter) null, (List<BL.UnitPosition>) null);
                BL.ExecuteSkillEffectResult skillEffectResult = skillEffectResultArray1[index1];
    ...
    public BL.ClassValue<List<BL.UnitPosition>> currentActionUnitsList() => this.getActionUnitsList(this.phaseState.state);
    ...
    public int getActionUnitsIndexOf(BL.Unit unit, ref BL.ForceID forceId) => unit == (BL.Unit) null ? -1 : this.getActionUnitsIndexOf(this.getUnitPosition(unit), ref forceId);
    ...
    public BL.Story getStorySpawn(BL.Unit unit)
    {
      foreach (BL.Story story in this.storyList.value)
      {
        if (story.type == BL.StoryType.spawn_unit && story.datas.Length != 0 && (story.datas[0] is int && (int) story.datas[0] == unit.unit.ID))
          return story;
      }
      return (BL.Story) null;
    }
    ...
    public BL.Story getStoryUnitForUnit(BL.Unit from, BL.Unit to)
    {
      foreach (BL.Story story in this.storyList.value)
      {
        if (story.type == BL.StoryType.unit_for_unit && story.datas.Length != 0 && story.datas[0] is int)
        {
          int data = (int) story.datas[0];
    ...
    {
      List<BL.Story> storyList = new List<BL.Story>();
      foreach (BL.Story story in this.storyList.value)
      {
    ...
    public List<BL.Story> getStoryWaveOffense(int turn, int wave) => this.findStoryTurnStart(BL.StoryPhase.offense, turn, wave);

    public List<BL.Story> getStoryWaveDefense(int turn, int wave) => this.findStoryTurnStart(BL.StoryPhase.defense, turn, wave);

    public List<BL.Story> getStoryOffense(int turn) => this.findStoryTurnStart(BL.StoryPhase.offense, turn);

    public List<BL.Story> getStoryDefense(int turn) => this.findStoryTurnStart(BL.StoryPhase.defense, turn);
    ...
    public List<BL.Story> getStoryOffenseInArea(int row, int column) => this.findStoryInArea(BL.StoryPhase.offense, row, column);

    public List<BL.Story> getStoryDefenseInArea(int row, int column) => this.findStoryInArea(BL.StoryPhase.defense, row, column);
    ...
    public List<BL.Story> getStoryDefeat(int unitId, bool swRead = true)
    {
      ...
      this.storyList.value.ForEach((System.Action<BL.Story>) (s =>
      {
    ...
    public void setCurrentUnitByPlayerInput(BL.Unit unit, System.Action<BL.UnitPosition> f) => this.unitCurrent.setCurrentByPlayerInput(unit, this, f);

    public void setCurrentUnitWith(BL.Unit unit, System.Action<BL.UnitPosition> f) => this.unitCurrent.setCurrentWith(unit, this, f);

    public void setCurrentUnitWithSetOnly(BL.Unit unit) => this.unitCurrent.setCurrentWithSetOnly(unit);
    ...
    public void updateUnitBattleStatus(DuelResult duelResult, BL.Unit attack, BL.Unit defense)
    {
      ...
      if (attack.hp <= 0)
      {
        ...
        int oDamage = 0;
        ((IEnumerable<BL.DuelTurn>) duelResult.turns).ForEach<BL.DuelTurn>((System.Action<BL.DuelTurn>) (x =>
        {
          if (x.isAtackker)
            return;
        ...
        if ((UnityEngine.Object) Singleton<NGBattleManager>.GetInstance() != (UnityEngine.Object) null && Singleton<NGBattleManager>.GetInstance().isGvg && (!attack.isFacility && !defense.isFacility))
    ...
    public bool checkDeadCount(int id, int count, bool isAI) => count == 0 ? this.enemyUnits.value.Where<BL.Unit>((Func<BL.Unit, bool>) (x =>
    {
      if (id == 0)
        return true;
    ...
    public bool isReinforceUnitForSmash(PlayerUnit pu, bool isAI = false) => pu.reinforcement != null && pu.reinforcement.reinforcement_logic.Enum == BattleReinforcementLogicEnum.smash && this.checkDeadCount(pu.reinforcement.arg1_value, pu.reinforcement.arg2_value, isAI);
    ...
    public static BL.BattleModified<T> Observe<T>(T v) where T : BL.ModelBase => new BL.BattleModified<T>(v);
    ...
    public class AIUnit : BL.UnitPosition, BL.ISkillEffectListUnit
    {
      ...
      public Judgement.BattleParameter parameter => Judgement.BattleParameter.FromBeUnit((BL.ISkillEffectListUnit) this);
      ...
      public bool isHealer => (uint) this.healRange.Length > 0U;
      ...
      public IEnumerable<BL.Skill> commandSkills => (IEnumerable<BL.Skill>) this.mCommandSkills.Values;
      ...
      public bool IsDontEvasion => this.mSkillEffectList.HasAilmentEffectLogic(BattleskillEffectLogicEnum.paralysis, BattleskillEffectLogicEnum.sleep);

      public bool IsCharm => this.mSkillEffectList.HasAilmentEffectLogic(BattleskillEffectLogicEnum.charm);

      public bool IsJumping => this.mSkillEffectList.Where(BattleskillEffectLogicEnum.jump).Any<BL.SkillEffect>();

      public bool IsDontUseCommand(BL.Skill skill) => BattleFuncs.isDontUseCommand((BL.ISkillEffectListUnit) this, skill);

      public bool IsDontUseOugi(BL.Skill skill) => BattleFuncs.isDontUseOugi((BL.ISkillEffectListUnit) this, skill);

      public bool IsDontUseSkill(int skill_id) => this.mSkillEffectList.HasAilmentEffectLogic(BattleskillEffectLogicEnum.seal) && this.skillEffects.IsSealedSkill(skill_id);

      public bool IsDontUseSkillEffect(BL.SkillEffect effect) => this.mSkillEffectList.HasAilmentEffectLogic(BattleskillEffectLogicEnum.seal) && this.skillEffects.IsSealedSkillEffect(effect);

      public bool CantChangeCurrent => this.mSkillEffectList.HasAilmentEffectLogic(BattleskillEffectLogicEnum.can_not_change_current);

      public bool CanHeal(BattleskillSkillType skillType = (BattleskillSkillType) 0) => BattleFuncs.canHeal((BL.ISkillEffectListUnit) this, skillType);

      public bool HasEnabledSkillEffect(BattleskillEffectLogicEnum logic) => this.enabledSkillEffect(logic).Any<BL.SkillEffect>();

      public IEnumerable<BL.SkillEffect> enabledSkillEffect(
      ...
      public IEnumerable<BL.SkillEffect> getSkillEffects(
        BattleskillEffectLogicEnum logic)
      {
        return this.mSkillEffectList.Where(logic);
      }

      public bool checkTargetAttribute(BL.Unit.TargetAttribute ta) => this.mUnit.checkTargetAttribute(ta);

      public int transformationGroupId => BattleFuncs.getTransformationGroupId((BL.ISkillEffectListUnit) this);

      public bool checkEnableSkill(BattleskillSkill skill) => BattleFuncs.checkEnableUnitSkill((BL.ISkillEffectListUnit) this, skill);
      ...
      public BL.Unit.GearRange gearRange() => BattleFuncs.getGearRange((BL.ISkillEffectListUnit) this);

      public BL.Unit.MagicRange magicRange(BL.MagicBullet mb) => BattleFuncs.getMagicRange((BL.ISkillEffectListUnit) this, mb);
      ...
      public BL.AIUnitNetwork ToNetwork(BL env) => new BL.AIUnitNetwork()
      {
      ...
      private void completedPositionExecuteSkillEffects(BL env, HashSet<BL.AIUnit> charmActionUnits)
      {
        ...
        foreach (BL.ISkillEffectListUnit skillEffectListUnit in deads)
          this.doDead(skillEffectListUnit as BL.AIUnit, env);
      ...
      {
        foreach (BL.AIUnit u in env.aiUnitPositions.value)
        {
          if (u.isDead)
          {
            BL.SkillEffect immediateRebirthEffect = BattleFuncs.getImmediateRebirthEffects((BL.ISkillEffectListUnit) u).FirstOrDefault<BL.SkillEffect>();
            if (immediateRebirthEffect != null)
            {
              BattleFuncs.useImmediateRebirthEffect((BL.ISkillEffectListUnit) u, immediateRebirthEffect);
              bool resetCompleted = (uint) immediateRebirthEffect.effect.GetInt(BattleskillEffectLogicArgumentEnum.is_reset_completed) > 0U;
              this.doRebirth(u, env, false, resetCompleted);
              int key = immediateRebirthEffect.effect.GetInt(BattleskillEffectLogicArgumentEnum.skill_id);
              if (key != 0 && MasterData.BattleskillSkill.ContainsKey(key))
              {
                BattleFuncs.ApplyChangeSkillEffectsOne changeSkillEffectsOne = new BattleFuncs.ApplyChangeSkillEffectsOne((BL.UnitPosition) u, (BL.ISkillEffectListUnit) u, true);
                ...
                foreach (BattleskillEffect effect in skill.Effects)
                  u.skillEffects.Add(BL.SkillEffect.FromMasterData(effect, skill, 1, investUnit: u.originalUnit, investTurn: env.phaseState.absoluteTurnCount), checkEnableUnit: ((BL.ISkillEffectListUnit) u));
      ...
      public bool actionAIUnit(BL env, bool useCost = true)
      {
        ...
        int num1 = !useCost || this.mCompletedCount <= 1 ? 0 : env.getRouteCostNonCache((BL.UnitPosition) this, fieldPanel2, fieldPanel1, this.movePanels, this.completePanels);
        int num2 = BL.fieldDistance(fieldPanel1, fieldPanel2);
        this.mMoveDistance += num2;
        foreach (BL.SkillEffect skillEffect1 in this.skillEffects.All())
        {
          if (skillEffect1.moveDistance.HasValue)
          {
            ...
            int num3 = num2;
            skillEffect2.moveDistance = moveDistance.HasValue ? new int?(moveDistance.GetValueOrDefault() + num3) : new int?();
          }
          else
            skillEffect1.moveDistance = new int?(num2);
        }
        ...
        if (duelResult != null)
        {
          ...
          num4 = aiUnit1.hp - duelResult.attackDamage;
          num5 = aiUnit2.hp - duelResult.defenseDamage;
        }
        ...
        if (this.mActionResults != null)
        {
          foreach (ActionResult mActionResult in this.mActionResults)
          {
            if (!mActionResult.isCompleted)
            {
              try
              {
                num6 = this.applyActionResult(mActionResult, env, unitPositionList);
              }
              catch (Exception ex)
              {
        ...
        BL.AIUnit aiUnit3 = (BL.AIUnit) null;
        BL.AIUnit aiUnit4 = (BL.AIUnit) null;
        int defenseHp = 0;
        ...
        else
        {
          if (duelResult != null)
          {
            ...
            aiUnit3 = env.getAIUnit(!flag ? duelResult.attack : duelResult.defense);
            aiUnit4 = env.getAIUnit(!flag ? duelResult.defense : duelResult.attack);
            defenseHp = !flag ? num5 : num4;
          }
          if (this.mHp > 0)
          {
            Tuple<int, int> completeActionCount = BattleFuncs.getNextCompleteActionCount((BL.ISkillEffectListUnit) this, (BL.UnitPosition) this, (BL.ISkillEffectListUnit) aiUnit3, (BL.ISkillEffectListUnit) aiUnit4, defenseHp);
        ...
        if (useCost)
        {
          this.mUsedMoveCost += num1 - BattleFuncs.getRunAwayValue((BL.ISkillEffectListUnit) this);
          ...
          Tuple<int, int> completeActionCount = BattleFuncs.getNextCompleteActionCount((BL.ISkillEffectListUnit) this, (BL.UnitPosition) this, (BL.ISkillEffectListUnit) aiUnit3, (BL.ISkillEffectListUnit) aiUnit4, defenseHp, true);
      ...
      private int applyActionResult(ActionResult ar, BL env, List<BL.UnitPosition> spawnUnits)
      {
        ...
        if (duelResult != null)
        {
          ...
          if (duelResult.isHeal)
          {
            ...
            List<BL.Panel> panels = new List<BL.Panel>();
            BL env1 = env;
            BL.AIUnit aiUnit3 = aiUnit1;
            bool? callIsPlayer = new bool?();
            IEnumerable<BL.AIUnit> second = bl.setSkillEffect(skill, 1, targets, panels, (BL.BattleSkillResult) null, env1, (BL.ISkillEffectListUnit) aiUnit3, true, callIsPlayer: callIsPlayer).Item1.Select<BL.Unit, BL.AIUnit>((Func<BL.Unit, BL.AIUnit>) (x => env.getAIUnit(x)));
            ...
            int healHpTotal = 0;
            foreach (BL.AIUnit aiUnit4 in list)
            {
              if (dictionary.ContainsKey(aiUnit4) && aiUnit4.hp > dictionary[aiUnit4])
                healHpTotal += aiUnit4.hp - dictionary[aiUnit4];
              if (aiUnit4.hp <= 0 && aiUnit4 != aiUnit1 && aiUnit4 != aiUnit2)
              {
                this.doDead(aiUnit4, env);
                flag2 = true;
              }
            }
            if (aiUnit1.hp >= 1)
        ...
        else if (bsr != null)
        {
          ...
          if (tuple.Item2 != null)
          {
            foreach (BL.ISkillEffectListUnit skillEffectListUnit1 in tuple.Item2)
            {
              HashSet<BL.ISkillEffectListUnit> deads = new HashSet<BL.ISkillEffectListUnit>();
              env.completedPositionExecuteSkillEffects(skillEffectListUnit1 as BL.UnitPosition, out List<List<BL.ExecuteSkillEffectResult>> _, deads);
              foreach (BL.ISkillEffectListUnit skillEffectListUnit2 in deads)
                this.doDead(skillEffectListUnit2 as BL.AIUnit, env);
            }
          }
        }
        ar.isCompleted = true;
      ...
      private void doDead(BL.AIUnit u, BL env)
      {
        ...
        foreach (BL.AIUnit aiUnit in env.aiUnitPositions.value)
        {
          if (BattleFuncs.hasEnabledDeadCountEffects((BL.ISkillEffectListUnit) aiUnit, (BL.ISkillEffectListUnit) u))
            aiUnit.skillEffects.commit();
        }
      ...
      private void spawnAIUnits(IEnumerable<BL.UnitPosition> units, BL env)
      {
        ...
        foreach (BL.UnitPosition unit in units)
        {
          ...
          aiUnit.resetSpawnPosition(isAI: true);
          env.aiUnitPositions.value.Add(aiUnit);
          env.aiUnitPositions.commit();
          env.aiUnits.value.Add(aiUnit);
          env.aiUnits.commit();
          flag = true;
          env.addZocPanels((BL.ISkillEffectListUnit) aiUnit, aiUnit.originalRow, aiUnit.originalColumn, true);
          aiUnit.addPanelSkillEffects();
        }
        if (!flag)
          return;
        foreach (BL.AIUnit aiUnit in env.aiUnitPositions.value)
    ...
    public class Condition : BL.ModelBase
    {
      ...
      public BattleVictoryAreaCondition[] winAreaConditoin => this.condition.win_area_confition_group_id.HasValue ? ((IEnumerable<BattleVictoryAreaCondition>) MasterData.BattleVictoryAreaConditionList).Where<BattleVictoryAreaCondition>((Func<BattleVictoryAreaCondition, bool>) (x =>
      {
      ...
      public BattleVictoryAreaCondition[] loseAreaConditoin => this.condition.lose_area_confition_group_id.HasValue ? ((IEnumerable<BattleVictoryAreaCondition>) MasterData.BattleVictoryAreaConditionList).Where<BattleVictoryAreaCondition>((Func<BattleVictoryAreaCondition, bool>) (x =>
      {
        int groupId = x.group_id;
    ...
    public class DuelTurn
    {
      ...
      public BL.DuelTurnNetwork ToNetwork(BL env) => new BL.DuelTurnNetwork()
      {
        damage = this.damage,
      ...
      public BattleskillSkill[] skills => ((IEnumerable<int>) this.skillIds).Select<int, BattleskillSkill>((Func<int, BattleskillSkill>) (x => MasterData.BattleskillSkill[x])).ToArray<BattleskillSkill>();
    ...
    public class Panel : BL.ModelBase
    {
      ...
      public bool checkVictoryConditionAtrribute(BL.PanelVictoryConditionAtrribute attr) => (this.mConditionAttribute & attr) == attr;
      ...
      public bool checkReinforcementConditionAtrribute(BL.PanelReinforcementConditionAtrribute attr) => (this.mReinforcementConditionAttribute & attr) == attr;
      ...
      public int[] getReinforcementIDsToPanel(BL.ForceID forceID)
      {
        ...
        switch (forceID)
        {
          case BL.ForceID.player:
            ...
            {
              numArray = this.reinforcementIDs;
              break;
            }
            break;
          case BL.ForceID.enemy:
      ...
      public BL.ClassValue<List<BL.SkillEffect>> getSkillEffects(bool isAI = false) => !isAI ? this.skillEffects : this.skillEffectsAI;

      private int skillEffectCompare(BL.SkillEffect se0, BL.SkillEffect se1) => se0 == null ? (se1 == null ? 0 : -1) : (se1 == null ? 1 : se0.effectId - se1.effectId);
      ...
      public bool hasLandTag(bool isAI, params int[] tags) => this.getSkillEffects(isAI).value.Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x =>
      {
    ...
    public class UnitPosition : BL.ModelBase
    {
      ...
      public static BL.UnitPosition FromNetwork(int? nw, BL env) => nw.HasValue ? env.getUnitPositionById(nw.Value) : (BL.UnitPosition) null;
      ...
      public override string ToString() => "[" + this.mUnit.unit.name + " (" + (object) this.mRow + ", " + (object) this.mColumn + ") direction:" + (object) this.mDirection + "]";
      ...
      public bool isLocalMoved => this.mOriginalRow != this.mRow || this.mOriginalColumn != this.mColumn;
      ...
      public HashSet<BL.Panel> getAllMoveSkillRangePanels(BL.Skill skill)
      {
        ...
        HashSet<BL.Panel> panelSet1;
        if (!this.mAllMoveSkillRangePanelsDic.TryGetValue(skill.id, out panelSet1))
        {
          HashSet<BL.Panel> panelSet2 = (HashSet<BL.Panel>) null;
          if (this.mSkillMovePanelsDic != null)
            this.mSkillMovePanelsDic[skill.id] = panelSet2 = new HashSet<BL.Panel>();
          ...
          HashSet<BL.Panel> positionPanels = panelSet2;
          HashSet<BL.Panel> panelSet3;
          panelSet1 = panelSet3 = BattleFuncs.allMoveActionRangePanels_(this, isAI: (num != 0), skill: skill2, positionPanels: positionPanels);
          skillRangePanelsDic[id] = panelSet3;
        }
        return panelSet1;
      }
      ...
      public int moveCost => BattleFuncs.unitPositionToISkillEffectListUnit(this).parameter.Move - this.mUsedMoveCost;
      ...
      public void recoveryCompleteUnit() => this.mUsedMoveCost = this.mActionCount = this.mCompletedCount = 0;
        ...
        bool isKilledByPanelLandformEffect = false)
      {
        ...
        BL.Panel fieldPanel1 = env.getFieldPanel(this.mOriginalRow, this.mOriginalColumn);
        BL.Panel fieldPanel2 = env.getFieldPanel(this.mRow, this.mColumn);
        int num1 = !useCost || this.mCompletedCount <= 0 ? 0 : env.getRouteCostNonCache(this, fieldPanel2, fieldPanel1, this.movePanels, this.completePanels);
        int num2 = BL.fieldDistance(fieldPanel1, fieldPanel2);
        this.mMoveDistance += num2;
        ...
        {
          this.mUsedMoveCost += num1 - BattleFuncs.getRunAwayValue((BL.ISkillEffectListUnit) this.mUnit);
    ...
    public class MagicBullet : BL.ModelBase
    {
      ...
      {
        get => this.sAttackMethod.ID == 0 ? (IAttackMethod) null : this.attackMethod_ ?? (this.attackMethod_ = this.sAttackMethod.from == BL.MagicBullet.From.Normal ? MasterData.AttackMethod[this.sAttackMethod.ID].CreateInterface() : (this.sAttackMethod.from == BL.MagicBullet.From.Guest ? MasterData.BattleStageGuestAttackMethod[this.sAttackMethod.ID].CreateInterface() : MasterData.BattleStageEnemyAttackMethod[this.sAttackMethod.ID].CreateInterface()));
        set
        {
      ...
      public bool isDrain => this.skill.genre1.HasValue && this.skill.genre1.Value == BattleskillGenre.attack && this.skill.genre2.HasValue && this.skill.genre2.Value == BattleskillGenre.heal;
    ...
    public class Skill : BL.ModelBase
    {
      ...
      public static bool HasDontActionEffect(BL.Skill[] skills) => BL.Skill.HasEffect(skills, BattleskillEffectLogicEnum.paralysis, BattleskillEffectLogicEnum.do_not_act, BattleskillEffectLogicEnum.sleep);

      public static bool HasDontMoveEffect(BL.Skill[] skills) => BL.Skill.HasEffect(skills, BattleskillEffectLogicEnum.paralysis, BattleskillEffectLogicEnum.do_not_move, BattleskillEffectLogicEnum.sleep);

      public static bool HasDontEvasionEffect(BL.Skill[] skills) => BL.Skill.HasEffect(skills, BattleskillEffectLogicEnum.paralysis, BattleskillEffectLogicEnum.sleep);
      ...
      public bool canUseTurn(int absoluteTurn)
      {
        ...
        {
          int num2 = battleskillEffect.GetInt(BattleskillEffectLogicArgumentEnum.turn_cycle);
          if (num2 != 0 && (absoluteTurn - num1) % num2 != 0)
            return false;
        }
        return true;
      }
      ...
      public bool isDeadTargetOnly => this.targetType == BattleskillTargetType.dead_player_single || this.targetType == BattleskillTargetType.dead_player_range;
    ...
    public class SkillEffect : BL.ModelBase
    {
      ...
      public bool deathDisable => !(this.unit == (BL.Unit) null) && this.timeOfDeathDisable && this.unit.isDead;

      public bool isLandTagEffect => this.effect.GetPackedSkillEffect().HasKey(BattleskillEffectLogicArgumentEnum.land_tag1);
      ...
      public static BL.SkillEffect FromRecovery(RecoverySkillEffect rse, BL env) => new BL.SkillEffect()
      {
    ...
    public class SkillEffectList : BL.ModelBase
    {
      ...
      public List<Tuple<BattleskillEffectLogicEnum, BL.SkillEffect, int>> FixEffectParams => this.fixEffectParams;

      public List<Tuple<BattleskillEffectLogicEnum, BL.SkillEffect, float>> RatioEffectParams => this.ratioEffectParams;

      public List<BL.SkillEffect> RemovedBaseSkillEffects => this.removedBaseSkillEffects;

      public List<Tuple<BattleskillEffectLogicEnum, BL.SkillEffect, int>> RemovedFixEffectParams => this.removedFixEffectParams;

      public List<Tuple<BattleskillEffectLogicEnum, BL.SkillEffect, float>> RemovedRatioEffectParams => this.removedRatioEffectParams;
      ...
      public List<BL.SkillEffect> WaitingTransformationSkillEffects => this.waitingTransformationSkillEffects;
      ...
      public bool IsSealedSkill(int skill_id, BL.SkillEffect effect = null) => this.effects.Where<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.EffectLogic.Enum == BattleskillEffectLogicEnum.seal)).Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x =>
      {
      ...
      public bool IsSealedSkillEffect(BL.SkillEffect effect) => this.IsSealedSkill(effect.baseSkillId, effect);
      ...
      public void RemoveEffect(int logic, BL env, BL.ISkillEffectListUnit unit) => this.Clean(logic, env, unit);
      ...
      public void RemoveEffect(BL.Unit invocationUnit, BL env, BL.ISkillEffectListUnit unit)
      {
        ...
        if (list.Any<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.EffectLogic.Enum == BattleskillEffectLogicEnum.zoc)))
        {
          ...
          env.unitPositions.value.ForEach((System.Action<BL.UnitPosition>) (x => x.clearMovePanelCache()));
        }
      ...
      public void RemoveAilmentEffect(BL env, BL.ISkillEffectListUnit unit) => this.RemoveEffect(0, 0, 9, env, unit);
      ...
      private void Clean()
      {
        this.effects = this.effects.Where<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (effect => !effect.turnRemain.HasValue || (uint) effect.turnRemain.Value > 0U)).ToList<BL.SkillEffect>();
        this.effects.AddRange(this.RecoveryOverwriteEffects());
        this.ClearCache();
        this.InitEffects();
        ++this.revision;
      ...
      public IEnumerable<BL.SkillEffect> Where(BattleskillEffectTag tag1) => this.effects.Where<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.EffectLogic.HasTag(tag1)));
      ...
      public IEnumerable<BL.SkillEffect> Where(BattleskillEffectLogicEnum e) => (IEnumerable<BL.SkillEffect>) this.GetEffects(e);
...
label_8:
      ...
      public IEnumerable<BL.SkillEffect> GetAgainEffects() => this.effects.Where<BL.SkillEffect>((Func<BL.SkillEffect, bool>) (x => x.effect.EffectLogic.Enum == BattleskillEffectLogicEnum.again));
      ...
      public void AddRemovedBaseSkillEffect(BL.SkillEffect effect) => this.removedBaseSkillEffects.Add(effect);
      ...
      public List<Tuple<BattleskillEffectLogicEnum, BL.SkillEffect, int>> GetAllRemovedFixEffectParams() => this.removedFixEffectParams;

      public List<Tuple<BattleskillEffectLogicEnum, BL.SkillEffect, float>> GetAllRemovedRatioEffectParams() => this.removedRatioEffectParams;

      public int GetDuelSkillEffectIdInvokeCount(int skillEffectId) => !this.duelSkillEffectIdInvokeCount.ContainsKey(skillEffectId) ? 0 : this.duelSkillEffectIdInvokeCount[skillEffectId];

      public int GetDuelSkillIdInvokeCount(int skillId) => !this.duelSkillIdInvokeCount.ContainsKey(skillId) ? 0 : this.duelSkillIdInvokeCount[skillId];

      public int GetDuelSkillIdInvokeCount2(int skillId) => !this.duelSkillIdInvokeCount2.ContainsKey(skillId) ? 0 : this.duelSkillIdInvokeCount2[skillId];
      ...
      public void SetDuelSkillEffectIdInvokeCount(int skillEffectId, int count) => this.duelSkillEffectIdInvokeCount[skillEffectId] = count;

      public void SetDuelSkillIdInvokeCount(int skillId, int count) => this.duelSkillIdInvokeCount[skillId] = count;

      public void SetDuelSkillIdInvokeCount2(int skillId, int count) => this.duelSkillIdInvokeCount2[skillId] = count;
      ...
      private int GetOverwriteEffectPriority(BL.SkillEffect effect) => effect.isBaseSkill ? (effect.gearIndex != 1 ? (effect.gearIndex < 2 ? 0 : 1) : 2) : 3;
      ...
      public void AddRemovedOverwriteSkillEffect(BL.SkillEffect effect) => this.removedOverwriteSkillEffects.Add(effect);

      public List<BL.SkillEffect> GetAllRemovedOverwriteSkillEffects() => this.removedOverwriteSkillEffects;

      public void AddWaitingTransformationSkillEffect(BL.SkillEffect effect) => this.waitingTransformationSkillEffects.Add(effect);

      public List<BL.SkillEffect> GetAllWaitingTransformationSkillEffects() => this.waitingTransformationSkillEffects;
      ...
      public void ResetTransformationSkillEffects(int transformationGroupId)
      {
        ...
        foreach (BL.SkillEffect transformationSkillEffect in this.waitingTransformationSkillEffects)
        {
          int? transformationGroupId1 = transformationSkillEffect.baseSkill.transformationGroupId;
          int num = transformationGroupId;
          if (transformationGroupId1.GetValueOrDefault() == num & transformationGroupId1.HasValue)
      ...
      public bool IsMoveSkillActionWaiting() => this.Where(BattleskillEffectLogicEnum.do_not_move, (Func<BL.SkillEffect, bool>) (x => x.baseSkillId == 300001381)).Any<BL.SkillEffect>();
    ...
    public class SkillResultUnit : BL.SkillResult
    {
      ...
      private BL.UnitPosition getTarget()
      {
        ...
        BL.UnitPosition unitPosition = (BL.UnitPosition) new BL.AIUnit(this._up, BL.AIType.normal);
        this._bl.aiUnitPositions.value.Add(unitPosition as BL.AIUnit);
        return unitPosition;
      }
    ...
    public abstract class FacilitySkillLogicEffect
    {
        ...
        BL.UnitPosition fromUp)
      {
        ...
        else
        {
          ...
          foreach (BL.UnitPosition unitPosition in ((IEnumerable<BL.ForceID>) forceId).SelectMany<BL.ForceID, BL.UnitPosition>((Func<BL.ForceID, IEnumerable<BL.UnitPosition>>) (x => (IEnumerable<BL.UnitPosition>) this.env.getExecuteSkillEffectsTargets(fromUp, range, x, this.isAI))))
          {
            switch (range_form)
            {
              case 1:
                if (fromUp.row == unitPosition.row || fromUp.column == unitPosition.column)
                  break;
                continue;
              case 2:
                if (Mathf.Abs(fromUp.row - unitPosition.row) != Mathf.Abs(fromUp.column - unitPosition.column))
                  continue;
                break;
            }
            yield return unitPosition;
      ...
      protected IEnumerable<BL.UnitPosition> getRangeTargets(BL.ForceID[] forceId) => this.getRangeTargets(forceId, this.up);
      ...
      private int calcRatioValue() => (int) Math.Ceiling((Decimal) ((float) this.tup.unit.parameter.Hp * (this.pse.GetFloat(BattleskillEffectLogicArgumentEnum.percentage) + (float) this.pse.skillEffect.baseSkillLevel * this.pse.GetFloat(BattleskillEffectLogicArgumentEnum.skill_ratio))));

      private int calcFixValue() => this.pse.GetInt(BattleskillEffectLogicArgumentEnum.value) + this.pse.skillEffect.baseSkillLevel * this.pse.GetInt(BattleskillEffectLogicArgumentEnum.skill_ratio);
      ...
      protected void executeRatioDamageEffect(int damageCutType = -1) => this.addTargetHp(-this.calcRatioValue(), damageCutType);

      protected void executeFixDamageEffect(int damageCutType = -1) => this.addTargetHp(-this.calcFixValue(), damageCutType);
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_ratio_heal;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.myForce);

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.heal;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_fix_heal;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_ratio_attack;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.targetForce);

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.attack;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_fix_attack;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_enemy_invest_skilleffect;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.targetForce);

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.investEnemy;
    ...
    private class FacilitySkillLogicEffectRangeEffectPlayerInvestSkilleffect : 
      BL.FacilitySkillLogicEffect
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_player_invest_skilleffect;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.myForce);

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.investPlayer;
    ...
    private class FacilitySkillLogicEffectRangeEffectEnemyRemoveSkilleffect : 
      BL.FacilitySkillLogicEffect
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_enemy_remove_skilleffect;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.targetForce);

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.removeEnemy;
    ...
    private class FacilitySkillLogicEffectRangeEffectPlayerRemoveSkilleffect : 
      BL.FacilitySkillLogicEffect
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_player_remove_skilleffect;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.myForce);

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.removePlayer;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_invest_land_tag;

      public override IEnumerable<BL.Panel> getPanelTargets() => this.getRangePanelTargets();

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.investLandTag;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_enemy_steal;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.targetForce);

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.stealEnemy;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_player_steal;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.myForce);

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.stealPlayer;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_enemy_provide;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.targetForce);

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.provideEnemy;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.range_effect_player_provide;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.myForce);

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.providePlayer;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_ratio_attack;

      public override bool checkMoveUnitInvoke(BL.UnitPosition upMove) => this.checkMoveUnitInvokeDistance(upMove, this.targetForce);
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_fix_attack;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_ratio_heal;

      public override bool checkMoveUnitInvoke(BL.UnitPosition upMove) => this.checkMoveUnitInvokeDistance(upMove, this.myForce);
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_fix_heal;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_enemy_invest_skilleffect;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_player_invest_skilleffect;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_enemy_remove_skilleffect;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_player_remove_skilleffect;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_invest_land_tag;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_enemy_steal;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_player_steal;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_enemy_provide;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap_player_provide;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_ratio_attack;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_fix_attack;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_ratio_heal;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_fix_heal;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_enemy_invest_skilleffect;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_player_invest_skilleffect;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_enemy_remove_skilleffect;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_player_remove_skilleffect;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_invest_land_tag;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_enemy_steal;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_player_steal;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_enemy_provide;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.trap2_player_provide;
    ...
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.jump_effect_immediate_attack;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.targetForce, (BL.UnitPosition) this.extData);
    ...
    private class FacilitySkillLogicEffectJumpEffectEnemyInvestSkilleffect : 
      BL.FacilitySkillLogicEffect
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.jump_effect_enemy_invest_skilleffect;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.targetForce, (BL.UnitPosition) this.extData);

      public override BL.FacilitySkillLogicEffect.Category category() => BL.FacilitySkillLogicEffect.Category.investEnemy;

      protected override void executeEffect() => this.executeInvestSkillEffect(this.pse.skillEffect != null ? this.pse.skillEffect.investUnit : (BL.Unit) null);
    ...
    private class FacilitySkillLogicEffectJumpEffectPlayerInvestSkilleffect : 
      BL.FacilitySkillLogicEffect
    {
      public override BattleskillEffectLogicEnum logicEnum() => BattleskillEffectLogicEnum.jump_effect_player_invest_skilleffect;

      public override IEnumerable<BL.UnitPosition> getTargets() => this.getRangeTargets(this.myForce, (BL.UnitPosition) this.extData);
...
label_22:
    ...
    public class UnitParameterCache : IDisposable
    {
      ...
      public UnitParameterCache(BL env_)
      {
        ...
        this.units.ForEach((System.Action<BL.UnitPosition>) (x => x.unit.enableCache()));
      }

      public void Dispose() => this.units.ForEach((System.Action<BL.UnitPosition>) (x => x.unit.Dispose()));
    }
    ...
    public class Unit : BL.ModelBase, IDisposable, BL.ISkillEffectListUnit
    {
      ...
      public static BL.Unit FromNetwork(int? nw, BL env) => nw.HasValue ? BL.UnitPosition.FromNetwork(nw, env).unit : (BL.Unit) null;
      ...
      public MasterDataTable.UnitJob job => this.mUnitJob ?? (this.mUnitJob = this.playerUnit.getJobData());

      public SkillMetamorphosis metamorphosis => BattleFuncs.getMetamorphosis((BL.ISkillEffectListUnit) this);
      ...
      public BL.Panel GetTargetPanel(BL env) => env.getFieldPanel(this.mAIMoveTargetY, this.mAIMoveTargetX);
      ...
      public Judgement.BattleParameter calcParameter() => Judgement.BattleParameter.FromBeUnit((BL.ISkillEffectListUnit) this);

      public void setParameter(Judgement.BattleParameter parameter) => this.mParameterCache = parameter;
      ...
      public bool checkTargetAttribute(BL.Unit.TargetAttribute ta) => ta == BL.Unit.TargetAttribute.all || (ta & BL.Unit.TargetAttribute.attack) == BL.Unit.TargetAttribute.attack && this.isAttackTarget || (ta & BL.Unit.TargetAttribute.heal) == BL.Unit.TargetAttribute.heal && this.isHealTarget;
      ...
      public bool hasMapSkill => ((IEnumerable<BL.Skill>) this.mArySkill).Any<BL.Skill>((Func<BL.Skill, bool>) (x => x.skill.checkEnableUnit((BL.ISkillEffectListUnit) this)));
              ...
              if (MasterData.BattleskillSkill.TryGetValue(mSkillUsesId, out battleskillSkill))
              {
                switch (battleskillSkill.skill_type)
                {
                  case BattleskillSkillType.command:
                    numArray[1] += this.mSkillUsesCounts[index];
                    break;
                  case BattleskillSkillType.release:
                    numArray[0] += this.mSkillUsesCounts[index];
                    break;
                  case BattleskillSkillType.SEA:
                    numArray[3] += this.mSkillUsesCounts[index];
                    break;
                }
              }
              ++index;
      ...
      public bool IsAllEquipUnit => ((IEnumerable<string>) Consts.GetInstance().ALL_GEAR_EQUIP_UNIT_IDS.Split(',')).Contains<string>(this.unit.ID.ToString());
      ...
      public bool IsDontEvasion => this.skillEffects.HasAilmentEffectLogic(BattleskillEffectLogicEnum.paralysis, BattleskillEffectLogicEnum.sleep);
      ...
      public bool IsJumping => this.skillEffects.Where(BattleskillEffectLogicEnum.jump).Any<BL.SkillEffect>();

      public bool IsDontUseCommand(BL.Skill skill) => BattleFuncs.isDontUseCommand((BL.ISkillEffectListUnit) this, skill);

      public bool IsDontUseOugi(BL.Skill skill) => BattleFuncs.isDontUseOugi((BL.ISkillEffectListUnit) this, skill);

      public bool IsDontUseSEA(BL.Skill skill) => BattleFuncs.isDontUseSEA((BL.ISkillEffectListUnit) this, skill);

      public bool IsDontUseSkill(int skill_id) => this.skillEffects.HasAilmentEffectLogic(BattleskillEffectLogicEnum.seal) && this.skillEffects.IsSealedSkill(skill_id);

      public bool IsDontUseSkillEffect(BL.SkillEffect effect) => this.skillEffects.HasAilmentEffectLogic(BattleskillEffectLogicEnum.seal) && this.skillEffects.IsSealedSkillEffect(effect);

      public bool CantChangeCurrent => this.skillEffects.HasAilmentEffectLogic(BattleskillEffectLogicEnum.can_not_change_current);
      ...
      public static bool operator ==(BL.Unit lhs, BL.Unit rhs) => (object) lhs == null ? (object) rhs == null : lhs.Equals(rhs);
      ...
      public bool IsRemainingCommandSkills => ((IEnumerable<BL.Skill>) this.skills).Any<BL.Skill>((Func<BL.Skill, bool>) (x =>
      {
    ...
    public class CurrentUnit : BL.ModelBase
    {
      ...
      public void setCurrentByPlayerInput(BL.Unit unit, BL env, System.Action<BL.UnitPosition> f) => this.canSetCurrent(unit, env, (System.Action<BL.UnitPosition>) (prevUp =>
      {
        this.setCurrent(unit, f, prevUp);
        this.IsPlayerInput = true;
      }));

      public void setCurrentWith(BL.Unit unit, BL env, System.Action<BL.UnitPosition> f) => this.canSetCurrent(unit, env, (System.Action<BL.UnitPosition>) (prevUp =>
      {
        this.setCurrent(unit, f, prevUp);
        this.IsPlayerInput = false;
      }));

      private void canSetCurrent(BL.Unit unit, BL env, System.Action<BL.UnitPosition> action)
      {
        if (!(this.mUnit != unit))
          return;
      ...
      private void setCurrent(BL.Unit unit, System.Action<BL.UnitPosition> f, BL.UnitPosition prevUp)
      {
        f(prevUp);
        this.mUnit = unit;
        ++this.revision;
    ...
    public class Weapon : BL.ModelBase
    {
      ...
      public IAttackMethod attackMethod
      {
        get => this.sAttackMethod.ID == 0 ? (IAttackMethod) null : this.attackMethod_ ?? (this.attackMethod_ = this.sAttackMethod.from == BL.Weapon.From.Normal ? MasterData.AttackMethod[this.sAttackMethod.ID].CreateInterface() : (this.sAttackMethod.from == BL.Weapon.From.Guest ? MasterData.BattleStageGuestAttackMethod[this.sAttackMethod.ID].CreateInterface() : MasterData.BattleStageEnemyAttackMethod[this.sAttackMethod.ID].CreateInterface()));
        private set
        {
          this.attackMethod_ = value;
          if (value == null)
      ...
      public GearGear assist => this.mAssistId == 0 ? (GearGear) null : MasterData.GearGear[this.mAssistId];
