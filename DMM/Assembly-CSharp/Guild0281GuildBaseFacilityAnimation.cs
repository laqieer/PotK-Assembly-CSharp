...
public class Guild0281GuildBaseFacilityAnimation : MonoBehaviour
{
  ...
  public void Initialize(GuildBaseType type, GuildImageCache guildImageCache)
  {
    ...
    if (pattern == null)
      return;
    UnityEngine.Sprite facilitySprite = guildImageCache.GetFacilitySprite(type);
    if (!((UnityEngine.Object) facilitySprite != (UnityEngine.Object) null))
      return;
    ui2Dsprite.GetComponent<UI2DSprite>().SetDimensions(facilitySprite.texture.width, facilitySprite.texture.height);
    ui2Dsprite.GetComponent<UI2DSprite>().sprite2D = facilitySprite;
    ...
    for (int index = 0; index < array.Length; ++index)
      this.SetAnimation(ui2Dsprite.gameObject, array[index].animPrefixSprite, array[index].animframe, array[index].animXpos, array[index].animYpos, index + 1, guildImageCache);
  }
