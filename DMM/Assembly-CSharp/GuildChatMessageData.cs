...
public class GuildChatMessageData
{
  ...
  public string GetFormattedSendTime()
  {
    ...
    if (totalDays > 1.0)
    {
      str = Mathf.FloorToInt((float) totalDays).ToString() + Consts.GetInstance().GUILD_CHAT_SEND_TIME_DAYS_AGO;
    }
    else
    {
      ...
      if (totalHours > 1.0)
      {
        str = Mathf.FloorToInt((float) totalHours).ToString() + Consts.GetInstance().GUILD_CHAT_SEND_TIME_HOURS_AGO;
      }
      else
      {
        ...
        if (totalMinutes > 1.0)
        {
          str = Mathf.FloorToInt((float) totalMinutes).ToString() + Consts.GetInstance().GUILD_CHAT_SEND_TIME_MINUTES_AGO;
        }
        else
        {
          double totalSeconds = timeSpan.TotalSeconds;
          if (totalSeconds < 10.0)
            str = Consts.GetInstance().GUILD_CHAT_SEND_TIME_JUST_NOW;
          else
            str = Consts.Format(Consts.GetInstance().GUILD_CHAT_SEND_TIME_10_SECONDS_AGO, (IDictionary) new Hashtable()
            {
              {
                (object) "seconds",
