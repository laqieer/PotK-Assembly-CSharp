...
public class Guide01142Menu : Unit00443Menu
{
  ...
  public IEnumerator onStartSceneAsync(GearGear gear, bool isDispNumber, int index = 0)
  {
    ...
    {
      gear
    }, (int[]) null, (int[]) null, isDispNumber, index);
    while (e.MoveNext())
    ...
    int index)
  {
    ...
    else
    {
      ...
      {
        guide01142Menu.detailObject[index1] = UnityEngine.Object.Instantiate<GameObject>(guide01142Menu.buguDetail);
      ...
      guide01142Menu.scroll.scrollView.transform.localPosition = new Vector3(-guide01142Menu.scroll.grid.cellWidth * (float) guide01142Menu.currentIndex, 0.0f, 0.0f);
      ...
      yield return (object) null;
      for (int index1 = 0; index1 < guide01142Menu.objectList.Count; ++index1)
        guide01142Menu.objectList[index1].transform.localPosition = end < guide01142Menu.gearList.Length - 1 ? new Vector3(guide01142Menu.scroll.grid.cellWidth * (float) (end + index1 + 1), 0.0f, 0.0f) : new Vector3(guide01142Menu.scroll.grid.cellWidth * (float) (start - (index1 + 1)), 0.0f, 0.0f);
  ...
  {
    Guide01142Menu guide01142Menu = this;
    GameObject go = guide01142Menu.objectList[0];
    Unit00443BuguList d = guide01142Menu.detailPrefabDict[go];
    guide01142Menu.objectList.RemoveAt(0);
    Vector3 gridPos = guide01142Menu.scroll.grid.transform.localPosition;
    go.transform.localPosition = new Vector3(guide01142Menu.scroll.grid.cellWidth * (float) gearIndex, 0.0f, 0.0f);
    yield return (object) null;
    IEnumerator e;
    if (guide01142Menu.quantityList != null && guide01142Menu.quantityList.Length >= gearIndex)
    {
      e = d.Init((Unit00443Menu) guide01142Menu, guide01142Menu.gearList[gearIndex], guide01142Menu.quantityList[gearIndex]);
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
    }
    ...
    {
      e = d.Init((Unit00443Menu) guide01142Menu, guide01142Menu.gearList[gearIndex], guide01142Menu.isDispNumber);
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
    }
    d.index = gearIndex;
    go.transform.localPosition = new Vector3(guide01142Menu.scroll.grid.cellWidth * (float) gearIndex, 0.0f, 0.0f);
    guide01142Menu.scroll.grid.transform.localPosition = gridPos;
    if (PerformanceConfig.GetInstance().IsLowMemory && guide01142Menu.chacheCount > 0)
    {
      GC.Collect();
  ...
  private IEnumerator RefreshInfo()
  {
    ...
    guide01142Menu.indicatorPage1.transform.localScale = Vector3.one;
    if (guide01142Menu.quantityList != null)
      guide01142Menu.SetQuanlity(guide01142Menu.quantityList[guide01142Menu.currentIndex]);
    else if ((UnityEngine.Object) guide01142Menu.NumPossession != (UnityEngine.Object) null)
  ...
  {
    if (!((UnityEngine.Object) this.NumPossession != (UnityEngine.Object) null))
      return;
    this.txtUnit.SetTextLocalize(num);
    this.NumPossession.SetActive(true);
  }

  public override void IbtnZoom() => Unit00446Scene.changeScene(true, this.gearList[this.currentIndex]);
