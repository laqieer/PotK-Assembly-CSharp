...
public class Bugu0052Menu : Bugu005ItemListMenuBase
{
  ...
  public override Persist<Persist.ItemSortAndFilterInfo> GetPersist() => Persist.bugu0052SortAndFilter;

  protected override List<PlayerItem> GetItemList() => ((IEnumerable<PlayerItem>) SMManager.Get<PlayerItem[]>()).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.isWeapon())).ToList<PlayerItem>();
