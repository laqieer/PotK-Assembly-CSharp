...
public class BattleFinalize : BattleMonoBehaviour
{
  ...
  public IEnumerator DoAPI()
  {
    ...
    foreach (BL.Unit unit1 in battleFinalize.env.core.playerUnits.value)
    {
      ...
      int num = skillUsesIds != null ? __nonvirtual (skillUsesIds.Count) : 0;
    ...
    battleFinalize.apiResult = WebAPI.BattleFinish(request, battleFinalize.env, new System.Action<WebAPI.Response.UserError>(battleFinalize.errorCallback));
  ...
  {
    List<WebAPI.Request.BattleFinish.GearResult> gearResultList = new List<WebAPI.Request.BattleFinish.GearResult>();
    foreach (BL.Unit unit in bl.playerUnits.value)
    {
      if (!unit.is_helper && !unit.playerUnit.is_guest)
      {
        ...
        if ((equippedGear = unit.playerUnit.equippedGear) != (PlayerItem) null)
          gearResultList.Add(new WebAPI.Request.BattleFinish.GearResult()
          {
