...
public class RaidBattleGuestSelectPopup : GuildGuestListBase
{
  ...
  public static int GetSelectedCategoryId() => RaidBattleGuestSelectPopup.currentElement != CommonElement.none ? (int) RaidBattleGuestSelectPopup.currentElement : 0;
  ...
  public IEnumerator Show()
  {
    ...
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = -1;
    guestSelectPopup.gameObject.SetActive(true);
    guestSelectPopup.InitScrollPosition();
    return false;
  }
  ...
  private IEnumerator GetElementHelpers()
  {
    ...
    if (!guestSelectPopup.elementDic.ContainsKey(RaidBattleGuestSelectPopup.currentElement))
    {
      ...
      Future<WebAPI.Response.GuildraidBattleHelper> ft = WebAPI.GuildraidBattleHelper(RaidBattleGuestSelectPopup.GetSelectedCategoryId(), new System.Action<WebAPI.Response.UserError>(guestSelectPopup.webErrorCallback));
  ...
  public IEnumerator InitFriendListScroll(GuildRaidPlayerHelpers[] helpers)
  {
    ...
    {
      string[] strArray = guestSelectPopup.usedUnitDic[RaidBattleGuestSelectPopup.currentElement];
      for (int index = 0; index < helpers.Length; ++index)
      {
        if (!((IEnumerable<string>) strArray).Contains<string>(helpers[index].target_player_id))
  ...
  protected override IEnumerator CreateScroll(int info_index, int bar_index)
  {
    ...
    if (bar_index < guestSelectPopup.allGuestBar.Count && info_index < guestSelectPopup.allGuestInfo.Count)
    {
      ...
      IEnumerator e = scrollParts.InitializeAsync(guestBarInfo.friend, GuildGuestSelectScroll.MODE.Atk, new System.Action<GvgCandidate>(guestSelectPopup.onGuestDecided), true);
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
