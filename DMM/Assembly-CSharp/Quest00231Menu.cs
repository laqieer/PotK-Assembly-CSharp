...
public class Quest00231Menu : BackButtonMenuBase
{
  ...
  private void ResetScrollObject(EventPointType type)
  {
    ...
    if (this.listType == EventPointType.all)
    {
      this.scrollContainerAll.gameObject.SetActive(true);
      this.scrollContainerGuild.gameObject.SetActive(false);
      this.scrollContainerPersonal.gameObject.SetActive(false);
      this.ibtnAll.SetColor(Color.white);
      this.ibtnPresonal.SetColor(Color.gray);
    }
    else if (this.listType == EventPointType.personal)
    {
      this.scrollContainerPersonal.gameObject.SetActive(true);
      ...
      {
        this.scrollContainerAll.gameObject.SetActive(false);
    ...
    else
    {
      if (this.listType != EventPointType.guild)
        return;
      this.scrollContainerGuild.gameObject.SetActive(true);
      this.scrollContainerAll.gameObject.SetActive(false);
      this.scrollContainerPersonal.gameObject.SetActive(false);
    ...
    PunitiveExpeditionEventReward[] rewardList)
  {
    ...
    if (this.isGuild)
    {
      this.ibtnGuild.transform.parent.gameObject.SetActive(true);
      ...
      if (PlayerAffiliation.Current.isGuildMember())
      {
        ...
        this.txtHuntingPoint.SetAnchor(this.txtHuntingPointGuild.gameObject);
        this.txtHuntingPointValue.leftAnchor.target = this.txtHuntingPointGuildValue.transform;
      }
      else
      {
    ...
    {
      this.ibtnAll.transform.parent.gameObject.SetActive(true);
      ...
      this.txtHuntingPoint.SetAnchor(this.txtHuntingPointAll.gameObject);
      this.txtHuntingPointValue.leftAnchor.target = this.txtHuntingPointAllValue.transform;
    }
    ...
    if ((UnityEngine.Object) this.marginPrefab == (UnityEngine.Object) null)
    {
    ...
    if (rewardListGroup != null && rewardListGroup.Count<IGrouping<Tuple<EventPointType, int>, PunitiveExpeditionEventReward>>() > 0)
    {
      ...
      this.scrollContainerPersonal.Scroll.transform.Clear();
      ...
      tmpScroll.Scroll.transform.Clear();
      tmpScroll.Reset();
