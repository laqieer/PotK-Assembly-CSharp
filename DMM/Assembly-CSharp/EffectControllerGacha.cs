...
public class EffectControllerGacha : EffectController
{
  ...
  private void SetGachaEffect() => this.gachaEffects = new Dictionary<GachaDirectionType, EffectControllerGacha.GachaEffect>()
  {
  ...
  private EffectControllerGacha.GachaEffect CurrentEffect => this.gachaEffects[this.CurrentDirectionType];
  ...
  private void SetBallEffect()
  {
    this.gachaEffects.ForEach<KeyValuePair<GachaDirectionType, EffectControllerGacha.GachaEffect>>((System.Action<KeyValuePair<GachaDirectionType, EffectControllerGacha.GachaEffect>>) (pair =>
    {
      if (pair.Key == this.CurrentDirectionType)
        return;
      pair.Value.AnimatorList.ForEach((System.Action<Animator>) (animator => animator.gameObject.SetActive(false)));
    }));
    this.gachaEffects[this.CurrentDirectionType].AnimatorList.ForEach((System.Action<Animator>) (animator => animator.gameObject.SetActive(true)));
  }
  ...
  private void InitBall()
  {
    ...
    {
      foreach (UnityEngine.Object @object in ball.transform)
  ...
  private void CreateDetail() => this.loadResourcesCoroutine = this.StartCoroutine(this.Detail.CreateResult(this.Menu.isPreview));
  ...
  public void StartProbabilityEffectShow(bool isFirst)
  {
    foreach (KeyValuePair<GachaDirectionType, EffectControllerGacha.GachaEffect> gachaEffect in this.gachaEffects)
      gachaEffect.Value.AnimatorList.ForEach((System.Action<Animator>) (animator => animator.gameObject.SetActive(false)));
    this.TurnOffProbabilityEffect();
    if (this.CurrentResultData.directionType == GachaDirectionType.pickup)
    {
      GachaDirectionType? changeDirectionType = this.CurrentResultData.changeDirectionType;
  ...
  {
    this.animatorBlueToRainbow.gameObject.SetActive(false);
    this.animatorYellowToRainbow.gameObject.SetActive(false);
    this.animatorRedToRainbow.gameObject.SetActive(false);
    this.animatorBlueToRed.gameObject.SetActive(false);
    this.animatorYellowToRed.gameObject.SetActive(false);
    this.animatorBackGroundChangeToRed.gameObject.SetActive(false);
    this.animatorCameraChangeToRed.gameObject.SetActive(false);
    this.animatorBackGroundChangeToRainbow.gameObject.SetActive(false);
    this.animatorCameraChangeToRainbow.gameObject.SetActive(false);
  }
