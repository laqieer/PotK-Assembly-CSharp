...
public class UnitSortAndFilter : SortAndFilter
{
  ...
  private System.Action<SortInfo> SortActionExt;
  public System.Action<SortInfo> SortFilterUnitNum;
    ...
    string spriteName)
  {
    ...
    gameObject.GetComponent<UnitSortAndFilterGroupButton>().Init(menu, type, id, title, spriteName);
    return gameObject;
  }
    ...
    if (groupLargeCategories != null)
    {
      foreach (UnitGroupLargeCategory groupLargeCategory in groupLargeCategories)
      {
        if (!string.IsNullOrEmpty(groupLargeCategory.name))
        {
          ...
          groupBtn.transform.SetParent(menu.groupLargeGrid.transform);
    ...
    if (groupSmallCategories != null)
    {
      foreach (UnitGroupSmallCategory groupSmallCategory in groupSmallCategories)
      {
        if (!string.IsNullOrEmpty(groupSmallCategory.name))
        {
          ...
          groupBtn.transform.SetParent(menu.groupSmallGrid.transform);
          groupBtn.transform.localScale = Vector3.one;
          groupBtn.transform.localPosition = Vector3.zero;
        }
      }
    ...
    if (clothingCategories != null)
    {
      foreach (UnitGroupClothingCategory clothingCategory in clothingCategories)
      {
        if (!string.IsNullOrEmpty(clothingCategory.name))
        {
          ...
          groupBtn.transform.SetParent(menu.groupClothingGrid.transform);
          groupBtn.transform.localScale = Vector3.one;
          groupBtn.transform.localPosition = Vector3.zero;
        }
      }
    ...
    if (generationCategories != null)
    {
      foreach (UnitGroupGenerationCategory generationCategory in generationCategories)
      {
        if (!string.IsNullOrEmpty(generationCategory.name))
        {
          ...
          groupBtn.transform.SetParent(menu.groupGenerationGrid.transform);
          groupBtn.transform.localScale = Vector3.one;
          groupBtn.transform.localPosition = Vector3.zero;
        }
      }
  ...
  {
    this.selectedGroupIDs[UnitGroupHead.group_large].Where<int>((Func<int, bool>) (x => !this.menu.AllGroupIDs[UnitGroupHead.group_large].Contains(x))).ForEach<int>((System.Action<int>) (x => this.selectedGroupIDs[UnitGroupHead.group_large].Remove(x)));
    this.selectedGroupIDs[UnitGroupHead.group_small].Where<int>((Func<int, bool>) (x => !this.menu.AllGroupIDs[UnitGroupHead.group_small].Contains(x))).ForEach<int>((System.Action<int>) (x => this.selectedGroupIDs[UnitGroupHead.group_small].Remove(x)));
    this.selectedGroupIDs[UnitGroupHead.group_clothing].Where<int>((Func<int, bool>) (x => !this.menu.AllGroupIDs[UnitGroupHead.group_clothing].Contains(x))).ForEach<int>((System.Action<int>) (x => this.selectedGroupIDs[UnitGroupHead.group_clothing].Remove(x)));
    this.selectedGroupIDs[UnitGroupHead.group_generation].Where<int>((Func<int, bool>) (x => !this.menu.AllGroupIDs[UnitGroupHead.group_generation].Contains(x))).ForEach<int>((System.Action<int>) (x => this.selectedGroupIDs[UnitGroupHead.group_generation].Remove(x)));
  }
  ...
  public IEnumerator Initialize(
    System.Action<SortInfo> SortAction,
    ...
    bool isFriendSupport = false)
  {
    ...
    {
      this.battleFirstBtn.gameObject.SetActive(true);
      this.towerEntryFirseBtn.gameObject.SetActive(true);
    ...
    {
      this.battleFirstBtn.gameObject.SetActive(false);
      this.towerEntryFirseBtn.gameObject.SetActive(false);
    }
  }
  ...
  public void SetValueWindow()
  {
    ...
    {
      if (!((UnityEngine.Object) filterBtn == (UnityEngine.Object) null))
      {
