...
public class Raid032BattleResultBossView : MonoBehaviour
{
  ...
  {
    Future<UnityEngine.Sprite> future = bossUnit.unit.LoadSpriteLarge();
    ...
    {
      Future<UnityEngine.Sprite> prefabBgF = new ResourceObject(guildRaidPeriod.bg_path).Load<UnityEngine.Sprite>();
      e = prefabBgF.Wait();
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      this.background.sprite = prefabBgF.Result;
      this.background.transform.localScale = new Vector3(1f, 1f);
      prefabBgF = (Future<UnityEngine.Sprite>) null;
    }
    this.dir_camera.transform.position = Singleton<CommonRoot>.GetInstance().getCamera().transform.position;
  }
