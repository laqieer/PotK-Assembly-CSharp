...
public class GuildChatManager : BackButtonMenuBase
{
  ...
  public GuildChatManager.GuildChatStatus GetCurrentGuildChatStatus() => this.currentGuildChatStatus;

  public bool GetGuildChatPaused() => this.isGuildChatPaused;

  public bool isDetailViewOpened => this.GetCurrentGuildChatStatus() == GuildChatManager.GuildChatStatus.DetailedView;
  ...
  private void Start()
  {
    ...
    this.chatMessageInput.transform.localPosition = (Vector3) new Vector2(176f, 80f);
    this.chatMessageInput.GetComponent<UIWidget>().SetDimensions(416, 110);
    this.chatMessageLabel.maxLineCount = 3;
    this.chatMessageLabel.overflowMethod = UILabel.Overflow.ShrinkContent;
    this.chatMessageLabel.GetComponent<UIWidget>().SetDimensions(382, 99);
    this.sendMessageButton.transform.localPosition = new Vector3(252f, 77f, 0.0f);
    this.stampOpenButton.transform.localPosition = new Vector3(-279f, 77f, 0.0f);
  }

  private new void Update()
  {
    base.Update();
    if (!this.sendMessageButton.isEnabled || !Input.GetKeyDown(KeyCode.KeypadEnter) && !Input.GetKey(KeyCode.Return))
      return;
  ...
  public void OpenDetailedChat()
  {
    ...
    if ((UnityEngine.Object) Singleton<NGSceneManager>.GetInstance().sceneBase != (UnityEngine.Object) null && (UnityEngine.Object) Singleton<NGSceneManager>.GetInstance().sceneBase.GetComponent<NGMenuBase>() != (UnityEngine.Object) null && Singleton<NGSceneManager>.GetInstance().sceneBase.GetComponent<NGMenuBase>().IsPushAndSet())
      return;
  ...
  public void CloseDetailedChat()
  {
    ...
    else
    {
      ...
      if ((UnityEngine.Object) Singleton<NGSceneManager>.GetInstance().sceneBase != (UnityEngine.Object) null && Singleton<NGSceneManager>.GetInstance().sceneBase.currentSceneGuildChatDisplayingStatus != NGSceneBase.GuildChatDisplayingStatus.Closed)
  ...
  public void OnPullDownMessageListFromTop() => this.updateEarliestMessageCoroutine = this.StartCoroutine(this.UpdateEarlierMessage());
  ...
  public void OnInputChatMessageChange() => this.sendMessageButton.isEnabled = !this.chatMessageInput.value.isEmptyOrWhitespace();
  ...
  private IEnumerator SendMessage()
  {
    ...
    if (!(log_text == ""))
    {
      ...
      Future<WebAPI.Response.GuildlogWrite> future = WebAPI.SilentGuildlogWrite(log_text, new System.Action<WebAPI.Response.UserError>(guildChatManager.SendMessageErrorCallback));
  ...
  private IEnumerator OpenNGWordDialog()
  {
    ...
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    Singleton<PopupManager>.GetInstance().open(prefab.Result).GetComponent<Guild028NgWordPopup>().Initialize((System.Action) (() => {}));
  }
  ...
  private IEnumerator SendStampCoroutine(int stampID)
  {
    GuildChatManager guildChatManager = this;
    Singleton<CommonRoot>.GetInstance().loadingMode = 1;
    Future<WebAPI.Response.GuildlogSendStamp> future = WebAPI.SilentGuildlogSendStamp(stampID, new System.Action<WebAPI.Response.UserError>(guildChatManager.SendStampErrorCallback));
    IEnumerator e = future.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    ...
    if (PlayerAffiliation.Current.isGuildMember())
    {
      while (true)
      {
        ...
        do
        {
          ...
          Future<WebAPI.Response.GuildlogAutoupdate> future = WebAPI.SilentGuildlogAutoupdate(num1.ToString(), new System.Action<WebAPI.Response.UserError>(guildChatManager.\u003CUpdateLatestMessageRepeating\u003Eb__107_0));
          e = future.Wait();
          while (e.MoveNext())
            yield return e.Current;
          e = (IEnumerator) null;
  ...
  private IEnumerator UpdateLatestMessageNow()
  {
    ...
    do
    {
      ...
      Singleton<CommonRoot>.GetInstance().loadingMode = 2;
      // ISSUE: reference to a compiler-generated method
      Future<WebAPI.Response.GuildlogAutoupdate> future = WebAPI.SilentGuildlogAutoupdate(num1.ToString(), new System.Action<WebAPI.Response.UserError>(guildChatManager.\u003CUpdateLatestMessageNow\u003Eb__111_0));
      IEnumerator e = future.Wait();
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
  ...
  private IEnumerator UpdateEarlierMessage()
  {
    ...
    if (!guildChatManager.isUpdatingMessage && PlayerAffiliation.Current.isGuildMember())
    {
      ...
      Singleton<CommonRoot>.GetInstance().loadingMode = 2;
      // ISSUE: reference to a compiler-generated method
      Future<WebAPI.Response.GuildlogShowPast> future = WebAPI.SilentGuildlogShowPast(num1.ToString(), new System.Action<WebAPI.Response.UserError>(guildChatManager.\u003CUpdateEarlierMessage\u003Eb__112_0));
      IEnumerator e = future.Wait();
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
  ...
  private IEnumerator OpenGuildChatCoroutine()
  {
    ...
    if (guildChatManager.currentGuildChatStatus == GuildChatManager.GuildChatStatus.SimpleView)
    {
      ...
      guildChatManager.gameObject.SetActive(true);
  ...
  public void CloseGuildChat()
  {
    ...
    else
    {
      ...
      this.isUpdatingMessage = false;
      if ((UnityEngine.Object) this.simpleListController != (UnityEngine.Object) null)
  ...
  protected new void StopCoroutine(Coroutine co) => Singleton<NGSceneManager>.GetInstance().StopCoroutine(co);
  ...
  private IEnumerator PauseAndHideCoroutine()
  {
    ...
    guildChatManager.isUpdatingMessage = false;
    guildChatManager.gameObject.SetActive(false);
  }
    ...
    List<GuildChatMessageData> dataList)
  {
    ...
    if (dataList != null)
    {
      ...
      {
        case GuildChatManager.GuildChatMessageFilterType.All:
          guildChatMessageDataList = dataList;
          break;
        case GuildChatManager.GuildChatMessageFilterType.ChatOnly:
          for (int index = 0; index < dataList.Count; ++index)
          {
            if (dataList[index].messageType == GuildChatMessageData.GuildChatMessageType.MemberChat || dataList[index].messageType == GuildChatMessageData.GuildChatMessageType.PlayerChat || (dataList[index].messageType == GuildChatMessageData.GuildChatMessageType.MemberStamp || dataList[index].messageType == GuildChatMessageData.GuildChatMessageType.PlayerStamp))
              guildChatMessageDataList.Add(dataList[index]);
          }
          break;
        case GuildChatManager.GuildChatMessageFilterType.GuildRaidOnly:
          for (int index = 0; index < dataList.Count; ++index)
    ...
    return guildChatMessageDataList;
  }
    ...
    List<GuildChatMessageData> deletedOldDataList = null)
  {
    ...
    int num1 = addedNewDataList == null ? 0 : addedNewDataList.Count;
    int num2 = addedOldDataList == null ? 0 : addedOldDataList.Count;
    int num3 = deletedOldDataList == null ? 0 : deletedOldDataList.Count;
    if (num1 <= 0 && num2 - num3 <= 0)
      return;
    ...
    GuildChatManager.GuildChatMessageFilterType filterType)
  {
    for (int index = 0; index < this.messageFilterTypeIdleButtons.Length; ++index)
    {
      if (filterType == (GuildChatManager.GuildChatMessageFilterType) index)
        this.messageFilterTypeIdleButtons[index].gameObject.SetActive(false);
      else
        this.messageFilterTypeIdleButtons[index].gameObject.SetActive(true);
    }
    for (int index = 0; index < this.messageFilterTypeHighlightButtons.Length; ++index)
    {
      if (filterType == (GuildChatManager.GuildChatMessageFilterType) index)
        this.messageFilterTypeHighlightButtons[index].gameObject.SetActive(true);
      else
        this.messageFilterTypeHighlightButtons[index].gameObject.SetActive(false);
    }
  }
  ...
  public void StartMaintenanceMode()
  {
    ...
    this.simpleListController.gameObject.SetActive(false);
  }

  public void EndMaintenanceMode()
  {
    this.maintenanceBlock.SetActive(false);
    this.simpleListController.gameObject.SetActive(true);
  }
  ...
  public void SetSprite(UI2DSprite sprite, int spriteID)
  {
    if (spriteID != 0)
    {
      ...
      sprite.sprite2D = (UnityEngine.Sprite) null;
  }

  private IEnumerator SetSpriteFromResourceSchedule(UI2DSprite sprite, int spriteID)
  {
    Future<UnityEngine.Sprite> spriteF = Singleton<ResourceManager>.GetInstance().Load<UnityEngine.Sprite>(string.Format("AssetBundle/Resources/Units/{0}/2D/c_thum", (object) spriteID));
  ...
  public void SetStampSprite(UISprite targetSprite, int stampID)
  {
    ...
    if (guildStamp != null)
    {
      ...
      if (GuildChatManager.stampAtlasCache.TryGetValue(guildStamp.groupID.ID, out uiAtlas))
      {
        ...
        UIButton component = targetSprite.GetComponent<UIButton>();
    ...
    UISprite targetSprite)
  {
    ...
    targetSprite.spriteName = str;
    UIButton component2 = targetSprite.GetComponent<UIButton>();
    ...
    EventDelegate.Callback finishCallback)
  {
    ...
    UITweener[] componentsInChildren = this.gameObject.GetComponentsInChildren<UITweener>();
  ...
  public void OpenMemberLogDialog(string playerID) => this.StartCoroutine(this.OpenMemberLogCoroutine(playerID));
  ...
  private void LogGuildChatStatusError() => Debug.Log((object) ("<color=red>Status not right: " + this.currentGuildChatStatus.ToString() + "</color>"));
  ...
  {
    public UnityEngine.Sprite unitIconSprite;

    public SpriteCache(UnityEngine.Sprite sprite) => this.unitIconSprite = sprite;
  }
