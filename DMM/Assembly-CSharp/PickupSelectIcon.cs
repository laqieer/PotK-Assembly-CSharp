...
public class PickupSelectIcon : MonoBehaviour
{
  ...
  [SerializeField]
  private UnityEngine.Sprite backGearBody_;
  [SerializeField]
  private UnityEngine.Sprite backReisou_;
  ...
  public static Future<GameObject> createLoader() => new ResourceObject("Prefabs/UnitIcon/UnitItem_Icon").Load<GameObject>();
  ...
  public PickupSelectIcon.Type type => !this.current_.HasValue ? PickupSelectIcon.Type.Blank : this.current_.Value;
  ...
  public void initialize(GearGear gear, bool bGearBody, int? quantity = null)
  {
    ...
    UnityEngine.Sprite sprite = 6 == (int) current.GetValueOrDefault() & current.HasValue ? this.backGearBody_ : (gear.isReisou() ? this.backReisou_ : (gear.hasSpecificationOfEquipmentUnits ? this.backSpecificationOfEquipmentUnits_[gear.customize_flag] : this.back_[gear.customize_flag]));
    back.sprite2D = sprite;
    if (!gear.isReisou() && gear.hasAttackClass)
    {
      this.attackClass_.gameObject.SetActive(true);
      this.attackClass_.Initialize(gear.gearClassification.attack_classification, gear.attachedElement);
    }
    else
      this.attackClass_.gameObject.SetActive(false);
  ...
  private void setType(PickupSelectIcon.Type type)
  {
    ...
    for (int index = 0; index < this.molds_.Length; ++index)
    {
      ...
      if ((bool) (UnityEngine.Object) mold.top)
        mold.top.SetActive(mold.type == moldType);
      if (this.molds_[index].type == moldType)
        this.mold_ = mold;
    }
    this.current_ = new PickupSelectIcon.Type?(type);
    this.objOptions_.SetActive((uint) moldType > 0U);
    this.kind_.gameObject.SetActive(type != PickupSelectIcon.Type.GearMaterial);
  }
  ...
  private IEnumerator doLoadIcon<T>(PickupSelectIcon.Mold mold, T data) where T : class
  {
    ...
    if ((object) obj != null)
    {
      ...
      {
        case UnitUnit unit:
          e = UnitIcon.LoadSprite((System.Action<UnityEngine.Sprite>) (s => mold.icon.sprite2D = s), unit);
          while (e.MoveNext())
            yield return e.Current;
          e = (IEnumerator) null;
          break;
        case GearGear gear:
          e = ItemIcon.LoadSprite(gear, (System.Action<UnityEngine.Sprite>) (s => mold.icon.sprite2D = s));
          while (e.MoveNext())
            yield return e.Current;
          e = (IEnumerator) null;
          break;
  ...
  private void setGearKindIcon(GearKindEnum kind, CommonElement element) => this.kind_.sprite2D = GearKindIcon.LoadSprite(kind, element);
  ...
  private UITweener[] tweeners => this.tweeners_ ?? (this.tweeners_ = this.GetComponentsInChildren<UITweener>(true));
  ...
  private void setQuantity(int? quantity = null) => this.quantity_.set(this.spriteNumbers_, quantity);
  ...
  private class SpriteArray
  {
    ...
    public UnityEngine.Sprite this[int i] => i < 0 || i >= this.sprites.Length ? this.sprites[0] : this.sprites[i];
  }
    ...
    public void set(PickupSelectIcon.SpriteArray nums, int? quantity = null)
    {
      ...
      else
      {
        ...
        if (!this.offsetSymbol_.HasValue)
          this.offsetSymbol_ = new Vector3?(this.posSymbol.localPosition - this.digits[0].transform.localPosition);
        int num1 = 1;
        for (int index = 0; index < this.digits.Length; ++index)
          num1 *= 10;
        int b = num1 - 1;
        string str = Mathf.Min(quantity.Value, b).ToString();
        int num2 = str.Length - 1;
        int num3 = this.isRightAligned ? 0 : this.digits.Length - num2 - 1;
        for (int index1 = 0; index1 < str.Length; ++index1)
        {
          UnityEngine.Sprite num4 = nums[(int) str[num2 - index1] - 48];
          int index2 = index1 + num3;
          this.digits[index2].sprite2D = num4;
          this.digits[index2].width = num4.texture.width;
          this.digits[index2].gameObject.SetActive(true);
        }
        if (this.isRightAligned)
        {
          for (int index = num2 + 1; index < this.digits.Length; ++index)
            this.digits[index].gameObject.SetActive(false);
        }
        else
        {
        ...
        this.posSymbol.localPosition = this.digits[this.isRightAligned ? num2 : num3].transform.localPosition + this.offsetSymbol_.Value;
      }
    }
  }
