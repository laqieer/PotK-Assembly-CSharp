...
public class Unit0046CustomDeckTutorial : MonoBehaviour
{
  ...
  private IEnumerator doInitialize(bool bDemoMode = false)
  {
    ...
    switch (customDeckTutorial.status_)
    {
      case Unit0046CustomDeckTutorial.Status.Unlocked:
        customDeckTutorial.btnEditCustomDeck_.isEnabled = true;
        break;
      case Unit0046CustomDeckTutorial.Status.Locked:
        customDeckTutorial.btnEditCustomDeck_.isEnabled = false;
        customDeckTutorial.animator_.enabled = false;
  ...
  private void onClickedLock() => Singleton<NGMessageUI>.GetInstance().SetMessageByPosType(Consts.GetInstance().CUSTOMDECK_UNLOCK_EDITOR, NGMessageUI.PosType.LITTLEUP);
  ...
  private IEnumerator doPlayUnlockAndTutorial()
  {
    ...
    popup.GetComponent<UIRect>().alpha = 0.0f;
    IEnumerator e = popup.Initialize((System.Action) null);
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    popupManager.startOpenAnime(popup.gameObject);
  }
