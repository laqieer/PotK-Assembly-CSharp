...
public class GUISelectBox<T>
{
  ...
  public T Render(System.Action<T> changeCallback = null) => changeCallback == null ? this.RenderWithIndex() : this.RenderWithIndex((System.Action<int, T>) ((_, val) => changeCallback(val)));
  ...
  public T RenderWithIndex(System.Action<int, T> changeCallback = null)
  {
    if (this.toggle)
    {
      ...
      if (GUILayout.Button("back", GUILayout.Width(120f)))
        this.toggle = false;
      string str = GUILayout.TextArea(this.finder, 10);
      if (str != this.finder)
      {
        this.finder = str;
        this.selectedIndex = 0;
      ...
      this.toggle = GUILayout.Toggle(this.toggle, this.title + " " + this.labels[this.selectedIndex]);
    return this.Value();
  }

  public T Value() => this.options.Length < 1 ? this.orgOptions[0] : this.options[this.selectedIndex];
