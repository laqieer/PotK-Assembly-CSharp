...
public class CorpsQuestManualMenu : ManualMenuBase
{
  ...
  protected override string getTitle() => Array.Find<CorpsHowto>(MasterData.CorpsHowtoList, (Predicate<CorpsHowto>) (x => x.kind == 1 && x.setting_CorpsSetting == this.settingID_))?.body ?? string.Empty;

  protected override ManualMenuBase.BodyParam[] getBodies() => ((IEnumerable<CorpsHowto>) MasterData.CorpsHowtoList).Where<CorpsHowto>((Func<CorpsHowto, bool>) (x => x.kind >= 2 && x.setting_CorpsSetting == this.settingID_)).Select<CorpsHowto, ManualMenuBase.BodyParam>((Func<CorpsHowto, ManualMenuBase.BodyParam>) (y => new ManualMenuBase.BodyParam()
  {
    body = y.body,
