...
public class Bootstrap : MonoBehaviour
{
  ...
  private static IEnumerator _InitializeApplication()
  {
    ...
    IEnumerator e = WebAPI.RpcReviewEnvConnect((System.Action<bool[]>) (ret =>
    {
    ...
    e = (IEnumerator) null;
    UnityErrorLogSender.Instance.Callback = new System.Action<CustomHeaders, Dictionary<string, object>, Dictionary<string, object>, Dictionary<string, object>>(GsccBridge.UnityErrorLogCallback);
    LocalLogUtil.Init();
    ...
    if (SDK.hasError)
      ModalWindow.Show("エラー", "アプリを直接起動することはできません。", (System.Action) (() => Application.Quit()));
    else
  ...
  private static IEnumerator InitPurchase()
  {
    ...
    Bootstrap.productIDs = ((IEnumerable<CoinProduct>) MasterData.CoinProductList).Where<CoinProduct>((Func<CoinProduct, bool>) (x => x.platform == Gsc.Auth.Device.Platform && (uint) x.type > 0U)).Select<CoinProduct, string>((Func<CoinProduct, string>) (x => x.product_id)).ToArray<string>();
    if (Bootstrap.productIDs.Length == 0)
    {
      string platformName = "";
      platformName = "windows";
      Bootstrap.productIDs = ((IEnumerable<CoinProduct>) MasterData.CoinProductList).Where<CoinProduct>((Func<CoinProduct, bool>) (x => x.platform == platformName && (uint) x.type > 0U)).Select<CoinProduct, string>((Func<CoinProduct, string>) (x => x.product_id)).ToArray<string>();
    }
    NativeRootObject.CreateInstance();
    PurchaseFlow.Init(Bootstrap.productIDs, (IPurchaseGlobalListener) new PurchaseListener());
  }

  private static void RunOnUiThread() => PurchaseFlow.Init(Bootstrap.productIDs, (IPurchaseGlobalListener) new PurchaseListener());
}
