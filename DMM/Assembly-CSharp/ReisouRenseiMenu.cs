...
public class ReisouRenseiMenu : BackButtonMenuBase
{
  ...
  public IEnumerator onInitAsync()
  {
    ...
    {
      ReisouRenseiIcon component = ReisouRenseiMenu.reisouIconPrefab.Clone(this.grid.transform).GetComponent<ReisouRenseiIcon>();
    ...
    bool canDriling = true)
  {
    ...
    this.slider.foregroundWidget.GetComponent<UISprite>().enabled = false;
    this.uiDrag.enabled = false;
    ...
    if (materials != null)
    {
      ...
      this.uiDrag.enabled = materials.Count > 0;
      this.slider.foregroundWidget.GetComponent<UISprite>().enabled = materials.Count > 0;
    ...
    Transform child = this.uiDrag.transform.GetChild(0);
    if ((UnityEngine.Object) child != (UnityEngine.Object) null)
    {
      Vector4 baseClipRegion = child.GetComponent<UIPanel>().baseClipRegion;
      child.GetComponent<UIPanel>().baseClipRegion = new Vector4(baseClipRegion.x, 0.0f, baseClipRegion.z, baseClipRegion.w);
    }
  }
  ...
  private IEnumerator SetReisouInfo()
  {
    ...
    else
    {
      ...
      foreach (Component component in reisouRenseiMenu.DynReisouIconForBase.transform)
        UnityEngine.Object.Destroy((UnityEngine.Object) component.gameObject);
      if ((UnityEngine.Object) reisouRenseiMenu.reisouIcon != (UnityEngine.Object) null)
        UnityEngine.Object.Destroy((UnityEngine.Object) reisouRenseiMenu.reisouIcon);
      ...
      else
      {
        ...
        reisouRenseiMenu.reisouIcon.onClick = (System.Action<ItemIcon>) (x => this.OpenReisouDetailPopup(dispInfoTemp, dispReisouInfo, new System.Action(this.cbRemoveReisou)));
        reisouRenseiMenu.EnableLongPressEventReisou(dispInfoTemp, reisouRenseiMenu.reisouIcon, dispReisouInfo, new System.Action(reisouRenseiMenu.cbRemoveReisou));
    ...
    ReisouRenseiMenu.ReisouExpGauge reisouExpGauge)
  {
    ...
    int width1 = reisouExpGauge.SlcReisouGauge.width;
    float x1 = (float) baseReisouItem.gear_exp / (float) (baseReisouItem.gear_exp + baseReisouItem.gear_exp_next);
    if ((double) x1 == 0.0 || baseReisouItem.gear_exp + baseReisouItem.gear_exp_next == 0)
    {
      reisouExpGauge.SlcReisouGauge.gameObject.SetActive(false);
    }
    else
    {
      reisouExpGauge.SlcReisouGauge.gameObject.SetActive(true);
      if ((double) x1 > 1.0)
        x1 = 1f;
      reisouExpGauge.SlcReisouGauge.transform.localScale = new Vector3(x1, 1f, 1f);
    }
    if (dispReisouItem.gear_exp > baseReisouItem.gear_exp || dispReisouItem.gear_level > baseReisouItem.gear_level)
    {
      int width2 = reisouExpGauge.SlcReisouGaugeAfter.width;
      float x2 = (float) dispReisouItem.gear_exp / (float) (dispReisouItem.gear_exp + dispReisouItem.gear_exp_next);
      if (dispReisouItem.gear_level > baseReisouItem.gear_level)
        x2 = 1f;
      if ((double) x2 == 0.0)
      {
        reisouExpGauge.SlcReisouGaugeAfter.gameObject.SetActive(false);
      }
      else
      {
        reisouExpGauge.SlcReisouGaugeAfter.gameObject.SetActive(true);
        reisouExpGauge.SlcReisouGaugeAfter.transform.localScale = new Vector3(x2, 1f, 1f);
      }
    }
    else
      reisouExpGauge.SlcReisouGaugeAfter.gameObject.SetActive(false);
  ...
  {
    icon.gear.button.gameObject.SetActive(true);
  ...
  public void OpenReisouDetailPopup(GameCore.ItemInfo item, PlayerItem playerItem, System.Action removeCallback = null)
  {
    ...
    System.Action removeCallback = null)
  {
    ...
    else
    {
      ...
      if (item.gear.isMythologyReisou())
      {
        ...
        if ((UnityEngine.Object) ReisouRenseiMenu.reisouPopupDualSkillPrefab == (UnityEngine.Object) null)
        {
      ...
      {
        if ((UnityEngine.Object) ReisouRenseiMenu.reisouPopupPrefab == (UnityEngine.Object) null)
        {
  ...
  private void SetRarity(GearGear gear, UI2DSprite dstSprite)
  {
    ...
    if (gear.rarity.index <= 0)
      return;
    UnityEngine.Sprite sprite = Resources.Load<UnityEngine.Sprite>(ReisouRenseiMenu.spriteNameTBL[gear.rarity.index - 1]);
    ...
    ReisouRankIncr rankIncr)
  {
    ...
    if (num3 > 0 && num3 != 100)
    {
      double num2 = (double) num3 / 100.0;
      string str = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_MAG_POW.F((object) num2);
      paramTextQueue.Enqueue(str);
    }
    if (rankIncr.physical_defense > 0 && rankIncr.physical_defense != 100)
    {
      double num2 = (double) rankIncr.physical_defense / 100.0;
      string str = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_PHY_DEF.F((object) num2);
      paramTextQueue.Enqueue(str);
    }
    if (rankIncr.magic_defense > 0 && rankIncr.magic_defense != 100)
    {
      double num2 = (double) rankIncr.magic_defense / 100.0;
      string str = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_MAG_DEF.F((object) num2);
      paramTextQueue.Enqueue(str);
    }
    if (rankIncr.hit > 0 && rankIncr.hit != 100)
    {
      double num2 = (double) rankIncr.hit / 100.0;
      string str = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_HIT.F((object) num2);
      paramTextQueue.Enqueue(str);
    }
    if (rankIncr.critical > 0 && rankIncr.critical != 100)
    {
      double num2 = (double) rankIncr.critical / 100.0;
      string str = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_CRITICAL.F((object) num2);
      paramTextQueue.Enqueue(str);
    }
    if (rankIncr.evasion <= 0 || rankIncr.evasion == 100)
      return;
    double num4 = (double) rankIncr.evasion / 100.0;
    string str1 = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_EVASION.F((object) num4);
    paramTextQueue.Enqueue(str1);
  }
    ...
    GearGear holyReisou)
  {
    ...
    {
      double num2 = (double) num3 / 100.0;
      string str = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_MAG_POW_FOR_MYTHOLOGY.F((object) holyReisou.name, (object) num2);
      paramTextQueue.Enqueue(str);
    }
    if (rankIncr.physical_defense > 0 && rankIncr.physical_defense != 100)
    {
      double num2 = (double) rankIncr.physical_defense / 100.0;
      string str = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_PHY_DEF_FOR_MYTHOLOGY.F((object) holyReisou.name, (object) num2);
      paramTextQueue.Enqueue(str);
    }
    if (rankIncr.magic_defense > 0 && rankIncr.magic_defense != 100)
    {
      double num2 = (double) rankIncr.magic_defense / 100.0;
      string str = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_MAG_DEF_FOR_MYTHOLOGY.F((object) holyReisou.name, (object) num2);
      paramTextQueue.Enqueue(str);
    }
    if (rankIncr.hit > 0 && rankIncr.hit != 100)
    {
      double num2 = (double) rankIncr.hit / 100.0;
      string str = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_HIT_FOR_MYTHOLOGY.F((object) holyReisou.name, (object) num2);
      paramTextQueue.Enqueue(str);
    }
    if (rankIncr.critical > 0 && rankIncr.critical != 100)
    {
      double num2 = (double) rankIncr.critical / 100.0;
      string str = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_CRITICAL_FOR_MYTHOLOGY.F((object) holyReisou.name, (object) num2);
      paramTextQueue.Enqueue(str);
    }
    if (rankIncr.evasion <= 0 || rankIncr.evasion == 100)
      return;
    double num4 = (double) rankIncr.evasion / 100.0;
    string str1 = Consts.GetInstance().UNIT_0044_CHAOS_REISOU_EVASION_FOR_MYTHOLOGY.F((object) holyReisou.name, (object) num4);
    paramTextQueue.Enqueue(str1);
  }
  ...
  private IEnumerator OpenMaterialCheckPopup()
  {
    ...
    IEnumerator e = obj.GetComponent<Popup00591Menu>().Init((GameCore.ItemInfo) null, reisouRenseiMenu.m_After, reisouRenseiMenu.reisouInfo, reisouRenseiMenu.reisouInfoAfter, reisouRenseiMenu.m_Materials, new System.Action(reisouRenseiMenu.\u003COpenMaterialCheckPopup\u003Eb__63_0), reisouRenseiMenu.m_ItemIconPrefab);
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
