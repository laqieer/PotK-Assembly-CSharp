...
public class BootLoader : MonoBehaviour
{
  ...
  private IEnumerator LunchCore()
  {
    ...
    if (boot.is_end_service)
    {
      ...
      ModalWindow.Show("", message, (System.Action) (() => StartScript.Restart()));
    }
    else
    {
        ...
        else
        {
          ...
          try
          {
            if (Persist.battleEnvironment.Exists && Persist.battleEnvironment.Data.core != null)
              num = Persist.battleEnvironment.Data.core.continueCount;
            num = Mathf.Max(num, WebAPI.LastPlayerBoot.continue_count);
          }
          catch (Exception ex)
          {
            Debug.LogException(ex);
          }
          e = WebAPI.BattleRetire(num).Wait();
          while (e.MoveNext())
            yield return e.Current;
          e = (IEnumerator) null;
  ...
  private class BootSequence
  {
    ...
    public IEnumerator Wait()
    {
      ...
      else if (this.boot.player_during_pvp)
      {
        do
        {
          NGGameDataManager.SeaChangeFlag = false;
          this.status = BootLoader.BootSequence.SequenceStatus.Wait;
          ModalWindow.ShowYesNo(c.boot_continue_pvp_title, c.boot_continue_pvp_text, (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Break), (System.Action) (() => ModalWindow.ShowYesNo(c.boot_continue_pvp_exit_confirm_title, c.boot_continue_pvp_exit_confirm_text, (System.Action) (() => ModalWindow.Show(c.boot_continue_pvp_exit_done_title, c.boot_continue_pvp_exit_done_text, (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Noop))), (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Confirm))));
          while (this.status == BootLoader.BootSequence.SequenceStatus.Wait)
            yield return (object) null;
        }
        while (this.status == BootLoader.BootSequence.SequenceStatus.Confirm);
      ...
      else if (this.boot.player_during_pvp_result)
      {
        NGGameDataManager.SeaChangeFlag = false;
        this.status = BootLoader.BootSequence.SequenceStatus.Wait;
        ModalWindow.Show(c.boot_continue_pvp_result_title, c.boot_continue_pvp_result_text, (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Noop));
        while (this.status == BootLoader.BootSequence.SequenceStatus.Wait)
          yield return (object) null;
      }
      else if (this.boot.player_during_sea_date)
      {
        ...
        {
          this.status = BootLoader.BootSequence.SequenceStatus.Wait;
          ModalWindow.ShowYesNo(c.boot_continue_date_title, c.boot_continue_date_text, (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Break), (System.Action) (() => ModalWindow.ShowYesNo(c.boot_continue_date_exit_confirm_title, c.boot_continue_date_exit_confirm_text, (System.Action) (() => ModalWindow.Show(c.boot_continue_date_exit_done_title, c.boot_continue_date_exit_done_text, (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Noop))), (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Confirm))));
          while (this.status == BootLoader.BootSequence.SequenceStatus.Wait)
            yield return (object) null;
        }
        while (this.status == BootLoader.BootSequence.SequenceStatus.Confirm);
      ...
      else if (NGGameDataManager.SeaChangeFlag && !this.boot.sea_is_opened)
      {
        NGGameDataManager.SeaChangeFlag = false;
        this.status = BootLoader.BootSequence.SequenceStatus.Wait;
        ModalWindow.Show(c.boot_sea_not_open_confirm_title, c.boot_sea_not_open_confirm_text, (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Noop));
        while (this.status == BootLoader.BootSequence.SequenceStatus.Wait)
          yield return (object) null;
      }
      else if (this.boot.during_retry_gacha)
      {
        NGGameDataManager.SeaChangeFlag = false;
        this.status = BootLoader.BootSequence.SequenceStatus.Wait;
        ModalWindow.Show(c.GACHA_NOT_END_TITLE, c.GACHA_NOT_END_DESCRIPTION, (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Noop));
        while (this.status == BootLoader.BootSequence.SequenceStatus.Wait)
          yield return (object) null;
      }
    }
    ...
    private void procLocalBattleResumeConfirmPopup(bool enableContinue)
    {
      ...
      ModalWindow.ShowYesNo(resumeTitle, message, (System.Action) (() =>
      {
        if (Persist.battleEnvironment.Exists)
          this.status = BootLoader.BootSequence.SequenceStatus.Break;
        else
          ModalWindow.ShowYesNo(resumeTitle, resumeMsg2, (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Break), (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Confirm));
      }), (System.Action) (() => ModalWindow.ShowYesNo(retireTitle, retireMsg, (System.Action) (() => ModalWindow.Show(retireTitle2, retireMsg2, (System.Action) (() =>
      {
        this.isCancelContinueBattle = true;
        this.status = BootLoader.BootSequence.SequenceStatus.Noop;
      }))), (System.Action) (() => this.status = BootLoader.BootSequence.SequenceStatus.Confirm))));
    }
