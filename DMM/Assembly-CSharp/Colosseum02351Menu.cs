...
public class Colosseum02351Menu : BackButtonMenuBase
{
    ...
    int nextPoint)
  {
    ...
    e = Singleton<CommonRoot>.GetInstance().GetColosseumHeaderComponent().SetInfo(CommonColosseumHeader.BtnMode.Back, new System.Action(colosseum02351Menu.IbtnBack));
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    ...
    {
      Future<UnityEngine.Sprite> sprF = EmblemUtility.LoadEmblemSprite(colosseumRankReward.reward_id);
      e = sprF.Wait();
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
    ...
    for (int index = 0; index < colosseumRankRewardArray.Length; ++index)
    {
      ...
      if (reward.reward_type != MasterDataTable.CommonRewardType.emblem)
      {
        ...
        {
          colosseum02351Menu.ZoomBtn.gameObject.SetActive(true);
          ...
          {
            colosseum02351Menu.ZoomBtn.gameObject.SetActive(false);
            uniquePrefabF = Res.Icons.UniqueIconPrefab.Load<GameObject>();
            e = uniquePrefabF.Wait();
            while (e.MoveNext())
              yield return e.Current;
  ...
  {
    if (this.IsPushAndSet() || !this.canChangeScene | ((UnityEngine.Object) Singleton<PopupManager>.GetInstanceOrNull() != (UnityEngine.Object) null && Singleton<PopupManager>.GetInstance().isOpen))
      return;
