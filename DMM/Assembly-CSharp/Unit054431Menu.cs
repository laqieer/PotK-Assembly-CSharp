...
{
  protected override PlayerItem[] GetAllGears(PlayerItem[] items) => ((IEnumerable<PlayerItem>) items).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x.gear != null)).ToArray<PlayerItem>();
  ...
  protected override void ChangeDetailScene(PlayerUnit unit) => Unit0542Scene.changeSceneEvolutionUnit(true, unit, this.getUnits());
  ...
  protected override PlayerUnit[] getCanEquippedUnits(PlayerUnit[] playerUnits)
  {
    ...
    foreach (PlayerUnit playerUnit in enumerable)
    {
      ...
      else
      {
        ...
        if (num2 == valueOrDefault & classificationPattern.HasValue || kind.isNonWeapon)
          playerUnitList.Add(playerUnit);
      }
    }
