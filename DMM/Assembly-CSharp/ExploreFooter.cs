...
public class ExploreFooter : MonoBehaviour
{
  ...
  public IEnumerator UpdateDeckUnitIconsAsync()
  {
    ...
    for (int i = 0; i < this.mUnitIconAnchors.Length; ++i)
    {
      ...
      this.mUnitIcons.Add(icon.gameObject);
  ...
  {
    if ((Object) this.mProgressGauge == (Object) null)
      return;
  ...
  private void setChallengeEnable(bool enable)
  {
    if (Singleton<NGGameDataManager>.GetInstance().challenge_point < 1)
      enable = false;
    if (!this.mChallengeButton.isEnabled & enable)
    {
      if (!this.mChallengeButton.isEnabled)
        this.mManager.PlaySe("SE_2407");
      this.mChallengeEffectAnime.Play("explore_BtnChallenge_Idle");
      this.mChallengeButton.isEnabled = true;
    }
    else
    {
      if (!this.mChallengeButton.isEnabled || enable)
      ...
      this.mChallengeButton.isEnabled = false;
    }
  }
  ...
  private IEnumerator LoadResources()
  {
    ...
    Future<GameObject> f;
    IEnumerator e;
    if ((Object) exploreFooter.mProgressGauge != (Object) null)
    {
    ...
    if ((Object) exploreFooter.mChallengeButtonAnchor != (Object) null)
    {
      ...
      exploreFooter.mChallengeButton.isEnabled = false;
      f = (Future<GameObject>) null;
    }
  ...
  {
    if ((Object) this.mWinRateLbl == (Object) null)
      return;
  ...
  {
    if ((Object) this.mBoxMaxBadge == (Object) null)
      return;
  ...
  public void RefreshLog()
  {
    ...
    foreach (LogData logData in Singleton<ExploreDataManager>.GetInstance().GetAllLog())
    {
      ...
      this.mLogScrollView.Add(component.gameObject, true);
      component.SetMessage(logData.Message, logData.Color);
      this.mLastGotLog = logData;
    }
    this.mLogScrollView.ResolvePosition(this.mLogScrollView.grid.transform.childCount - 1);
  }

  private void updateLog()
  {
    if ((Object) this.logItemPrefab == (Object) null)
      return;
    ExploreDataManager instance = Singleton<ExploreDataManager>.GetInstance();
    ...
    foreach (LogData newLogData in instance.GetNewLogDatas(this.mLastGotLog))
    {
      ...
      if (this.mLogScrollView.grid.transform.childCount > 20)
      {
        Transform child = this.mLogScrollView.grid.transform.GetChild(0);
