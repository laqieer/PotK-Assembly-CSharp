...
public class CalcUnitCompose
{
    ...
    CalcUnitCompose.PriceMode mode)
  {
    ...
    foreach (PlayerUnit materialUnit in material_units)
    {
      ...
      {
        int num2 = unit.IsBreakThrough || unit.IsPureValueUp ? 1 : (unit.rarity.index < index ? materialUnit.unity_value + 1 : materialUnit.breakthrough_count + 1);
        num3 += (long) num2;
      }
    }
    ...
    for (int breakthroughCount = base_unit.breakthrough_count; (long) breakthroughCount < num3; ++breakthroughCount)
      num4 += (long) (int) ((double) CalcUnitCompose.BREAKTHROUGH_BASE * (double) CalcUnitCompose.BREAKTHROUGH_REVISION[breakthroughCount]);
    long num5 = num4;
    return num1 + num5;
  }
