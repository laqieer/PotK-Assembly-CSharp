...
public class Unit0044Menu : Bugu005SelectItemListMenuBase
{
  ...
  public PlayerUnit BasePlayerUnit
  {
    set
    {
      ...
      else
      {
        ...
        this.baseEquippedGearIds = ((IEnumerable<int?>) nullableArray).Where<int?>((Func<int?, bool>) (x => x.HasValue)).Select<int?, int>((Func<int?, int>) (y => y.Value)).ToList<int>();
  ...
  private (PlayerItem src, PlayerItem[] dests, int[] destIndexes) swapGears
  {
    get
    {
      ...
      PlayerItem[] array1 = ((IEnumerable<int>) numArray1).Select<int, PlayerItem>((Func<int, PlayerItem>) (i => equippedGears[i])).ToArray<PlayerItem>();
      if (this.SpecialGear != null && (equippedGears[0] != (PlayerItem) null || equippedGears[2] != (PlayerItem) null))
      {
        GearGear specialGear = this.SpecialGear;
        PlayerItem playerItem3 = equippedGears[2];
        GearGear gearGear = (object) playerItem3 != null ? playerItem3.gear : (GearGear) null;
        if (specialGear != gearGear)
        {
          numArray1[0] = -1;
      ...
      List<int> intList = new List<int>(numArray1.Length);
      List<PlayerItem> playerItemList = new List<PlayerItem>(numArray1.Length);
      for (int index = 0; index < numArray1.Length; ++index)
      {
        if (numArray1[index] >= 0)
        {
          intList.Add(numArray1[index]);
          playerItemList.Add(array1[index]);
        }
      }
      PlayerItem[] array3 = playerItemList.ToArray();
      int[] numArray4 = array3.Length != 0 ? intList.ToArray() : (int[]) null;
      this.swapGears_ = new (PlayerItem, PlayerItem[], int[])?((playerItem2, array3, numArray4));
      return this.swapGears_.Value;
    }
  }
  ...
  public override Persist<Persist.ItemSortAndFilterInfo> GetPersist() => Persist.unit0044SortAndFilter;
  ...
  {
    PlayerItem[] targets = this.targetGears;
    if (this.SpecialGear != null)
    {
      ...
      targets = this.sortSpecialGears(((IEnumerable<PlayerItem>) targets).Where<PlayerItem>((Func<PlayerItem, bool>) (x => includeGears.Contains(x.id) || x.gear == this.SpecialGear))).ToArray<PlayerItem>();
    }
    (PlayerItem src, PlayerItem[] dests, int[] destIndexes) swapGears = this.swapGears;
    return swapGears.destIndexes != null ? Gears.makeList(this.basePlayerUnit, targets, this.changeGearIndex - 1, ((IEnumerable<PlayerItem>) swapGears.dests).Where<PlayerItem>((Func<PlayerItem, bool>) (x => x != (PlayerItem) null)).Select<PlayerItem, int>((Func<PlayerItem, int>) (y => y.id)).ToList<int>()) : Gears.makeList(this.basePlayerUnit, targets, this.changeGearIndex - 1);
  }

  private IEnumerable<PlayerItem> sortSpecialGears(
    IEnumerable<PlayerItem> playerGears)
  {
    ...
    PlayerItem afterGear)
  {
    ...
    if (this.EditParam != null)
    {
      e = component.SetGear((PlayerUnit) null, baseUnit, beforeGear, afterGear, beforeGearIcon, afterGearIcon, beforeSkillTypeIcons.ToArray(), afterSkillTypeIcons.ToArray(), this.changeGearIndex, this.isEarthMode, (System.Action) (() => this.changeCustomDeckGear(afterGear.id)), bCustomSwap, customReisous);
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
    }
    ...
    else
    {
      ...
      switch (GuildUtil.gvgPopupState)
      {
        ...
        case GuildUtil.GvGPopupState.AtkTeam:
          ...
          e = GuildUtil.UpdateGuildDeckAttack(PlayerAffiliation.Current.guild_id, Player.Current.id, new System.Action(unit0044Menu.\u003CRemoveGearAsync\u003Eb__48_1));
          while (e.MoveNext())
            yield return e.Current;
          e = (IEnumerator) null;
          break;
        case GuildUtil.GvGPopupState.DefTeam:
          // ISSUE: reference to a compiler-generated method
          e = GuildUtil.UpdateGuildDeckDefanse(PlayerAffiliation.Current.guild_id, Player.Current.id, new System.Action(unit0044Menu.\u003CRemoveGearAsync\u003Eb__48_2));
          while (e.MoveNext())
            yield return e.Current;
          e = (IEnumerator) null;
          break;
  ...
  protected override InventoryItem CreateInventoryItem(PlayerItem target) => this.EditParam == null ? new InventoryItem(target, this.enabledExpireDate) : new InventoryItem(target, this.enabledExpireDate, (Func<bool>) (() => this.EditParam.dicReference.ContainsKey(target.id)));
