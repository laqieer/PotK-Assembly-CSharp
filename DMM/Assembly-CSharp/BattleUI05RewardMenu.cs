...
public class BattleUI05RewardMenu : ResultMenuBase
{
  ...
  public override IEnumerator Init(BattleInfo info, BattleEnd result)
  {
    ...
    if (playerMaterialUnitArray != null && result.drop_material_unit_entities != null)
    {
      Dictionary<int, PlayerMaterialUnit> dictionary2 = ((IEnumerable<PlayerMaterialUnit>) playerMaterialUnitArray).ToDictionary<PlayerMaterialUnit, int>((Func<PlayerMaterialUnit, int>) (x => x.id));
    ...
    PlayerMaterialGear[] playerMaterialGearArray = SMManager.Get<PlayerMaterialGear[]>();
    if (playerMaterialGearArray != null && result.drop_material_gear_entities != null)
    {
      Dictionary<int, PlayerMaterialGear> dictionary2 = ((IEnumerable<PlayerMaterialGear>) playerMaterialGearArray).ToDictionary<PlayerMaterialGear, int>((Func<PlayerMaterialGear, int>) (x => x.id));
      foreach (BattleEndDrop_material_gear_entities materialGearEntity in result.drop_material_gear_entities)
        this.AddReward((object) dictionary2[materialGearEntity.reward_id.Value], materialGearEntity.is_new, materialGearEntity.reward_quantity, materialGearEntity.reward_id.Value, materialGearEntity.reward_type_id);
    }
  ...
  public override IEnumerator Run()
  {
    ...
    BattleUI05ResultMenu component = battleUi05RewardMenu.GetComponent<BattleUI05ResultMenu>();
  ...
  public override IEnumerator OnFinish()
  {
    foreach (BattleUI05RewardMenu.Reward playReward in this.PlayRewards)
    {
      ...
      if ((UnityEngine.Object) component1 != (UnityEngine.Object) null)
        component1.buttonBoxCollider.enabled = false;
      if ((UnityEngine.Object) component2 != (UnityEngine.Object) null)
      {
        component2.gear.button.gameObject.SetActive(false);
        component2.supply.button.gameObject.SetActive(false);
      }
    }
    this.mainObject.SetActive(false);
    this.mainTitleObject.SetActive(false);
  ...
  public IEnumerator SetReward()
  {
    ...
    foreach (BattleUI05RewardMenu.Reward playReward in this.PlayRewards)
    {
      BattleUI05RewardMenu.Reward reward = playReward;
      PlayerUnit unit;
      PlayerItem gear;
      UnitIcon icon1;
      IEnumerator e;
      ...
      {
        this.createWrapper(reward, grid.transform, "unit wrapper", unknownUnit, this.mUnitPrefab);
        ...
        EventDelegate.Add(icon1.Button.gameObject.GetOrAddComponent<UIButton>().onClick, this.UnitClickEvent(unit, reward.is_new));
      ...
      {
        this.createWrapper(reward, grid.transform, "material unit wrapper", unknownUnit, this.mUnitPrefab);
        ...
        icon1.Button.onLongPress.Clear();
        EventDelegate.Add(icon1.Button.gameObject.GetOrAddComponent<UIButton>().onClick, this.UnitClickEvent(unitData, reward.is_new));
      ...
      {
        this.createWrapper(reward, grid.transform, "gear wrapper", unknownItem, this.mItemIconPrefab);
        ...
        EventDelegate.Add(icon2.gear.button.gameObject.GetOrAddComponent<UIButton>().onClick, this.GearClickEvent(gear, reward.is_new));
      ...
      else
      {
        ...
        {
          this.createWrapper(reward, grid.transform, "material gear wrapper", unknownItem, this.mItemIconPrefab);
          icon2 = reward.gameObject.GetComponent<ItemIcon>();
        ...
        else if (reward.master is SupplySupply master)
        {
          this.createWrapper(reward, grid.transform, "supply wrapper", unknownItem, this.mItemIconPrefab);
          icon2 = reward.gameObject.GetComponent<ItemIcon>();
          e = icon2.InitBySupply(master);
          while (e.MoveNext())
            yield return e.Current;
          e = (IEnumerator) null;
          icon2.EnableQuantity(reward.quantity);
        ...
        {
          this.createWrapper(reward, grid.transform, "gacha ticket wrapper", unknownItem, this.mUniqueIconsPrefab);
          e = reward.gameObject.GetComponent<UniqueIcons>().SetGachaTicket(reward.quantity, reward.entity_id);
          while (e.MoveNext())
            yield return e.Current;
          e = (IEnumerator) null;
        }
        else if (reward.master is UnitTypeTicket master)
        {
          this.createWrapper(reward, grid.transform, "unit type ticket wrapper", unknownItem, this.mUniqueIconsPrefab);
          e = reward.gameObject.GetComponent<UniqueIcons>().SetReincarnationTypeTicket(master.ID, reward.quantity);
          while (e.MoveNext())
            yield return e.Current;
          e = (IEnumerator) null;
        }
        else if (reward.master is PlayerQuestKey master)
        {
          this.createWrapper(reward, grid.transform, "quest key wrapper", unknownItem, this.mUniqueIconsPrefab);
          yield return (object) reward.gameObject.GetComponent<UniqueIcons>().SetKey(master.quest_key_id, reward.quantity);
        }
        else if (reward.master is MasterDataTable.SelectTicket master)
        {
          this.createWrapper(reward, grid.transform, "unit ticket wrapper", unknownItem, this.mUniqueIconsPrefab);
          UniqueIcons component = reward.gameObject.GetComponent<UniqueIcons>();
          if (master.category == SelectTicketCategory.Unit)
          {
            e = component.SetKillersTicket(master.ID, reward.quantity);
            while (e.MoveNext())
              yield return e.Current;
            e = (IEnumerator) null;
          }
          ...
          {
            e = component.SetMaterialTicket(master.ID, reward.quantity);
            while (e.MoveNext())
              yield return e.Current;
            e = (IEnumerator) null;
          }
        }
        else if (reward.master is CommonTicket master)
        {
          this.createWrapper(reward, grid.transform, "common ticket wrapper", unknownItem, this.mUniqueIconsPrefab);
          yield return (object) reward.gameObject.GetComponent<UniqueIcons>().SetCommonTicket(master.ID, reward.quantity);
        }
        else
      ...
      reward = (BattleUI05RewardMenu.Reward) null;
    }
  ...
  {
    GameObject gameObject = this.mUnitPrefab.Clone(parent);
  ...
  {
    GameObject gameObject = this.mItemIconPrefab.Clone(parent);
  ...
  private EventDelegate.Callback UnitClickEvent(PlayerUnit unit, bool isNew) => (EventDelegate.Callback) (() => {});

  private EventDelegate.Callback GearClickEvent(PlayerItem item, bool isNew) => (EventDelegate.Callback) (() => {});
  ...
  private IEnumerator InitPopup(GameObject popup, IEnumerator setter)
  {
    ...
    UIWidget component = popup.transform.Find("Top").GetComponent<UIWidget>();
    popup.transform.Find("Bottom").GetComponent<UIWidget>().topAnchor.absolute -= 120;
    component.bottomAnchor.absolute += 195;
