...
namespace MasterDataTable
{
  ...
  public class ShopArticle
  {
    ...
    public static ShopArticle Parse(MasterDataReader reader) => new ShopArticle()
    {
      ID = reader.ReadInt(),
    ...
    public ShopShop shop
    {
      ...
      {
        ShopShop shopShop;
        if (!MasterData.ShopShop.TryGetValue(this.shop_ShopShop, out shopShop))
    ...
    public ShopContent[] ShopContents => ((IEnumerable<ShopContent>) MasterData.ShopContentList).Where<ShopContent>((Func<ShopContent, bool>) (x => x.article.ID == this.ID)).ToArray<ShopContent>();
    ...
    public Future<UnityEngine.Sprite> LoadSpriteThumbnail()
    {
      ...
      return shopContent.entity_type == CommonRewardType.quest_key ? MasterData.QuestkeyQuestkey[shopContent.entity_id].LoadSpriteThumbnail() : Singleton<ResourceManager>.GetInstance().Load<UnityEngine.Sprite>("Sprites/1x1_pink");
    }
  }
}
