...
namespace MasterDataTable
{
  ...
  public class GvgRule
  {
    ...
    public static GvgRule Parse(MasterDataReader reader) => new GvgRule()
    {
      ID = reader.ReadInt(),
    ...
    private bool checkGearKind(PlayerUnit target) => ((IEnumerable<int>) this.values_).Contains<int>(target.unit.kind_GearKind);
    ...
    private bool checkElement(PlayerUnit target) => ((IEnumerable<PlayerUnitSkills>) target.GetAcquireSkills()).Any<PlayerUnitSkills>((Func<PlayerUnitSkills, bool>) (x => ((IEnumerable<int>) this.values_).Contains<int>(x.skill_id)));
    ...
    {
      if (((object) target != null ? target.unit : (UnitUnit) null) == null)
        return false;
    ...
    public static GvgRule[] getRules(int rule_no) => ((IEnumerable<GvgRule>) MasterData.GvgRuleList).Where<GvgRule>((Func<GvgRule, bool>) (x => x.rule_no == rule_no)).ToArray<GvgRule>();
  }
}
