...
namespace MasterDataTable
{
  ...
  public class DuelDuelConfig
  {
    ...
    public List<string> preloadEffectFileNameList => ((IEnumerable<DuelEffectPreload>) MasterData.DuelEffectPreloadList).Where<DuelEffectPreload>((Func<DuelEffectPreload, bool>) (x => this.ID == x.duel_controller_id)).Select<DuelEffectPreload, string>((Func<DuelEffectPreload, string>) (x => x.effect_file_name)).ToList<string>();

    public static DuelDuelConfig Parse(MasterDataReader reader) => new DuelDuelConfig()
    {
      ID = reader.ReadInt(),
