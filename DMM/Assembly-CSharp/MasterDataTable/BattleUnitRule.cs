...
namespace MasterDataTable
{
  ...
  public class BattleUnitRule
  {
    ...
    public static BattleUnitRule Parse(MasterDataReader reader) => new BattleUnitRule()
    {
      ID = reader.ReadInt(),
    ...
    public BattleUnitRuleOperator value_operator => (BattleUnitRuleOperator) this.value_operator_BattleUnitRuleOperator;
    ...
    private bool checkGearKind(PlayerUnit playerUnit, UnitUnit unit) => ((IEnumerable<int>) this.values_).Contains<int>(unit.kind_GearKind);
    ...
    private bool checkElement(PlayerUnit playerUnit, UnitUnit unit) => ((IEnumerable<PlayerUnitSkills>) playerUnit.GetAcquireSkills()).Any<PlayerUnitSkills>((Func<PlayerUnitSkills, bool>) (x => ((IEnumerable<int>) this.values_).Contains<int>(x.skill_id)));

    private bool checkSameCharacterID(PlayerUnit playerUnit, UnitUnit unit) => ((IEnumerable<int>) this.values_).Contains<int>(unit.same_character_id);
    ...
    {
      UnitUnit unit = (object) target != null ? target.unit : (UnitUnit) null;
      if (unit == null)
        return false;
    ...
    public static BattleUnitRule[] getRules(int rule_no) => ((IEnumerable<BattleUnitRule>) MasterData.BattleUnitRuleList).Where<BattleUnitRule>((Func<BattleUnitRule, bool>) (x => x.rule_no == rule_no)).ToArray<BattleUnitRule>();
  }
}
