...
namespace MasterDataTable
{
  ...
  public class BattleReinforcement
  {
    ...
    public BattleVictoryAreaCondition[] area => this.reinforcement_logic.Enum == BattleReinforcementLogicEnum.enemy_area_invasion || this.reinforcement_logic.Enum == BattleReinforcementLogicEnum.player_area_invasion ? ((IEnumerable<BattleVictoryAreaCondition>) MasterData.BattleVictoryAreaConditionList).Where<BattleVictoryAreaCondition>((Func<BattleVictoryAreaCondition, bool>) (x => x.group_id == this.arg1_value)).ToArray<BattleVictoryAreaCondition>() : (BattleVictoryAreaCondition[]) null;
    ...
    public static BattleReinforcement Parse(MasterDataReader reader) => new BattleReinforcement()
    {
      ID = reader.ReadInt(),
