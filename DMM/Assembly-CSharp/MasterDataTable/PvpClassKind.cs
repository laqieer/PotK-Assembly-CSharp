...
namespace MasterDataTable
{
  ...
  public class PvpClassKind
  {
    ...
    public static PvpClassKind Parse(MasterDataReader reader) => new PvpClassKind()
    {
      ID = reader.ReadInt(),
    ...
    public bool isTopClass => ((IEnumerable<PvpClassKind>) MasterData.PvpClassKindList).OrderByDescending<PvpClassKind, int>((Func<PvpClassKind, int>) (x => x.weight)).ToList<PvpClassKind>().First<PvpClassKind>() == this;

    public bool isLowestClass => ((IEnumerable<PvpClassKind>) MasterData.PvpClassKindList).OrderBy<PvpClassKind, int>((Func<PvpClassKind, int>) (x => x.weight)).ToList<PvpClassKind>().First<PvpClassKind>() == this;

    public PvpClassKind NextClass => ((IEnumerable<PvpClassKind>) MasterData.PvpClassKindList).OrderBy<PvpClassKind, int>((Func<PvpClassKind, int>) (x => x.weight)).ToList<PvpClassKind>().FirstOrDefault<PvpClassKind>((Func<PvpClassKind, bool>) (x => x.weight > this.weight));

    public PvpClassKind PreviousClass => ((IEnumerable<PvpClassKind>) MasterData.PvpClassKindList).OrderByDescending<PvpClassKind, int>((Func<PvpClassKind, int>) (x => x.weight)).ToList<PvpClassKind>().FirstOrDefault<PvpClassKind>((Func<PvpClassKind, bool>) (x => x.weight < this.weight));
    ...
    public PvpClassKind.Condition NextCondition(int nowWin) => this.NextCondition(this.ClassCondition(nowWin));
