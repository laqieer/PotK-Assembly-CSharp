...
namespace MasterDataTable
{
  ...
  public class ReisouRankIncr
  {
    ...
    public static ReisouRankIncr Parse(MasterDataReader reader) => new ReisouRankIncr()
    {
      ID = reader.ReadInt(),
    ...
    public static ReisouRankIncr FromRank(GearKind kind, int group_id, int rank) => ((IEnumerable<ReisouRankIncr>) MasterData.ReisouRankIncrList).SingleOrDefault<ReisouRankIncr>((Func<ReisouRankIncr, bool>) (x => x.gear_kind.Enum == kind.Enum && x.group_id == group_id && x.level == rank)) ?? ((IEnumerable<ReisouRankIncr>) MasterData.ReisouRankIncrList).SingleOrDefault<ReisouRankIncr>((Func<ReisouRankIncr, bool>) (x => x.gear_kind.Enum == kind.Enum && x.group_id == 1 && x.level == rank));
  }
}
