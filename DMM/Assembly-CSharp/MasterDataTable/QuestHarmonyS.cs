...
namespace MasterDataTable
{
  ...
  public class QuestHarmonyS
  {
    ...
    public static QuestHarmonyS Parse(MasterDataReader reader) => new QuestHarmonyS()
    {
      ID = reader.ReadInt(),
    ...
    public UnitUnit target_unit
    {
      ...
      {
        UnitUnit unitUnit;
        if (!MasterData.UnitUnit.TryGetValue(this.target_unit_UnitUnit, out unitUnit))
    ...
    public UnitUnit target_unit2
    {
      get
      {
        ...
        UnitUnit unitUnit;
        if (!MasterData.UnitUnit.TryGetValue(this.target_unit2_UnitUnit.Value, out unitUnit))
    ...
    public QuestHarmonyM quest_m
    {
      get
      {
        QuestHarmonyM questHarmonyM;
        if (!MasterData.QuestHarmonyM.TryGetValue(this.quest_m_QuestHarmonyM, out questHarmonyM))
          Debug.LogError((object) ("Key not Found: MasterData.QuestHarmonyM[" + (object) this.quest_m_QuestHarmonyM + "]"));
        return questHarmonyM;
      }
    }
    ...
    public StoryPlaybackHarmonyDetail[] HarmonyDetail() => ((IEnumerable<StoryPlaybackHarmonyDetail>) MasterData.StoryPlaybackHarmonyDetailList).Where<StoryPlaybackHarmonyDetail>((Func<StoryPlaybackHarmonyDetail, bool>) (x => x.quest.ID == this.ID)).ToArray<StoryPlaybackHarmonyDetail>();

    public StoryPlaybackHarmonyDetail GetHarmonyDetail(
