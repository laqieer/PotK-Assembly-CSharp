...
namespace MasterDataTable
{
  ...
  public class UnityPureValueUpPattern
  {
    ...
    public static UnityPureValueUpPattern Parse(MasterDataReader reader) => new UnityPureValueUpPattern()
    {
      ID = reader.ReadInt(),
    ...
    public UnitUnit target_same_character_id
    {
      get
      {
        ...
        UnitUnit unitUnit;
        if (!MasterData.UnitUnit.TryGetValue(this.target_same_character_id_UnitUnit.Value, out unitUnit))
    ...
    public bool HasCheckUnitGroup => this.group_large_category_UnitGroupLargeCategory.HasValue || this.group_small_category_UnitGroupSmallCategory.HasValue || !string.IsNullOrEmpty(this.group_clothing_categories) || this.group_generation_category_UnitGroupGenerationCategory.HasValue;
    ...
    public bool checkUnityPureValueUP(UnitUnit target)
    {
      ...
      if (!string.IsNullOrEmpty(this.group_clothing_categories))
      {
        ...
        if (array.Length > 1)
        {
          if (!((IEnumerable<int>) array).SequenceEqual<int>((IEnumerable<int>) ((IEnumerable<int>) numArray).OrderBy<int, int>((Func<int, int>) (i => i))))
            return false;
        }
        else if (!((IEnumerable<int>) numArray).Contains<int>(array[0]))
          return false;
      }
