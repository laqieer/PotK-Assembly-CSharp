...
namespace MasterDataTable
{
  ...
  public class BattleLandformEffect
  {
    ...
    public string GetString(BattleskillEffectLogicArgumentEnum key) => this.Get<string>(key, new Func<float, string>(this.ValueToString));

    public float GetFloat(BattleskillEffectLogicArgumentEnum key) => this.Get<float>(key, new Func<float, float>(this.ValueToFloat));

    public int GetInt(BattleskillEffectLogicArgumentEnum key) => this.Get<int>(key, new Func<float, int>(this.ValueToInt));
    ...
    public static BattleLandformEffect Parse(MasterDataReader reader) => new BattleLandformEffect()
    {
      ID = reader.ReadInt(),
    ...
    public BattleskillEffectLogic effect_logic
    {
      get
      {
        BattleskillEffectLogic battleskillEffectLogic;
        if (!MasterData.BattleskillEffectLogic.TryGetValue(this.effect_logic_BattleskillEffectLogic, out battleskillEffectLogic))
          Debug.LogError((object) ("Key not Found: MasterData.BattleskillEffectLogic[" + (object) this.effect_logic_BattleskillEffectLogic + "]"));
        return battleskillEffectLogic;
      }
    }

    public BattleLandformEffectPhase landform_effect_phase => (BattleLandformEffectPhase) this.landform_effect_phase_BattleLandformEffectPhase;
  }
}
