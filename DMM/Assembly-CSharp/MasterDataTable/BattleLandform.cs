...
namespace MasterDataTable
{
  ...
  public class BattleLandform
  {
    ...
    private static int MakeIncrKey(BattleLandform landform, UnitMoveType move_type) => (int) ((landform.ID << 8) + move_type);

    public static void CacheClear() => BattleLandform.incrDic = GameGlobalVariable<AssocList<int, BattleLandformIncr>>.Null();
    ...
    public BattleLandformIncr[] GetAllIncr() => ((IEnumerable<BattleLandformIncr>) MasterData.BattleLandformIncrList).Where<BattleLandformIncr>((Func<BattleLandformIncr, bool>) (x => x.landform_BattleLandform == this.ID)).ToArray<BattleLandformIncr>();
    ...
    public static BattleLandform Parse(MasterDataReader reader) => new BattleLandform()
    {
      ID = reader.ReadInt(),
    ...
    public BattleLandformFootstepType footstep_type
    {
      get
      {
        ...
        if (!MasterData.BattleLandformFootstepType.TryGetValue(this.footstep_type_BattleLandformFootstepType, out landformFootstepType))
