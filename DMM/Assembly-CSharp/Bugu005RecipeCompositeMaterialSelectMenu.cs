...
public class Bugu005RecipeCompositeMaterialSelectMenu : Bugu005SelectItemListMenuBase
{
  ...
  public void SetAllGearRecipes(List<GearCombineRecipe> gearRecipes) => this.allGearRecipes = gearRecipes;

  public void SetOriginalSelectedItem(List<GameCore.ItemInfo> items) => this.originalSelectedItems = items;

  public override Persist<Persist.ItemSortAndFilterInfo> GetPersist() => Persist.bugu0052CompositeSortAndFilter;

  protected override List<PlayerItem> GetItemList() => ((IEnumerable<PlayerItem>) SMManager.Get<PlayerItem[]>()).Where<PlayerItem>((Func<PlayerItem, bool>) (x => !x.broken)).ToList<PlayerItem>();
  ...
  protected override void BottomInfoUpdate()
  {
    ...
    this.ibtnYes.isEnabled = this.isSelectedItemAreInRecipe();
  }
...
label_24:
    for (int i = 0; i < this.allGearRecipes.Count; i++)
    {
      ...
      if (gearGearList.Any<GearGear>(closure_1 ?? (closure_1 = (Func<GearGear, bool>) (x => x.group_id == item.Item.gear.group_id))))
        return true;
    }
    return false;
  }
