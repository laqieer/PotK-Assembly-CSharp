...
public class MovePathCore
{
  ...
  private float B_SplineCurve(float x1, float x2, float x3, float t) => (float) ((1.0 - (double) t) * (1.0 - (double) t) * (double) x1 + 2.0 * (double) t * (1.0 - (double) t) * (double) x2 + (double) t * (double) t * (double) x3);

  private Vector3 B_SplineCurve(Vector3 p1, Vector3 p2, Vector3 p3, float t) => new Vector3(this.B_SplineCurve(p1.x, p2.x, p3.x, t), this.B_SplineCurve(p1.y, p2.y, p3.y, t), this.B_SplineCurve(p1.z, p2.z, p3.z, t));
}
