...
public class Explore033TopScene : NGSceneBase
{
  ...
  public static void changeScene(bool stack = true, bool serverSync = true) => Singleton<NGSceneManager>.GetInstance().changeScene("explore033_Top", (stack ? 1 : 0) != 0, (object) serverSync);
  ...
  public IEnumerator onStartSceneAsync(bool serverSync)
  {
    ...
    if (serverSync && !exploreMgr.ReloadDirty)
    {
      ...
      yield return (object) data.SaveSuspendData((System.Action) (() => saveFailed = true));
      if (saveFailed)
      {
  ...
  public override void onEndScene() => Singleton<ExploreSceneManager>.GetInstance().OnEndExploreScene();
  ...
  public override IEnumerator onDestroySceneAsync()
  {
    ...
    if (Singleton<NGGameDataManager>.GetInstance().IsEarth || Singleton<NGBattleManager>.GetInstance().isEarth)
      flag = true;
    else if ((bool) (UnityEngine.Object) Singleton<EarthDataManager>.GetInstance() && Singleton<EarthDataManager>.GetInstance().isPrologue)
      flag = true;
    if (!instance.ReloadDirty && !dataMgr.IsRankingAcceptanceFinish && !flag)
      yield return (object) dataMgr.SaveSuspendData((System.Action) (() => saveFailed = true));
    Singleton<NGDuelDataManager>.GetInstance().Init();
