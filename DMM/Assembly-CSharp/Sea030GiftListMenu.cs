...
public class Sea030GiftListMenu : Bugu005ItemListMenuBase
{
  ...
  public override IEnumerator Init()
  {
    ...
    if ((UnityEngine.Object) sea030GiftListMenu.callGiftRecipeWindow == (UnityEngine.Object) null)
    {
      ...
      sea030GiftListMenu.callGiftRecipeWindow = sea030GiftListMenu.giftDetailsSea.Clone(sea030GiftListMenu.transform);
    ...
    float y = sea030GiftListMenu.scroll.scrollView.verticalScrollBar.value;
    sea030GiftListMenu.Sort(ItemSortAndFilter.SORT_TYPES.CallItem, SortAndFilter.SORT_TYPE_ORDER_BUY.ASCENDING, false);
    sea030GiftListMenu.scroll.ResolvePosition(new Vector2(0.0f, y));
  ...
  protected override List<PlayerMaterialGear> GetMaterialList()
  {
    ...
    foreach (GearGear gearGear in array)
    {
      ...
      playerMaterialGearList.Add(playerMaterialGear1);
    }
  ...
  protected override void CreateItemIconAdvencedSetting(int inventoryItemIdx, int allItemIdx)
  {
    ...
    if (list.Count >= 1 && !((IEnumerable<int>) Sea030GiftListMenu.playerRecipeIDList).Contains<int>(list[0].ID))
      ...
      itemIcon.Gray = false;
    itemIcon.onClick = (System.Action<ItemIcon>) (playeritem => this.OpenCallGiftRecipe(playeritem.ItemInfo.gear, playeritem.Gray));
    ...
    itemIcon.EnableLongPressEvent(true, new System.Action<ItemIcon>(this.GearDetail));
  }
