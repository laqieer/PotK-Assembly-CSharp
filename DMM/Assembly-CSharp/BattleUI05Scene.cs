...
public class BattleUI05Scene : NGSceneBase
{
  ...
  public static void ReturnQuestScene(BattleInfo info, bool isWin, BattleEnd result)
  {
    ...
    if (instance2.HasSavedChangeSceneParam() && instance1.IsFromPopupStageList)
    {
      ...
      if (!(sceneName == "unit004_JobChange"))
      {
        ...
        {
          if (!(sceneName == "unit004_training"))
          {
            if (sceneName == "quest002_20_1")
            {
      ...
      instance1.returnSceneFromQuest = (System.Action) null;
    ...
    else
    {
      ...
      switch (info.quest_type)
      {
        ...
        case CommonQuestType.Extra:
          ...
          if (info.isTempDeck)
          {
            PlayerExtraQuestS[] playerExtraQuestSArray = ((IEnumerable<PlayerExtraQuestS>) SMManager.Get<PlayerExtraQuestS[]>()).S(questExtraS.quest_l_QuestExtraL, questExtraS.quest_m_QuestExtraM, true);
            if ((playerExtraQuestSArray.Length != 0 ? (UnityValueUpItemQuest.makeSkipSortieQuestUnityValueUp(((IEnumerable<PlayerExtraQuestS>) playerExtraQuestSArray).Select<PlayerExtraQuestS, int>((Func<PlayerExtraQuestS, int>) (x => x._quest_extra_s))).Count == playerExtraQuestSArray.Length ? 1 : 0) : 0) != 0)
            {
          ...
          if (questM == null)
          {
            ...
            num = questL != null ? questL.category_QuestExtraCategory : 0;
          }
          else
            num = questM.category_QuestExtraCategory;
          int activeTabIndex = num;
          ...
          {
            PlayerQuestGate[] playerQuestGateArray = SMManager.Get<PlayerQuestGate[]>();
            PlayerQuestGate playerQuestGate = ((IEnumerable<PlayerQuestGate>) playerQuestGateArray).FirstOrDefault<PlayerQuestGate>((Func<PlayerQuestGate, bool>) (x => ((IEnumerable<int>) x.quest_ids).Any<int>((Func<int, bool>) (y => ((IEnumerable<PlayerExtraQuestS>) resultExtraData).Any<PlayerExtraQuestS>((Func<PlayerExtraQuestS, bool>) (z => z._quest_extra_s == y))))));
            ...
            IEnumerable<int> idGateS = ((IEnumerable<PlayerQuestGate>) playerQuestGateArray).SelectMany<PlayerQuestGate, int>((Func<PlayerQuestGate, IEnumerable<int>>) (s => (IEnumerable<int>) s.quest_ids));
            int questLQuestExtraL = nextQuest.quest_l_QuestExtraL;
            if (((IEnumerable<PlayerExtraQuestS>) ((IEnumerable<PlayerExtraQuestS>) playerExtraQuestSArray).M(questLQuestExtraL)).All<PlayerExtraQuestS>((Func<PlayerExtraQuestS, bool>) (x => idGateS.Contains<int>(x._quest_extra_s))))
            {
    ...
    List<NGSceneManager.SavedSceneLog> logs)
  {
    ...
    foreach (NGSceneManager.SavedSceneLog log in logs)
    {
      ...
      if (!(name == "unit004_unit_list") && !(name == "unit004_6_8"))
      {
        if (!(name == "unit004_UnitTraining_List"))
        {
          if (name == "tower029_unit_selection" && log.args.Length == 3)
          {
      ...
      else if (log.args.Length != 0 && log.args[0] is Unit00468Scene.Mode)
      {
    ...
    if (!battleUi05Scene.isInitialized)
    {
      ...
      switch (info.quest_type)
      {
        case CommonQuestType.Story:
          ...
          {
            e = WebAPI.QuestProgressStory((System.Action<WebAPI.Response.UserError>) (error => WebAPI.DefaultUserErrorCallback(error))).Wait();
            while (e.MoveNext())
              yield return e.Current;
            e = (IEnumerator) null;
        ...
        case CommonQuestType.Character:
          ...
          {
            e = WebAPI.QuestProgressCharacter((System.Action<WebAPI.Response.UserError>) (error => WebAPI.DefaultUserErrorCallback(error))).Wait();
            while (e.MoveNext())
              yield return e.Current;
            e = (IEnumerator) null;
        ...
        case CommonQuestType.Harmony:
          ...
          {
            e = WebAPI.QuestProgressHarmony((System.Action<WebAPI.Response.UserError>) (error => WebAPI.DefaultUserErrorCallback(error))).Wait();
            while (e.MoveNext())
              yield return e.Current;
            e = (IEnumerator) null;
            WebAPI.SetLatestResponsedAt("QuestProgressHarmony");
      ...
      {
        (ResultMenuBase) battleUi05Scene.GetComponent<BattleUI05ResultMenu>(),
        (ResultMenuBase) battleUi05Scene.GetComponent<BattleUI05RewardMenu>(),
        (ResultMenuBase) null
      };
      ...
      if (!enabledBtnResortie && result.battle_helpers.Length != 0)
        battleUi05Scene.sequences.Add((ResultMenuBase) battleUi05Scene.GetComponent<FriendMenu>());
      if (result.stage_clear_rewards.Length != 0)
        battleUi05Scene.sequences.Add((ResultMenuBase) battleUi05Scene.GetComponent<BattleUI05ClearBonusMenu>());
      if (result.boost_stage_clear_rewards.Length != 0)
        battleUi05Scene.sequences.Add((ResultMenuBase) battleUi05Scene.GetComponent<BattleUI05ClearBonusLimitedMenu>());
      if (result.unlock_messages.Length != 0)
      {
        BattleUI05HardModeOpenMenu popup = battleUi05Scene.GetComponent<BattleUI05HardModeOpenMenu>();
      ...
      if (result.score_campaigns.Length != 0)
      {
        ResultMenuBase component = (ResultMenuBase) battleUi05Scene.GetComponent<BattleUI05RankingMenu>();
        if ((UnityEngine.Object) component != (UnityEngine.Object) null)
        {
          battleUi05Scene.sequences.Add(component);
          if (!result.score_campaigns[0].score_ranking_disabled)
            battleUi05Scene.sequences.Add((ResultMenuBase) battleUi05Scene.GetComponent<BattleUI05NowRankingPopupMenu>());
          if (result.score_campaigns[0].score_achivement_rewards.Length != 0)
            battleUi05Scene.sequences.Add((ResultMenuBase) battleUi05Scene.GetComponent<BattleUI05ScoreRewardPopupMenu>());
          if (result.score_campaigns[0].total_reward_exists && result.score_campaigns[0].score_total_rewards.Length != 0)
            battleUi05Scene.sequences.Add((ResultMenuBase) battleUi05Scene.GetComponent<BattleUI05TotalScoreRewardPopupMenu>());
        }
      }
      if (result.events != null && result.events.Length != 0)
        battleUi05Scene.sequences.Add((ResultMenuBase) battleUi05Scene.GetComponent<BattleUI05PunitiveExpeditionResultMenu>());
      if (result.get_sea_album_piece_counts != null && result.get_sea_album_piece_counts.Length != 0)
        battleUi05Scene.sequences.Add((ResultMenuBase) battleUi05Scene.GetComponent<Popup030SeaPieceGetMenu>());
      if (result.receive_sea_album_ids != null && result.receive_sea_album_ids.Length != 0)
        battleUi05Scene.sequences.Add((ResultMenuBase) battleUi05Scene.GetComponent<BattleUI05AlbumCompleteMenu>());
      if (enabledBtnResortie)
        battleUi05Scene.sequences.Add((ResultMenuBase) battleUi05Scene.GetComponent<ResortieWithFriendMenu>());
  ...
  private IEnumerator RunMenus(BattleInfo info, bool isWin, BattleEnd result)
  {
    ...
    while (seqe.MoveNext())
    {
      ...
      if (!((UnityEngine.Object) this.nowPlayBase == (UnityEngine.Object) null))
      {
        ...
        while (e.MoveNext())
        {
          if (this.isQuestAutoLap && (UnityEngine.Object) this.nowPlayBase != (UnityEngine.Object) null)
            this.nowPlayBase.isSkip = true;
          yield return e.Current;
        }
        e = (IEnumerator) null;
  ...
  public void onBackScene(BattleInfo info, bool isWin, BattleEnd result) => BattleUI05Scene.ReturnQuestScene(info, isWin, result);
