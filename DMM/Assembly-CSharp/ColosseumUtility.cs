...
public class ColosseumUtility
{
  ...
  public static IEnumerator CreateUniqueIcon(
    ...
    bool isBottom = true)
  {
    IEnumerator e = parent.gameObject.GetOrAddComponent<CreateIconObject>().CreateThumbnail((MasterDataTable.CommonRewardType) rewardTypeID, rewardID, rewardQuantity, isBottom);
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
  }
  ...
  public static IEnumerator CreateUniqueIcon(
    ...
    bool isBottom = true)
  {
    IEnumerator e = parent.gameObject.GetOrAddComponent<CreateIconObject>().CreateThumbnail(rewardTypeID, rewardID, rewardQuantity, isBottom);
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
  }

  public static int GetNextRankPoint(int point, int maxPoint) => ColosseumUtility.GetRankInfo(point, maxPoint).to_point - point;
  ...
  public static string GetRankName(int point) => ColosseumUtility.GetRankInfo(point, int.MaxValue).name;
  ...
  public static ColosseumRank GetPrevRankInfo(int point, int maxPoint)
  {
    ...
    ColosseumRank colosseumRank1 = array[0];
    foreach (ColosseumRank colosseumRank2 in array)
    {
      if (point <= colosseumRank2.to_point)
        return colosseumRank1;
      colosseumRank1 = colosseumRank2;
    }
    return colosseumRank1;
  }
  ...
  public static ColosseumRankReward[] GetRankRewardFromID(int id) => ((IEnumerable<ColosseumRankReward>) MasterData.ColosseumRankRewardList).Where<ColosseumRankReward>((Func<ColosseumRankReward, bool>) (v => v.rank_id == id)).ToArray<ColosseumRankReward>();
  ...
  private static ColosseumRank GetRankInfo(int point, int maxPoint)
  {
    ...
    foreach (ColosseumRank colosseumRank in (IEnumerable<ColosseumRank>) source)
    {
      if (point <= colosseumRank.to_point)
        return colosseumRank;
    }
