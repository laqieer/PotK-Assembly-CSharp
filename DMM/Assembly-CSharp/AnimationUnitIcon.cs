...
public class AnimationUnitIcon : MonoBehaviour
{
  ...
  [SerializeField]
  private List<UnityEngine.Sprite> rank_list_;
  [SerializeField]
  private List<UnityEngine.Sprite> num_list_;
  [SerializeField]
  private List<UnityEngine.Sprite> princessTypeList;
  [SerializeField]
  private SpriteRenderer rarityStar;

  public void SetType(GearKindEnum kind, CommonElement element) => this.type_.sprite = Resources.Load<UnityEngine.Sprite>(string.Format("Icons/Materials/GearKind_Element_Icon/slc_{0}_{1}_34_30", (object) kind.ToString(), (object) element.ToString()));

  public void SetStar()
  {
    UnityEngine.Sprite sprite = RarityIcon.GetSprite(this.player_unit_.unit, false);
    this.rarityStar.sprite = UnityEngine.Sprite.Create(sprite.texture, sprite.textureRect, new Vector2(0.5f, 0.0f), 1f, 100U, SpriteMeshType.FullRect);
  }
  ...
  private void SetLevelNum()
  {
    ...
    if (index2 != 0)
      return;
    this.level2_.gameObject.SetActive(false);
  }

  private void SetPrincessType(PlayerUnit unit)
  {
    this.princessType.gameObject.SetActive(false);
    if (!unit.unit.IsNormalUnit)
      return;
    int index = unit.unit_type.ID - 1;
    if (this.princessTypeList.Count <= index || !((Object) this.princessTypeList[index] != (Object) null))
      return;
    this.princessType.gameObject.SetActive(true);
  ...
  public void Set(PlayerUnit player_unit, bool is_new = false)
  {
    ...
    {
      this.frame_.sprite = Resources.Load<UnityEngine.Sprite>(string.Format("Prefabs/UnitIcon/Materials/s2_gold"));
      this.back_.sprite = Resources.Load<UnityEngine.Sprite>(string.Format("Prefabs/UnitIcon/Materials/s_back_gold"));
    }
