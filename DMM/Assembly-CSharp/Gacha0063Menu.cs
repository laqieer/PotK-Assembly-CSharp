...
public class Gacha0063Menu : BackButtonMenuBase
{
  ...
  public bool CheckGachaCharge(GachaModuleGacha gachaData, int gachaNumber) => !this.isSheetPopup && this.GachaCheckSelectedPickup(gachaData, gachaNumber) && (this.GachaCheckSufficiency(gachaData) && this.GachaCheckUnit(gachaData));

  public bool CheckGachaPt(GachaModuleGacha gachaData) => !this.isSheetPopup && this.GachaCheckFriend(gachaData) && (this.ManaGachaCheckUnit() && this.GachaCheckItem()) && this.GachaCheckReisou();

  public bool CheckGachaTicket(GachaModuleGacha gacha_data) => !this.isSheetPopup && this.TicketGachaCheckUnit() && (this.GachaCheckItem() && this.GachaCheckReisou());
  ...
  private IEnumerator PickupEmptyError(GachaModuleGacha gachaData, int gachaNumber)
  {
    ...
    if (!gacha0063Menu.IsPushAndSet())
    {
      ...
      int nWait = 0;
      PopupCommonNoYes.Show(instance.PICKUPSELECT_POPUP_PICKUPEMPTY_TITLE, instance.PICKUPSELECT_POPUP_PICKUPEMPTY_TEXT, (System.Action) (() => nWait = 1), (System.Action) (() => nWait = 2), NGUIText.Alignment.Left);
      while (nWait == 0)
        yield return (object) null;
      if (nWait == 1)
      {
        Singleton<NGGameDataManager>.GetInstance().currentGachaNumber = gachaNumber;
        GachaPickupSelectScene.changeScene(true, gachaData, (System.Action) (() => this.forceApiUpdate()));
      }
      else
  ...
  {
    if (!(bool) (UnityEngine.Object) this.scene)
      return;
    ...
    else
    {
      if (gachaData.payment_type_id == 5)
      {
        if (gachaData.payment_id.HasValue)
        {
          ...
          int num = playerGachaTicket == null ? 0 : playerGachaTicket.quantity;
          if (gachaData.payment_amount <= num)
            return true;
        }
  ...
  {
    if ((UnityEngine.Object) this.gachaPtErrorPrefab == (UnityEngine.Object) null)
    {
