...
public class Bugu005711Menu : BackButtonMenuBase
{
  ...
  {
    yield return (object) Bugu005711Menu.InitIcon(item, this.LinkItem.transform.parent.Find("dir_thum"));
    this.LinkItem.enabled = false;
    this.item = item;
    this.menu = menu;
    this.Set(item);
  }
  ...
  public static IEnumerator InitIcon(InventoryItem item, Transform dir_thum)
  {
    ...
    itemIcon.supply.button.GetComponent<BoxCollider>().enabled = false;
