...
public class GuideUnitDetailScrollViewJob : MonoBehaviour
{
  ...
  public IEnumerator init(UnitUnit unit, MasterDataTable.UnitJob job)
  {
    ...
    if ((UnityEngine.Object) this.abilityDialogPrefab == (UnityEngine.Object) null)
    {
...
label_31:
  ...
  {
    int index1 = 0;
    UnitFamily[] unitFamilyArray = this.unit.FamiliesWithJob(this.job.ID);
    int num = ((IEnumerable<UnitFamily>) unitFamilyArray).Count<UnitFamily>();
    for (int index2 = 0; index1 < this.dyn_FamilyIcon.Length && index2 < num; ++index2)
    {
      UnitFamily family = unitFamilyArray[index2];
    ...
    for (int index2 = index1; index2 < this.dyn_FamilyIcon.Length; ++index2)
      this.dyn_FamilyIcon[index2].SetActive(false);
  }

  private void setJobAbility()
  {
    ((IEnumerable<GameObject>) this.dir_JobAbility).ForEach<GameObject>((System.Action<GameObject>) (x => x.gameObject.SetActive(false)));
  ...
  {
    if ((UnityEngine.Object) this.abilityDialogPrefab == (UnityEngine.Object) null || this.jobAbilities == null || this.jobAbilities.Length <= range)
      return;
