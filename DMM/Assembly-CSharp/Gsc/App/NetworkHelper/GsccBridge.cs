...
namespace Gsc.App.NetworkHelper
{
  public static class GsccBridge
  {
    ...
    private static WebResponse MakeResponse(BinaryResponse br) => new WebResponse()
    {
      Json = br.jsonData,
    ...
    private static WebResponse MakeResponse(IErrorResponse er) => new WebResponse()
    {
    ...
    private static void OnResponse(WebResponse response, System.Action<WebResponse> callback)
    {
      if (response.Json.ContainsKey("response_for_the_request"))
      {
        object obj1;
        if (response.Json.TryGetValue("gamekit", out obj1))
          GameKit.SyncAchievements(((Dictionary<string, object>) obj1)["achievements"]);
        object obj2;
        if (response.Json.TryGetValue("server_time", out obj2))
          ServerTime.SetSyncServerTime(DateTime.Parse((string) obj2));
        response.Json = (Dictionary<string, object>) response.Json["response_for_the_request"];
      }
      callback(response);
    }

    private static void OnError(WebResponse response, System.Action<WebResponse> callback) => callback(response);
    ...
    public static IEnumerator OnFailed(WebTaskResult result, IWebTask task)
    {
      ...
      {
        WebRequest request = new WebRequest((string) null, WebRequest.RequestMethod.GET, (string) null, (System.Action<WebResponse>) null);
