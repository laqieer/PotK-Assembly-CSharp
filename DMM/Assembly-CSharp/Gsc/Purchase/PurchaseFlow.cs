...
namespace Gsc.Purchase
{
  public class PurchaseFlow
  {
    ...
    public static bool initialized => PurchaseFlow._Handler != null && PurchaseFlow._Handler.initialized;

    public static bool unavailabled => PurchaseFlow._Handler != null && PurchaseFlow._Handler.unavailabled;

    public static ProductInfo[] ProductList => PurchaseFlow._Handler == null ? (ProductInfo[]) null : PurchaseFlow._Handler.ProductList;
    ...
    public static void LaunchFlow<T>(T listener) where T : MonoBehaviour, IPurchaseFlowListener => PurchaseFlow.LaunchFlow<T>(listener, false);
    ...
    public void InputBirthday(int year, int month, int date) => PurchaseFlow._Handler.InputBirthday(year, month, date);

    public void Confirmed(bool isOK) => PurchaseFlow._Handler.Confirmed(isOK);

    public static bool IsEnable(ProductInfo productInfo) => !PurchaseFlow._Handler.HasCreditLimit || (double) productInfo.Price <= (double) PurchaseFlow._Handler.CreditLimit;
  }
}
