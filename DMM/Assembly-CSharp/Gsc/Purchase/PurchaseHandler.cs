...
namespace Gsc.Purchase
{
  public class PurchaseHandler
  {
    ...
    {
      GameObject gameObject = ImmortalObject.Instance.gameObject;
      if (!((UnityEngine.Object) gameObject.GetComponent<PurchaseHandler.Observer>() == (UnityEngine.Object) null))
        return;
    ...
    public void Confirm(ProductInfo product) => ListenerSupport.Call<PurchaseFlow, ProductInfo>(this.enabledInactiveCallback, new Action<PurchaseFlow, ProductInfo>(this.flowListener.Confirm), PurchaseFlow.Instance, product);
    ...
    public class Observer : MonoBehaviour
    {
      ...
      private void Awake() => this.cachedInstanceId = this.GetInstanceID();

      private void OnApplicationFocus(bool focusState)
      {
        if ((!focusState ? 0 : (this.cachedInstanceId == this.GetInstanceID() ? 1 : 0)) == 0)
          return;
        PurchaseFlow.Resume();
      }
    }
