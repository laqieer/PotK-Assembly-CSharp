...
namespace Gsc.Auth.GAuth.GAuth
{
  public class Session : ISession
  {
    ...
    public Session(string envName, IAccountManager accountManager)
    {
      ...
      ImmortalObject.Instance.gameObject.AddComponent<Session.AccessTokenChecker>();
    }
    ...
    public virtual bool CanRefreshToken(System.Type requestType) => !requestType.Equals(typeof (Gsc.Auth.GAuth.GAuth.API.Request.AccessToken));

    public virtual IRefreshTokenTask GetRefreshTokenTask() => (IRefreshTokenTask) new Session.RefreshTokenTask(this);
    ...
    public class AccessTokenChecker : MonoBehaviour
    {
      ...
      {
        if (!(!this.isRunning & (focusState && this.cachedInstanceId == this.GetInstanceID())) || WebQueue.defaultQueue.isPause)
          return;
