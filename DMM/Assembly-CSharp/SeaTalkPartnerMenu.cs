...
public class SeaTalkPartnerMenu : BackButtonMenuBase
{
    ...
    Sea030HomeMenu seaHomeMenu)
  {
    ...
    for (int index = 0; index < playerTalkPartnerArray.Length; ++index)
    {
      ...
      GameObject go = this.itemPrefab.Clone(this.scrollView.transform);
    ...
    {
      this.scrollBoxColider.enabled = this.dragScrollView.enabled = false;
      foreach (Component component in this.scrollView.transform)
        component.GetComponent<UIDragScrollView>().enabled = false;
    }
  ...
  public void OnMenu() => Singleton<CommonRoot>.GetInstance().GetSeaHeaderComponent().OpenMenuForTalk();
