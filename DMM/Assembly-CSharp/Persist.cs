...
public static class Persist
{
  ...
  public abstract class ISaveUnit
  {
    ...
    private static string Sanitize(string str) => str.Select<char, char>((Func<char, char>) (c => !Persist.ISaveUnit.permitChars.Contains(c) ? '_' : c)).ToStringForChars();

    public string FilePath => Path.Combine(PersistentPath.Value, Persist.ISaveUnit.Sanitize(this.fileName));

    public string FileFallbackPath => Path.Combine(PersistentPath.Fallback, Persist.ISaveUnit.Sanitize(this.fileName));
  ...
  public class CharacterQuestFilterInfo
  {
    ...
    private void setDefault() => this.groupIDs = Persist.CharacterQuestFilterInfo.createMapUnitGroupHead();

    public static Dictionary<UnitGroupHead, List<int>> createMapUnitGroupHead() => new Dictionary<UnitGroupHead, List<int>>()
    {
      [UnitGroupHead.group_all] = new List<int>()
      {
        2
  ...
  public class Tutorial
  {
    ...
    public int DuringSeconds() => (int) (DateTime.Now - Persist.tutorial.Data.ChangeNextPageAt).TotalSeconds;
  ...
  public class ColosseumDeckOrganized
  {
    ...
    public DeckInfo getSelectedDeck() => Persist.DeckUtility.getSelectedDeck(this.isCustom, this.number, this.customNumber);
  }
  ...
  public class PvpUnitPositions
  {
    ...
    public bool check(int stageId, int nbPositions, int order) => this.stageId == stageId && this.positions.Length == nbPositions && this.order == order;

    public bool check(MpStage stage, int nbPositions, int order) => this.check(stage.stage_id, nbPositions, order);
    ...
    public static Persist.PvpUnitPositions getData(
      MpStage stage,
      int nbPositions,
      int order)
    {
  ...
  public class EventStoryPlay
  {
    ...
    private void setPlayedIDdict() => this.playedIDdict = new Dictionary<Persist.EventStoryPlay.ControlType, List<int>>()
    {
      {
    ...
    private bool checkServerControlScene(string sceneName) => sceneName == "mypage" || sceneName == "quest002_17" || sceneName == "quest002_SideStory_List";
    ...
    public bool isExistEventScript(string sceneName, int arg1) => this.reserveIDList.Select<int, StoryPlaybackEventPlay>((Func<int, StoryPlaybackEventPlay>) (x => MasterData.StoryPlaybackEventPlay[x])).Any<StoryPlaybackEventPlay>((Func<StoryPlaybackEventPlay, bool>) (x =>
    {
    ...
    public bool PlayEventScript(string sceneName, int arg1, bool needEndLoadingLayer = false)
    {
      ...
      if (playbackEventPlay != null)
      {
        ...
        Story0093Scene.changeScene(true, playbackEventPlay.script_id, new bool?(Singleton<NGGameDataManager>.GetInstance().IsSea), needEndLoadingLayer ? action : (System.Action) null);
      }
  ...
  public class GuildSettingInfo
  {
    ...
    private void resetBadge()
    {
      ...
      for (int index = 0; index < Enum.GetValues(typeof (GuildUtil.GuildBadgeInfoType)).Length; ++index)
        this.setBadgeState((GuildUtil.GuildBadgeInfoType) index, false);
    }
