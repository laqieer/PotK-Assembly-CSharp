...
public class TextTips : CommonTips
{
  ...
  private IEnumerator LoadTipsPrefabs()
  {
    ...
    if (MasterData.TipsTextTips.TryGetValue(textTips.makeIdList[0], out tipsTextTips))
    {
      ...
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      textTips.backgroundTips = UnityEngine.Object.Instantiate<GameObject>(textTips.backgroundTipsPrefab, textTips.backgroundTipsPosition);
    ...
    for (int index = 0; index < textTips.makeIdList.Count; ++index)
    {
      ...
      {
        GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(textTips.textTipsPrefab, textTips.textTipsPosition);
  ...
  private IEnumerator AlphaBlend(UI2DSprite sprite)
  {
    ...
    do
    {
      ...
      sprite.alpha += 1f * Time.deltaTime;
    }
    while ((double) sprite.alpha < 1.0);
    sprite.alpha = 1f;
  }
