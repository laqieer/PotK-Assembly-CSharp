...
public class PopupRecommendPartUnitType : PopupRecommendPart
{
  ...
  public override IEnumerator doInitialize(PlayerUnit playerUnit, UnitUnit target)
  {
    ...
    if (!((IEnumerable<UnitSkill>) MasterData.UnitSkillList).Any<UnitSkill>((Func<UnitSkill, bool>) (x => x.unit_UnitUnit == target.ID && (uint) x.unit_type > 0U)))
    {
      recommendPartUnitType.gameObject.SetActive(false);
    }
    else
    {
      recommendPartUnitType.playerUnit_ = playerUnit;
      recommendPartUnitType.target_ = target;
      foreach (UnityEngine.Object @object in recommendPartUnitType.dicSkills_.Where<KeyValuePair<UnitTypeEnum, Tuple<BattleskillSkill, GameObject>[]>>((Func<KeyValuePair<UnitTypeEnum, Tuple<BattleskillSkill, GameObject>[]>, bool>) (x => x.Value != null && (uint) x.Value.Length > 0U)).SelectMany<KeyValuePair<UnitTypeEnum, Tuple<BattleskillSkill, GameObject>[]>, GameObject>((Func<KeyValuePair<UnitTypeEnum, Tuple<BattleskillSkill, GameObject>[]>, IEnumerable<GameObject>>) (y => ((IEnumerable<Tuple<BattleskillSkill, GameObject>>) y.Value).Select<Tuple<BattleskillSkill, GameObject>, GameObject>((Func<Tuple<BattleskillSkill, GameObject>, GameObject>) (z => z.Item2)))))
  ...
  private void setTab(UnitTypeEnum type)
  {
    for (int index = 0; index < this.tabs_.Length; ++index)
    {
      ...
      else if (tab.button.gameObject.activeSelf)
        tab.button.isEnabled = true;
    }
  }
  ...
  private IEnumerator doChange(UnitTypeEnum type)
  {
    ...
    {
      if ((UnityEngine.Object) this.iconPrefab_ == (UnityEngine.Object) null)
      {
  ...
  private void setEventClickedSkill(UIButton btn, UnitTypeEnum unitType, int index) => EventDelegate.Set(btn.onClick, (EventDelegate.Callback) (() =>
  {
    if ((UnityEngine.Object) this.menu.skillDetailPrefab == (UnityEngine.Object) null)
      return;
