...
public class Quest0529Menu : UnitSelectMenuBase
{
  ...
  {
    if ((UnityEngine.Object) this.supplyIcon == (UnityEngine.Object) null)
    {
  ...
  private void UpdateSelectUnitText() => this.PossessionUnit.SetTextLocalize(Consts.Format(Consts.GetInstance().UNIT_054_6_8_Possession, (IDictionary) new Hashtable()
  {
    {
      (object) "select",
  ...
  public IEnumerator Initialize(PlayerUnit[] playerUnits, int maxNum)
  {
    ...
    if (maxNum == -1)
    {
      ...
      quest0529Menu.selectPlayerUnitsCache = ((IEnumerable<EarthCharacter>) Singleton<EarthDataManager>.GetInstance().characters).Where<EarthCharacter>((Func<EarthCharacter, bool>) (x => (uint) x.battleIndex > 0U)).ToArray<EarthCharacter>();
    }
    else
    {
      quest0529Menu.isEventQuest = true;
      quest0529Menu.SelectMax = maxNum;
      quest0529Menu.selectPlayerUnitsCache = ((IEnumerable<EarthCharacter>) Singleton<EarthDataManager>.GetInstance().characters).Where<EarthCharacter>((Func<EarthCharacter, bool>) (x => (uint) x.eventQuestBattleIndex > 0U)).ToArray<EarthCharacter>();
    }
    ...
    quest0529Menu.InitializeInfo((IEnumerable<PlayerUnit>) playerUnits, (IEnumerable<PlayerMaterialUnit>) null, (Persist<Persist.UnitSortAndFilterInfo>) null, false, false, false, false, false, new System.Action(quest0529Menu.\u003CInitialize\u003Eb__11_2));
  ...
  {
    ItemIcon itemIcon = this.SuppleIconPositions[pos].GetComponentInChildren<ItemIcon>();
