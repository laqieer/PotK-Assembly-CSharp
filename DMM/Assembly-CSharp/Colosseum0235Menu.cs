...
public class Colosseum0235Menu : BackButtonMenuBase
{
  ...
  public IEnumerator Initialize(Colosseum0235Scene.Param param)
  {
    ...
    IEnumerator e = Singleton<CommonRoot>.GetInstance().GetColosseumHeaderComponent().SetInfo(CommonColosseumHeader.BtnMode.Back, new System.Action(colosseum0235Menu.IbtnBack));
    while (e.MoveNext())
    ...
    for (int i = 1; i < colosseum0235Menu.cellsPerPage; ++i)
    {
      ...
      {
        int num1 = i + 1 < rankArray.Length ? rankArray[i + 1].to_point : 0;
        int num2 = i == 0 ? (rankArray[0].ID <= param.maxRank ? rankArray[0].to_point : -1) : (rankArray[i - 1].ID <= param.maxRank + 1 ? rankArray[i - 1].to_point : -1);
        e = component.Set(rankArray[i], rankArray[i].ID == param.nowId, num1 + 1, num2 + 1, gearPrefab, unitPrefab, uniquePrefab, new System.Action<ColosseumRank, int, int>(colosseum0235Menu.IbtnRankDetail));
        while (e.MoveNext())
          yield return e.Current;
        e = (IEnumerator) null;
      }
