...
public class Unit004ExtraSkillListMenuBase : BackButtonMenuBase
{
  ...
  private Dictionary<int, UnityEngine.Sprite> spriteCache = new Dictionary<int, UnityEngine.Sprite>();
  ...
  protected Future<GameObject> GetSortAndFilterPopupGameObject() => new ResourceObject("Prefabs/popup/popup_Extraskill_Sort__anim_popup01").Load<GameObject>();

  public virtual Persist<Persist.ExtraSkillSortAndFilterInfo> GetPersist() => (Persist<Persist.ExtraSkillSortAndFilterInfo>) null;
  ...
  protected int GetItemListCnt(List<PlayerAwakeSkill> itemList) => itemList.Count<PlayerAwakeSkill>();
  ...
  protected virtual InventoryExtraSkill CreateInventoryExtraSkill(
    PlayerAwakeSkill skill)
  {
  ...
  protected void CreateExtraSkillIcon()
  {
    ...
    for (int index = 0; index < Mathf.Min(ItemIcon.MaxValue, this.InventoryExtraSkills.Count); ++index)
      this.AllExtraSkillIcon.Add(UnityEngine.Object.Instantiate<GameObject>(this.ItemIconPrefab).GetComponent<ExtraSkillIcon>());
  ...
  private void ShowSortAndFilterPopup()
  {
    if (!Singleton<PopupManager>.GetInstance().isOpen)
    {
      ...
      sortAndFilter.SortFilterSkillNum = (System.Action) (() => sortAndFilter.SetSkillNum(this.InventoryExtraSkills.FilterBy(this.filter).ToList<InventoryExtraSkill>(), this.InventoryExtraSkills));
    ...
    SortAndFilter.SORT_TYPE_ORDER_BUY order)
  {
    ...
    this.scroll.Reset();
    this.AllExtraSkillIcon.ForEach((System.Action<ExtraSkillIcon>) (x =>
    {
    ...
    {
      this.scroll.Add(this.AllExtraSkillIcon[index].gameObject, this.iconWidth, this.iconHeight);
      this.AllExtraSkillIcon[index].gameObject.SetActive(true);
    }
    this.InventoryExtraSkills.ForEach((System.Action<InventoryExtraSkill>) (v => v.icon = (ExtraSkillIcon) null));
  ...
  private void ScrollUpdate()
  {
    ...
    do
    {
      ...
      foreach (GameObject gameObject in this.scroll)
      {
        ...
        int? nullable = this.DisplaySkills.FirstIndexOrNull<InventoryExtraSkill>((Func<InventoryExtraSkill, bool>) (v => (UnityEngine.Object) v.icon != (UnityEngine.Object) null && (UnityEngine.Object) v.icon.gameObject == (UnityEngine.Object) item));
  ...
  private void ResetItemIcon(int allItemIdx) => this.AllExtraSkillIcon[allItemIdx].gameObject.SetActive(false);

  private IEnumerator CreateItemIconRange(int max)
  {
    Singleton<CommonRoot>.GetInstance().isTouchBlock = true;
    for (int index = 0; index < this.AllExtraSkillIcon.Count; ++index)
      this.AllExtraSkillIcon[index].gameObject.SetActive(false);
    for (int i = 0; i < max; ++i)
    {
  ...
  private IEnumerator CreateItemIcon(int inventoryItemIdx, int allItemIdx)
  {
    ...
    this.DisplaySkills.Where<InventoryExtraSkill>((Func<InventoryExtraSkill, bool>) (a => (UnityEngine.Object) a.icon == (UnityEngine.Object) skillIcon)).ForEach<InventoryExtraSkill>((System.Action<InventoryExtraSkill>) (b => b.icon = (ExtraSkillIcon) null));
  ...
  private void CreateItemIconCache(int inventoryItemIdx, int allItemIdx)
  {
    ...
    this.DisplaySkills.Where<InventoryExtraSkill>((Func<InventoryExtraSkill, bool>) (a => (UnityEngine.Object) a.icon == (UnityEngine.Object) itemIcon)).ForEach<InventoryExtraSkill>((System.Action<InventoryExtraSkill>) (b => b.icon = (ExtraSkillIcon) null));
  ...
  public UnityEngine.Sprite SpriteCache(BattleskillSkill skill) => this.IsCache(skill) ? this.spriteCache[skill.ID] : (UnityEngine.Sprite) null;

  public void ClearCache() => this.spriteCache.Clear();

  public void AllDeselect() => this.InventoryExtraSkills.ForEach((System.Action<InventoryExtraSkill>) (x => x.select = false));
