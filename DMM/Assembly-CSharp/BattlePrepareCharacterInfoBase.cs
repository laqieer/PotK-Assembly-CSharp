...
public abstract class BattlePrepareCharacterInfoBase : NGBattleMenuBase
{
    ...
    BattleUI04Menu baseMenu)
  {
    ...
    this.ailmentsDetailBtn.isEnabled = BattleFuncs.getInvestSkills(this.currentUnit).Any<BattleFuncs.InvestSkill>();
  ...
  private void InitGearIcon(BL.Unit unit)
  {
    ...
    this.iconGear.transform.Clear();
    GameObject gameObject1 = this.gearIconPrefab.Clone(this.iconGear.transform);
    gameObject1.GetComponent<UIWidget>().depth = this.iconGear.depth + 1;
    ...
    if ((UnityEngine.Object) this.iconSecondGear != (UnityEngine.Object) null)
    {
      this.iconSecondGear.transform.Clear();
      if (equippedGear2 != (PlayerItem) null)
      {
        GameObject gameObject2 = this.gearIconPrefab.Clone(this.iconSecondGear.transform);
        gameObject2.GetComponent<UIWidget>().depth = this.iconSecondGear.depth + 1;
    ...
    else
    {
      if (!(equippedGear == (PlayerItem) null) || !(equippedGear2 != (PlayerItem) null))
        return;
      this.tweenAlpha_first_weapon.GetComponent<UIWidget>().alpha = 0.0f;
  ...
  public int getCurrentAttackOriginIndex() => ((IEnumerable<AttackStatus>) this.originAttacks).FirstIndexOrNull<AttackStatus>((Func<AttackStatus, bool>) (x => x == this.currentAttack)).Value;
  ...
  {
    if (!((UnityEngine.Object) this.TxtDsActivationRate != (UnityEngine.Object) null))
      return;
    if (this.getCurrentAttack() == null)
    {
      this.TxtDsActivationRate.SetTextLocalize("-");
      this.iconDs.alpha = 0.0f;
    }
    else
    {
  ...
  private IEnumerator LoadDuelSkillIcon(BL.Skill skill)
  {
    ...
    {
      Future<UnityEngine.Sprite> ft = battleskillSkill.LoadBattleSkillIcon();
