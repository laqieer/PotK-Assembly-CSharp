...
public class Gacha0063TicketItem : MonoBehaviour
{
  ...
  public void Init(GachaModule gachaModule, GachaModuleGacha gacha, Gacha0063Menu menu)
  {
    ...
    this.mDetailButton.isEnabled = false;
    ...
    {
      this.mPullObject.isEnabled = true;
      this.mPullDisableObject.SetActive(false);
    }
    else
    {
      this.mPullObject.isEnabled = false;
  ...
  public IEnumerator LoadResource(GameObject detailPopup)
  {
    ...
    {
      Future<UnityEngine.Sprite> thumF = MasterData.GachaTicket[this.targetGacha.payment_id.Value].LoadSpriteOrDefault();
      ...
      {
        this.mDetailButton.isEnabled = true;
  ...
  private void SetAmountCount(int amountCount, int haveCount)
  {
    ...
    {
      this.mTicketProgressLabel.gameObject.SetActive(true);
      this.mTicketProgressLabelRed.gameObject.SetActive(false);
      this.mTicketProgressLabel.SetTextLocalize(haveCount);
    }
    else
    {
      this.mTicketProgressLabel.gameObject.SetActive(false);
      this.mTicketProgressLabelRed.gameObject.SetActive(true);
      this.mTicketProgressLabelRed.SetTextLocalize(haveCount);
    }
  }

  private void SetNumberSprite(int number, UISprite sprite) => sprite.spriteName = Gacha0063TicketItem.NumberSpriteNameList[number];

  private void SetLimitDate(DateTime? date)
  {
    if (date.HasValue)
    {
      this.mNonLimitDateLabel.gameObject.SetActive(false);
      this.mLimitDateLabel.gameObject.SetActive(true);
      this.mLimitDateLabel.SetTextLocalize(date.Value.ToString(Gacha0063TicketItem.DateTimeFormat));
    }
    else
    {
      this.mNonLimitDateLabel.gameObject.SetActive(true);
      this.mLimitDateLabel.gameObject.SetActive(false);
    }
  }

  private void SetDetailButton(GachaModuleGacha gacha, GameObject detailPopup) => EventDelegate.Add(this.mDetailButton.onClick, (EventDelegate.Callback) (() =>
  {
  ...
  private IEnumerator OpenDetailPopup(GachaModuleGacha gacha, GameObject detailPopup)
  {
    ...
    {
      Debug.LogError((object) ("Can't get popup object from PopupManager: " + gacha0063TicketItem.gameObject.name));
    ...
    else
    {
      ...
      {
        Debug.LogError((object) ("Can't get menu object from popup object: " + gacha0063TicketItem.gameObject.name));
        Singleton<CommonRoot>.GetInstance().isLoading = false;
        Singleton<CommonRoot>.GetInstance().loadingMode = 0;
      }
      else
