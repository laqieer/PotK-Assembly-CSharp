...
public class Guild0281FacilityInfoController : MonoBehaviour
{
  ...
  public void OnScaffoldInvestmentButtonClicked() => this.OpenFacilityInvestmentDialog(GuildBaseType.scaffold);

  public void OnWallsInvestmentButtonClicked() => this.OpenFacilityInvestmentDialog(GuildBaseType.walls);

  public void OnTowerInvestmentButtonClicked() => this.OpenFacilityInvestmentDialog(GuildBaseType.tower);
  ...
  private IEnumerator StartLvupEffect(Transform target)
  {
    ...
    while ((double) animator.GetCurrentAnimatorStateInfo(0).normalizedTime < 1.0);
  ...
  private string GetFormattedStatusValue(int value, bool isValueOnly = false) => !isValueOnly ? string.Format("{0}%UP", (object) value) : value.ToString();

  public void InvestFacility(GuildHq selectedFacility) => this.StartCoroutine(this.InvestFacilityCoroutine(selectedFacility));
  ...
  private IEnumerator InvestFacilityCoroutine(GuildHq selectedFacility)
  {
    ...
    Future<WebAPI.Response.GuildBaseInvest> future = WebAPI.GuildBaseInvest((int) selectedFacility.base_type, selectedFacility.rank, (System.Action<WebAPI.Response.UserError>) (e =>
    {
      ...
      ModalWindow.Show(Consts.GetInstance().Guild0281MENU_FACILITY_INVESTMENT_ERROR_POPUP_TITLE, Consts.GetInstance().Guild0281MENU_FACILITY_INVESTMENT_ERROR_POPUP_CONTENT, (System.Action) (() =>
      {
