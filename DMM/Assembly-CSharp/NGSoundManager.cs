...
public class NGSoundManager : Singleton<NGSoundManager>
{
  ...
  protected override void Finlaize()
  {
    if (!this.isInitialize)
      return;
    ((IEnumerable<NGSoundManager.BgmPlayer>) this.bgmPlayer).ForEach<NGSoundManager.BgmPlayer>((System.Action<NGSoundManager.BgmPlayer>) (v => v.player.Dispose()));
    ((IEnumerable<CriAtomExPlayback?>) this.bgmPlayback).ForEach<CriAtomExPlayback?>((System.Action<CriAtomExPlayback?>) (v => v = new CriAtomExPlayback?()));
    ((IEnumerable<CriAtomExPlayer>) this.sePlayer).ForEach<CriAtomExPlayer>((System.Action<CriAtomExPlayer>) (v => v.Dispose()));
    ((IEnumerable<CriAtomExPlayer>) this.voicePlayer).ForEach<CriAtomExPlayer>((System.Action<CriAtomExPlayer>) (v => v.Dispose()));
  ...
  public void CheckInitialize(bool forcingCompleted = false)
  {
    if (!this.isInitialize)
    {
      ...
      this.audioObject.transform.position = this.transform.position;
      this.audioObject.transform.parent = this.transform;
    ...
    try
    {
      if (!this.isTitleScene && ((!ResourceDownloader.Completed ? 0 : (ResourceDownloader.Error == null ? 1 : 0)) | (forcingCompleted ? 1 : 0)) != 0)
      {
        ...
        CriAtom.AddCueSheet("BB", Singleton<ResourceManager>.GetInstance().ResolveStreamingAssetsPath(this.platform + "/" + this.afterSeName + "_acb"), Singleton<ResourceManager>.GetInstance().ResolveStreamingAssetsPath(this.platform + "/" + this.afterSeName + "_awb"));
        CriAtom.AddCueSheet("BB2", Singleton<ResourceManager>.GetInstance().ResolveStreamingAssetsPath(this.platform + "/" + this.afterSeName + "_2_acb"), Singleton<ResourceManager>.GetInstance().ResolveStreamingAssetsPath(this.platform + "/" + this.afterSeName + "_2_awb"));
  ...
  {
    ((IEnumerable<NGSoundManager.BgmPlayer>) this.bgmPlayer).ForEach<NGSoundManager.BgmPlayer>((System.Action<NGSoundManager.BgmPlayer>) (v =>
    {
      v.player.SetVolume(this.volume.mute ? 0.0f : this.volume.bgmVolume);
      v.player.UpdateAll();
    }));
    ((IEnumerable<CriAtomExPlayer>) this.sePlayer).ForEach<CriAtomExPlayer>((System.Action<CriAtomExPlayer>) (v =>
    {
      v.SetVolume(this.volume.mute ? 0.0f : this.volume.seVolume);
      v.UpdateAll();
    }));
    ((IEnumerable<CriAtomExPlayer>) this.voicePlayer).ForEach<CriAtomExPlayer>((System.Action<CriAtomExPlayer>) (v =>
    {
    ...
    float fadeOutTime = 0.5f)
  {
    ...
    if (ngSoundManager.initializeState != NGSoundManager.InitializeState.DLC_BEFORE || ngSoundManager.bgmAcb.Exists(clip))
    {
      if (channel == -1)
      {
        int? nullable = ((IEnumerable<NGSoundManager.BgmPlayer>) ngSoundManager.bgmPlayer).FirstIndexOrNull<NGSoundManager.BgmPlayer>((Func<NGSoundManager.BgmPlayer, bool>) (v => v.player.GetStatus() == CriAtomExPlayer.Status.Stop || v.player.GetStatus() == CriAtomExPlayer.Status.PlayEnd));
  ...
  public void SetNextBGMBlock(int channel, int blockID)
  {
    ...
    if (criAtomExPlayback.GetCurrentBlockIndex() == -1)
      return;
    criAtomExPlayback = nullable.Value;
    criAtomExPlayback.SetNextBlockIndex(blockID);
  }
  ...
  public void StopBgm(int channel = -1, float time = 0.5f)
  {
    ...
    if (channel < 0)
    {
      ((IEnumerable<NGSoundManager.BgmPlayer>) this.bgmPlayer).ForEach<NGSoundManager.BgmPlayer>((System.Action<NGSoundManager.BgmPlayer>) (v => v.Stop(time)));
      ((IEnumerable<CriAtomExPlayback?>) this.bgmPlayback).ForEach<CriAtomExPlayback?>((System.Action<CriAtomExPlayback?>) (v => v = new CriAtomExPlayback?()));
    }
    else
    {
  ...
  public string GetBgmName(int channel = 0) => !this.isInitialize || channel < 0 || channel >= this.bgmPlayer.Length ? "" : this.bgmPlayer[channel].nowName;

  public string GetCueName(int channel = 0) => !this.isInitialize || channel < 0 || channel >= this.bgmPlayer.Length ? "" : this.bgmPlayer[channel].nowCueName;
  ...
  public int PlaySe(string clip, bool isLoop = false, float delay = 0.0f, int channel = -1)
  {
    ...
    for (int i = 0; i < this.seAcbList.Count; i++)
    {
      if (this.seAcbList[i].Exists(clip))
        return this.PlaySe(isLoop, delay, channel, true, (System.Action<CriAtomExPlayer>) (player => player.SetCue(this.seAcbList[i], clip)));
    }
    return -1;
  }
  ...
  private int PlaySe(
    ...
    int channel,
    bool busEffect,
    System.Action<CriAtomExPlayer> action)
  {
  ...
  public int playVoiceByID(UnitVoicePattern voicePattern, int id, int channel = -1, float delay = 0.0f) => voicePattern == null ? -1 : this.PlayVoice(voicePattern.file_name, id, voicePattern.SelectorLabel, channel, delay);
    ...
    float delay = 0.0f)
  {
    ...
    if (fileName == "VO_9999" && name.Contains("durin"))
      id = "VO_9999_durin";
    return this.PlayVoice(id, delay, channel, true, selectorLabel, (System.Action<CriAtomExPlayer, CriAtomExAcb>) ((player, acb) => player.SetCue(acb, name)));
  }

  public int PlayVoice(string fileName, int id, string selectorLabel, int channel = -1, float delay = 0.0f) => this.PlayVoice(fileName, delay, channel, true, selectorLabel, (System.Action<CriAtomExPlayer, CriAtomExAcb>) ((player, acb) => player.SetCue(acb, id)));
  ...
  private int PlayVoice(
    ...
    bool busEffect,
    string selectorLabel,
    System.Action<CriAtomExPlayer, CriAtomExAcb> action)
  {
  ...
  public int PlayVoicePriorityFirst(UnitUnit unit, int id, int channel, float delay = 0.0f) => !this.IsVoicePlaying(channel) ? this.playVoiceByID(unit.unitVoicePattern, id, channel, delay) : -1;
  ...
  public bool LoadVoiceData(string fileName) => this.checkSoundData(this.voiceCueSheetName, this.maxStreamVoice, fileName);
  ...
  private void PauseSources(CriAtomExPlayer[] sources, bool enable) => ((IEnumerable<CriAtomExPlayer>) sources).ForEach<CriAtomExPlayer>((System.Action<CriAtomExPlayer>) (v => v.Pause(enable)));

  private void PauseSources(NGSoundManager.BgmPlayer[] sources, bool enable) => ((IEnumerable<NGSoundManager.BgmPlayer>) sources).ForEach<NGSoundManager.BgmPlayer>((System.Action<NGSoundManager.BgmPlayer>) (v => v.player.Pause(enable)));
  ...
  private void stop(CriAtomExPlayer[] playerList, int channel, float time)
  {
    if (channel < 0)
    {
      ((IEnumerable<CriAtomExPlayer>) playerList).ForEach<CriAtomExPlayer>((System.Action<CriAtomExPlayer>) (v =>
      {
  ...
  private bool IsFile(string s) => !string.IsNullOrEmpty(s) && new FileInfo(Path.Combine(Common.streamingAssetsPath, s)).Exists;
  ...
  public void SetVolumeBgm(float volume) => ((IEnumerable<NGSoundManager.BgmPlayer>) this.bgmPlayer).ForEach<NGSoundManager.BgmPlayer>((System.Action<NGSoundManager.BgmPlayer>) (v =>
  {
    v.player.SetVolume(volume);
    v.player.UpdateAll();
  }));

  public void SetVolumeSe(float volume) => ((IEnumerable<CriAtomExPlayer>) this.sePlayer).ForEach<CriAtomExPlayer>((System.Action<CriAtomExPlayer>) (v =>
  {
    v.SetVolume(volume);
    v.UpdateAll();
  }));

  public void SetVolumeVoice(float volume) => ((IEnumerable<CriAtomExPlayer>) this.voicePlayer).ForEach<CriAtomExPlayer>((System.Action<CriAtomExPlayer>) (v =>
  {
    v.SetVolume(volume);
    v.UpdateAll();
  }));
  ...
  public void playBGM(AudioClip clip, float startTime = 0.1f) => this.PlayBgm(clip.name);
  ...
  private IEnumerator BgmCrossFade(
    NGSoundManager.BgmPlayer player,
    float aisac,
    float duration)
  {
  ...
  public void crossFadeCurrentBGM(float duration, float aisac) => this.StartCoroutine(this.BgmCrossFade(this.bgmPlayer[0], aisac, duration));
  ...
  public void playBGMWithCrossFade(AudioClip clip, float duration, float startTime = 0.1f) => this.playBGMWithCrossFade(clip.name, duration, startTime);

  public void playBGMWithCrossFade(string clip, float duration, float startTime = 0.1f) => this.PlayBgm(clip, fadeInTime: duration, fadeOutTime: duration);

  public void playBGMWithCombineTimeFade(AudioClip clip, float duration, float startTime = 0.1f) => this.playBGMWithCombineTimeFade(clip.name, duration, startTime);
  ...
  public int playSE(AudioClip clip, bool isLoop = false, float delay = 0.0f) => this.PlaySe(clip.name, isLoop, delay);

  public int playSE(string clip, bool isLoop = false, float delay = 0.0f, int seChannel = -1) => this.PlaySe(clip, isLoop, delay, seChannel);
  ...
  public bool IsEffectiveCueID(string name, int id)
  {
    ...
    return cueSheet != null && cueSheet.acb.Exists(id) && cueSheet.acb.GetCueInfo(id, out info) && info.length > 20L;
  }
