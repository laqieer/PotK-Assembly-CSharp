...
public class GearQuestScrollItem : MonoBehaviour
{
  ...
  public static Future<GameObject> createLoader() => new ResourceObject("Prefabs/unit/dir_DropQuest_Menu").Load<GameObject>();
  ...
  public void setButton(System.Action<QuestConverterData> onSelected, bool bDisabledSelect)
  {
    if (!this.objDisabled_.activeSelf && onSelected != null)
    {
      ...
      if (!bDisabledSelect)
        ...
        this.button_.GetComponent<BoxCollider>().enabled = false;
    }
    else
      this.button_.GetComponent<BoxCollider>().enabled = false;
  }
