...
public class GuildMemberLogDialogController : MonoBehaviour
{
  ...
  public IEnumerator Initialize()
  {
    ...
    dialogController.grid.transform.localPosition = new Vector3(0.0f, dialogController.scrollView.panel.finalClipRegion.w / 2f, 0.0f);
    ...
    dialogController.scrollView.transform.localPosition = Vector3.zero;
    dialogController.scrollView.panel.clipOffset = new Vector2(-dialogController.scrollView.transform.localPosition.x, -dialogController.scrollView.transform.localPosition.y);
    dialogController.scrollView.UpdateScrollbars(true);
    if ((UnityEngine.Object) dialogController.memberLogItemPrefab == (UnityEngine.Object) null)
    {
    ...
    {
      GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(dialogController.memberLogItemPrefab);
  ...
  private IEnumerator UpdateLatestMemberLogData()
  {
    ...
    if (!dialogController.isUpdatingMemberLogData)
    {
      ...
      Future<WebAPI.Response.GuildlogMemberShowLatest> future = WebAPI.GuildlogMemberShowLatest(dialogController.playerID, new System.Action<WebAPI.Response.UserError>(dialogController.\u003CUpdateLatestMemberLogData\u003Eb__19_0));
  ...
  private IEnumerator UpdateEarlierMemberLogData()
  {
    ...
    if (!dialogController.isUpdatingMemberLogData)
    {
      ...
      Singleton<CommonRoot>.GetInstance().loadingMode = 1;
      // ISSUE: reference to a compiler-generated method
      Future<WebAPI.Response.GuildlogMemberShowPast> future = WebAPI.GuildlogMemberShowPast(num.ToString(), dialogController.playerID, new System.Action<WebAPI.Response.UserError>(dialogController.\u003CUpdateEarlierMemberLogData\u003Eb__20_0));
      IEnumerator e = future.Wait();
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
  ...
  {
    this.headPlaceholder.transform.localPosition += new Vector3(0.0f, (float) (this.memberLogItemHeight * (addedOldItemCount - deletedOldItemCount)), 0.0f);
    ...
    {
      this.scrollView.transform.localPosition = new Vector3(0.0f, -(this.tailPlaceholder.transform.localPosition.y + this.scrollView.panel.finalClipRegion.w), 0.0f);
      this.scrollView.panel.clipOffset = new Vector2(0.0f, -this.scrollView.transform.localPosition.y);
    }
  ...
  private void UpdateMemberLogItemList()
  {
    ...
    for (int index = num1; index <= num2 && index <= this.displayingMemberLogDataList.Count - 1; ++index)
    {
      ...
      bufferedMemberLogItem.transform.SetParent(this.grid.transform);
  ...
  public void OnPullDownMemberLogListFromTop() => this.updateMemberLogDataCoroutine = this.StartCoroutine(this.UpdateEarlierMemberLogData());
}
