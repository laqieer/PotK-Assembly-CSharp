...
public class EpTaskEnemyBattle : ExploreTask, IEpBattleTask
{
  ...
  public override IEnumerator UpdateAsync()
  {
    ...
    if (__nonvirtual (epTaskEnemyBattle.IsWin()))
    {
      ...
      data.AddLog(string.Format("{0}に敗北した", (object) enemyName), (Color) new Color32((byte) 250, (byte) 0, (byte) 0, byte.MaxValue));
  ...
  public void SetBattleCache() => Singleton<ExploreDataManager>.GetInstance().SetLastDuelCache(this.mEnemy, this.mDuelResult);
  ...
  public override long OnBackGroundWork(long calcTime)
  {
    ...
    if (this.IsWin())
    {
      ...
      instance.AddLog(string.Format("{0}に敗北した", (object) name), (Color) new Color32((byte) 250, (byte) 0, (byte) 0, byte.MaxValue));
  ...
  public override void OnContinue(long restReqTime, long takeOverTime) => this.mReqiredTime = restReqTime;
}
