...
public class CorpsQuestBattleResultScene : NGSceneBase
{
  ...
  {
    this.TouchToNext.gameObject.SetActive(false);
    ...
    WebAPI.Response.QuestCorpsBattleFinish result)
  {
    ...
    battleResultScene.Sequences.Enqueue((ResultMenuBase) battleResultScene.GetComponent<CorpsQuestResultMenu>());
    battleResultScene.Sequences.Enqueue((ResultMenuBase) battleResultScene.GetComponent<BattleUI05RewardMenu>());
    if (result.stage_clear_rewards.Length != 0)
      battleResultScene.Sequences.Enqueue((ResultMenuBase) battleResultScene.GetComponent<BattleUI05ClearBonusMenu>());
  ...
  private IEnumerator RunMenus()
  {
    while (this.Sequences.Count > 0)
    {
      ...
      this.TouchToNext.gameObject.SetActive(true);
    ...
    this.TouchToNext.gameObject.SetActive(true);
    this.TouchToNext.isEnabled = false;
    ...
    if (MasterData.CorpsStage.TryGetValue(stageId, out corpsStage))
    {
      ...
      if (backGround != null)
      {
        ...
        while (e.MoveNext())
          yield return e.Current;
        e = (IEnumerator) null;
        if ((Object) bgSpriteF.Result != (Object) null)
        {
  ...
  public void OnTouchToNext()
  {
    ...
    if ((Object) this.PlayingMenu != (Object) null)
