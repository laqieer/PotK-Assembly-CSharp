...
public class Friend00811Menu : BackButtonMenuBase
{
  ...
  private IEnumerator FriendApplyAsync()
  {
    ...
    if (friend00811Menu.target_friend_ids.Count >= 1)
    {
      ...
      else
      {
        ...
        Future<WebAPI.Response.FriendApply> future = WebAPI.FriendApply(friend00811Menu.target_friend_ids.ToArray(), new System.Action<WebAPI.Response.UserError>(friend00811Menu.\u003CFriendApplyAsync\u003Eb__14_0));
  ...
  public IEnumerator SetTargetUserData()
  {
    ...
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    Future<UnityEngine.Sprite> sprF = EmblemUtility.LoadEmblemSprite(0);
    e = sprF.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
  ...
  public IEnumerator SetTargetUserData(WebAPI.Response.PlayerSearch result)
  {
    ...
    Future<UnityEngine.Sprite> sprF = EmblemUtility.LoadEmblemSprite(result.target_player_helper.current_emblem_id);
    e = sprF.Wait();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
