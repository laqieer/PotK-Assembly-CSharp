...
public abstract class Unit004TrainingPage : MonoBehaviour
{
  ...
  protected System.Action exceptionBackScene => this.mainMenu_.exceptionBackScene;

  public bool isLoadingResources { get; private set; }

  protected void setBackSceneFromResult(PlayerUnit result) => this.mainMenu_.gameObject.GetComponent<Unit004TrainingScene>().setBackSceneFromResult(this.page, result);
  ...
  private void playStartEffect(bool bForward, bool nonEffect)
  {
    ...
    if (!bForward && !this.gameObject.activeSelf)
    {
    ...
    else
    {
      if (bForward && !nonEffect)
        this.gameObject.GetComponent<UIRect>().alpha = 0.0f;
      NGTweenParts component = this.gameObject.GetComponent<NGTweenParts>();
      ...
      else if (bForward && this.gameObject.activeSelf)
  ...
  private void initialzeTopTweenAlpha()
  {
    ...
    UITweener[] componentsInChildren = this.gameObject.GetComponentsInChildren<UITweener>(true);
    int[] groups = new int[3]{ 11, 12, 13 };
    float a = 0.0f;
    foreach (UITweener uiTweener in ((IEnumerable<UITweener>) componentsInChildren).Where<UITweener>((Func<UITweener, bool>) (x => !x.enabled && x.style == UITweener.Style.Once && ((IEnumerable<int>) groups).Contains<int>(x.tweenGroup))))
      a = Mathf.Max(a, uiTweener.delay + uiTweener.duration);
    component.duration = a;
  }
  ...
  protected virtual void onEnabledEffectFinished() => this.gameObject.GetComponent<UIRect>().alpha = 1f;

  protected virtual void onDisabledEffectFinished() => this.gameObject.SetActive(false);
  ...
  protected void setCostZeny(long cost)
  {
    ...
    this.txtCost_.color = cost <= current.money ? Color.white : Color.red;
  }

  protected IEnumerator doLoadCommonPrefab()
  {
    Future<GameObject> unitIconPrefabF;
    if ((UnityEngine.Object) this.unitIconPrefab_ == (UnityEngine.Object) null)
    {
    ...
    if ((UnityEngine.Object) this.statusDetailPrefab_ == (UnityEngine.Object) null)
    {
  ...
  {
    UnitIcon ui = goParent.transform.GetComponentInChildren<UnitIcon>(true);
    if ((UnityEngine.Object) ui == (UnityEngine.Object) null)
    {
      while ((UnityEngine.Object) this.unitIconPrefab_ == (UnityEngine.Object) null)
    ...
    {
      ui.onClick = (System.Action<UnitIconBase>) (_ => {});
      ui.onLongPress = (System.Action<UnitIconBase>) (_ => {});
    }
    else if (before)
    {
      ui.onClick = (System.Action<UnitIconBase>) (_ =>
    ...
    {
      ui.Gray = false;
      ui.buttonBoxCollider.enabled = true;
      ui.onClick = (System.Action<UnitIconBase>) (_ =>
      {
        if (this.IsPushAndSet())
          return;
  ...
  protected virtual void onLongPressedBeforeUnitIcon(PlayerUnit unit) => Unit0042Scene.changeScene(true, unit, (PlayerUnit[]) null, true);
  ...
  private IEnumerator doPopupStatusDetail(
    PlayerUnit unit,
    Unit004TrainingPage.SpriteNo no)
  {
    ...
    UnitUnit[] materials)
  {
    ...
    for (int index = 0; index < materials.Length; ++index)
    {
      ...
      if (x.IsNormalUnit)
      {
        ...
        {
          if (!intList.Contains(playerUnit.id))
          {
            intList.Add(playerUnit.id);
            break;
          }
        }
      }
  ...
  protected List<int> getMaterialMaterialUnitIDs(
    PlayerMaterialUnit[] sources,
    UnitUnit[] materials)
  {
  ...
  protected class UnitSpriteManager
  {
    public int unit_id_;
    public int job_id_;
    public UnityEngine.Sprite sprite_;

    public IEnumerator doLoad(PlayerUnit unit)
    {
      UnitUnit unit1 = unit.unit;
      int id = unit.getJobData().ID;
      if (!((UnityEngine.Object) this.sprite_ != (UnityEngine.Object) null) || this.unit_id_ != unit1.ID || this.job_id_ != id)
      {
        this.sprite_ = (UnityEngine.Sprite) null;
        this.unit_id_ = unit1.ID;
        this.job_id_ = id;
        Future<UnityEngine.Sprite> ld = unit1.LoadSpriteLarge(id, 1f);
