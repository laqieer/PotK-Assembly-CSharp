...
public class Sea030_questMenu : BackButtonMenuBase
{
    ...
    else
    {
      if (sea030QuestMenu.cacheMapChangeButtons_ == null)
      {
        ...
        if (sea030QuestMenu.isEnabledGridMapChangeButtons_)
        {
          ...
          Vector3 localScale = sea030QuestMenu.gridMapChangeButtons_.transform.localScale;
          foreach (Sea030_questMenu.MapChangeButtonInfo info in (IEnumerable<Sea030_questMenu.MapChangeButtonInfo>) ((IEnumerable<Sea030_questMenu.MapChangeButtonInfo>) sea030QuestMenu.cacheMapChangeButtons_).OrderByDescending<Sea030_questMenu.MapChangeButtonInfo, int>((Func<Sea030_questMenu.MapChangeButtonInfo, int>) (x => x.XL)))
          {
            ...
            info.button.transform.parent = sea030QuestMenu.gridMapChangeButtons_.transform;
            info.button.transform.localScale = Vector3.one;
            info.button.GetComponent<UISprite>().autoResizeBoxCollider = false;
      ...
      if (sea030QuestMenu.isEnabledGridMapChangeButtons_)
      {
        foreach (Sea030_questMenu.MapChangeButtonInfo cacheMapChangeButton in sea030QuestMenu.cacheMapChangeButtons_)
          cacheMapChangeButton.button.GetComponent<UIButton>().isEnabled = true;
      }
      else
      {
      ...
      else
      {
        ...
        PlayerSeaQuestS[] playerSeaQuestSArray;
        if (sea030QuestMenu.dicQuests_.TryGetValue(key, out playerSeaQuestSArray) && (((IEnumerable<PlayerSeaQuestS>) playerSeaQuestSArray).Any<PlayerSeaQuestS>((Func<PlayerSeaQuestS, bool>) (x => x.is_clear)) && MasterData.QuestSeaS.TryGetValue(Persist.seaLastSortie.Data.clearedS, out sea030QuestMenu.focusS_)))
        {
          sea030QuestMenu.nextXL_ = sea030QuestMenu.focusS_.quest_xl_QuestSeaXL;
        }
        else
  ...
  private IEnumerator doWaitStartMapEffect()
  {
    ...
    sea030QuestMenu.invokeCurrentMap((System.Action<Sea030QuestMap>) (cm => cm.activateEffects(true)));
  }
  ...
  private IEnumerator doChangeMap()
  {
    ...
    if (!sea030QuestMenu.maps_.ContainsKey(sea030QuestMenu.nextXL_))
    {
      ...
      while (!sea030QuestMenu.dicMapPrefab_.TryGetValue(sea030QuestMenu.nextXL_, out self))
        yield return (object) null;
      Sea030QuestMap component = self.Clone(sea030QuestMenu.scrollView.transform).GetComponent<Sea030QuestMap>();
    ...
    if (sea030QuestMenu.maps_.TryGetValue(sea030QuestMenu.nextXL_, out m))
    {
      ...
      go.SetActive(true);
      sea030QuestMenu.scrollView.transform.localScale = Vector3.one;
      PlayerSeaQuestS[] StoryData;
      if (!sea030QuestMenu.dicQuests_.TryGetValue(sea030QuestMenu.nextXL_, out StoryData))
        StoryData = new PlayerSeaQuestS[0];
      int Lid = sea030QuestMenu.focusS_ != null ? sea030QuestMenu.focusS_.quest_l_QuestSeaL : 0;
      int Mid = sea030QuestMenu.focusS_ != null ? sea030QuestMenu.focusS_.quest_m_QuestSeaM : 0;
      yield return (object) m.Init(StoryData, sea030QuestMenu.forceFirstInitialize_, Lid, Mid);
      ...
      sea030QuestMenu.resetScrollViewPosition((UnityEngine.Object) m.lastButton != (UnityEngine.Object) null ? m.lastButton.transform.localPosition : Vector3.zero);
    ...
    {
      GameObject gameObject = sea030QuestMap.gameObject;
  ...
  private IEnumerator doWaitStartChangeButton()
  {
    ...
    if (sea030QuestMenu.isEnabledGridMapChangeButtons_)
      sea030QuestMenu.cacheMapChangeButtons_[index1].button.GetComponent<UIButton>().isEnabled = false;
  ...
  {
    GameObject gameObject = this.scrollView.gameObject;
  ...
  private void changeMap(int XL)
  {
    ...
    if (this.isEnabledGridMapChangeButtons_)
    {
      ...
      if (changeButtonInfo != null)
        changeButtonInfo.button.GetComponent<UIButton>().isEnabled = true;
      Array.Find<Sea030_questMenu.MapChangeButtonInfo>(this.cacheMapChangeButtons_, (Predicate<Sea030_questMenu.MapChangeButtonInfo>) (x => x.XL == XL)).button.GetComponent<UIButton>().isEnabled = false;
    }
    else
    {
  ...
  private IEnumerator doStartTransition()
  {
    ...
    return (UnityEngine.Object) this.animTransition != (UnityEngine.Object) null && info != null ? this.doTransition(info, Array.Find<Sea030_questMenu.TransitionDetail>(this.transitionDetails, (Predicate<Sea030_questMenu.TransitionDetail>) (x => x.from == this.currentXL_ && x.to == this.nextXL_))) : this.doNoneTransition();
  }
    ...
    Sea030_questMenu.TransitionDetail detail)
  {
    ...
    sea030QuestMenu.animTransition.enabled = false;
    yield return (object) sea030QuestMenu.doChangeMap();
    ...
    while ((double) sea030QuestMenu.animTransition.GetCurrentAnimatorStateInfo(0).normalizedTime < 1.0);
    sea030QuestMenu.animTransition.enabled = false;
    sea030QuestMenu.scrollView.transform.localScale = Vector3.one;
    sea030QuestMenu.scrollView.RestrictWithinBounds(true, true, true);
  ...
  private void playAnime(Animator anime, string statusName)
  {
    ...
    this.animTransition.enabled = true;
  }

  private void invokeCurrentMap(System.Action<Sea030QuestMap> act)
  {
    Sea030QuestMap sea030QuestMap;
  ...
  public void onEndScene()
  {
    ...
    this.invokeCurrentMap((System.Action<Sea030QuestMap>) (cm => cm.activateEffects(false)));
  }
