...
public class CommonCorpsHeader : CommonHeaderBase
{
  ...
  private void UpdateBikkuriIcon() => this.kisekiBikkuriIcon.SetActive(Singleton<NGGameDataManager>.GetInstance().receivableGift);
  ...
  private IEnumerator LoadCoinIcon()
  {
    ...
    if (MasterData.CommonTicket.TryGetValue(this.commonTicketId, out commonTicket))
    {
      ...
      this.corpsMedalIcon.gameObject.SetActive(true);
      future = (Future<UnityEngine.Sprite>) null;
    }
    else
      this.corpsMedalIcon.gameObject.SetActive(false);
  }
}
