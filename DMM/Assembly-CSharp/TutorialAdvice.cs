...
public class TutorialAdvice : MonoBehaviour
{
  ...
  private IEnumerable<string> getBackgrounds() => ((IEnumerable<TutorialAdvice.Advice>) this.currentAdvices).Select<TutorialAdvice.Advice, string>((Func<TutorialAdvice.Advice, string>) (x => x.Background)).Where<string>((Func<string, bool>) (x => !string.IsNullOrEmpty(x) && x != "empty")).Distinct<string>();
  ...
  private void setBackgroundPrefabs()
  {
    ...
    foreach (string background in this.getBackgrounds())
    {
      ...
      if (flag)
        Singleton<ResourceManager>.GetInstance().LoadOrNull<GameObject>(path).RunOn<GameObject>((MonoBehaviour) this, (System.Action<GameObject>) (prefab =>
        {
  ...
  private void showMessage()
  {
    ...
    if (currentAdvice.MessageBoxType >= 0)
    {
      for (int index = 0; index < this.messageBox.Length; ++index)
      {
        ...
        this.messageBox[index].gameObject.SetActive(flag);
        if (flag)
      ...
      ((IEnumerable<TutorialMessageBox>) this.messageBox).ForEach<TutorialMessageBox>((System.Action<TutorialMessageBox>) (x => x.gameObject.SetActive(false)));
    ...
    for (int index = 0; index < this.backgroundRoot.childCount; ++index)
    {
      ...
      bool flag = child.name == currentAdvice.Background;
      child.gameObject.SetActive(flag);
      if (flag && this.next_button_info != null && this.next_button_info.ContainsKey(child.name))
      {
        UIButton uiButton = this.next_button_info[child.name](child);
  ...
  private TutorialAdvice.Advice[] parseMessage(string message)
  {
    ...
    foreach (string str2 in str1.Split(separator, StringSplitOptions.RemoveEmptyEntries))
    {
      ...
      foreach (string input in str2.Split(chArray))
      {
        if (input.StartsWith("#"))
        {
          int num1 = this.find_int1(input, this.messageBoxPattern, (System.Action<int>) (num => messageBoxType = num)) || this.find_int1(input, this.characterPattern, (System.Action<int>) (num => characterIndex = num)) || (this.find_int1(input, this.yPattern, (System.Action<int>) (num => positionY = num)) || this.find_string1(input, this.backgroundPattern, (System.Action<string>) (s => background = s))) || this.find_string1(input, this.facePattern, (System.Action<string>) (s => face = s)) ? 1 : (this.find_string1(input, this.itemPattern, (System.Action<string>) (s => item = s)) ? 1 : 0);
          if (num1 != 0)
            this.find_int1(input, this.keyPattern, (System.Action<int>) (num => key = num));
          if (num1 == 0)
  ...
  private bool find_int1(string input, Regex regex, System.Action<int> callback) => this.find_string1(input, regex, (System.Action<string>) (s =>
  {
    int result = 0;
  ...
  private bool find_string1(string input, Regex regex, System.Action<string> callback)
  {
