...
public class CutSceneUnitModel : NGDuelUnitBase
{
    ...
    GameObject subCameraObj)
  {
    ...
    cutSceneUnitModel.mCharacterAnimator = cutSceneUnitModel.gameObject.GetComponent<Animator>();
    cutSceneUnitModel.mBipTransform = cutSceneUnitModel.transform.GetChildInFind("Bip");
    cutSceneUnitModel.mIsMonster = cutSceneUnitModel.mMyUnitData.unit.character.category == UnitCategory.enemy;
    cutSceneUnitModel.transform.localScale = new Vector3(cutSceneUnitModel.mMyUnitData.unit.duel_model_scale, cutSceneUnitModel.mMyUnitData.unit.duel_model_scale, cutSceneUnitModel.mMyUnitData.unit.duel_model_scale);
  ...
  {
    CutSceneUnitModel cutSceneUnitModel = this;
    Object.Destroy((Object) cutSceneUnitModel.mCharacterAnimator.gameObject.GetComponent<clipEffectPlayer>());
    cutSceneUnitModel.mCharacterAnimator.gameObject.AddComponent<CutSceneClipEffectPlayer>();
    Transform transform = cutSceneUnitModel.baseGameObject.transform;
    transform.position = Vector3.zero;
    transform.rotation = Quaternion.identity;
    if (cutSceneUnitModel.mSkillEffect.vehicle_link_off && (Object) cutSceneUnitModel.mVehicleObject != (Object) null)
    {
      cutSceneUnitModel.transform.parent = cutSceneUnitModel.mVehicleObject.transform.parent;
      cutSceneUnitModel.transform.localPosition = Vector3.zero;
      cutSceneUnitModel.transform.localRotation = Quaternion.identity;
    }
    ...
    if (!string.IsNullOrEmpty(cutSceneUnitModel.mSkillEffect.duel_vehicle_animator_name) && (Object) cutSceneUnitModel.mVehicleAnimator != (Object) null)
    {
  ...
  public ClipEventEffectData GetPreLoadClipDataList(string name) => !this.mPreLoadClipDataList.ContainsKey(name) ? (ClipEventEffectData) null : this.mPreLoadClipDataList[name];

  public GameObject GetPreLoadEffectPrefab(string name) => !this.mPreLoadEffectPrefabList.ContainsKey(name) ? (GameObject) null : this.mPreLoadEffectPrefabList[name];
  ...
  private IEnumerator PreLoadClipEventEffect()
  {
    ...
    foreach (string effectDataFileName in this.mSkillEffect.preloadClipEventEffectDataFileNameList)
    {
      string listName = effectDataFileName;
