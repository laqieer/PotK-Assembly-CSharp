...
public class HenshinContainer : MonoBehaviour
{
  ...
  private void Awake()
  {
    this.bReady_ = this.schedule_ != null && (uint) this.schedule_.Length > 0U;
    this.resetScheduler();
  }
  ...
  private void initHenshin(HenshinContainer.Schedule s)
  {
    ...
    {
      if ((UnityEngine.Object) this.dirAlways != (UnityEngine.Object) null)
        this.dirAlways.SetActive(true);
      this.dirBefore.SetActive(false);
      this.dirAfter.transform.localPosition = (Vector3) Vector2.zero;
  ...
  private void setUnitSetting(GameObject go, HenshinContainer.UnitSetting usetting)
  {
    ...
    {
      go.SetActive(true);
      go.transform.localPosition = (Vector3) usetting.position_;
    }
    else
