...
public class GuildBattleRecordScroll : MonoBehaviour
{
    ...
    GameObject memberScorePopup)
  {
    ...
    this.lblData.gameObject.SetActive(record.created_at.HasValue);
    if (record.created_at.HasValue)
    {
      ...
      hashtable.Add((object) "month", (object) dateTime.Month);
      dateTime = record.created_at.Value;
      hashtable.Add((object) "day", (object) dateTime.Day);
      string text = Consts.Format(battleRecordDate, (IDictionary) hashtable);
      lblData.SetTextLocalize(text);
    }
    Future<UnityEngine.Sprite> emblemF = EmblemUtility.LoadGuildEmblemSprite(record.target_guild_emblem_id.Value);
    ...
    for (int index = 0; index < this.spriteStarPlayer.Count; ++index)
      this.spriteStarPlayer[index].gameObject.SetActive(false);
    ...
    for (int index = 0; index < this.spriteStarOpponent.Count; ++index)
      this.spriteStarOpponent[index].gameObject.SetActive(false);
    for (int index = 0; index < str2.Length; ++index)
    {
      this.spriteStarOpponent[index].gameObject.SetActive(true);
    ...
    this.btnMemberScore.GetComponent<BoxCollider>().size = (Vector3) this.btnMemberScore.GetComponent<UISprite>().localSize;
  }
