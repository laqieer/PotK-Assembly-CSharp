...
public class PopupBuguSlotRelease : BackButtonPopupBase
{
  ...
  private IEnumerator Start()
  {
    ...
    UIPanel panel = popupBuguSlotRelease.GetComponent<UIPanel>();
    panel.alpha = 0.0f;
    panel.UpdateAnchors();
    ...
    {
      popupBuguSlotRelease.toutaTotalNumerator_.color = popupBuguSlotRelease.toutaTotalDenominator_.color;
      popupBuguSlotRelease.toutaDefaultNumerator_.color = popupBuguSlotRelease.toutaDefaultDenominator_.color;
    }
    else
    {
      popupBuguSlotRelease.toutaTotalNumerator_.color = Color.white;
      popupBuguSlotRelease.toutaDefaultNumerator_.color = Color.white;
    }
    if ((double) total < (double) extensionUnity.total_unity_value)
    {
      popupBuguSlotRelease.toutaTotalNumerator_.color = Color.red;
      popupBuguSlotRelease.releaseCheck_ = false;
    }
    if (popupBuguSlotRelease.playerUnit_.unity_value < extensionUnity.true_unity_value)
    {
      popupBuguSlotRelease.toutaDefaultNumerator_.color = Color.red;
      popupBuguSlotRelease.releaseCheck_ = false;
    }
    ...
    popupBuguSlotRelease.colliderMaterialDialog_.size = (Vector3) panel.GetViewSize();
    popupBuguSlotRelease.colliderMaterialDialog_.transform.position = Vector3.zero;
    popupBuguSlotRelease.btnRelease_.isEnabled = popupBuguSlotRelease.releaseCheck_;
    Singleton<PopupManager>.GetInstance().startOpenAnime(popupBuguSlotRelease.gameObject);
  }
    ...
    if (src != null)
    {
      ...
      if (MasterData.GearGear.TryGetValue(src.materialId, out gear))
      {
        ...
        if (haveCount < (long) src.quantity)
          info.txtRequired_.color = Color.red;
        ...
        icon.onClick = (System.Action<ItemIcon>) (x => popupBuguSlotRelease.showMaterialQuestInfo(gear));
        icon = (ItemIcon) null;
      }
    }
    else
    ...
    List<PopupBuguSlotRelease.GearReleaseItem> releaseItemList)
  {
    ...
    PlayerMaterialUnit[] playerMaterialUnitArray = new PlayerMaterialUnit[releaseItemList.Count];
    for (int index = 0; index < playerMaterialUnitArray.Length; ++index)
    {
      int id = releaseItemList[index].materialId;
      playerMaterialUnitArray[index] = Array.Find<PlayerMaterialUnit>(array, (Predicate<PlayerMaterialUnit>) (x => x._unit == id));
      if (playerMaterialUnitArray[index] == null)
      {
        playerMaterialUnitArray[index] = new PlayerMaterialUnit();
        playerMaterialUnitArray[index].id = index;
        playerMaterialUnitArray[index]._unit = id;
      }
    }
    return playerMaterialUnitArray;
  }
  ...
  private UITweener[] boxAnimations => this.boxAnimations_ ?? (this.boxAnimations_ = NGTween.findTweeners(this.boxMaterialDialog_, true));
