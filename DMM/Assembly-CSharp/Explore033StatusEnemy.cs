...
public class Explore033StatusEnemy : Battle0181CharacterStatus
{
  ...
  public void SetWeakPoint(WeakPoint weakPoint)
  {
    ...
    foreach (GearKindEnum gearKindEnum in weakPoint.Gearkind)
    {
      this.createWeakPointIcon(this.go_weaponTypeIcon, this.mWeakPointIconGridAnchor).GetComponent<GearKindIcon>().Init((int) gearKindEnum);
      ++num1;
    }
    int num2 = 0;
    foreach (UnitTypeEnum unitTypeEnum in weakPoint.PrincessType)
    {
      this.mWeakPointLblPrefab.Clone(this.mWeakPointIconListAnchor.transform).GetComponentInChildren<UILabel>().SetTextLocalize(this.getPrincessTypeString(unitTypeEnum));
      ++num2;
      ++num1;
    }
    UIWidget component = this.mWeakPointIconListAnchor.GetComponent<UIWidget>();
  ...
  {
    GameObject gameObject = prefab.Clone(anchor.transform);
