...
public class SeaHeaderPopupPlayerStatus : MonoBehaviour
{
  ...
  public void SetUnitCount(int now, int max) => this.princessNumberLabel.SetTextLocalize(Consts.Format(Consts.GetInstance().HEADER_POPUP_UNIT_COUNT, (IDictionary) new Hashtable()
  {
    {
      (object) nameof (now),
      (object) now
  ...
  public void SetItemCount(int now, int max) => this.itemNumberLabel.SetTextLocalize(Consts.Format(Consts.GetInstance().HEADER_POPUP_ITEM_COUNT, (IDictionary) new Hashtable()
  {
    {
      (object) nameof (now),
      (object) now
