...
public class GuildDefenseMemberListPopup : GuildMemberListPopup
{
  ...
  public IEnumerator Initialize(GameObject memberPrefab, GuildRegistration guildInfo)
  {
    ...
    UIWidget component = defenseMemberListPopup.GetComponent<UIWidget>();
  ...
  protected virtual IEnumerator InitMemberScroll(
  ...
  protected new virtual IEnumerator CreateScrollBase(GameObject prefab)
  {
    ...
    {
      Guild0282MemberScroll component = UnityEngine.Object.Instantiate<GameObject>(prefab).GetComponent<Guild0282MemberScroll>();
    ...
    for (int index = 0; index < Mathf.Min(popup.MaxValue, popup.allMemberBar.Count); ++index)
      popup.scroll.AddColumn1(popup.allMemberBar[index].gameObject, popup.barWidth, popup.barHeight);
  ...
  public void onClearButton()
  {
    ...
    this.allMemberBar.ForEach((System.Action<Guild0282MemberScroll>) (x => this.StartCoroutine(x.SetData())));
  }
