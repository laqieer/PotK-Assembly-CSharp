...
public class PopupSkillDetail : MonoBehaviour
{
  ...
  public void resetIcons(UnityEngine.Sprite icon, UnityEngine.Sprite genreL, UnityEngine.Sprite genreR)
  {
  ...
  private void setSprite(UI2DSprite uiSprite, UnityEngine.Sprite sprite)
  {
  ...
  private class LevelObject
  {
    ...
    public void setValue(int numer, int denom) => this.txtNumber.SetTextLocalize((denom > 0 || numer > 0 ? numer.ToString() : "-") + "/" + (denom <= 0 ? "-" : denom.ToString()));

    public void setValue(int? numer, string denom) => this.txtNumber.SetTextLocalize((numer.HasValue ? numer.Value.ToString() : "-") + "/" + denom);
  }
}
