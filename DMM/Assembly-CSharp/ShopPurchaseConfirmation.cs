...
public class ShopPurchaseConfirmation : BackButtonMenuBase
{
  ...
  public IEnumerator Init(ShopItemIconInfo info, string shopTime)
  {
    ...
    if ((UnityEngine.Object) purchaseConfirmation.Loupe != (UnityEngine.Object) null)
    ...
    if (info.playerShopArticle.icon_resource != "")
    {
      ...
      purchaseConfirmation.PackThum.gameObject.SetActive(true);
    ...
    {
      purchaseConfirmation.Decoration.gameObject.SetActive(true);
      yield return (object) ShopCommon.CreateShopIcon(purchaseConfirmation.Decoration, info.playerShopArticle.decoration_resource);
    }
    else
      purchaseConfirmation.Decoration.gameObject.SetActive(false);
    if (info.IsPack)
      purchaseConfirmation.PackMark.SetActive(true);
    if ((UnityEngine.Object) purchaseConfirmation.MarkGrid != (UnityEngine.Object) null)
      purchaseConfirmation.MarkGrid.Reposition();
    if ((UnityEngine.Object) purchaseConfirmation.HaveItemCount != (UnityEngine.Object) null)
    {
    ...
    if ((UnityEngine.Object) purchaseConfirmation.Grid != (UnityEngine.Object) null && (UnityEngine.Object) purchaseConfirmation.ScrollView != (UnityEngine.Object) null)
    {
      purchaseConfirmation.Grid.gameObject.SetActive(false);
      for (int i = 0; i < info.playerShopArticle.contents.Length; ++i)
        yield return (object) purchaseConfirmation.ItemPrefab.Clone(purchaseConfirmation.Grid.transform).GetComponent<ShopPurchaseConfirmationItem>().Init(info.playerShopArticle.contents[i], purchaseConfirmation.ScrollView);
      purchaseConfirmation.Grid.gameObject.SetActive(true);
      yield return (object) null;
      purchaseConfirmation.Grid.Reposition();
      purchaseConfirmation.ScrollView.ResetPosition();
    }
    if ((UnityEngine.Object) purchaseConfirmation.Slider != (UnityEngine.Object) null)
    {
      purchaseConfirmation.TxtSelectMin.text = "1";
      int b = info.LimitCount.HasValue ? Mathf.Min(info.LimitCount.Value, 999) : 999;
      if (info.PayType != CommonPayType.paid_coin && info.PayType != CommonPayType.coin)
        b = Mathf.Min((int) (ShopCommon.GetHaveCount(info.PayType) / (long) info.PayCount), b);
      int num = b <= 0 ? 1 : b;
      purchaseConfirmation.TxtSelectMax.text = num.ToString();
      purchaseConfirmation.selectMaxCount = num;
    ...
    if (info.LimitCount.HasValue)
    {
      ...
      {
        purchaseConfirmation.BuyButton.isEnabled = false;
        if ((UnityEngine.Object) purchaseConfirmation.Slider != (UnityEngine.Object) null)
        {
          ...
          purchaseConfirmation.MinButton.isEnabled = false;
          purchaseConfirmation.MinusButton.isEnabled = false;
          purchaseConfirmation.MaxButton.isEnabled = false;
          purchaseConfirmation.PlusButton.isEnabled = false;
        }
      }
    }
  ...
  public void SliderChange()
  {
    ...
    this.sliderCount = Mathf.RoundToInt(this.Slider.value * (float) (this.selectMaxCount - 1));
  ...
  private void ChangeCurrentValue()
  {
    ...
    else
    {
      ...
      this.Slider.value = (float) this.sliderCount / ((float) this.selectMaxCount - 1f);
    }
  ...
  private bool HaveMaxOver()
  {
    switch (this.info.CommonRewardType)
    {
      ...
      case MasterDataTable.CommonRewardType.quest_key:
        ...
        int num = playerQuestKey == null ? 0 : playerQuestKey.quantity;
        if (playerQuestKey.max_quantity < num + this.selectedCount)
        {
