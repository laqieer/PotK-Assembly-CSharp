...
public class Shop00742Bugu : MonoBehaviour
{
  ...
  public IEnumerator Init(GearGear target)
  {
    ...
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    loader.Result.Clone(this.weaponIcon.transform).GetComponent<GearKindIcon>().Init(target.kind, target.GetElement());
    ...
    if (!target.isReisou())
    {
      ...
      if (this.dirSpAttack.activeSelf)
      {
        ...
        this.WeaponSpAttack01.gameObject.GetComponent<UI2DSprite>().depth = 400;
        if (specialAttackTargets.Length == 2)
        {
          this.WeaponSpAttack02.InitKindId(specialAttackTargets[1]);
          this.WeaponSpAttack02.gameObject.GetComponent<UI2DSprite>().depth = 400;
        }
      }
    }
    else
  ...
  protected virtual IEnumerator BuguSpriteCreate(Future<UnityEngine.Sprite> spriteF)
  {
    ...
    this.SlcTarget.transform.localScale = new Vector3(0.5f, 0.5f);
  }
