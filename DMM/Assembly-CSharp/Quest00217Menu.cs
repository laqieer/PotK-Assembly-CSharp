...
public class Quest00217Menu : BackButtonMenuBase
{
  ...
  private T elementAt<T>(T[] a, int index) where T : class => a.Length <= index ? default (T) : a[index];
    ...
    int activeTabIndex)
  {
    ...
    foreach (Quest00217Menu.BannerClass bannerClass in bannerList)
    {
      Quest00217Menu.BannerClass banner = bannerClass;
      ...
      if (banner != null)
      {
        switch (banner.type)
        {
          case Quest00217Menu.BannerType.Extra:
            if (!(bool) (UnityEngine.Object) prefabs[(int) banner.type])
            {
      ...
      banner = (Quest00217Menu.BannerClass) null;
    }
  ...
  private bool checkHasDescriptionScoreCampaing(int Lid) => this.checkHasDescriptionScoreCampaing(this.getScoreCampaingInfo(Lid));
  ...
  private void createCorpsButton(CorpsPeriod info, GameObject prefab) => this.categories[2].AddRequest(info, prefab);
  ...
  private IEnumerator SetupCategory(int categoryId)
  {
    ...
    foreach (Quest00217TabPage quest00217TabPage in this.categories.Values)
      quest00217TabPage.SetPageActive(false, (UnityEngine.Object) page == (UnityEngine.Object) quest00217TabPage);
    ...
    Quest00217TabPage.RequestParam request)
  {
    ...
    switch (request.Param)
    {
      ...
      case EventInfo info:
        ...
        e = (IEnumerator) null;
        break;
      case SM.TowerPeriod info:
        e = this.coInitScrollTower(page, info, request.Prefab);
        while (e.MoveNext())
          yield return e.Current;
        e = (IEnumerator) null;
        break;
      case CorpsPeriod info:
        e = this.coInitScrollCorps(page, info, request.Prefab);
        while (e.MoveNext())
          yield return e.Current;
        e = (IEnumerator) null;
        break;
      default:
        if (!(bool) (UnityEngine.Object) request.Prefab)
          break;
