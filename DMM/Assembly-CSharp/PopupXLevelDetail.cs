...
public class PopupXLevelDetail : BackButtonPopupBase
{
  ...
  private System.Action<PlayerUnit> resultLevelUp_;
  private System.Action<PlayerUnit> resultReincarnation_;

  public static Future<GameObject> createLoader() => new ResourceObject("Prefabs/unit004_2/popup_Detail_Lv").Load<GameObject>();

  public static void show(
    GameObject prefab,
    bool bLimitMode,
    PlayerUnit playerUnit,
    System.Action<PlayerUnit> resultLevelUp,
    System.Action<PlayerUnit> resultReincarnation)
  {
  ...
  private IEnumerator Start()
  {
    ...
    popupXlevelDetail.gameObject.GetComponent<UIRect>().alpha = 0.0f;
    ...
    if (popupXlevelDetail.isLimitMode_ || !jobData.is_vertex_x)
    {
      ...
      flag2 = false;
    bool flag3 = false;
    popupXlevelDetail.btnLevelUp_.isEnabled = flag1;
    popupXlevelDetail.btnReincarnation_.isEnabled = flag3;
    ...
    HashSet<int> intSet = popupXlevelDetail.target_.changed_job_ids != null ? new HashSet<int>(((IEnumerable<int?>) popupXlevelDetail.target_.changed_job_ids).Where<int?>((Func<int?, bool>) (x => x.HasValue && (uint) x.Value > 0U)).Select<int?, int>((Func<int?, int>) (y => y.Value))) : new HashSet<int>();
    PlayerUnit target = popupXlevelDetail.target_;
    int[] numArray = ((object) target != null ? target.x_job_status?.levels : (int[]) null) ?? new int[unitJobArray.Length];
    ...
    ldIcon = (Future<GameObject>) null;
    Singleton<PopupManager>.GetInstance().startOpenAnime(popupXlevelDetail.gameObject);
  }
  ...
  private void setParamIcons(XParamIcon paramIcon, UI2DSprite[] targets, XJobInformation info)
  {
    ...
    {
      UnityEngine.Sprite sprite = paramKeys == null || paramKeys.Length <= index ? (UnityEngine.Sprite) null : paramIcon[paramKeys[index]];
      UI2DSprite target = targets[index];
      if ((UnityEngine.Object) sprite != (UnityEngine.Object) null)
      {
        target.sprite2D = sprite;
        target.width = uiSprite.width;
        target.height = uiSprite.height;
  ...
  public void onClickedLevelUp()
  {
    ...
    else
    {
      if (this.IsPushAndSet())
        return;
      this.StartCoroutine(this.doPopup(PopupXLevelUp.createLoader(), (System.Action<GameObject>) (prefab => PopupXLevelUp.show(prefab, this.target_, new System.Action<PlayerUnit>(this.onLevelUp), (System.Action) (() => this.IsPush = false)))));
    }
  }
  ...
  private IEnumerator doPopup(Future<GameObject> ldPrefab, System.Action<GameObject> open)
  {
    yield return (object) ldPrefab.Wait();
    open(ldPrefab.Result);
  }

  private void onLevelUp(PlayerUnit result) => Singleton<PopupManager>.GetInstance().monitorCoroutine(PopupXLevelDetail.doLevelUp(this.prefab_, this.target_, result, this.resultLevelUp_, this.resultReincarnation_));
    ...
    System.Action<PlayerUnit> resultReincarnation)
  {
    ...
    bool bWait = true;
    PopupXLevelUpResult.show(ldResult.Result, before, after, (System.Action) (() => bWait = false));
    while (bWait)
