...
public class Unit004JobAnimJobUp : BackButtonMenuBase
{
  ...
  private void OpenDialogAndQuitAnim() => Singleton<PopupManager>.GetInstance().monitorCoroutine(this.OpenJobDialogPanel());
  ...
  private void DisplayMasterBonus() => Singleton<PopupManager>.GetInstance().monitorCoroutine(this.SwitchMasterBonusPanel());

  private IEnumerator SwitchMasterBonusPanel()
  {
    this.dirJobAfter.gameObject.SetActive(false);
    if (!Singleton<NGGameDataManager>.GetInstance().IsSea)
      this.Vertex_Lv5_logo.SetActive(true);
    this.txt_top2.SetActive(true);
    this.txt_top.SetActive(false);
    Future<UnityEngine.Sprite> CharacterF = MasterData.UnitUnit[this._playerUnit._unit].LoadSpriteLarge(this._playerUnit.job_id, 1f);
    ...
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    this.dirJobAfter2.gameObject.SetActive(true);
  }
}
