...
public class PopupReisouDetails : BackButtonPopupBase
{
  ...
  protected System.Action removeCallback;
    ...
    PlayerItem customGearBase = null)
  {
    ...
    else
    {
      ...
      float num = (float) this.slcReisouGauge.width * ((float) playerItemTemp.gear_exp / (float) (playerItemTemp.gear_exp_next + playerItemTemp.gear_exp));
    ...
    if (limited)
    {
      ...
      if ((UnityEngine.Object) this.btnDrilling != (UnityEngine.Object) null)
      {
        this.btnDrilling.isEnabled = false;
        this.btnDrilling.GetComponent<BoxCollider>().enabled = false;
      }
      if ((UnityEngine.Object) this.btnDrillingDisabled != (UnityEngine.Object) null)
        this.btnDrillingDisabled.isEnabled = false;
      if ((UnityEngine.Object) this.btnFusion != (UnityEngine.Object) null)
        this.btnFusion.isEnabled = false;
    }
    else
    {
      if ((UnityEngine.Object) this.btnDrilling != (UnityEngine.Object) null)
      {
        ...
        if ((UnityEngine.Object) this.btnDrilling != (UnityEngine.Object) null)
        {
          this.btnDrilling.isEnabled = !playerItemTemp.isLevelMax() & isDrillingPossible;
          this.btnDrilling.GetComponent<BoxCollider>().enabled = !playerItemTemp.isLevelMax() & isDrillingPossible;
        }
        if ((UnityEngine.Object) this.btnDrillingDisabled != (UnityEngine.Object) null)
          this.btnDrillingDisabled.isEnabled = false;
      }
      if (removeCallback == null)
      {
        ...
        {
          if (isFusionPossible)
            this.btnFusion.isEnabled = item.playerItem.isReisouFusionPossible(SMManager.Get<PlayerItem[]>());
          else
            this.btnFusion.isEnabled = false;
        }
      }
      else
      {
        ...
        this.btnRemove.isEnabled = this.weaponItem != (PlayerItem) null & isRemovePossible;
        if ((UnityEngine.Object) this.btnFusion != (UnityEngine.Object) null)
        {
          if (isFusionPossible)
            this.btnFusion.isEnabled = item.playerItem.isReisouFusionPossible(SMManager.Get<PlayerItem[]>());
  ...
  protected IEnumerator setHolyParamSkillDetail(UIGrid _grid, PlayerItem playerItem)
  {
    ...
    if (paramTextQueue.Count > 0)
    {
      ...
      for (int i = 0; i < page; ++i)
      {
        ...
        yield return (object) this.prefabReisouSkillDetail02.Clone(_grid.transform).GetComponent<ReisouSkillDetail_02>().Init(this.scrollview, paramTextList);
      }
    }
  }
    ...
    if (paramTextQueue.Count > 0)
    {
      ...
      for (int i = 0; i < page; ++i)
      {
        ...
        for (int index = 0; index < 3 && paramTextQueue.Count > 0; ++index)
          paramTextList.Add(paramTextQueue.Dequeue());
        yield return (object) this.prefabReisouSkillDetail03.Clone(_grid.transform).GetComponent<ReisouSkillDetail_02>().Init(this.scrollview, paramTextList);
      }
    }
  }
  ...
  protected IEnumerator setSkillDetail(UIGrid _grid, GearGear gear, int gearLevel)
  {
    ...
    foreach (List<GearReisouSkill> rememberReisouSkill in gear.rememberReisouSkills)
    {
      foreach (GearReisouSkill gearReisouSkill in rememberReisouSkill)
      {
        ...
        {
          if (!(popupReisouDetails.equipGear == (PlayerItem) null) && PlayerItem.isReisouSkillAwakeWeaponGroup(popupReisouDetails.equipGear.entity_id, gearReisouSkill.awake_weapon_group))
            flag = true;
          else
            continue;
        }
    ...
    for (int i = 0; i < skillList.Count; ++i)
    {
      ...
      yield return (object) popupReisouDetails.prefabReisouSkillDetail01.Clone(_grid.transform).GetComponent<ReisouSkillDetail_01>().Init(popupReisouDetails.scrollview, skill, gearLevel, skillParamsArray, is_release, popupReisouDetails.equipGear, new System.Action<GearReisouSkill>(popupReisouDetails.OpenPopupReisouTargetWeapon), (BackButtonMenuBase) popupReisouDetails);
    }
  }
  ...
  public IEnumerator OpenReisouRemovePopupAsync()
  {
    PopupReisouDetails popupReisouDetails = this;
    IEnumerator e;
    if ((UnityEngine.Object) popupReisouDetails.reisouRemovePopupPrefab == (UnityEngine.Object) null)
    {
  ...
  public IEnumerator OpenPopupReisouTargetWeaponAsync(GearReisouSkill skill)
  {
    PopupReisouDetails popupReisouDetails = this;
    IEnumerator e;
    if ((UnityEngine.Object) popupReisouDetails.prefabPopupReisouTargetWeapon == (UnityEngine.Object) null)
    {
      ...
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      popupReisouDetails.prefabPopupReisouTargetWeapon = popupPrefabF.Result;
      popupPrefabF = (Future<GameObject>) null;
    }
    GameObject popup = popupReisouDetails.prefabPopupReisouTargetWeapon.Clone();
    PopupReisouTargetWeapon component = popup.GetComponent<PopupReisouTargetWeapon>();
    popup.SetActive(false);
    e = component.Init(skill, (BackButtonMenuBase) popupReisouDetails);
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    popup.SetActive(true);
