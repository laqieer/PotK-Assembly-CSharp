...
public class ExploreDataManager : Singleton<ExploreDataManager>
{
  ...
  public int GetWinRate(int win, int lose)
  {
    ...
    int num2 = 0;
    if (num1 > 0)
      num2 = Mathf.FloorToInt((float) win * 100f / (float) num1);
    return num2;
  }
  ...
  private BL.Unit ConvertPlayerUnitToBlUnit(PlayerUnit pUnit, bool isEnemy)
  {
    ...
    unit.unitId = pUnit.unit.ID;
    unit.playerUnit = pUnit;
    unit.lv = pUnit.total_level;
    unit.spawnTurn = pUnit.spawn_turn;
    unit.friend = false;
    unit.ougi = (BL.Skill) null;
    unit.skills = new BL.Skill[0];
    unit.magicBullets = source.ToArray<BL.MagicBullet>();
    unit.skillfull_shield = XorShift.Range(1, 5);
    unit.skillfull_weapon = XorShift.Range(1, 5);
    if (!isEnemy)
    {
      unit.weapon = new BL.Weapon(pUnit);
      unit.gearLeftHand = pUnit.isLeftHandWeapon;
      unit.gearDualWield = pUnit.isDualWieldWeapon;
    }
    ...
    if (skill1 != null)
      unit.duelSkills = new BL.Skill[1]{ skill1 };
    else
      unit.duelSkills = new BL.Skill[0];
    return unit;
  }
  ...
  public DuelEnvironment CreateDuelEnviroment() => new DuelEnvironment()
  {
    stage = new BL.Stage(505)
  };

  public void SetLastDuelCache(ExploreEnemy eEnemy, DuelResult result) => this.mLastDuelCache = new DuelCache(eEnemy, result.defenseDamage);

  public ExploreEnemy GetLastDuelEnemy() => this.mLastDuelCache.Enemy;

  public bool IsAliveLastDuelEnemy() => this.mLastDuelCache.Enemy != null && this.mLastDuelCache.Enemy.hp > this.mLastDuelCache.Damage;

  public DuelResult CreateRevengeDuelResult() => this.CreateDuelResult(this.mLastDuelCache.Enemy, this.mLastDuelCache.Damage);
  ...
  private BL.DuelTurn CreateEpDuelTurn() => new BL.DuelTurn()
  {
    isHit = true,
    isCritical = false,
    skillIds = new int[0],
  ...
  public DateTime ConvertMasterTimeToLocal(DateTime time) => this.MasterTimeZone == null ? time : TimeZoneInfo.ConvertTime(time, this.MasterTimeZone, TimeZoneInfo.Local);
  ...
  public IEnumerator ResumeExplore(DateTime calcStartTime)
  {
    ...
    if (exploreDataManager.LastSyncTime <= now)
    {
      ...
      // ISSUE: reference to a compiler-generated method
      yield return (object) Singleton<ExploreLotteryCore>.GetInstance().CalcTaskOnBackGround(span, errorCallback: new System.Action(exploreDataManager.\u003CResumeExplore\u003Eb__72_4));
  ...
  public IEnumerator SaveSuspendData(System.Action errorCallback, bool noErrorPopup = false)
  {
    SuspendData suspendData = this.CreateSuspendData();
    Future<WebAPI.Response.ExploreExploreUpdate> api = WebAPI.ExploreExploreUpdate(this.ExploreBox.GetRewardsId().ToArray(), suspendData.RandCnt, suspendData.DefeatEnemyCounts, suspendData.DefeatEnemyIds, this.FloorData.ID, suspendData.EnemyDamage, new int?(suspendData.EnemyId), this.LoseCount, this.Progress, suspendData.State, suspendData.TakeOver, suspendData.Rest, this.WinCount, (System.Action<WebAPI.Response.UserError>) (e =>
    {
      if (noErrorPopup)
        return;
  ...
  public IEnumerator GetBoxReward(System.Action errorCallback, System.Action isLimitCallback)
  {
    SuspendData suspendData = this.CreateSuspendData();
  ...
  public IEnumerator MoveFloor(int targetFloorId, System.Action errorCallback)
  {
    SuspendData suspendData = this.CreateSuspendData();
  ...
  public bool IsRankingPeriodFinished => this.RankingPeriod == null || this.ConvertMasterTimeToLocal(this.RankingPeriod.end_at.Value) <= ServerTime.NowAppTimeAddDelta();
  ...
  public int Progress => (int) ((Decimal) this.FloorElapsedTime / (Decimal) this.FloorRequiredTime * 1000000M);

  public long FloorRequiredTime => this.FloorData == null ? long.MaxValue : (long) (this.FloorData.required_seconds * 1000);
  ...
  public bool IsLatestLog(LogData logData) => this.mLogQueue.Count > 0 && this.mLogQueue.Last<LogData>() == logData;
  ...
  public PlayerUnit[] GetExploreUnits() => ((IEnumerable<EpPlayerUnit>) this.mBlUnits).Select<EpPlayerUnit, PlayerUnit>((Func<EpPlayerUnit, PlayerUnit>) (x => x?.BlUnit.playerUnit)).ToArray<PlayerUnit>();
  ...
  public PlayerDeck GetDeck(int deck_index)
  {
    ...
    playerDeck.player_unit_ids = new int?[5];
    int index = 0;
    foreach (int? playerUnitId in exploreDeckArray[deck_index].player_unit_ids)
    {
      ...
      {
        playerDeck.player_units[index] = playerUnit;
        playerDeck.player_unit_ids[index] = unit_id;
        ++index;
      }
    }
    return playerDeck;
  ...
  public IEnumerator RegistrationInitialDeck(System.Action errorCallback)
  {
    ExploreDataManager exploreDataManager = this;
    bool isError = false;
  ...
  public bool IsReopenPopup => (uint) this.mPopupState > 0U;

  public bool IsPopupStateDeckEdit => this.mPopupState == ExploreDataManager.PopupStateType.ePopup_DeckEdit;

  public bool IsPopupStateChallenge => this.mPopupState == ExploreDataManager.PopupStateType.ePopup_Challenge;

  public void InitReopenPopupState() => this.mPopupState = ExploreDataManager.PopupStateType.ePopup_None;

  public void SetReopenPopupStateDeckEdit() => this.mPopupState = ExploreDataManager.PopupStateType.ePopup_DeckEdit;

  public void SetReopenPopupStateChallenge() => this.mPopupState = ExploreDataManager.PopupStateType.ePopup_Challenge;

  public bool NeedShowBadge => this.IsFrontFloor && this.Progress >= 1000000 && Singleton<NGGameDataManager>.GetInstance().challenge_point > 0 || this.ExploreBox.IsRewardsMax;
  ...
  private struct EpBattleParam
  {
    ...
    {
      ExploreDataManager.EpBattleParam epBattleParam = new ExploreDataManager.EpBattleParam();
      int num1 = 0;
      int num2 = 0;
      int num3 = 0;
      foreach (EpPlayerUnit epUnit in team)
      {
        if (epUnit != null)
        {
          ...
          num3 += num5;
          num2 += num4;
          if ((double) epBattleParam.WeakPointCoefficient < (double) effectiveCoefficient)
            epBattleParam.WeakPointCoefficient = effectiveCoefficient;
        }
      }
      epBattleParam.Hp = num1;
      epBattleParam.Atk = num2;
      epBattleParam.Def = num3;
      return epBattleParam;
    }

    public static ExploreDataManager.EpBattleParam CreateEnemyParam(BL.Unit enemy) => new ExploreDataManager.EpBattleParam()
    {
