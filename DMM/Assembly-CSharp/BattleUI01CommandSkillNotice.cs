...
public class BattleUI01CommandSkillNotice : MonoBehaviour
{
  ...
  private IEnumerator View()
  {
    ...
    UITweener[] tweeners = NGTween.findTweenersGroup(commandSkillNotice.gameObject, 0, true);
  ...
  private void SetEffectDescription(BL.Skill skill, bool isPlayer)
  {
    ...
    {
      string text1 = !string.IsNullOrEmpty(skill.skill.viewShortDescription) ? skill.skill.viewShortDescription : skill.skill.shortDescription;
      if (string.IsNullOrEmpty(text1))
      {
        this.mOwnEffectLbl.SetTextLocalize(string.Empty);
        this.mOwnObj.SetActive(false);
      ...
      string text2 = !string.IsNullOrEmpty(skill.skill.viewShortDescriptionEnemy) ? skill.skill.viewShortDescriptionEnemy : skill.skill.shortDescriptionEnemy;
      if (string.IsNullOrEmpty(text2))
      {
        this.mOpponentEffectLbl.SetTextLocalize(string.Empty);
        this.mOpponentObj.SetActive(false);
