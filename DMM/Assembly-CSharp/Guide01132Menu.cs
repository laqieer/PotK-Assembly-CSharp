...
public class Guide01132Menu : BackButtonMenuBase
{
  ...
  public void IbtnGearChange(GuideEnemyGearChange GuideEnemyGearChange) => this.StartCoroutine(this.onStartSceneAsync(GuideEnemyGearChange.unitData, false));
  ...
  {
    int defeat = 0;
    ((IEnumerable<PlayerEnemyHistory>) historyList).ForEach<PlayerEnemyHistory>((System.Action<PlayerEnemyHistory>) (obj =>
    {
  ...
  public void SetNumber(UnitUnit unit) => this.TxtNumber.SetTextLocalize("NO." + (unit.history_group_number % 10000).ToString().PadLeft(4, '0'));

  public void SetUnitRarity(UnitUnit unit) => RarityIcon.SetRarity(unit, this.rarityStarsIcon, true, true);

  public IEnumerator SetUnitImg(UnitUnit unit)
  {
    Future<UnityEngine.Sprite> fSprite = unit.LoadFullSprite();
    if (fSprite != null)
    {
  ...
  public void SetGearChange(UnitUnit unit, UnitUnit[] commonUnitList, PlayerEnemyHistory[] history)
  {
    ...
    {
      if (sabun > i)
        this.enemyGearChangeList[i].gameObject.SetActive(false);
