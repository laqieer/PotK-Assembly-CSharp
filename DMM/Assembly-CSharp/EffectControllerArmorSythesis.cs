...
public class EffectControllerArmorSythesis : EffectController
{
  ...
  private void Awake()
  {
    UIRoot componentInParent = this.remainingManaSeedLabel.GetComponentInParent<UIRoot>();
  ...
  private IEnumerator initialize()
  {
    ...
    IEnumerator e;
    if ((Object) controllerArmorSythesis.armor_sythesis_prefab_ == (Object) null)
    {
    ...
    gameObject.name = controllerArmorSythesis.armor_sythesis_prefab_.name;
    gameObject.transform.localEulerAngles = Vector3.zero;
    controllerArmorSythesis.armor_sythesis_anim_ = gameObject.GetComponent<ArmorSythesisAnim>();
    if ((Object) controllerArmorSythesis.AnimationItemIconPrefab == (Object) null)
    {
    ...
    ItemInfo baseItem)
  {
    ...
    if (kindGearKind == 9)
    {
      ...
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      controllerArmorSythesis.common_rarity_anim_.image_.gameObject.SetActive(false);
      controllerArmorSythesis.common_rarity_anim_.image400_.gameObject.SetActive(true);
    }
    else
    {
      ...
      while (e.MoveNext())
        yield return e.Current;
      e = (IEnumerator) null;
      controllerArmorSythesis.common_rarity_anim_.image_.gameObject.SetActive(true);
      controllerArmorSythesis.common_rarity_anim_.image400_.gameObject.SetActive(false);
    }
    for (int i = 0; i < controllerArmorSythesis.armor_sythesis_anim_.thum_list.Count; ++i)
    {
      if (i < thum_list.Count)
      {
        controllerArmorSythesis.armor_sythesis_anim_.thum_list[i].gameObject.SetActive(true);
        ...
        controllerArmorSythesis.armor_sythesis_anim_.thum_list[i].gameObject.SetActive(false);
    }
    if (baseItem != null)
    {
      controllerArmorSythesis.armor_sythesis_anim_.target_thum.gameObject.SetActive(true);
    ...
    GameObject commonRariry)
  {
    if (!((Object) target == (Object) null))
    {
      ...
      {
        componentsInChild.transform.SetParent(gameObject.transform);
        componentsInChild.transform.localScale = Vector3.one;
        componentsInChild.transform.localPosition = Vector3.zero;
        componentsInChild.transform.localRotation = Quaternion.identity;
      }
