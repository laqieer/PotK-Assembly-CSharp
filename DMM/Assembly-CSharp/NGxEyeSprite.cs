...
public class NGxEyeSprite : MonoBehaviour
{
  ...
  private Dictionary<string, UnityEngine.Sprite> eyeSpriteDict = new Dictionary<string, UnityEngine.Sprite>();
  [SerializeField]
  private int unitID;
  ...
  public IEnumerator ChangeEye(string eyeName)
  {
    ...
    if (this.EyeName == "normal")
      this.updateEyeSprite(this.EyeName, (UnityEngine.Sprite) null);
  ...
  public void updateEyeSprite(string eyeName, UnityEngine.Sprite sprite)
  {
    this.eyeSpriteDict[eyeName] = sprite;
    this.EyeUI2DSprite.sprite2D = sprite;
    if ((UnityEngine.Object) sprite == (UnityEngine.Object) null)
      return;
    this.EyeUI2DSprite.SetDimensions(sprite.texture.width, sprite.texture.height);
    Vector3 localPosition = this.EyeUI2DSprite.gameObject.transform.localPosition;
    ...
    {
      UI2DSprite component = this.EyeUI2DSprite.transform.parent.gameObject.GetComponent<UI2DSprite>();
    ...
    this.EyeUI2DSprite.gameObject.transform.localScale = new Vector3(this.mask.scale, this.mask.scale, 1f);
    this.EyeUI2DSprite.gameObject.transform.localPosition = new Vector3(-num1, -num2, localPosition.z);
    this.EyeUI2DSprite.depth = this.EyeUI2DSprite.transform.parent.gameObject.GetComponent<UIWidget>().depth + 2;
  }

  private void Update()
  {
    if ((double) this.mask.spriteAlpha < 1.0)
      this.EyeUI2DSprite.enabled = false;
    else
      this.EyeUI2DSprite.enabled = true;
  }
}
