...
public class ShopGiftWeekItem : MonoBehaviour
{
    ...
    DateTime now)
  {
    ...
    if (!packInfo.player_pack.rest_receive_day.HasValue)
    {
      ...
      shopGiftWeekItem.receiveRestDayText.gameObject.SetActive(false);
    }
    else
    {
      shopGiftWeekItem.receiveRestDay.SetActive(true);
      shopGiftWeekItem.receiveRestDayText.gameObject.SetActive(true);
    ...
    if (!shopGiftWeekItem.isBuy)
    {
      Transform transform1 = shopGiftWeekItem.rewardIcons1Row.gameObject.transform;
      transform1.localPosition = new Vector3(transform1.localPosition.x, transform1.localPosition.y + 48f, 0.0f);
      Transform transform2 = shopGiftWeekItem.rewardIcons2Row.gameObject.transform;
      ...
      Transform transform4 = shopGiftWeekItem.receiveRestDayText.transform;
      transform4.localPosition = new Vector3(transform4.localPosition.x, transform4.localPosition.y + 48f, 0.0f);
      Transform transform5 = shopGiftWeekItem.priceOrReceiveButton.transform;
    ...
    if (packInfo.player_pack.is_purchasable || !packInfo.player_pack.is_received)
      shopGiftWeekItem.priceOrReceiveButton.isEnabled = true;
    else
      shopGiftWeekItem.priceOrReceiveButton.isEnabled = false;
    UISprite component1 = shopGiftWeekItem.GetComponent<UISprite>();
    if (packInfo.rewards.Length <= 5)
    {
      if (!shopGiftWeekItem.isBuy)
        component1.height = 224;
    }
    else if (shopGiftWeekItem.isBuy)
      component1.height = 384;
    else
      component1.height = 333;
    BoxCollider component2 = shopGiftWeekItem.GetComponent<BoxCollider>();
    ...
    if (packInfo.rewards.Length <= 5)
      shopGiftWeekItem.rewardIcons2Row.gameObject.SetActive(false);
  }
