...
public class PopupRecommendPartJobChange : PopupRecommendPart
{
  ...
  public override IEnumerator doInitialize(PlayerUnit playerUnit, UnitUnit target)
  {
    ...
    {
      recommendPartJobChange.gameObject.SetActive(false);
    }
    else
    {
  ...
  private void setEventClickedTab(UIButton btn, int index) => EventDelegate.Set(btn.onClick, (EventDelegate.Callback) (() => this.onClickedTab(index)));
  ...
  private IEnumerator doChangeTab(int next)
  {
    ...
    if (old >= 0 && old != next)
    {
      this.tabs_[old].button.isEnabled = true;
      this.tabs_[old].topSkills.SetActive(false);
    }
    nextTab.button.isEnabled = false;
    nextTab.topSkills.SetActive(true);
    this.current_ = next;
    this.isWait_ = false;
  }
    ...
    int index)
  {
    ...
    if ((UnityEngine.Object) recommendPartJobChange.genrePrefab_ == (UnityEngine.Object) null)
    {
    ...
    container.genre1.gameObject.SetActive(genre1.HasValue);
    if (genre1.HasValue)
    {
      ...
      component.iconSprite.depth = container.depth + 1;
    }
    container.genre2.gameObject.SetActive(genre2.HasValue);
    if (genre2.HasValue)
    {
