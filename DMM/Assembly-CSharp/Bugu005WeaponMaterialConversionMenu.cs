...
public class Bugu005WeaponMaterialConversionMenu : Bugu005SelectItemListMenuBase
{
  ...
  protected override long GetRevisionItemList() => this.mode == Bugu005WeaponMaterialConversionScene.Mode.Weapon ? SMManager.Revision<PlayerItem[]>() : 0L;
  ...
  protected override long GetRevisionMaterialList() => this.mode == Bugu005WeaponMaterialConversionScene.Mode.WeaponMaterial ? SMManager.Revision<PlayerMaterialGear[]>() : 0L;
  ...
  {
    Bugu005WeaponMaterialConversionMenu materialConversionMenu = this;
    if ((UnityEngine.Object) materialConversionMenu.selectPopupPrefab == (UnityEngine.Object) null)
    {
  ...
  protected IEnumerator ExecConvertAgreementPopupProc()
  {
    ...
    PopupCommonNoYes.Show(title, message, (System.Action) (() =>
    {
      selectYes = true;
      waitSelect = false;
    }), (System.Action) (() =>
  ...
  private IEnumerator ExecConvertToMaterialAPI()
  {
    ...
    if (!Singleton<CommonRoot>.GetInstance().isLoading)
    {
      ...
      Future<WebAPI.Response.ItemGearExchange> future = WebAPI.ItemGearExchange(materialConversionMenu.SelectItemList.Select<InventoryItem, int>((Func<InventoryItem, int>) (x => x.Item.itemID)).ToArray<int>(), (System.Action<WebAPI.Response.UserError>) (e =>
      {
  ...
  private IEnumerator ExecConvertToGearAPI()
  {
    ...
    if (!Singleton<CommonRoot>.GetInstance().isLoading)
    {
      Singleton<CommonRoot>.GetInstance().loadingMode = 1;
      Singleton<CommonRoot>.GetInstance().isLoading = true;
      Future<WebAPI.Response.ItemBodyExchange> future = WebAPI.ItemBodyExchange(materialConversionMenu.SelectItemList.Select<InventoryItem, int>((Func<InventoryItem, int>) (x => x.Item.masterID)).ToArray<int>(), materialConversionMenu.SelectItemList.Select<InventoryItem, int>((Func<InventoryItem, int>) (x => x.selectCount)).ToArray<int>(), (System.Action<WebAPI.Response.UserError>) (e =>
      {
        Singleton<CommonRoot>.GetInstance().loadingMode = 0;
        WebAPI.DefaultUserErrorCallback(e);
        MypageScene.ChangeSceneOnError();
  ...
  protected override void UpdateInventoryItemList()
  {
    ...
    if (list2 != null && list2.Count<InventoryItem>() > 0)
    {
      ...
      {
        InventoryItem invItem = inventoryItem;
        PlayerMaterialGear playerMaterialGear = ((IEnumerable<PlayerMaterialGear>) playerMaterialGearArray).FirstOrDefault<PlayerMaterialGear>((Func<PlayerMaterialGear, bool>) (x => x.id == invItem.Item.itemID));
