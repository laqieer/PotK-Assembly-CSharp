...
public class GuideRaritySelectDialog : BackButtonMenuBase
{
    ...
    int select_job_id)
  {
    ...
    foreach (UnitUnit commonUnit in this.commonUnitList)
    {
      ...
      if (source.Any<int>((Func<int, bool>) (x => (uint) x > 0U)))
      {
    ...
    if (this.isExJobHave)
    {
      ...
      this.SlcBase.SetDimensions((int) this.sclBaseDemision[0].x, (int) this.sclBaseDemision[0].y);
      this.setExJob(unit, new int?(select_job_id));
    }
    else
    {
      this.dirJobChange.SetActive(false);
      this.SlcBase.SetDimensions((int) this.sclBaseDemision[1].x, (int) this.sclBaseDemision[1].y);
    }
    ((IEnumerable<GuideUnitEvolution>) this.guideUnitEvolutions).ForEach<GuideUnitEvolution>((System.Action<GuideUnitEvolution>) (x => x.gameObject.SetActive(false)));
    this.setEvolution();
  }
  ...
  private void setEvolution()
  {
    ...
    for (int i = 0; i < this.dispEvolutionCnt; i++)
    {
      ...
      this.guideUnitEvolutions[i].gameObject.SetActive(true);
    }
  }
  ...
  private void setExJob(UnitUnit unit, int? select_job_id = null)
  {
    ...
    ((IEnumerable<GuideUnitJob>) this.guideUnitJobs).ForEach<GuideUnitJob>((System.Action<GuideUnitJob>) (x => x.gameObject.SetActive(false)));
    ...
    for (int index = 0; index < this.dispJobCnt; ++index)
    {
      ...
      this.guideUnitJobs[index].gameObject.SetActive(true);
      if (isEnable)
