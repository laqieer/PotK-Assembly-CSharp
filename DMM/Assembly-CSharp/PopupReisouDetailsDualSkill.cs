...
public class PopupReisouDetailsDualSkill : PopupReisouDetails
{
    ...
    PlayerItem customGearBase = null)
  {
    ...
    {
      detailsDualSkill.btnRemove.isEnabled = false;
    }
    else
    {
      ...
      detailsDualSkill.btnRemove.isEnabled = detailsDualSkill.weaponItem != (PlayerItem) null;
    }
    if (limited || !isDrillingPossible)
    {
      if ((UnityEngine.Object) detailsDualSkill.btnDrilling != (UnityEngine.Object) null)
        detailsDualSkill.btnDrilling.isEnabled = false;
      if ((UnityEngine.Object) detailsDualSkill.btnDrillingDisabled != (UnityEngine.Object) null)
        detailsDualSkill.btnDrillingDisabled.isEnabled = false;
    }
    else
    {
      PlayerMythologyGearStatus mythologyGearStatus = playerItemTemp.GetPlayerMythologyGearStatus();
      ...
      detailsDualSkill.btnDrilling.isEnabled = flag;
      detailsDualSkill.btnDrillingDisabled.isEnabled = false;
    }
  }
    ...
    bool isDispRank)
  {
    if (!isDispRank)
    {
      ...
      reisouUI.slcReisouGauge.gameObject.SetActive(false);
    }
    else
    {
      ...
      float num = (float) reisouUI.slcReisouGauge.width * ((float) playerItem.gear_exp / (float) (playerItem.gear_exp_next + playerItem.gear_exp));
      ...
      {
        reisouUI.slcReisouGauge.gameObject.SetActive(true);
        reisouUI.slcReisouGauge.width = (int) num;
      }
    }
  }
  ...
  public override void scrollResetPosition()
  {
    ...
    this.chaosReisou.scrollview.gameObject.SetActive(false);
    ...
    if (holyReisou != null)
    {
      ...
      if (paramTextQueue.Count > 0)
      {
        ...
        for (int i = 0; i < page; ++i)
        {
          ...
          yield return (object) detailsDualSkill.prefabReisouSkillDetail03.Clone(_grid.transform).GetComponent<ReisouSkillDetail_02>().Init(detailsDualSkill.chaosReisou.scrollview, paramTextList);
        }
      }
    }
  }
  ...
  public void onBtnHolyReisou()
  {
    if (this.IsPushAndSet())
      return;
    this.holyReisou.scrollview.gameObject.SetActive(true);
    this.chaosReisou.scrollview.gameObject.SetActive(false);
  ...
  public void onBtnChaosReisou()
  {
    if (this.IsPushAndSet())
      return;
    this.holyReisou.scrollview.gameObject.SetActive(false);
    this.chaosReisou.scrollview.gameObject.SetActive(true);
