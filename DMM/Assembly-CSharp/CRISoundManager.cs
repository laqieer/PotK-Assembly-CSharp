...
public class CRISoundManager : Singleton<CRISoundManager>
{
  ...
  {
    CriAtom.AddCueSheet("SECueSheet", "SECueSheet.acb", "");
    CriAtom.AddCueSheet("VO_1001", "VO_1001.acb", "");
    CriAtom.AddCueSheet("BgmCueSheet", "BgmCueSheet.acb", "");
  ...
  public CriAtomSource getNextBGMAudioSource() => this.bgmSource[(this.mainBGMChannel + 1) % this.maxBGM];
  ...
  {
    if (clip.name == this.bgmSource[this.mainBGMChannel].cueName)
      return;
    float delay = startTime;
  ...
  public int playSE(AudioClip clip, bool isLoop = false)
  {
    ...
    if ((UnityEngine.Object) criAtomSource != (UnityEngine.Object) null)
    {
      criAtomSource.cueName = clip.name;
      if (isLoop)
  ...
  {
    int index;
    for (index = 0; index < this.voiceSources.Length; ++index)
    {
      if (index != this.reserveVoiceAudioChannel && this.voiceSources[index].status != CriAtomSource.Status.Playing)
        goto label_5;
    }
    index = -1;
label_5:
    this.reserveVoiceAudioChannel = index;
    return index;
  }
  ...
  public int playVoice(AudioClip clip, int channel = -1)
  {
    ...
    if (channel != -1)
    {
      this.voiceSources[channel].cueName = clip.name;
  ...
  public CriAtomSource getVoiceAudioSource(int channel) => channel == -1 || channel >= this.voiceSources.Length ? (CriAtomSource) null : this.voiceSources[channel];
