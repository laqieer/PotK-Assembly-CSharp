...
public class PopupXLevelUp : BackButtonPopupBase
{
  ...
  private System.Action<PlayerUnit> result_;
  ...
  public static Future<GameObject> createLoader() => new ResourceObject("Prefabs/unit004_2/popup_XLvUp_MaterialSelect").Load<GameObject>();

  public static void show(
    GameObject prefab,
    PlayerUnit playerUnit,
    System.Action<PlayerUnit> result,
  ...
  private IEnumerator Start()
  {
    ...
    popup.gameObject.GetComponent<UIRect>().alpha = 0.0f;
    ...
    int num = xJobStatus != null ? xJobStatus.total_exp : 0;
    popupXlevelUp.baseExp_ = num;
    ...
    for (int n = 0; n < masters.Length; ++n)
    {
      ...
      if (key != null)
      {
        PopupXLevelUpExpEditor component = ldIcon1.Result.Clone(popup.grid_.transform).GetComponent<PopupXLevelUpExpEditor>();
    ...
    // ISSUE: reference to a compiler-generated method
    popup.grid_.onReposition = new UIGrid.OnReposition(popup.\u003CStart\u003Eb__33_4);
  ...
  public void onClickedExecute()
  {
    if (this.IsPushAndSet())
      return;
    this.StartCoroutine(this.doWaitLoad(PopupXLevelUpConfirm.createLoader(), (System.Action<GameObject>) (prefab =>
    {
      ...
      PopupXLevelUpConfirm.show(prefab, array, new System.Action<bool>(this.onEndConfirm));
    })));
  }

  private IEnumerator doWaitLoad(
    Future<GameObject> ldPrefab,
    System.Action<GameObject> onLoadedPrefab)
  {
  ...
  private IEnumerator doLevelUp()
  {
    ...
    Future<WebAPI.Response.UnitXLevelUp> webApi = WebAPI.UnitXLevelUp(popupXlevelUp.target_.id, material_player_material_unit_ids, material_player_material_unit_nums, (System.Action<WebAPI.Response.UserError>) (error =>
    {
  ...
  private void updateInfo(bool bImmediate = false)
  {
    ...
    this.btnClear_.isEnabled = num1 > 0;
    this.btnExecute_.isEnabled = num1 > 0;
    ...
    int num3 = this.baseLevelToExp_ - this.baseLevelFromExp_;
    int n = num3 > 0 ? Mathf.Min(num3, exp - this.baseLevelFromExp_) : 0;
    if (bImmediate)
      this.expGauge_.setValue(n, num3, false);
    else
      this.expGauge_.setValue(n, num3, duration: 0.5f);
  }
