...
public class UIGyroController : MonoBehaviour
{
  ...
  private void Update()
  {
    if (!this.isGyroAvailable || !this.isControllerStarted)
      return;
    float num = Time.deltaTime * 180f;
    this.currentRotationRateX = Mathf.Clamp(this.currentRotationRateX + Input.gyro.rotationRateUnbiased.x * num, this.bottomMax, this.topMax);
    this.currentRotationRateY = Mathf.Clamp(this.currentRotationRateY + Input.gyro.rotationRateUnbiased.y * num, this.leftMax, this.rightMax);
    this.tX = new float?();
    ...
    for (int index = 0; index < count1; ++index)
    {
      ...
      if ((UnityEngine.Object) controlledTransform.targetTransform != (UnityEngine.Object) null)
      {
        float x = controlledTransform.originalLocalPosition.x;
        if (this.tY.HasValue)
          x = Mathf.Lerp(controlledTransform.originalLocalPosition.x + controlledTransform.xMin, controlledTransform.originalLocalPosition.x + controlledTransform.xMax, this.tY.Value);
        float y = controlledTransform.originalLocalPosition.y;
        if (this.tX.HasValue)
          y = Mathf.Lerp(controlledTransform.originalLocalPosition.y + controlledTransform.yMax, controlledTransform.originalLocalPosition.y + controlledTransform.yMin, this.tX.Value);
        controlledTransform.targetTransform.localPosition = new Vector3(x, y, controlledTransform.targetTransform.localPosition.z);
      }
    }
  ...
  {
    if ((UnityEngine.Object) this.stopAnimator != (UnityEngine.Object) null)
      this.stopAnimator.enabled = false;
