...
public static class ResourceDownloader
{
  ...
  public static ResourceDownloader.ProgressInfo Progress
  {
    get
    {
      ...
      ResourceDownloader.ProgressInfo progressInfo;
      if (ResourceDownloader.isFirstDownLoadConfirm && ResourceDownloader.additions != null)
        progressInfo = new ResourceDownloader.ProgressInfo()
        {
    ...
    if (dlcVersionChanged)
    {
      ...
      if (confirmDLC)
      {
        ...
        foreach (string str in streamingAssetPaths.Where<string>((Func<string, bool>) (x => x.Contains("BgmCueSheet") || x.Contains("SECueSheet") || x.Contains("punk_music"))))
          addLstPath.Add(str);
        yield return (object) ResourceDownloader.appendDownload(resourceInfo, (System.Action<DLC>) (r => ResourceDownloader.additions = r), addLstPath);
        DLC[] dlcArray = new DLC[1]
        {
          ResourceDownloader.additions
        };
        bool toNext = false;
        yield return (object) ModalDownloadWindow.Show((IEnumerable<DLC>) dlcArray, (System.Action) (() => toNext = true), string.Empty);
        if (!toNext)
        {
          actionAbort();
          yield break;
...
label_109:
  ...
  public static IEnumerator DeleteContents(System.Action<int, int> progress)
  {
    ...
    foreach (string str in stringSet)
    {
      if (isAllMoved)
        ...
        paths.Add(System.IO.Path.Combine(DLC.ResourceDirectory, str));
    }
    int count = 0;
    int num = 0;
  ...
  public static IEnumerator DeleteContents(List<string> fileNames)
  {
    ...
    foreach (string fileName in fileNames)
    {
      try
      {
        File.Delete(!isMoved ? System.IO.Path.Combine(DLC.ResourceDirectory, fileName) : DLC.GetPath(fileName));
      }
      catch (Exception ex)
      {
        Debug.LogException(ex);
  ...
  public static IEnumerator MoveAllDLC()
  {
    ...
    ModalWindow window = ModalWindow.Show("ファイル更新中", "", (System.Action) (() => {}));
    ...
    foreach (string str in fileNames)
    {
      ...
      ResourceManager.CreateSaveDLCDir(System.IO.Path.GetDirectoryName(path));
      try
      {
        File.Move(System.IO.Path.Combine(DLC.ResourceDirectory, str), path);
      }
      catch (Exception ex)
      {
