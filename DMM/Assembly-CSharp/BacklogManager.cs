...
public class BacklogManager : MonoBehaviour
{
  ...
  private void Update()
  {
    if (!this.logContainer.activeSelf || !Input.GetKeyUp(KeyCode.Escape))
      return;
    this.endAction();
  }
  ...
  private IEnumerator Initialize()
  {
    ...
    if (this.colliders_ != null)
      this.colliders_.ForEach((System.Action<BoxCollider>) (c => c.enabled = true));
  ...
  public void StartBacklog(System.Action endAction)
  {
    ...
    if (this.colliders_ != null)
      this.colliders_.ForEach((System.Action<BoxCollider>) (c => c.enabled = false));
    int num1 = this.scroll_height / 2;
    int num2 = 0;
    foreach (string str in this.log_data)
    {
      int crlf = this.GetCRLF(str);
      GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(this.message_obj, new Vector3(0.0f, 0.0f, 0.0f), Quaternion.identity);
    ...
    {
      float y = this.scroll_view.GetComponent<UIPanel>().GetViewSize().y;
  ...
  public void End()
  {
    foreach (UnityEngine.Object @object in this.clone_data)
  ...
  private int GetCRLF(string s) => s.Split(new char[1]
  {
    '\n'
