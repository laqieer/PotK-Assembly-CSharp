...
public class Bugu005MaterialListMenu : Bugu005ItemListMenuBase
{
  ...
  public override Persist<Persist.ItemSortAndFilterInfo> GetPersist() => Persist.bugu005MaterialListSortAndFilter;

  protected override List<PlayerMaterialGear> GetMaterialList() => ((IEnumerable<PlayerMaterialGear>) SMManager.Get<PlayerMaterialGear[]>()).Where<PlayerMaterialGear>((Func<PlayerMaterialGear, bool>) (x => !x.isWeapon() && !x.isSupply() && !x.isWeaponMaterial())).ToList<PlayerMaterialGear>();
  ...
  public IEnumerator GoldSell()
  {
    ...
    if (materialListMenu.showPopupAutoSell)
    {
      ...
      yield return (object) materialListMenu.StartCoroutine(Popup0071SellIngotMenu.show(new System.Action<bool>(materialListMenu.onIngotSellResult), actionReload: new System.Action(materialListMenu.Reload)));
    }
  }
