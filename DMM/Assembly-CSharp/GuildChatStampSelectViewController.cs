...
public class GuildChatStampSelectViewController : MonoBehaviour
{
  ...
  {
    if ((double) this.stampScrollView.panel.finalClipRegion.y - (double) this.stampScrollView.panel.finalClipRegion.w > (double) this.headPlaceholder.transform.localPosition.y)
    {
      this.stampScrollView.transform.localPosition = new Vector3(this.stampScrollView.transform.localPosition.x, (float) -((double) this.headPlaceholder.transform.localPosition.y + (double) this.stampScrollView.panel.finalClipRegion.w - 1.0), this.stampScrollView.transform.localPosition.z);
      this.stampScrollView.panel.clipOffset = new Vector2(-this.stampScrollView.transform.localPosition.x, -this.stampScrollView.transform.localPosition.y);
    }
    else if ((double) this.tailPlaceholder.transform.localPosition.y > (double) this.stampScrollView.panel.finalClipRegion.y)
    {
      this.stampScrollView.transform.localPosition = new Vector3(this.stampScrollView.transform.localPosition.x, (float) -((double) this.tailPlaceholder.transform.localPosition.y + 1.0), this.stampScrollView.transform.localPosition.z);
      this.stampScrollView.panel.clipOffset = new Vector2(-this.stampScrollView.transform.localPosition.x, -this.stampScrollView.transform.localPosition.y);
    }
    ...
    else
    {
      ...
      {
        int num1 = Mathf.FloorToInt(this.stampScrollView.transform.localPosition.y / this.stampSelectItemHeight) * 4;
        int num2 = Mathf.FloorToInt((this.stampScrollView.transform.localPosition.y + this.stampScrollView.panel.finalClipRegion.w) / this.stampSelectItemHeight) * 4 + 4 - 1;
        if (num1 < 0)
          num1 = 0;
        ...
        for (int index = num1; index <= num2; ++index)
        {
          ...
          if (!this.bufferedStampSelectItems.Any<GuildChatStampSelectItemController>((Func<GuildChatStampSelectItemController, bool>) (x => x.stampID == currentItemID)))
          {
            ...
            if ((UnityEngine.Object) selectItemController != (UnityEngine.Object) null)
            {
              selectItemController.InitializeGuildChatStampItem(currentItemID);
              selectItemController.transform.localScale = Vector3.one;
              selectItemController.transform.localPosition = new Vector3((float) (index % 4) * this.stampSelectItemWidth, (float) -(index / 4) * this.stampSelectItemHeight, 0.0f);
              selectItemController.GetComponent<UIDragScrollView>().scrollView = this.stampScrollView;
              selectItemController.gameObject.SetActive(true);
            }
            else
      ...
      if (!(this.stampScrollView.currentMomentum != Vector3.zero) && !this.springPanel.isActiveAndEnabled)
        return;
  ...
  {
    while ((UnityEngine.Object) Singleton<CommonRoot>.GetInstance().guildChatManager.stampSelectItemPrefab == (UnityEngine.Object) null)
    {
    ...
    {
      GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(selectItemPrefab);
  ...
  {
    this.shouldUpdateStampList = true;
    this.springPanel.enabled = false;
    UIPanel component = this.stampScrollView.GetComponent<UIPanel>();
    this.stampScrollView.transform.localPosition = new Vector3(0.0f, -component.clipSoftness.y, 0.0f);
    component.clipOffset = new Vector2(-this.stampScrollView.transform.localPosition.x, -this.stampScrollView.transform.localPosition.y);
  ...
  public void SelectStampGroup(int stampGroupID)
  {
    ...
    for (int index = 0; index < this.stampGroupSelectItems.Count; ++index)
    {
      ...
      {
        num1 = index;
        uiWidget = this.stampGroupSelectItems[index].GetComponent<UIWidget>();
      }
    }
    if (!this.stampGroupScrollView.shouldMoveHorizontally)
      return;
    Vector3 vector3_1 = this.stampGroupScrollView.transform.worldToLocalMatrix.MultiplyPoint(uiWidget.worldCorners[0]);
    Vector3 vector3_2 = this.stampGroupScrollView.panel.localCorners[0] + new Vector3(this.stampGroupScrollView.panel.clipSoftness.x, 0.0f, 0.0f);
    Vector3 vector3_3 = this.stampGroupScrollView.transform.worldToLocalMatrix.MultiplyPoint(uiWidget.worldCorners[2]);
    Vector3 vector3_4 = this.stampGroupScrollView.panel.localCorners[2] - new Vector3(this.stampGroupScrollView.panel.clipSoftness.x, 0.0f, 0.0f);
    ...
    {
      this.stampGroupSpringPanel.target = new Vector3(this.stampGroupScrollView.transform.localPosition.x + ((float) uiWidget.width - num2), this.stampGroupScrollView.transform.localPosition.y, this.stampGroupScrollView.transform.localPosition.z);
      this.stampGroupSpringPanel.enabled = true;
    }
    else if (num1 < this.availableStampGroupIDs.Length - 1 && (double) num3 >= (double) -uiWidget.width && (double) num3 < (double) uiWidget.width)
    {
      this.stampGroupSpringPanel.target = new Vector3(this.stampGroupScrollView.transform.localPosition.x - ((float) uiWidget.width - num3), this.stampGroupScrollView.transform.localPosition.y, this.stampGroupScrollView.transform.localPosition.z);
      this.stampGroupSpringPanel.enabled = true;
    }
    else if (num1 == 0)
    {
      this.stampGroupSpringPanel.target = new Vector3(this.stampGroupScrollView.transform.localPosition.x + (vector3_2.x - vector3_1.x), this.stampGroupScrollView.transform.localPosition.y, this.stampGroupScrollView.transform.localPosition.z);
      this.stampGroupSpringPanel.enabled = true;
    }
    else
    {
      if (num1 != this.availableStampGroupIDs.Length - 1)
        return;
      this.stampGroupSpringPanel.target = new Vector3(this.stampGroupScrollView.transform.localPosition.x - (vector3_3.x - vector3_4.x), this.stampGroupScrollView.transform.localPosition.y, this.stampGroupScrollView.transform.localPosition.z);
      this.stampGroupSpringPanel.enabled = true;
    }
  }
  ...
  {
    while ((UnityEngine.Object) Singleton<CommonRoot>.GetInstance().guildChatManager.stampGroupSelectItemPrefab == (UnityEngine.Object) null)
    {
    ...
    for (int index = 0; index < this.stampGroupItemContainer.transform.childCount; ++index)
      UnityEngine.Object.Destroy((UnityEngine.Object) this.stampGroupItemContainer.transform.GetChild(index).gameObject);
    for (int index = 0; index < this.availableStampGroupIDs.Length; ++index)
    {
      ...
      gameObject.transform.SetParent(this.stampGroupItemContainer.transform);
