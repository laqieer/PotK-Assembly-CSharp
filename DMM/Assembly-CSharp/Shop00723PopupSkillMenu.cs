...
public class Shop00723PopupSkillMenu : BackButtonMenuBase
{
  ...
  private List<Shop00723PopupSkillMenu.UnitTree> unitTreeToList(
    ...
    SelectTicketChoices choice)
  {
    ...
    this.mainScroll_.enabled = false;
  ...
  private IEnumerator Start()
  {
    ...
    component1.SetAnchor(component2.transform.GetParentInFind(component2.targetParentName));
    component1.Update();
    shop00723PopupSkillMenu.container_.transform.localPosition = Vector3.zero;
    UIRect component3 = shop00723PopupSkillMenu.container_.GetComponent<UIRect>();
    ...
    {
      shop00723PopupSkillMenu.container_.gameObject
    };
    if (shop00723PopupSkillMenu.listUnit_.Count > 1)
    {
      ...
      Vector3 vector3 = new Vector3(0.0f, shop00723PopupSkillMenu.container_.transform.localPosition.y, 0.0f);
      for (int index = 1; index < shop00723PopupSkillMenu.listUnit_.Count; ++index)
      {
        GameObject gameObject = shop00723PopupSkillMenu.container_.gameObject.Clone(shop00723PopupSkillMenu.mainScroll_.transform);
    ...
    IEnumerator e = shop00723PopupSkillMenu.coInitUnit(shop00723PopupSkillMenu.container_.gameObject, shop00723PopupSkillMenu.unitTree_);
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    shop00723PopupSkillMenu.uiCentering_.CenterOn(shop00723PopupSkillMenu.container_.transform);
    shop00723PopupSkillMenu.pageTrigger_ = shop00723PopupSkillMenu.container_.gameObject;
    e = shop00723PopupSkillMenu.coInitializeDelay();
    while (e.MoveNext())
      yield return e.Current;
    e = (IEnumerator) null;
    Singleton<PopupManager>.GetInstance().startOpenAnime(shop00723PopupSkillMenu.gameObject);
  ...
  private IEnumerator coInitializeDelay()
  {
    ...
    this.mainScroll_.enabled = this.hscrollItems_.Length > 1;
    this.isDelayInitialize_ = false;
  }

  private IEnumerator coInitUnit(
    GameObject goList,
    Shop00723PopupSkillMenu.UnitTree utree)
  {
    ...
    int idxScroll = 0;
    UnitBattleSkillOrigin[] datas = utree.unit_.MakeLeaderSkillOrigins();
    GameObject go;
    if (datas.Length != 0)
    {
      ...
      num = idxScroll++;
      go.GetComponent<Shop00723PopupSkillLeader>().initialize(datas);
      if (datas.Length > 1)
      {
        List<UnitBattleSkillOrigin> list = ((IEnumerable<UnitBattleSkillOrigin>) datas).ToList<UnitBattleSkillOrigin>();
        while (true)
        {
          list.RemoveAt(0);
    ...
    foreach (UnitBattleSkillOrigin[] battleSkillOriginArray in (IEnumerable<UnitBattleSkillOrigin[]>) ((IEnumerable<UnitBattleSkillOrigin[]>) utree.unit_.MakeSkillOrigins()).OrderBy<UnitBattleSkillOrigin[], int>((Func<UnitBattleSkillOrigin[], int>) (sa =>
    {
    ...
    {
      if (((IEnumerable<UnitBattleSkillOrigin>) battleSkillOriginArray).Any<UnitBattleSkillOrigin>((Func<UnitBattleSkillOrigin, bool>) (sd => sd.skill_ != null)))
      {
        List<UnitBattleSkillOrigin> slst = ((IEnumerable<UnitBattleSkillOrigin>) battleSkillOriginArray).ToList<UnitBattleSkillOrigin>();
        do
        {
    ...
    if (this.ticketChoice == null)
      this.ViewTabs(false, container.gameObject);
    else
      this.EnableButtons();
  }
  ...
  protected override void Update()
  {
    ...
    if ((UnityEngine.Object) this.pageTrigger_ != (UnityEngine.Object) centeredObject)
    {
      this.playPageSE();
      this.pageTrigger_ = centeredObject;
    }
    if ((UnityEngine.Object) this.currentItem_ == (UnityEngine.Object) centeredObject)
      return;
    SpringPanel component = this.mainScroll_.GetComponent<SpringPanel>();
    ...
    {
      if ((UnityEngine.Object) this.hscrollItems_[index] == (UnityEngine.Object) centeredObject)
      {
        this.current_ = index;
        break;
      }
  ...
  private void updateArrow(int index) => this.setActiveArrow(index - 1 >= 0, index + 1 < this.listUnit_.Count);
  ...
  private void setActiveArrow(GameObject go, bool isactive) => go.GetComponent<UIButton>().isEnabled = isactive;
  ...
  {
    this.btnKing.gameObject.SetActive(false);
    this.btnLife.gameObject.SetActive(false);
    this.btnAttack.gameObject.SetActive(false);
    this.btnMagic.gameObject.SetActive(false);
    this.btnDefense.gameObject.SetActive(false);
    this.btnAgility.gameObject.SetActive(false);
    int[] unitTypes = this.ticketChoice.unit_types;
    if (((IEnumerable<int>) unitTypes).Contains<int>(1))
      this.btnKing.gameObject.SetActive(true);
    if (((IEnumerable<int>) unitTypes).Contains<int>(2))
      this.btnLife.gameObject.SetActive(true);
    ...
    if (((IEnumerable<int>) unitTypes).Contains<int>(4))
      this.btnMagic.gameObject.SetActive(true);
    if (((IEnumerable<int>) unitTypes).Contains<int>(5))
      this.btnDefense.gameObject.SetActive(true);
    if (((IEnumerable<int>) unitTypes).Contains<int>(6))
      this.btnAgility.gameObject.SetActive(true);
  ...
  {
    this.btnKing.SetColor(this.btnKing.disabledColor);
    this.btnLife.SetColor(this.btnLife.disabledColor);
    this.btnAttack.SetColor(this.btnAttack.disabledColor);
    this.btnMagic.SetColor(this.btnMagic.disabledColor);
    this.btnDefense.SetColor(this.btnDefense.disabledColor);
    this.btnAgility.SetColor(this.btnAgility.disabledColor);
    switch (btn)
    {
  ...
  private void SetVisibleSkill(Shop00723PopupSkillMenu.buttonPushed btn)
  {
    foreach (Shop00723PopupSkillContainer popupSkillContainer in this.allContainer)
    {
      ...
      foreach (GameObject gameObject in c.scroll_.Arr)
      {
        ...
        if ((UnityEngine.Object) gameObject.GetComponent<Shop00723PopupSkillLeader>() != (UnityEngine.Object) null)
        {
          gameObject.SetActive(true);
        }
        else
        {
          ...
          if ((UnityEngine.Object) popupSkill != (UnityEngine.Object) null)
          {
      ...
      this.ViewTabs(showIt, c.gameObject);
      c.scroll_.Resize();
  ...
  private void ViewTabs(bool showIt, GameObject container)
  {
    ...
    UIPanel component1 = container.transform.Find("ScrollView").GetComponent<UIPanel>();
    UIPanel component2 = container.transform.Find("dir_VScrollBar").GetComponent<UIPanel>();
    if (showIt)
    {
      component1.SetAnchor(container, 0, 0, 0, -62);
      component2.SetAnchor(container, 0, -8, 0, -66);
    }
    else
    {
      component1.SetAnchor(container, 0, 0, 0, 15);
      component2.SetAnchor(container, 0, -8, 0, 9);
    }
  }

  private void CheckScrollHorizontalMoving()
  {
    if ((double) this.mainScroll_.transform.position.x != (double) this._lastX)
    {
      this.StateAllButtons(false);
      this._lastX = this.mainScroll_.transform.position.x;
    }
    else
  ...
  {
    this.btnKing.GetComponent<BoxCollider>().enabled = active;
    this.btnLife.GetComponent<BoxCollider>().enabled = active;
    this.btnAttack.GetComponent<BoxCollider>().enabled = active;
    this.btnMagic.GetComponent<BoxCollider>().enabled = active;
    this.btnDefense.GetComponent<BoxCollider>().enabled = active;
    this.btnAgility.GetComponent<BoxCollider>().enabled = active;
  }
