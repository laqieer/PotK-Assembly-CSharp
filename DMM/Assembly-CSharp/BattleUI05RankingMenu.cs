...
public class BattleUI05RankingMenu : ResultMenuBase
{
  ...
  public override IEnumerator Init(BattleInfo info, BattleEnd result)
  {
    ...
    if ((UnityEngine.Object) this.txt_TotalPt != (UnityEngine.Object) null)
    {
    ...
    bool activeSpecialRate)
  {
    for (int index = 0; index < scores.Length; ++index)
      this.breakDownPrefab.Clone(this.grid.transform).GetComponent<BattleUI05BreakDown>().SetPoint(scores[index].description, scores[index].score);
    if (!activeSpecialRate)
      return;
    this.breakDownPrefab.Clone(this.grid.transform).GetComponent<BattleUI05BreakDown>().SetSpecialRate(specialRate);
  }
  ...
  {
    if (!((UnityEngine.Object) this.obj_EventTotalPt != (UnityEngine.Object) null))
      return;
  ...
  public void DispNowScoreUpdate()
  {
    if (this.campaign.battle_score_max_updated)
      this.CreateHighScore(this.obj_NowHighScore.transform, new System.Action(this.setEndDispNowScoreUpdate));
    else
  ...
  private void CreateHighScore(Transform parent, System.Action callback = null)
  {
