...
public static class AlgorithmUtil
{
  ...
  public static IEnumerable<IEnumerable<T>> Permutation<T>(IEnumerable<T> xs) where T : IComparable => AlgorithmUtil._permutation<T>((IEnumerable<T>) new List<T>(), xs);

  private static IEnumerable<IEnumerable<T>> _permutation<T>(
    IEnumerable<T> xs,
    IEnumerable<T> ys)
    where T : IComparable
  {
    ...
    else
    {
      foreach (T y1 in ys)
      {
        ...
        T[] objArray = new T[1]{ y };
        foreach (IEnumerable<T> objs in AlgorithmUtil._permutation<T>(first.Concat<T>((IEnumerable<T>) objArray), ys.Where<T>((Func<T, bool>) (y2 => (uint) y2.CompareTo((object) y) > 0U))))
          yield return objs;
      }
    }
  }
