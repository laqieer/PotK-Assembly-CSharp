...
public class Guild028OptionEditPopup : BackButtonMenuBase
{
  ...
  public void Initialize(Guild0283Menu menu)
  {
    ...
    ((IEnumerable<GuildAtmosphere>) MasterData.GuildAtmosphereList).ForEach<GuildAtmosphere>((System.Action<GuildAtmosphere>) (x => this.atmospherePulldown.items.Add(x.name)));
    ((IEnumerable<GuildApprovalPolicy>) MasterData.GuildApprovalPolicyList).ForEach<GuildApprovalPolicy>((System.Action<GuildApprovalPolicy>) (x => this.conditionPulldown.items.Add(x.name)));
    ((IEnumerable<GuildAutoApproval>) MasterData.GuildAutoApprovalList).ForEach<GuildAutoApproval>((System.Action<GuildAutoApproval>) (x => this.approvalPulldown.items.Add(x.name)));
    ((IEnumerable<GuildAutokick>) MasterData.GuildAutokickList).ForEach<GuildAutokick>((System.Action<GuildAutokick>) (x => this.autokickPulldown.items.Add(x.name)));
  ...
  public void onChangeGuildName() => this.decideButton.isEnabled = !this.guildNameInput.value.isEmptyOrWhitespace();
