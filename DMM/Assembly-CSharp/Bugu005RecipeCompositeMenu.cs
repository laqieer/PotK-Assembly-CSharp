...
public class Bugu005RecipeCompositeMenu : BackButtonMenuBase
{
  ...
  {
    this.slider.GetComponent<BoxCollider>().enabled = false;
    ...
    if (this.maxCount <= 1)
    {
      this.slider.numberOfSteps = this.maxCount + 1;
      this.slider.enabled = false;
      this.slider.GetComponent<BoxCollider>().enabled = false;
    ...
    {
      this.slider.GetComponent<BoxCollider>().enabled = true;
      this.slider.enabled = true;
      this.slider.numberOfSteps = this.maxCount;
    ...
    GameObject materialNumPrefab)
  {
    foreach (GameCore.ItemInfo gear1 in gears)
    {
      ...
      GameObject gameObject1 = iconPrefab.Clone(this.grid.transform);
      GameObject gameObject2 = materialNumPrefab.Clone(gameObject1.transform);
      int count = 0;
      this.inventoryItems.ForEach((System.Action<InventoryItem>) (x =>
      {
      ...
      gameObject2.transform.Find("txt_Num").GetComponentInChildren<UILabel>().SetTextLocalize(count.ToString());
      if (!this.rankMoreTwo)
  ...
  {
    this.sliderCount = Mathf.RoundToInt(this.slider.value * ((float) this.maxCount - 1f));
    this.UpdateInfo();
  }
  ...
  private void UpdateInfo()
  {
    ...
    this.yesButton.isEnabled = this.selectedCount > 0;
    this.changeMatButton.isEnabled = this.selectedCount == 1;
    this.txtCount.SetTextLocalize(this.selectedCount);
    this.txtRightCount.SetTextLocalize(this.selectedCount);
    this.slider.value = (float) this.sliderCount / ((float) this.maxCount - 1f);
    this.txtCost.SetTextLocalize((this.amount * this.selectedCount).ToString());
  }
  ...
  public void IbtnYes()
  {
    ...
    if (this.selectedCount == 1)
    {
      if (this.rankMoreTwoUnique)
        this.nowPopup = ModalWindow.ShowYesNo(Consts.GetInstance().Bugu005RecipeCompositeMenu_Title, Consts.GetInstance().Bugu005RecipeCompositeMenu_Message, (System.Action) (() => this.CreateOneGears()), (System.Action) (() =>
        {
          this.IsPush = false;
      ...
      this.nowPopup = ModalWindow.ShowYesNo(Consts.GetInstance().Bugu005RecipeCompositeMenu_Title, Consts.GetInstance().Bugu005RecipeCompositeMenu_Message, (System.Action) (() => this.CreateGearsWithMultipleMaterials()), (System.Action) (() =>
      {
        this.IsPush = false;
        this.ConfirmationNoBtn();
      })).gameObject;
  ...
  {
    this.menu.dirRecipe.gameObject.SetActive(true);
    foreach (Component component in UnityEngine.Object.FindObjectsOfType<Bugu0053DirRecipePopup>())
  ...
  private void ConfirmationNoBtn() => UnityEngine.Object.Destroy((UnityEngine.Object) this.nowPopup);
