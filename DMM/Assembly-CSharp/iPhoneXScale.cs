...
public class iPhoneXScale : MonoBehaviour
{
  ...
  private void AdjustIPhoneXScale()
  {
    ...
    if (!this.newLocalScale.HasValue)
      this.newLocalScale = !this.isRelativeScale ? new Vector3?(this.localScale) : new Vector3?(new Vector3(this.transform.localScale.x * this.localScale.x, this.transform.localScale.y * this.localScale.y, this.transform.localScale.z * this.localScale.z));
    this.transform.localScale = this.newLocalScale.Value;
  }
